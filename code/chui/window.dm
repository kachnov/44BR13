...
/chui/window/proc/generate(var/client/victim, var/body)
	var/list/params = list( "js" = list(), "css" = list(), "title" = name, data = list( "ref" = "\ref[src]", "flags" = flags ) )//todo, better this.
...
/chui/window/proc/Subscribe( var/client/who )
	CDBG1( "[who] subscribed to [name]" )
	subscribers += who
	//theme.streamToClient( who )
	who << browse( generate(who, GetBody()), "window=\ref[src];titlebar=0;can_close=0;can_resize=0;can_scroll=0;border=0" )
...
/chui/window/proc/IsSubscribed( var/client/whom )
...
/chui/window/proc/bbrowse( client/who, var/body, var/options )
...
/chui/window/proc/CallJSFunction( var/fname, var/list/arguments, var/client/exclude )
...
/chui/window/proc/OnTopic( href, href_list[] )
	return FALSE

//Called when a theme button is clicked.
/chui/window/proc/OnClick( var/id )
	return FALSE
	
//Called when Javascript sends a request; return with data to be given back to JS.
/chui/window/proc/OnRequest( var/method, var/list/data )
...
/mob/proc/Browse( var/html, var/opts )
	if ( client )
		chui.staticinst.bbrowse( client, html, opts )
