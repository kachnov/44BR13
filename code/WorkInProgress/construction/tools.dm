...
/obj/ai_frame
	...
	New()
		..()
		overlays += image('icons/mob/ai.dmi', "topopen")
		overlays += image('icons/mob/ai.dmi', "batterymode")
...
/obj/machinery/turretid/computer
	...
	attack_hand(var/mob/user as mob)
		...
		user.machine = src
		var/t = "<TT><strong>Turret Control Panel</strong><BR><strong>Controlled turrets:</strong> [turrets.len] (<A href='?src=\ref[src];rescan=1'>Rescan</a>)<HR>"
...
/obj/item/room_marker
	...
	mats = 6
	var/using = 0
	var/progress/designated = null

	attack_self(var/mob/user)
		if (!(ticker && ticker.mode && istype(ticker.mode, /game_mode/construction)))
		...
		for (var/progress/P in C.milestones)
...
/obj/item/material_shaper
	...
	var/mode = 0
	var/material/metal = null
	var/metal_count = 0
	var/material/glass = null
	var/glass_count = 0
		...
		if (T.density)
			...
			for (var/atom/movable/O in T)
				if ((istype(O, /obj) && O.density) || istype(O, /mob/living))
					...
					filling = 0
					return
		var/material/metal = origin.metal
		var/material/glass = origin.glass
...
/obj/plan_marker/wall
	...
	proc/check()
		...
		if (T.type == /turf/simulated/wall)
			...
			T.dir = dir
			T:allows_vehicles = allows_vehicles
			T.opacity = turf_op
			loc = null
			qdel(src)
