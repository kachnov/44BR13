/obj/machinery/fogmachine
	...
	process()
		if (reagents.has_reagent("radium"))
			...
			visible_message("<span style=\"color:red\">The <strong>[src]</strong> makes an odd sound, and releases a puff of green steam.</span>")
		...
		if (on == 1)
			if (reagents.reagent_list.len < 1 || reagents.total_volume < 1)
				...
				visible_message("<span style=\"color:red\">The <strong>[src]</strong> splutters to a halt.</span>")
				playsound(src, 'sound/machines/ding.ogg', 50, 1)
			else
				spawn (50)
					var/chemical_reaction/smoke/S = new
		...
		if (istype(C, /obj/item/reagent_containers/glass))
			if (C.reagents.reagent_list.len < 1)
				...
				for (var/current_id in C.reagents.reagent_list)
					var/reagent/current_reagent = C.reagents.reagent_list[current_id]
...
/obj/machinery/bathtub
	...
	on_reagent_change()
		...
		if (myuser)
			overlays += myuser
			if (reagents.total_volume)
				overlays += fluid_icon_feet

		overlays += bath_edge

	attack_hand(mob/user as mob)
		if (myuser)
			boutput(user, "<span style=\"color:red\">You pull [myuser] out of the bath!</span>")
			eject_user()
		else
	...
	process()
		if (myuser)
			if (myuser.loc != src)
				...
				return
			if (reagents.total_volume > 5)
				reagents.reaction(myuser, TOUCH)
				reagents.trans_to(myuser,5,1)
			else
				src.reagents.clear_reagents()
				on_reagent_change()
	...
	MouseDrop_T(mob/living/carbon/human/target, mob/user)
		if (myuser || !istype(target) || target.buckled || LinkBlocked(target.loc,loc) || get_dist(user, src) > 1 || get_dist(user, target) > 1 || user.paralysis || user.stunned || user.weakened || user.stat || istype(user, /mob/living/silicon/ai))
			return
...
/obj/item/clothing/head/apprentice
	proc/fantasia()
		if (ticker && ticker.mode && istype(ticker.mode, /game_mode/wizard))
			for (var/obj/item/mop/M in orange(5,src))
				visible_message("<span style=\"color:red\">[src] begins to twitch and move!</span>")
				var/moveto = locate(M.x + rand(-1,1),M.y + rand(-1, 1),z)
				//make the mops move
				...
				spawn (50)
					visible_message("<span style=\"color:blue\">Thankfully, [src] settles down.</span>")
		else
			for (var/obj/item/mop/M in orange(5,src))
				visible_message("<span style=\"color:red\">[src] begins to twitch and mov- oh. No. No it doesn't.</span>")
