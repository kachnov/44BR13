	...
	proc/melt_away(atom/movable/O)
		if (istype(O, /mob))
			if (istype(O, /mob/living))
				...
				visible_message("<span style=\"color:red\"><strong>[M]</strong> falls into the [src] and melts away!</span>")
				M.firegib() // thanks ISN!
		else
			visible_message("<span style=\"color:red\"><strong>[O]</strong> falls into the [src] and melts away!</span>")
			qdel(O)
...
/obj/shifting_wall/sneaky/cave
	...
	update()
		...
		if (prob(25)) // don't let all of them spam the noise at once
			spawn (rand(1,10))
				playsound(loc, 'sound/effects/rockscrape.ogg', 40, 1)
...
/obj/item/whip
	...
	afterattack(atom/target as mob|obj|turf, mob/user as mob)
		...
		var/list/affected = DrawLine(loc, target_r, /obj/line_obj/whip ,'icons/obj/projectiles.dmi',"WholeWhip",1,1,"HalfStartWhip","HalfEndWhip",OBJ_LAYER,1)
...
/obj/boulder_trap_boulder
	...
	proc/go()
		while (!disposed)
			...
			if (prob(30))
				playsound(loc, 'sound/effects/rockscrape.ogg', 60, 1) // having some noise might be rad

			if (!next || next.density)
				playsound(loc, 'sound/effects/Explosion2.ogg', 40, 1)
				new/obj/item/raw_material/rock(loc)
				new/obj/item/raw_material/rock(loc)
				new/obj/item/raw_material/rock(loc)
				new/obj/item/raw_material/rock(loc)
				spawn (0)
...
/obj/item/runetablet
	...
	attack_self()
		...
		var/dat = ""
		dat += "<strong>There's several runes inscribed here ...</strong><BR><BR>"
	...
	Topic(href, href_list)
		...
		if (href_list["north"])
			...
			movedir = NORTH
			playsound(loc, 'sound/machines/ArtifactEld1.ogg', 30, 1)
			playsound(loc, 'sound/effects/rockscrape.ogg', 40, 1)
...
/obj/item/clothing/suit/armor/ancient/equipped(var/mob/user, var/slot)
	...
	playsound(loc, 'sound/machines/ArtifactEld2.ogg', 30, 1)
	...
	spawn (50)
		...
		bleed(user, 5, 5)
		spawn (90)
			user.visible_message("<span style=\"color:red\"><strong>[src] violently contracts around [user]!</strong></span>")
			...
			bleed(user, 5, 1)
			spawn (50)
				user.visible_message("<span style=\"color:red\"><strong>[src] digs into [user]!</strong></span>")
				playsound(user.loc, 'sound/effects/bloody_stab.ogg', 50, 1, -1)
				random_brute_damage(user, 15)
				user.emote("scream")
				take_bleeding_damage(user, null, 0, DAMAGE_STAB, 0)
				...
				spawn (50)
					...
					H.visible_message("<span style=\"color:red\"><strong>[src] absorbs some of [user]'s skin!</strong></span>")
