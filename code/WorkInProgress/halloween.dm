...
/obj/item/unkill_shield
	...
	pickup(mob/user)
		...
		if (istype(H))
			boutput(user, "<em><strong><font face = Tempus Sans ITC>EI NATH</font></strong></em>")
...
/obj/submachine/mind_switcher
	...
	attack_hand(mob/user as mob)
		...
		if (user.a_intent == "harm")
			user.visible_message("<span class='combat'><strong>[user]</strong> punches the [src]!</span>","You punch the [src].  Your hand hurts.")
			...
			visible_message("<strong>[user]</strong> thumps the [src]!  Ayy!")
	...
	proc/mindswap()
		visible_message("<span style=\"color:red\">The [src] activates!</span>")
		playsound(loc,"sound/effects/ghost2.ogg", 100, 1)
		...
		if (!to_transfer.len || to_transfer.len == 1)
			visible_message("The [src] buzzes.")
			last_switch = 0
			if (teleport_next_switch)
				teleport_next_switch = 0
				telehop()
			return

		for (var/mob/living/M in to_transfer)
			if (!transfer_targets.len) //Welp, sucks for you dudes!
				visible_message("The [src] whirrs.  A small cry comes from within.")
				last_switch = max(0,last_switch - 60) //Reduce the wait a bit.
	...
	proc/telehop()
		...
		if (T)
			visible_message("<span style=\"color:red\">[src] disappears!</span>")
			playsound(loc,"sound/effects/singsuck.ogg", 100, 1)
			set_loc(T)
		return

	disposing()
		for (var/mob/M in to_transfer)
			M.gib(1)
		to_transfer = null
		..()
...
/obj/item/disk/data/fixed_disk/hjam_rdrive
	...
	New()
		...
		newfolder.add_file( new /computer/file/text/hjam_passlog(src))
		...
		root.add_file( newfolder )
		newfolder.add_file( new /computer/file/text/hjam_rlog_1(src))
		newfolder.add_file( new /computer/file/text/hjam_rlog_2(src))
		newfolder.add_file( new /computer/file/text/hjam_rlog_3(src))
...
/obj/item/disk/data/fixed_disk/outpost_rdrive
	...
	New()
		...
		root.add_file( new /computer/file/text/outpost_rlog_1(src))
		root.add_file( new /computer/file/text/outpost_rlog_2(src))
		//root.add_file( new /computer/file/text/hjam_rlog_3(src))
...
/obj/item/camera_test/haunted
	...
	afterattack(atom/target as mob|obj|turf|area, mob/user as mob, flag)
		...
		pictures_left--
		if (pictures_left <= 0)
			pictures_left = initial(pictures_left)
		desc = "A one use - polaroid camera. [pictures_left] photos left."
...
/obj/stocking
	...
	attack_hand(mob/user as mob)
		...
		if (booby_trapped)
			...
			if (dangerous)
				user.visible_message("<span class='combat'><strong>[user.name]</strong> takes [gift] out of [src]!</span>", "<span class='combat'>You take [gift] out of [src]!<br>This looks dangerous...</span>")
			else
				user.visible_message("<span style=\"color:blue\"><strong>[user.name]</strong> takes [gift] out of [src]!</span>", "<span style=\"color:blue\">You take [gift] out of [src]!</span>")
		return
...
/obj/storage/closet/haunted
	...
	HasEntered(atom/movable/A as mob|obj, atom/OldLoc)
		...
		if (M.throw_count || istype(OldLoc, /turf/space) || (M.m_intent != "walk"))
			var/flingdir = turn(get_dir(loc, OldLoc), 180)
			throw_at(get_edge_target_turf(src, flingdir), throw_strength, 1)
		return
...
/obj/machinery/bot/guardbot/bad
	...
	setup_default_startup_task = /computer/file/guardbot_task/security/crazy
		...
		switch(scramblemode)
			...
			if (7)
				for (var/mob/O in hearers(src, null))
					O.show_message("<span class='combat'><strong>[src]'s speaker crackles oddly!</strong></span>", 2)
				return
...
/obj/decal/glow
	...
	New()
		...
		light.set_brightness(brightness / 5)
		light.enable()
...
/obj/item/disk/data/memcard/h7
	...
	New()
		...
		newfolder.add_file( new /computer/file/mainframe_program/h7init(src) )
		...
		subfolder.add_file( new /computer/file/mainframe_program/driver/secdetector(src) )
		subfolder.add_file( new /computer/file/mainframe_program/driver/apc(src) )
		subfolder.add_file( new /computer/file/mainframe_program/driver/hept_emitter(src) )
		...
		newfolder.add_file( new /computer/file/mainframe_program/hept_interface(src) )
		...
		newfolder.add_file( new /computer/file/guardbot_task/bodyguard(src) )
		newfolder.add_file( new /computer/file/guardbot_task/security(src) )
...
/obj/item/disk/data/tape/h7_logs
	...
	New()
		..()
		root.add_file( new /computer/file/record/h7_analysislog(src) )
		root.add_file( new /computer/file/record/h7_memo1(src) )
		root.add_file( new /computer/file/record/h7_memo2(src) )
		root.add_file( new /computer/file/record/h7_memo3(src) )

//
/computer/file/record/h7_analysislog
	name = "analysis_log"
...
<ol>
<li>Insert 1-5 <strong>Precut Telecrystal Modules</strong> into inactive HEPT emitter.</li>
...
/obj/item/clothing/head/void_crown
	...
	proc/abandonHost()
		...
		for (var/mob/N in viewers(host, null))
			...
			if (N.client)
				shake_camera(N, 6, 4)
				N.show_message("<span class='combat'><strong>A blinding light envelops [host]!</strong></span>")

		playsound(loc, "sound/weapons/flashbang.ogg", 50, 1)

		set_loc(get_turf(host))
		processing_items.Remove(src)
		processing = 0
		cant_self_remove = 1
		cant_other_remove = 1
		...
		if (istype(W,/obj/item/paint_can))
			if (repair_stage == 4)
				...
				if (!paint_needed)
					...
					repair_stage = 5
					name = "Very-Nearly-Functional Paint Dispenser"
			...
			switch(repair_stage)
				if (0)
					if (istype(W, /obj/item/screwdriver))
						...
						desc = "Would dispense paint, if it were not broken. The maintenance panel has been unscrewed."
					else
				...
				if (1)
					if (istype(W, /obj/item/crowbar))
						...
						var/obj/item/tile/steel/panel = new /obj/item/tile/steel(loc)
						panel.name = "maintenance panel"
						panel.desc = "A panel that is clearly from a paint dispenser. Obviously."

						desc = "Would dispense paint, if it were not broken. The maintenance panel has been removed."
					else
				...
				if (2)
					if (istype(W, /obj/item/wrench))
						...
						desc = "Would dispense paint, if it were not broken. The maintenance panel has been removed and the service module has been loosened."
					else
				...
				if (3)
					if (istype(W, /obj/item/cable_coil))
						...
						name = "Partially-Repaired Paint Dispenser"
						desc = "Would dispense paint, if it were not broken. The maintenance panel has been removed and the wiring has been replaced. A \"check paint cartridge\" light is blinking."
					else
				...
				if (5)
					if (istype(W, /obj/item/wrench))
						...
						name = "Almost Fully Repaired and Properly Serviced Paint Dispenser"
						desc = "Would dispense paint, if only the maintenance panel was replaced."
					else
				...
				if (6)
					if (istype(W, /obj/item/tile))
						...
						name = "One-Step-Away-from-Being-Fully-Repaired Paint Dispenser"
						desc = "Would dispense paint, if only the maintenance panel was secured so as to allow operation."
					else
				...
				if (7)
					if (istype(W, /obj/item/screwdriver))
						...
						if (prob(5))
							...
							new /obj/machinery/vending/paint(loc)
							qdel(src)
							return
						user.visible_message("<span style=\"color:red\"><strong>[user] slips, knocking the paint dispenser over!.</strong></span>")
						boutput(user, "<strong><font color=red>OH FUCK</font></strong>")

						name = "Irreparably Destroyed Paint Dispenser"
						desc = "Damaged beyond all repair, this will never dispense paint ever again."

						flick("vendbreak", src)
						spawn (8)
							icon_state = "fallen"
							sleep(70)
							playsound(loc, "sound/effects/Explosion2.ogg", 100, 1)

							var/obj/effects/explosion/delme = new /obj/effects/explosion(loc)
							delme.fingerprintslast = fingerprintslast
...
/obj/window/cubicle
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/screwdriver))
			...
			user << (anchored ? "You have fastened the cubicle panel to the floor." : "You have unfastened the cubicle panel.")
			return
...
/obj/item/broken_rcd
	...
	var/effects/system/spark_spread/spark_system

	New()
		..()
		icon_state = "bad_rcd[rand(0,2)]"
		spark_system = unpool(/effects/system/spark_spread)
...
/obj/effects/void_break
	...
	proc/void_shatter()
		playsound(loc, "sound/misc/meteorimpact.ogg", 80, 1)
...
/obj/effects/pressurizer
	...
	proc/do_pressurize()
		...
		var/gas_mixture/GM = T.return_air()
		if (!istype(GM))
			return
...
/obj/machinery/bot/guardbot/future
	...
	setup_charge_maximum = 800
	setup_default_startup_task = /computer/file/guardbot_task/future
	setup_default_tool_path = /obj/item/device/guardbot_tool/taser
	...
	wakeup()
		if (on)
			playsound(loc, 'sound/machines/futurebuddy_beep.ogg', 50, 1)
			return ..()

	interact(mob/user as mob)
		var/dat = "<tt><strong>PR-7 Robuddy v2.9</strong></tt><br><br>"
...
/computer/file/guardbot_task/future
	name = "wally"
	task_id = "1STMATE"
	...
	task_act()
		...
		for (var/mob/living/maybe_that_somebody in view(7, master)) //We don't want to talk to the dead.
			if (istype(maybe_that_somebody, /mob/living/carbon/human/future))
				if (!(dialogChecklist & WD_SLEEPER_SCREAM))
					...
					master.speak("Oh no oh no oh no no no no")
					master.visible_message( "<span style=\"color:red\">[master] points repeatedly at [maybe_that_somebody]![prob(50) ? "  With both arms, no less!" : null]</span>")
					master.set_emotion("screaming")
					spawn (40)
						if (master)
		...
		if (somebody_to_talk_to)
			if (!(dialogChecklist & WD_HELLO))
				...
				master.speak( pick(greet_strings) )
				return
				...
				master.speak("Aaa! Please stay away from there! You can't wake him up, okay? It's not safe!")
				spawn (15)
					master.speak("I mean, for him.  Sleepers slow down aging, but it turns out that DNA or whatever still ages really, really slowly.")
					sleep(10)
				...
				master.speak( pick(idle_dialog_strings) )

		if (istype(get_area(master), /area/solarium) && !(dialogChecklist & WD_SOLARIUM))
			dialogChecklist |= WD_SOLARIUM

			src.master.speak( "Oh, this place is familiar!  It looks like a ship, a model...um...")
			spawn (10)
				master.speak("I'm sorry, I don't recognize this ship!  Maybe I can interface with its onboard computer though?")
				sleep(20)
				master.speak("Okay, it's yelling at me in a language I do not understand!  Weird!")
				sleep(20)
...
/obj/machinery/sleeper/future
	...
	dead_man_sleeping
		New()
			..()
			spawn (10)
				occupant = new /mob/living/carbon/human/future (src)
...
/mob/living/carbon/human/future
	...
	New()
		...
		spawn (0)
			...
			equip_if_possible(new /obj/item/clothing/under/color/white(src), slot_w_uniform)
			equip_if_possible(new /obj/item/device/key {name = "futuristic key"; desc = "It appears to be made of some kind of space-age material.  Like really fancy aluminium or something.";} (src), slot_l_store)

	Life(controller/process/mobs/parent)
		if (..(parent))
			return TRUE
		if (!stat && !istype(loc, /obj/machinery/sleeper))
			if (prob(40) || !death_countdown)
				say( pick("Buhh...", "I'm...awake? No...", "I can't be awake...", "Who are you?  Where am I?", "Who--nghh. It...hurts..") )

			if (ckey && !had_thought && !death_countdown)
				//7848(2)9(1) = 7848b9a = hex for 126127002 = 126 127 002 = coordinates to cheget key
				//A fucker is me
				src.show_text("<strong><em>A foreign thought flashes into your mind... <font color=red>Rem..e...mbe...r 78... 4... 8(2)... 9... (1) alw..a...ys...</font></em></strong>")
				had_thought = 1
...
/proc/helldrone_wakeup()
	...
	if (helldrone_awake != 2)
		logTheThing("debug", null, null, "<strong>Halloween Event Error</strong>: Unable to locate helldrone areas.")
		return
...
/obj/critter/ancient_repairbot/helldrone_guard
	...
	proc/wakeup()
		...
		sleeping = 0
		task = "thinking"
		desc = "A machine.  Of some sort.  It looks mad"

		visible_message("<span class='combat'>[src] seems to power up!</span>")
...
/obj/item/device/audio_log/timewarp_00
	...
	New()
		..()
		tape = new /obj/item/audio_tape/timewarp_00(src)
...
/obj/item/device/audio_log/timewarp_01
	...
	New()
		..()
		tape = new /obj/item/audio_tape/timewarp_01(src)
...
/obj/item/toy/halloween2014spellbook
	...
	attack_self(var/mob/user as mob)
				...
				for (var/mob/living/carbon/C in range(4,user))
					...
					if (C.bioHolder)
						...
						spawn (600)
							C.bioHolder.age -= 125
			if ("NULL MOSS NOOK")
				particleMaster.SpawnSystem(new /particleSystem/skull_rain(get_turf(user)))
				user.show_text("You hear something rattling above you!","#8218A8")
			if ("ONION SLUG CANDY")
				particleMaster.SpawnSystem(new /particleSystem/spooky_mist(get_turf(user)))
		...
		if (used)
			user.visible_message("<span class='combat'><strong>[user.name]</strong> reads a spell from the book!</span>")
			uses--
			if (uses == 0)
