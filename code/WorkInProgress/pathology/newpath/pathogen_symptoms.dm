...
/pathogeneffects
	...
	proc/disease_act(var/mob/M as mob, var/pathogen/origin)

	// infect(mob, pathogen) : void
	...
	proc/infect(var/mob/M as mob, var/pathogen/origin)
		for (var/mob/I in view(infect_type, M.loc))
			if (I != M && ((isturf(I.loc) && isturf(M.loc) && can_line(M, I, 5)) || I.loc == M.loc))
				...
				if (permeability < permeability_score)
					continue
	...
	// infect_direct(mob, pathogen) : void
	...
	// OVERRIDE: Generally, you do not need to override this.
	proc/infect_direct(var/mob/target as mob, var/pathogen/origin, contact_type = "touch")
	...
	proc/onadd(var/pathogen/origin)
		return
	...
	// ondisarm(mob, mob, boolean, pathogen) : float
	// OVERRIDE: Overriding this is situational. ..() is expected to be called.
	proc/ondisarm(var/mob/M as mob, var/mob/V as mob, isPushDown, var/pathogen/origin)
		if (infect_type == INFECT_TOUCH)
			infect_direct(V, origin, "disarm")
		return TRUE

	// onpunch(mob, mob, string, pathogen) : float
	// OVERRIDE: Overriding this is situational. ..() is expected to be called.
	proc/onpunch(var/mob/M as mob, var/mob/V as mob, zone, var/pathogen/origin)
		if (infect_type == INFECT_TOUCH)
			infect_direct(V, origin, "punching")
		return TRUE

	// onpunched(mob, mob, string, pathogen) : float
	// OVERRIDE: Overriding this is situational. ..() is expected to be called.
	proc/onpunched(var/mob/M as mob, var/mob/A as mob, zone, var/pathogen/origin)
		if (infect_type == INFECT_TOUCH)
			infect_direct(A, origin, "being punched")
		return TRUE

	// onshocked(mob, mob, shockparam, pathogen) : shockparam
	// OVERRIDE: Overriding this is situational.
	proc/onshocked(var/mob/M as mob, var/shockparam/ret, var/pathogen/origin)
		return ret

	// ongrab(mob, mob, pathogen) : void
	// TODO: Make this a veto event.
	// OVERRIDE: Overriding this is situational. ..() is expected to be called.
	proc/ongrab(var/mob/M as mob, var/mob/V as mob, var/pathogen/origin)
		if (infect_type == INFECT_TOUCH)
			infect_direct(V, origin, "grab")
		return

	// onsay(mob, string, pathogen) : string
	// OVERRIDE: Overriding this is situational.
	proc/onsay(var/mob/M as mob, message, var/pathogen/origin)
		return message

	// onemote(mob, string, pathogen) : string
	// OVERRIDE: Overriding this is situational.
	proc/onemote(var/mob/target, act, var/pathogen/P)
		return TRUE
...
/pathogeneffects/malevolent/coughing
	name = "Coughing"
	...
	disease_act(var/mob/M as mob, var/pathogen/origin)
		switch (origin.stage)
			if (1)
				if (prob(3))
...
/pathogeneffects/malevolent/indigestion
	name = "Indigestion"
...
/pathogeneffects/malevolent/muscleache
	name = "Muscle Ache"
...
/pathogeneffects/malevolent/sneezing
	name = "Sneezing"
...
/pathogeneffects/malevolent/gasping
	name = "Gasping"
...
/pathogeneffects/malevolent/moaning
	name = "Moaning"
...
/pathogeneffects/malevolent/hiccups
	name = "Hiccups"
	desc = "This is literally pointless."
	infect_type = INFECT_NONE
	rarity = RARITY_VERY_COMMON
...
/pathogeneffects/malevolent/shivering
	name = "Shivering"
	desc = "This is literally pointless."
	infect_type = INFECT_NONE
	rarity = RARITY_VERY_COMMON
...
/pathogeneffects/malevolent/deathgasping
	name = "Deathgasping"
	desc = "This is literally pointless."
	infect_type = INFECT_NONE
...
/pathogeneffects/malevolent/sweating
	name = "Sweating"
...
/pathogeneffects/malevolent/disorientation
	name = "Disorientation"
...
/pathogeneffects/malevolent/serious_paranoia
	...
	proc/trader(var/mob/M as mob, var/mob/living/O as mob)
		...
		if (prob(50))
			boutput(M, "<strong>[O]</strong> points at [M].")
			var/point = new /obj/decal/point(get_turf(M))
			spawn (30)
				qdel(point)
		boutput(M, "<strong>[O]</strong> [action], \"[what]\"")
	...
	proc/acidspit(var/mob/M, var/mob/living/O, var/mob/living/O2)
		if (O2)
			boutput(M, "<span style=\"color:red\"><strong>[O] spits acid at [O2]!</strong></span>")
		else
			boutput(M, "<span style=\"color:red\"><strong>[O] spits acid at you!</strong></span>")
	...
	proc/vampirebite(var/mob/M, var/mob/living/O, var/mob/living/O2)
		if (O2)
			boutput(M, "<span style=\"color:red\"><strong>[O] bites [O2]!</strong></span>")
		else
			boutput(M, "<span style=\"color:red\"><strong>[O] bites you!</strong></span>")
...
/pathogeneffects/malevolent/serious_paranoia/mild
	name = "Paranoia"
...
/pathogeneffects/malevolent/teleportation
	name = "Teleportation"
...
/pathogeneffects/malevolent/gibbing
	name = "Gibbing"
...
/pathogeneffects/malevolent/shakespeare
	name = "Shakespeare"
...
"The lady doth protest too much, methinks.")
	...
	onsay(var/mob/M as mob, message, var/pathogen/origin)
		if (!(message in shk))
...
/pathogeneffects/malevolent/fluent
	name = "Fluent Speech"
...
/pathogeneffects/malevolent/capacitor
	name = "Capacitor"
	...
	proc/discharge(var/mob/M as mob, var/pathogen/origin)
	...
	proc/load_check(var/mob/M as mob, var/pathogen/origin)
		var/load = origin.symptom_data["capacitor"]
		if (load > capacity)
	...
	onadd(var/pathogen/origin)
		origin.symptom_data["capacitor"] = 0

	onshocked(var/mob/M as mob, var/shockparam/ret, var/pathogen/origin)
		var/amt = ret.amt
	...
	ondisarm(var/mob/M as mob, var/mob/V as mob, isPushDown, var/pathogen/origin)
		var/load = origin.symptom_data["capacitor"]
	...
	onpunch(var/mob/M as mob, var/mob/V as mob, zone, var/pathogen/origin)
		var/load = origin.symptom_data["capacitor"]
		if (load > 2e6)
			if (prob(25))
	...
	onpunched(var/mob/M as mob, var/mob/A as mob, zone, var/pathogen/origin)
		var/load = origin.symptom_data["capacitor"]
		if (load > 5000)
			if (prob(25))
			...
			if (2)
				if (prob(9))
					...
					if (C && PN.get_avail() > 0)
						...
						s.start()
						M.visible_message("<span style=\"color:red\">A spark jumps from the power cable at [M].</span>", "<span style=\"color:red\">A spark jumps at you from a nearby cable.</span>", "<span style=\"color:red\">You hear something spark.</span>")
						var/amt = max(250000, PN.get_avail())
						PN.newload -= amt
...
/pathogeneffects/malevolent/capacitor/unlimited
	name = "Unlimited Capacitor"

	load_check(var/mob/M as mob, var/pathogen/origin)
		return null
...
/pathogeneffects/malevolent/sunglass
...
/pathogeneffects/malevolent/liverdamage
	name = "Hepatomegaly"
	...
	react_to(var/R, var/zoom)
		...
		if (R == "ethanol")
			...
			var/reagents/H = new /reagents(5)
			H.add_reagent(R, 5)
			var/RE = H.get_reagent(R)
			if (istype(RE, /reagent/fooddrink/alcoholic))
				alcoholic = RE:name
		if (alcoholic)
			return "The pathogen appears to react violently to the [alcoholic]."

/pathogeneffects/malevolent/fever
	name = "Fever"
...
/pathogeneffects/malevolent/acutefever
	name = "Acute Fever"
...
/pathogeneffects/malevolent/ultimatefever
	name = "Dragon Fever"
...
/pathogeneffects/malevolent/chills
	name = "Common Chills"
...
/pathogeneffects/malevolent/seriouschills
	name = "Acute Chills"
...
/pathogeneffects/malevolent/seriouschills/ultimate
	name = "Arctic Chills"
...
/pathogeneffects/malevolent/farts
	name = "Farts"
	...
	proc/fart(var/mob/M, var/pathogen/origin)
		M.emote("fart")
		infect(M, origin)

	disease_act(var/mob/M, var/pathogen/origin)
		if (!origin.symptomatic)
			return
		if (prob(origin.stage * 3))
			fart(M, origin)
...
/pathogeneffects/malevolent/farts/smoke
	name = "Smoke Farts"
	desc = "The infected individual occasionally farts reagent smoke."
	rarity = RARITY_RARE

	fart(var/mob/M, var/pathogen/origin)
		..()
...
/pathogeneffects/malevolent/farts/plasma
	...
	fart(var/mob/M, var/pathogen/origin)
		...
		var/gas_mixture/gas = unpool(/gas_mixture)
		gas.zero()
...
/pathogeneffects/malevolent/farts/co2
	name = "CO2 Farts"
...
/pathogeneffects/malevolent/leprosy
	name = "Leprosy"
	desc = "The infected individual is losing limbs."
	rarity = RARITY_VERY_RARE

	disease_act(var/mob/living/carbon/human/M, var/pathogen/origin)
...
/pathogeneffects/malevolent/senility
	name = "Senility"
