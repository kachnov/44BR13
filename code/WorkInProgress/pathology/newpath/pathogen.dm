...
/controller/pathogen
	...
	proc/get_microbody(var/strength)
		...
		while (!finished)
			...
			for (var/choice/C in microbody_choices)
	...
	proc/mob_infected(var/pathogen/P, var/mob/living/carbon/human/H)
		var/pathogen_cdc/CDC = pathogen_trees[P.name_base]
		if (!CDC)
			return
		if (!CDC.patient_zero)
			CDC.patient_zero = H
		...
		if (!(P.name in CDC.mutations))
			...
			var/pathogen/template = unpool(/pathogen)
	...
	proc/mob_cured(var/pathogen/P, var/mob/living/carbon/human/H)
		var/pathogen_cdc/CDC = pathogen_trees[P.name_base]
		if (!CDC)
			return
		if (H in CDC.infections)
	...
	proc/patient_zero(var/pathogen_cdc/CDC, var/topic_holder)
		if (CDC.patient_zero)
	...
	Topic(href, href_list)
		...
		switch(href_list["action"])
			...
			if ("strain_cure")
				...
				var/pathogen_cdc/CDC = pathogen_trees[strain]
				var/count = 0
			...
			if ("pathogen_creator")
				var/pathogen/P = cdc_creator[usr.ckey]
				switch (href_list["do"])
					if ("reset")
						gen_empty(usr.ckey)
					...
					if ("body_type")
						var/list/types = list()
						for (var/btpath in path_to_microbody)
							var/microbody/MB = path_to_microbody[btpath]
							types += MB.name
							types[MB.name] = MB
					...
					if ("suppressant")
						var/list/types = list()
						for (var/spath in path_to_suppressant)
							var/suppressant/S = path_to_suppressant[spath]
							types += S.name
							types[S.name] = S
					...
					if ("add")
						var/list/types = list()
						for (var/efpath in path_to_symptom)
							var/pathogeneffects/EF = path_to_symptom[efpath]
							types += EF.name
							types[EF.name] = EF
						var/chosen = input("Which symptom?", "Add new symptom", types[1]) in types
						if (!(types[chosen] in P.effects))
							P.effects += types[chosen]
							var/pathogeneffects/EF = types[chosen]
							EF.onadd(P)

					if ("remove")
						var/pathogeneffects/EF = locate(href_list["which"])
						if (EF in P.effects)
							P.effects -= EF
					...
					if ("stages")
						...
						if (newval >= 3 && newval <= 5)
							P.stages = newval
					if ("create")
						P.dnasample = new/pathogendna(P)
						P.pathogen_uid = "[next_uid]"
						next_uid++

						pathogen_trees += P.name_base
						var/pathogen_cdc/CDC = new /pathogen_cdc(P.pathogen_uid)
			...
			if ("strain_data")
				var/pathogen_cdc/CDC = locate(href_list["which"])
				var/name = href_list["name"]
				var/pathogen/reference = CDC.mutations[name]
				switch (href_list["data"])
					if ("advance_speed")
						...
						for (var/mob/living/carbon/human/H in CDC.infections)
							if (CDC.uid in H.pathogens)
								var/pathogen/target = H.pathogens[CDC.uid]
								if (target.name == name)
					...
					if ("cure")
						...
						for (var/mob/living/carbon/human/H in world)
							if (CDC.uid in H.pathogens)
								var/pathogen/P = H.pathogens[CDC.uid]
								if (P.name == name)
									H.cured(P)
									count++
					...
					if ("spawn")
						...
						var/reagent/blood/pathogen/RE = new /reagent/blood/pathogen()
						RE.volume = 5
			...
			if ("microbody_data")
				var/microbody/MB = locate(href_list["which"])
				switch (href_list["data"])
					if ("stages")
			...
			if ("mutation_data")
				var/pathogen_mutation/MUT = locate(href_list["which"])
				switch (href_list["data"])
					if ("info")
						alert(usr, MUT.desc)
					if ("session_maximum")
					...
					if ("disable")
						path_to_mutation -= MUT.type
						disabled_mutations += MUT.type
						disabled_mutations[MUT.type] = MUT
						message_admins("[key_name(usr)] disabled pathogen mutation [MUT].")
					if ("enable")
						disabled_mutations -= MUT.type
						path_to_mutation += MUT.type
						path_to_mutation[MUT.type] = MUT
	...
	proc/severity_color(var/pathogeneffects/EF)
		if (EF.rarity == RARITY_ABSTRACT)
			return "[EF]"
		var/color_value = round(255 / EF.rarity)
		if (istype(EF, /pathogeneffects/malevolent))
...
.pathology-table .name { font-weight: bold; }
		...
		if (istext(state))
			...
			if (state in pathogen_trees)
				var/pathogen_cdc/CDC = pathogen_trees[state]
				...
				for (var/name in CDC.mutations)
					output += "<tr>"
					var/pathogen/P = CDC.mutations[name]
					output += "<td>[name]</td>"
					var/symptoms = ""
					var/first = 1
					for (var/pathogeneffects/EF in P.effects)
						if (first)
			...
			switch (states[state])
				if ("strains")
					...
					for (var/pathogen_name in pathogen_trees)
						var/pathogen_cdc/CDC = pathogen_trees[pathogen_name]
						output += "<tr>"
				...
				if ("microbodies")
					...
					for (var/microbody_path in path_to_microbody)
						output += "<tr>"
						var/microbody/MB = path_to_microbody[microbody_path]
				...
				if ("mutations")
					...
					for (var/mutation_path in path_to_mutation)
						output += "<tr>"
						var/pathogen_mutation/MUT = path_to_mutation[mutation_path]
					...
					for (var/mutation_path in disabled_mutations)
						output += "<tr>"
						var/pathogen_mutation/MUT = disabled_mutations[mutation_path]
						output += "<td class='name'>[MUT]</td>"
						output += "<td><a href='?src=\ref[src];action=mutation_data;which=\ref[MUT];data=info;topic_holder=\ref[topic_holder]'>Show information</a></td>"
						output += "<td><a href='?src=\ref[src];action=mutation_data;which=\ref[MUT];data=session_maximum;topic_holder=\ref[topic_holder]'>[MUT.session_maximum]</a></td>"
						output += "<td><a href='?src=\ref[src];action=mutation_data;which=\ref[MUT];data=chance_base;topic_holder=\ref[topic_holder]'>[MUT.chance_base]%</a></td>"
				...
				if ("symptoms")
					...
					for (var/sym_path in path_to_symptom)
						var/pathogeneffects/EF = path_to_symptom[sym_path]
						output += "<tr>"
						output += "<td>[EF]</td>"
						...
						var/DNA = symptom_to_UID[EF.type]
						output += "<td>[DNA]</td>"
						output += "</tr>"
				if ("suppressants")
					output += "<table class='pathology-table'><thead><tr><th>Name</th><th>Suppression reagents</th></thead><tbody>"
					for (var/sup_path in path_to_suppressant)
						output += "<tr>"
						var/suppressant/S = path_to_suppressant[sup_path]
						output += "<td>[S]</td>"
						var/first = 1
						var/supp = ""
						...
						output += "</tr>"
				if ("pathogen creator")
					if (!(usr.ckey in cdc_creator))
						gen_empty(usr.ckey)
					var/pathogen/P = cdc_creator[usr.ckey]
					output += "<h3>Pathogen Creator</h3>"
					output += "<strong>Strain: </strong> [P.name_base]<br>"
					output += "<strong>Base mutation:</strong> [P.mutation]<br>" 
					output += "<strong>Name: </strong> [P.name]<br>"
					if (P.suppressant)
						output += "<strong>Description: </strong> [P.desc]<br>"

					if (!P.body_type)
						output += "<a href='?src=\ref[src];action=pathogen_creator;do=body_type;topic_holder=\ref[topic_holder]'>Assign microbody</a><br>"
					else
						output += "<strong>Microbody:</strong> [P.body_type]<br>"
						output += "<strong>Stages:</strong> <a href='?src=\ref[src];action=pathogen_creator;do=stages;topic_holder=\ref[topic_holder]'>[P.stages]</a><br>"
						output += "<strong>Advance speed:</strong> <a href='?src=\ref[src];action=pathogen_creator;do=advance_speed;topic_holder=\ref[topic_holder]'>[P.advance_speed]</a><br>"
						output += "<strong>Maliciousness:</strong> <a href='?src=\ref[src];action=pathogen_creator;do=maliciousness;topic_holder=\ref[topic_holder]'>[P.maliciousness]</a><br>"
						output += "<strong>Mutation speed:</strong> <a href='?src=\ref[src];action=pathogen_creator;do=mutation_speed;topic_holder=\ref[topic_holder]'>[P.mutation_speed]</a><br>"
						output += "<strong>Mutativeness:</strong> <a href='?src=\ref[src];action=pathogen_creator;do=mutativeness;topic_holder=\ref[topic_holder]'>[P.mutativeness]</a><br>"
						output += "<strong>Symptomatic:</strong> <a href='?src=\ref[src];action=pathogen_creator;do=symptomatic;topic_holder=\ref[topic_holder]'>[P.symptomatic ? "Yes" : "No"]</a><br>"
						output += "<strong>Suppression threshold:</strong> <a href='?src=\ref[src];action=pathogen_creator;do=suppression_threshold;topic_holder=\ref[topic_holder]'>[P.suppression_threshold]</a><br>"
						if (!P.suppressant)
							output += "<a href='?src=\ref[src];action=pathogen_creator;do=suppressant;topic_holder=\ref[topic_holder]'>Assign suppressant</a><br>"
						else
							output += "<strong>Suppressant: </strong>[P.suppressant]<br><br>"
							output += "<strong>Effects: </strong><br>"
							if (P.effects.len)
								for (var/pathogeneffects/EF in P.effects)
	...
	New()
		...
		for (var/T in subtypesof(/microbody))
			var/microbody/B = new T()
		...
		var/list/available_mutations = subtypesof(/pathogen_mutation)
		for (var/mutation in available_mutations)
			path_to_mutation[mutation] += new mutation()

		var/list/eff = typesof(/pathogeneffects)
		l_vc = list()
		l_c = list()
		l_u = list()
		l_r = list()
		l_vr = list()
		for (var/E in eff)
			var/pathogeneffects/inst = new E()
		...
		for (var/T in (subtypesof(/suppressant)))
			var/r
			do
		...
		/*for (var/T in typesof(/pathogen_carrier))
			var/r
			do
				r = num2hex(rand(0, 4095), 3)
			while (r in used)
...
var/global/controller/pathogen/pathogen_controller = new()
...
/pathogen
	...
	var/microbody/body_type					// The body type of the pathogen, which determines the base mutativeness and the method of synthesizing a cure.
	...
	var/pathogendna/dnasample = null			// A reference of what the pathogen's DNA looks like. This reference is passed around
													// and copied only when coming into contact with something that directly works with
													// the DNA itself.

	var/suppressant/suppressant				// The method of suppression for the pathogen. This is picked from all children of /suppressant with an equal
													...
													// just define a child for /suppressant and please make sure it's not an already existing colour.
	...
	proc/generate_name()
		mutation = pick(pathogen_controller.lnums)
		do
			name_base = pick(pathogen_controller.lalph) + pick(pathogen_controller.lnums) + pick(pathogen_controller.lalph)
		while (name_base in pathogen_controller.pathogen_trees)
		name = name_base + mutation
		mutation = text2num(mutation)
		base_mutation = 0

		pathogen_uid = "[pathogen_controller.next_uid]"
		pathogen_controller.next_uid++

		pathogen_controller.pathogen_trees += name_base
		var/pathogen_cdc/cdc = new /pathogen_cdc(pathogen_uid)
		pathogen_controller.pathogen_trees[name_base] = cdc
		pathogen_controller.next_mutation[pathogen_uid] = mutation + 1

//		if (ticker)
//			if (ticker.current_state == GAME_STATE_PLAYING)
//				message_admins("Pathogen tree [name_base] entering play.")

		return cdc

	proc/generate_components(var/pathogen_cdc/cdc, var/strength)
		...
		if (!forced_microbody)
			body_type = pathogen_controller.get_microbody(strength + 5)
			cdc.microbody_type = "[body_type]"
		else
			body_type = pathogen_controller.path_to_microbody[forced_microbody]
			cdc.microbody_type = "[body_type]"
	...
	proc/generate_attributes(var/strength)
		var/adj_strength = strength - 8
		maliciousness = rand(-2, 6) + rand(adj_strength - 8, adj_strength + 8)
		mutation_speed = rand(-2, 10) + rand(adj_strength - 8, adj_strength + 8)

		suppression_threshold += rand(1,3) * maliciousness + rand(-5, 10) + rand(adj_strength - 8, adj_strength + 8)
		advance_speed = rand(-2, 7) + rand(adj_strength - 8, adj_strength + 8)

		mutativeness = body_type.mutativeness

		maliciousness *= body_type.maliciousness_multiplier
		mutation_speed *= body_type.mutation_speed_multiplier

		if (curable_by_suppression < 0 && strength < 10)
			curable_by_suppression = rand(-10 + strength, 10 - strength)
			if (curable_by_suppression < 0)	
				curable_by_suppression = 0
		else
			curable_by_suppression = 0

		stages = body_type.stages

		var/shape = pick("stringy", "snake", "blob", "spherical", "tetrahedral", "star shaped", "tesselated")
		desc = "[suppressant.color] [shape] [body_type.plural]"
	...
	proc/randomize(var/strength)
		var/pathogen_cdc/cdc = generate_name()
		cdc.mutations += name
		cdc.mutations[name] = src
	...
	proc/setup(status, var/pathogen/origin, may_mutate, var/pathogendna/sample = null)
		if (status == 0 && !origin)
			return
		in_remission = 0
		if (origin)
			...
			name = origin.name
			name_base = origin.name_base
			stages = origin.stages
			desc = origin.desc
			carriers = list()
			src.effects = origin.effects.Copy()
			for (var/pathogeneffects/E in effects)
				E.onadd(src)
			pathogen_uid = origin.pathogen_uid
			src.suppressant = new origin.suppressant.type()
			suppressant.onadd(src)
			stage = 1
			generation = origin.generation
			symptomatic = origin.symptomatic
			advance_speed = origin.advance_speed
			mutativeness = origin.mutativeness
			mutation_speed = origin.mutation_speed
			body_type = origin.body_type
			maliciousness = origin.maliciousness
			suppression_threshold = origin.suppression_threshold
			if (mutativeness && may_mutate)
				if (prob(mutativeness))
					...
					if (sample)
						...
						dnasample = origin.dnasample
			else
				if (sample)
					dnasample = sample
				else
					dnasample = origin.dnasample
			curable_by_suppression = origin.curable_by_suppression
		else if (status == 1)
			randomize(8)
			if (sample)
				dnasample = sample
			else
				dnasample = new/pathogendna(src)
	...
	proc/generate_weak_effect()
		switch(rand(1,100))
			if (1 to 70)
				return add_new_symptom(pathogen_controller.l_vc)
			if (71 to 97)
				return add_new_symptom(pathogen_controller.l_c)
			if (98 to 100)
				return add_new_symptom(pathogen_controller.l_u)
	...
	proc/generate_effect()
		switch(rand(1,100))
			...
			if (86 to 94) // 8%
				return add_new_symptom(pathogen_controller.l_u)
			if (95 to 98) // 4%
				return add_new_symptom(pathogen_controller.l_r)
			if (99 to 100) // 2%
				return add_new_symptom(pathogen_controller.l_vr)
	...
	proc/mutate()
		...
		mutation = pathogen_controller.next_mutation[pathogen_uid]
		pathogen_controller.next_mutation[pathogen_uid] += 1
		name = name_base + "[mutation]"
		...
		for (var/i = 1, i <= count, i++)
			...
			var/pathogen_mutation/mut = pathogen_controller.path_to_mutation[T]
	...
	proc/process()
		...
		if (infected)
			...
			if (istype(M))
				...
				rads += rad
				if (M.reagents)
					var/reagent/R = M.reagents.get_reagent("mutagen")
					if (istype(R))
						rads += R.volume
	...
	proc/disease_act()
		...
		if (order)
			for (var/effect in effects)
				var/pathogeneffects/E = effect
				if (E.type in acted)
					continue
				acted += E.type
				if (prob(body_type.activity[stage]))
					E.disease_act(infected, src)
		else
			for (var/i = effects.len, i > 0, i--)
				var/pathogeneffects/E = effects[i]
				if (E.type in acted)
					continue
				acted += E.type
				if (prob(body_type.activity[stage]))
	...
	// 2) Define a default event handler in /pathogeneffects. This is necessary so that all symptoms continue working, even if they don't respond to that event.
	// 3) Define a default event handler in /suppressant. This is necessary so that all suppression methods continue working, even if they don't respond to that event.
	...
	proc/add_new_symptom(var/list/allowed, var/allow_duplicates = 0)
		for (var/i = 0, i < 10, i++)
			var/T = pick(allowed)
			var/pathogeneffects/E = pathogen_controller.path_to_symptom[T]
			if (add_symptom(E, allow_duplicates))
				return TRUE
		return FALSE

	proc/add_symptom(var/pathogeneffects/E, var/allow_duplicates = 0)
	...
	proc/remove_symptom(var/pathogeneffects/E, var/all = 0)
		if (all)
			var/rem = 0
			while (E in effects)
				effects -= E
