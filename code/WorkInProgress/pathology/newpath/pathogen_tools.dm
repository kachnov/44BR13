...
/obj/item/reagent_containers/glass/petridish
	...
	New()
		var/reagents/R = new/reagents(40)
		reagents = R
		R.my_atom = src
	...
	examine()
		...
		if (reagents.reagent_list["pathogen"])
			var/reagent/blood/pathogen/P = reagents.reagent_list["pathogen"]
			boutput(usr, "<span style=\"color:blue\">It contains [P.volume] units of harvestable pathogen.</span>")
		if (medium)
			boutput(usr, "<span style=\"color:blue\">The petri dish is coated with [medium.name].</span>")
	...
	afterattack(obj/target, mob/user , flag)
		...
		..(target, user, flag)
		if (amount && !reagents.total_volume)
	...
	process()
		...
		if (ctime <= 0)
			...
			for (var/N in PT.body_type.nutrients)
				if (nutrition[N] < PT.body_type.amount * P.volume)
					starvation = 1
					nutrition[N] = 0
				else
					starving = 5
					nutrition[N] -= PT.body_type.amount * P.volume
	...
	on_reagent_change()
		...
		if (stage > 0)
			...
			for (var/R in reagents.reagent_list)
				var/reagent/RE = reagents.reagent_list[R]
			...
			if (reagents.reagent_list.len == 1 && reagents.reagent_list[1] == "pathogen")
				return
			for (var/R in reagents.reagent_list)
				var/reagent/RE = reagents.reagent_list[R]
				if (R == "pathogen")
					...
					if (medium && medium.id != R)
						...
						reagents.reagent_list -= R
						reagents.update_total()
						if (reagents.has_reagent("pathogen"))
							if (!(src in processing_items))
								processing_items.Add(src)
					else
						if (RE.pathogen_nutrition)
...
/obj/item/reagent_containers/glass/vial/prepared
	...
	New()
		...
		spawn (20)
			...
			P.create_weak()
			var/reagents/RE = reagents
			RE.add_reagent("pathogen", 5)
			var/reagent/blood/pathogen/R = RE.get_reagent("pathogen")
...
/obj/item/reagent_containers/glass/beaker/parasiticmedium
	...
	New()
		..()
		reagents.add_reagent("parasiticmedium", 50)
...
/obj/item/reagent_containers/glass/beaker/stablemut
	...
	New()
		..()
		reagents.add_reagent("dna_mutagen", 50)
...
/obj/item/reagent_containers/glass/beaker/bacterial
	...
	New()
		..()
		reagents.add_reagent("bacterialmedium", 50)
...
/obj/item/reagent_containers/glass/beaker/fungal
	...
	New()
		..()
		reagents.add_reagent("fungalmedium", 50)
...
/obj/item/reagent_containers/glass/beaker/antiviral
	...
	New()
		..()
		reagents.add_reagent("antiviral", 50)
...
/obj/item/reagent_containers/glass/beaker/biocides
	...
	New()
		..()
		reagents.add_reagent("biocide", 50)
...
/obj/item/reagent_containers/glass/beaker/spaceacillin
	...
	New()
		..()
		reagents.add_reagent("spaceacillin", 50)
...
/obj/item/reagent_containers/glass/beaker/inhibitor
	...
	New()
		..()
		reagents.add_reagent("inhibitor", 50)
...
/obj/item/serum_injector
	...
	icon = 'icons/obj/pathology.dmi'
	icon_state = "serum"
	var/pathogen/pathogen = null
	var/used = 1
	var/is_cure = 0
	var/is_vaccine = 0

	New(Location, var/pathogen/P, cure, vaccine)
		if (P && istype(P))
			name = "[name] (strain [P.name_base])"
			icon_state = "serum"
			pathogen = P
			is_cure = cure
			is_vaccine = vaccine
	...
	proc/inject(var/mob/living/carbon/human/target, var/mob/user)
		if (is_cure)
			if (!is_vaccine)
				logTheThing("pathology", user, target, "injects %target% with the cure for [pathogen.name].")
				target.remission(pathogen)
			else
				logTheThing("pathology", user, target, "injects %target% with a vaccine for [pathogen.name].")
				target.immunity(pathogen)
		else
			if (target.infected(pathogen))
	...
	attack(mob/M as mob, mob/user as mob, def_zone)
		if (used)
			boutput(user, "<span style=\"color:red\">The [name] is empty.</span>")
			return
		if (istype(M, /mob/living/carbon/human))
			if (M != user)
				for (var/mob/V in viewers(M))
					boutput(V, "<span style=\"color:red\"><strong>[user] is trying to inject [M] with the [name]!</strong></span>")
				var/ML = M.loc
				var/UL = user.loc
				spawn (30)
					if (used)
					...
					if (user.equipped() == src && M.loc == ML && user.loc == UL)
						used = 1
						for (var/mob/V in viewers(M))
							boutput(V, "<span style=\"color:red\"><strong>[user] is injects [M] with the [name]!</strong></span>")
						name = "empty [name]"
						icon_state = "serum0"
						inject(M, user)
			else
				used = 1
				for (var/mob/V in viewers(M))
					boutput(V, "<span style=\"color:red\"><strong>[user] injects [M] with the [name]!</strong></span>")
				icon_state = "serum0"
				name = "empty [name]"
				inject(user, user)
