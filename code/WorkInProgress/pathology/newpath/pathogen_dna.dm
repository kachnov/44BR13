 ...
 *   - the unique identifier of the microbody of the pathogen, which must be defined in the /microbody subclass.
...
/pathogendna
	...
	var/pathogen/reference = null

	New(var/pathogen/P)
		if (P)
			reference = unpool(/pathogen)
	...
	proc/clone()
		var/pathogendna/D = new(reference)
		return D

	proc/manipulate(value, direction)
		var/pathogendna/this = src
		src = null
		if (prob(2))
			del this
			return FALSE
		...
		if (direction > 0)
			...
			direction = -1
		if (!this)
			return FALSE // We somehow lost the DNA.
		switch (value)
			if ("mutativeness"):
	...
	proc/recalculate()
		...
		var/uid = num2hexoc(round(reference.body_type.uniqueid), 4)
		var/mut = num2hexoc(round(reference.mutativeness), 4)
		var/mts = num2hexoc(round(reference.mutation_speed), 4)
		var/adv = num2hexoc(round(reference.advance_speed), 4)
		var/mal = num2hexoc(round(reference.maliciousness), 4)
		var/sup = num2hexoc(round(reference.suppression_threshold), 4)
		seqnumeric = "[uid][mut][mts][adv][mal][sup][reference.stages][reference.symptomatic]"
	...
	proc/reevaluate_numeric()
		...
		reference.mutativeness = mut
		reference.mutation_speed = mts
		reference.advance_speed = adv
		reference.maliciousness = mal
		reference.suppression_threshold = sup
		reference.stages = stages
		reference.symptomatic = symptomatic
		for (var/T in pathogen_controller.path_to_microbody)
			var/microbody/B = pathogen_controller.path_to_microbody[T]
			if (B.uniqueid == uid)
				reference.body_type = B
				reference.stages = B.stages
				break

	// Pathogen data -> DNA/Public
	proc/reverse_engineer()
		seqsplice = pathogen_controller.suppressant_to_UID[reference.suppressant.type]
		seqsplice += "|"
		//for (var/pathogen_carrier/C in reference.carriers)
		//	seqsplice += pathogen_controller.carrier_to_UID[C.type]
		seqsplice += "|"
		var/first = 1
		for (var/pathogeneffects/E in reference.effects)
			if (!first)
				seqsplice += "|"
			else
				first = 0
			seqsplice += pathogen_controller.symptom_to_UID[E.type]
	...
	proc/reevaluate()
		...
		if (!(parts[1] in pathogen_controller.UID_to_suppressant))
			...
			if (this)
				...
				return FALSE // Somehow, we lost the DNA.
		if (parts[2] != "|")
			//log_game("[this.seqsplice] collapses: no separator after suppressant.")
			qdel(this)
			return FALSE // Bad DNA: no separator after suppressant.
		var/pos = 2
		if (parts[3] == "|")
			pos = 4 // No carriers.
		else
	...
	proc/regenerate_uid()
		reference.pathogen_uid = "[pathogen_controller.next_uid]"
		reference.mutation = pick(pathogen_controller.lnums)
		reference.name_base = pick(pathogen_controller.lalph) + pick(pathogen_controller.lnums) + pick(pathogen_controller.lalph)
		reference.name = reference.name_base + "[reference.mutation]"
		pathogen_controller.next_uid++

	proc/move_mutation()
		reference.mutation = pathogen_controller.next_mutation[reference.pathogen_uid]
		pathogen_controller.next_mutation[reference.pathogen_uid] += 1
		reference.name = reference.name_base + "[reference.mutation]"
