...
/obj/machinery/disposal/mail
	...
	New()
		..()
		if (autoname == 1 && !isnull(mail_tag))
			name = "mail chute ([mail_tag])"
	...
	interact(mob/user, var/ai=0)
		...
		var/dat = "<head><title>Mail Transport Unit</title></head><body><TT><strong>Mail Transport Unit: [mail_tag ? (capitalize(mail_tag)) : "GENERIC"]</strong><HR>"

		if (flush)
			dat += "Shipping handle: <A href='?src=\ref[src];handle=0'>Disengage</A> <strong>Engaged</strong>"
		else
			dat += "Shipping handle: <strong>Disengaged</strong> <A href='?src=\ref[src];handle=1'>Engage</A>"

		dat += "<BR><HR><A href='?src=\ref[src];eject=1'>Eject contents</A><HR>"

		dat += "<br>Destination: <A href='?src=\ref[src];set_dest=1'>[destination_tag ? destination_tag : "NONE"]</a>"
	...
	Topic(href, href_list)
		...
		if (in_range(src, usr) && istype(loc, /turf))
			...
			if (href_list["set_dest"])
				if (destinations)
					var/dest = input("Select Parcel Destination", "Delivery Target Selection", destination_tag) as null|anything in destinations
					if (dest && in_range(src, usr))
						destination_tag = dest
						update()
	...
	receive_signal(signal/signal)
		...
		if (signal.data["command"] == "mail_reply")
			...
			if (!(destination in destinations))
				...
				destinations = sortList(destinations)

		else if (signal.data["command"] == "mail_inquire")
			spawn (4)
				if (src) post_radio_status()
			return
