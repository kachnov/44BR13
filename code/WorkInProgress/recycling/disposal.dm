...
/obj/disposalholder
	invisibility = 101
	var/gas_mixture/gas = null	// gas used to flush, will appear at exit point
	...
	proc/merge(var/obj/disposalholder/other)
		...
		if (other.has_fat_guy)
			has_fat_guy = 1
		if (other.mail_tag && !mail_tag)
			mail_tag = other.mail_tag
		qdel(other)
...
/obj/disposalpipe
	...
	New()
		...
		pipeimg = image(icon, loc, icon_state, 3, dir)
	...
	proc/broken(var/remains = 0)
		...
		if (remains)
			for (var/D in cardinal)
				if (D & dpdir)
					var/obj/disposalpipe/broken/P = new(loc)
					P.dir = D
	...
	proc/welded(var/user)
		...
		var/obj/disposalconstruct/C = new (loc)
...
/obj/disposalpipe/switch_junction
	...
	New()
		...
		if (mail_tag)
			if (islist(mail_tag))
				name = "mail junction (multiple destinations)"
			else
				name = "mail junction ([mail_tag])"
				mail_tag = params2list(mail_tag)
		return
	...
	transfer(var/obj/disposalholder/H)
		var/same_group = 0
		if (mail_tag && (H.mail_tag in mail_tag))
			same_group = 1
...
/obj/disposalpipe/switch_junction/biofilter
	...
	welded()
		...
		var/obj/disposalconstruct/C = new (loc)
		C.ptype = (icon_state == "pipe-sj1" ? 8 : 9)
		C.dir = dir
		C.mail_tag = mail_tag
		C.update()
...
/obj/disposalpipe/loafer
	...
	transfer(var/obj/disposalholder/H)
		...
		if (H.contents.len)
			...
			//visible_message("<strong>[src] activates!</strong>") // Processor + loop = SPAM
			...
			if (nugget_mode)
				...
				for (var/atom/movable/newIngredient in H)
					...
					if (istype(newIngredient, /mob/living))
						playsound(loc, pick("sound/effects/splat.ogg","sound/effects/slosh.ogg","sound/effects/zhit.ogg","sound/effects/attackblob.ogg","sound/effects/blobattack.ogg","sound/effects/bloody_stab.ogg"), 50, 1)
			...
			playsound(loc, pick("sound/machines/mixer.ogg","sound/machines/mixer.ogg","sound/machines/mixer.ogg","sound/machines/hiss.ogg","sound/machines/ding.ogg","sound/machines/buzz-sigh.ogg","sound/effects/robogib.ogg","sound/effects/pop.ogg","sound/machines/warning-buzzer.ogg","sound/effects/Glassbr1.ogg","sound/effects/gib.ogg","sound/effects/spring.ogg","sound/machines/engine_grump1.ogg","sound/machines/engine_grump2.ogg","sound/machines/engine_grump3.ogg","sound/effects/Glasshit.ogg","sound/effects/bubbles.ogg","sound/effects/brrp.ogg"), 50, 1)
			sleep(3)

			playsound(loc, "sound/machines/engine_grump1.ogg", 50, 1)
			sleep(30)
			icon_state = "pipe-loaf0"
			//visible_message("<strong>[src] deactivates!</strong>") // Processor + loop = SPAM
	...
	welded()
		...
		visible_message("<span style=\"color:red\">[src] emits a weird noise!</span>")
	...
	update()
		..()
		if (nugget_mode)
			name = "disciplinary nugget processor"
		else
			name = initial(name)
...
/obj/item/reagent_containers/food/snacks/einstein_loaf
	...
	New()
		...
		reagents = R
		R.my_atom = src
		reagents.add_reagent("liquid spacetime",11)
	...
	proc/update()
		...
		switch ( orderOfLoafitude )
			...
			if (4)
				...
				throwforce = 11
				throw_range = 6
				reagents.add_reagent("thalmerite",25)
			...
			if (10)
				...
				/*spawn (rand(100,1000))
					if (src)
						visible_message("<span style=\"color:red\"><strong>[src] collapses into a black hole! Holy fuck!</strong></span>")
						world << sound("sound/effects/kaboom.ogg")
						new /obj/bhole(get_turf(loc))*/
...
/obj/disposalpipe/block_sensing_outlet
	...
	transfer(var/obj/disposalholder/H)
		...
		for (var/atom/movable/blockingJerk in get_step(src, dir))
...
/obj/disposalpipe/type_sensing_outlet
	...
	transfer(var/obj/disposalholder/H)
		...
		for (var/atom/movable/A in H)
			var/dump_this = 1
			for (var/thing in allowed_types)
...
/obj/disposalpipe/trunk
	...
	proc/getlinked()
		...
		var/obj/disposaloutlet/O = locate() in loc
		if (O)
			linked = O
