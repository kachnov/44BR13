...
/obj/machinery/conveyor/HasEntered(var/atom/movable/AM, /atom/oldloc)
	...
	if (!loc)
		return
	//DEBUG("[AM] entered conveyor at [showCoords(x, y, z)] and is being moved.")
	move_thing(AM)
...
/obj/machinery/conveyor/HasExited(var/atom/movable/AM, var/atom/newloc)
	...
	if (next_conveyor && next_conveyor.loc == newloc)
		...
		//DEBUG("[AM] exited conveyor at [showCoords(x, y, z)] onto another conveyor! Wow!.")
		var/mob/M = AM
		...
		//DEBUG("[AM] exited conveyor at [showCoords(x, y, z)] onto the cold, hard floor.")
		var/mob/M = AM
...
/obj/machinery/conveyor/attackby(var/obj/item/I, mob/user)
	if (istype(I, /obj/item/grab))	// special handling if grabbing a mob
		...
		if (M)
			if (M == user)
				visible_message("<span style=\"color:blue\">[M] ties \himself to the conveyor.</span>")
		...
		if (M && M.buckled == src)
			...
			add_fingerprint(user)
			if (M == user)
				visible_message("<span style=\"color:blue\">[M] cuts \himself free from the conveyor.</span>")
			else
				visible_message("<span style=\"color:blue\">[M] had been cut free from the conveyor by [user].</span>")
			return
