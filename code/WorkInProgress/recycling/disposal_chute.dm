...
/obj/machinery/disposal
	...
	density = 1
	flags = NOSPLASH
	var/gas_mixture/air_contents	// internal reservoir
	...
	attackby(var/obj/item/I, var/mob/user)
		...
		if (istype(I,/obj/item/electronics/scanner))
			user.visible_message("<span style=\"color:red\"><strong>[user] hits [src] with [I]!</strong></span>")
			return
		if (istype(I,/obj/item/satchel))
			...
			if (action == "Empty it into the Chute")
				...
				user.visible_message("<strong>[user.name]</strong> dumps out [S] into [src].")
				return
		if (issilicon(user))
		...
		if (istype(G))	// handle grabbed mob
			if (ismob(G.affecting))
				...
				if (istype(src, /obj/machinery/disposal/mail) && !GM.canRideMailchutes())
					...
					return
				GM.set_loc(src)
				user.visible_message("<span style=\"color:red\"><strong>[user.name] stuffs [GM.name] into [src]!</strong></span>")
				qdel(G)
		...
		if (istype(MO, /obj/item))
			...
			if (prob(20)) //It might land!
				...
				if (prob(30)) //It landed cleanly!
					I.set_loc(src)
					visible_message("<span style=\"color:red\">\The [I] lands cleanly in \the [src]!</span>")
				else	//Aaaa the tension!
					visible_message("<span style=\"color:red\">\The [I] teeters on the edge of \the [src]!</span>")
					var/delay = rand(5, 15)
					spawn (0)
						var/in_x = I.pixel_x
					...
					if (I && I.loc == loc)
						if (prob(40)) //It goes in!
							visible_message("<span style=\"color:red\">\The [I] slips into \the [src]!</span>")
							I.set_loc(src)
						else
							visible_message("<span style=\"color:red\">\The [I] slips off of the edge of \the [src]!</span>")

		else if (ishuman(MO))
			var/mob/living/carbon/human/H = MO
			H.set_loc(get_turf(src))
			if (prob(30))
				H.visible_message("<span style=\"color:red\"><strong>[H] falls into the disposal outlet!</strong></span>")
				logTheThing("combat", H, null, "is thrown into a [name] at [log_loc(src)].")
				H.set_loc(src)
				if (prob(20))
					src.visible_message("<span style=\"color:red\"><strong><em>...accidentally hitting the handle!</em></strong></span>")
					H.show_text("<strong><em>...accidentally hitting the handle!</em></strong>", "red")
					flush = 1
					update()
		else
			return ..()
	...
	proc/interact(mob/user, var/ai=0)
		...
		var/dat = "<head><title>Waste Disposal Unit</title></head><body><TT><strong>Waste Disposal Unit</strong><HR>"

		if (!ai)  // AI can't pull flush handle
			if (flush)
				dat += "Disposal handle: <A href='?src=\ref[src];handle=0'>Disengage</A> <strong>Engaged</strong>"
			else
				dat += "Disposal handle: <strong>Disengaged</strong> <A href='?src=\ref[src];handle=1'>Engage</A>"

			dat += "<BR><HR><A href='?src=\ref[src];eject=1'>Eject contents</A><HR>"

		if (mode == 0)
			dat += "Pump: <strong>Off</strong> <A href='?src=\ref[src];pump=1'>On</A><BR>"
		else if (mode == 1)
			dat += "Pump: <A href='?src=\ref[src];pump=0'>Off</A> <strong>On</strong> (pressurizing)<BR>"
		else
			dat += "Pump: <A href='?src=\ref[src];pump=0'>Off</A> <strong>On</strong> (idle)<BR>"
	...
	Topic(href, href_list)
		...
		if (in_range(src, usr) && isturf(loc))
			...
			if (!isturf(loc))
				DEBUG_MESSAGE("[src]'s loc is not a turf: [loc]")
			if (!in_range(src, usr))
	...
	proc/update()
		...
		if (flush)
			if (!handle_image)
				handle_image = image(icon, "[icon_style]-over-handle")
			else if (!handle_state)
				handle_image.icon_state = "[icon_style]-over-handle"
			UpdateOverlays(handle_image, "handle")
		else
			if (handle_state)
				if (!handle_image)
					handle_image = image(icon, handle_state)
				else
					handle_image.icon_state = handle_state
				UpdateOverlays(handle_image, "handle")
			else
	...
	process()
		...
		var/gas_mixture/env = L.return_air()
		if (!air_contents)
			air_contents = unpool(/gas_mixture)
		...
		if (env.temperature > 0)
			...
			var/gas_mixture/removed = env.remove(transfer_moles)
	...
	suicide(var/mob/user as mob)
		if (mode != 2 || !hasvar(user,"organHolder")) return FALSE

		user.visible_message("<span style=\"color:red\"><strong>[user] sticks \his head into the [name] and pulls the flush!</strong></span>")
