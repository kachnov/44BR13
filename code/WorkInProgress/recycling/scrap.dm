...
/obj/item/scrap/CanPass(var/obj/item/scrap/O)
	...
	if (istype(O))
		...
		add_scrap(O)
//		if (O)
//			return FALSE		// O still exists if not all could be transfered, so block it
	return TRUE
...
/*/obj/item/scrap/attack_hand(mob/user)
	...
	if (is_single_piece())
		return ..(user)
	var/obj/item/scrap/S = get_single_piece()
	S.attack_hand(user)
	return
*/
...
/obj/item/scrap/attackby(obj/item/I, mob/user)
	if (istype(I, /obj/item/scrap))
		...
		if ( (S.total()+total() ) > MAX_SCRAP )
			boutput(user, "The pile is full.")
			return
		if (ismob(loc))		// can't combine scrap in hand
			return
...
/obj/item/scrap/proc/get_single_piece()
	...
	switch(cmp)
		if (1)
			...
			set_components(m_amt - amount, g_amt, w_amt)

		if (2)
			if (g_amt < amount)
				amount = g_amt
			S.set_components(0, amount, 0)
			set_components(m_amt, g_amt - amount, w_amt)

		if (3)
			if (w_amt < amount)
				amount = w_amt
			S.set_components(0, 0, amount)
			set_components(m_amt, g_amt, w_amt - amount)
