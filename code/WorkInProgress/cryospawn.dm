...
/obj/cryotron
	...
	disposing()
		...
		for (var/mob/M in src)
			...
			if (isliving(M))
				var/mob/living/L = M
				L.hibernating = 0
				L.removeOverlayComposition(/overlayComposition/blinded)
	...
	proc/add_person_to_queue(var/mob/living/person)
		...
		boutput(person, "<strong>Cryo-recovery process initiated.  Please wait . . .</strong>")
		person.removeOverlayComposition(/overlayComposition/blinded)
		return TRUE
	...
	//return TRUE if there is another person to spawn afterward
	proc/spawn_next_person()
		...
		spawn (19)
			if (!thePerson)
				busy = 0
				return (folks_to_spawn.len != 0)
			var/turf/firstLoc = locate(x + 1, y, z)
	...
	proc/add_person_to_storage(var/mob/living/L as mob)
		...
		if (stored_mobs.Find(L))
			if (L.loc == src)
				...
				L.hibernating = 1
				if (L.client)
					L.addOverlayComposition(/overlayComposition/blinded)
	...
	proc/mob_can_enter_storage(var/mob/living/L as mob)
		if (!ticker)
			boutput(L, "<strong>You can't enter cryogenic storage before the game's started!</strong>")
			return FALSE
		if (!istype(L) || L.stat == 2)
			boutput(L, "<strong>You have to be alive to enter cryogenic storage!</strong>")
			return FALSE
		if (L.stat || L.restrained() || L.paralysis || L.sleeping)
			boutput(L, "<strong>You can't enter cryogenic storage while incapacitated!</strong>")
			return FALSE
		if (get_dist(src, L) > 1)
			boutput(L, "<strong>You need to be closer to [src] to enter cryogenic storage!</strong>")
			return FALSE
		return TRUE
	...
	proc/exit_prompt(var/mob/living/user as mob)
		...
		if ((entered + CRYOSLEEP_DELAY) > world.timeofday) // is the time entered plus 15 minutes greater than the current time? the mob hasn't waited long enough
			...
			if (time_left >= 0)
				boutput(user, "<strong>You must wait [time_left] minute[s_es(time_left)] before you can leave cryosleep.</strong>")
