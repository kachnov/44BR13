...
/materialProc/wendigo_temp_onlife
	id = "wendigo_temp"
	desc = "It feels furry."
...
/materialProc/generic_radiation_onenter
...
/materialProc/generic_reagent_onattacked
...
/materialProc/generic_explode_attack
...
/materialProc/generic_fireflash
...
/materialProc/generic_itchy_onlife
...
/materialProc/generic_reagent_onattack_depleting
...
/materialProc/generic_reagent_onattack
...
/materialProc/generic_reagent_onlife
...
/materialProc/generic_reagent_onlife_depleting
...
/materialProc/generic_explosive
...
/materialProc/gold_add
	id = "gold_add"
	desc = "It's very shiny."

	execute(var/location)
		if (!particleMaster.CheckSystemExists(/particleSystem/sparkles, location))
			particleMaster.SpawnSystem(new /particleSystem/sparkles(location))
		return

/materialProc/telecrystal_entered
	...
	execute(var/location) //exp and temp both have the location as first argument so i can use this for both.
		for (var/turf/simulated/floor/target in range(1,location))
			if (!target.blocks_air && target.air)
				...
				var/gas_mixture/payload = unpool(/gas_mixture)
				payload.toxins = 100
				target.air.merge(payload)
		return

/materialProc/miracle_add
	id = "miracle_add"
