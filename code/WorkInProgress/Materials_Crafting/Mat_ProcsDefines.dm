...
/atom/var/material/material = null
...
/proc/mergeProperties(var/list/l1, var/list/l2, var/bias=0.5)
	...
	for (var/o in l1)
		merged.Add(o)
		var/material_property/Base = l1[o]
		var/material_property/newProp = new/material_property()
		merged[o] = newProp
		newProp.name = o
	...
	if (l2)
		for (var/x in l2)
			if (merged.Find(x))
				var/material_property/Base = l2[x]
				var/material_property/Modded = merged[x]
...
/proc/copyMaterial(var/material/base)
	if (!base || !istype(base, /material))
		var/material/M = new/material()
		return M
	else
		var/material/M = new base.type ()
...
/atom/proc/removeMaterial()
	...
	UpdateOverlays(null, "material")

	material = null
	return

//Time for some super verbose proc names.
/proc/get_material_trait_desc(var/material/mat1)
	var/string = ""
	...
	for (var/materialProc/P in allTriggers)
		if (length(P.desc))
			if (length(string))
...
/atom/proc/setMaterial(var/material/mat1)
	if (!mat1 ||!istype(mat1, /material)) return
	...
	if (mat_changename) // now that I've cleaned up the name prefix/suffix stuff, let's try this, in the hopes that eveything that SHOULD override UpdateName() actually DOES - Haine
		remove_prefixes(99) // janky but better than ending up with uhh
		remove_suffixes(99) // "This is a good gold good gold good gold good gold good gold good gold good gold good gold good gold good gold bedsheet."
		name_prefix(strPrefix ? strPrefix : "")
		name_prefix(length(getQualityName(mat1.quality)) ? getQualityName(mat1.quality) : "")
		name_prefix(mat1.name ? mat1.name : "")
		name_suffix(strSuffix ? "of [strSuffix]" : "")
		UpdateName()
/*		if (istype(src, /obj))
			var/obj/O = src
			...
			name = "[strPrefix ? strPrefix + " ":""][length(getQualityName(mat1.quality)) ? getQualityName(mat1.quality) + " ":""][mat1.name ? mat1.name + " ":""][initial(name)][strSuffix ? (" of " + strSuffix) : ""]"
*/
	if (mat_changedesc)
		if (istype(src, /obj))
			var/obj/O2 = src
			...
			desc = "[initial(desc)] It is made of [mat1.name].[length(traitDesc) ? " " + traitDesc : ""]"
	if (mat1.texture)
		setTexture(mat1.texture, mat1.texture_blend, "material")
	alpha = mat1.alpha
	color = mat1.color
	material = mat1
	mat1.owner = src
...
/proc/getProcessedMaterialForm(var/material/MAT)
	if (!istype(MAT))
...
/proc/handleTriggerGenerations(var/list/toDo)
	for (var/materialProc/current in toDo)
...
/proc/getFusedTriggers(var/list/L1 , var/list/L2)
	...
	for (var/materialProc/toCopy in L1) //Copy list 1 with new instances of trigger datum.
		var/materialProc/P = new toCopy.type()
	...
	for (var/materialProc/A in L2) //Go through second list
		if ((locate(A.type) in newList))	//We already have that trigger type from the other list
			var/materialProc/existing = (locate(A.type) in newList) //Get the trigger datum from the other list
			...
			var/materialProc/newProc = new A.type()	//Create a new instance
...
/proc/getFusedMaterial(var/material/mat1,var/material/mat2)
	//Merges two materials and returns result as new material.
	return getInterpolatedMaterial(mat1, mat2, 0.5)

/proc/getInterpolatedMaterial(var/material/mat1,var/material/mat2,var/t)
	var/ot = 1 - t
	var/material/newMat = new()
...
/proc/matchesMaterialRecipe(var/material/M)
	for (var/material_recipe/R in materialRecipes)
...
/proc/hasParentMaterial(var/material/M, var/search_id, var/max_generations = 3)
	return searchMatTree(M, search_id, 0, max_generations)

/proc/searchMatTree(var/material/M, var/id, var/current_depth, var/max_depth = 3)
	if (!M || !id) return null
...
/proc/applyCableMaterials(var/atom/C, var/material/insulator, var/material/conductor)
