/obj/machinery/computer/material
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (W.material != null)
			...
			if (!editing)
				visible_message("<span style=\"color:blue\">[user] puts [W] into [src]</span>")
				user.drop_item()
				editing = W.material
				editing_obj = W
				W.loc = src
	...
	proc/handleRecipes()
		...
		var/material_recipe/RE = matchesMaterialRecipe(editing)
		if (RE)
			if (RE.result_item)
				var/atom/movable/A = new RE.result_item(loc)
				A.set_loc(src)
				editing = null
				del(editing_obj)
				editing_obj = A
			...
			switch(href_list["jscall"])
				...
				if ("selMod")
					...
					switch(href_list["name"])
						...
						if ("Wendigo King thermal protection")
							...
							editing.addDelegate(editing.triggersOnLife, new /materialProc/wendigo_temp_onlife())
							editing_obj.setMaterial(editing_obj.material)
							handleRecipes()
							newhtml += "<th>AFTER<br>[generateMaterialInfo()]</th>" //After
							newhtml += "</tr></table>"
						...
						if ("Supernatural enhancement")
							...
							editing.addDelegate(editing.triggersOnAdd, new /materialProc/ethereal_add())
				...
				if ("research")
					if (href_list["id"] && materialsResearch.research.Find(href_list["id"]))
						var/materialResearch/R = materialsResearch.research[href_list["id"]]
		...
		switch(content)
			...
			if ("Research")
				...
				for (var/x in materialsResearch.research)
					var/materialResearch/R = materialsResearch.research[x]
					...
					if (istype(R, /materialResearch/scanner))
						newhtml += "<br>Required Scans:<br>"
						var/linkcount = 0
						for (var/materialResearchScan/S in R:requiredScans)
...
/obj/machinery/smelter_portable
	...
	attack_hand(mob/user as mob)
		if (output)
			var/material_recipe/R = matchesMaterialRecipe(output)
			if (R)
				if (R.result_item)
					var/atom/A = new R.result_item(loc)
			...
			var/obj/item/material_piece/M = new bar_type(loc)

			M.add_fingerprint(user) // May not be the same person who smelted the materials (Convair880).
			add_fingerprint(user) // Add some prints to the smelter too.
		...
		if (components.len > 0)
			...
			light.enable()
			playsound(loc, sound_bubble, 40, 1)
			if (components.len == 1)
				...
				var/material/mat1 = obj1.material
				var/material/mat2 = obj2.material
				...
				output.generation++
			logTheThing("station", user, null, "creates a [output] bar (<strong>Material:</strong> <em>[output.mat_id]</em>) with the [src] at [log_loc(src)].") //  Re-added/fixed because of erebite, plasmastone etc. alloys (Convair880).
			spawn (8)
				light.disable()
		else
...
/obj/machinery/loom
	...
	proc/produceMaterial(var/material/O)
		if (!O) return

		var/material_recipe/R = matchesMaterialRecipe(O)
		if (R)
			if (R.result_item)
				new R.result_item(loc)
				resetMats()
				return
			else if (length(R.result_id))
				O = getCachedMaterial(R.result_id)

		var/obj/item/material_piece/cloth/M = new/obj/item/material_piece/cloth(loc)
		M.setMaterial(O)
		resetMats()
		if (M.material && prob(M.material.getProperty(PROP_INSTABILITY)))
			M.visible_message("<span style=\"color:red\">[M] [getMatFailString(M.material.material_flags)]!</span>")
	...
	attack_hand(mob/user as mob)
		...
		if (components.len > 0)
			if (components.len == 1)
				...
				playsound(loc, "sound/machines/loom.ogg", 35, 0)
				active = 1
				sleep(25)
				active = 0
				var/atom/obj1 = components[1]
				...
				var/material/output = getFusedMaterial(mat1, mat2)
				output.generation++
...
/obj/machinery/smelter
	...
	attack_hand(mob/user as mob)
		if (output)
			...
			if (R)
				if (R.result_item)
					var/atom/A = new R.result_item(locate(x + 1, y, z))
					boutput(user, "<span style=\"color:blue\">You remove [A.name] from the [src].</span>")
					playsound(loc, sound_thunk, 40, 1)
					resetMats()
					return
				else if (length(R.result_id))
					output = getCachedMaterial(R.result_id)
			...
			var/bar_type = getProcessedMaterialForm(output)
			var/obj/item/material_piece/M = new bar_type(locate(x + 1, y, z))
		...
		if (components.len > 0)
			...
			if (components.len == 1)
				...
				output = getFusedMaterial(mat1, mat2)
				output.generation++
				logTheThing("station", user, null, "creates a [output] bar (<strong>Material:</strong> <em>[output.mat_id]</em>) with the [src] at [log_loc(src)].") // Sorry for code duplication, but I'm regularly seeing runtime errors for some reason if this proc is called after handleSlag (Convair880).
				handleSlag()
			spawn (8)
				playsound(loc, sound_hiss, 45, 1)
				light.disable()
		else
			boutput(user, "<span style=\"color:red\">There is nothing in the [src].</span>")
		return
	...
	proc/handleSlag()
		switch(slag_level)
			if (500 to 1000)
				particleMaster.SpawnSystem(new /particleSystem/localSmoke("#967360", 10, locate(x +1, y, z)))
				//output.adjustProperty(PROP_INSTABILITY , 5)
				output.quality += rand(5, -15)
			if (1000 to INFINITY)
				particleMaster.SpawnSystem(new /particleSystem/localSmoke("#221511", 10, locate(x +1, y, z)))
	...
	suicide(var/mob/user)
		...
		user.visible_message("<span style=\"color:red\"><strong>[user] hops right into [src]! Jesus!</strong></span>")
		user.unequip_all()
		user.set_loc(src)


		var/material/M = new /material/organic/flesh {desc="A disgusting wad of flesh."; color="#881111";} ()
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/slag_shovel))
			if (slag_level)
				visible_message("<span style=\"color:blue\">[user] removes slag from the [src]</span>")
				slag_level = 0
				new/obj/item/material_piece/slag(loc)
				return
			else
