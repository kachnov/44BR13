...
/obj/workbench_ui/menubutton
	...
	var/modular_recipe/recipe = null
	New(var/modular_recipe/C)
		recipe = C
		return
...
/obj/workbench_ui/slotbutton
	...
	desc = "flerpderp"
	icon = 'icons/misc/ManuUI.dmi'
	var/modular_component/component = null

	New(var/modular_component/C)
		component = C
		return
...
/modular_recipe
	...
	New(var/obj/workbench/w)
		...
		for (var/modular_component/C in recipe_parts)
...
/obj/workbench
	...
	var/modular_recipe/active_recipe = null
	...
	Del()
		active_recipe = null
		for (var/modular_recipe/R in recipe_list)
			R.unload()
			for (var/modular_component/C in R.recipe_parts)
	...
	New()
		if (!(src in processing_items))
			processing_items.Add(src)
		var/list/types = subtypesof(/modular_recipe)
		for (var/r in types)
		...
		if (active_recipe)
			...
			var/html = {"<!DOCTYPE html>
				<html>
					...
					<body id="manubody">"}
			html += {"<div id="manudiv">"}
			for (var/modular_recipe/x in recipe_list)
	...
	Topic(href, href_list)
		...
		if (href_list["loadrecipe"])
			...
			var/modular_recipe/sel = locate(href_list["loadrecipe"])
			if (sel && istype(sel, /modular_recipe))
				load_recipe(sel)
				if (!users.Find(usr)) users.Add(usr)
				show_to(usr)
				usr << browse(null, "window=modmanu2")
	...
	proc/unload_recipe()
		if (active_recipe)
			hide_from_all()
			users.Cut()
			for (var/modular_component/C in active_recipe.recipe_parts)
	...
	proc/load_recipe(var/modular_recipe/R)
		active_recipe = R
		return
	...
	proc/build_recipe()
		if (active_recipe)
			...
			if (fail)
				...
				M.set_loc(loc)
				for (var/modular_component/CB in active_recipe.recipe_parts)
					if (CB.slot_object)
