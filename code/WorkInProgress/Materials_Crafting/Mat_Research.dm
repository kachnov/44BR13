var/materialResearchHolder/materialsResearch = null

/materialResearchHolder
	...
	proc/setup()
		var/types = (subtypesof(/materialResearch)) - /materialResearch/scanner
		for (var/x in types)
			var/materialResearch/R = new x(src)
			research.Add(R.id)
			research[R.id] = R
		return
...
/obj/machinery/matscanner
	...
	attackby(obj/item/W as obj, mob/user as mob)
		...
		playsound(loc, "sound/machines/ArtifactPre1.ogg", 50, 0)
		...
		for (var/x in materialsResearch.research)
			...
			if (istype(R, /materialResearch/scanner))
				var/materialResearch/scanner/R2 = R
				for (var/materialResearchScan/S in R2.requiredScans)
...
/materialResearch
	...
	proc/beginResearch()
		...
		return FALSE //Again, 0 means everything is good.
	...
	proc/canStart()
		...
		return FALSE //0 means "yes, we can start this". This is so we can return a string when we CAN'T.
...
/materialResearchScan/wendigoHide
	name = "Wendigo Hide"
...
/materialResearchScan/wendigoKingHide
...
/materialResearchScan/iridiumAlloy
	name = "Iridium alloy"
...
/materialResearch/supernatural1
...
/materialResearch/supernatural2
	...
	canShow()
		if (!..()) return FALSE
		if (materialsResearch.completed.Find("supernatural1")) return TRUE
		return FALSE

/materialResearch/efficientresearch
...
/materialResearch/fastresearch
	...
	canShow()
		if (!..()) return FALSE
		if (materialsResearch.completed.Find("effresearch")) return TRUE
		return FALSE
	...
	onComplete()
		...
		return
/*
/materialResearch/reconditioning1
	...
	canShow()
		if (!..()) return FALSE
		if (materialsResearch.completed.Find("fstresearch")) return TRUE
		return FALSE

/materialResearch/scanner/reconditioning2
	...
	New()
		requiredScans.Add(new/materialResearchScan/iridiumAlloy(src))

	canShow()
		if (!..()) return FALSE
		if (materialsResearch.completed.Find("recondition1")) return TRUE
		return FALSE
*/

/materialResearch/scanner/wendigo
	...
	New()
		requiredScans.Add(new/materialResearchScan/wendigoHide(src))

/materialResearch/scanner/wendigoking
	...
	canShow()
		if (!..()) return FALSE
		if (materialsResearch.completed.Find("wendigo")) return TRUE
		return FALSE

	New()
		requiredScans.Add(new/materialResearchScan/wendigoKingHide(src))

/materialResearch/scanner //This subtype of research required certain scanned things to allow research.
	var/list/requiredScans = list()

	canStart()
		if (!..())
			var/completeCount = 0
			for (var/materialResearchScan/x in requiredScans)
