...
/client/proc/wireTest()
	...
	boutput(world, "hi")
	//chui.reload()
	//Browse(grabResource("html/wireTest.html"), "window=wireTest;size=500x650;title=Wire Test;")
	//mob.deathConfetti()
...
/proc/queryAPI(route, query, forceResponse = 0)
	...
	if (centralConn)
		...
		if (!response)
			logTheThing("debug", null, null, "<strong>API Error</strong>: No response from server during query to <strong>[req]</strong>. Try count is: <strong>[centralConnTries]</strong>")
			...
			//	centralConn = 0
			//	logTheThing("debug", null, null, "<strong>Critical API Error</strong>: <strong>Max remote API tries exceeded, switching to local fallback system.</strong>")
		...
		if (forceResponse)
			...
			if (!contentExists)
				logTheThing("debug", null, null, "<strong>API Error</strong>: Malformed response from server during <strong>[req]</strong>")
	...
	if (!theProc)
		...
		logTheThing("debug", null, null, "<strong>Local API Error</strong> - No proc specified for route: <strong>[route]</strong>")
		logTheThing("diary", null, null, "<strong>Local API Error</strong> - No proc specified for route: [route]", "debug")
		*/
		return (centralConn ? data : 0)

	var/localData = call(theProc)(query)
	if (!localData)
		//logTheThing("debug", null, null, "<strong>Local API Error</strong> - Nothing returned from <strong>[theProc]</strong>")
		//logTheThing("diary", null, null, "<strong>Local API Error</strong> - Nothing returned from [theProc]", "debug")
		return (centralConn ? data : 0)

	if (istype(localData, /list))
		var/list/ldata = localData
		if (ldata["error"])
			//logTheThing("debug", null, null, "<strong>Local API Error</strong> - Callback failed in <strong>[theProc]</strong> with message: <strong>[ldata["error"]]</strong>")
			//logTheThing("diary", null, null, "<strong>Local API Error</strong> - Callback failed in [theProc] with message: [ldata["error"]]", "debug")
			if (ldata["showAdmins"])
				message_admins("<span style=\"color:blue\"><strong>Failed for route [route]BanApiFallback</strong>: [ldata["error"]]</span>")
...
/mob/proc/deathConfetti()
	particleMaster.SpawnSystem(new /particleSystem/confetti(loc))
	spawn (10)
		playsound(loc, "sound/effects/yayyy.ogg", 50, 1)
