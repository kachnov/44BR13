...
/obj/infared_icon/examine()
	set src in view()
	if (usr.see_infrared)
		usr << browse(text("<HTML><HEAD><TITLE>[]</TITLE></HEAD><BODY><TT>[]</TT></BODY></HTML>", name, info), text("window=[]", name))
		onclose(usr, "[name]")
	return
...
//I still need a way for the AI to actually make these
	...
	switch(select)
		if ("Add")
			var/t = input(user, "What text do you wish to add?", text("[]", name), null)  as message
		...
		if ("View")
			if (usr.see_infrared)
				user << browse(text("<HTML><HEAD><TITLE>[]</TITLE></HEAD><BODY><TT>[]</TT></BODY></HTML>", name, info), text("window=[]", name))
				onclose(usr, "[name]")
	return
