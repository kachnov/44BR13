...
/obj/machinery/cruiser
	...
	var/particleSystem/barrelSmoke/smokeParticles
	var/list/crew = list()
	...
	bullet_act(var/obj/projectile/P)
		...
		if (shields)
			internal_sound(loc, 'sound/machines/shieldimpact.ogg', 65, 1, 1)
			damageShields(damage, PD.damage_type)
		else
			internal_sound(loc, 'sound/machines/armorimpact.ogg', 65, 1, 1)
	...
	ex_act(var/severity)
		...
		if (shields <= 0 && prob(75))
			var/atom/source = pick(interior_area.contents)
	...
	New()
		...
		frames = image('icons/obj/160x160.dmi',src,"frames",layer+1)
		overframes = image('icons/obj/160x160.dmi',src,"overframes",layer+2)
		bar_top = image('icons/obj/160x160.dmi',src,"bartop",layer+1)
		bar_middle = image('icons/obj/160x160.dmi',src,"barmiddle",layer+1)
		bar_bottom = image('icons/obj/160x160.dmi',src,"barbottom",layer+1)

		bar_top.color = "#8A1919"
		bar_middle.color = "#19688A"
		bar_bottom.color = "#CF9417"

		camera = new(locate(x + 2, y + 2, z))
		camera.name = name
	...
	Move(NewLoc,Dir=0,step_x=0,step_y=0)
		var/preserve_dir = dir
		. = ..(NewLoc,Dir,step_x,step_y)
		dir = preserve_dir
		camera.set_loc(locate(x + 2, y + 2, z))
		shield_obj.set_loc(loc)
		return
	...
	proc/handleLifeSupport()
		if (hasPower() && life_support)
			...
			if ((atmos_fail_count - 1) == 2)
				internal_sound(loc, 'sound/machines/alarm_a.ogg', 80, 1, -1)
			if ((atmos_fail_count - 1) == 0)
				internal_sound(loc, 'sound/machines/decompress.ogg', 100, 1, -1)
	...
	Bump(atom/O)
		...
		if (ramming)
			...
			if (istype(O, /atom/movable))
				if (!O:anchored)
					step(O,dir)
			internal_sound(loc, 'sound/machines/armorimpact.ogg', 80, 1, -1)
	...
	proc/enableRamming()
		addPowerUse("rammingMode", 90, -1)
		internal_sound(loc, 'sound/machines/boost.ogg', 100, 1, -1)
		speed_mod -= 2
		ramming += 8
		spawn (100)
			speed_mod += 2
			removePowerUse("rammingMode")
			ramming = max(ramming - 8, 0)
			walk(src, 0)
			flying = 0
		return

	proc/overload_weapons()
		addPowerUse("weaponOverload", 90, -1)
		internal_sound(loc, 'sound/machines/weaponoverload.ogg', 80, 1, -1)
		weapon_cooldown_mod -= 3
		spawn (100)
			weapon_cooldown_mod += 3
			removePowerUse("weaponOverload")
		return

	proc/overload_shields()
		addPowerUse("shieldOverload", 90, -1)
		shield_regen_always += 1
		shield_regen_boost += 10
		var/image/I = image('icons/obj/160x160.dmi',shield_obj,"shieldoverload",shield_obj.layer+1)
		I.alpha = 150
		shield_obj.overlays += I
		internal_sound(loc, 'sound/machines/shieldoverload.ogg', 80, 0, -1)
		spawn (150)
			removePowerUse("shieldOverload")
			shield_regen_always -= 1
			shield_regen_boost -= 5
			shield_obj.overlays.Cut()
		return

	proc/toggleShieldModulation()
		shield_modulation = !shield_modulation
		internal_sound(loc, 'sound/machines/ArtifactAnc1.ogg', 65, 1, -1)

		if (shield_modulation)
			addPowerUse("shieldModulation", 90, -1)
			shield_obj.color = "#FF0000"
		else
			removePowerUse("shieldModulation")
	...
	proc/handlePowerCosts()
		...
		if (engine)
			if (flying) addPowerUse("thrusters", 10, 1)
		...
		if (hasPower() && shields < shields_max && ((world.time - shields_timer) >= shields_delay || shield_regen_always))
			addPowerUse("shieldRecharge", shields_recharge_cost, 1)
			adjustShields(shields_regen + shield_regen_boost)
	...
	proc/hasPower()
		if (power_used_last > power_produced_last || health <= 0) return FALSE
		else return TRUE
		return
	...
	proc/adjustShields(var/amount, var/type = D_SPECIAL)
		...
		if (shields_last > 0 && shields <= 0) //Collapse
			if (shield_obj.icon_state != "shield_collapse")
				internal_sound(loc, 'sound/machines/shielddown.ogg', 100, 1, -1)
			...
			if (shield_obj.icon_state != "shield_reboot")
				internal_sound(loc, 'sound/machines/shieldup.ogg', 100, 1, -1)
				shield_obj.alpha = 170
	...
	proc/warp()
		warping = 1
		internal_sound(loc, 'sound/machines/cruiser_warp.ogg', 80, 0, 1)
	...
	proc/checkHealth()
		...
		if (health_last <= 0 && health)
			particleMaster.RemoveSystem(/particleSystem/cruiserSmoke, src)
	...
	proc/destroy()
		particleMaster.SpawnSystem(new /particleSystem/cruiserSmoke(src))
		return
	...
	proc/getProjectileOrigins()
		...
		switch(dir)
			if (NORTH)
				return list("left"=locate(x, y + 5, z), "right"=locate(x + 4, y + 5, z))
			if (EAST)
				return list("left"=locate(x + 5, y + 4, z), "right"=locate(x + 5, y, z))
			if (SOUTH)
				return list("left"=locate(x + 4, y - 1, z), "right"=locate(x, y - 1, z))
			if (WEST)
				return list("left"=locate(x - 1, y, z), "right"=locate(x - 1, y + 4, z))
		*/
		switch(dir) //Inner 2 version
			if (NORTH)
				return list("left"=locate(x + 1, y + 5, z), "right"=locate(x + 3, y + 5, z))
			if (EAST)
				return list("left"=locate(x + 5, y + 3, z), "right"=locate(x + 5, y + 1, z))
			if (SOUTH)
				return list("left"=locate(x + 3, y - 1, z), "right"=locate(x + 1, y - 1, z))
			if (WEST)
				return list("left"=locate(x - 1, y + 1, z), "right"=locate(x - 1, y + 3, z))
	...
	proc/fireAt(var/atom/target)
		...
		if (turret_left)
			if (get_dir(origins["left"], target) != dir && get_dir(origins["left"], target) != turn(dir,45) && get_dir(origins["left"], target) != turn(dir,-45))
				//internal_sound(loc, 'sound/machines/shielddown.ogg', 100, 1, -1)
			else
				if (!(firemode & CRUISER_FIREMODE_RIGHT))
					if ((firemode & CRUISER_FIREMODE_ALT && alt_weapon == 0) || !(firemode & CRUISER_FIREMODE_ALT))
						...
						if (turret_left.current_projectile.shot_sound)
							internal_sound(loc, turret_left.current_projectile.shot_sound, 80, 1, 1)

		if (turret_right)
			if (get_dir(origins["right"], target) != dir && get_dir(origins["right"], target) != turn(dir,45) && get_dir(origins["right"], target) != turn(dir,-45))
				//internal_sound(loc, 'sound/machines/shielddown.ogg', 100, 1, -1)
			else
				if (!(firemode & CRUISER_FIREMODE_LEFT))
					if ((firemode & CRUISER_FIREMODE_ALT && alt_weapon == 1) || !(firemode & CRUISER_FIREMODE_ALT))
						...
						if (turret_right.current_projectile.shot_sound)
							internal_sound(loc, turret_right.current_projectile.shot_sound, 80, 1, 1)
	...
	proc/getExitLoc()
		switch(dir)
			...
			if (SOUTH)
				return locate(x + 2, y + 5, z)
			if (WEST)
				return locate(x + 5, y + 2, z)
...
/obj/machinery/cruiser_status_panel
	...
	New()
		barTop = image('icons/obj/ship.dmi',src,"statpanel1",layer+1)
		barTop.color = "#8A1919"

		barMid = image('icons/obj/ship.dmi',src,"statpanel2",layer+1)
		barMid.color = "#19688A"

		barBot = image('icons/obj/ship.dmi',src,"statpanel3",layer+1)
		barBot.color = "#CF9417"
...
/obj/machinery/cruiser_destroyable
	...
	attackby(obj/item/W as obj, mob/user as mob)
		...
		if (istype(W, tool_type) && (broken || health < health_max))
			playsound(loc, "sound/machines/repairing.ogg", 85, 1)
			var/health_adj = 1 - (health / health_max) //90% = 0,1, 10% = 0,9
			var/repair_time_adj = round(repair_time * health_adj)
			actions.start(new/action/bar/icon/cruiser_repair(src, W, repair_time_adj), user)
			return TRUE
		return FALSE
...
/obj/machinery/cruiser_destroyable/cruiser_component_slot
	...
	setIcon()
		if (broken)
			setTexture("damaged", BLEND_MULTIPLY, "damaged")
		else
	...
	proc/open(var/mob/user = null, var/ignore_blocked = 0)
		...
		if (check_blocked && blocked && !ignore_blocked)
			...
			ready = 0
			spawn (10) ready = 1
			playsound(loc, 'sound/machines/hydraulic.ogg', 50, 0, -1)
			open = 1
			setIcon()
			uninstall_component()
			density = 1
	...
	proc/close(var/mob/user = null)
		...
		ready = 0
		spawn (10) ready = 1
		playsound(loc, 'sound/machines/weapons-deploy.ogg', 60, 0, -1)
		open = 0
		setIcon()
		install_component()
		density = 0
...
/obj/machinery/cruiser_destroyable/cruiser_component_slot/weapon/left
	...
	uninstall_component()
		...
		if (interior && interior.ship && interior.ship.turret_left)
			interior.ship.turret_left.set_loc(loc)
...
/obj/machinery/cruiser_destroyable/cruiser_component_slot/weapon/right
	...
	uninstall_component()
		...
		if (interior && interior.ship && interior.ship.turret_right)
			interior.ship.turret_right.set_loc(loc)
...
/obj/machinery/cruiser_destroyable/cruiser_component_slot/engine
	...
	uninstall_component()
		...
		if (interior && interior.ship && interior.ship.engine)
			interior.ship.engine.set_loc(loc)
...
/obj/machinery/cruiser_destroyable/cruiser_component_slot/life_support
	...
	uninstall_component()
		...
		if (interior && interior.ship && interior.ship.life_support)
			interior.ship.life_support.set_loc(loc)
...
/obj/machinery/cruiser_camera_screen
	...
	attack_hand(mob/user as mob)
		if (istype(user.abilityHolder, /abilityHolder/composite))
			var/abilityHolder/composite/C = user.abilityHolder
			C.addHolder(/abilityHolder/cruiser)
			C.addAbility(/targetable/cruiser/cancel_camera)
			user.client.view = 11
			var/area/ship_interior/I = get_area(src)
			user.set_eye(I.ship)
		return
...
/obj/machinery/cruiser_destroyable/cruiser_pod
	...
	var/abilityHolder/cruiser/AbHolder
	...
	New()
		...
		AbHolder = new()
		AbHolder.addAbility(/targetable/cruiser/exit_pod)
	...
	proc/enterPod(mob/user as mob)
		...
		user.client.view = 11
		if (ishuman(user) && istype(user.abilityHolder, /abilityHolder/composite))
			var/abilityHolder/composite/H = user.abilityHolder
	...
	proc/exitPod()
		...
		if (ishuman(using) && istype(using.abilityHolder, /abilityHolder/composite))
			using.targeting_spell = null
			using.update_cursor()
			var/abilityHolder/composite/H = using.abilityHolder
			AbHolder.suspendAllAbilities()
			H.removeHolder(/abilityHolder/cruiser)
...
/obj/machinery/cruiser_destroyable/cruiser_pod/movement
	...
	abilities = list(/targetable/cruiser/ram)
...
/obj/machinery/cruiser_destroyable/cruiser_pod/security
	...
	abilities = list(/targetable/cruiser/fire_weapons, /targetable/cruiser/firemode, /targetable/cruiser/weapon_overload)
...
/obj/machinery/cruiser_destroyable/cruiser_pod/engineering
	...
	abilities = list(/targetable/cruiser/warp, /targetable/cruiser/shield_overload, /targetable/cruiser/shield_modulation)
