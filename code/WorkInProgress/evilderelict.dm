	...
	proc/process()
		if (target)
			if (get_dist(src, target) <= 1)
				if (prob(40))
					visible_message("<span style=\"color:red\"><strong>[src] slices through [target.name] with the axe!</strong></span>")
					...
					visible_message("<span style=\"color:red\"><strong>[src] swings at [target.name] with the axe!</strong></span>")
			...
			if (isliving(AM))
				if (AM:client)
					if (prob(75))
						...
						spawn (rand(10,30))
							var/obj/chaser/maniac/C = new /obj/chaser/maniac(loc)
							C.target = AM
		...
		if (!alert)
			if (iscarbon(AM))
				...
				if (id == "evilreaverbridge")
					playsound(loc, 'sound/machines/driveclick.ogg', 50, 1)
					var/obj/item/paper/PA = new /obj/item/paper(loc)
