...
/obj/item/soulskull
	...
	attack_hand(mob/M)
		if (!being_mean)
			..()
			M.show_text("<strong><em>It burns...!</em></strong>", "red")
...
*/
	proc/evil_act(mob/living/carbon/human/H)
		...
		if (ticker && ticker.mode) //Yes, I'm sure my runtimes will matter if the goddamn TICKER is gone.
			for (var/mind/M in (ticker.mode.Agimmicks | ticker.mode.traitors)) //We want an EVIL ghost
		...
		playsound(loc, 'sound/ambience/voidfx4.ogg', 40, 1)
		...
		spawn (70)
			...
			H.emote("faint")
			H.paralysis = 10
			H.show_text("<em><font size=5>You feel your mind drifting away from your body!</font></em>", "red")
			...
			if (!H.mind)
				...
				if (priority_targets.len) //Do we have an evil ghost?
					...
					H.show_text("<em><strong>You hear a sinister voice in your head... \"I have brought you back to do evil once more!\"</strong></em>")
			...
			spawn (15) playsound(loc, 'sound/effects/ghostlaugh.ogg', 70, 1)
...
/mob/proc/tommyize()
	...
	var/mob/living/carbon/human/tommy/T = new(loc)
	if (mind)
		mind.transfer_to(T)
	else
		T.key = key
...
/mob/living/carbon/human/proc/tommyize_reshape()
	...
	sound_malescream = 'sound/voice/tommy_you-are-tearing-me-apart-lisauh.ogg'
	sound_fingersnap = 'sound/voice/tommy_did-not-hit-hehr.ogg'

	if (bioHolder)
		bioHolder.mobAppearance = AH
...
/obj/item/gun/energy/tommy_gun
	...
	New()
		...
		current_projectile = new/projectile/tommy
		projectiles = list(new/projectile/tommy)
		..()
	...
	shoot(var/target,var/start,var/mob/user,var/POX,var/POY)
		for (var/mob/O in AIviewers(user, null))
			O.show_message("<span style=\"color:red\"><strong>[user] fires the [src] at [target]!</strong></span>", 1, "<span style=\"color:red\">You hear a loud crackling noise.</span>", 2)
		sleep(1)
		return ..(target, start, user)

	update_icon()
		if (cell)
			icon_state = "tommy[cell.charge > 0]"
			return
...
/spectro_analysis
	...
	proc/analyze_reagents(var/reagents/R, var/check_recipes = 0)
	...
	proc/analyze_reagent_components(var/list/reagent_ids)
		if (reagent_ids.len)
			...
			for (var/id in reagent_ids)
				...
				var/chemical_reaction/recipe = chem_reactions_by_id[id]
		...
		if (reagent_ids.len)
			...
			for (var/RID in reagent_ids)
				if (reagents_cache[RID])
					...
					logTheThing("debug", null, null, "<strong>SpyGuy/spectro:</strong> attempted to analyze invalid reagent id: [RID]")
...
/obj/trigger/throw
	...
	on_trigger(var/atom/movable/triggerer)
		...
		var/atom/target = get_edge_target_turf(src, throw_dir)
		spawn (0)
			if (target)
...
////////////////////////////// Donald Trumpet
/projectile/energy_bolt_v/trumpet
	name = "trumpet bolt"
...
/obj/item/gun/energy/dtrumpet
	...
	New()
		cell = new/obj/item/ammo/power_cell/high_power
		current_projectile = new/projectile/energy_bolt_v/trumpet
		projectiles = list(new/projectile/energy_bolt_v/trumpet)
		..()
...
/obj/solar_control/proc/extend_panel()
	...
	DEBUG_MESSAGE("Extending panel at [showCoords(x, y, z)]. extension_dir: [extension_dir] ([dir2text(extension_dir)]), paneldir1: [paneldir1] ([dir2text(paneldir1)]), paneldir2: [paneldir2] ([dir2text(paneldir2)])")
...
/obj/movedummy/proc/mimic_turf(var/turf_type, var/TTL)
	...
	density = initial(T.density)
	opacity = initial(T.opacity)
	dir = initial(T.dir)
	layer = initial(T.layer)
	invisibility = 0
	if (TTL)
		spawn (TTL)
			pool(src)
