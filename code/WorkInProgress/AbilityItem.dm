...
/mob/proc/update_item_abilities()
	if (!client || !need_update_item_abilities) return

	need_update_item_abilities = 0
	//client.screen -= item_abilities
	for (var/obj/ability_button/B in client.screen)
		client.screen -= B
	...
	//for (var/obj/ability_button/B2 in item_abilities)
...
*/
	proc/dropped(mob/user as mob)
		if (material) material.triggerDrop(user)
		for (var/obj/ability_button/B in ability_buttons)
			B.OnDrop()
		hide_buttons()
		clear_mob()
	...
	proc/pickup(mob/user)
		if (material) material.triggerPickup(user)
		set_mob(user)
		show_buttons()
		return
...
/obj/ability_button
	...
	MouseEntered(location, control, params)
		usr.client.tooltip.show(src, params, title = name, content = (desc ? desc : null))
