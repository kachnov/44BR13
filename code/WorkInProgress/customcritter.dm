...
/obj/critter/custom
	...
	var/critterDeath/on_death = null
	var/list/abil = list()
	var/critterLootTable/loot_table = null
	...
	ex_act(severity)
		...
		switch(severity)
			if (1.0)
				health -= 200 * explosivevuln
				if (health <= 0)
					CritterDeath()
				return
			if (2.0)
				health -= 75 * explosivevuln
				if (health <= 0)
					CritterDeath()
				return
			else
				health -= 25 * explosivevuln
				if (health <= 0)
					CritterDeath()
				return
	...
	process()
		...
		for (var/critterAbility/A in abil)
			A.tick()
		if (on_death)
			on_death.tick()
	...
	attackby(obj/item/W as obj, mob/living/user as mob)
		...
		if (W.force || istype(W, /obj/item/artifact/melee_weapon))
			for (var/critterEvent/E in events)
	...
	proc/dodamage(var/mob/M, var/atype, var/damage)
		switch (atype)
			...
			if ("burn")
				random_burn_damage(target, damage)
			if ("toxin")
	...
	clone()
		...
		for (var/i = 1, i <= abil.len, i++)
			var/critterAbility/A = abil[i]
			if (istype(A))
				var/critterAbility/B = A.clone()
				C.abil[i] = B
				B.attach(C)
				B.C = C
		return C
	...
	deserialize(var/savefile/F, var/path, var/sandbox/sandbox)
		...
		F["[path].ambient_sound"] >> ambient_sound
		F["[path].dead_change_icon"] >> dead_change_icon
		var/iconDeserializerData/IDS = icon_deserializer(F, "[path].dead_icon", sandbox, dead_icon, dead_icon_state)
		dead_icon = IDS.icon
		...
		for (var/i = 1, i <= abil.len, i++)
			...
			if (T)
				var/critterAbility/A = new T()
				if (istype(A))
					A.deserialize(F, "[path].abil.[i]", sandbox)
					A.attach(src)
					abil[i] = A
				else
					logTheThing("debug", null, null, "<strong>Marquesas/CritterCreator:</strong> Cannot deserialize type [T].")

	proc/play_optional_sound(var/sound/sound)
		if (sound)
			playsound(src, sound, 50, 1)

	proc/addUntiedEvent(var/critterEvent/E)
		events += E
		E.attachment_point = EVENT_ATTACHMENT_POINT_NONE

	proc/addAttackEvent(var/critterEvent/E)
		events += E
		E.attachment_point = EVENT_ATTACHMENT_POINT_MELEE

/critterCreatorHolder
	...
	proc/blank(var/mob/M)
		...
		if (!(key in critterCreators))
			critterCreators += key
		critterCreators[key] = new /critterCreator
...
var/global/critterCreatorHolder/critter_creator_controller = new()

/critterLoot
	var/critterLootTable/lootTable
	var/dropped = null
	var/amount = 1
	var/chance = 100

	proc/configuration(var/critterCreator/configurer)
	...
	proc/change_configuration(var/critterCreator/configurer, var/which)
		switch (which)
			if ("amount")
...
/critterLootTable
	...
	proc/change_configuration(var/critterCreator/configurer, var/which)
		switch (which)
			...
			if ("addNew")
				var/critterLoot/L = new()
				L.lootTable = src
				loot += L

	proc/clone()
		var/critterLootTable/LT = new type()
		LT.maxDropped = maxDropped
		LT.loot.len = loot.len
		for (var/i = 1, i <= loot.len, i++)
			var/critterLoot/L = loot[i]
			var/critterLoot/L2 = L.clone()
			L2.lootTable = LT
			LT.loot[i] = L2
		return LT
...
/critterDeath/gib
	...
	configuration(var/critterCreator/configurer)
		. = "<span class='attribute-name'>Gib type: </span>"
		. += configurer.clickable_link("deathconf", gibtype ? "organic" : "machine", "machine", "gibtype")

	change_configuration(var/critterCreator/configurer, var/which)
		if (which == "gibtype")
			gibtype = !gibtype

	clone()
		var/critterDeath/gib/D = ..()
		D.gibtype = gibtype
		return D
	...
	deserialize(var/savefile/F, path, var/sandbox/sandbox)
		F["[path].gibtype"] >> gibtype
...
/critterEvent/varCrossed/health
	...
	clone()
		var/critterEvent/varCrossed/health/E = ..()
...
/critterEvent/varRange/health
	...
	clone()
		var/critterEvent/varRange/health/E = ..()
		E.minimum = minimum
		E.maximum = maximum
		return E
...
/critterEvent/varIs/healthLow
	...
	clone() // boy i sure do miss implicit copy constructors
		var/critterEvent/varIs/healthLow/E = ..()
		E.threshold = threshold
		return E
...
/critterEvent/varIs/healthHigh
	...
	clone()
		var/critterEvent/varIs/healthHigh/E = ..()
		E.threshold = threshold
		return E
...
/critterAbility
	...
	proc/deserialize(var/savefile/F, var/path, var/sandbox/sandbox)
		...
		if (ET)
			...
			logTheThing("debug", "usr", null, "<strong>Marquesas/CritterCreator: </strong> Failed to deserialize event for ability.")
	...
	proc/clone()
		var/critterAbility/copy = new type()
		copy.chance = chance
	...
	proc/build_events_cache()
		for (var/evtype in typesof(/critterEvent))
			var/critterEvent/ev = new evtype()
			if (ev.abstract)
				qdel(ev)
			else
				events_cache += ev
	...
	proc/change_configuration(var/critterCreator/configurer, var/which)
		switch(which)
			if ("event")
				if (!events_cache.len)
					build_events_cache()
				var/critterEvent/ev = input("Which event?", "Event", null) in events_cache
				if (event)
...
/critterAbility/criticalStrike
	...
	use_ability()
		if (C.target && ismob(C.target))
			...
			logTheThing("debug", null, null, "<strong>Marquesas/CritterCreator: </strong> Cannot reagent inject target, target is [C.target].")
			return FALSE
	...
	clone()
		var/critterAbility/criticalStrike/A = ..()
...
/critterAbility/reagent
	...
	use_ability()
		if (!C)
			logTheThing("debug", null, null, "<strong>Marquesas/CritterCreator: </strong> Error using ability \ref[src] ([type]). C: \ref[C] [C].")
			return FALSE
		if (C.target && ismob(C.target))
			var/mob/M = C.target
			if (melee && get_dist(C, M) > C.attack_range)
...
/critterAbility/projectile
	name = "shoot projectile"
	var/projectile_type = /projectile/laser
	...
	change_configuration(var/critterCreator/configurer, var/which)
		...
		switch(which)
			if ("projectile_type")
				projectile_type = configurer.getTypeExclusive("projectile type", projectile_type, /projectile)
			if ("fire_text")
	...
	clone()
		var/critterAbility/projectile/A = ..()
		A.projectile_type = projectile_type
		A.fire_text = fire_text
		return A

/critterAbility/projectile/burst
	...
	clone()
		var/critterAbility/projectile/burst/A = ..()
		A.turn_angle = turn_angle
		A.count = count
		A.fire_text = fire_text
		return A

/critterAbility/frenzy
	name = "frenzy"
	abstract = 0
	...
	configuration(var/critterCreator/configurer)
		...
		. += " a stunlock.<br>"
		. += "<strong>When going into frenzy</strong>, the following text is displayed: <br>"
		...
		. += "<br/><strong>When attacking in frenzy</strong>, the following text is displayed: <br>"
	...
	serialize(var/savefile/F, var/path, var/sandbox/sandbox)
		..()
		F["[path].frenzy_text"] << frenzy_text
		F["[path].frenzy_attack"] << frenzy_attack
		F["[path].frenzy_duration"] << frenzy_duration
		F["[path].attack_power"] << attack_power
		F["[path].attacktype"] << attacktype
		F["[path].attack_cooldown"] << attack_cooldown
		F["[path].stunlocks"] << stunlocks
		F["[path].frenzy_sound"] << frenzy_sound
		F["[path].frenzy_attack_sound"] << frenzy_attack_sound
...
/critterAbility/shockwave
	name = "shockwave"
	abstract = 0
	var/abilityHolder/revenant/dummyHolder
	var/targetable/revenantAbility/shockwave/ability
	...
	serialize(var/savefile/F, var/path, var/sandbox/sandbox)
		...
		F["[path].shockwave_text"] << shockwave_text
		F["[path].shockwave_sound"] << shockwave_sound
	...
	clone()
		var/critterAbility/shockwave/A = ..()
...
/critterAbility/spawnCritter
	...
	clone()
		var/critterAbility/spawnCritter/A = ..()
	...
	serialize(var/savefile/F, var/path, var/sandbox/sandbox)
		...
		F["[path].spawn_text"] << spawn_text
		F["[path].spawn_sound"] << spawn_sound
...
"Thunder" = 'sound/effects/thunder.ogg')
	...
	proc/getTypeExclusive(var/name, var/default, var/parent_type)
		return input(usr, "New value for [name]", name, default) in subtypesof(parent_type)
	...
	Topic(href, href_list)
		if (href_list["name"])
			...
			if (abilid)
				var/critterAbility/A = template.abil[abilid]
				if (A)
			...
			var/critterDeath/A = template.on_death
			if (A)
			...
			if (!abilid)
				return
			var/critterAbility/A = template.abil[abilid]
			var/critterAbility/ab = input("Which ability?", "Ability", null) in ability_cache
			if (A)
				if (ab.type == A.type)
					return
				qdel(A)
			...
			if (!death_cache.len)
				build_death_cache()
			var/critterDeath/D = template.on_death
			var/critterDeath/ab = input("Which death event?", "On death", null) in death_cache
			if (D)
				if (D.type == ab.type)
					return
				qdel(D)
	...
	proc/build_ability_cache()
		for (var/abtype in typesof(/critterAbility))
			var/critterAbility/ab = new abtype()
			if (ab.abstract)
				qdel(ab)
			else
				ability_cache += ab
	...
	proc/build_death_cache()
		for (var/abtype in typesof(/critterDeath))
			var/critterDeath/ab = new abtype()
			death_cache += ab
...
/client/proc/critter_creator_debug()
	...
	var/critterCreator/CR = critter_creator_controller.getCreator(mob)
	if (CR)
		CR.show_interface(mob)
