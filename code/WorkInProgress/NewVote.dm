...
var/vote_manager/vote_manager = new/vote_manager()
...
/mob/verb/vote_new()
	set name = "Vote"
	//if (!config.allow_vote_restart && !config.allow_vote_mode && !client.holder)
	...
	if (vote_manager.active_vote) vote_manager.show_vote(client)
	else
		if (ckey in vote_log)
			if (vote_log[ckey] < max_votes_per_round)
				if (vote_manager.show_vote_selection(client)) vote_log[ckey]++
			else
				boutput(src, "<span style=\"color:red\">You may not start any more votes this round. (Maximum reached : [max_votes_per_round])</span>")
		else
			if (vote_manager.show_vote_selection(client)) vote_log[ckey] = 1

/vote_manager
	var/vote_new/active_vote = null

	proc/show_vote_selection(var/client/C)
		if (active_vote) return FALSE
		var/choice
	/*
    ...
    */
		...
		switch(choice)
			...
			if ("Restart")
				...
				if (C.mob.stat && !C.holder)
					...
					return
				if (active_vote) return FALSE
				active_vote = new/vote_new/restart()
				boutput(world, "<span style=\"color:green\"><BIG><strong>Vote restart initiated by [C.ckey]</strong></BIG></span>")
				show_vote(C)
				return TRUE
			if ("Gamemode")
				...
				if (!mode) return FALSE //Magic?!
				if (active_vote) return FALSE
				active_vote = new/vote_new/mode(mode)
				boutput(world, "<span style=\"color:green\"><BIG><strong>Vote gamemode ([mode]) initiated by [C.ckey]</strong></BIG></span>")
				show_vote(C)
				return TRUE
			if ("Player Ban")
				if (world.time < 6000)
				...
				if (!player || player == "***CANCEL***") return FALSE
				var/mob/picked = bannable[player]
				if (!picked) return FALSE
				if (active_vote) return FALSE
				active_vote = new/vote_new/ban(picked.client)
				boutput(world, "<span style=\"color:green\"><BIG><strong>Vote Ban ([player]) initiated by [C.ckey]</strong></BIG></span>")
				show_vote(C)
				return TRUE
			/*if ("Player Mute")
				if (world.time < 6000)
				...
				if (!picked) return FALSE
				if (active_vote) return FALSE
				active_vote = new/vote_new/mute(picked.client)
				boutput(world, "<span style=\"color:green\"><BIG><strong>Vote Mute ([player]) initiated by [C.ckey]</strong></BIG></span>")
				show_vote(C)
				return TRUE*/
		return FALSE
	...
	proc/cancel_vote()
		if (active_vote)
			...
			qdel(active_vote)
			active_vote = null
			boutput(world, "<span style=\"color:green\"><BIG><strong>Voting aborted.</strong></BIG></span>")
...
/vote_new/mode
	...
	end_vote()
		vote_manager.active_vote = null
		boutput(world, "<span style=\"color:green\"><BIG><strong>Vote gamemode result: [get_winner()]</strong></BIG></span>")
		if (get_winner() == "Yes")
			if (get_winner_num() < 3)
				boutput(world, "<span style=\"color:green\"><BIG><strong>Minimum mode votes not reached (3)</strong></BIG></span>")
				qdel(src)
				return
			boutput(world, "<span style=\"color:green\"><BIG><strong>Gamemode will be changed to [data] after the next reboot.</strong></BIG></span>")
...
/vote_new/restart
	...
	end_vote()
		vote_manager.active_vote = null
		boutput(world, "<span style=\"color:green\"><BIG><strong>Vote restart result: [get_winner()]</strong></BIG></span>")
		if (get_winner() == "Yes")
			if (get_winner_num() < 5)
				boutput(world, "<span style=\"color:green\"><BIG><strong>Minimum restart votes not reached (5).</strong></BIG></span>")
				qdel(src)
				return
			Reboot_server()
		qdel(src)
...
/vote_new/ban
	...
	end_vote()
		vote_manager.active_vote = null
		boutput(world, "<span style=\"color:green\"><BIG><strong>Vote ban result: [get_winner()]</strong></BIG></span>")
		if (get_winner() == "Yes")
			if (get_winner_num() < 5)
				boutput(world, "<span style=\"color:green\"><BIG><strong>Minimum ban votes not reached (5) - Player not banned.</strong></BIG></span>")
				qdel(src)
				return
			if (data)
				boutput(world, "<span style=\"color:green\"><BIG><strong>[data:mob:name] / [data:ckey] has been banned for 3 hours.</strong></BIG></span>")
				...
				boutput(world, "<span style=\"color:green\"><BIG><strong>[data:backup_ckey] has been banned for 1 week for logging out while vote was in progress.</strong></BIG></span>")
...
/vote_new/mute
	...
	end_vote()
		...
		if (get_winner() == "Yes")
			if (get_winner_num() < 3)
				boutput(world, "<span style=\"color:green\"><BIG><strong>Minimum mute votes not reached (3) - Player not muted.</strong></BIG></span>")
				qdel(src)
				return
			if (data)
				boutput(world, "<span style=\"color:green\"><BIG><strong>[data:mob:name] has been muted.</strong></BIG></span>")
				//data:mob:muted = 1
			else //Logged out while vote was in progress.
				boutput(world, "<span style=\"color:green\"><BIG><strong>[data:backup_ckey] has been banned for 15 minutes for logging out while vote was in progress.</strong></BIG></span>")
