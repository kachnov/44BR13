...
/proc/take_bleeding_damage(var/mob/some_idiot as mob, var/mob/some_jerk as mob, var/damage as num, var/damage_type = DAMAGE_CUT, var/bloodsplatter = 1, var/turf/T as turf)
	...
	if (haine_blood_debug) logTheThing("debug", some_idiot, null, "<strong>HAINE BLOOD DEBUG: [some_idiot] begins bleed damage proc</strong>")

	if (!ishuman(some_idiot)) // maybe later borgs will bleed.  not now though.
		if (haine_blood_debug) logTheThing("debug", some_idiot, null, "<strong>HAINE BLOOD DEBUG: [some_idiot] is not human so bleed damage was canceled</strong>")
		return
	...
	if (H.stat ==  2 || H.nodamage || !H.can_bleed)
		...
		if (haine_blood_debug) logTheThing("debug", H, null, "<strong>HAINE BLOOD DEBUG: [H] is dead or immortal or otherwise not supposed to bleed, so their bleeding has been set to 0 and bleed damage was canceled</strong>")
		return
	...
	if ((!isvampire(H) && H.blood_volume > 0) || (isvampire(H) && H.get_vampire_blood() > 0)) // make sure we have blood to bleed
		...
		if (isvampire(H)) // we can go now, we don't need to do anything else for vamps
			return
		if (haine_blood_debug) logTheThing("debug", H, null, "<strong>HAINE BLOOD DEBUG:</strong> [H]'s blood level is [H.blood_volume]")

	else
		H.bleeding = 0 // if we don't have any blood to bleed, just stop okay, just stop.
		H.bleeding_internal = 0
		if (haine_blood_debug) logTheThing("debug", H, null, "<strong>HAINE BLOOD DEBUG: [H] has no blood and their bleeding has been set to 0 and bleed damage was canceled</strong>")
		return

	if (H.bleeding >= 10) // don't bleed more if you're already at bleeding 10 tia
		if (haine_blood_debug) logTheThing("debug", H, null, "<strong>HAINE BLOOD DEBUG: [H]'s bleeding was [H.bleeding] and has been set to 10 and bleed damage was canceled</strong>")
		H.bleeding = 10
		return

	var/increase_chance = rand(30, 50)
	if (haine_blood_debug) logTheThing("debug", H, null, "<strong>HAINE BLOOD DEBUG:</strong> [H]'s initial increase chance is [increase_chance]")

	if (haine_blood_debug) logTheThing("debug", H, null, "<strong>HAINE BLOOD DEBUG:</strong> [H]'s initial bleeding is [H.bleeding]")
	switch (H.bleeding)
		if (-INFINITY to 0)
			increase_chance += rand(30, 50)
			if (haine_blood_debug) logTheThing("debug", H, null, "<strong>HAINE BLOOD DEBUG:</strong> [H]'s increase chance increased to [increase_chance]")
		if (1 to 3)
		...
		if (7 to 8)
			...
			if (haine_blood_debug) logTheThing("debug", H, null, "<strong>HAINE BLOOD DEBUG:</strong> [H]'s increase chance increased to [increase_chance]")
		if (9 to INFINITY)
			if (haine_blood_debug) logTheThing("debug", H, null, "<strong>HAINE BLOOD DEBUG:</strong> [H]'s bleeding was already high and chance was not increased")

	if (haine_blood_debug) logTheThing("debug", H, null, "<strong>HAINE BLOOD DEBUG:</strong> [H]'s damage type is [dam_num2name(damage_type)]")
	switch (damage_type)
		if (DAMAGE_STAB)
		...
		if (DAMAGE_BLUNT)
			...
			if (haine_blood_debug) logTheThing("debug", H, null, "<strong>HAINE BLOOD DEBUG:</strong> [H]'s increase chance decreased to [increase_chance]")
		if (DAMAGE_BURN)
	...
	if (ischangeling(H))
		...
		if (haine_blood_debug) logTheThing("debug", H, null, "<strong>HAINE BLOOD DEBUG:</strong> [H] is a changeling - [H]'s increase chance decreased to [increase_chance]")
/*
	if (isvampire(H))
		increase_chance -= rand(10, 20)
		if (haine_blood_debug) logTheThing("debug", H, null, "<strong>HAINE BLOOD DEBUG:</strong> [H] is a vampire - [H]'s increase chance decreased to [increase_chance]")
*/
	if (H.traitHolder && H.traitHolder.hasTrait("hemophilia"))
		increase_chance *= 3

	var/final_increase_chance = min(increase_chance, 100)
	if (haine_blood_debug) logTheThing("debug", H, null, "<strong>HAINE BLOOD DEBUG:</strong> [H]'s final increase chance is [final_increase_chance]")
	if (prob(final_increase_chance))
		H.bleeding ++
		switch (H.bleeding)
			if (-INFINITY to 1)
				H.show_text("<strong>You start bleeding!</strong>", "red")
			if (1 to 8)
				if (prob(33))
					H.show_text("<strong>You[pick(" start bleeding even worse", " start bleeding even more", " start bleeding more", "r bleeding worsens", "r bleeding gets worse")]!</strong>", "red")
			if (9 to INFINITY)
				H.show_text("<strong>You can't go on very long with blood pouring out of you like this!</strong>", "red")

		if (haine_blood_debug) logTheThing("debug", H, null, "<strong>HAINE BLOOD DEBUG: [H] rolls bleeding increase, bleeding is now [H.bleeding]</strong>")
	else
		if (haine_blood_debug) logTheThing("debug", H, null, "<strong>HAINE BLOOD DEBUG: [H]'s bleeding does not increase</strong>")
		if (damage_type == DAMAGE_BURN && !H.is_heat_resistant() && H.bleeding > 0)
			if (prob(rand(30,50)))
				repair_bleeding_damage(H, rand(30,50), rand(1,3))
				if (haine_blood_debug) logTheThing("debug", H, null, "<strong>HAINE BLOOD DEBUG: [H] rolls bleeding repair due to DAMAGE_BURN</strong>")
...
/proc/repair_bleeding_damage(var/mob/some_idiot as mob, var/repair_chance as num, var/repair_amount as num)
	...
	if (haine_blood_debug) logTheThing("debug", some_idiot, null, "<strong>HAINE BLOOD DEBUG: [some_idiot] begins bleeding repair</strong>")

	if (!ishuman(some_idiot)) // if they aren't human they shouldn't be able to bleed!!
		if (haine_blood_debug) logTheThing("debug", some_idiot, null, "<strong>HAINE BLOOD DEBUG: [some_idiot] is not human so repair was canceled</strong>")
		return

	var/mob/living/carbon/human/H = some_idiot

	if (H.stat ==  2)
		if (haine_blood_debug) logTheThing("debug", H, null, "<strong>HAINE BLOOD DEBUG: [H] is dead and their bleeding has been set to 0 and repair was canceled</strong>")
		H.bleeding = 0 // no just stop bleeding entirely okay, you're dead, stop it
		H.bleeding_internal = 0
		return

	if (isvampire(H))
		if (haine_blood_debug) logTheThing("debug", H, null, "<strong>HAINE BLOOD DEBUG: [H] is a vampire and their bleeding has been set to 0 and repair was canceled</strong>")
		H.bleeding = 0 // bleh im a vampar
		H.bleeding_internal = 0
		return

	if (H.blood_volume <= 0)
		if (haine_blood_debug) logTheThing("debug", H, null, "<strong>HAINE BLOOD DEBUG: [H] has no blood and their bleeding has been set to 0 and repair was canceled</strong>")
		H.bleeding = 0 // you have no blood so stop trying to bleed
		H.bleeding_internal = 0
		return

	if (repair_amount <= 0)
		if (haine_blood_debug) logTheThing("debug", H, null, "<strong>HAINE BLOOD DEBUG: [H]'s repair_amount was set as [repair_amount] so repair was canceled</strong>")
	...
	if (repair_chance < 100) // if it's already 100 we don't need to go through all the crap down here
		if (haine_blood_debug) logTheThing("debug", H, null, "<strong>HAINE BLOOD DEBUG:</strong> [H]'s initial repair chance is [repair_chance]")
		switch (H.bleeding)
			if (-INFINITY to 0)
				if (haine_blood_debug) logTheThing("debug", H, null, "<strong>HAINE BLOOD DEBUG:</strong> [H] isn't bleeding and repair has been stopped")
				return // there's nothing to fix here, go home
			if (1 to 2)
				if (haine_blood_debug) logTheThing("debug", H, null, "<strong>HAINE BLOOD DEBUG:</strong> [H]'s bleeding was already low and chance was not increased")
			if (3 to 4)
				repair_chance += rand(0, 5)
				if (haine_blood_debug) logTheThing("debug", H, null, "<strong>HAINE BLOOD DEBUG:</strong> [H]'s repair chance increased to [repair_chance]")
			if (5 to 7)
	...
	if (haine_blood_debug) logTheThing("debug", H, null, "<strong>HAINE BLOOD DEBUG:</strong> [H]'s final repair chance is [final_repair_chance]")
	...
	if (prob(final_repair_chance))
		...
		switch (H.bleeding)
			if (-INFINITY to 0)
				H.show_text("<strong>Your bleeding stops!</strong>", "red")
			if (1 to 8)
				switch (repair_amount)
					if (1 to 3)
						H.show_text("<strong>Your bleeding [pick("slows", "slows down", "slightly slows", "slows a little", "gets slightly slower", "barely slows")]!</strong>", "red")
					if (4 to 6)
						H.show_text("<strong>Your bleeding [pick("slows", "slows down", "slows a lot", "slows down a lot", "really slows down")]!</strong>", "red")
					if (7 to 10)
						H.show_text("<strong>Your bleeding [pick("slows", "slows down", "slows a lot", "slows down a lot", "really slows down", "slows way down", "nearly stops", "has almost stopped", "is barely a trickle now")]!</strong>", "red")
			if (9 to INFINITY)
				H.show_text("<strong>You can't go on very long with blood pouring out of you like this!</strong>", "red")
		if (haine_blood_debug) logTheThing("debug", H, null, "<strong>HAINE BLOOD DEBUG: [H]'s bleeding repaired by [repair_amount], now [H.bleeding]</strong>")
		if (H.bleeding < 0)
			H.bleeding = 0
			if (haine_blood_debug) logTheThing("debug", H, null, "<strong>HAINE BLOOD DEBUG:</strong> [H]'s bleeding dropped below 0 and was reset to 0")

	else if (haine_blood_debug) logTheThing("debug", H, null, "<strong>HAINE BLOOD DEBUG: [H] rolled no repair</strong>")
...
/proc/bleed(var/mob/living/some_idiot as mob, var/num_amount as num, var/vis_amount as num, var/turf/T as turf)
	...
	if (haine_blood_debug) logTheThing("debug", some_idiot, null, "<strong>HAINE BLOOD DEBUG: [some_idiot] begins bleed</strong>")

	if (!istype(H)) // if they aren't human they shouldn't be able to bleed!!
		if (haine_blood_debug) logTheThing("debug", some_idiot, null, "<strong>HAINE BLOOD DEBUG: [some_idiot] is not human so bleed was canceled</strong>")
		return
	...
	if (H.stat == 2 || H.nodamage || !H.can_bleed)
		...
		if (haine_blood_debug) logTheThing("debug", some_idiot, null, "<strong>HAINE BLOOD DEBUG: [some_idiot] is either dead, immortal, or has can_bleed disabled, so bleed was canceled</strong>")
		return

	if (isvampire(H)) // vampires should be special
		if (H.bleeding)
			H.bleeding = 0 // we don't need this to be anything above 0 for vamps
			if (haine_blood_debug) logTheThing("debug", some_idiot, null, "<strong>HAINE BLOOD DEBUG: [some_idiot] is a vampire with a bleeding above 0, so it was reset to 0</strong>")

	if ((!isvampire(H) && H.blood_volume > 0) || (isvampire(H) && H.get_vampire_blood() > 0)) // you shouldn't bleed unless you have blood okay
		if (haine_blood_debug) logTheThing("debug", some_idiot, null, "<strong>HAINE BLOOD DEBUG:</strong> [H] blood level [H.blood_volume]")
		var/obj/decal/cleanable/blood/dynamic/B = locate(/obj/decal/cleanable/blood/dynamic) in T

		if (!B) // look for an existing dynamic blood decal and add to it if you find one
			B = new /obj/decal/cleanable/blood/dynamic(T)
			B.add_volume(H.blood_color, vis_amount)
			if (haine_blood_debug) logTheThing("debug", some_idiot, null, "<strong>HAINE BLOOD DEBUG:</strong> [H] creates new blood decal")
		else
			B.add_volume(H.blood_color, vis_amount)
			if (haine_blood_debug) logTheThing("debug", some_idiot, null, "<strong>HAINE BLOOD DEBUG:</strong> [H] adds volume to existing blood decal")

		if (H.is_changeling())
			B.ling_blood = 1

		if (H.bleeding > 4 && B.reagents && B.reagents.total_volume < 90 && H.reagents && H.reagents.total_volume)
			if (haine_blood_debug) logTheThing("debug", some_idiot, null, "<strong>HAINE BLOOD DEBUG:</strong> [H] transfers reagents to blood decal [log_reagents(H)]")
		...
		if (isvampire(H))
			...
			if (haine_blood_debug) logTheThing("debug", some_idiot, null, "<strong>HAINE BLOOD DEBUG: [H] bleeds -5 from vamp_blood_remaining and their vamp_blood_remaining becomes [H.get_vampire_blood()]</strong>")

		else
			H.blood_volume -= num_amount // time to bleed
			if (haine_blood_debug) logTheThing("debug", some_idiot, null, "<strong>HAINE BLOOD DEBUG: [H] bleeds [num_amount] and their blood level becomes [H.blood_volume]</strong>")

			if (H.blood_volume < 0) // you shouldn't have negative blood okay
				H.blood_volume = 0
				if (haine_blood_debug) logTheThing("debug", some_idiot, null, "<strong>HAINE BLOOD DEBUG:</strong> [H]'s blood volume dropped below 0 and was reset to 0")
	else
		return
...
/proc/transfer_blood(var/mob/living/some_idiot as mob, var/atom/A as obj|mob, var/amount = 5)
	...
	var/bioHolder/bloodHolder = null
	...
	// Ignore that second container of blood entirely if it's a vampire (Convair880).
		...
		bloodHolder = new/bioHolder(null)
...
/proc/blood_slash(var/mob/some_idiot as mob, var/bleed_amount as num, var/atom/A as mob|obj|turf, var/direction, var/splatters = 4)
	...
	if (!some_idiot) // what do you want and also what are you doing here?!
		if (haine_blood_debug) logTheThing("debug", null, null, "<strong>HAINE BLOOD DEBUG:</strong> blood_slash: not passed a mob, exiting")
		return

	if (!ishuman(some_idiot)) // no stop trying to bleed you aren't human
		if (haine_blood_debug) logTheThing("debug", some_idiot, null, "<strong>HAINE BLOOD DEBUG:</strong> blood_slash: passed non-human mob [some_idiot], exiting")
		return

	if (!A) // if we aren't provided with a separate target, use some_idiot
		target = some_idiot.loc
		if (haine_blood_debug) logTheThing("debug", some_idiot, null, "<strong>HAINE BLOOD DEBUG:</strong> blood_slash: no origin point specified, using [some_idiot]'s location")
	else
		target = A.loc
		if (haine_blood_debug) logTheThing("debug", some_idiot, null, "<strong>HAINE BLOOD DEBUG:</strong> blood_slash: origin point set to [A]")

	take_bleeding_damage(some_idiot, null, bleed_amount, DAMAGE_CUT, 1, target)
	if (haine_blood_debug) logTheThing("debug", some_idiot, null, "<strong>HAINE BLOOD DEBUG:</strong> [some_idiot] enters take_bleeding_damage from blood_slash")

	if (!direction) // if we aren't provided with a direction, use some_idiot again
		direction = some_idiot.dir
		if (haine_blood_debug) logTheThing("debug", some_idiot, null, "<strong>HAINE BLOOD DEBUG:</strong> blood_slash: no direction specified, using [some_idiot]'s dir")
	...
	for (var/i = 0, i < splatters, i++)
		...
		if (haine_blood_debug) logTheThing("debug", some_idiot, null, "<strong>HAINE BLOOD DEBUG:</strong> [some_idiot] enters bleed from blood_slash")
...
/mob/proc/staunch_bleeding(var/mob/some_idiot) // stolen from ISN's shake_awake() proc
	...
	if (istype(some_idiot,/mob/living/carbon/human))
		...
		if (H.being_staunched)
			show_text("[H == src ? "You're" : "Someone's"] already putting pressure on [H == src ? "your" : "[H]'s"] wounds!", "red")
			return
...
//			H.w_uniform.add_fingerprint(src)
		...
		tri_message("<span style=\"color:blue\"><strong>[src]</strong> puts pressure on [src == H ? "[his_her]" : "[H]'s"] wounds, trying to stop the bleeding!</span>",\
		src, "<span style=\"color:blue\">You put pressure on [src == H ? "your" : "[H]'s"] wounds, trying to stop the bleeding!</span>",\
		H, "<span style=\"color:blue\">[H == src ? "You put" : "<strong>[src]</strong> puts"] pressure on your wounds, trying to stop the bleeding!</span>")
		...
		if (do_mob(src, H, 100))
			...
			if (original_bleed > H.bleeding)
				switch (H.bleeding)
					if (-INFINITY to 0)
						show_text("The bleeding stops!", "blue")
					if (1 to 8)
						show_text("The bleeding slows!", "blue")
					if (9 to INFINITY)
						show_text("It barely helps!", "red")

			else if (original_bleed == H.bleeding)
				show_text("The bleeding doesn't slow at all!", "red")

			else if (original_bleed < H.bleeding) // what
				show_text("Oh fuck somehow the bleeding got WORSE!", "red")

			H.being_staunched = 0
			return TRUE

		else
			show_text("You were interrupted!", "red")
			H.being_staunched = 0
			return FALSE
	else
		return FALSE
...
/proc/internal_bleeding_damage(var/mob/some_idiot as mob, var/mob/some_jerk as mob, var/damage as num)
	...
	if (haine_blood_debug) logTheThing("debug", some_idiot, null, "<strong>HAINE BLOOD DEBUG: [some_idiot] begins internal bleed damage proc</strong>")

	if (!ishuman(some_idiot)) // maybe later borgs will bleed.  not now though.
		if (haine_blood_debug) logTheThing("debug", some_idiot, null, "<strong>HAINE BLOOD DEBUG: [some_idiot] is not human so internal bleed damage was canceled</strong>")
		return
	...
	if (H.stat ==  2 || H.nodamage || !H.can_bleed || isvampire(H))
		if (H.bleeding)
			H.bleeding = 0
			H.bleeding_internal = 0
		if (haine_blood_debug) logTheThing("debug", H, null, "<strong>HAINE BLOOD DEBUG: [H] is dead/immortal/a vampire/otherwise not supposed to bleed, so their bleeding has been set to 0 and internal bleed damage was canceled</strong>")
		return

	if (!(H.blood_volume > 0)) // make sure we have blood to bleed
		H.bleeding = 0 // if we don't have any blood to bleed, just stop okay, just stop.
		H.bleeding_internal = 0
		if (haine_blood_debug) logTheThing("debug", H, null, "<strong>HAINE BLOOD DEBUG: [H] has no blood and their bleeding has been set to 0 and internal bleed damage was canceled</strong>")
		return

	if (H.bleeding_internal >= 10) // don't bleed more if you're already at bleeding 10 tia
		if (haine_blood_debug) logTheThing("debug", H, null, "<strong>HAINE BLOOD DEBUG: [H]'s internal bleeding was [H.bleeding_internal] and has been set to 10 and internal bleed damage was canceled</strong>")
	...
	if (haine_blood_debug) logTheThing("debug", H, null, "<strong>HAINE BLOOD DEBUG:</strong> [H]'s initial internal bleeding is [H.bleeding_internal]")
	switch (H.bleeding_internal)
		...
		if (4)
			...
			if (haine_blood_debug) logTheThing("debug", H, null, "<strong>HAINE BLOOD DEBUG:</strong> [H]'s increase chance increased to [increase_chance]")
		if (5 to INFINITY)
			if (haine_blood_debug) logTheThing("debug", H, null, "<strong>HAINE BLOOD DEBUG:</strong> [H]'s internal bleeding was already high and chance was not increased")

	if (some_jerk && some_jerk.zone_sel && some_jerk.zone_sel.selecting)
		if (haine_blood_debug) logTheThing("debug", H, null, "<strong>HAINE BLOOD DEBUG:</strong> [some_jerk]'s target zone is [some_jerk.zone_sel.selecting]")
		switch (some_jerk.zone_sel.selecting)
			if ("head")
				increase_chance += rand(0, 10)
				if (haine_blood_debug) logTheThing("debug", H, null, "<strong>HAINE BLOOD DEBUG:</strong> [H]'s increase chance increased to [increase_chance]")
	...
	if (prob(final_increase_chance))
		H.bleeding_internal ++
		if (haine_blood_debug) logTheThing("debug", H, null, "<strong>HAINE BLOOD DEBUG: [H] rolls internal bleeding increase, internal bleeding is now [H.bleeding_internal]</strong>")
	else
		if (haine_blood_debug) logTheThing("debug", H, null, "<strong>HAINE BLOOD DEBUG: [H]'s internal bleeding does not increase</strong>")
	H.bleeding_internal = minmax(H.bleeding_internal, 0, 5)
	if (haine_blood_debug) logTheThing("debug", H, null, "<strong>HAINE BLOOD DEBUG: [H]'s internal bleeding is [H.bleeding_internal] after minmax</strong>")
...
/obj/item/test_toilet
	...
	attack(mob/M as mob, mob/user as mob)
		user.visible_message("<span class='combat'><strong>[user]</strong> attacks [M] with [src], set to <strong>[dam_num2name(damage_type)]</strong>!</span>", "<span class='combat'>You attack [M] with [src], set to <strong>[damage_type]</strong>!</span>")
		switch(damage_type)
			if (DAMAGE_STAB)
		...
		take_bleeding_damage(M, user, 1, damage_type)
