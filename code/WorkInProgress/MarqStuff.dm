...
/obj/item/arrow
	...
	// placeholder
	var/material/head_material
	var/material/shaft_material
	var/image/shaft
	var/image/head
	amount = 1
	...
	proc/setHeadMaterial(var/material/M)
	...
	proc/setShaftMaterial(var/material/M)
	...
	afterattack(var/atom/target, var/mob/user, reach)
		...
		if (istype(target, /mob/living))
			if (prob(50))
				user.visible_message("<span style='color:red'><strong>[user] tries to stab [target] with [src] but misses!</strong></span>")
				playsound(get_turf(user), 'sound/weapons/punchmiss.ogg', 25, 1, 1)
				return
			user.visible_message("<span style='color:red'><strong>[user] stabs [target] with [src]!</strong></span>")
			user.u_equip(src)
...
/projectile/arrow
	...
	on_hit(var/atom/A, angle, var/obj/projectile/P)
		if (ismob(A))
			...
			take_bleeding_damage(A, null, round(power / 3), hit_type)
...
/obj/item/gun/bow
	...
	var/action/bar/aim/aim = null
	current_projectile = new/projectile/arrow
	spread_angle = 30
	force = 5
