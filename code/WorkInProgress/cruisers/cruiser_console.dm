...
/computer/file/terminal_program/cruiser
	name = "Cruiser"
	size = 32
	...
	initialize()
		print_text("Cruiser Control Assistant<br>Type \"help\" for commands.")
		...
		switch(lowertext(command))
			...
			if ("reboot")
				if (command_list.len >= 1)
					...
					if (lowertext(t) in list("turret_l","turret_r","engine","life_support","pod_weapons","pod_navigation","pod_defense"))
						...
						peripheral_command("reboot", newsignal, "\ref[adapter]")
					else
			...
			if ("set_power","spwr")
				if (command_list.len == 2)
					...
					peripheral_command("set_power", newsignal, "\ref[adapter]")
				else
					print_text("Usage: set_power (thrusters/weapons/shields) (percentage)")

			if ("power_ratios", "p%", "pwrr")
				peripheral_command("power_ratios", null, "\ref[adapter]")

			if ("read_power", "power", "pwr")
				peripheral_command("read_power", null, "\ref[adapter]")

			if ("damage", "dmg")
				peripheral_command("get_damage", null, "\ref[adapter]")
	...
	receive_command(obj/source, command, signal/signal)
		...
		if (command == "read_power")
			...
			print_text("Power: [signal.data["used_last"]]Pu / [signal.data["prod_last"]]Pu")
			for (var/X in recieved)
				print_text("--[X]: [recieved[X]]")
			print_text("<br>")
			return

		else if (command == "get_damage")
			print_text("Number of damaged systems: [signal.data["damage_num"]]")
			print_text("Total damage: [signal.data["damage_total"]]")
			print_text("Ship degradation: [signal.data["degradation"]]%")
			if (signal.data["damage_breakdown"])
				print_text(signal.data["damage_breakdown"])
			return

		else if (command == "power_ratios")
			print_text("Thrusters power: [signal.data["ratio_movement"]]%")
			print_text("Weapons power: [signal.data["ratio_offense"]]%")
			print_text("Shields power: [signal.data["ratio_defense"]]%")
			return
		else if (command == "set_power")
			print_text("[signal.data["info"]]")
			return
...
/obj/machinery/computer3/generic/cruiser
	...
	setup_starting_os = /computer/file/terminal_program/os/main_os/no_login
	setup_starting_peripheral1 = /obj/item/peripheral/cruiser
	setup_starting_program = /computer/file/terminal_program/cruiser
