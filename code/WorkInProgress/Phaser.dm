...
/obj/item/oldgun/energy/phaser
	...
	verb/remove_mods()
		...
		var/energy_old = min(125,charges)
		P.safeties = safeties
	...
	proc/generate_overlays()
		overlays = null
		if (extension_mod)
			overlays += icon('icons/obj/gun.dmi',extension_mod.overlay_name)
		if (converter_mod)
			overlays += icon('icons/obj/gun.dmi',converter_mod.overlay_name)
		...
		spawn (0)
			...
			while (O.loc != O.dest)
				...
				if (impact)
					...
					PBul.proj_data = new /projectile/bullet/revolver_357(  )
					PLas.proj_data = new /projectile/laser(  )
