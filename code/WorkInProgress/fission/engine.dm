...
/obj/machinery/power/fission/engine
	...
	attack_hand(mob/user as mob)
		if (stat & (BROKEN | NOPOWER))
			...
			active = !active
			boutput(user, "You turn [active ? "on" : "off"] the engine.")
			if (active == 0) lastpower = 0
			updateicon()
			return
			...
			if (FR.amount > 0)
				...
				if (istype(T, /turf/simulated) && T:air)
					...
					if (T:air:trace_gases && T:air:trace_gases.len)
						for (var/gas/G in T:air:trace_gases)
							if (istype(G, /gas/rad_particles))
								count++
								G.moles += 0.1

					if (count == 0)
						var/gas/rad_particles/rad = new
						rad.moles = 0.1
