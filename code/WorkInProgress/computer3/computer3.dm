...
/obj/machinery/computer3
	...
	var/temp = "<strong>Thinktronic BIOS V2.1</strong><br>"
	var/temp_add = null
	var/obj/item/disk/data/fixed_disk/hd = null
	var/computer/file/terminal_program/active_program
	var/computer/file/terminal_program/host_program //active is set to this when the normal active quits, if available
	var/computer/file/terminal_shell/os_shell
	...
	generic //Generic computer, standard os and card scanner
		...
		setup_starting_os = /computer/file/terminal_program/os/main_os
		...
		personal
			...
			setup_starting_program = /computer/file/terminal_program/email
		...
		med_data
			...
			setup_starting_program = /computer/file/terminal_program/medical_records
		...
		secure_data
			...
			setup_starting_program = /computer/file/terminal_program/secure_records

		communications
			name = "Communications Console"
			icon_state = "comm"
			setup_starting_program = /computer/file/terminal_program/communications
			setup_starting_peripheral1 = /obj/item/peripheral/network/powernet_card
		...
		disease_research
			...
			icon_state = "resdis"
			setup_starting_program = /computer/file/terminal_program/disease_research
			setup_drive_size = 48

		artifact_research
			name = "Artifact Database"
			icon_state = "resart"
			setup_starting_program = /computer/file/terminal_program/artifact_research
			setup_drive_size = 48

		hangar_control
			name = "Hangar Control"
			icon_state = "comm"
			//setup_starting_program = /computer/file/terminal_program/hangar_control
			setup_drive_size = 48
		hangar_research
			name = "Hangar Research"
			icon_state = "resrob"
			//setup_starting_program = /computer/file/terminal_program/hangar_research
			setup_drive_size = 48
		robotics_research
			name = "Robotics Database"
			icon_state = "resrob"
			setup_starting_program = /computer/file/terminal_program/robotics_research
			setup_drive_size = 48
/*
		dna_scan
...
*/
		...
		engine
			...
			setup_starting_program = /computer/file/terminal_program/engine_control
			#endif
			setup_starting_peripheral1 = /obj/item/peripheral/network/powernet_card
			setup_starting_peripheral2 = /obj/item/peripheral/network/radio/locked/pda
			setup_drive_size = 48
		...
		basic_test
			name = "personal computer"
			//setup_starting_program = /computer/file/terminal_program/basic
			setup_starting_peripheral1 = /obj/item/peripheral/network/powernet_card
	...
	terminal //Terminal computer, stripped down with less cards.
		...
		setup_starting_os = /computer/file/terminal_program/os/terminal_os
...
/obj/machinery/computer3/New()
	...
	spawn (4)
		...
		if (ispath(setup_starting_peripheral1))
			new setup_starting_peripheral1(src) //Peripherals add themselves automatically if spawned inside a computer3

		if (ispath(setup_starting_peripheral2))
			new setup_starting_peripheral2(src)


		if (setup_idscan_path)
			new setup_idscan_path(src)
		...
		if (ispath(setup_starting_program))
			var/computer/file/terminal_program/starting = new setup_starting_program

			hd.file_amount = max(hd.file_amount, starting.size)

			starting.transfer_holder(hd)
			//processing_programs += active_program

		if (ispath(setup_starting_os) && hd)
			var/computer/file/terminal_program/os/os = new setup_starting_os
...
/obj/machinery/computer3/attack_hand(mob/user as mob)
	...
	if ((user.machine == src) && (current_user == user))
		if (!temp)
			user << output(null, "comp3.browser:con_clear")

		if (temp_add)
			user << output(url_encode(temp_add), "comp3.browser:con_output")
/*
...
*/
		...
		#consolelog
		{
			...
			background-color:[setup_bg_color];
			color:[setup_font_color];
...
function lineEnter ()
{
	if (document.getElementById("consoleinput_text").value != null)
	{
		...
		if (setup_has_internal_disk)
			dat += "<td id=\"internaldisk\">Disk: <a href='byond://?src=\ref[src];disk=1'>[diskette ? "Eject" : "-----"]</a></td>"
		else
			dat += "<td id = \"internaldisk\" style=\"display: none;\"></td>"

		//Show up to two card "badges," so ID scanners can present a slot, etc
		var/count = 0
		for (var/obj/item/peripheral/C in peripherals)
...
/obj/machinery/computer3/proc/update_peripheral_menu(mob/user as mob)
	var/count = 0
	for (var/obj/item/peripheral/pCard in peripherals)
...
/obj/machinery/computer3/Topic(href, href_list)
	...
	if ((href_list["command"]) && active_program)
		usr << output(null, "comp3.browser:input_clear")
		active_program.input_text(href_list["command"])
		...
		if (diskette)
			...
			for (var/computer/file/terminal_program/P in processing_programs)
				P.disk_ejected(diskette)

			diskette.set_loc(get_turf(src))
			diskette = null
...
/obj/machinery/computer3/attackby(obj/item/W as obj, mob/user as mob)
	if (istype(W, /obj/item/disk/data/floppy)) //INSERT SOME DISKETTES
		if ((!diskette) && setup_has_internal_disk)
			user.machine = src
			user.drop_item()
			W.set_loc(src)
			diskette = W
		...
		if (do_after(user, 20))
			...
			var/obj/computer3frame/A = new setup_frame_type( loc )
			...
			A.mainboard.integrated_floppy = setup_has_internal_disk
...
/obj/machinery/computer3/disposing()
	...
	if (processing_programs)
		processing_programs.len = 0
		processing_programs = null
...
/obj/machinery/computer3/proc
	...
	run_program(computer/file/terminal_program/program)
	...
	//		boutput(world, "Not in src")
		...
		if (!src.host_program && istype(program, /computer/file/terminal_program/os))
		...
		active_program.initialize()
		return TRUE

	//Stop processing the current active program and make the host active again
	unload_program(computer/file/terminal_program/program)
		if (!program)
			return FALSE
	...
	send_command(command, signal/signal, target_ref)
		//for (var/obj/item/peripheral/P in peripherals)
		//	P.receive_command(src, command, signal)

		. = 1
		var/obj/item/peripheral/P = locate(target_ref) in peripherals
		if (istype(P))
			. = P.receive_command(src, command, signal)
		//qdel(signal)
		if (signal)
...
//				src.host_program.receive_command(src, command, signal)
	...
	post_system()
		...
		if (!hd)
			temp_add += "<font color=red>1701 - NO FIXED DISK</font><br>"
		...
		if (src.host_program) //Let the starting programs set up vars or whatever
			...
			if (diskette && diskette.root)
				var/computer/file/terminal_program/os/newos = locate(/computer/file/terminal_program/os) in diskette.root.contents

				if (newos && istype(newos))
					src.temp_add += "Booting from diskette...<br>"
					run_program(newos)
				else
					temp_add += "<font color=red>Non-system disk or disk error.</font><br>"

			if (!src.host_program && src.hd && src.hd.root)
				var/computer/file/terminal_program/os/newos = locate(/computer/file/terminal_program/os) in hd.root.contents

				if (newos && istype(newos))
					src.temp_add += "Booting from fixed disk...<br>"
					run_program(newos)
				else
					temp_add += "<font color=red>Unable to boot from fixed disk.</font><br>"
			...
			if (!src.host_program)
				...
				for (var/obj/item/disk/data/D in src)
					...
					var/computer/file/terminal_program/os/newos = locate() in D.root.contents
				...
				if (!success)
					temp_add += "<font color=red>ERR - BOOT FAILURE</font><br>"
...
/obj/machinery/computer3/clone()
	...
	cloneComp.setup_starting_peripheral1 = setup_starting_peripheral1
	cloneComp.setup_starting_peripheral2 = setup_starting_peripheral2

	cloneComp.setup_starting_os = null
	cloneComp.setup_idscan_path = setup_idscan_path
	cloneComp.setup_has_internal_disk = setup_has_internal_disk

	cloneComp.setup_font_color = setup_font_color
	cloneComp.setup_bg_color = setup_bg_color
...
/obj/item/luggable_computer
	...
	New()
		...
		spawn (10)
			if (!luggable)
				luggable = new luggable_type (src)
				luggable.case = src
				luggable.deployed = 0
		return
	...
	proc/deploy(mob/user as mob)
		...
		luggable.deployed = 1
		set_loc(luggable)
		for (var/obj/item/peripheral/P in luggable)
			P.installed(luggable)

		user.visible_message("<strong>[user]</strong> deploys [luggable]!","You deploy [luggable]!")
...
/obj/machinery/computer3/luggable
	...
	verb/fold_up()
		...
		visible_message("<span style=\"color:red\">[usr] folds [src] back up!</span>")
		undeploy()
		return

	proc/undeploy()
		if (!case)
			case = new /obj/item/luggable_computer(src)
			case.luggable = src

		for (var/obj/item/peripheral/peripheral in peripherals)
			peripheral.uninstalled()

		case.set_loc(get_turf(src))
		set_loc(case)
		deployed = 0
		return
...
/obj/machinery/computer3/luggable/personal
	...
	undeploy()
		if (!case)
			case = new /obj/item/luggable_computer/personal(src)
			case.luggable = src
