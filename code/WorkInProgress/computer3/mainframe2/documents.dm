...
/computer/file/record/dwaine_help
	...
	New()
		...
		fields["basics3"] = "DWAINE is, primarily, a networked system.  All devices, from user terminals to lineprinters and tape drives, are connected remotely to the central mainframe.  Devices that interact with files, such as drives or printers, may appear as part of the filesystem as a sub-directory of /mnt.|nFor example, the contents of tape databank \"Main\" would appear within /mnt/main and applications and other files could be accessed by setting it to your working directory with \"cd /mnt/main\""
		fields["basics4"] = "Application software is often bundled with a \"readme\" file of some sort, which can be viewed with the CAT command.|nEx: \"cat readme\" will view the contents of a readme file in the user's working directory."
		fields["ls"] = "View contents of a directory.  If no filepath is supplied, the working directory will be listed.|nExpanded file information, as well as hidden files, may be viewed through use of the -l argument.|nUsage: ls \[-l] \[filepath]"
		fields["cd"] = "Change working directory to the filepath supplied as argument.|nUsage: cd \"filepath\""
		...
		fields["chown"] = "Change the owner and/or group of a file. System operator status required to invoke.|nUsage: chown \[exact username]:\[group ID] \"filename\""
		fields["mount"] = "Mount device driver file space to filesystem. System operator status required to invoke.|nUsage: mount \"device net ID\" \"mountpoint name (Name for folder in /mnt)\""
		fields["scnt"] = "Scan network for unconnected peripheral devices, and then automatically connect to them. System operator status required to invoke.|nSpecific net IDs may be provided as arguments to connect to them directly. |nUsage: \"scnt\""
		fields["logoff"] = "Log out current user and ready system to accept new login.|nUsage: \"logoff\""

		fields["index"] = "Usage: help \"topic\"|nValid Topics: [english_list(fields, "None")]"

/computer/file/record/pr6_readme
	name = "readme"

	New()
		..()
		fields = list("Readme for PR-6 Control Interface",
...
/computer/file/record/patrol_script
	name = "patrol_script"
...
/computer/file/record/bodyguard_script
	name = "guard_script"
...
/computer/file/record/roomguard_script
...
/computer/file/record/bodyguard_conf
	name = "demo_conf"

	New()
		..()
		fields = list("#demonstration of configuration file",
...
/computer/file/record/random_email

	New(mailName as text)
		..()
		name = "[copytext("\ref[src]", 4, 12)]GENERIC"
		if (mailName)
			fields = strings("mainframe/documents/randomEmail.txt", mailName)