...
/computer/file/mainframe_program/driver
	...
	proc
		terminal_input(var/data, var/computer/file/file)
			if (!data || !initialized)
				return TRUE
			return FALSE

		message_device(var/data, var/computer/file/file)
			if (!initialized || (length(name) != 8))
				return ESIG_NOTARGET

			return signal_program(1, list("command"=DWAINE_COMMAND_MSG_TERM, "data" = data, "term" = name), file)

/computer/file/mainframe_program/driver/mountable
	...
	Del()
		for (var/computer/D in contents_mirror)
			D.holding_folder = null
			//qdel(D)
			D.dispose()

		for (var/i in to_add)
			var/computer/D = to_add[i]
			if (D)
				D.holding_folder = null
				//qdel(D)
				D.dispose()
	...
	*/
	...
	disposing()
		...
		if (mountpoints)
			for (var/computer/folder/mountpoint/MP in mountpoints)
				MP.driver = null
				MP.dispose()
		...
		to_remove = null
		mountpoints = null
		contents_mirror = null

		..()

	proc
		add_file(var/computer/file/file)

		remove_file(var/computer/file/file)

		change_metadata(var/computer/file/file, var/field, var/newval)


/computer/file/mainframe_program/driver/mountable/user_terminal
	...
	initialize(var/initparams)
		...
		signal_program(1, list("command"=DWAINE_COMMAND_MOUNT, "id"=name, "link"="term"))
		return

	add_file(var/computer/file/a_file, var/mainframe2_user_data/user)
		if (!initialized || !istype(a_file) || !istype(user) || !user.user_id)
			return FALSE

		if (istype(a_file, /computer/file/record))
...
/computer/file/mainframe_program/driver/mountable/databank
	name = "data_bank"
	...
	initialize(var/initparams)
		...
		if (bank_name)
			signal_program(1, list("command"=DWAINE_COMMAND_MOUNT, "id"=name, "link"=lowertext(bank_name)))
		return
	...
	terminal_input(var/data, var/computer/file/file)
		...
		switch(lowertext(datalist["command"]))
			...
			if ("register")
				...
				if (datalist["data"] != bank_name)
					bank_name = datalist["data"]
					signal_program(1, list("command"=DWAINE_COMMAND_MOUNT, "id"=name, "link"=lowertext(bank_name)))
					message_device("command=sync")

			if ("sync")
				if (istype(file, /computer/file/archive))
					update_from_archive(file)
			...
			if ("status")
				...
				if (sessionid in to_adjust)
					...
					if (istype(worklist) && worklist.len == 3 && datalist["status"] == "success")
						var/computer/file/workfile = worklist[1]
						if (istype(workfile))
					...
					var/computer/file/workfile = to_add[sessionid]
					if (!istype(workfile))
						to_add -= sessionid
						return
					...
					var/computer/file/workfile = to_remove[sessionid]
					if (!istype(workfile))
						to_remove -= sessionid
						return
	...
	add_file(var/computer/file/file) //Note: Folders are not (And really should not) be accepted.
	...
	change_metadata(var/computer/file/file, var/field, var/newval)
		...
		var/list/checklist = contents_mirror + to_add + to_remove
	...
	proc
		update_from_archive(var/computer/file/archive/arc)
			//boutput(world, "update_from_archive for [name] with supplied \ref[arc]")
			if (!arc || !arc.contained_files)
				//boutput(world, "blug")
				return

			for (var/computer/F in contents_mirror)
			...
			for (var/computer/F in arc.contained_files)
				if (istype(F, /computer/folder))
					//qdel(F)
...
/computer/folder/mountpoint
	gen = 10

	var/computer/file/mainframe_program/driver/mountable/driver = null

	New(var/computer/file/mainframe_program/driver/mountable/newdriver)
		..()
		if (gen != 10) gen = 10
		if (istype(newdriver))
			//qdel(contents)
			contents = newdriver.contents_mirror
			driver = newdriver
			driver.mountpoints += /computer/folder/mountpoint
		return
	...
	*/
	...
	disposing()
		contents = null
		if (driver && driver.mountpoints)
			driver.mountpoints -= src
	...
	add_file(computer/R, misc)
		if (!driver || driver.holder != holder)
			return FALSE
...
/computer/file/mainframe_program/driver/mountable/printer
	name = "printdevc"
	setup_processes = 1
	...
	var/tmp/computer/file/record/statusfile = null
	...
	initialize(var/initparams)
		...
		if (printer_name)
			signal_program(1, list("command"=DWAINE_COMMAND_MOUNT, "id"=name, "link"="lp-[lowertext(printer_name)]"))

		if (!statusfile)
			for (var/computer/R in contents_mirror)
				if (R.name == setup_statusfile_name)
					if (istype(R, /computer/file/record))
						statusfile = R
					return

			statusfile = new /computer/file/record(  )
			statusfile.name = setup_statusfile_name
			statusfile.fields += "Status: OK"
			contents_mirror += statusfile

		return

	add_file(var/computer/file/theFile)
		if (!initialized || !istype(theFile))
			return FALSE

		if (istype(theFile, /computer/file/record) || !isnull(theFile.asText()))
	...
	process()
		...
		printer_wait = 5
		var/computer/file/record/printFile = to_print[1]
		...
		else if (istype(printFile, /computer/file) && printFile.asText() != null)
			var/computer/file/oldPrintFile = printFile
			printFile = new /computer/file/record( )
	...
	terminal_input(var/data, var/computer/file/file)
		...
		switch(lowertext(datalist["command"]))
			...
			if ("register")
				...
				if (datalist["data"] != printer_name)
					printer_name = datalist["data"]
					signal_program(1, list("command"=DWAINE_COMMAND_MOUNT, "id"=name, link="lp-[lowertext(printer_name)]"))
			...
			if ("status")
				...
				switch(status)
					if ("success", "badfile", "lowpaper")
						...
						var/computer/file/F = to_print[1]
						contents_mirror -= F
						to_print.Cut(1,2)
						//qdel(F)
						F.dispose()
					...
					if ("nopaper")
						...
						to_print.len = 0
						for (var/computer/file/F in contents_mirror)
							if (F == statusfile)
								continue
							//qdel(F)
							F.dispose()

						contents_mirror.len = 1
						printer_wait = 0
						return
...
//telescience driver
/computer/file/mainframe_program/driver/telepad
	name = "s_telepad"
	setup_processes = 1
	...
	receive_progsignal(var/sendid, var/list/data, var/computer/file/file)
		...
		switch (lowertext(data["command"]))
			if ("set_coords")
				...
				var/computer/file/coords/new_coords = unpool(/computer/file/coords)
...
/computer/file/mainframe_program/srv/telecontrol
	name = "teleman"
	size = 1
...
/computer/file/mainframe_program/driver/nuke
	name = "nuccharge"
	setup_processes = 1
	...
	terminal_input(var/data, var/computer/file/file)
		...
		switch(lowertext(datalist["command"]))
			...
			if ("n_status")
				...
				nuke_time = min(max(stat_time, 0), 512)

				nuke_active = (datalist["active"] == "1")
...
/computer/file/mainframe_program/nuke_interface
			...
			if ("activate", "n_act")
				...
				switch(success)
					if (ESIG_SUCCESS)
						...
						if (istype(useracc) && useracc.user_id)
							callobj = locate("\[0x[copytext(useracc.user_id,2)]]") in world
						...
						if (istype(calling_term))
							if (calling_term.current_user)
								...
								logUser = "Terminal \[[useracc.user_id]]"
...
/computer/file/mainframe_program/driver/mountable/guard_dock
	name = "pr6_charg"
	setup_processes = 1
	...
	initialize(var/initparams)
		...
		signal_program(1, list("command"=DWAINE_COMMAND_MOUNT, "id"=name))
		if (!statusfile)
			for (var/computer/R in contents_mirror)
				if (R.name == setup_statusfile_name)
					if (istype(R, /computer/file/record))
	...
	terminal_input(var/data, var/computer/file/file)
		...
		switch(lowertext(datalist["command"]))
			...
			if ("reply")
				...
				else if (bot_id && datalist["status"] != bot_id)
					return

				bot_id = datalist["status"]
				bot_taskid = (datalist["curtask"] ? datalist["curtask"] : "NONE")
				bot_taskid_default = (datalist["deftask"] ? datalist["deftask"] : "NONE")
				bot_toolid = (datalist["tool"] ? datalist["tool"] : "NONE")
				bot_charge = (datalist["charge"] ? datalist["charge"] : "nocell")
				update_status()
	...
	proc
		...
		parse_record(var/computer/file/record/rec)
			if (!rec)
				return FALSE
			...
			if (lowertext(rec.name) == "command") //This is some sort of driver command from the user.
				...
				switch(lowertext(parsedFields["command"]))
					...
					if ("upload")//See if it's a set of parameters to bundle with a guardbot task. If so, send the configured task.
						if (bot_id)//Oh and um the target guardbot task should be placed in the device folder first (By the user)
							var/computer/file/guardbot_task/locatedTask = null
		...
		update_status()
			...
			statusfile.fields.len = 0
			if (bot_id)
				statusfile.fields["id"] = bot_id
				statusfile.fields["curtask"] = bot_taskid
				statusfile.fields["deftask"] = bot_taskid_default
				statusfile.fields["charge"] = bot_charge
				statusfile.fields["tool"] = bot_toolid
				status = bot_toolid
			else
	...
	add_file(var/computer/file/theFile)
		...
		if (istype(theFile, /computer/file/record))
			//contents_mirror += file
		...
		else if (istype(theFile, /computer/file/guardbot_task))
			contents_mirror += theFile
	...
	remove_file(var/computer/file/theFile)
...
/computer/file/mainframe_program/guardbot_interface
	name = "prman"
	size = 4
	...
	initialize(var/initparams)
		...
		if ((. > metadata["group"]) && (. != 0)) //User isn't sysop.
		...
		switch(command)
			...
			if ("stat")
				...
				var/computer/file/record/statrec = signal_program(1, list("command"=DWAINE_COMMAND_FGET,"path"="/mnt/_[lowertext(initlist[2])]/status"))
				if (!istype(statrec))
			...
			if ("upload")
				...
				var/computer/file/guardbot_task/task = signal_program(1, list("command"=DWAINE_COMMAND_FGET,"path"=initlist[3]))
				if (istype(task))
					//If we're uploading a task, first we need to hand the driver a copy of that task!
					var/computer/file/guardbot_task/taskCopy = task.copy_file()
					...
					var/computer/file/record/commandRec = new /computer/file/record(  )
					...
					if (initlist.len >= 4)
						if (initlist.len > 4 && cmptext(initlist[4], "-f"))
							...
							var/computer/file/record/configfile = signal_program(1, list("command"=DWAINE_COMMAND_FGET,"path"=initlist[5]))
			...
			if ("recall")
				...
				if (radioDriver & ESIG_DATABIT)
					...
					var/computer/file/record/sigFile = new
...
//Radio driver
/computer/file/mainframe_program/driver/mountable/radio
	name = "pr6_radio"
	setup_processes = 1
	...
	initialize(var/initparams)
		...
		signal_program(1, list("command"=DWAINE_COMMAND_MOUNT, "id"=name, "link"="radio"))
		return
	...
	terminal_input(var/data, var/computer/file/theFile)
		if (..() || !data)
			return TRUE
		...
		if (cmptext(dataList["command"],"register") && !initial_reqistry_complete)
			...
			for (var/newFreq in newFreqs)
				var/computer/folder/folder = new
				folder.name = newFreq
				add_file(folder)
		...
		var/computer/file/record/signalRecord = new
		...
		signalRecord.holder = holder

		for (var/computer/folder/radio_channel/rc in contents_mirror)
			if (cmptext(rc.name, dataList["_freq"]))
				rc.contents += signalRecord
				signalRecord.holding_folder = rc
				if (rc.contents.len > 32)
					var/computer/to_delete = rc.contents[1]
					if (to_delete)
						qdel(to_delete)
				return FALSE

		contents_mirror += signalRecord
		return FALSE
	...
	add_file(var/computer/file/theFile, var/freq)
		if (!initialized)
			return FALSE

		if (istype(theFile, /computer/file/record))
			if (istext(freq))
				var/computer/file/record/R = theFile
				if (R.fields.len)
		...
		else if (istype(theFile, /computer/folder))
			...
			for (var/computer/folder/otherFolder in contents_mirror)
				if (otherFolder.name == newFreqName)
					theFile.dispose()
					return FALSE

			theFile.dispose()
			theFile = new /computer/folder/radio_channel ( src )
	...
	remove_file(var/computer/file/file)
		...
		if (file in contents_mirror)
			if (istype(file, /computer/folder/radio_channel))
...
/computer/folder/radio_channel
	var/computer/file/mainframe_program/driver/mountable/radio/ourDriver

	New(var/computer/file/mainframe_program/driver/mountable/radio/newDriver)
		..()

		if (istype(newDriver))
			ourDriver = newDriver

	can_add_file(computer/file/R)
	...
	add_file(var/computer/file/theFile)
		...
		return ourDriver.add_file(theFile, name)
...
//IR detector driver
/computer/file/mainframe_program/driver/secdetector
	name = "ir_detect"
	setup_processes = 1
	status = 0
...
//APC driver
/computer/file/mainframe_program/driver/apc
	name = "pwr_cntrl"
	setup_processes = 1
	status = "0;0;0;0"
	var/tmp/apcID = null
...
//HEPT emitter
/computer/file/mainframe_program/driver/hept_emitter
	name = "hept_emit"
	setup_processes = 1
	status = 0
...
/computer/file/mainframe_program/hept_interface
	name = "heptman"
	size = 4
...
/computer/file/mainframe_program/h7init
	name = "init"
	size = 16
...
/computer/file/mainframe_program/driver/test_apparatus
	name = "test_appt"
	setup_processes = 1
	status = "UNKNOWN"
	var/tmp/active = 0
...
/computer/file/mainframe_program/driver/mountable/service_terminal
	name = "srv_terminal"
	setup_processes = 1
	status = "INVALID"
	var/userid = null
	...
	*/
	...
	initialize(initparams)
		if (..() || !istype(initparams, /computer/file/record))
			return

		var/computer/file/record/initRec = initparams
		if (!initRec.fields || !initRec.fields["userid"])// || !initRec.fields["service"])
			return

		userid = ckeyEx(initRec.fields["userid"])

		var/mainframe2_user_data/srvUser = new
		useracc = srvUser
		...
		if (initRec.fields["service"])
			var/computer/file/mainframe_program/exec = signal_program(1, list("command"=DWAINE_COMMAND_FGET, "path"="/sys/srv/[initRec.fields["service"]]"))
			if (istype(exec))
	...
	terminal_input(var/data, var/computer/file/theFile)
		...
		var/computer/file/mainframe_program/exec = signal_program(1, list("command"=DWAINE_COMMAND_FGET, "path"="/sys/srv/[command]"))
		if (istype(exec))
			...
			if (istype(theFile))
				var/computer/file/copy = theFile.copy_file()

				if (copy)
					copy.name = "tmp[copytext("\ref[copy]", 4, 12)]"
					contents_mirror += copy
					if (contents_mirror.len > 8)
						contents_mirror.Cut(1,2)
					siglist["args"] = siglist["args"] + " /mnt/_[name]/[copy.name]"
	...
	receive_progsignal(var/sendid, var/list/data = null, var/computer/file/file)
		if (..())
			return ESIG_GENERIC
		...
		if (data["command"]  == DWAINE_COMMAND_MSG_TERM)
			if (findtext(data["render"],"record"))
				...
				var/dataMessage = ""
				var/computer/file/record/messageRec = null
				if (dataList.len)
				...
				if (dataList.len) //Cutting out the first term may have left us with a rather brief list (Of nothing)
					messageRec = new /computer/file/record( )
...
/computer/file/mainframe_program/srv/print
	name = "print"
	size = 1
	...
	initialize(var/initparams)
		...
		switch (command)
			if ("index")
				...
				var/computer/folder/mount = signal_program(1, list("command"=DWAINE_COMMAND_FGET,"path"="/mnt"))
				if (istype(mount))
					var/response = "print_index"
					for (var/computer/potentialPrinter in mount.contents)
			...
			if ("status")
				...
				if (initlist.len > 1)
					...
					var/computer/file/record/printerStatus = signal_program(1, list("command"=DWAINE_COMMAND_FGET,"path"="/mnt/lp-[printerName]/status"))
					var/theStatus = "???"
			...
			if ("print")
				if (initlist.len > 2)
					...
					var/computer/file/record/toPrintFile = signal_program(1, list("command"=DWAINE_COMMAND_FGET,"path"=toPrintPath))
			...
			if ("printall")
				if (initlist.len > 1)
					...
					var/computer/file/record/toPrintFile = signal_program(1, list("command"=DWAINE_COMMAND_FGET,"path"=toPrintPath))
					if (istype(toPrintFile))
						var/computer/folder/mnt = signal_program(1, list("command"=DWAINE_COMMAND_FGET,"path"="/mnt"))
						if (istype(mnt))
							var/failure = 0
							for (var/computer/folder/printFolder in mnt.contents)
...
/computer/file/mainframe_program/driver/mountable/comm_dish
	name = "com_array"
	setup_processes = 1
	var/tmp/computer/file/record/statusfile = null
	...
	initialize(var/initparams)
		...
		//signal_program(1, list("command"="mount", "id"=name, "link"="comm"))
	...
	terminal_input(var/data, var/computer/file/record/recfile)
		if (..() || !initialized)
			return
...
*/
			...
			if ("remove_report")
				...
				for (var/computer/file/record/deletion_candidate in contents_mirror)
