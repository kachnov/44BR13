...
/computer/file/mainframe_program/srv/email
	name = "mail"
	size = 1
		...
		switch (command)
			...
			if ("get")
				...
				if (initlist.len > 1)
					...
					var/computer/file/record/targetMail = mailList[index]
			...
			if ("send")
				if (initlist.len > 1)
					...
					var/computer/file/record/sentMail = signal_program(1, list("command"=DWAINE_COMMAND_FGET, "path"="[sentMailPath]"))
					if (istype(sentMail))
	...
	proc
		format_outgoing_mail(var/computer/file/record/mail)
			if (!istype(mail) || !mail.fields || (mail.fields.len < 6))
				return null

			var/computer/file/record/newMail = mail.copy_file()
		...
		send_mail_to(var/computer/file/record/mail)
			...
			var/computer/folder/mailgroupTable = null
		...
		list_mail_for (var/target, var/mailgroup=null)//"*all")
			...
			var/computer/folder/mailFolder = signal_program(1, list("command"=DWAINE_COMMAND_FGET, "path"="[setup_email_folder]"))
			if (!istype(mailFolder))
				return mailList

			var/computer/file/record/mailgroupTable = null
			...
			for (var/computer/file/record/emailRec in mailFolder.contents)
