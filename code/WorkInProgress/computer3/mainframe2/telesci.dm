...
/obj/machinery/networked/telepad
	...
	examine()
		...
		..()
		if (!src.host_id)
			boutput(usr, "<span style=\"color:red\">The [name]'s \"disconnected from host\" light is flashing.</span>")
	...
	receive_signal(signal/signal)
		...
		switch(sigcommand)
			...
			if ("term_message","term_file")
				...
				switch(data["command"])
					if ("set_coords")
						var/computer/file/coords/coords = signal.data_file
						if (!istype(coords))
							message_host("command=nack")
							return

						realx = round(  max(0, min(coords.destx, world.maxx+1)) )
						realy = round(  max(0, min(coords.desty, world.maxy+1)) )
						realz = round(  max(0, min(coords.destz, world.maxz+1)) )
					...
					if ("send")
						...
						spawn (0)
							...
							if (!istype(turfcheck))
								badsend()
								if (isnum(turfcheck))
									message_host("command=nack&cause=bad[turfcheck & 1 ? "x" : null][turfcheck & 2 ? "y" : null][turfcheck & 4 ? "z" : null]")
								else
									message_host("command=nack&cause=badxyz")
							else if (!is_allowed(turfcheck))
								message_host("command=nack&cause=interference")
							else
								message_host("command=ack")
								sleep(10)
								send(turfcheck)
					...
					if ("scan")
						...
						if (!scanTurf)
							...
							if (isnum(scanTurf))
								message_host("command=nack&cause=bad[scanTurf & 1 ? "x" : null][scanTurf & 2 ? "y" : null][scanTurf & 4 ? "z" : null]")

							else if (!istype(scanTurf, /turf/space))
								var/gas_mixture/GM = scanTurf.return_air()
								var/burning = 0
	...
	proc/message_host(var/message, var/computer/file/file)
	...
	proc/send(var/turf/target)
		...
		var/list/stuff = list()
		for (var/atom/movable/O as obj|mob in loc)
		...
		if (stuff.len)
			...
			if (ismob(which))
				logTheThing("station", usr, which, "sent %target% to [showCoords(target.x, target.y, target.z)] from [showCoords(x, y, z)] with a telepad")
		...
		use_power(1500)
		if (prob(2) && prob(2))
			visible_message("<span style=\"color:red\">The console emits a loud pop and an acrid smell fills the air!</span>")
			XSUBTRACT = rand(0,100)
			YSUBTRACT = rand(0,100)
			ZSUBTRACT = rand(0,world.maxz)
			spawn (10)
	...
	proc/receive(var/turf/receiveturf)
		...
		if (stuff.len)
			var/atom/movable/which = pick(stuff)
			if (ismob(which))
				logTheThing("station", usr, which, "received %target% from [showCoords(which.x, which.y, which.z)] to [showCoords(x, y, z)] with a telepad")
			which.set_loc(loc)
		showswirl(loc)
		leaveresidual(loc)
	...
	proc/doubleportal(var/turf/target)
		...
		for (var/atom/movable/O in send)
			O.set_loc(target)
			if (ismob(O))
				logTheThing("station", usr, O, "sent %target% to [showCoords(target.x, target.y, target.z)] from [showCoords(x, y, z)] with a telepad")

		for (var/atom/movable/O in receive)
			if (ismob(O))
				logTheThing("station", usr, O, "received %target% from [showCoords(O.x, O.y, O.z)] to [showCoords(x, y, z)] with a telepad")
			O.set_loc(loc)
		showswirl(loc)
		showswirl(target)
		use_power(500000)
		...
		if (prob(5) && !locate(/obj/dfissure_to) in get_step(src, EAST))
			...
			start_portal = makeportal(loc, target)
			if (start_portal)
				end_portal = makeportal(target, loc)
	...
	proc/processbadeffect(var/effect)
		switch(effect)
			...
			if ("scatter") //stolen from hand tele, heh
				...
				for (var/turf/T in orange(5,loc))
			...
			if ("rads")
				...
				return
			if ("fire")
				fireflash(loc, 6) // cogwerks - lowered from 8, too laggy
			...
			if ("widescatter")
				var/list/turfs = new
				var/turf/target = null
				for (var/turf/T in orange(30,loc))
					if (T.x>world.maxx-4 || T.x<4)	continue
					if (T.y>world.maxy-4 || T.y<4)	continue
					if (is_allowed(T))
						turfs += T
			...
			if ("areascatter")
				var/list/turfs = new
				var/turf/target = null
				for (var/turf/T in orange(10,loc))
					if (T.x>world.maxx-4 || T.x<4)	continue
					if (T.y>world.maxy-4 || T.y<4)	continue
					if (is_allowed(T))
						turfs += T
...
/obj/machinery/networked/teleconsole
	...
	var/readout = "&nbsp;"
	var/computer/file/record/user_data
	var/padNum = 1
	...
	receive_signal(signal/signal)
		...
		switch(sigcommand)
			if ("term_connect") //Terminal interface stuff.
				...
				if (signal.data["data"] != "noreply" && data_link)
					//post_status(target, "command","term_connect","data","noreply","device",device_tag)
					...
					newsignal.data["device"] = device_tag
			...
			if ("term_message","term_file")
				...
				if (message)
					...
					readout = copytext(message,9,256)
	...
	attack_hand(var/mob/user as mob)
		...
		if (!host_id)
			dat += "<center id = \"readout\"><tt><font color=red><strong>NO CONNECTION TO HOST</strong></font></tt><br><a href='?src=\ref[src];reconnect=1'>Retry</a></center><br>"
		else
	...
	function updateReadout(t)
	{
		...
		dat += "<strong>Target Coordinates</strong><BR>"
		...
		if (panel_open)
			dat += "<br>Linked Pad Number: <a href='?src=\ref[src];setpad=1'>[padNum]</a><br>"
			dat += net_switch_html()
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/screwdriver))
			...
			boutput(user, "You [panel_open ? "unscrew" : "secure"] the cover.")
			updateUsrDialog()
			return
	...
	updateUsrDialog(var/updateReadout)
		...
		for (var/mob/M in nearby)
			if ((M.client && M.machine == src))
				if (updateReadout)
					M << output(url_encode(readout), "t_computer.browser:updateReadout")
				else
					attack_hand(M)
		...
		if (istype(usr, /mob/living/silicon))
			if (!(usr in nearby))
				if (usr.client && usr.machine==src)
					if (updateReadout)
						usr << output(url_encode(readout), "t_computer.browser:updateReadout")
					else
						attack_ai(usr)
	...
	Topic(href, href_list)
		...
		if (href_list["reconnect"])
			if ((host_id && href_list["reconnect"] != "2") || !old_host_id || !data_link)
				return
