...
/computer/file/mainframe_program/test_interface
	name = "gptio"
	size = 8
...
/computer/file/mainframe_program/driver/artifact_console
	name = "artconsol"
	setup_processes = 1
		...
		switch(sigcommand)
			if ("term_connect") //Terminal interface stuff.
				if (target == src.host_id)
					src.host_id = null
					entries[5] = "No Connection"
					updateUsrDialog(REASON_ALERT)
					spawn (3)
				...
				if (message_list)
					...
					switch (lowertext(message_list["command"]))
						if ("alert")	//It wants us to display a single message in the center of the screen.
							entries[5] = copytext(message_list["msg"], 1, 65)
							highlightMap = 0

							if (message_list["title"])
								screen_title = copytext(message_list["title"], 15)
								updateUsrDialog(REASON_ALERT | REASON_TITLE)
						...
						if ("message")
							...
							for (var/entryInfo in splittext(message_list["data"], "|n"))
								if (++entryOffset > entries.len)
									break

								if (dd_hasprefix(entryInfo, "|h1" ))
									entries[entryOffset] = copytext(entryInfo, 4, 69)
									highlightMap |= (1<<(entryOffset-1))
									...
									entries[entryOffset] = copytext(entryInfo, 1, 65)
							...
							if (message_list["title"])
								...
								updateUsrDialog(REASON_FIELDS | REASON_HIGHLIGHT | REASON_TITLE)
							else
								updateUsrDialog(REASON_FIELDS|REASON_HIGHLIGHT)
						...
						if ("highlight")
							...
							if (message_list["state"] == "1")
								if (message_list["mode"] == "single")
									highlightMap = (1<<lineToAdjust)
								else
									highlightMap |= (1<<lineToAdjust)
							else
								highlightMap &= ~(1<<lineToAdjust)

							updateUsrDialog(REASON_HIGHLIGHT)

						if ("title")
							if (message_list["title"])
								screen_title = copytext(message_list["title"], 1, 15)
								updateUsrDialog(REASON_TITLE)
...
<body scroll=no><br>
		...
		dat += "<div id='entry7' class='consolelog_entry'>[entries[entries.len]]</div>"
...
"}
	...
	Topic(href, href_list)
		...
		if (href_list["button"])
			...
			DEBUG_OUT(1)
			if ((host_id && href_list["reset"] != "2") || !old_host_id || !data_link)
				DEBUG_OUT(2)
				return
			...
			post_status(old, "command","term_disconnect")
			sleep(5)
			DEBUG_OUT(6)
			post_status(old, "command", "term_connect", "device", device_tag)
		...
		for (var/mob/M in nearby)
			...
			if ((M.client && M.machine == src))
				...
				if (forceUpdate || entryUpdateFlags)
					DEBUG_OUT("[M] ! !  !")
					dynamicUpdate(M, forceUpdate|entryUpdateFlags)
		...
		if (istype(usr, /mob/living/silicon))
			if (!(usr in nearby))
				if (usr.client && usr.machine==src)
					if (forceUpdate || entryUpdateFlags)
						dynamicUpdate(usr, forceUpdate|entryUpdateFlags)
						entryUpdateFlags = REASON_NONE
					else
						attack_ai(usr)
