...
/computer/file/mainframe_program/shell
	name = "Msh"
	size = 8
	executable = 0
	var/tmp/piping = 0
...
#define ERR_UNDEFINED -3
	...
	input_text(var/text, var/scripting = 0)
		...
		while (piped_list.len && (script_counter < MAX_SCRIPT_ITERATIONS))
			...
			var/list/command_list = parse_string(text, (scripting ? scriptvars : null))
			...
			if (execpath("/bin/[command]", current, command, command_list, scripting ? script_iteration : 0))
				...
				switch(lowertext(command))
					...
					if ("if")
						...
						switch (script_evaluate2(command_list, 1))
							...
							if (0)
								...
								return FALSE //Move to the next line of the script, dropping any following commands on this line.
							if (null)
								return TRUE
					...
					if ("help", "man")
						var/computer/file/record/helpRec = signal_program(1, list("command"=DWAINE_COMMAND_CONFGET,"fname"=setup_helprec_name))
						if (istype(helpRec))
					...
					if ("logout","logoff")
						...
						if (pipetemp)
							...
							var/computer/file/record/rec = new /computer/file/record(  )
	...
	proc
		execpath(var/fpath, var/current, var/command, var/list/command_list, var/scripting=0)
			...
			var/computer/file/record/exec = signal_program(1, list("command"=DWAINE_COMMAND_FGET, "path"=fpath))
			if (istype(exec, /computer/file/mainframe_program))
			...
			spawn (0)
				while (shscript.len)
					...
					for (var/i = 1, i <= 5, i++)
						...
						//Input_text should return TRUE on an error, so break if we get that, I guess.
						if (input_text(shscript[1], script_iteration+1))
				...
				switch ( lowertext(current_command) )
					...
					if ("d","e","f","x")
						...
						if (!istype(., /computer))
							...
							. = istype(., /computer/folder)

						else if (cmptext(current_command, "x"))
							. = (istype(., /computer/file/mainframe_program))// || (istype(., /computer/file/record) && .:fields && .:fields[1] == "#!"))

						else if (cmptext(current_command, "f"))
							. = istype(., /computer/file)
	...
	receive_progsignal(var/sendid, var/list/data, var/computer/file/theFile)
		if (..())
			return ESIG_GENERIC
