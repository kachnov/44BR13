...
/computer/file/record/accesslog_default_config
	name = "accesslog"
	New()
		..()
...
.conn-error {
	background-color: #888888;
		...
		if (!refreshing)
			if (machine_screen == 1)
				dat += "<strong>Connection</strong> | <a href='?src=\ref[src];screen=2'>Data</a>"
			else if (machine_screen == 2)
				dat += "<a href='?src=\ref[src];screen=1'>Connection</a> | <strong>Data</strong>"
			else if (!refreshing)
			...
			if (machine_screen == 1)
				dat += "<strong>Host Connection:</strong>"
				...
				if (spoofed)
					dat += "<em>Warning: could not verify authenticity of displayed data.</em><br>"
				if (timed_out)
					dat += "<em>Warning: last request to refresh records timed out with no response.</em><br>"
				if (spoofed || timed_out)
					dat += "<br>"
				dat += "<strong>Filters</strong><br>"
				dat += "<strong>Card name (partial):</strong> <a href='?src=\ref[src];card_name=1'>[filter_name ? filter_name : "&lt;not set&gt;"]</a>[filter_name ? " <a href='?src=\ref[src];card_name_clear=1'>\[X\]</a>" : null]<br>"
				dat += "<strong>Time interval:</strong> "
				...
				dat += "<br>"
				dat += "<strong>Action:</strong> <a href='?src=\ref[src];action=1'>[filter_action ? filter_action : "&lt;not set&gt;"]</a>[filter_action ? " <a href='?src=\ref[src];action_clear=1'>\[X\]</a>" : null]<br>"
				dat += "<strong>Door net ID:</strong> <a href='?src=\ref[src];door_id=1'>[filter_door_id ? filter_door_id : "&lt;not set&gt;"]</a>[filter_door_id ? " <a href='?src=\ref[src];door_id_clear=1'>\[X\]</a>" : null]<br>"
				dat += "<a href='?src=\ref[src];refresh=1'>Refresh records</a><br><br>"

				dat += "<strong>Record listing: </strong><br>"
				if (!records.len)
					dat += "<em>No records currently loaded. Refresh the records to load data.</em><br>"
				else
					for (var/rec in records)
						dat += "[rec]<br>"
		else if (refreshing)
			dat += "<em>Refreshing data, please wait...</em>"
	...
	receive_signal(signal/signal)
		...
		if (lowertext(signal.data["address_1"]) != lowertext(net_id))
		...
		switch(sigcommand)
			...
			if ("term_message","term_file")
				...
				var/computer/file/archive/archive = signal.data_file
				if (!istype(archive))
					post_status(target,"command","term_message","data","command=status&status=failure")
					return

				records.len = 0
				for (var/computer/file/record/R in archive.contained_files)
					var/bad = 0
...
/proc/accesslog_digest(var/computer/file/record/R, formatted = 0)
	if (!istype(R))
	...
	if (action != "reject")
		if (!formatted)
			...
			return "<strong>[time_t]</strong> [card_name] <strong>[action]s</strong> [door_name]"
	else
		if (!formatted)
			return "[time_t] [door_name] rejected entry for [card_name]"
		else
			return "<strong>[time_t]</strong> [door_name] <strong>rejected entry for</strong> [card_name]"

/computer/file/mainframe_program/srv/accesslog
	name = "accesslog"
	size = 1
	...
	initialize(var/initparams)
		...
		var/computer/file/record/conf_file = signal_program(1, list("command"=DWAINE_COMMAND_FGET, "path"="/etc/accesslog"))
		if (istype(conf_file))
		...
		if (opts["a"])
			if (opts["s"] && opts["t"] && opts["m"] && params.len)
				...
				var/computer/file/record/logfile = new()
				...
				var/computer/folder/logs_dir = signal_program(1, list("command"=DWAINE_COMMAND_FGET, "path"=log_to))
				if (istype(logs_dir))
					var/idx = 0
					while (logs_dir.contents.len >= ACCESSLOG_RECORDS_LIMIT)
						...
						if (idx > logs_dir.contents.len)
							...
							mainframe_prog_exit
							return
						var/computer/to_delete = logs_dir.contents[idx]
						if (istype(to_delete, /computer/file/record))
			...
			if (opts["t"])
				...
				if (!isnum(time_begin) || !isnum(time_end))
					...
					mainframe_prog_exit
					return
			var/computer/folder/logs_folder = signal_program(1, list("command"=DWAINE_COMMAND_FGET, "path"=log_to))
			...
			if (!records.len)
				...
				var/max = -1
				var/computer/file/record/C = null
				for (var/Q in records)
					if (!istype(Q, /computer/file/record))
						records -= Q
						continue
					var/computer/file/record/R = Q
				...
				while (printed < count && records.len)
					max = -1
					for (var/computer/file/record/R in records)
						var/TS = R.fields["timestamp"]
						if (!isnum(TS))
							TS = text2num(TS)
							R.fields["timestamp"] = TS
				...
				for (var/i = printing.len, i >= 1, i--)
					var/computer/file/record/R = printing[i]
					if (opts["f"])
...
/computer/file/mainframe_program/driver/mountable/logreader
	name = "logreader"
	setup_processes = 1
	...
	initialize(var/initparams)
		...
		signal_program(1, list("command"=DWAINE_COMMAND_MOUNT, "id"=name, "link"="logreader"))
		return
	...
	terminal_input(var/data, var/computer/file/theFile)
		...
		switch (lowertext(dataList["command"]))
			if ("record_query")
				records.len = 0
				var/computer/file/mainframe_program/P = signal_program(1, list("command"=DWAINE_COMMAND_FGET, "path"="/sys/srv/accesslog"))
				if (istype(P))
					...
					if (!archive_path)
						return TRUE
					var/computer/file/archive/archive = signal_program(1, list("command"=DWAINE_COMMAND_FGET, "path"="[archive_path]"))
					if (!istype(archive))
						return TRUE
	...
	add_file(var/computer/file/theFile)
		...
		if (istype(theFile, /computer/file/archive))
			var/computer/file/archive/R = theFile
