...
//List directory
/computer/file/mainframe_program/utility/ls
	name = "ls"
	size = 1
	...
	initialize(var/initparams)
		...
		var/computer/folder/listfolder = signal_program(1, list("command"=DWAINE_COMMAND_FGET,"path"=initparams))

		if (istype(listfolder))
			var/message
			for (var/computer/P in listfolder.contents)
				...
				if (descriptive)
					...
					var/is_folder = istype(P, /computer/folder)
			...
			if (descriptive && istype(listfolder, /computer/file))
				var/computer/file/P = listfolder
	...
	proc/print_file_permissions(var/computer/cdatum)
		if (!cdatum)
			return
...
/computer/file/mainframe_program/utility/cd
	name = "cd"
	size = 1
...
*/
		...
		var/computer/folder/checkfolder = signal_program(1, list("command"=DWAINE_COMMAND_FGET,"path"=initparams))
...
/computer/file/mainframe_program/utility/rm
	name = "rm"
	size = 1
	...
	initialize(var/initparams)
		...
		var/computer/checkdatum = signal_program(1, list("command"=DWAINE_COMMAND_FGET,"path"=initparams))
		if (!istype(checkdatum))
			message_user("Error: Invalid path.")
		else
			if (istype(checkdatum, /computer/folder) && !recursive)
	...
	input_text(var/text)
		...
		if (command == "yes" || command == "y")
			var/computer/checkdatum = signal_program(1, list("command"=DWAINE_COMMAND_FGET,"path"=target_path))
			if (!istype(checkdatum))
...
//Copy files
/computer/file/mainframe_program/utility/cp
	name = "cp"
	size = 1
	...
	initialize(var/initparams)
		...
		var/computer/file/prototype = signal_program(1, list("command"=DWAINE_COMMAND_FGET, "path"=initlist[1]))
		if (!istype(prototype))
		...
		if (dest_check != ESIG_NOFILE)
			if (istype(dest_check, /computer/folder) && !get_computer_datum(prototype.name, dest_check))
		...
		var/computer/file/copy = prototype.copy_file()
		copy.name = copyname
...
//Move files
/computer/file/mainframe_program/utility/mv
	name = "mv"
	size = 1
...
//Concatenate...stuff
/computer/file/mainframe_program/utility/cat
	name = "cat"
	size = 1
	...
	initialize(var/initparams)
		...
		while (initlist.len)
			...
			var/computer/file/currfile = signal_program(1, list("command"=DWAINE_COMMAND_FGET,"path"=initlist[1]))
			initlist -= initlist[1]
			if (!istype(currfile))
				break
...
/*
			if (istype(currfile, /computer/file/record))
			...
			else if (istype(currfile, /computer/file/text))
...
//Link folders
/computer/file/mainframe_program/utility/ln
	name = "ln"
	size = 1
	...
	initialize(var/initparams)
		...
		var/computer/folder/target_folder = signal_program(1, list("command"=DWAINE_COMMAND_FGET,"path"=initlist[1]))
		...
		var/computer/folder/link/symlink = new /computer/folder/link(target_folder)
...
/computer/file/mainframe_program/utility/mkdir
	name = "mkdir"
	size = 1
	...
	initialize(var/initparams)
		...
		while (initlist.len)
			...
			var/computer/folder/new_folder = new /computer/folder(  )
...
/computer/file/mainframe_program/utility/chmod
	name = "chmod"
	size = 1
...
/computer/file/mainframe_program/utility/chown
	name = "chown"
	size = 1
...
/computer/file/mainframe_program/utility/su
	name = "su"
	size = 1
...
/computer/file/mainframe_program/utility/mount
	name = "mount"
	size = 1
...
/computer/file/mainframe_program/utility/grep
	name = "grep"
	size = 1
	...
	initialize(var/initparams)
		...
		if (initlist.len > 1)
			...
			for (var/i = 2, i <= initlist.len, i++)
				...
				var/computer/file/to_check = signal_program(1, list("command"=DWAINE_COMMAND_FGET,"path"=initlist[i]))
				if (!istype(to_check))
					break


				//WIP
				if (istype(to_check, /computer/file/record))
...
/computer/file/mainframe_program/utility/scnt
	name = "scnt"
	size = 1
...
/computer/file/mainframe_program/utility/getopt
	name = "getopt"
	size = 1
	var/err = null
...
//
/computer/file/mainframe_program/utility/date
	name = "date"
	size = 1
	var/opt_data
...
/computer/file/mainframe_program/utility/tar
	name = "tar"
	size = 3
	var/opt_data
	...
	proc/recursive_list(var/computer/F, cpath = "", depth = 0)
		if (depth >= 8)
			message_user("tar: Stack overflow.")
			return null
		message_reply_and_user("[cpath][F.name]")
		if (istype(F, /computer/folder))
			var/computer/folder/FO = F
			var/lpath = "[cpath][FO.name]/"
			for (var/computer/C in FO.contents)
				recursive_list(C, lpath, depth + 1)

	proc/recursive_extract(var/computer/C, var/computer/folder/F, target, var/list/opts, cpath = "", depth = 0)
		if (depth >= 8)
			if (!opts["q"])
				message_user("tar: Stack overflow.")
			return null
		var/computer/T = signal_program(1, list("command"=DWAINE_COMMAND_FGET, "path"="[target][C.name]"))
		if (opts["v"])
	...
	proc/deep_copy(var/computer/C, var/list/opts, var/cpath = "", var/depth = 0)
		if (depth >= 8)
			if (!opts["q"])
				message_user("tar: Stack overflow.")
			return null
		if (istype(C, /computer/file/archive))
			if (!opts["q"])
				message_user("tar: Cannot handle file [cpath][C]")
			return null
		if (istype(C, /computer/folder))
			var/computer/folder/F = new()
			for (var/computer/C2 in C:contents)
				var/dcpath = "[cpath][C.name]/"
				var/computer/NC = deep_copy(C2, opts, dcpath, depth + 1)
				if (istype(NC))
					F.add_file(NC)
			return F
		else if (istype(C, /computer/file))
	...
	initialize(var/initparams)
		...
		if (initparams)
			...
			if (opts["l"] || opts["x"])
				var/computer/file/archive/archive = signal_program(1, list("command"=DWAINE_COMMAND_FGET,"path"=arcfile))
				if (!istype(archive))
				...
				if (opts["l"])
					for (var/computer/F in archive.contained_files)
						recursive_list(F, "")
				else if (opts["x"])
					var/target = curpath
					if (unaff.len)
						target = abspath(unaff[1], curpath)
					var/computer/folder/F = signal_program(1, list("command"=DWAINE_COMMAND_FGET,"path"=target))
					if (!istype(F))
					...
					if (copytext(target, length(target)) != "/")
						target += "/"
					for (var/computer/C in archive.contained_files)
				...
				for (var/path in unaff)
					var/computer/C = signal_program(1, list("command"=DWAINE_COMMAND_FGET, "path"=abspath(path, curpath)))
					if (!istype(C))
						message_user("tar: File [path] does not exist.")
						mainframe_prog_exit
						return
					var/computer/C2 = deep_copy(C, opts)
					if (!istype(C2))
