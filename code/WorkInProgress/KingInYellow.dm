...
/obj/kingyellow_phantom
	...
	proc/vanish()
		new/obj/kingyellow_vanish(loc, target)
		spawn (3)	qdel(src)
...
/obj/item/book_kinginyellow
	...
	suicide(var/mob/user as mob)
		if (ishuman(user))
			...
			if (!farting_allowed)
				return FALSE
			user.visible_message("<span style=\"color:red\">[user] farts on [src].<br><strong>A mysterious force sucks [user] into the Book!!</strong></span>")
			user.u_equip(src)
			layer = initial(layer)
			set_loc(user.loc)
			if (user.gender == MALE)
