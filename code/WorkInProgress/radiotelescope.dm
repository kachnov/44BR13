var/telescope_manager/tele_man = null
var/list/special_places = list() //associative List of name=/computer/file/coords that can not be reached normally. For use with the telescope
...
/obj/lrteleporter
	...
	attack_hand(mob/user as mob)
		...
		if (special_places.len)
			for (var/A in special_places)
				var/computer/file/coords/C = special_places[A]
	...
	Topic(href, href_list)
		if (busy) return
		if (get_dist(usr, src) > 1 || usr.z != z) return

		if (href_list["send"])
			var/computer/file/coords/C = locate(href_list["send"])
		...
		if (href_list["recieve"])
			var/computer/file/coords/C = locate(href_list["recieve"])
			var/turf/target = locate(C.destx, C.desty, C.destz)
			if (target)
				busy = 1
				flick("lrport1", src)
...
/telescope_manager
	...
	proc/setup()
		var/types = (subtypesof(/telescope_event))
		for (var/x in types)
			var/telescope_event/R = new x(src)
...
/telescope_event/geminorum
	...
	onActivate(var/obj/machinery/computer/telescope/T)
		...
		if (!special_places.Find(name))
			...
			var/computer/file/coords/C = new()
			C.destx = 255
			C.desty = 3
			C.destz = 2
		...
		switch(content)
			if ("EventList")
				if (tele_man)
					for (var/A in tele_man.events_active)
						var/telescope_event/E = tele_man.events_active[A]
					...
					for (var/A in tele_man.events_found)
						var/telescope_event/E = tele_man.events_found[A]
			...
			if ("Starmap")
				...
				if (tele_man && tele_man.events_found.len)
					for (var/A in tele_man.events_found)
						var/telescope_event/E = tele_man.events_found[A] //Clicking on the icons doesnt work.
			...
			switch(href_list["jscall"])
				if ("contact")
					if (tele_man.events_found.Find(tracking_id))
						var/telescope_event/E = tele_man.events_found[tracking_id]
						var/html = ""
				...
				if ("trackId")
					...
					if (tele_man && tracking_id)
						if (tele_man.events_active.Find(tracking_id))
							var/telescope_event/E = tele_man.events_active[tracking_id]
