...
var/particleMaster/particleMaster = new
...
/particleMaster
	...
	New()
		...
		for (var/ptype in subtypesof(/particleType))
			var/particleType/typeDatum = new ptype()
			particleTypes[typeDatum.name] = typeDatum

	proc/SpawnSystem(var/particleSystem/system)
		if (!istype(system))
			return
	...
	proc/CheckSystemExists(var/systemType, var/atom/location)
		for (var/particleSystem/system in particleSystems)
	...
	proc/SpawnParticle(var/atom/location, var/particleTypeName = null, var/particleTime = null, var/particleColor = null, var/atom/target = null, var/particleSprite = null) //This should be the only thing you access from the outside.
		var/particleType/pType = particleTypes[particleTypeName]
...
/particleType/warp_star
	...
	Apply(var/obj/particle/par)
		if (..())
			par.dir = star_direction
			if (prob(40))
				par.icon_state = "starlarge"

			if (star_direction & NORTH|SOUTH)
				par.pixel_x += rand(-240,240)
			if (star_direction & EAST|WEST)
...
/particleType/warp_star/warp_star_s
	name = "warp_star_s"
	star_direction = SOUTH

/particleType/warp_star/warp_star_e
	name = "warp_star_e"
	star_direction = EAST

/particleType/warp_star/warp_star_w
	name = "warp_star_w"
	star_direction = WEST
...
/particleSystem/swoosh/endless
	endless = 1

/particleSystem/swoosh
	var/endless = 0
	New(var/atom/location = null)
...
/particleSystem/areaSmoke/blueTest
	New(var/atom/location = null)
...
/particleSystem/radevent_warning
	New(var/atom/location = null)
...
/particleSystem/chemSmoke
	...
	New(var/atom/location = null, var/reagents/source, var/duration = 100, var/size = 3)
		smoke_size = size
		...
		if (source)
			affected = list()
			copied = new/reagents(source.maximum_volume)
...
/particleSystem/chemspray
	var/runLength = 10
	var/reagents/copied = null

	New(var/atom/location, var/atom/destination, var/reagents/source)
