/obj/item/shipcomponent/sensor
	...
	opencomputer(mob/user as mob)
		...
		if (active)
			...
			dat += {"<HR><strong>[ships] Ships Detected:</strong><BR>"}
			if (shiplist.len)
				for (var/shipname in shiplist)
					dat += {"<HR>[shipname] | "}
			dat += {"<HR>[lifeforms] Lifeforms Detected:</strong><BR>"}
			if (lifelist.len)
	...
	proc/scan(mob/user as mob)
		...
		ship.visible_message("<strong>[ship] begins a sensor sweep of the area.</strong>")
		boutput(usr, "<span style=\"color:blue\">Scanning...</span>")
		sleep(30)
		boutput(usr, "<span style=\"color:blue\">Scan complete.</span>")
		for (var/mob/living/C in range(seekrange,ship.loc))
			if (C.stat != 2)
				lifeforms++
				lifelist += C.name
		for (var/obj/critter/C in range(seekrange,ship.loc))
			if (C.alive && !istype(C,/obj/critter/gunbot))
				lifeforms++
				lifelist += C.name
		for (var/obj/npc/C in range(seekrange,ship.loc))
			if (C.alive)
				lifeforms++
				lifelist += C.name
		for (var/obj/machinery/vehicle/V in range(seekrange,ship.loc))
			if (V != ship)
				ships++
				shiplist += "[V.name] [dir_name(get_dir(ship, V))]"
		for (var/obj/critter/gunbot/drone/V in range(seekrange,ship.loc))
			ships++
			shiplist += "[V.name] [dir_name(get_dir(ship, V))]"
		updateDialog()
		sleep(10)
