...
/obj/item/shipcomponent/secondary_system/cloak
	...
	opencomputer(mob/user as mob)
		...
		if (active)
			dat+=  {"<strong>SYSTEM ONLINE</strong>"}
		else
...
/obj/item/shipcomponent/secondary_system/orescoop
	...
	opencomputer(mob/user as mob)
		...
		var/dat = "<TT><strong>[src] Console</strong><BR><HR><BR>"
		dat += {"<BR><strong>Capacity: [contents.len]/[capacity]</strong><HR>"}
		user << browse(dat, "window=ship_sec_system")
		onclose(user, "ship_sec_system")
		return
...
/obj/item/shipcomponent/secondary_system/cargo
	...
	opencomputer(mob/user as mob)
		...
		var/dat = "<TT><strong>[src] Console</strong><BR><HR><BR>"
		dat+=  {"<BR><strong>Current Contents:</strong><HR>"}
		for (var/cargoitem in load)
			dat += "<BR><strong>[cargoitem]</strong><HR>"
			dat += "<BR>"
		user << browse(dat, "window=ship_sec_system")
		onclose(user, "ship_sec_system")
		return
	...
	Clickdrag_ObjectToPod(var/mob/living/user,var/atom/A)
		...
		switch(load(A))
			...
			if (0)
				// success
				visible_message("<span style=\"color:blue\">[user] loads the [A] into [src]'s cargo bay.</span>")
				return
	...
	proc/load(var/atom/movable/C)
		...
		for (var/X in acceptable)
			if (istype(C,X))
				acceptable_cargo = 1
				break
		if (!acceptable_cargo)
			playsound(loc, "sound/machines/buzz-sigh.ogg", 50, 0)
			return TRUE // invalid cargo
...
/obj/item/shipcomponent/secondary_system/tractor_beam
	...
	run_component()
		if (settingup)
			return
		if (target in view(seekrange,ship.loc))
	...
	activate()
		...
		for (var/atom/movable/a in view(seekrange,ship.loc))
			if (!a.anchored)
				targets += a
	...
	opencomputer(mob/user as mob)
		...
		var/dat = "<TT><strong>[src] Console</strong><BR><HR><BR>"
		if (active)
			dat+=  {"<BR><strong>Current Target</strong>: [target]"}
		else
			dat += {"<strong><span style=\"color:red\">SYSTEM OFFLINE</span></strong>"}
		user << browse(dat, "window=ship_sec_system")
		onclose(user, "ship_sec_system")
...
/obj/item/shipcomponent/secondary_system/repair
	...
	opencomputer(mob/user as mob)
		...
		var/dat = "<TT><strong>[src] Console</strong><BR><HR><BR>"
		if (active)
			dat+=  {"<strong>Current Ammo</strong>:[ammo]/[maxammo]"}
		else
			dat += {"<strong><span style=\"color:red\">SYSTEM OFFLINE</span></strong>"}
		user << browse(dat, "window=ship_sec_system")
		onclose(user, "ship_sec_system")
...
/obj/item/shipcomponent/secondary_system/gps
	...
	opencomputer(mob/user as mob)
		...
		var/dat = "<TT><strong>[src] Console</strong><BR><HR><BR>"
		dat+=  {"<BR><strong>Located at:</strong><HR>
			<strong>X</strong>: [ship.x]<BR><strong>Y</strong>: [ship.y]"}
		user << browse(dat, "window=ship_sec_system")
		onclose(user, "ship_sec_system")
...
/obj/item/shipcomponent/secondary_system/UFO
	...
	Use(mob/user as mob)
		...
		if (target)
			boutput(target, "<span style=\"color:red\"><strong>You have been abducted!</strong></span>")
			showswirl(get_turf(target))
			target.set_loc(ship)
		return

	opencomputer(mob/user as mob)
		var/dat = "<TT><strong>[src] Console</strong><BR><HR>"
		for (var/mob/M in ship)
			if (M == ship.pilot) continue
			dat +="<A href='?src=\ref[src];release=[M.name]'><strong><U>[M.name]</U></strong></A><BR>"
		user << browse(dat, "window=ship_sec_system")
		onclose(user, "ship_sec_system")
...
<script language="JavaScript">
	...
	Topic(href, href_list)
		...
		if ((usr.contents.Find(src) || (in_range(ship, usr) && istype(loc, /turf))) || (istype(usr, /mob/living/silicon)))
			usr.machine = src
...
/obj/item/shipcomponent/secondary_system/crash
	...
	activate()
		if (crashable == 0) // To avoid spam. SEEDs can't be deactivated (Convair880).
			logTheThing("vehicle", usr, null, "activates a SEED, turning [ship] into a flying bomb at [log_loc(ship)]. Direction: [dir2text(ship.dir)].")
		crashable = 1
		return
...
/obj/item/shipcomponent/secondary_system/crash/proc/crashtime2(atom/A as mob|obj|turf)
	...
	if (isturf(A))
		...
		if (istype(A, /turf/simulated/wall))
			...
			boutput(ship.pilot, "<span style=\"color:red\"><strong>You crash through the wall!</strong></span>")
			in_bump = 0
		if (istype(A, /turf/simulated/floor))
			qdel(A)
	if (ismob(A))
		var/mob/M = A
		boutput(ship.pilot, "<span style=\"color:red\"><strong>You crash into [M]!</strong></span>")
		shake_camera(M, 8, 3)
		boutput(M, "<span style=\"color:red\"><strong>The [src] crashes into [M]!</strong></span>")
		M.stunned = 8
		M.weakened = 5
	...
	if (isobj(A))
		var/obj/O = A
		if (O.density)
			boutput(ship.pilot, "<span style=\"color:red\"><strong>You crash into [O]!</strong></span>")
			boutput(O, "<span style=\"color:red\"><strong>[ship] crashes into you!</strong></span>")
			var/turf/target = get_edge_target_turf(ship, ship.dir)
			playsound(loc, "sound/misc/meteorimpact.ogg", 40, 1)
			playsound(src, "sound/misc/meteorimpact.ogg", 40, 1)
