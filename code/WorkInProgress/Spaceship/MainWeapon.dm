/obj/item/shipcomponent/mainweapon
	...
	var/r_gunner = 0
	var/mob/gunner = null
	var/projectile/current_projectile = new/projectile/laser/light
	var/firerate = 8
	var/isfiring = 0
...
/obj/item/shipcomponent/mainweapon/proc/Fire(var/mob/user)
	...
	if ((rdir - 1) & rdir)
		rdir &= 12
	logTheThing("combat", usr, null, "driving [ship.name] fires [name] (<strong>Dir:</strong> <em>[dir2text(rdir)]</em>, <strong>Projectile:</strong> <em>[current_projectile]</em>) at [log_loc(ship)].") // Similar to handguns, but without target coordinates (Convair880).
...
/obj/item/shipcomponent/mainweapon/light_longrange
	...
	current_projectile = new/projectile/laser/light/longrange
...
/obj/item/shipcomponent/mainweapon/russian
	...
	weapon_score = 0.6
	current_projectile = new/projectile/laser/glitter
	firerate = 5
...
/obj/item/shipcomponent/mainweapon/rockdrills
	...
	power_used = 100
	weapon_score = 1.0
	current_projectile = new/projectile/laser/drill
...
/obj/item/shipcomponent/mainweapon/UFO
	...
	weapon_score = 1.1
	var/projectile/ufo = new/projectile/bullet/flare/UFO
	var/projectile/hlaser = new/projectile/laser/heavy
	var/mode = 0
	...
	opencomputer(mob/user as mob)
		...
		var/dat = "<TT><strong>Weapon Console</strong><BR><HR>"
		if (active)
			dat +="<strong>Weapon Mode:</strong><BR>"
			if (mode == 0)
				dat+="Heat Beam<BR>"
	...
	Fire(var/mob/user)
		switch(mode)
			if (0)
				...
				logTheThing("combat", usr, null, "driving [ship.name] fires [name], creating metal foam at [log_loc(ship)].")
