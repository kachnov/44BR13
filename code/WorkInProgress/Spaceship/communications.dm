...
/obj/item/shipcomponent/communications
	...
	mining
		...
		External()
			for (var/obj/machinery/mining_magnet/MM in range(7,ship))
	...
	proc/External()
		var/broadcast = copytext(html_encode(input(usr, "Please enter what you want to say over the external speaker.", "[name]")), 1, MAX_MESSAGE_LEN)
		if (!broadcast)
			return
		...
		for (var/mob/M in ship)
			boutput(M, "<font color='green'><strong>[bicon(ship)]\[[.]\]</strong> says, \"[broadcast]\"</font>")
		for (var/mob/O in hearers(ship, null))
			boutput(O, "<font color='green'><strong>[bicon(ship)]\[[.]\]</strong> says, \"[broadcast]\"</font>")
