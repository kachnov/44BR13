...
/obj/machinery/vehicle/recon/New()
	..()
	/////////Weapon Setup
	m_w_system = new /obj/item/shipcomponent/mainweapon( src )
	m_w_system.ship = src
	components += m_w_system
	m_w_system.activate()
	////////Secondary System
	sec_system = new /obj/item/shipcomponent/secondary_system/cloak( src )
	sec_system.ship = src
...
/obj/machinery/vehicle/cargo/New()
	..()
	sec_system = new /obj/item/shipcomponent/secondary_system/cargo( src )
	sec_system.ship = src
	components += sec_system
	myhud.update_systems()
	myhud.update_states()
...
/obj/machinery/vehicle/UFO
	...
	New()
		..()
		name = "Flying Saucer"
		sec_system = new /obj/item/shipcomponent/secondary_system/UFO( src )
		sec_system.ship = src
		components += sec_system
		m_w_system = new /obj/item/shipcomponent/mainweapon/UFO( src )
		m_w_system.ship = src
		components += m_w_system
		m_w_system.activate()
		myhud.update_systems()
...
/obj/machinery/vehicle/miniputt
	...
	New()
		..()
		//Cargo hold
		sec_system = new /obj/item/shipcomponent/secondary_system/cargo/small( src )
		sec_system.ship = src
		components += sec_system
...
/obj/machinery/vehicle/miniputt/armed
	New()
		..()
		//Phaser
		m_w_system = new /obj/item/shipcomponent/mainweapon
		m_w_system.ship = src
		components += m_w_system
...
/obj/machinery/vehicle/miniputt/wizard
	...
	New()
		...
		m_w_system = new /obj/item/shipcomponent/mainweapon
		m_w_system.ship = src
		com_system = new /obj/item/shipcomponent/communications/syndicate(src)
		com_system.ship = src
		lock = new /obj/item/shipcomponent/secondary_system/lock(src)
		lock.ship = src
		components += m_w_system
...
/obj/machinery/vehicle/miniputt/soviputt
	...
	New()
		..()
		m_w_system = new /obj/item/shipcomponent/mainweapon/russian(src)
		m_w_system.ship = src
		com_system = new /obj/item/shipcomponent/communications/syndicate(src)
		com_system.ship = src
		components += m_w_system
...
/obj/machinery/vehicle/miniputt/indyputt
	...
	armed
		New()
			..()
			m_w_system = new /obj/item/shipcomponent/mainweapon/foamer(src)
			m_w_system.ship = src
			components += m_w_system
			myhud.update_systems()
			myhud.update_states()
...
/obj/structure/puttframe/verb/deconstruct()
	...
	if (stage == 4)
		...
		C.fingerprintshidden = fingerprintshidden
	...
	switch(stage)
		if (0)
			if (istype(W, /obj/item/wrench))
				...
				overlays += image('icons/obj/ship.dmi', "[pick("putt_frame1", "putt_frame2")]")
				stage = 1
			else
		...
		if (1)
			if (istype(W, /obj/item/wrench))
				...
				overlays -= image('icons/obj/ship.dmi', "putt_frame1")
				overlays -= image('icons/obj/ship.dmi', "putt_frame2")
		...
		if (3)
			if (istype(W, /obj/item/cable_coil))
				...
				overlays += image('icons/obj/ship.dmi', "putt_wires")
				stage = 4
			else
		...
		if (4)
			if (istype(W, /obj/item/putt/boards))
				...
				user.u_equip(W)
				qdel(W)
				overlays += image('icons/obj/ship.dmi', "putt_circuits")
				stage = 5
			else
		...
		if (5)
			if (istype(W, /obj/item/sheet))
				...
				if (S.material && S.material.material_flags & MATERIAL_METAL)
					...
					overlays += image('icons/obj/ship.dmi', "putt_covers")
					stage = 6
				else
		...
		if (6)
			if (istype(W, /obj/item/putt/engine))
				...
				user.u_equip(W)
				qdel(W)
				overlays += image('icons/obj/ship.dmi', "putt_engine")
				stage = 7
			else
		...
		if (7)
			if (istype(W, /obj/item/pod/armor_light))
				...
				user.u_equip(W)
				qdel(W)
				overlays += image('icons/obj/ship.dmi', "pod_skin1")
				stage = 8
				armor_type = 1
				...
				user.u_equip(W)
				qdel(W)
				overlays += image('icons/obj/ship.dmi', "pod_skin2")
				stage = 8
				armor_type = 2
				...
				user.u_equip(W)
				qdel(W)
				overlays += image('icons/obj/ship.dmi', "pod_skin3")
				stage = 8
				armor_type = 3
				...
				user.u_equip(W)
				qdel(W)
				overlays += image('icons/obj/ship.dmi', "pod_skin4")
				stage = 8
				armor_type = 6
		...
		if (9)
			if (istype(W, /obj/item/putt/control))
				...
				user.u_equip(W)
				qdel(W)
				overlays += image('icons/obj/ship.dmi',"putt_control")
				stage = 10
			else
...
/obj/machinery/vehicle/pod_smooth
	...
	onMaterialChanged()
		..()
		if (istype(material))
			maxhealth = max(75, material.getProperty(PROP_TOUGHNESS) * 5)
			health = maxhealth
			speed = max(0.75,min(5, (100 - material.getProperty(PROP_DIELECTRIC)) / 40))
		return
	...
	ShootProjectiles(var/mob/user, var/projectile/PROJ, var/shoot_dir)
...
/obj/machinery/vehicle/pod_smooth/heavy // pods made with reinforced armor
	...
	/*prearmed // this doesn't seem to work yet, dangit
		New()
			..()
			sec_system = new /obj/item/shipcomponent/pod_weapon/laser( src )
			sec_system.ship = src
			components += sec_system
			return */
...
/obj/machinery/vehicle/pod_smooth/syndicate
	...
	/*prearmed
		New()
			..()
			sec_system = new /obj/item/shipcomponent/pod_weapon/gun( src )
			sec_system.ship = src
			components += sec_system
			return*/
...
/obj/machinery/vehicle/pod_smooth/board()
	...
	usr.make_shipcrewmember(weapon_class)
...
/obj/machinery/vehicle/pod_smooth/proc/getGunSlotTile(var/leftGun = 1)
	switch(dir)
		if (NORTH)
			if (leftGun) return get_step(loc, NORTH)
			else return get_step(get_step(loc, NORTH), EAST)
		if (EAST)
			if (leftGun) return get_step(get_step(loc, NORTH), EAST)
			else return get_step(loc, EAST)
		if (SOUTH)
			if (leftGun) return get_step(loc, EAST)
			else return loc
		if (WEST)
			if (leftGun) return loc
			else return get_step(loc, NORTH)
	...
	switch(stage)
		if (0)
			if (istype(W, /obj/item/wrench))
				...
				if (!do_after(user, 30))
					...
					return
				boutput(user, "You wrench some of the frame parts together.")
				overlays += image('icons/effects/64x64.dmi', "pod_frame1")
				stage = 1
			else
				boutput(user, "If only there was some way to secure all this junk together! You should get a wrench.")
		...
		if (3)
			if (istype(W, /obj/item/cable_coil))
				...
				overlays += image('icons/effects/64x64.dmi', "pod_wires")
				stage = 4
			else
		...
		if (4)
			if (istype(W, /obj/item/pod/boards))
				...
				user.u_equip(W)
				qdel(W)
				overlays += image('icons/effects/64x64.dmi', "pod_circuits")
				stage = 5
			else
				boutput(user, "Maybe those wires should be connecting something together. Some kind of circuitry, perhaps.")
			...
			if (istype(W, /obj/item/sheet))
				...
				if (S.material && S.material.material_flags & MATERIAL_METAL)
					...
					if (S.amount < 1)
						...
						qdel(S)
					boutput(user, "You construct internal covers over the circuitry systems.")
					overlays += image('icons/effects/64x64.dmi', "pod_covers")
					stage = 6
				else
					boutput(user, "<span style=\"color:red\">These sheets aren't the right kind of material. You need metal!</span>")
			else
		...
		if (6)
			if (istype(W, /obj/item/pod/engine))
				...
				user.u_equip(W)
				qdel(W)
				overlays += image('icons/effects/64x64.dmi', "pod_engine")
				stage = 7
			else
				boutput(user, "Having an engine might be nice.")
		...
		if (7)
			if (istype(W, /obj/item/pod/armor_light))
				...
				user.u_equip(W)
				qdel(W)
				overlays += image('icons/effects/64x64.dmi', "pod_skin1")
				stage = 8
				armor_type = 1
			else if (istype(W, /obj/item/pod/armor_heavy))
				boutput(user, "You begin to install the heavy armor plating...")
				...
				user.u_equip(W)
				qdel(W)
				overlays += image('icons/effects/64x64.dmi', "pod_skin2")
				stage = 8
				armor_type = 2
			else if (istype(W, /obj/item/pod/armor_black))
				boutput(user, "You begin to install the strange armor plating...")
				...
				user.u_equip(W)
				qdel(W)
				overlays += image('icons/effects/64x64.dmi', "pod_skin3")
				stage = 8
				armor_type = 3
			else if (istype(W, /obj/item/pod/armor_red))
				boutput(user, "You begin to install the syndicate armor plating...")
				...
				user.u_equip(W)
				qdel(W)
				overlays += image('icons/effects/64x64.dmi', "pod_skin4")
				stage = 8
				armor_type = 6
			else if (istype(W, /obj/item/pod/armor_custom) && W.material)
				boutput(user, "You begin to install the custom armor plating...")
		...
		if (9)
			if (istype(W, /obj/item/pod/control))
				...
				user.u_equip(W)
				qdel(W)
				overlays += image('icons/effects/64x64.dmi',"pod_control")
				stage = 10
			else
				boutput(user, "It's not gonna get very far without a control system!")
...
/obj/machinery/vehicle/escape_pod
	...
	proc/escape()
		if (!launched)
			...
			anchored = 0
			var/opened_door = 0
			var/turf_in_front = get_step(src,dir)
			for (var/obj/machinery/door/poddoor/D in turf_in_front)
				D.open()
				opened_door = 1
			if (opened_door) sleep(20) //make sure it's fully open
			playsound(loc, "sound/effects/bamf.ogg", 100, 0)
			sleep(5)
			playsound(loc, "sound/effects/flameswoosh.ogg", 100, 0)
			while (!failing)
				var/_loc = loc
				step(src,dir)
				if (loc == _loc) //we hit something
		...
		switch(fail_type)
			...
			if (3) //falls apart
				...
				while (src)
					step(src,dir)
					if (prob(50))
						new /obj/decal/cleanable/robot_debris/gib (loc)
					if (prob(20) && pilot)
			...
			if (4) //flies off course
				...
				while (src)
					if (prob(10)) dir = turn(dir,pick(90,-90))
					var/_loc = loc
					step(src,dir)
					if (loc == _loc) //we hit something
						explosion(src, loc, 1, 1, 2, 3)
