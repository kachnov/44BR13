...
/atom/verb/textureTest()
	...
	setTexture("shiny", BLEND_ADD, "shiny")
*/
//
...
/targetable/pixelpicker
	...
	handleCast(var/atom/selected)
		...
		var/mob/M = usr
		var/targetable/pixelpicker/R = new()
		M.targeting_spell = R
		M.update_cursor()
		return TRUE
...
/obj/item/craftedmelee/spear
	...
	rebuild()
		...
		overlays.Cut()
		overlays.Add(image(icon = core,loc = src, layer = HUD_LAYER+2)) //This will cause the item to draw above the ui even when its on the ground. That's crap
	...
	Click(location,control,params)
		if (istype(usr, /mob/dead) || istype(usr, /mob/wraith))
			...
			if ((world.time - users[usr]) >= use_delay)
				...
				if (words.len)
					var/selected = input(usr, "Select a word:", name) as null|anything in words
					if (!selected) return
					...
					spawn (0)
						if (src && selected)
							...
							for (var/mob/O in observersviewers(7, src))
								O.show_message("<strong><span style=\"color:blue\">The board spells out a message ... \"[selected]\"</span></strong>", 1)
			else
				usr.show_text("Please wait a moment before using the board again.", "red")
		else
...
/obj/peninscription
	...
	attackby(obj/item/W as obj, mob/user as mob)
		...
		if (istype(W,/obj/item/pen))
			...
			playsound(loc, "hellhorn_12.ogg", 100, 1)
		return
...
/obj/burning_barrel
	...
	var/particleSystem/barrelSmoke/particles
	var/light/light

	New()
		particles = particleMaster.SpawnSystem(new /particleSystem/barrelSmoke(src))
	...
	Del()
		particleMaster.RemoveSystem(/particleSystem/barrelSmoke, src)
		..()
...
/obj/hh_sfrag
	...
	HasEntered(atom/A)
		...
		particleMaster.SpawnSystem(new /particleSystem/elecburst(M))

		if (M.adventure_variables.hh_soul > 15)
			M.original.set_loc(loc)
...
/obj/hh_energyorb
	...
	HasEntered(atom/A)
		...
		particleMaster.SpawnSystem(new /particleSystem/energysp(M))
		return
...
/obj/proctrigger		  //Oh boy. This thing calls a proc of or on an object when an object enter it's tile. It's a bit difficult to explain, best not touch this.
	...
	proc/copy_to(var/turf/T)
		...
		P.procName = procName
		P.procTarget = procTarget
		P.procArgs = src.procArgs.Copy()
		P.procCooldown = procCooldown
		return
	...
	Click()
		...
		for (var/i=0, i<argnum, i++)
			...
			switch(class)
				...
				if ("type")
					listargs += input("Enter type:","Type", null) in subtypesof(/datum)

				if ("reference")
					listargs += input("Select reference:","Reference", null) as anything in world
	...
	attack_hand(mob/user as mob)
		...
		spawn (0)
			for (var/i=0, i<rand(30,40), i++)
				particleMaster.SpawnSystem(new /particleSystem/fireworks(loc))
				sleep(rand(2, 15))
...
/obj/item/alchemy/stone
	...
	New()
		..()
		visible_message("<span style=\"color:blue\"><strong>[src] appears out of thin air!</strong></span>")
		new /obj/effects/shockwave {name = "mystical energy";} (loc)
		light = new /light/point
		light.attach(src)
...
/obj/alchemy/circle
	...
	proc/activate()
		...
		if ((north.symbol && istype(north.symbol,north.requiredType)) && (southwest.symbol && istype(southwest.symbol,southwest.requiredType)) && (southeast.symbol && istype(southeast.symbol,southeast.requiredType)))
			var/turf/middle = locate(x + 2, y + 2, z)
			var/blood = 0
			...
			if (blood == 1)
				...
				playsound(loc, "sound/effects/chanting.ogg", 50, 1)
				sleep(10)
	...
	New(var/location)
		...
		for (var/A in subtypesof(/obj/item/alchemy/symbol))
			types += A
...
/obj/shuttle_cannon
	...
	relaymove(mob/user, direction)
		...
		if (direction == turn(dir, 180))
			ready = 1
			return
		var/turf/fire_target_tile = get_step(get_step(get_step(get_step(src, dir), dir), direction), direction)
		...
		spawn (1)
			...
			R.set_loc(get_step(src, dir))
			R.density = 1
			R.state = 1
			R.current = fire_target_tile
			R.yo = fire_target_tile.y - y
			R.xo = fire_target_tile.x - x
...
/obj/movable_area_controler
	...
	verb/setup_area()
		set src in view(1)
		set name = "Setup"
		verbs -= /obj/movable_area_controler/verb/setup_area
...
/proc/is_free(var/atom/A)
	...
	for (var/atom/curr in A)
		if (curr.density) return FALSE
	if (istype(A, /turf/space)) return FALSE
	return TRUE
...
/mob/living/carbon/human/proc/zombify()
	var/ailment_data/disease/ZOM = contract_disease(/ailment/disease/necrotic_degeneration, null, null, 1)
	if (!istype(ZOM,/ailment/disease))
		return
	ZOM.stage = 5
	...
	throw_impact(atom/hit_atom)
		...
		if (hit_atom == usr)
			if (prob(prob_clonk))
				...
				user.visible_message("<span style=\"color:red\"><strong>[user] fumbles the catch and is clonked on the head!</strong></span>")
...
/obj/shifting_wall
	...
	proc/update()
		...
		if (loc.invisibility) loc.invisibility = 0
		set_loc(picked)
...
/obj/shifting_wall/sneaky
	...
	proc/someone_can_see(var/atom/A)
		for (var/mob/living/L in view(sightrange,A))
			...
			if (A in view(sightrange,L)) return TRUE
		return FALSE

	proc/someone_can_see_me()
		for (var/mob/living/L in view(sightrange,src))
			if (L.sight_check(1)) continue
			if (src in view(sightrange,L)) return TRUE
		return FALSE
...
/obj/pool_springboard
	...
	attack_hand(mob/user as mob)
		if (in_use)
			...
			user:weakened += 2
			if (suiciding || deadly)
				visible_message("<span style=\"color:red\"><strong>[user.name] dives headfirst at the [target.name]!</strong></span>")
...
/obj/mirror
	...
	proc
		...
		break_it()
			...
			new /obj/item/raw_material/shard/plasmacrystal( loc )
...
/obj/item/engibox
	...
	var/engibox_mode/active_mode = null
	var/ckey_lock = null
	flags = FPRINT | EXTRADELAY | TABLEPASS | CONDUCT
	w_class = 1.0
	afterattack(atom/target as mob|obj|turf|area, mob/user as mob)
	...
	attack_self(mob/user as mob)
		...
		for (var/engibox_mode/D in modes)
	...
	Topic(href, href_list)
		...
		if (href_list["set_mode"])
			...
			if (istype(active_mode,/engibox_mode/transmute)) //You only have yourself to blame for this. This shitty code is the fault of whoever changed this!!!
			...
			if (istype(active_mode,/engibox_mode/replicate))
	...
	New()
		for (var/D in subtypesof(/engibox_mode))
			modes += new D
