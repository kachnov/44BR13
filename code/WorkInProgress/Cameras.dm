...
/obj/item/camera_test
	...
	examine()
		..()
		boutput(usr, "There are [pictures_left] pictures left!")
		return
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/camera_film))
			...
			pictures_left = min(pictures_left + C.pictures, pictures_max)
			user.u_equip(C)
			qdel(C)
			user.show_text("You replace the film cartridge. The camera can now take [pictures_left] pictures.", "blue")
...
/obj/item/camera_film
	...
	examine()
		..()
		boutput(usr, "It is good for [pictures] pictures.")
		return
...
/obj/item/camera_test/afterattack(atom/target as mob|obj|turf|area, mob/user as mob, flag)
	...
	pictures_left = max(0, pictures_left - 1)
