...
*/
	...
	//This first part here is to do with making sure the areas are the same size
		...
		var/list/turfs_pointer = get_area_turfs(type)
	...
	//of the lists above)
		...
		for (var/turf/T in turfs_pointer)
			refined_pointer += T
			refined_pointer[T] = new/coords
			var/coords/C = refined_pointer[T]
		...
		for (var/turf/T in turfs_proj)
			refined_proj += T
			refined_proj[T] = new/coords
			var/coords/C = refined_proj[T]
	...
	proc/project(var/list/refined_pointer, var/list/refined_proj)
		...
		while (projection_enabled)
			...
			for (var/turf/T in refined_pointer)
				var/coords/C_pointer = refined_pointer[T]

				for (var/turf/B in refined_proj)
					var/coords/C_proj = refined_proj[B]
...
/obj/projection
	...
	proc/Life()
		if (pointer)
			...
			var/turf/the_turf = get_turf(pointer)

			icon = make_icon_with_turf(the_turf)

			spawn (5) Life()

		else
			logTheThing("admin", null, null, "[src]/(%coords([x], [y], [z])%) not defined properly with ID = [id] and PTP = [pointer]")
			logTheThing("diary", null, null, "[src]/(%coords([x], [y], [z])%) not defined properly with ID = [id] and PTP = [pointer]", "admin")
