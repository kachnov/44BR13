...
/obj/machinery/buddyfactory
	...
	process()
		...
		var/obj/item/guardbot_frame/frame = locate() in get_step(src,dir)
			...
			switch (input_stage)
				if (1)
					...
					if (istype(cell_to_add))
						...
						if (frame.stage > 1)
							visible_message("[src] inserts [cell_to_add] into [frame].")
				...
				if (2)
					...
					if (!frame.created_module && istype(tool_to_add))
						...
						if (frame.created_module)
							visible_message("[src] attaches [tool_to_add] into [frame].")
							flick("factory_arm_active",src)
					else
						...
						if (istype(core))
							...
							if (frame.stage == 3)
								visible_message("[src] attaches [core] into [frame].")
								flick("factory_arm_active",src)
...
/computer/file/record/tds_basketball
	name = "journal1103"
...
/computer/file/record/tds_desperate
	name = "journal1107"
...
/computer/file/record/tds_repairlog1
	name = "servlog47"
...
/computer/file/record/tds_repairlog2
	name = "servlog48"
...
/computer/file/record/tds_repairlog3
	name = "servlog49"
...
/computer/file/record/tds_repairlog4
	name = "servlog50"
...
/computer/file/record/tds_repairlog5
	name = "servlog51"
...
/obj/item/disk/data/tape/tds_journaltape
	...
	New()
		..()
		root.add_file( new /computer/file/record/tds_basketball(src))
		root.add_file( new /computer/file/record/tds_desperate(src))

/obj/item/disk/data/tape/tds_replogtape

	New()
		..()
		root.add_file( new /computer/file/record/tds_repairlog1(src))
		root.add_file( new /computer/file/record/tds_repairlog2(src))
		root.add_file( new /computer/file/record/tds_repairlog3(src))
		root.add_file( new /computer/file/record/tds_repairlog4(src))
		root.add_file( new /computer/file/record/tds_repairlog5(src))
...
/obj/storage/secure/filing_cabinet
	...
	attackby(obj/item/I, mob/user)
		if (istype(I, /obj/item/device/key/filing_cabinet))
			...
			user.visible_message("<strong>[user]</strong> [locked ? "" : "un"]locks [src].", "You [locked ? "" : "un"]lock [src].")
			return
...
/obj/storage/secure/filing_cabinet/lunarium
	...
	attackby(obj/item/I, mob/user)
		var/oldlocked = locked	//This is all to stop them from just blowing the cabinet open or something.
		. = ..()
		if (!stuffHasBeenSpawned && oldlocked && !locked)
