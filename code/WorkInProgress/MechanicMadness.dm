...
/mechanicsMessage
	...
	proc/addNode(var/mechanics_holder/H)
		nodes.Add(H)

	proc/removeNode(var/mechanics_holder/H)
		nodes.Remove(H)

	proc/hasNode(var/mechanics_holder/H)

		return nodes.Find(H)

	proc/isTrue() //Thanks for not having bools , byond.
		if (istext(signal))
			if (lowertext(signal) == "true" || lowertext(signal) == "1" || lowertext(signal) == "one") return TRUE
...
/mechanics_holder
	...
	proc/fireInput(var/name, var/mechanicsMessage/msg)
	...
	proc/fireOutgoing(var/mechanicsMessage/msg)
	...
	proc/cloneMessage(var/mechanicsMessage/msg)
		var/mechanicsMessage/msg2 = newSignal(msg.signal)
		msg2.nodes = msg.nodes.Copy()
		return msg2

	//ALWAYS use this to create new messages!!!
	proc/newSignal(var/sig)
		var/mechanicsMessage/ret = new/mechanicsMessage
		ret.signal = sig
		return ret
	...
	proc/allowChange(var/mob/M)
		if (hasvar(M, "l_hand") && istype(M:l_hand, /obj/item/device/multitool)) return TRUE
		if (hasvar(M, "r_hand") && istype(M:r_hand, /obj/item/device/multitool)) return TRUE
...
/obj/item/mechanics
	...
	proc/cutParticles()
		if (particles.len)
			for (var/particleSystem/mechanic/M in particles)
				M.Die()
			particles.Cut()
		return
	...
	process()
		...
		if (particles.len != mechanics.connected_outgoing.len)
			...
			for (var/atom/X in mechanics.connected_outgoing)
				particles.Add(particleMaster.SpawnSystem(new /particleSystem/mechanic(loc, X.loc)))
		...
		if (istype(W,/obj/item/wrench))
			switch(level)
				...
				if (2) //Level 2 = loose
					...
					logTheThing("station", user, src, "placed a %target% at [showCoords(x, y, z)]")
					level = 1
					anchored = 1
	...
	proc/componentSay(var/string)
		...
		for (var/mob/O in all_hearers(7, loc))
			O.show_message("<span class='game radio'><span class='name'>[src]</span><strong> [bicon(src)] [pick("squawks", "beeps", "boops", "says", "screeches")], </strong> <span class='message'>\"[string]\"</span></span>",2)
...
/obj/item/mechanics/cashmoney
	...
	proc/emoney(var/mechanicsMessage/input)
		if (level == 2) return
		if (input)
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (..(W, user)) return
			...
			if (current_buffer >= price)
				...
				var/mechanicsMessage/msg = mechanics.newSignal(mechanics.outputSignal)
...
/obj/item/mechanics/flushcomp
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (..(W, user))
			if (level == 1) //wrenched down
				...
				if (trunk)
					trunk.linked = src
					air_contents = unpool(/gas_mixture)
	...
	proc/flushp(var/mechanicsMessage/input)
		if (level == 2) return
...
/obj/item/mechanics/thprint
	...
	proc/print(var/mechanicsMessage/input)
		if (level == 2 || !ready) return
		if (input)
			ready = 0
			spawn (50) ready = 1
			flick("comp_tprint1",src)
			playsound(loc, "sound/machines/printer_thermal.ogg", 60, 0)
			var/obj/item/paper/thermal/P = new/obj/item/paper/thermal(loc)
...
/obj/item/mechanics/pscan
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (..(W, user)) return
			...
			playsound(loc, "sound/machines/twobeep2.ogg", 90, 0)
			var/obj/item/paper/P = W
			var/saniStr = strip_html(sanitize(html_encode(P.info)))
			var/mechanicsMessage/msg = mechanics.newSignal(saniStr)
			mechanics.fireOutgoing(msg)
			if (del_paper)
				del(W)
		return
	...
	attack_hand(mob/user as mob)
		if (level != 2 && ready)
			if (istype(user, /mob/living/carbon/human) && user.bioHolder)
				...
				var/mechanicsMessage/msg = mechanics.newSignal(sendstr)
				mechanics.fireOutgoing(msg)
			else
...
/obj/item/mechanics/accelerator
	...
	proc/activateproc(var/mechanicsMessage/input)
		if (level == 2) return
		if (input)
			if (active) return
			particleMaster.SpawnSystem(new /particleSystem/gravaccel(loc, dir))
			spawn (0)
				if (src)
	...
	proc/throwstuff(atom/movable/AM as mob|obj)
		...
		var/atom/target = get_edge_target_turf(AM, dir)
...
/obj/item/mechanics/pausecomp
	...
	proc/delayproc(var/mechanicsMessage/input)
		if (level == 2) return
		if (input)
			if (active) return
			spawn (0)
...
/obj/item/mechanics/andcomp
	...
	proc/fire1(var/mechanicsMessage/input)
		if (level == 2) return
		if (inp1) return
	...
	proc/fire2(var/mechanicsMessage/input)
		if (level == 2) return
		if (inp2) return
...
/obj/item/mechanics/orcomp
	...
	proc/fire(var/mechanicsMessage/input)
		if (level == 2) return
...
/obj/item/mechanics/wifisplit
	...
	proc/split(var/mechanicsMessage/input)
		if (level == 2) return
...
/obj/item/mechanics/regreplace
	...
	proc/checkstr(var/mechanicsMessage/input)
...
/obj/item/mechanics/sigbuilder
	...
	proc/clrbff(var/mechanicsMessage/input)
		if (level == 2) return
		buffer = ""
		return

	proc/sendstr(var/mechanicsMessage/input)
		if (level == 2) return
	...
	proc/addstr(var/mechanicsMessage/input)
		if (level == 2) return
		buffer = "[buffer][input.signal]"
		return

	proc/addstrsend(var/mechanicsMessage/input)
		if (level == 2) return
		buffer = "[buffer][input.signal]"
		sendstr(input)
		return
...
/obj/item/mechanics/relaycomp
	...
	proc/relay(var/mechanicsMessage/input)
		if (level == 2 || !ready) return
		ready = 0
		spawn (30) ready = 1
...
/obj/item/mechanics/wificomp
	...
	proc/setfreq(var/mechanicsMessage/input)
		var/newfreq = text2num(input.signal)
		if (!newfreq) return
		set_frequency(newfreq)
		return

	proc/send(var/mechanicsMessage/input)
		if (level == 2) return
		var/list/converted = params2list(input.signal)
		...
		var/signal/sendsig = get_free_signal()

		sendsig.source = src
		sendsig.data["sender"] = net_id
		sendsig.transmission_method = TRANSMISSION_RADIO

		for (var/X in converted)
			sendsig.data["[X]"] = "[converted[X]]"

		spawn (0) radio_connection.post_signal(src, sendsig, range)
	...
	receive_signal(signal/signal)
		...
		if ((only_directed && signal.data["address_1"] == net_id) || !only_directed || (signal.data["address_1"] == "ping"))

			if (send_full)
				var/mechanicsMessage/msg = mechanics.newSignal(html_decode(list2params_noencode(signal.data)))
				mechanics.fireOutgoing(msg)
			...
			if ((signal.data["address_1"] == "ping") && signal.data["sender"])
				...
				spawn (5) //Send a reply for those curious jerks
					radio_connection.post_signal(src, pingsignal, range)

			else if (signal.data["command"] == "sendmsg" && signal.data["data"])
				var/mechanicsMessage/msg = mechanics.newSignal(html_decode(signal.data["data"]))
				mechanics.fireOutgoing(msg)
				animate_flash_color_fill(src,"#00FF00",2, 2)

			else if (signal.data["command"] == "text_message" && signal.data["message"])
				var/mechanicsMessage/msg = mechanics.newSignal(html_decode(signal.data["message"]))
				mechanics.fireOutgoing(msg)
				animate_flash_color_fill(src,"#00FF00",2, 2)
...
/obj/item/mechanics/selectcomp
	...
	proc/selitem(var/mechanicsMessage/input)
		if (!input) return
	...
	proc/remitem(var/mechanicsMessage/input)
		if (!input) return
	...
	proc/remallitem(var/mechanicsMessage/input)
		if (!input) return
	...
	proc/additem(var/mechanicsMessage/input)
		if (!input) return
	...
	proc/sendRand(var/mechanicsMessage/input)
		if (!input) return
	...
	proc/sendCurrent(var/mechanicsMessage/input)
		if (!input) return
	...
	proc/next(var/mechanicsMessage/input)
		if (level == 2) return
	...
	proc/nextplus(var/mechanicsMessage/input)
		if (level == 2) return
		if (!signals.len) return
		if ((current_index + 1) > signals.len)
			current_index = 1
	...
	proc/previous(var/mechanicsMessage/input)
		if (level == 2) return
		if (!signals.len) return
	...
	proc/previousplus(var/mechanicsMessage/input)
		if (level == 2) return
		if (!signals.len) return
		if ((current_index - 1) < 1)
			current_index = signals.len
...
/obj/item/mechanics/togglecomp
	...
	proc/activate(var/mechanicsMessage/input)
		if (level == 2) return
		on = 1
	...
	proc/deactivate(var/mechanicsMessage/input)
		if (level == 2) return
		on = 0
	...
	proc/toggle(var/mechanicsMessage/input)
		if (level == 2) return
		on = !on
	...
	proc/state(var/mechanicsMessage/input)
		if (level == 2) return
		input.signal = (on ? signal_on : signal_off)
		spawn (0)
			mechanics.fireOutgoing(input)
...
/obj/item/mechanics/telecomp
	...
	proc/setidmsg(var/mechanicsMessage/input)
		if (level == 2) return
		if (input.signal)
			teleID = input.signal
	...
	proc/activate(var/mechanicsMessage/input)
		...
		particleMaster.SpawnSystem(new /particleSystem/tpbeam(get_turf(loc)))
		playsound(loc, "sound/mksounds/boost.ogg", 50, 1)
		var/list/destinations = new/list()

		for (var/obj/item/mechanics/telecomp/T in mechanics_telepads)
			if (T == src || T.level == 2 || !isturf(T.loc) || T.z != z  || isrestrictedz(T.z)|| T.send_only) continue
			if (T.teleID == teleID)
				destinations.Add(T)

		if (destinations.len)
			var/atom/picked = pick(destinations)
			particleMaster.SpawnSystem(new /particleSystem/tpbeam(get_turf(picked.loc)))
			for (var/atom/movable/M in loc)
	...
	verb/togglesendonly()
		...
		if (send_only)
			overlays += image('icons/misc/mechanicsExpansion.dmi', icon_state = "comp_teleoverlay")
		else
			overlays.Cut()
...
/obj/item/mechanics/ledcomp
	...
	proc/setrgb(var/mechanicsMessage/input)
	...
	proc/turnon(var/mechanicsMessage/input)
		if (level == 2) return
		if (usr && usr.stat)
			return
		active = 1
	...
	proc/turnoff(var/mechanicsMessage/input)
		if (level == 2) return
		if (usr && usr.stat)
			return
		active = 0
...
/obj/item/mechanics/miccomp
	...
	hear_talk(mob/M as mob, msg, real_name, lang_id)
		...
		if (real_name)
			heardname = real_name
		var/mechanicsMessage/sigmsg = mechanics.newSignal((add_sender ? "[heardname] : [message]":"[message]"))
...
/obj/item/mechanics/gunholder
	...
	proc/getTarget()
		...
		for (var/i=0, i<7, i++)
			trg = get_step(trg, dir)
			for (var/mob/living/L in trg)
				return get_turf_loc(L)
		return get_edge_target_turf(src, dir)
	...
	proc/fire(var/mechanicsMessage/input)
		...
		if (input && Gun)
			if (Gun.canshoot())
				...
				visible_message("<span class='game say'><span class='name'>[src]</span> beeps, \"The [Gun.name] has no [istype(Gun, /obj/item/gun/energy) ? "charge" : "ammo"] remaining.\"</span>")
				playsound(loc, "sound/machines/buzz-two.ogg", 50, 0)
		else
			visible_message("<span class='game say'><span class='name'>[src]</span> beeps, \"No gun installed.\"</span>")
			playsound(loc, "sound/machines/buzz-two.ogg", 50, 0)
		return

	attackby(obj/item/W as obj, mob/user as mob)
		if (..(W, user)) return
		else if (istype(W, compatible_guns))
			if (!Gun)
...
/obj/item/mechanics/gunholder/recharging
	...
	process()
		...
		if (!E.rechargeable)
			visible_message("<span class='game say'><span class='name'>[src]</span> beeps, \"This gun cannot be recharged manually.\"</span>")
			playsound(loc, "sound/machines/buzz-two.ogg", 50, 0)
			charging = 0
			updateIcon()
			return
	...
	proc/recharge(var/mechanicsMessage/input)
	...
	fire(var/mechanicsMessage/input)
