...
/module_research_controller
	...
	New()
		...
		for (var/mt in cyborg_modules)
			...
			for (var/obj/item/I in M.modules)
				var/module_tech/T = locate_item(I)
				if (T)
					unlock_now(T)
			qdel(M)
	...
	proc/setup()
		if (!needs_setup)
			return
		needs_setup = 0
		for (var/tech in subtypesof(/module_tech))
			var/module_tech/T = new tech()
	...
	proc/check_unlocks()
		...
		var/unlocked = 0
		for (var/module_tech/T in locked_tech)
			var/passed = 1
	...
	proc/locate_item(var/obj/item/I)
		...
		for (var/module_tech/T in locked_tech)
			if (T.item_path == path)
				return T
		for (var/module_tech/T in unlocked_tech)
			if (T.item_path == path)
				return T
		return null

	proc/unlock_now(var/module_tech/T)
		if (T in locked_tech)
			locked_tech -= T
			unlocked_tech += T

var/global/module_research_controller/module_control = new
...
/obj/submachine/module_researcher
	...
	proc/check_unlocks()
		...
		if (unlocked)
			...
			visible_message("<strong>[src]</strong> unlocks [unlocked == 1 ? "a new item!" : "[unlocked] new items!"]")

	proc/process()
		if (researching)
			playsound(loc, pick(grumps), 50, 1)
			if (research_speed)
			...
			if (research_status >= 100)
				...
				research_object = ""
				playsound(loc, 'sound/machines/ding.ogg', 50, 1)
				visible_message("<strong>[src]</strong> finishes its research process!")
	...
	Topic(href, href_list)
		...
		if (!researching)
			...
			if (href_list["shred"])
				...
				if (I && I.loc == src)
					researching = 1
					visible_message("<strong>[src]</strong> powers up and begins its research process!")
					var/list/res = I.module_research.Copy()
					if (I.artifact)
						var/artifact/art = I.artifact
			...
			if (href_list["add"])
				var/module_tech/T = locate(href_list["add"])
			...
			if (href_list["create"])
				...
				if (!modules.len)
					...
					for (var/i = 1, i <= current_module.len, i++)
						var/module_tech/T = current_module[i]
		...
		switch (machine_state)
			...
			if (MR_MACHINE_STATE_ITEMS)
				if (!researching)
					...
					state_screen = "<h2>Research in progress</h2><strong>Item name:</strong> [research_object]<br><strong>Progress:</strong> [research_status]%<br>"
					state_screen += "<strong>Research budget available: </strong>[wagesystem.research_budget] credits<br>"
					if (boosted < 5)
						var/boost_cost = (100 * (1 << boosted)) / module_control.cost_divisor
						state_screen += "<a href='?src=\ref[src];boost=1'>Boost research speed</a> for [boost_cost] credits<br>"
					else
						state_screen += "<strong>Cannot boost</strong> any further!<br>"
					state_screen += "<strong>Boosted:</strong> [boosted] time[boosted != 1 ? "s" : null]<br>"
			...
			if (MR_MACHINE_STATE_TECHNOLOGY)
				state_screen += "<table><tr><td colspan='3'><strong>Technology overview</strong></td></tr>"
				state_screen += "<tr><td><strong>Unlocked</strong></td><td><strong>Locked</strong></td><td><strong>Researched</strong></td></tr>"
				state_screen += "<tr><td>"
				for (var/module_tech/T in module_control.unlocked_tech)
					state_screen += "- <a href='?src=\ref[src];unlocked=\ref[T]'>[T.name]</a><br>"
				state_screen += "&nbsp;</td><td>"
				for (var/module_tech/T in module_control.locked_tech)
					if (!T.hidden)
			...
			if (MR_MACHINE_STATE_MODULE_BUILDER)
				state_screen = "<strong>(WORK IN PROGRESS)</strong><br><strong>Available modules: </strong>[modules.len]<br><br>"
				state_screen += "<strong>Currently constructed module:</strong><br>"
				state_screen += "<strong>Module name:</strong> <a href='?src=\ref[src];name=1'>[module_name]</a><br>"
				state_screen += "<strong>Module icon:</strong> <img src='module-icon.png' /> <a href='?src=\ref[src];icon=1'>(change)</a><ul>"
				var/total_cost = 0
				var/total_size = 0
				for (var/i = 1, i <= current_module.len, i++)
					var/module_tech/T = current_module[i]
					...
					total_size += T.size
					state_screen += "<li><strong>[T.name]</strong> (<strong>Size: </strong> [T.size] -- <strong>Cost: </strong> [T.cost / module_control.cost_divisor] data points)<div class='right'><a href='?src=\ref[src];remove=[i]'>(remove)</a></div></li>"
				state_screen += "</ul>"
				state_screen += "<br><strong>Size: </strong>[total_size]/[module_control.maximum_size]"
				state_screen += "<br><strong>Total cost: </strong>[total_cost] data points"
				state_screen += "<br><strong>Available data points: </strong>[module_control.data_points] data points"
				state_screen += "<br><strong>Buy</strong> <a href='?src=\ref[src];buydp=1'>1 point for 30 credits</a> | <a href='?src=\ref[src];buydp=10'>10 point for 300 credits</a> | <a href='?src=\ref[src];buydp=100'>100 points for 3000 credits</a>"
				state_screen += "<br><strong>Research budget: </strong>[wagesystem.research_budget] credits"
				state_screen += "<br><a href='?src=\ref[src];create=1'>(CREATE)</a> - <a href='?src=\ref[src];reset=1'>(RESET)</a>"
				state_screen += "<br><br><strong>Unlocked technologies:<strong><br><ul>"
				state_screen += ""
				for (var/module_tech/T in module_control.unlocked_tech)
					state_screen += "<li><strong>[T.name]</strong> <a href='?src=\ref[src];add=\ref[T]'>(add)</a><br><span class='tiny'>[T.desc]</span><br><span class='tiny'><strong>Size: </strong>[T.size] - <strong>Cost: </strong>[T.cost / module_control.cost_divisor]</span></li>"
				state_screen += "</ul>"
			if (MR_MACHINE_STATE_LOCKED)
				var/module_tech/T = substate
				...
				title = T.name
				state_screen += "<em>[T.desc]</em><br><br><strong>Required topics: </strong><br><ul>"
				for (var/rt in T.research_requirements)
					state_screen += "<li>"
				...
				if (T.additional_requirements.len)
					state_screen += "<br><br><strong>Additional requirements: </strong><br><ul>"
			...
			if (MR_MACHINE_STATE_UNLOCKED)
				...
				if (!istype(T) || !(T in module_control.unlocked_tech))
					...
					return generate_interface()
				title = T.name
				state_screen += "<em>[T.desc]</em><br><br><strong>Required topics: </strong><br>"
				if (T.hidden)
					state_screen += "<em>???</em>"
				else
					state_screen += "<ul>"
			...
			if (MR_MACHINE_STATE_RESEARCHED)
				...
				state_screen += "<strong>Item research value per topic: </strong><br><ul>"
				for (var/rt in data)
					if (rt == "--item-name")
						continue
					var/pts = data[rt]
