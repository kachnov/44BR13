...
/obj/machinery/vending/pizza
	...
	attack_hand(mob/user as mob)
		...
		var/dat
		if (pizcooking)
			dat += "<TT><strong>Cooking your pizza, please wait!</strong></TT><BR>"
		else
			dat += "<TT><strong>PizzaVend 0.5b</strong></TT><BR>"
			if (emagged)
				dat += "<BR><strong>Available Credits:</strong> CREDIT CALCULATION ERROR<BR>"
			else
				dat += "<BR><strong>Available Credits:</strong> $[credit]<BR>"
		...
		if (usr.contents.Find(src) || in_range(src, usr) && istype(loc, /turf))
			...
			if (href_list["cook"])
				if (!pizcooking)
					...
					var/obj/item/reagent_containers/food/snacks/pizza/P = new /obj/item/reagent_containers/food/snacks/pizza(loc)
					P.quality = 0.6
					P.heal_amt = 2
...
/obj/monkeyplant
	...
	attackby(obj/item/W as obj, mob/user as mob)
		visible_message("<strong>[src]</strong> screams!",1)
		if (narrator_mode)
...
/obj/critter/marsrobot
		...
		if (active)
			...
			for (var/mob/living/C in hearers(seekrange,src))
				...
				if (attack)
					...
					if (startup)
						visible_message("<span class='combat'>The <strong>[src]</strong> suddenly turns on!</span>")
						name = "malfunctioning robot"
						speak("Lev##LLl 7 SEV-s-E infraAAAAAaction @leRT??!")
						visible_message("The <strong>[src]</strong> points at [C.name]!")
						playsound(loc, 'sound/voice/robot_scream.ogg', 50, 1)
						startup = 0
						wanderer = 1
					visible_message("<span style=\"color:red\">The <strong>[src]</strong> charges at [C:name]!</span>")
					speak(pick("DooN'T Wor##y I'M hERE!!!","LawwSS UpdAA&$.A.!!.!","CANIHELPYO&�%SIR","REsREACH!!!!!","NATAS&$%LIAHLLA ERROR CODE #736"))
					playsound(loc, 'sound/machines/glitch3.ogg', 50, 1)
					icon_state = "mars_bot"
					task = "chasing"
					break
				else
	...
	ChaseAttack(mob/M)
		visible_message("<span class='combat'>The <strong>[src]</strong> launches itself towards [M]!</span>")
		if (prob(20)) M.stunned += rand(1,3)
		random_brute_damage(M, rand(2,5))

	CritterAttack(mob/M)
		attacking = 1
		visible_message("<span class='combat'>The <strong>[src]</strong> slams itself against [target]!</span>")
		random_brute_damage(target, rand(7,17))
		spawn (10)
			attacking = 0
	...
	Move()
		..()
		playsound(loc, 'sound/effects/airbridge_dpl.ogg', 30, 10, -2)
	...
	CritterDeath()
		...
		density = 0
		walk_to(src,0)
		visible_message("<strong>[src]</strong> collapses!")
		playsound(loc, 'sound/voice/robot_scream.ogg', 50, 1)
...
/obj/vehicle/marsrover/eject_rider(var/crashed, var/selfdismount)
	...
	if (crashed)
		...
		boutput(rider, "<span class='combat'><strong>You are flung over the [src]'s handlebars!</strong></span>")
		rider.stunned = 8
		rider.weakened = 5
		for (var/mob/C in AIviewers(src))
			if (C == rider)
				continue
			C.show_message("<span class='combat'><strong>[rider] is flung over the [src]'s handlebars!</strong></span>", 1)
	...
	switch(M.a_intent)
		if ("harm", "disarm")
			if (prob(60))
				...
				visible_message("<span class='combat'><strong>[M] has shoved [rider] off of the [src]!</strong></span>")
				rider.weakened = 2
				eject_rider()
			else
				playsound(loc, 'sound/weapons/punchmiss.ogg', 25, 1, -1)
				visible_message("<span class='combat'><strong>[M] has attempted to shove [rider] off of the [src]!</strong></span>")
	return
...
/obj/vehicle/marsrover/disposing()
	if (rider)
		boutput(rider, "<span class='combat'><strong>Your rover is destroyed!</strong></span>")
		eject_rider()
	..()
	return
	...
	Entered(atom/movable/O)
		...
		if (istype(O, /mob/living/carbon/human))
			...
			if (jerk.stat != 2)
				...
				step(jerk,EAST)
				if (prob(50))
					playsound(loc, 'sound/effects/bloody_stabOLD.ogg', 50, 1)
...
/obj/critter/gunbot/heavy
	...
	atksilicon = 1
	var/overheat = 0
	var/projectile/my_bullet = new/projectile/bullet/revolver_38
	...
	proc/speak(var/message)
		...
		for (var/mob/O in hearers(src, null))
			O.show_message("<span class='game say'><span class='name'>[src]</span> blares, \"<strong>[processedMessage]</strong>\"",2)
		...
		if (overheat == 10)
			...
			for (var/mob/living/C in hearers(seekrange,src))
				...
				if (attack)
					...
					target = C
					oldtarget_name = C.name
					visible_message("<span class='combat'><strong>[src]</strong> rapidly fires at [target]!</span>")


					playsound(loc, 'sound/weapons/ak47shot.ogg', 50, 1)
