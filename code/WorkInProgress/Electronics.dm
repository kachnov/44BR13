...
/obj/item/electronics/New()
	desc = "A [name] used in electronic projects."
	return
...
/obj/item/electronics/battery/New()
	icon_state = pick("batt1", "batt2", "batt3")
	randompix()
	..()
////////////////////////////////////////////////////////////////up
...
/obj/item/electronics/board/New()
	icon_state = pick("board1", "board2", "board3")
	randompix()
	..()
////////////////////////////////////////////////////////////////
...
/obj/item/electronics/fuse/New()
	icon_state = pick("fuse1", "fuse2", "fuse3")
	randompix()
	..()
////////////////////////////////////////////////////////////////
...
/obj/item/electronics/switc/New()
	icon_state = pick("switch1", "switch2", "switch3")
	randompix()
	..()
////////////////////////////////////////////////////////////////up
...
/obj/item/electronics/keypad/New()
	icon_state = pick("keypad1", "keypad2", "keypad3")
	randompix()
	..()
////////////////////////////////////////////////////////////////up
...
/obj/item/electronics/screen/New()
	icon_state = pick("screen1", "screen2", "screen3")
	randompix()
	..()
////////////////////////////////////////////////////////////////
...
/obj/item/electronics/capacitor/New()
	icon_state = pick("capacitor1", "capacitor2", "capacitor3")
	randompix()
	..()
////////////////////////////////////////////////////////////////up
...
/obj/item/electronics/resistor/New()
	icon_state = pick("resistor1", "resistor2")
	randompix()
	..()
////////////////////////////////////////////////////////////////
	...
	if (istype(W,/obj/item/electronics))
		...
		if (!(istype(E,/obj/item/electronics/disk)||istype(E,/obj/item/electronics/scanner)||istype(E,/obj/item/electronics/soldering)||istype(E,/obj/item/electronics/frame)))
			...
			if (!secured)
				...
				logTheThing("station", user, null, "deploys a [name] in [user.loc.loc] ([showCoords(x, y, z)])")
				deploy()
			return
	..()
...
/obj/item/electronics/frame/attack_self(mob/user as mob)
	...
	switch(viewstat)
		...
		if (0)
			for (var/obj/item/electronics/P in contents)
...
//	if (contents.len != needed_parts.len)
//		return FALSE

	//for (var/tracker = 1, tracker <= parts:len, tracker ++)
	var/list/checkList = needed_parts.Copy()
	for (var/tracker = 1, tracker <= contents:len, tracker ++)
		var/partID
		//var/obj/T = parts[tracker]
		var/obj/T = contents[tracker]
...
/obj/item/electronics/scanner/afterattack(var/obj/O, mob/user as mob)
	if (istype(O,/obj/machinery/rkit))
		return
	if (istype(O,/obj))
		if (O.mats == 0 || (O.is_syndicate != 0 && is_syndicate == 0))
...
/obj/machinery/rkit/receive_signal(signal/signal)
	if (stat & NOPOWER)
		return
	...
	if (signal.data["address_1"] != net_id || !target || signal.data["command"] != "add" || !istype(signal.data_file, /computer/file/electronics_scan))
		return

	var/computer/file/electronics_scan/scanFile = signal.data_file
	for (var/electronics/scanned_item/O in mechanic_controls.scanned_items)
...
/obj/machinery/rkit/attack_hand(mob/user as mob)
	add_fingerprint(user)
	var/dat
	dat = "<strong>Ruckingenur Kit</strong><HR>"

	dat += "<strong>Scanned Items:</strong><br>"
	for (var/electronics/scanned_item/S in mechanic_controls.scanned_items)
...
/obj/machinery/rkit/Topic(href, href_list)
	...
	if ((in_range(src, usr) && istype(loc, /turf)) || (istype(usr, /mob/living/silicon)))
		...
		switch(href_list["tp"])
			...
			if ("done")
				if (href_list["op"])
					var/electronics/scanned_item/O = locate(href_list["op"])
					if (istype(O,/electronics/scanned_item))
						var/obj/item/electronics/frame/F = new/obj/item/electronics/frame(loc)
						F.name = "[O.name]-frame"
			...
			if ("blueprint")
				if (href_list["op"])
					if (no_print_spam && world.time < no_print_spam + 50)
						usr.show_text("[src] isn't done with the previous print job.", "red")
					else
						var/electronics/scanned_item/O = locate(href_list["op"])
						if (istype(O.blueprint, /manufacture/mechanics))
							usr.show_text("Print job started...", "blue")
							var/manufacture/mechanics/M = O.blueprint
							playsound(loc, 'sound/machines/printer_thermal.ogg', 50, 1)
							no_print_spam = world.time
							spawn (50)
								if (src)
									new /obj/item/paper/manufacturer_blueprint(loc, M.name)
...
/obj/item/deconstructor
	...
	afterattack(atom/target as mob|obj|turf|area, mob/user as mob)
		...
		if (do_after(user, 20))
			...
			user.visible_message("<strong>[user.name]</strong> deconstructs [O].")

			var/manufacture/mechanics/R = null
			for (var/manufacture/mechanics/M in manuf_controls.custom_schematics)
