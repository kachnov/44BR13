			...
			if (isliving(AM))
				if (AM:client)
					if (prob(75))
						...
						spawn (rand(10,30))
							var/obj/chaser/hospital/C = new /obj/chaser/hospital(loc)
							C.target = AM
	...
	proc/process()
		if (target)
			if (get_dist(src, target) <= 1)
				if (prob(40))
					visible_message("<span style=\"color:red\"><strong>[src] passes its arm through [target]!</strong></span>")
					//playsound(loc, 'sound/effects/bloody_stab.ogg', 50, 1)
					target.change_eye_blurry(10)
					boutput(target, "<span><strong>no no no no no no no no no no no no non&#9617;NO&#9617;NNnNNO</strong></span>")
					target.vaporize(,1)
					maniac_active &= ~1
					qdel(src)
...
/obj/item/device/audio_log/hospital_01
	...
	New()
		..()
		tape = new /obj/item/audio_tape/hospital_01(src)
...
/obj/item/device/audio_log/hospital_02
	...
	New()
		..()
		tape = new /obj/item/audio_tape/hospital_02(src)
...
/obj/item/device/audio_log/hospital_03
	...
	New()
		..()
		tape = new /obj/item/audio_tape/hospital_03(src)
...
/obj/item/device/audio_log/hospital_04
	...
	New()
		..()
		tape = new /obj/item/audio_tape/hospital_04(src)

/obj/item/audio_tape/hospital_04
	New()
		..()
		speakers = list("Male voice","Different male voice", "Myron Roberts", "Myron Roberts",
		...
		messages = list("Welcome, horror fans to GHOST WATCH, with your host, Myron Roberts!",
...
/obj/trigger/spooky_emote
	...
	on_trigger(var/mob/living/somebody)
		...
		if (prob(5))
			...
			boutput(somebody, "<strong>[pick(stuff_around)]</strong> [pick("whimpers!", "whispers...","cries!","gently sings.", "stares.","shrieks!","shakes.","shudders.","laughs.","mutters.")]")
