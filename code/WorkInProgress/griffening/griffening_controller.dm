...
/griffening_controller
	...
	proc/announce(message)
		var/formatted = "<b style='font-weight: bold; color: #008800'>\[GRIFENING\]: [message]</strong>"
	...
	proc/action(var/obj/griffening_card_holder/holder, var/mob/M)
		...
		var/playing_card/griffening/C
		if (card)
			if (card.cards.len == 1)
				C = card.cards[1]

			for (var/playing_card/Card in card.cards)
				if (!istype(Card, /playing_card/griffening))
			...
			if (HOLDER_ROLE_CREATURE)
				if (phase == PHASE_MAIN)
					...
					if (!C)
						...
						return
					if (!istype(C, /playing_card/griffening/creature))
					...
					var/playing_card/griffening/creature/Cr = C
	...
	proc/register_player(var/playerid, var/mob/M, var/obj/item/playing_cards/deck)
		...
		if (!game_id)
			...
			next_game_id++
		for (var/playing_card/griffening/Card in deck.cards)
	...
	proc/player_creatures_with_attribute_face_up(var/card_attribute, var/active_player)
		...
		for (var/i = 1, i <= 5, i++)
			...
			if (holder.card && holder.card.face_up)
				var/playing_card/griffening/creature/Card = holder.card.cards[1]
	...
	proc/player_discarded_creatures_with_attribute(var/card_attribute, var/active_player)
		...
		for (var/i = 1, i <= PC.cards.len, i++)
			var/playing_card/griffening/creature/Card = PC.cards[i]
			if (!istype(Card))
				continue
			if (Card.attributes & card_attribute == card_attribute)
				ret += Card
	...
	proc/player_effects_face_up(var/active_player)
		...
		for (var/i = 1, i <= 5, i++)
			...
			if (holder.card && holder.card.face_up)
				var/playing_card/griffening/effect/Card = holder.card.cards[1]
...
.answer {
	...
	margin-left: 5px;
		...
		addinfo("<h2>Spacemen the Grifening game</h2><strong>Player 1: </strong>[players[1]]<br/><strong>Player 2: </strong>[players[2]]<br/><br/>")
		/*if (current_player == 1)
			if (active_question)
				if (!question_output)
					generate_question_output()
				addinfo1("<div class='attention'>[question_output]</div>", "<strong>Player 1</strong> is currently playing.")
			else
				addinfo("<strong>Player 1</strong> is currently playing.")
		else if (current_player == 2)
			if (active_question)
				if (!question_output)
					generate_question_output()
				addinfo2("<div class='attention'>[question_output]</div>", "<strong>Player 2</strong> is currently playing.")
			else
				addinfo("<strong>Player 2</strong> is currently playing.")*/
		addinfo("<strong>Player [current_player] is currently playing.</strong><br><br>")
		addinfo("<h3>Field information</h3><strong>Area in play: </strong>")
		var/playing_card/griffening/area/area = get_face_up_area_card()
		if (istype(area))
			addinfo("[area.card_name]<br/>[area.card_data]<br/><br/>")
		else
			addinfo("&lt;none&gt;<br/><br/>")
		addinfo("<table><thead><tr><th>Player 1</th><th>Player 2</th></tr></thead><tbody><tr><td><strong>Creatures (left to right)</strong><br>")
		var/list/card_holders = mobs_card_holder[1]
		for (var/i = 1, i <= 5, i++)
			addinfo("<strong>Creature #[i]: </strong>")
			var/obj/griffening_card_holder/holder = card_holders[i]
			if (holder.card)
				var/obj/item/playing_cards/PC = holder.card
				var/playing_card/griffening/C = PC.cards[1]
				if (PC.face_up)
				...
				addinfo("&lt;none&gt;<br/>")
		addinfo("<br><strong>Effects (left to right)</strong><br/>")
		card_holders = effects_card_holder[1]
		for (var/i = 1, i <= 5, i++)
			addinfo("<strong>Effect #[i]: </strong>")
			var/obj/griffening_card_holder/holder = card_holders[i]
			if (holder.card)
				var/obj/item/playing_cards/PC = holder.card
				var/playing_card/griffening/C = PC.cards[1]
		...
		addinfo("<br><strong>Discard pile:</strong> [discards.len] cards<br/>")
		p1 += "<ul>"
		for (var/playing_card/griffening/C in discards)
		...
		if (discard)
			discards = discard.cards
		addinfo("<br><strong>Gibbed pile:</strong> [discards.len] cards<br/>")
		p1 += "<ul>"
		for (var/playing_card/griffening/C in discards)
			p1 += "<li>[C.card_name]</li>"
		p1 += "</ul>"

		addinfo("</td><td><strong>Creatures (left to right)</strong><br>")
...
/obj/griffening_area
	...
	var/griffening_controller/controller = null
	var/playing_card/griffening/area/card = null
	var/area_owner = 0
	anchored = 1
	density = 1
	...
	examine()
		if (!controller || !card)
			...
			boutput(usr, "Area card in effect: <em>[card.card_name]</em>")
	...
	Click(location, control, params)
		...
		if (!controller.players[player])
			...
			if (held.cards.len < 40 || held.cards.len > 80)
				...
				return
			for (var/playing_card/Card in held.cards)
				if (!istype(Card, /playing_card/griffening))
