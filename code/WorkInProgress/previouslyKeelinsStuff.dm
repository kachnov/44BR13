...
/proc/is_null_or_space(var/input)
	if (input == " " || input == "" || isnull(input)) return TRUE
	else return FALSE

/proc/lolcat_parse(var/text_roamer/R)
	var/new_string = ""
	var/used = 0
...
/proc/lolcat(var/string)
	...
	for (var/i = 0, i < length(string), i=i)
		var/parse_result/P = lolcat_parse(T)
		modded += P.string
		i += P.chars_used
...
//		for (var/obj/alien/weeds/V in T) qdel(V)
	...
	if (istype(center, /obj) || istype(center, /mob))
		var/message = "<strong>A [pick("burst", "gout", "pillar", "column")] of flame bursts from"
		if (istype(center, /obj))
			message += " the"
		message += " [center]"
		for (var/mob/living/L in range(2,get_turf(center)))
			if (center.fingerprintslast == L.key)
				L.visible_message("[message], striking you squarely in the [pick("face", "chest", "groin")]! It burns!</strong>")
				if (prob(10))
					L.ex_act(2.0)
				else
					L.ex_act(3.0)
			else
				L.visible_message("[message]!</strong>")
		center.ex_act(2.0)
		*/
	spawn (30)
		for (var/obj/hotspot/A in hotspots)
...
/obj/fake_attacker/attackby()
	...
	for (var/mob/M in oviewers(world.view,my_target))
		boutput(M, "<span style=\"color:red\"><strong>[my_target] flails around wildly.</strong></span>")
	my_target.show_message("<span style=\"color:red\"><strong>[src] has been attacked by [my_target] </strong></span>", 1) //Lazy.
	return
	...
	if (M == my_target)
		...
		if (prob(30))
			for (var/mob/O in oviewers(world.view , my_target))
				boutput(O, "<span style=\"color:red\"><strong>[my_target] stumbles around.</strong></span>")
	...
	if (get_dist(src,my_target) > 1)
		...
		if (prob(15))
			if (weapon_name)
				...
				my_target.show_message("<span style=\"color:red\"><strong>[my_target] has been attacked with [weapon_name] by [name] </strong></span>", 1)
				...
				my_target.show_message("<span style=\"color:red\"><strong>[name] has punched [my_target]!</strong></span>", 1)
				if (prob(33))
					if (!locate(/obj/overlay) in my_target.loc)
						fake_blood(my_target)
...
/proc/can_line(var/atom/source, var/atom/target, var/length=5)
	...
	while (current != target_turf)
		...
		if (current.density) return FALSE
