...
<em><strong>On the back of the note is a scribbling:</strong> When Blaise says the name of the target, the key of the other unlocks the knowledge.</em>"}

/obj/item/paper/otp
	name = "gibberish note"
	info = {"<em>There are multiple pages of gibberish in this notepad. The first page says:</em>
...
/obj/item/paper/intelligence_report
	...
	info = {"<strong>Clandestine Positronics intelligence report on possible threats regarding the facility</strong><br><br>

<em>This information is for the eyes of the facility administrator. If you are not authorized to read this report, please report to your facility administrator for bloodline termination.</em><br><br>
...
/obj/item/paper/empty_bloody
	...
	icon_state = "paper_caution_bloody"
	info = "<em>The paper is full of eraser marks and scratches but there is nothing legible on here.</em>"
	New()
		..()
		pixel_x = rand(-8, 8)
		pixel_y = rand(-8, 8)
...
/obj/item/paper/diner_invoice
	name = "bloody piece of paper"
	desc = "It seems to have some interesting scribblings."
	info = {"<html><head></head><body><strong>Delivery invoice</strong><br><br>
...
<strong>Total:</strong> 4134 space credits<br>
<strong>Thank</strong>... <em>the rest of the invoice is covered in blood and illegible</em>"}
	icon_state = "paper_caution_bloody"
...
/numbers_station // This is not a physical entity!
	...
	proc/process()
		...
		if (ticker.round_elapsed_ticks >= next_play)
			...
			logTheThing("debug", null, null, "<strong>Numbers station</strong>: [name] is broadcasting on frequency [frequency / 10].")
			gather_listeners()
			if (!listeners.len)
				logTheThing("debug", null, null, "<strong>Numbers station:</strong> [name] broadcast aborted: no listeners.")
				return
			spawn (10)
...
var/global/numbers_station/lincolnshire = new
...
/proc/lincolnshire_numbers(phase, data)
	var/list/ldata = data
	if (istype(ldata))
		logTheThing("data", null, null, "<strong>Numbers station</strong>: numbers: [ldata["numbers"]]")
		var/TP = ldata["numbers"]
		if (TP == null)
			return TRUE
		var/list/nums = splittext(TP, " ")
		if (nums.len < 21)
			logTheThing("debug", null, null, "<strong>Numbers station</strong> got too few numbers.")
			return 2
		for (var/i = 1, i <= 21, i++)
			lincolnshire.numbers[i] = text2num(nums[i])
		logTheThing("debug", null, null, "<strong>Numbers station<strong> woo success")
		return FALSE
	return 3
...
/turf/unsimulated/wall/adaptive/concrete
	...
	desc = "The last recorded incident of someone constructing something out of concrete was in 2017, and we all know how <strong>that</strong> ended."
...
/obj/critter/shade/scientist
	...
	CritterDeath()
		...
		walk_to(src,0)
		report_death()
		particleMaster.SpawnSystem(new /particleSystem/localSmoke("#000000", 5, get_turf(src)))
		qdel(src)
