...
/obj/tug_cart/
	...
	MouseDrop_T(var/atom/movable/C, mob/user)
		...
		load(C)
		visible_message("<strong>[user]</strong> loads [C] onto [src].")
	...
	MouseDrop(obj/over_object as obj, src_location, over_location)
		...
		if (!load)
			return
		visible_message("<strong>[user]</strong> unloads [load] from [src].")
		unload(over_object)
...
//	sealed_cabin = 0
	...
	eject_rider(var/crashed, var/selfdismount)
		...
		if (crashed)
			...
			boutput(rider, "<span style=\"color:red\"><strong>You are flung off of [src]!</strong></span>")
			rider.stunned = 8
			rider.weakened = 5
			for (var/mob/C in AIviewers(src))
				if (C == rider)
					continue
				C.show_message("<span style=\"color:red\"><strong>[rider] is flung off of [src]!</strong></span>", 1)
		...
		if (selfdismount)
			...
			for (var/mob/C in AIviewers(src))
				if (C == rider)
					continue
				C.show_message("<strong>[rider]</strong> dismounts from [src].", 1)
		rider.buckled = null
		rider = null
		overlays = null
		return
	...
	MouseDrop_T(var/atom/movable/C, mob/user)
		...
		if (rider || target.buckled || LinkBlocked(target.loc,loc) || isAI(user))
			return
	...
	disposing()
		if (rider)
			boutput(rider, "<span style=\"color:red\"><strong>[src] is destroyed!</strong></span>")
			eject_rider()
		..()
		return
