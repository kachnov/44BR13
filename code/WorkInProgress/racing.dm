...
/obj/racing_powerup_spawner
	...
	proc/spawnit()
		if (!(locate(/obj/racing_powerupbox) in loc))
			new/obj/racing_powerupbox(loc)
...
/obj/powerup/bananapeel
	...
	Click()
		...
		if (!istype(loc,/obj/racing_clowncar))
			qdel(src)
			return
...
/obj/racing_clowncar
	...
	Bump(var/atom/A)
		if (super && istype(A,/obj/racing_clowncar))
			var/obj/racing_clowncar/R = A
			if (!R.super)
				R.dir = pick(turn(dir,90),turn(dir,-90))
				step(R,R.dir)
				R.spin(6)
		return

	remove_air(amount as num)
		var/gas_mixture/Air = unpool(/gas_mixture)
		Air.oxygen = amount
		Air.temperature = 310
		return Air
