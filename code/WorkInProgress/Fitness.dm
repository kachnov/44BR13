	...
	clown
		...
		attack_hand(mob/user as mob)
			...
			if (narrator_mode)
				...
				playsound(loc, 'sound/vox/honk.ogg', 50, 1, -1)
			else
				playsound(loc, pick(hit_sounds), 25, 1, -1)
...
/obj/fitness/stacklifter
	...
	attack_hand(mob/user as mob)
		if (in_use)
			...
			usr.visible_message(text("<span style=\"color:red\"><strong>[usr] is [bragmessage]!</strong></span>"))
			var/lifts = 0
			while (lifts++ < 6)
				if (user.loc != loc)
					break
...
/obj/item/rubberduck/attack_self(mob/user as mob)
	if (spam_flag == 0)
		...
		spam_flag = 1
		if (narrator_mode)
			playsound(loc, 'sound/vox/duct.ogg', 50, 1)
		else
			playsound(loc, 'sound/items/rubberduck.ogg', 50, 1)
		if (prob(1))
			user.drop_item()
			playsound(loc, 'sound/ambience/lavamoon_strange_fx1.ogg', 50, 1) // this is gonna spook some people!!
			var/wacka = 0
			while (wacka++ < 50)
				sleep(2)
				pixel_x = rand(-6,6)
