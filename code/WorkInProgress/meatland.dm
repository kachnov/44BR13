...
/obj/stomachacid
	...
	New()
		..()
		var/reagents/R = new/reagents(25)
		reagents = R
		R.my_atom = src
...
/obj/meatlight
	...
	attackby(obj/item/O as obj, mob/user as mob)
		if (alive && O.force)
			health -= O.force / 4
			visible_message("<span style=\"color:red\"><strong>[user] bops [src] with [O]!</strong></span>")
			if (health <= 0)
				alive = 0
				visible_message("<span style=\"color:red\"><strong>[src]</strong> dies!</span>")
				icon_state = "light-dead"
				light.disable()
...
/obj/critter/monster_door
	...
	floor
		...
		New()
			..()
			if (loc)
				loc.invisibility = 100 //Hide the floor below us so people don't just right click and see two floors.

		attackby(obj/item/O as obj, mob/user as mob)
			if (alive && istype(O, /obj/item/crowbar))
				user.visible_message("<span style=\"color:red\"><strong>[user] jabs [src] with [O]!</strong></span>", "<span style=\"color:red\">You jab [src] with [O] and begin to pull!  Hold on!</span>")
				if (do_after(user, 20))
					playsound(loc, "sound/items/Crowbar.ogg", 50, 1)
					gibs(loc)
					if (loc)
						new /obj/item/tile/steel (loc)
						loc.invisibility = 0
	...
	CritterAttack(mob/M)
		if (ismob(M))
			attacking = 1
			visible_message("<span style=\"color:red\"><strong>[src]</strong> chomps down on [M]!</span>")
	...
	ai_think()
		if (task == "chasing")
			...
			icon_state = "[initial(icon_state)]-attack"
			opacity = 0
			if (target)
...
/obj/critter/monster_door/meat_head //Clash at Meathead
	...
	ai_think()
		if (task == "chasing")
			...
			if (target)
				if (prob(30))
					playsound(loc, pick(meathead_noises), 40, 1)
				if (get_dist(src, target) <= attack_range)
					var/mob/living/carbon/M = target
					if (M)
						CritterAttack(M)
	...
	CritterAttack(mob/M)
		...
		attacking = 1
		visible_message("<span style=\"color:red\">[src] slaps [M] with a meaty tendril!</span>")
	...
	attackby(obj/item/O as obj, mob/user as mob)
		if (istype(O, /obj/item/clothing/head))
			...
			O.layer = layer+0.1
			animate(O, pixel_x = 11 + (16 * (x - O.x)), pixel_y = (32 * (1 + y - O.y)), time = 2, loop = 1, easing = SINE_EASING)
			animate(pixel_x = 11 + (32 * (x - O.x)), pixel_y = (32 * (y - O.y)) + 45, time = 3, loop = 1, easing = SINE_EASING)
			...
			if (hat)
				hat.set_loc( loc )
				overlays -= hat
				animate(hat, pixel_x = 13, pixel_y = 27, transform = matrix(180, MATRIX_ROTATE), time = 2, loop = 1, easing = SINE_EASING)
				animate(pixel_x = 0, pixel_y = 0, transform = null, time = 30, loop = 3, easing = SINE_EASING)
				var/obj/item/clothing/head/old_hat = hat
				spawn (5)
					if (old_hat)
...
/obj/critter/blobman/meaty_martha
	...
	New()
		..()
		name = "[pick("grody", "clotty", "greasy", "meaty", "fleshy", "vile", "chunky", "putrid")] [pick("nugget", "bloblet", "pustule", "corpuscle", "viscera")]"
		icon_state = pick("meaty_mouth", "polyp", "goop")

	ChaseAttack(mob/M)
		. = target_missing_limb(M)
		if (. == "r_arm" || . == "l_arm" && M:limbs)
			visible_message("<span style=\"color:red\"><strong>[src] latches onto [M]'s stump!!</strong></span>")
			...
			visible_message("<span style=\"color:red\"><strong>[src]</strong> smacks against [M]!</span>")
			set_loc(M.loc)
			playsound(loc, "sound/effects/fleshbr1.ogg", 50, 1)
			if (iscarbon(M))
				if (prob(25))
	...
	CritterDeath()
		if (!alive) return
		src.visible_message("<strong>[src]</strong> collapses into viscera...")
		if (loc)
			gibs(loc)
		qdel(src)
...
/obj/critter/zombie/meatmonaut
	...
	CritterAttack(mob/M)
		if (prob(20))
			playsound(loc, "sound/misc/meatmonaut1.ogg", 50, 0)
		return ..()
...
/obj/item/disk/data/fixed_disk/meatland
	...
	New()
		...
		newfolder.name = "doc"
		root.add_file( newfolder )
		newfolder.add_file( new /computer/file/record/meatland/clueless (src))
		newfolder.add_file( new /computer/file/record/iomoon_corrupt {name = "log_00000000";} (src))
		newfolder.add_file( new /computer/file/record/meatland/solarium_ha_ha_ha (src))
		newfolder.add_file( new /computer/file/record/iomoon_corrupt {name = "log_aaaaaaa";} (src))
		newfolder.add_file( new /computer/file/record/meatland/technobabble_bs (src))
		newfolder.add_file( new /computer/file/record/meatland/david_is_dead (src))
...
/obj/item/disk/data/fixed_disk/meatland_medical
	...
	New()
		...
		newfolder.name = "doc"
		root.add_file( newfolder )
		newfolder.add_file( new /computer/file/record/meatland/whiskerdeath0 (src))
		newfolder.add_file( new /computer/file/record/iomoon_corrupt {name = "MEDLOG08";} (src))
		newfolder.add_file( new /computer/file/record/meatland/whiskerdeath1 (src))
...
<script language="JavaScript">
	...
	Topic(href, href_list)
		...
		if ("enter" in href_list)
			...
			if (uppertext(href_list["enter"]) == code)
				usr << output("!OK!&0", "cheget.browser:updateReadout")


				if (locked)
					locked = 0
					icon_state = "cheget_unlocked"
					visible_message("<span style=\"color:red\">[src]'s lock mechanism clicks unlocked.</span>")
					playsound(loc, "sound/items/Deconstruct.ogg", 65, 1)
					if (prob(50))
						visible_message("<span style=\"color:red\">[src] emits a happy bleep.</span>")
						playsound(loc, "sound/machines/cheget_goodbloop.ogg", 30, 1)

				else
					locked = 1
					icon_state = "cheget_closed"
					visible_message("<span style=\"color:red\">[src]'s lock mechanism clunks locked.</span>")
					playsound(loc, "sound/items/Deconstruct.ogg", 65, 1)
				...
				if (prob(33))
					...
					for (var/i = 1, i <= 4, i++)
						if (cmptext( copytext(href_list["enter"], i,i+1), copytext(code, i, i+1)))
							any_of_them_right++

					visible_message("<span style=\"color:red\">[src] emits a[(any_of_them_right > 1) ? "couple" : null] grumpy boop[(any_of_them_right > 1) ? "s" : null].</span>")
					playsound(loc, "sound/machines/cheget_grumpbloop.ogg", 30, 1)
...
/obj/machinery/computer3/luggable/cheget
	...
	setup_starting_os = /computer/file/terminal_program/os/cheget
	temp = ""
...
/computer/file/terminal_program/os/cheget
	name = "AUTH2"
	...
	initialize()
		...
		master.temp = ""
		master.temp_add = ""
		print_text("&#x041F;&#x043E;&#x0436;a&#x043B;y&#x0439;c&#x0442;a, &#x0432;c&#x0442;a&#x0432;&#x044C;&#x0442;e &#x043A;&#x043B;&#x044E;&#x0447;&#x0438; a&#x0432;&#x0442;o&#x0440;&#x0438;&#x044D;a&#x0446;&#x0438;&#x0438;.")

	input_text(text)
		if (..() || !inPasswordRequestMode)
			return

		print_text("&#x41D;&#x415;&#x412;&#x415;&#x420;&#x41D;&#x42B;&#x419; &#x041F;&#x410;&#x0420;&#x41E;&#x41B;&#x42C;")
	...
	receive_command(obj/source, command, signal/signal)
		...
		if (command == "key_auth")
			if (signal && signal.data["authcode"] && !(signal.data["authcode"] in knownKeys))
...
/obj/item/peripheral/cheget
	...
	Topic(href, href_list)
		...
		if (href_list["key"] && istype(usr.equipped(), /obj/item/device/key))
			...
			host.visible_message("<span style=\"color:red\">[loc] emits a grumpy boop.</span>")
			playsound(loc, "sound/machines/cheget_grumpbloop.ogg", 30, 1)
...
/obj/item/peripheral/cheget_key
	...
	return_status_text()
		return inserted_key ? "UNLOCKED" : "LOCKED"

	return_badge()
		if (inserted_key)
			. = "Key: <A HREF='?src=\ref[src];eject_key=1'>[inserted_key.name]</A>"
		else
	...
	Topic(href, href_list)
		...
		if (href_list["key"])
			if (istype(usr.equipped(), /obj/item/device/key/cheget) && !inserted_key)
				...
				spawn (10)
					if (inserted_key)
						...
						var/signal/authSignal = get_free_signal()
			...
			if (inserted_key)
				...
				playsound(loc, "sound/effects/thunk.ogg", 30, 1)
				inserted_key.set_loc(get_turf(loc))
				inserted_key = null
				spawn (10)
					if (!inserted_key)
						...
						var/signal/deauthSignal = get_free_signal()
	...
	attackby(obj/item/O as obj, mob/user as mob)
		if (istype(O, /obj/item/device/key))
			...
			if (findtext(O.name, "onyx"))
				opened = 1
				user.visible_message("<span style=\"color:red\"><strong>[user] inserts [O] into [src]!</strong></span>")
				playsound(loc, "sound/effects/thunk.ogg", 60, 1)
				qdel(O)

				visible_message("<span style=\"color:red\">Something pops out of [src]!</span>")
...
/obj/iomoon_puzzle/meat_ganglion
	...
	desc = "It's a big ol' ball of nerves.  Normally, these aren't the size of patio furniture, but.  um.  <em>future.</em>"
	icon = 'icons/misc/meatland.dmi'
	...
	activate()
		...
		playsound(loc, "sound/effects/fleshbr1.ogg", 50, 1)
		icon_state = "ganglion[++active]"
		light.enable()

		if (timer)
			if (timer > 3)
				icon_state = "ganglion_blink_slow"
				spawn ((timer - 3) * 10)
					icon_state = "ganglion_blink_fast"
					sleep(30)
					deactivate()
	...
	close()
		...
		for (var/mob/living/L in loc)
			if (prob(10))
				...
				L.visible_message("<span style=\"color:red\"><strong>[src] slams shut on [L]!</strong></span>")
				if (ishuman(L))
					L:sever_limb(pick("l_arm", "r_arm", "L_leg", "r_leg"))
				else
					random_brute_damage(L, 25)
			else
				L.visible_message("<span style=\"color:red\"><strong>[L] is gored by [src]!</strong></span>", "<span style=\"color:red\"><strong>OH SHIT</strong></span>")
				playsound(loc, "sound/effects/fleshbr1.ogg", 50, 1)
				L.gib()
...
/obj/item/parts/human_parts/meat_mutant
	...
	getMobIcon(var/lying)
		if (lying)
			...
			lyingImage = image('icons/mob/human.dmi', "[slot]_mutated_l")
			return lyingImage
			...
			standImage = image('icons/mob/human.dmi', "[slot]_mutated")
			return standImage
...
/obj/item/gun/gibgun
	...
	shoot(var/target,var/start,var/mob/user,var/POX,var/POY)
		...
		if ((last_shot + 15) <= world.time)
			...
			user.visible_message("<span style=\"color:red\"><strong>[user]</strong> blasts a lump of flesh at [target]!</span>")
			if (prob(15))
				user.emote("scream")
