...
/obj/machinery/abcu
	...
	proc/activate()
		...
		anchored = 1
		invalidCount = 0
		for (var/tileinfo/T in currentBp.roominfo)
			var/turf/pos = locate(text2num(T.posx) + x,text2num(T.posy) + y, z)
	...
	proc/build()
		...
		spawn (0)
			...
			for (var/tileinfo/T in currentBp.roominfo)
				...
				for (var/objectinfo/O in T.objects)
	...
	if (save.dir.Find("[key]"))
		...
		for (var/A in save.dir)
			...
			for (var/B in save.dir)
				...
				save.cd = "/[key]/[A]/[B]"
				var/objectinfo/O = new/objectinfo()
...
/*
	attack_self(mob/user as mob)
		for (var/tileinfo/T in roominfo)
