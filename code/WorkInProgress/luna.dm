...
/computer/file/record/moon_mail
	New()
		..()
...
/obj/item/disk/data/fixed_disk/lunar
	New()
		...
		var/computer/folder/currentFolder = new /computer/folder {name="mails";} (src)
		root.add_file(currentFolder)

		currentFolder.add_file( new /computer/file/record/moon_mail/renovation_it (src) )
		currentFolder.add_file( new /computer/file/record/moon_mail/no_journalists (src) )
		currentFolder.add_file( new /computer/file/record/moon_mail/no_radicals (src) )
		currentFolder.add_file( new /computer/file/record/moon_mail/ex_employee (src) )
		currentFolder.add_file( new /computer/file/record/moon_mail/saw_a_thing (src) )
		currentFolder.add_file( new /computer/file/record/moon_mail/saw_a_thing2 (src) )
		currentFolder.add_file( new /computer/file/record/moon_mail/weird_guest (src) )
		currentFolder.add_file( new /computer/file/record/moon_mail/weirder_guest (src) )

		currentFolder = new /computer/folder {name="bin";} (src)
		root.add_file(currentFolder)

		currentFolder.add_file( new /computer/file/terminal_program/email (src) )

		root.add_file( new /computer/file/terminal_program/secure_records {req_access = list(999);} (src) )
...
/obj/item/device/audio_log/lunar_01
	...
	New()
		..()
		tape = new /obj/item/audio_tape/lunar_01(src)
...
/obj/machinery/bot/guardbot/old/tourguide/lunar
	...
	setup_default_startup_task = /computer/file/guardbot_task/tourguide/lunar
	no_camera = 1
...
/computer/file/guardbot_task/tourguide/lunar

	wait_for_guests = 1

	look_for_neat_thing()

		if (istype(get_area(master), /area/solarium) && !(neat_things & NT_SOLARIUM))
			neat_things |= NT_SOLARIUM
		...
		for (var/atom/movable/AM in view(7, master))
			if (istype(AM, /mob/living/carbon/human))
				...
				if (!(neat_things & NT_BILL) && cmptext(H.real_name, "shitty bill"))
					neat_things |= NT_BILL

					master.visible_message("<strong>[master]</strong> points at [H].")
					master.speak("Oh no, not you again.  Uh, I mean...hey...guyyy.")

				if (!(neat_things & NT_DISCOUNT) && H.stat != 2 && (istype(H.wear_id, /obj/item/card/id) || (istype(H.wear_id, /obj/item/device/pda2) && H.wear_id:ID_card)))
					neat_things |= NT_DISCOUNT
				...
				if (!(neat_things & NT_CLOAKER) && H.invisibility > 0)
					master.speak("This is a reminder that cloaking technology is illegal within the inner solar system.  Please remain opaque to the visible spectrum as a courtesy to your fellow guests.  Thanks!")
					neat_things |= NT_CLOAKER
					return

				if (!(neat_things & NT_PONZI) && (locate(/obj/item/spacecash/buttcoin) in AM.contents))
					neat_things |= NT_PONZI
					master.speak("Um, I'm sorry [AM], we do not accept blockchain-based cryptocurrency as payment.  You aren't one of those guys who yell about gold on the apollo flag or something, right?")
					H.unlock_medal("To the Moon!",1)
					return

			if (istype(AM, /obj/machinery/bot/guardbot) && AM != master)
				if (istype(AM, /obj/machinery/bot/guardbot/old/tourguide) && !(neat_things & NT_OTHERGUIDE))
					neat_things |= NT_OTHERGUIDE
					src.master.visible_message("<strong>[master]</strong> nods professionally at [AM].<br>Well, really it's more of a shake from suddenly halting the drive motors, but you get the intent.")
			...
			if (istype(AM, /obj/critter/moonspy) && !(neat_things & NT_SPY))
				neat_things |= NT_SPY
				master.visible_message("<strong>[master]</strong> points at [AM].")
				master.speak("Attention guests, there appears to be a coat rack or something available if needed.")
				spawn (10)
					if (master)
						master.speak("You know, because of all the coats that you are clearly wearing.")

				return

			if (istype(AM, /obj/critter/domestic_bee) && !(neat_things & NT_BEE))
				neat_things |= NT_BEE
				master.visible_message("<strong>[master]</strong> points at [AM].")
				master.speak("Ah, a space bee!  Space bees count as minors under 12 for the purposes of ticket pricing.")

				return

			if ((istype(AM, /obj/item/luggable_computer/cheget) || istype(AM, /obj/machinery/computer3/luggable/cheget)) && !(neat_things & NT_CHEGET))
				neat_things |= NT_CHEGET
				master.speak("Huh, what's with the briefcase?  Did that come out of the security annex?  Somebody should probably call security or the embassy or something.")
				spawn (10)
					if (master)
						master.speak("I think they dealt with some kind of briefcase key the week before renovations started?  There's some sad office worker out there right now.")
				return
	...
	task_input(var/input)
		...
		if (input == "broken_door")
			...
			sleep(10)
			master.speak("Uh.  That isn't supposed to happen.")
			state = 0	//Yeah, let's find that route.

			spawn (10)
				if (master)
					master.speak("I guess we need to take another route.  Please follow me.")
					state = 0
					awaiting_beacon = 0
...
/obj/machinery/door/lunar_breakdoor
	...
	open()
		...
		playsound(loc, 'sound/machines/airlock_break_very_temp.ogg', 50, 1)
		spawn (0)
			flick("breakairlock1", src)
			icon_state = "breakairlock2"
			sleep (2)
			opacity = 0
			sleep(6)
...
/obj/decal/lunar_bootprint
	...
	attack_hand(mob/user as mob)
		...
		user.visible_message("<strong>[user]</strong> steps right into [name].", "<span style=\"color:blue\">You step into the footprint. Ha ha, oh man, your foot fits right into that!</span>")
...
/obj/critter/mannequin
	...
	CritterAttack(mob/M)
		attacking = 1
		visible_message("<span style=\"color:red\"><strong>[src]</strong> awkwardly bashes [target]!</span>")
		random_brute_damage(target, rand(5,15))
		playsound(loc, "sound/misc/automaton_spaz.ogg", 50, 1)
		spawn (10)
			attacking = 0
	...
	process()
		...
		if (prob(6))
			playsound(loc, "sound/misc/automaton_tickhum.ogg", 60, 1)
			visible_message("<span style=\"color:red\"><strong>[src] emits [pick("a soft", "a quiet", "a curious", "an odd", "an ominous", "a strange", "a forboding", "a peculiar", "a faint")] [pick("ticking", "tocking", "humming", "droning", "clicking")] sound.</span>")

		if (prob(6))
			playsound(loc, "sound/misc/automaton_ratchet.ogg", 60, 1)
			visible_message("<span style=\"color:red\"><strong>[src] emits [pick("a peculiar", "a worried", "a suspicious", "a reassuring", "a gentle", "a perturbed", "a calm", "an annoyed", "an unusual")] [pick("ratcheting", "rattling", "clacking", "whirring")] noise.</span>")

		if (prob(5))
			playsound(loc, "sound/misc/automaton_spaz.ogg", 50, 1)
			visible_message("<span style=\"color:red\"><strong>[src]</strong> [pick("turns", "pivots", "twitches", "spins")].</span>")
			dir = pick(alldirs)
...
/obj/critter/moonspy
	...
	ChaseAttack(mob/M)
		visible_message("<span style=\"color:red\"><strong>[src]</strong> launches itself towards [M]!</span>")
		if (prob(20)) M.stunned += rand(1,3)
		random_brute_damage(M, rand(2,5))

	CritterAttack(mob/M)
		attacking = 1
		visible_message("<span style=\"color:red\">The <strong>[name]</strong> [pick("conks", "whacks", "bops")] [target] with [pick(non_spy_weapons)]!</span>")
	...
	CritterDeath()
		...
		alive = 0
		walk_to(src,0)
		visible_message("<strong>[src]</strong> blows apart!  But not in a way at all like surveillance equipment.  More like a washing machine or something.")
...
/obj/machinery/embedded_controller/radio/maintpanel
	...
	New()
		...
		spawn (5)
			...
			if (!wired_connection)	//Find the data terminal if there is one around.
				var/turf/T = get_turf(src)
				var/obj/machinery/power/data_terminal/test_link = locate() in T
				if (test_link && !test_link.is_valid_master(test_link.master))
					wired_connection = test_link
					wired_connection.master = src

	attackby(obj/item/I, mob/user)
		if (istype(I, /obj/item/card/id))
			if (user && allowed(user, req_only_one_required))
	...
	post_signal(signal/signal, comm_line)
	...
	initialize()
		...
		var/computer/file/embedded_program/maintpanel/new_prog = new
		new_prog.master = src
		program = new_prog
		...
		for (var/mob/M in nearby)
			if ((M.client && M.machine == src))
				if (reason || updateFlags)
					dynamicUpdate(M, reason|updateFlags)
		...
		if (istype(usr, /mob/living/silicon))
			if (!(usr in nearby))
				if (usr.client && usr.machine==src)
					if (reason || updateFlags)
						dynamicUpdate(usr, reason|updateFlags)
						updateFlags = REASON_NONE
					else
						attack_ai(usr)
...
/computer/file/embedded_program/maintpanel
	name = "maintpnl"
	extension = "EPG"
	...
	proc/do_setup(var/setupString)
		...
		for (var/setupEntry in setupList)
			...
			if (cmptext(copytext(setupEntry, 1, 5), "fake"))
				. = text2path("/maintpanel_device_entry/dummy[copytext(setupEntry, 5)]")
				if (.)
					device_entries += new . (src, entryName)
				else
					device_entries += new /maintpanel_device_entry/dummy (src, entryName)
				continue
			...
			if (istype(controlTarget, /obj/machinery/door/airlock))
				device_entries += new /maintpanel_device_entry/airlock (src, controlTarget, entryName)

			else if (istype(controlTarget, /obj/machinery/door/poddoor))
				device_entries += new /maintpanel_device_entry/podlock (src, controlTarget, entryName)

			else if (istype(controlTarget, /obj/critter/mannequin))
				device_entries += new /maintpanel_device_entry/mannequin (src, controlTarget, entryName)

		while (device_entries.len < 16)
			device_entries += new /maintpanel_device_entry/dummy (src, pick("GEN$$E$C", "MANNEA83IN 13", "M@____$CC DOOR $$S9", "########?3"))
		...
		switch (command)
			...
			if ("button7")	//Activate
				if (state == PANELSTATE_ENTRY_MENU && selected_entry < device_entries.len)
					var/maintpanel_device_entry/currentEntry = device_entries[selected_entry + 1]
	...
	receive_signal(signal/signal, receive_method, receive_param)
		if (signal.data["sender"] in sessions)
			var/maintpanel_device_entry/entry = sessions[signal.data["sender"]]
			sessions -= signal.data["sender"]
			if (istype(entry) && (entry in device_entries) && signal.data["lock_status"])
	...
	proc/updateDisplay()
		if (state == PANELSTATE_MAIN_MENU)
			...
			for (printedCounter = 0, printedCounter < ENTRY_MAX, printedCounter++)
				var/maintpanel_device_entry/entry
			...
			var/maintpanel_device_entry/entry = device_entries[selected_entry + 1]
			if (!istype(entry))
				state = PANELSTATE_MAIN_MENU
				return
...
/maintpanel_device_entry
	var/name = "GENERIC"
	var/computer/file/embedded_program/maintpanel/master
	var/active = 0
	var/needs_hack = 0
	...
	proc/getControlMenu()
		return "  ACTIVE: [active ? "YES" : "NO"]"

	proc/receive_signal(signal/signal)
	...
	airlock
		...
		New(computer/file/embedded_program/maintpanel/newMaster, obj/machinery/door/airlock/targetDoor, entryName)
			..()
		...
		getControlMenu()
			return list("  SEALED: [active ? "NO" : "YES"]",\
	...
	podlock
		...
		New(computer/file/embedded_program/maintpanel/newMaster, obj/machinery/door/poddoor/targetDoor, entryName)
			..()
		...
		getControlMenu()
			if (ourDoor)
				active = !ourDoor.density

			return list("  DOOR TYPE: BLAST DOOR","  SEALED: [active ? "NO" : "YES"]","  ACTUATOR PRESSURE:","    ACT 1:  OK","    ACT 2:  OK","    ACT 3:  OK")
	...
	mannequin
		...
		getControlMenu()
			...
			"  ACTIVE: [(ourMannequin && ourMannequin.alive) ? (active ? "YES" : "NO") : "NO"]",\
			"  CONDITION: [ourMannequin && ourMannequin.alive ? "OK" : "REPAIRS NEEDED"]")

		New(computer/file/embedded_program/maintpanel/newMaster, obj/critter/mannequin/mannequin, entryName)
			..()
			...
			master = newMaster
			active = mannequin.does_creepy_stuff
			if (mannequin.typeName)
				mannequinName = uppertext(mannequin.typeName)
	...
	dummy	//This is a fake entry that exists to fill space, not to be confused with the mannequin type.
		...
		New(computer/file/embedded_program/maintpanel/newMaster, entryName)
			..()
		...
		if (user.r_hand == src || user.l_hand == src)
			if (amount == 0)
				...
				if (amount == 0)
					...
					name = "An empty Farty Snott's bag."
		else
			return ..()
		return
...
/obj/fake_dwarf_bomb
	...
	attack_hand(mob/living/user)
		...
		user.visible_message("<strong>[user]</strong> prods [src].", "You prod at [src].  It's a pretty accurate replica, it seems.  Neat.")
		spawn (10)
			visible_message("<span style='color:red'>[src] gives a grumpy beep! <strong><font style='font-size:200%;'>OH FUCK</font></strong></span>")

			playsound(loc, "sound/weapons/armbomb.ogg", 50)

			sleep(30)
			//do tiny baby explosion noise
			//Todo: a squeakier blast sound.
			playsound(loc, "sound/effects/Explosion2.ogg", 40, 0, 0, 4)

			new /obj/effects/explosion/tiny_baby (loc)
...
/obj/effects/explosion/tiny_baby
	New()
		..()
		transform = matrix(0.5, MATRIX_SCALE)
