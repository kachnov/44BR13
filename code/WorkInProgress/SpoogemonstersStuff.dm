...
/obj/machinery/chem_dispenser_admin/
	...
	proc/beakerSpecsToJson()
		...
		for (var/beakerSpec/S in beakerSpecs)
			if (!first)
				json += ","
			else
				first = 0
	...
	Topic(href, href_list)
		switch(href_list["action"])
			...
			if ("newBeaker")
				beakerSpecs.len++
				beakerSpecs[beakerSpecs.len] = new /beakerSpec()
	...
	proc/grenade(var/name as text)
		var/obj/item/chem_grenade/adminGrenade = new /obj/item/chem_grenade(loc)
	...
	proc/syringe(var/name as text)
		var/obj/item/reagent_containers/syringe/adminSyringe = new(loc)
		...
		var/beakerSpec/S = beakerSpecs[1]
		var/reagents/R = S.createReagents()
	...
	proc/lbeaker(var/name as text)
		var/obj/item/reagent_containers/glass/beaker/large/adminBeaker = new(loc)
	...
	proc/patch(var/name as text)
		var/obj/item/reagent_containers/patch/adminPatch = new(loc)
	...
	proc/panel()
		...
		//setup
		var/tag/page/html = new
		var/tag/heading/h = new (1)
		var/tag/title/title = new
		html.addToBody(h)
		...
		var/tag/div/span = new
		span.addClass("span8")
		row.addChildElement(span)
		var/tag/select/chemSelect = new
		...
		span = new
		var/tag/input/chemAmount = new ("text")
		...
		var/tag/div/beakerContainer = new
		...
		var/tag/cssinclude/bootstrapResponsive = new
		...
		var/tag/scriptinclude/jsviews = new
		jsviews.setSrc(resource("js/jsviews.min.js"))
		html.addToBody(jsviews)

		var/tag/script/beakerTemplate = new
		...
		span.addClass("span3")
		row.addChildElement(span)
		var/tag/button/add3 = new
		add3.setText("Add #3")
		add3.setId("addButton3")
		span.addChildElement(add3)
		var/tag/button/remove3 = new
		...
		span.addClass("span9")
		row.addChildElement(span)
		var/tag/select/multi/added3 = new
