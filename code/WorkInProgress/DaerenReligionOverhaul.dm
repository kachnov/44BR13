/obj/screen/ability/chaplain
	clicked(params)
		var/targetable/chaplain/spell = owner
		if (!istype(spell))
			return
		if (!spell.holder)
			return
...
/abilityHolder/religious
	...
	New()
		..()
		addAbility(/targetable/chaplain/chooseReligion)
...
/proc/assemble_name(var/abilityHolder/religious/religiousHolder)
...
/targetable/chaplain/chooseReligion
	...
	cast()
		var/abilityHolder/religious/religiousHolder = holder
		if (..())
			return TRUE
		...
		switch (domainChoice)
			...
			if (8)
				...
				religiousHolder.addAbility(/targetable/chaplain/chaplainDemag)
				religiousHolder.addAbility(/targetable/chaplain/sootheMachineSpirits)
				assemble_name(religiousHolder)
			if (9)
				religiousHolder.god_domain = "Nature"
				religiousHolder.addAbility(/targetable/chaplain/blessWeed)
				religiousHolder.addAbility(/targetable/chaplain/fortifySeed)
				assemble_name(religiousHolder)

		religiousHolder.removeAbility(/targetable/chaplain/chooseReligion)
		return
...
/targetable/chaplain/fortifySeed
	name = "Fortify Seed"
...
/targetable/chaplain/chaplainDemag
	name = "Demag"
...
/targetable/chaplain/sootheMachineSpirits
	...
	cast(atom/T)
		...
		spawn (30)
		E.grump = 0
		E.visible_message("<span style=\"color:blue\"><strong>The [E] suddenly seems very chill!</strong></span>")
