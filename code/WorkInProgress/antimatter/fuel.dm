...
/obj/item/fuel/attackby(obj/item/fuel/F, mob/user)
	if (istype(src, /obj/item/fuel/antiH))
		if (istype(F, /obj/item/fuel/antiH))
			fuel += F.fuel
			F.fuel = 0
			boutput(user, "You have added the anti-Hydorgen to the storage ring, it now contains [fuel]kg")
	...
	if (istype(src, /obj/item/fuel/H))
		if (istype(F, /obj/item/fuel/H))
			fuel += F.fuel
			F.fuel = 0
			boutput(user, "You have added the Hydorgen to the storage ring, it now contains [fuel]kg")
		if (istype(F, /obj/item/fuel/antiH))
			fuel += F.fuel
			qdel(src)
...
/obj/item/fuel/attack(mob/M as mob, mob/user as mob)
	if (user != M)
		user.visible_message("<span style=\"color:red\">[user] is trying to force [M] to eat the [content]!</span>")
		if (do_mob(user, M, 40))
			user.visible_message("<span style=\"color:red\">[user] forced [M] to eat the [content]!</span>")
			injest(M)
	else
