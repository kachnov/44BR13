...
var/global/wizard_zone_controller/wizard_zone_controller = null
...
/turf/unsimulated/wall/adaptive/wizard_fake
	...
	proc/toggle()
		if (density || opening == -1)
			open()
		else if (!density || opening == 1)
			close()
...
/obj/cover
	...
	New()
		..()
		dummies += new /obj/border_dummy { dir = 1; }(loc)
		dummies += new /obj/border_dummy { dir = 2; }(loc)
		dummies += new /obj/border_dummy { dir = 4; }(loc)
		dummies += new /obj/border_dummy { dir = 8; }(loc)
...
/obj/item/wizard_crystal
	...
	var/assoc_material = /material/crystal/wizard/quartz
	icon = 'icons/turf/adventure.dmi'

	proc/create_bar(var/obj/machinery/portable_reclaimer/creator)
		var/material/MAT = new assoc_material()
		...
		BAR.setMaterial(MAT)
		playsound(loc, creator.sound_process, 40, 1)
	...
	quartz
		name = "enchanted quartz"
		assoc_material = /material/crystal/wizard/quartz
	...
	topaz
		...
		light_g = 0.8
		light_b = 0.5
		assoc_material = /material/crystal/wizard/topaz

	amethyst
		name = "enchanted amethyst"
		light_r = 0.6
		light_g = 0.4
		assoc_material = /material/crystal/wizard/amethyst
	...
	ruby
		...
		light_g = 0.1
		light_b = 0.2
		assoc_material = /material/crystal/wizard/ruby
	...
	sapphire
		...
		light_g = 0.4
		light_b = 0.7
		assoc_material = /material/crystal/wizard/sapphire
	...
	emerald
		...
		light_g = 0.8
		light_b = 0.4
		assoc_material = /material/crystal/wizard/emerald
...
/obj/wizard_light
	...
	attack_hand(var/mob/user)
		if (crystal)
			user.put_in_hand_or_drop(crystal)
			overlays.len = 0
			crystal = null
			light.disable()
			name = "empty crystal socket"
...
/obj/bookcase
	...
	button
		...
		attack_hand(var/mob/user)
			if (user.y != y || user.x < x - 1 || user.x > x + 1)
				return FALSE
			if (!pressed)
				pressed = 1
