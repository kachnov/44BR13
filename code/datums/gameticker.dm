var/global/controller/gameticker/ticker = null
/* -- moved to _setup.dm
...
/controller/gameticker
	...
	var/hide_mode = 0
	var/game_mode/mode = null
	var/event_time = null
	var/event = 0

	var/list/mind/minds = list()
	...
	var/ai_laws/centralized_ai_laws

	var/skull_key_assigned = 0

/controller/gameticker/proc/pregame()
	pregame_timeleft = 120
	boutput(world, "<strong><FONT color='blue'>Welcome to the pre-game lobby!</FONT></strong>")
...
/controller/gameticker/proc/setup()
	set background = 1
	//Create and announce mode
	if (master_mode in list("secret","wizard","alien"))
		hide_mode = 1

	#define BATTLE_ROYAL_MODE

	#ifndef BATTLE_ROYAL_MODE
	if ((master_mode=="random") || (master_mode=="secret")) mode = config.pick_random_mode()
	else mode = config.pick_mode(master_mode)
	#else
	mode = new /game_mode/_44BR13
	#endif

	if (hide_mode)
		var/modes = sortList(config.get_used_mode_names())
		boutput(world, "<strong>The current game mode is a secret!</strong>")
		boutput(world, "<strong>Possibilities:</strong> [english_list(modes)]")
	else
		mode.announce()

	// uhh is this where this goes??
	centralized_ai_laws = new /ai_laws/asimov()
	...
	if (!can_continue)
		...
		boutput(world, "<strong>Error setting up [master_mode].</strong> Reverting to pre-game lobby.")

		spawn pregame()

		return FALSE
/*
	if (!istype(mode, /game_mode/construction) && map_setting != "DESTINY")
		ooc_allowed = 0
		boutput(world, "<strong>OOC has been automatically disabled until the round ends.</strong>")*/

	if (map_setting == "DESTINY")
		looc_allowed = 1
		boutput(world, "<strong>LOOC has been automatically enabled.</strong>")
	...
	spawn (0)
		...
		if (istype(random_events,/event_controller))

			if (!istype(mode, /game_mode/_44BR13))
		...
		logTheThing("ooc", null, null, "<strong>Current round begins</strong>")
		boutput(world, "<FONT color='blue'><strong>Enjoy the game!</strong></FONT>")
		boutput(world, "<span style=\"color:blue\"><strong>Protip:</strong> [pick(tips)]</span>")
...
/controller/gameticker
		...
		for (var/mob/player in mobs)
			...
			if (player.mind && !istype(player, /mob/new_player) && player.client)
				if (!(player.mind in ticker.minds))
					if (periodic_check == 1)
						logTheThing("debug", player, null, "<strong>Gameticker fallback:</strong> re-added player to ticker.minds.")
					else
						logTheThing("debug", player, null, "<strong>Gameticker setup:</strong> added player to ticker.minds.")
	...
	proc/implant_skull_key()
		...
		if (!skull_key_assigned && ticker.minds.len > 5) //Okay enough gaming the system you pricks
			...
			logTheThing("debug", null, null, "<strong>SpyGuy/collar key:</strong> Did not implant a key because there was not enough players.")
	...
	proc/equip_characters()
		...
		for (var/mob/living/critter/C in mobs)
			if (C.mind && C.mind.assigned_role && C.mind.assigned_role != "MODE")
				spawn (0)
					C.job_spawn(C.mind.assigned_role, FALSE)
	...
	proc/process()
		...
		if (last_readd_lost_minds_to_ticker && world.time > last_readd_lost_minds_to_ticker + 1800)
			add_minds(1)
			last_readd_lost_minds_to_ticker = world.time
		...
		if (mode.check_finished())
			...
			if (!ooc_allowed)
				ooc_allowed = 1
				boutput(world, "<strong>OOC is now enabled.</strong>")

			spawn (50)
				boutput(world, "<span style=\"color:blue\"><strong>Restarting soon</strong></span>")
...
/controller/gameticker/proc/declare_completion()
	set background = 1
	...
	for (var/mind/crewMind in minds)
		...
		var/count = 0
		var/allComplete = 1
		for (var/objective/crew/CO in crewMind.objectives)
			count++
			if (CO.check_completion())
				boutput(crewMind.current, "<strong>Objective #[count]</strong>: [CO.explanation_text] <span style=\"color:green\"><strong>Success</strong></span>")
				if (!isnull(CO.medal_name) && !isnull(crewMind.current))
					crewMind.current.unlock_medal(CO.medal_name, CO.medal_announce)
			else
				boutput(crewMind.current, "<strong>Objective #[count]</strong>: [CO.explanation_text] <span style=\"color:red\">Failed</span>")
				allComplete = 0
	...
	/*boutput(world, "<strong>The crew's final score is:</strong>")
	boutput(world, "<strong><font size='4'>[score_crewscore]</font></strong>")*/
	...
	for (var/mob/living/silicon/ai/aiPlayer in mobs)
		if (aiPlayer.stat != 2)
			boutput(world, "<strong>The AI, [aiPlayer.name] ([aiPlayer.key]) had the following laws at the end of the game:</strong>")
		else
			boutput(world, "<strong>The AI, [aiPlayer.name] ([aiPlayer.key]) had the following laws when it was deactivated:</strong>")
	...
	if (successfulCrew.len)
		boutput(world, "<strong>The following crewmembers completed all of their Crew Objectives:</strong>")
		for (var/i in successfulCrew)
			boutput(world, "<strong>[i]</strong>")
		boutput(world, "Good job!")
	else
		boutput(world, "<strong>Nobody completed all of their Crew Objectives!</strong>")
	#endif
	#ifdef MISCREANTS
	boutput(world, "<strong>Miscreants:</strong>")
	if (miscreants.len == 0) boutput(world, "None!")
	for (var/mind/miscreantMind in miscreants)
		...
		boutput(world, "<strong>[miscreant_info] was a miscreant!</strong>")
		for (var/objective/miscreant/O in miscreantMind.objectives)
			boutput(world, "Objective: [O.explanation_text] <strong>Maybe</strong>")
	#endif
...
/////
/*
/controller/gameticker/proc/game_process()
			...
			do
				if (!( shuttle_frozen ))
					if (timing == 1)
						...
						if (timing == -1.0)
							timeleft += 10
							if (timeleft >= shuttle_time_to_arrive)
								timeleft = null
								timing = 0
				if (prob(0.5))
					spawn_meteors()
				if (timeleft <= 0 && timing)
					timeup()
				sleep(10)
			while (processing)
			return
		...
		if ("AI malfunction")
			do
				...
				if (ticker.AItime == 6000)
					boutput(world, "<FONT size = 3><strong>Cent. Com. Update</strong> AI Malfunction Detected</FONT>")
