...
/obj/landmark/map
	...
	New()
		if (name && (name == "COG2" || name == "DESTINY"))
			map_setting = name
		qdel(src)
...
/turf/simulated/wall/auto
	...
	proc/update_icon()
		...
		for (var/dir in cardinal)
			...
			if (T.type == type || (T.type in connects_to))
				builtdir |= dir
			else if (connects_to)
		...
		icon_state = "[mod][builtdir][d_state ? "C" : null]"
		if (light_mod)
			RL_SetSprite("[light_mod][builtdir]")

		if (connect_overlay)
			if (overlaydir)
				if (!connect_image)
					connect_image = image(icon, "connect[overlaydir]")
				else
					connect_image.icon_state = "connect[overlaydir]"
				UpdateOverlays(connect_image, "connect")
			else
				UpdateOverlays(null, "connect")
...
/turf/simulated/wall/auto/reinforced
	...
	/* ----- End Deconstruction ----- */
		...
		if (material)
			var/fail = 0
			if (prob(material.getProperty(PROP_INSTABILITY)))
				fail = 1
			if (material.quality < 0) if (prob(abs(material.quality)))
				fail = 1
			if (fail)
