...
/limb/hitscan
	...
	attack_range(atom/target, var/mob/user, params)
		if (next_shot_at > ticker.round_elapsed_ticks)
			return
		user.visible_message("<b style='color:red'>[user] fires at [target] with the [holder.name]!</strong>")
...
/limb/gun
	...
	arm38
		proj = new/projectile/bullet/revolver_38
		shots = 3
		current_shots = 3
		cooldown = 30
		reload_time = 200
	...
	abg
		proj = new/projectile/bullet/abg
		shots = 6
		current_shots = 6
		cooldown = 30
		reload_time = 300
	...
	phaser
		proj = new/projectile/laser/light
		shots = 1
		current_shots = 1
		cooldown = 30
		reload_time = 30
	...
	cutter
		proj = new/projectile/laser/drill/cutter
		shots = 1
		current_shots = 1
		cooldown = 30
		reload_time = 30
	...
	artillery
		proj = new/projectile/bullet/autocannon
		shots = 1
		current_shots = 1
		cooldown = 50
		reload_time = 50
	...
	disruptor
		proj = new/projectile/disruptor/high
		shots = 1
		current_shots = 1
		cooldown = 40
		reload_time = 40
	...
	glitch
		proj = new/projectile/bullet/glitch
		shots = 1
		current_shots = 1
		cooldown = 40
		reload_time = 40
	...
	fire_elemental
		proj = new/projectile/bullet/flare
		shots = 1
		current_shots = 1
		cooldown = 40
		reload_time = 40
...
/limb/mouth
	...
	harm(mob/target, var/mob/user)
		...
		if (prob(80) || target.stunned || target.weakened || target.paralysis || target.stat || target.restrained())
			...
			var/attackResults/msgs = user.calculate_melee_attack(target, affecting, 3, 8, 0)
			user.attack_effects(target, affecting)
			msgs.base_attack_message = "<strong><span style='color:red'>[user] bites [target]!</span></strong>"
			msgs.played_sound = "sound/weapons/werewolf_attack1.ogg"
			msgs.flush(0)
			user.HealDamage("All", 2, 0)
		else
			user.visible_message("<strong><span style='color:red'>[user] attempts to bite [target] but misses!</span></strong>")
...
/limb/bear
	...
	harm(mob/target, var/mob/living/user, var/no_logs = 0)
		...
		var/obj/item/affecting = target.get_affecting(user)
		var/attackResults/msgs = user.calculate_melee_attack(target, affecting, 6, 10, 0)
		user.attack_effects(target, affecting)
		var/action = pick("maim", "maul", "mangle", "rip", "claw", "lacerate", "mutilate")
		msgs.base_attack_message = "<strong><span style='color:red'>[user] [action]s [target] with their [holder]!</span></strong>"
...
/limb/wendigo
	...
	proc/accident(mob/target, mob/living/user)
		if (prob(25))
			...
			user.visible_message("<span style=\"color:red\"><strong>[user] accidentally claws [target] while trying to [user.a_intent] them!</strong></span>", "<span style=\"color:red\"><strong>You accidentally claw [target] while trying to [user.a_intent] them!</strong></span>")
			harm(target, user, 1)
			return TRUE
		return FALSE
	...
	harm(mob/target, var/mob/living/user, var/no_logs = 0)
		...
		var/obj/item/affecting = target.get_affecting(user)
		var/attackResults/msgs = user.calculate_melee_attack(target, affecting, 6, 9, rand(5,9) * quality)
		user.attack_effects(target, affecting)
		var/action = pick("maim", "maul", "mangle", "rip", "claw", "lacerate", "mutilate")
		msgs.base_attack_message = "<strong><span style='color:red'>[user] [action]s [target] with their [holder]!</span></strong>"
		msgs.played_sound = "sound/effects/bloody_stab.ogg"
...
/limb/abomination
	...
	attack_hand(atom/target, var/mob/living/user, var/reach)
		...
		if (istype(target, /obj/critter))
			...
			if (weak == 1)
				...
				user.visible_message("<span style=\"color:red\"><strong>[user] slashes viciously at [victim]!</strong></span>")
				...
				if (prob(66) && T && isturf(T))
					user.visible_message("<span style=\"color:red\"><strong>[user] savagely punches [victim], sending them flying!</strong></span>")
					victim.health -= 6 * victim.brutevuln
					spawn (0)
						victim.throw_at(T, 10, 2)
				else
					user.visible_message("<span style=\"color:red\"><strong>[user] punches [victim]!</span>")
	...
	disarm(mob/target, var/mob/living/user)
		...
		var/obj/item/affecting = target.get_affecting(user)
		var/attackResults/disarm/msgs = user.calculate_disarm_attack(target, affecting)

		if (!msgs || !istype(msgs))
			return

		if (weak == 1) // Werewolves get a guaranteed knockdown.
			if (!target.anchored && prob(50))
				send_flying = 1

			msgs.played_sound = 'sound/weapons/thudswoosh.ogg'
			user.werewolf_audio_effects(target, "disarm")
			msgs.base_attack_message = "<span style=\"color:red\"><strong>[user] [pick("clocks", "strikes", "smashes")] [target] with a [pick("fierce", "fearsome", "supernatural", "wild", "beastly")] punch, forcing them to the ground!</strong></span>"
			...
			if (prob(25) && ishuman(target))
				...
				msgs.played_sound = 'sound/effects/bloody_stab.ogg'
				msgs.base_attack_message = "<span style=\"color:red\"><strong>[user] whips [HH] with the sharp edge of a chitinous tendril, shearing off their [limb_name]!</span>"
				...
				msgs.played_sound = 'sound/weapons/thudswoosh.ogg'
				msgs.base_attack_message = "<span style=\"color:red\"><strong>[user] shoves [target] with a [pick("powerful", "fearsome", "intimidating", "strong")] tendril[send_flying == 0 ? "" : ", forcing them to the ground"]!</strong></span>"
				msgs.damage = rand(1,2)

		logTheThing("combat", user, target, "diarms %target% with [weak == 1 ? "werewolf" : "abomination"] arms at [log_loc(user)].")
	...
	harm(mob/target, var/mob/living/user)
		...
		var/send_flying = 0 // 1: a little bit | 2: across the room
		var/obj/item/affecting = target.get_affecting(user)
		var/attackResults/msgs = user.calculate_melee_attack(target, affecting)
		...
		if (weak == 1)
			if (send_flying == 2)
				msgs.base_attack_message = "<span style=\"color:red\"><strong>[user] delivers a supernatural punch, sending [target] flying!</strong></span>"
			else
				if (prob(25))
					msgs.base_attack_message = "<span style=\"color:red\"><strong>[user] mauls [target] viciously[send_flying == 0 ? "" : ", forcing them to the ground"]!</strong></span>"
				else
					msgs.base_attack_message = "<span style=\"color:red\"><strong>[user] slashes viciously at [target][send_flying == 0 ? "" : ", forcing them to the ground"]!</strong></span>"
			...
			if (send_flying == 2)
				msgs.base_attack_message = "<span style=\"color:red\"><strong>[user] delivers a savage blow, sending [target] flying!</strong></span>"
			else
				msgs.base_attack_message = "<span style=\"color:red\"><strong>[user] punches [target] with a [pick("powerful", "fearsome", "intimidating", "strong")] tendril[send_flying == 0 ? "" : ", forcing them to the ground"]!</strong></span>"
		...
		logTheThing("combat", user, target, "punches %target% with [weak == 1 ? "werewolf" : "abomination"] arms at [log_loc(user)].")
		user.attack_effects(target, affecting)
		msgs.flush(SUPPRESS_LOGS)
		return
