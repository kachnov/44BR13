...
var/global/v_space/v_space_network/Station_VNet = null
...
//			return
		...
		if (user.mind.virtual)
			...
			character = V
			user.visible_message("<span style=\"color:blue\"><strong>[user] logs in!</strong></span>")
		else
		...
		character.show_text("<h2><font color=red><strong>Death in virtual reality will result in a log-out. You can also press one of the logout buttons to leave.</strong></font></h2>", "red")
	...
	proc/Leave_Vspace(var/mob/living/carbon/human/virtual/user)
		...
		for (var/mob/O in oviewers())
			boutput(O, "<span style=\"color:red\"><strong>[user] logs out!</strong></span>")
