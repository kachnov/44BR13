...
/buildmode_holder
	...
	New(var/client/C)
		...
		for (var/T in subtypesof(/buildmode))
			var/buildmode/M = new T(src)
			if (!mode)
				select_mode(M)
			modes_cache += M.name
			modes_cache[M.name] = M

	proc/select_mode(var/buildmode/M)
		if (mode)
			mode.deselected()
		mode = M
		M.selected()
...
/client/proc/togglebuildmode()
	...
	if (!buildmode)
		buildmode = new /buildmode_holder(src)
