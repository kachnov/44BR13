/targetable/grinch/poison
	...
	cast(mob/target)
		...
		if (RC && istype(RC))
			if (the_poison.len > 1)
				if (!RC.reagents)
					RC.reagents = new /reagents(amount_per_poison * the_poison.len)
					RC.reagents.my_atom = RC

				if (RC.reagents)
					for (var/P in the_poison)
						if (RC.reagents.total_volume + amount_per_poison >= RC.reagents.maximum_volume)
							RC.reagents.maximum_volume += amount_per_poison
						RC.reagents.add_reagent(P, amount_per_poison)
