...
/obj/icecube
	...
	attack_hand(mob/user as mob)
		user.visible_message("<span class='combat'><strong>[user]</strong> kicks [src]!</span>", "<span style=\"color:blue\">You kick [src].</span>")
	...
	disposing()
		for (var/atom/movable/AM in src)
			if (ismob(AM))
				var/mob/M = AM
				M.visible_message("<span style=\"color:red\"><strong>[M]</strong> breaks out of [src]!</span>","<span style=\"color:red\">You break out of [src]!</span>")
			AM.set_loc(loc)

		if (steam_on_death)
			if (!(locate(/effects/system/steam_spread) in loc))
