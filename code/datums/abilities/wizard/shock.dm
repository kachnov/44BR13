/targetable/spell/shock
	...
	cast(mob/target)
		if (!holder)
			return
		holder.owner.visible_message("<span style=\"color:red\"><strong>[holder.owner] begins to cast a spell on [target]!</strong></span>")
		...
		if (do_mob(holder.owner, target, 10))
			...
			if (holder.owner.wizard_spellpower())
				...
				var/count = 0
				for (var/mob/living/L in oview(arc_range, target))
					if (iswizard(L))
						continue
					count++
				for (var/mob/living/L in oview(arc_range, target))
					if (iswizard(L))
						continue
					arcFlash(target, L, max(wattage / count, 1)) // adds some randomness to the damage
