...
/proc/equip_wizard(mob/living/carbon/human/wizard_mob, var/robe = 0)
	...
	var/abilityHolder/H = wizard_mob.add_ability_holder(/abilityHolder/wizard)
	H.addAbility(/targetable/spell/phaseshift)
	H.addAbility(/targetable/spell/magicmissile)
	H.addAbility(/targetable/spell/clairvoyance)
...
/mob/living/carbon/human/wizard_spellpower()
	var/magcount = 0
	if (!src) return FALSE // ??
	if (bioHolder.HasEffect("arcane_power") == 2)
		magcount += 10
	for (var/obj/item/clothing/C in contents)
		if (C.magical) magcount += 1
	if (istype(r_hand, /obj/item/staff))
		magcount += 2
	if (istype(l_hand, /obj/item/staff))
		magcount += 2
	if (magcount >= 4) return TRUE
	else return FALSE
...
/mob/living/critter/wizard_spellpower()
	...
	for (var/obj/item/clothing/C in contents)
		if (C.magical) magcount += 1
	if (find_in_hands(/obj/item/staff))
		magcount += 2
	if (magcount >= 4) return TRUE
	else return FALSE
...
/mob/living/carbon/human/wizard_castcheck(var/offensive = 0)
	...
	if (getarea.name == "Chapel" || getarea.name == "Chapel Office")
		if (get_corruption_percent() < 40)
			...
			return FALSE
	if (offensive == 1 && bioHolder.HasEffect("arcane_shame"))
...
/obj/screen/ability/spell
	clicked(params)
		var/targetable/spell/spell = owner
...
/targetable/spell
	preferred_holder_type = /abilityHolder/wizard
	var
		requires_robes = 0
		offensive = 0
		cooldown_staff = 0
	...
	proc/calculate_cooldown()
		...
		if (user && user.bioHolder)
			switch (user.bioHolder.HasEffect("arcane_power"))
				...
				if (2)
					cool = 1
		if (cooldown_staff && !user.wizard_spellpower())
			cool *= 1.5
		return cool
	...
	tryCast(atom/target)
		if (!holder || !holder.owner)
			return TRUE
		var/abilityHolder/wizard/H = holder
		if (H.locked && ignore_holder_lock != 1)
			boutput(holder.owner, "<span style=\"color:red\">You're already casting an ability.</span>")
			return TRUE // ASSHOLES
		...
		if (dont_lock_holder != 1)
			H.locked = 1
		if (cooldown_staff && !holder.owner.wizard_spellpower())
	...
	updateObject()
		if (!holder || !holder.owner)
			qdel(src)
		if (!object)
			object = new /obj/screen/ability/spell()
		object.icon = icon
		if (last_cast > world.time)
	...
	castcheck()
		return holder.owner.wizard_castcheck(offensive)