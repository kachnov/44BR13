/abilityHolder/revenant
	topBarRendered = 1
	usesPoints = 1
	var/channeling = 0

	var/abilityHolder/wraith/relay = null
	var/bioEffect/hidden/revenant/revenant = null
...
/bioEffect/hidden/revenant
	...
	proc/wraithPossess(var/mob/wraith/W)
		...
		if (wraith.mind) // theoretically shouldn't happen
			wraith.mind.transfer_to(owner)
		else
			wraith.client.mob = owner
	...
	proc/RevenantDeath()
		...
		if (owner.mind)
			owner.mind.transfer_to(wraith)
		else if (owner.client)
			owner.client.mob = wraith
		wraith.invisibility = 10
		wraith.set_loc(get_turf(owner))
		wraith.abilityHolder.resumeAllAbilities()
		wraith.abilityHolder.regenRate /= 3
		owner.bioHolder.RemoveEffect("revenant")
		owner:decomp_stage = 4
		if (ishuman(owner) && owner:organHolder && owner:organHolder:brain)
			qdel(owner:organHolder:brain)
		particleMaster.SpawnSystem(new /particleSystem/localSmoke("#000000", 5, locate(owner.x, owner.y, owner.z)))
		animate(owner)
		wraith = null
		return
	...
	proc/addRevenantVerbs()
		var/abilityHolder/revenant/RH = owner.add_ability_holder(/abilityHolder/revenant)
		RH.relay = wraith.abilityHolder
		RH.revenant = src
		wraith.abilityHolder.regenRate *= 3
		RH.addAbility(/targetable/revenantAbility/massCommand)
		RH.addAbility(/targetable/revenantAbility/shockwave)
		RH.addAbility(/targetable/revenantAbility/touchOfEvil)
		RH.addAbility(/targetable/revenantAbility/push)
		RH.addAbility(/targetable/revenantAbility/crush)
		RH.addAbility(/targetable/revenantAbility/help)
...
/targetable/revenantAbility
	icon = 'icons/mob/wraith_ui.dmi'
	preferred_holder_type = /abilityHolder/revenant
	New()
...
/targetable/revenantAbility/massCommand
	name = "Mass Command"
	...
	cast(atom/target)
		if (istype(holder, /abilityHolder/revenant))
			var/abilityHolder/revenant/RH = holder
			RH.channeling = 0
...
/targetable/revenantAbility/shockwave
	name = "Shockwave"
		...
		spawn (0)
			for (var/i = 1, i <= iteration_depth, i++)
				for (var/turf/T in next)
					...
					if (!T.density)
						var/base_dir = next[T]
						var/left_dir = prev["[base_dir]"]
						var/right_dir = next["[base_dir]"] // ugly & fuck you byond for making me do this
...
/targetable/revenantAbility/touchOfEvil
	name = "Touch of Evil"
	...
	cast()
		if (istype(holder, /abilityHolder/revenant))
			var/abilityHolder/revenant/RH = holder
			RH.channeling = 0
			var/bioEffect/hidden/revenant/R = RH.revenant
...
/targetable/revenantAbility/push
	name = "Push"
...
/targetable/revenantAbility/crush
	name = "Crush"
	...
	cast(atom/target)
		...
		var/abilityHolder/revenant/RH
		if (istype(holder, /abilityHolder/revenant))
			RH = holder
			RH.channeling = 1
		if (!RH || !istype(RH, /abilityHolder/revenant))
			return
...
/targetable/revenantAbility/help
	...
	cast(atom/target)
		...
		object.icon_state = "help[holder.help_mode]"
		holder.updateButtons()
		return FALSE
