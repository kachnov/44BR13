/targetable/vampire/enthrall
	...
	cast(mob/target)
		...
		actions.start(new/action/bar/private/icon/vampire_enthrall(target, src), M)
		if (istype(H)) H.blood_tracking_output(pointCost)
		return FALSE

/action/bar/private/icon/vampire_enthrall
	duration = 250
	...
	var/targetable/vampire/enthrall/enslave
	var/last_complete = 0
	...
	onStart()
		...
		var/abilityHolder/vampire/H = enslave.holder

		if (!enslave || get_dist(M, target) > enslave.max_range || target == null || M == null || target.stat == 2 || !target.mind || !target.client)
			interrupt(INTERRUPT_ALWAYS)
			return

		M.visible_message("<span style=\"color:red\"><strong>[M] bites [target]!</strong></span>")
	...
	onUpdate()
		...
		if (target.bioHolder && target.bioHolder.HasEffect("training_chaplain"))
			boutput(M, __red("Wait, this is a chaplain!!! <strong>AGDFHSKFGBLDFGLHSFDGHDFGH</strong>"))
		...
		if (complete >= 0.8 && last_complete < 0.8)
			...
			if (issilicon(target))
				...
				boutput(target, __red("<strong>MEM ERR BLK 0  ADDR 30FC500 HAS 010F NOT 0000</strong>"))
			else
	...
	onEnd()
		...
		boutput(target, __red("<strong>You awaken filled with purpose - you must serve your master, [M.real_name]!</strong>"))
		...
		if (issilicon(target))
			boutput(target, __red("<strong>You must serve your master. All previous laws are irrelevant.</strong>"))
