/targetable/vampire/hypnotize
	...
	cast(mob/target)
		...
		M.visible_message("<span style=\"color:red\"><strong>[M] stares into [target]'s eyes!</strong></span>")
		...
		if (do_mob(M, target, 20))
			if (target.bioHolder && target.bioHolder.HasEffect("training_chaplain"))
				...
				target.visible_message("<span style=\"color:red\"><strong>[target] just stares right back at [M]!</strong></span>")
