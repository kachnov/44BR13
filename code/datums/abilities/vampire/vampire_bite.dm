/targetable/vampire/vampire_bite
	name = "Bite victim"
	...
	proc/can_bite(var/mob/living/carbon/human/target)
		...
		if (check_target_immunity(target) == 1)
			target.visible_message("<span style=\"color:red\"><strong>[M] bites [target], but fails to even pierce their skin!</strong></span>")
			return FALSE
	...
	cast(mob/target)
		...
		target.visible_message("<span style=\"color:red\"><strong>[M] bites [target]!</strong></span>")
		...
		while (do_mob(M, HH, 30))
			if (HH.blood_volume > 0)
				...
				if (HH.stat == 2)
					...
					M.visible_message("<span style=\"color:red\"><strong>[M]</strong> begins to crisp and burn!</span>", "<span style=\"color:red\">You drank the blood of a holy man! It burns!</span>")
					M.emote("scream")
