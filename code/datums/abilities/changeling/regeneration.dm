		...
		if (!H.in_fakedeath)
			...
			spawn (cooldown)
				if (C && C.stat != 2)
					...
					C.visible_message(__red("<strong>[C] appears to wake from the dead, having healed all wounds.</span>"))
	...
	if (!C.burning && C.stat != 2 && (C.health < 100 || !C.limbs.l_arm || !C.limbs.r_arm || !C.limbs.l_leg || !C.limbs.r_leg))
		...
		if (!C.limbs.l_arm || !C.limbs.r_arm || !C.limbs.l_leg || !C.limbs.r_leg)
			if (!C.limbs.l_arm && prob(25))
				...
				C.visible_message("<span style=\"color:red\"><strong> [C]'s left arm grows back!</span>")
			...
			if (!C.limbs.r_arm && prob(25))
				...
				C.visible_message("<span style=\"color:red\"><strong> [C]'s right arm grows back!</span>")
				C.set_body_icon_dirty()
			...
			if (!C.limbs.l_leg && prob(25))
				...
				C.visible_message("<span style=\"color:red\"><strong> [C]'s left leg grows back!</span>")
				C.set_body_icon_dirty()
			...
			if (!C.limbs.r_leg && prob(25))
				...
				C.visible_message("<span style=\"color:red\"><strong> [C]'s right leg grows back!</span>")
				C.set_body_icon_dirty()

		if (prob(25)) C.visible_message("<span style=\"color:red\"><strong>[C]'s flesh is moving and sliding around oddly!</strong></span>")

/targetable/changeling/regeneration
