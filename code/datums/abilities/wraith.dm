...
/targetable/wraithAbility
	...
	targeted = 1
	target_anything = 1
	preferred_holder_type = /abilityHolder/wraith
...
/targetable/wraithAbility/absorbCorpse
	name = "Absorb Corpse"
	...
	cast(atom/T)
		...
		particleMaster.SpawnSystem(new /particleSystem/localSmoke("#000000", 5, locate(M.x, M.y, M.z)))
	...
	doCooldown()         //This makes it so wraith early game is much faster but hits a wall of high absorb cooldowns after ~5 corpses
		if (!holder)	 //so wraiths don't hit scientific notation rates of regen without playing perfectly for a million years
			return
		var/abilityHolder/wraith/W = holder
		if (istype(W))
			if (W.corpsecount == 0)
				cooldown = 450
				W.corpsecount += 1
...
/targetable/wraithAbility/possessObject
	name = "Possess Object"
...
/targetable/wraithAbility/makeRevenant
	name = "Raise Revenant"
	icon_state = "revenant"
...
/targetable/wraithAbility/decay
	name = "Decay"
	icon_state = "decay"
...
/targetable/wraithAbility/command
	name = "Command"
	icon_state = "command"
...
/targetable/wraithAbility/raiseSkeleton
	name = "Raise Skeleton"
	icon_state = "skeleton"
...
/targetable/wraithAbility/animateObject
	name = "Animate Object"
...
/targetable/wraithAbility/haunt
	name = "Haunt"
	icon_state = "haunt"
...
/targetable/wraithAbility/poltergeist
			...
			switch (effect)
				...
				if (3)
					...
					for (var/atom/A in affected)
						...
						spawn (150) qdel(D)
					particleMaster.SpawnSystem(new/particleSystem/areaSmoke("#ffffff", 150, trgloc))
					return FALSE
				if (4)
					...
					return FALSE
				if (5)
					var/sapped_amt = holder.regenRate * 100
...
/targetable/wraithAbility/whisper
	name = "Whisper"
	icon_state = "whisper"
	...
	cast(atom/target)
		...
		if (ishuman(target))
			if (target:stat == 2)
				...
				boutput(usr, "<strong>You whisper to [target]:</strong> [message]")
				boutput(target, "<strong>A netherworldly voice whispers into your ears... </strong> [message]")
		else
