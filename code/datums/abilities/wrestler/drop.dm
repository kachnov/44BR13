/targetable/wrestler/drop
	...
	cast(mob/target)
		...
		if (surface && (ST && isturf(ST)))
			M.set_loc(ST)
			M.visible_message("<span style=\"color:red\"><strong>[M] climbs onto [surface]!</strong></span>")
			M.pixel_y = 10
			falling = 1
			sleep (10)
		...
		if (M && target)
			...
			if ((falling == 0 && get_dist(M, target) > max_range) || (falling == 1 && get_dist(M, target) > (max_range + 1))) // We climbed onto stuff.
				M.pixel_y = 0
				if (falling == 1)
					M.visible_message("<span style=\"color:red\"><strong>...and dives head-first into the ground, ouch!</strong></span>")
			...
			M.visible_message("<span style=\"color:red\"><strong>[M] [pick_string("wrestling_belt.txt", "drop")] [target]!</strong></span>")
