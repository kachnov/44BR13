/targetable/predator/predator_taketrophy
	name = "Take trophy"
	...
	cast(mob/target)
		...
		actions.start(new/action/bar/private/icon/predator_taketrophy(target, src), M)
		return FALSE

/action/bar/private/icon/predator_taketrophy
	duration = 60
	...
	icon_state = "grabbed"
	var/target
	var/targetable/predator/predator_taketrophy/trophy
	...
	onStart()
		...
		var/abilityHolder/A = trophy.holder
		...
		M.visible_message("<span style=\"color:red\"><strong>[M] unsheaths [his_or_her(M)] claws and begins to cut into [target]!</strong></span>")
		A.locked = 1
	...
	onEnd()
		...
		M.visible_message("<span style=\"color:red\"><strong>[M] completely rips [target] apart!</strong></span>")
		...
		switch (no_of_skulls)
			if (0)
				boutput(M, __red("<strong>Their skull was missing. No trophy for you.</strong>"))
			if (1)
				if (tvalue <= 0)
					boutput(M, __red("<strong>This trophy is completely worthless!</strong>"))
				if (tvalue == 1)
					boutput(M, __blue("<strong>This trophy has a value of [tvalue].</strong>"))
				if (tvalue > 1)
					boutput(M, __blue("<strong>You have slain a powerful opponent!<br>This trophy has a value of [tvalue].</strong>"))
			else
				boutput(M, __blue("<strong>You found mulitple trophies. They have a combined value of [tvalue].</strong>"))
