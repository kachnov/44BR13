...
/mob/proc/make_predator()
	if (ishuman(src))
		var/abilityHolder/predator/A = get_ability_holder(/abilityHolder/predator)
		if (A && istype(A))
			return

		var/abilityHolder/predator/P = add_ability_holder(/abilityHolder/predator)
		P.addAbility(/targetable/predator/predator_gearspawn)
		P.addAbility(/targetable/predator/predator_taketrophy)
		P.addAbility(/targetable/predator/predator_trophycount)
...
/mob/living/carbon/human/proc/predator_transform()
	if (ishuman(src))
		...
		if (M.mutantrace)
			qdel(M.mutantrace)
		M.set_mutantrace(/mutantrace/predator)
...
/obj/screen/ability/predator
	clicked(params)
		var/targetable/predator/spell = owner
		if (!istype(spell))
			return
		if (!spell.holder)
			return
...
/targetable/predator
	...
	last_cast = 0
	pointCost = 0
	preferred_holder_type = /abilityHolder/predator
	...
	updateObject()
		..()
		if (!object)
			object = new /obj/screen/ability/predator()
			object.icon = icon
			object.owner = src
