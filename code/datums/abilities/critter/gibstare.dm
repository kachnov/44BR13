...
/action/bar/icon/gibstareAbility
	duration = 60
	...
	var/targetable/critter/gibstare/gibstare
	...
	onStart()
		...
		for (var/mob/O in AIviewers(owner))
			O.show_message("<span style=\"color:red\"><strong>[owner]</strong> stares at [target]!</span>", 1)
	...
	onEnd()
		...
		if (owner && ownerMob && target && (target in view(owner)) && gibstare && gibstare.cooldowncheck())
			...
			for (var/mob/O in AIviewers(ownerMob))
				O.show_message("<span style=\"color:red\"><strong>[target.name]'s</strong> head explodes!</span>", 1)
			if (target == owner)
				boutput(owner, "<span class='color:green'>Good. Job.</span>")
			target.gib()
			gibstare.actionFinishCooldown()

/targetable/critter/gibstare
	name = "Psychic Stare"
	...
	cast(atom/target)
		...
		if (isturf(target))
			...
			if (!target)
				...
				return TRUE
		actions.start(new/action/bar/icon/gibstareAbility(target, src), holder.owner)
		return FALSE