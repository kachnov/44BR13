...
var/global/shuttle_controller/emergency_shuttle/emergency_shuttle = null
...
/shuttle_controller
	...
	proc/incall()
		...
		ircbot.event("shuttlecall", timeleft())

	proc/recall()
		if (online && direction == 1)
			setdirection(-1)
			ircbot.event("shuttlerecall", timeleft())
	...
	emergency_shuttle
		...
		New()
			...
			for (var/obj/machinery/computer/airbr/S in machines)
				if (S.emergency && !(S in airbridges))
					airbridges += S
			...
			switch (location)
				if (SHUTTLE_LOC_CENTCOM)
					if (timeleft>SHUTTLEARRIVETIME)
						...
						if (airbridges.len)
							for (var/obj/machinery/computer/airbr/S in airbridges)
								S.establish_bridge()

						boutput(world, "<strong>The Emergency Shuttle has docked with the station! You have [timeleft()/60] minutes to board the Emergency Shuttle.</strong>")
				...
				if (SHUTTLE_LOC_STATION)
					if (!announcement_done && timeleft < 60)
						boutput(world, "<strong>The Emergency Shuttle will be entering the wormhole to CentCom in 1 minute! Please prepare for wormhole traversal.</strong>")
						announcement_done = 1
						...
						for (var/mob/M in start_location)
							shake_camera(M, 32, 4)
							M.addOverlayComposition(/overlayComposition/shuttle_warp)
						...
						if (particle_location)
							...
							if (S && !particleMaster.CheckSystemExists(/particleSystem/warp_star, S))
								particleMaster.SpawnSystem(new /particleSystem/warp_star(S, particle_location.star_dir))
						...
						spawn (0)
							...
							boutput(world, "<strong>The Emergency Shuttle has left for CentCom! It will arrive in [timeleft()/60] minute[s_es(timeleft()/60)]!</strong>")
							//online = 0
				...
				if (SHUTTLE_LOC_TRANSIT)
					if (timeleft>0)
						...
						for (var/mob/M in start_location)
							M.removeOverlayComposition(/overlayComposition/shuttle_warp)
						...
						start_location.move_contents_to(end_location)
						boutput(world, "<strong>The Emergency Shuttle has arrived at CentCom!")
						online = 0
