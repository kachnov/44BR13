...
/controller/gameticker/proc
	...
	generate_miscreant_objectives(var/mind/crewMind)
		set background = 1
		...
		var/list/objectiveTypes = subtypesof(/objective/miscreant)
		...
		while (assignCount && objectiveTypes.len)
			...
			var/objective/miscreant/newObjective = new selectedType
			...
			if (obj_count <= 1)
				boutput(crewMind.current, "<strong>You are a miscreant!</strong>")
