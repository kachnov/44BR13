...
/reagent
		...
		on_mob_life(var/mob/M)
			...
			switch(counter++)
				...
				if (6 to 10)
					...
					if (prob(8))
						boutput(M, "<span style=\"color:red\"><strong>You feel [pick("weak", "horribly weak", "numb", "like you can barely move", "tingly")].</strong></span>")
						M.stunned++
					else if (prob(8))
						M.emote(pick("drool","pale", "gasp"))
				if (11 to INFINITY)
					...
					if (prob(20) && !M.stat)
						...
						boutput(M, "<span style=\"color:red\"><strong>You can't [pick("breathe", "move", "feel your legs", "feel your face", "feel anything")]!</strong></span>")
						M.losebreath++
	...
	harmful/initropidril
		...
		on_mob_life(var/mob/living/M)
			...
			if (prob(10))
				...
				M.stunned += 3
				M.weakened += 2
				M.contract_disease(/ailment/disease/flatline, null, null, 1) // path, name, strain, bypass resist
			M.updatehealth()
			..(M)
	...
	harmful/initrobeedril_old
		...
		on_mob_life(var/mob/living/M)
			...
			switch(data)
				...
				if (9 to 30)
					if (prob(33))
						boutput(M, "<span style=\"color:red\"><em>You feel very weak.</em></span>")
						M.stunned += 2
					if (prob(10))
						boutput(M, "<span style=\"color:red\"><em>You have trouble breathing!</em></span>")
						M.take_oxygen_deprivation(2)
						M.losebreath++
				if (31 to 50)
					if (prob(33))
						boutput(M, "<span style=\"color:red\"><strong>You feel horribly weak.</strong></span>")
						M.stunned += 3
					if (prob(10))
						boutput(M, "<span style=\"color:red\"><strong>You cannot breathe!</strong></span>")
						M.take_oxygen_deprivation(2)
						M.losebreath++
					if (prob(10))
						boutput(M, "<span style=\"color:red\"><strong>Your heart flutters in your chest!</strong></span>")
			...
			if (ishuman(M))
				...
				if (H.organHolder && H.organHolder.heart) // you can't barf up a bee heart if you ain't got no heart to barf
					switch (data)
						...
						if (5 to 15)
							if (prob(33))
								boutput(H, "<span style=\"color:red\"><em>You feel very weak.</em></span>")
								H.stunned += 2
							if (prob(10))
								boutput(H, "<span style=\"color:red\"><em>You have trouble breathing!</em></span>")
								H.take_oxygen_deprivation(2)
								H.losebreath++
						if (16 to 25)
							if (prob(33))
								boutput(H, "<span style=\"color:red\"><strong>You feel horribly weak.</strong></span>")
								H.stunned += 3
							if (prob(10))
								boutput(H, "<span style=\"color:red\"><strong>You cannot breathe!</strong></span>")
								H.take_oxygen_deprivation(2)
								H.losebreath++
							if (prob(10))
								boutput(H, "<span style=\"color:red\"><strong>Your heart flutters in your chest!</strong></span>")
						...
						if (26 to INFINITY)
							...
							H.visible_message("<span style=\"color:red\"><strong>A bee bursts out of [H]'s chest! Oh fuck!</strong></span>", \
							"<span style=\"color:red\"><strong>A bee bursts out of your chest! OH FUCK!</strong></span>")
				...
				if (H.organHolder && H.organHolder.heart) // you can't barf up a bee heart if you ain't got no heart to barf
					switch(data)
						...
						if (26 to INFINITY)
							...
							H.visible_message("<span style=\"color:red\"><strong>A huge bee bursts out of [H]! OH FUCK!</strong></span>")
							H.gib()
	...
	harmful/cholesterol
		...
		on_mob_life(var/mob/living/M)
			...
			//if (prob(5)) // this is way too annoying and ruins fried foods
				...
				//M.stunned ++
			else if (holder.get_reagent_amount(id) >= 25 && prob(holder.get_reagent_amount(id)*0.15))
			...
			else if (holder.get_reagent_amount(id) >= 45 && prob(holder.get_reagent_amount(id)*0.08))
				boutput(M, "<span style=\"color:red\">Your chest [pick("hurts","stings","aches","burns")]!</span>")
				M.take_toxin_damage(rand(2,4))
				M.stunned ++
			else if (holder.get_reagent_amount(id) >= 150 && prob(holder.get_reagent_amount(id)*0.01))
				boutput(M, "<span style=\"color:red\">Your chest is burning with pain!</span>")
				//M.losebreath++ //heartfailure handles this just fine
				M.stunned++
				M.weakened++
				M.contract_disease(/ailment/disease/heartfailure, null, null, 1) // path, name, strain, bypass resist
			M.updatehealth()
			..(M)
	...
	harmful/itching
		...
		on_mob_life(var/mob/M) // commence the tickling
			...
			if (prob(20))
				boutput(M, "<span style=\"color:blue\"><strong>Something tickles!</strong></span>")
			...
			if (prob(2))
				boutput(M, "<span style=\"color:red\"><strong><font size='[rand(2,5)]'>AHHHHHH!</font></strong></span>")
				random_brute_damage(M,5)
				M.weakened +=5
				M.make_jittery(6)
				M.visible_message("<span style=\"color:red\"><strong>[M.name]</strong> falls to the floor, scratching themselves violently!</span>")
				M.emote("scream")
			..(M)
			return
			...
			switch(counter++)
				...
				if (11 to INFINITY)
					...
					if (prob(10))
						...
						M.losebreath++
					if (prob(9))
						boutput(M, "<span style=\"color:red\"><strong>You can't [pick("move", "feel your legs", "feel your face", "feel anything")]!</strong></span>")
					if (prob(7))
						boutput(M, "<span style=\"color:red\"><strong>You can't breathe!</strong></span>")
						M.losebreath+=3
			..(M)
			return
	...
	harmful/histamine
		...
		on_mob_life(var/mob/M) // allergies suck fyi
			...
			if (prob(10))
				boutput(M, "<span style=\"color:blue\"><strong>Your eyes itch.</strong></span>")
			...
			if (prob(10))
				...
				M.emote("grumble")
			if (prob(5))
				boutput(M, "<span style=\"color:red\"><strong>You're getting a rash!</strong></span>")
				random_brute_damage(M, 2)
			..(M)
			return
		...
		reaction_mob(var/mob/M, var/method=TOUCH, var/volume)
			if (method == TOUCH)
				...
				boutput(M, "<span style=\"color:red\"><strong>You feel a burning sensation in your throat...</span>")
				M.make_jittery(30)
				M.emote(pick("drool"))
			return
		...
		do_overdose(var/severity, var/mob/M)
			...
			if (severity == 1)
				if (effect <= 2)
					boutput(M, "<span style=\"color:red\"><strong>You feel mucus running down the back of your throat.</strong></span>")
					M.take_toxin_damage(1)
					M.updatehealth()
					M.make_jittery(4)
					...
					boutput(M, "<span style=\"color:red\"><strong>Your chest hurts!</strong></span>")
					M.emote(pick("cough","gasp"))
					M.take_oxygen_deprivation(3)
					M.updatehealth()
			else if (severity == 2)
				if (effect <= 2)
					M.visible_message("<span style=\"color:red\"><strong>[M.name]<strong> breaks out in hives!</span>")
					random_brute_damage(M, 6)
				else if (effect <= 4)
					M.visible_message("<span style=\"color:red\"><strong>[M.name]</strong> has a horrible coughing fit!</span>")
					M.make_jittery(10)
					M.stuttering += rand(0,5)
					M.emote("cough")
					if (prob(40))
					...
					boutput(M, "<span style=\"color:red\"><strong>Your heartbeat is pounding inside your head!</strong></span>")
					...
					M.weakened += 3
					M.emote(pick("choke", "gasp"))
					boutput(M, "<span style=\"color:red\"><strong>You feel like you're dying!</strong></span>")
			...
			switch(counter++)
				...
				if (30 to 60)
					...
					if (prob(5))
						...
						M.visible_message("<span style=\"color:red\"><strong>[M] has a seizure!</strong></span>")
						M.make_jittery(1000)
					if (prob(5))
						boutput(M, "<span style=\"color:red\"><strong>You can't breathe!</strong></span>")
...
*/
		...
		on_mob_life(var/mob/M)
			...
			var/bioHolder/data_as_holder = null
			if (!data) //Do we have some BLOOODDD to use?
				var/reagent/blood/cheating = holder.reagent_list["blood"]
				if (cheating && istype(cheating.data, /bioHolder))
					data = cheating.data
					data_as_holder = data



			if (data && M.bioHolder && progress_timer <= 10)
	...
	harmful/madness_toxin
			...
			switch(data++)
				...
				if (18 to 25) //Oh god!
					if (prob(33))
						H.drowsyness = max(H.drowsyness,7)
						H.make_jittery(300)
						H.show_text("<strong>[pick_string("chemistry_reagent_messages.txt", "madness2")]</strong>", "red")
				...
				if (26) //Whew
					H.show_text("<strong>Your mind feels clearer.<strong>", "blue")
					H.drowsyness = 0
				if (29) //Oh no
					H.show_text("<font size=+2><strong>IT HURTS!!</strong></font>","red")
					H.emote("scream")
					H.drowsyness = max(H.drowsyness,30)
					ai_was_active = H.ai_active
					H.ai_init() //:getin:
					H.ai_aggressive = 1 //Fak
					logTheThing("combat", H, null, "has their AI enabled by [id]")
				...
				if (30 to 100) //FIGHTIN'
					if (prob(33))
						...
						H.make_jittery(600)
						H.show_text("<strong>[pick_string("chemistry_reagent_messages.txt", "madness3")]</strong>", "red")
					...
					if (data > 50) //Oh dear
						if (data == 51)
							H.add_stam_mod_regen(id, 100) //Buff
		...
		on_remove()
			if (holder && holder.my_atom && istype(holder.my_atom, /mob/living/carbon/human))
				...
				if (data >= 30)
					H.remove_stam_mod_regen(id) //Not so buff
					if (!ai_was_active)
						H.ai_stop()
					H.ai_aggressive = initial(H.ai_aggressive)
					H.ai_suicidal = 0
					logTheThing("combat", H, null, "has their AI disabled by [id]")
	...
	harmful/strychnine
		...
		on_mob_life(var/mob/M)
			...
			switch(data++)
				...
				if (25)
					H.remove_stam_mod_max(id)
					H.add_stam_mod_max(id, -50)
					H.add_stam_mod_regen(id, -2)
				if (26 to 35)
