...
/reagent
	...
	proc/reaction_mob(var/mob/M, var/method=TOUCH, var/volume) //By default we have a chance to transfer some
		var/reagent/self = src					  //of the reagent to the mob on TOUCHING it.
		switch(method)
			if (TOUCH)
				if (penetrates_skin)
			...
			if (INGEST)
				var/ailment_data/addiction/AD = M.addicted_to_reagent(src)
				...
				if (prob(addProb) && ishuman(M) && !AD)
					...
					boutput(M, "<span style=\"color:red\"><strong>You suddenly feel invigorated and guilty...</strong></span>")
					AD = new
					AD.associated_reagent = name
					AD.last_reagent_dose = world.timeofday
					AD.name = "[name] addiction"
					AD.affected_mob = M
					M.ailments += AD
				else if (AD)
					boutput(M, "<span style=\"color:blue\"><strong>You feel slightly better, but for how long?</strong></span>")
					AD.last_reagent_dose = world.timeofday
					AD.stage = 1
	...
	proc/on_mob_life(var/mob/M)
		...
		if (istype(M, /mob/living/carbon/human))
			...
			if (H.traitHolder.hasTrait("slowmetabolism"))
				deplRate /= 2
		holder.remove_reagent(id, deplRate) //By default it slowly disappears.

		if (M && overdose > 0) check_overdose(M)
		//if (M && M.stat == 2 && id != "montaguone" && id != "montaguone_extra") M.reagents.del_reagent(id) // no more puking corpses and such
		return
	...
	proc/check_overdose(var/mob/M)
		...
		if (!holder)
			holder = M.reagents
		var/amount = holder.get_reagent_amount(id)
		if (istype(M, /mob/living/carbon/human))
			var/mob/living/carbon/human/H = M
			if (H.traitHolder.hasTrait("chemresist"))
				amount *= 0.65
		if (amount >= overdose * 2)
			return do_overdose(2, M)
		else if (amount >= overdose)
	...
	proc/on_transfer(var/reagents/source, var/reagents/target, var/trans_amt)
		// NOTE: When this proc is invoked, the volume of the reagent will equal the total volume of this reagent.
		// Thus:
		// - the amount of this reagent in source before transfer = volume
		// - the amount of this reagent in target after transfer = trans_amt
		// - the amount of this reagent in source after transfer = volume - trans_amt
		return
