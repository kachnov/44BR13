/proc/ldmatter_reaction(var/reagents/holder, var/created_volume, var/id)
	var/cube = 0
...
/proc/smoke_reaction(var/reagents/holder, var/smoke_size, var/turf/location, var/vox_smoke = 0)
	if (narrator_mode || vox_smoke)
		playsound(location, 'sound/vox/smoke.ogg', 50, 1, -3)
	else
		playsound(location, 'sound/effects/smoke.ogg', 50, 1, -3)
	particleMaster.SpawnSystem(new /particleSystem/chemSmoke(location, holder, 100, smoke_size))
