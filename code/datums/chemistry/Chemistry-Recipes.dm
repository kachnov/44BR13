...
/chemical_reaction
	...
	proc/on_reaction(var/reagents/holder, var/created_volume)
		return
	...
	no_pyrosium_foam
		...
		on_reaction(var/reagents/holder, var/created_volume)
			if (holder)
...
/*
	...
	cheese
		...
		on_reaction(var/reagents/holder)
...
/*		lexorin
	...
	acid
		...
		on_reaction(var/reagents/holder, created_volume)
			var/location = get_turf(holder.my_atom)
		...
		on_reaction(var/reagents/holder, var/created_volume)
			...
			if ( (locate(/obj/alchemy/circle) in range(3,reaction_loc)) )
				...
				if (gathered >= 80)
					...
					if (!particleMaster.CheckSystemExists(/particleSystem/swoosh, my_atom))
						particleMaster.SpawnSystem(new /particleSystem/swoosh(my_atom))
				else
