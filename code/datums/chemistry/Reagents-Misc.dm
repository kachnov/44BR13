...
/reagent
	nitroglycerin // Yes, this is a bad idea.
		...
		on_transfer(var/reagents/source, var/reagents/target, var/trans_volume)
			var/reagent/nitroglycerin/target_ng = target.get_reagent("nitroglycerin")
	...
	anima //This stuff is not done. Don't use it. Don't spoil it.
		...
		on_add()
			...
			if (hascall(holder.my_atom,"addOverlayComposition"))
				holder.my_atom:addOverlayComposition(/overlayComposition/anima)

			if (ismob(A))
				if (!particleMaster.CheckSystemExists(/particleSystem/swoosh, A))
					particleMaster.SpawnSystem(new /particleSystem/swoosh/endless(A))
			return
		...
		on_remove()
			...
			if (hascall(holder.my_atom,"removeOverlayComposition"))
				holder.my_atom:removeOverlayComposition(/overlayComposition/anima)

			if (ismob(A))
				particleMaster.RemoveSystem(/particleSystem/swoosh, A)
			...
			if ((method == INGEST || (method == TOUCH && prob(25))) && (M.stat == 2))
				...
				if (ishuman(M))
					...
					if (H.ghost && H.ghost.mind && !(H.mind && H.mind.dnr)) // if they have dnr set don't bother shoving them back in their body
						H.ghost.show_text("<span style=\"color:red\"><strong>You feel yourself being dragged out of the afterlife!</strong></span>")
						H.ghost.mind.transfer_to(H)
						qdel(H.ghost)
					if (came_back_wrong || H.decomp_stage != 0 || (H.mind && H.mind.dnr)) //Wire: added the dnr condition here
						H.visible_message("<span style=\"color:red\"><strong>[H]</strong> starts convulsing violently!</span>")
						if (H.mind && H.mind.dnr)
							H.visible_message("<span style=\"color:red\"><strong>[H]</strong> seems to prefer the afterlife!</span>")
						H.make_jittery(1000)
						spawn (rand(20, 100))
							H.gib()
					else
		...
		reaction_turf(var/turf/target, var/volume)
			...
			if (hotspot)
				if (istype(target, /turf/simulated))
					...
					if (T.air)
						var/gas_mixture/lowertemp = T.remove_air( T.air.total_moles() )
...
// two different foaming agents is overly complicated imo - IM
	...
	ectoplasm
		...
		on_mob_life(var/mob/M)
			...
			if ((holder.get_reagent_amount(id) >= 10) && prob(8))
		...
		reaction_mob(var/mob/living/M, var/method=TOUCH, var/volume)
			if (method == INGEST)
				...
				if (ranchance == 1)
					...
					M.contract_disease(/ailment/disease/food_poisoning, null, null, 1) // path, name, strain, bypass resist
	...
	oil
		...
		reaction_temperature(exposed_temperature, exposed_volume)
			if (exposed_temperature > T0C + 600 && !reacting) // need this to be higher to make propylene possible
				reacting = 1
				var/location = get_turf(holder.my_atom)
				holder.my_atom.visible_message("<strong>The oil burns!</strong>")
				...
				smoke.start()
				if (holder)
					holder.add_reagent("ash", round(volume/2), null)
		...
		on_mob_life(var/mob/M)
			...
			switch(counter++)
				...
				if (11)
					...
					M.visible_message("<strong>[M]</strong> seizes up and falls limp, \his eyes dead and lifeless...")
					M.resting = 1
...
*/
	...
	werewolf_serum_fake4
		...
		on_mob_life(var/mob/M)
			...
			var/our_amt = holder.get_reagent_amount(id)
			if (prob(25))
			...
			if (our_amt < 5)
				...
				M.visible_message("<span style=\"color:red\"><strong>[M]</strong> starts convulsing violently!</span>", "You feel as if your body is tearing itself apart!")
	...
	ants
		...
		reaction_mob(var/mob/M, var/method=TOUCH, var/volume)
			src = null
			if (ishuman(M))
				boutput(M, "<span style=\"color:red\"><strong>OH SHIT ANTS!!!!</strong></span>")
				M.emote("scream")
	...
	spiders
		...
		reaction_mob(var/mob/M, var/method=TOUCH, var/volume)
			src = null
			if (method == TOUCH)
				boutput(M, "<span style=\"color:red\"><strong>OH [pick("SHIT", "FUCK", "GOD")] SPIDERS[pick("", " ON MY FACE", " EVERYWHERE")]![pick("", "!", "!!", "!!!", "!!!!")]</strong></span>")
			if (method == INGEST)
				boutput(M, "<span style=\"color:red\"><strong>OH [pick("SHIT", "FUCK", "GOD")] SPIDERS[pick("", " IN MY BLOOD", " IN MY VEINS")]![pick("", "!", "!!", "!!!", "!!!!")]</strong></span>")
			M.emote("scream")
			random_brute_damage(M, 2)
			if (ishuman(M))
				if (!M:spiders)
		...
		on_mob_life(var/mob/M)
			...
			if (prob(50))
				...
				M.visible_message("<span style=\"color:red\"><strong>[M]</strong> [pick("scratches", "digs", "picks")] at [pick("something under their skin", "their skin")]!</span>",\
				"<span style=\"color:red\"><strong>[pick("T", "It feels like t", "You feel like t", "Oh shit t", "Oh fuck t", "Oh god t")]here's something [pick("crawling", "wriggling", "scuttling", "skittering")] in your [pick("blood", "veins", "stomach")]!</strong></span>")
			else if (prob(10))
				random_brute_damage(M, 5)
				M.emote("scream")
				M.emote("twitch")
				if (!M.weakened) M.weakened += 1
				M.visible_message("<span style=\"color:red\"><strong>[M.name]</strong> tears at their own skin!</span>",\
				"<span style=\"color:red\"><strong>OH [pick("SHIT", "FUCK", "GOD")] GET THEM OUT![pick("", "!", "!!", "!!!", "!!!!")]</span>")
			else if (prob(10))
				if (!locate(/obj/decal/cleanable/vomit) in T)
					playsound(T, "sound/effects/splat.ogg", 50, 1)
					new /obj/decal/cleanable/vomit/spiders(T)
					random_brute_damage(M, rand(4))
					M.visible_message("<span style=\"color:red\"><strong>[M]</strong> [pick("barfs", "hurls", "pukes", "vomits")] up some [pick("spiders", "weird black stuff", "strange black goop", "wriggling black goo")]![pick("", " Gross!", " Ew!", " Nasty!")]</span>",\
					"<span style=\"color:red\"><strong>OH [pick("SHIT", "FUCK", "GOD")] YOU JUST [pick("BARFED", "HURLED", "PUKED", "VOMITED")] SPIDERS[pick("!", " FUCK THAT'S GROSS!", " SHIT THAT'S NASTY!", " OH GOD EW!")][pick("", "!", "!!", "!!!", "!!!!")]</strong></span>")
					if (prob(33))
	...
	fliptonium
		...
		reaction_mob(var/mob/M)
			...
			DEBUG_MESSAGE("<span style=\"color:blue\"><strong>Spun [M]: [dir_temp], [speed_temp]</strong></span>") // <- What's this?
...
/*			reaction_obj(var/obj/O, var/volume)
			if (volume >= 10)
				...
				DEBUG_MESSAGE("<span style=\"color:blue\"><strong>Spun [O]: [dir_temp], [speed_temp]</strong></span>")
*/
		on_add()
			if (istype(holder) && istype(holder.my_atom) && hascall(holder.my_atom,"add_stam_mod_regen"))
		...
		do_overdose(var/severity, var/mob/M)
			...
			if (severity == 1)
				if (effect <= 2)
					M.visible_message("<span style=\"color:red\"><strong>[M.name]</strong> can't seem to control their legs!</span>")
					M.change_misstep_chance(33)
					M.weakened += 2
				else if (effect <= 4)
					M.visible_message("<span style=\"color:red\"><strong>[M.name]'s</strong> hands flip out and flail everywhere!</span>")
					M.drop_item()
					M.hand = !M.hand
					M.drop_item()
					M.hand = !M.hand
				...
				if (effect <= 2)
					...
					M.visible_message("<span style=\"color:red\"><strong>[M.name]</strong> falls to the floor and flails uncontrollably!</span>")
					M.make_jittery(5)
					M.weakened += 5
				else if (effect <= 7)
					M.emote("laugh")
		...
		on_mob_life(var/mob/M)
			...
			if (prob(10))
				...
				if (nerds && nerds.len)
					...
					if (prob(50))
						M.visible_message("<strong>[M]</strong> flips off [some_idiot.name]!")
					else
						M.visible_message("<strong>[M]</strong> gives [some_idiot.name] the double deuce!")
			..(M)
			return
	...
	fartonium // :effort:
		...
		on_mob_life(var/mob/M)
			...
			if (M.reagents.has_reagent("anti_fart"))
				if (prob(25))
					boutput(M, "<span style=\"color:red\">[pick("Oh god, something doesn't feel right!", "<strong>IT HURTS!</strong>", "<strong>FUCK!</strong>", "Something is seriously wrong!", "<strong>THE PAIN!</strong>", "You feel like you're gunna die!")]</span>")
					random_brute_damage(M, 1)
				if (prob(10))
					M.emote("poo")
					random_brute_damage(M, 2)
...
// let us never forget the 3,267 parrot incident, the recipe for this just reacts instantly now
	...
	glitter
		...
		on_add()
			var/atom/A = holder.my_atom
			if (ismob(A))
				if (!particleMaster.CheckSystemExists(/particleSystem/glitter, A))
					particleMaster.SpawnSystem(new /particleSystem/glitter(A))
			if (isobj(A))
				if (!particleMaster.CheckSystemExists(/particleSystem/glitter, A))
					particleMaster.SpawnSystem(new /particleSystem/glitter(A))
		...
		on_remove()
			...
			if (ismob(A))
				if (particleMaster.CheckSystemExists(/particleSystem/glitter, A))
					particleMaster.RemoveSystem(/particleSystem/glitter, A)
			if (isobj(A))
				if (particleMaster.CheckSystemExists(/particleSystem/glitter, A))
					particleMaster.RemoveSystem(/particleSystem/glitter, A)
...
/* STOP FUCKING MURDERING THE SERVER YOU SHITASS
		reaction_turf(var/turf/T)
			if (isturf(T) && !istype(T, /turf/space))
				if (!particleMaster.CheckSystemExists(/particleSystem/glitter, T))
					particleMaster.SpawnSystem(new /particleSystem/glitter(T))

		reaction_obj(var/obj/O)
			if (isobj(O))
				if (!particleMaster.CheckSystemExists(/particleSystem/glitter, O))
					particleMaster.SpawnSystem(new /particleSystem/glitter(O))
*/
		on_mob_life(var/mob/M)
			if (!M) M = holder.my_atom
			if (prob(10))
				M.visible_message("<span style=\"color:red\"><strong>[M.name]</strong> scratches at an itch.</span>")
				random_brute_damage(M, 1)
				M.emote("grumble")
			if (prob(5))
				boutput(M, "<span style=\"color:red\"><strong>So itchy!</strong></span>")
				random_brute_damage(M, 2)
			if (prob(1))
...
*/
	...
	voltagen
		...
		reaction_mob(var/mob/M, var/method = TOUCH)
			if (method == TOUCH && volume >= 5)
				M.shock(holder.my_atom, min(7500 * multiplier, volume * 100 * multiplier), "chest", 1, 1)
	...
	/*lumen
		...
		proc/calculate_glow_color()
			...
			return new /color(fluid_r, fluid_g, fluid_b, transparency)

		reaction_turf(var/turf/T)
			if (locate(/obj/decal/glow) in T)
				return
			var/obj/decal/glow/G = new(T)
			var/color/mycolor = calculate_glow_color()
...
/*			var
		...
		reaction_mob(var/mob/M, var/method=TOUCH, var/volume_passed)
			...
			if (method == INGEST)
				if (M.mind)
					if (isvampire(M))
						var/abilityHolder/vampire/V = M.get_ability_holder(/abilityHolder/vampire)
						if (V && istype(V))
							// Blood as a reagent doesn't track DNA and blood type yet (or anymore).
							/*if (M.bioHolder && (blood_DNA == M.bioHolder.Uid))
...
*/
		...
		on_mob_life(var/mob/M)
			...
			if (holder.has_reagent("dna_mutagen")) //If there's stable mutagen in the mix and it doesn't have data we gotta give it a chance to get some
				var/reagent/SM = holder.get_reagent("dna_mutagen")
				if (SM.data) holder.del_reagent(id)
			else
				holder.del_reagent(id)
			return
		...
		on_transfer(var/reagents/source, var/reagents/target, var/trans_amt)
			...
			if (target_changed)
				for (var/reagent_id in pathogen_controller.pathogen_affected_reagents)
					if (target.has_reagent(reagent_id))
						var/reagent/blood/B = target.get_reagent(reagent_id)
						if (!istype(B))
							continue
	...
	urine
		...
		/*on_mob_life(var/mob/M) why
			for (var/ailment_data/disease/virus in M.ailments)
				if (prob(10))
