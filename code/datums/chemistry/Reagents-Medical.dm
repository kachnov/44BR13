...
/reagent
		...
		do_overdose(var/severity, var/mob/M)
			...
			if (severity == 1)
				...
				if ( effect <= 2)
					...
					M.visible_message("<span style=\"color:red\"><strong>[M.name]</strong> staggers and drools, their eyes bloodshot!</span>")
					M.dizziness += 8
					M.weakened += 4
				if (effect <= 15)
					M.take_toxin_damage(1)
	...
	medical/omnizine // COGWERKS CHEM REVISION PROJECT. magic drug, ought to use plasma or something
		...
		do_overdose(var/severity, var/mob/M)
			...
			if (severity == 1) //lesser
				M.stuttering += 1
				if (effect <= 1)
					M.visible_message("<span style=\"color:red\"><strong>[M.name]</strong> suddenly cluches their gut!</span>")
					M.emote("scream")
					M.stunned += 4
					M.weakened += 4
				else if (effect <= 3)
					M.visible_message("<span style=\"color:red\"><strong>[M.name]</strong> completely spaces out for a moment.</span>")
					M.change_misstep_chance(15)
				else if (effect <= 5)
					M.visible_message("<span style=\"color:red\"><strong>[M.name]</strong> stumbles and staggers.</span>")
					M.dizziness += 5
					M.weakened += 3
				else if (effect <= 7)
				...
				if (effect <= 2)
					...
					M.visible_message("<span style=\"color:red\"><strong>[M.name]</strong> jerks bolt upright, then collapses!</span>")
					M.paralysis += 5
					M.weakened += 4
				else if (effect <= 8)
					M.visible_message("<span style=\"color:red\"><strong>[M.name]</strong> stumbles and staggers.</span>")
	...
	medical/haloperidol // COGWERKS CHEM REVISION PROJECT. ought to be some sort of shitty illegal opiate or hypnotic drug
		...
		on_mob_life(var/mob/living/M)
			...
			if (prob(5))
				for (var/ailment_data/disease/virus in M.ailments)
					if (istype(virus.master,/ailment/disease/space_madness) || istype(virus.master,/ailment/disease/berserker))
	...
	medical/mutadone // COGWERKS CHEM REVISION PROJECT. - marked for revision. Magic bullshit chem, ought to be related to mutagen somehow
		...
		on_mob_life(var/mob/M)
			...
			if (M.bioHolder && M.bioHolder.effects && M.bioHolder.effects.len) //One per cycle. We're having superpowered hellbastards and this is their kryptonite.
				var/bioEffect/B = M.bioHolder.effects[pick(M.bioHolder.effects)]
	...
	medical/antihistamine
		...
		on_mob_life(var/mob/M)
			...
			if (prob(3))
				M.stunned += 2
				M.drowsyness++
				M.visible_message("<span style=\"color:blue\"><strong>[M.name]<strong> looks a bit dazed.</span>")
			..(M)
			return
	...
	medical/perfluorodecalin
		...
		on_mob_life(var/mob/M)
			...
			if (volume >= 4) // stop killing dudes goddamn
