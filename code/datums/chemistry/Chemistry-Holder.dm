...
var/list/reagents/active_reagent_holders = list()
...
/reagents
	...
	var/list/chemical_reaction/possible_reactions = list()
	var/list/chemical_reaction/active_reactions = list()
	...
	disposing()
		if (reagent_list)
			for (var/reagent_id in reagent_list)
				var/reagent/current_reagent = reagent_list[reagent_id]
				if (current_reagent)
	...
	proc/copy_to(var/reagents/target, var/multiplier = 1, var/do_not_react = 0)
	...
	proc/temperature_react() //Calls the temperature reaction procs without changing the temp.
		for (var/reagent_id in reagent_list)
			var/reagent/current_reagent = reagent_list[reagent_id]
			if (current_reagent)
				current_reagent.reaction_temperature(total_temperature, 100)
	...
	proc/remove_any(var/amount=1)
		...
		for (var/reagent_id in reagent_list)
			...
			if (current_reagent)
				...
				remove_reagent(reagent_id, transfer_amt)
	...
	proc/get_master_reagent_name()
		...
		for (var/reagent_id in reagent_list)
			...
			var/reagent/current = reagent_list[reagent_id]
	...
	proc/trans_to(var/obj/target, var/amount=1, var/multiplier=1)
		...
		var/reagents/target_reagents = target.reagents

		return trans_to_direct(target_reagents, amount, multiplier)

	proc/trans_to_direct(var/reagents/target_reagents, var/amount=1, var/multiplier=1)
		...
		for (var/reagent_id in reagent_list)
			...
			//if (istype(current_reagent, /reagent/disease))
			...
			//else
			target_reagents.add_reagent(reagent_id, receive_amt, current_reagent.data, total_temperature)
	...
	proc/aggregate_pathogens()
		...
		for (var/reagent_id in pathogen_controller.pathogen_affected_reagents)
			if (has_reagent(reagent_id))
				var/reagent/blood/B = get_reagent(reagent_id)
				if (!istype(B))
					continue
	...
	proc/metabolize(var/mob/target)
		for (var/current_id in reagent_list)
			var/reagent/current_reagent = reagent_list[current_id]
			if (current_reagent)
	...
	proc/handle_reactions()
		if (inert) return //We magically prevent all reactions inside ourselves.
		...
		reaction_loop:
			for (var/chemical_reaction/C in possible_reactions)
				...
				var/created_volume = maximum_volume
				...
				if (total_matching_reagents == C.required_reagents.len)
					...
					if (C.instant)
						if (C.consume_all)
							...
							for (var/B in C.required_reagents)
								remove_reagent(B, C.required_reagents[B] * created_volume / (C.result_amount ? C.result_amount : 1))
	...
	proc/process_reactions()
		...
		for (var/chemical_reaction/C in active_reactions)
			...
			for (var/reagent in C.required_reagents)
				remove_reagent(reagent, C.required_reagents[reagent] * speed / C.result_amount)
			if (C.result)
				add_reagent(C.result, speed,, total_temperature)
	...
	proc/del_reagent(var/reagent)
		var/reagent/current_reagent = reagent_list[reagent]
		...
		handle_reactions() // trigger inhibited reactions
	...
	proc/reaction(var/atom/A, var/method=TOUCH, var/react_volume)
		...
		if (isobserver(A)) // errrr
			return
		if (!react_volume)
			react_volume = total_volume
		var/volume_fraction = react_volume / total_volume
	...
	proc/add_reagent(var/reagent, var/amount, var/sdata, var/temp_new=T20C, var/donotreact = 0)
		...
		last_temp = total_temperature
		total_temperature = (total_temperature * total_volume + temp_new*new_amount) / (total_volume + new_amount)
		update_total()
	...
	proc/get_average_color()
		var/color/average = new(0,0,0,0)
		var/total_weight = 0

		for (var/id in reagent_list)

			var/reagent/current_reagent = reagent_list[id]
	...
	proc/get_average_rgb()
		var/color/average = get_average_color()
