...
/proc/build_chem_structure()
	...
	for (var/R in (subtypesof(/chemical_reaction)))
		var/chemical_reaction/CR = new R
	...
	logTheThing("debug", null, null, "<strong>SpyGuy/chem_struct</strong> Finished building reaction structure. Took [(world.timeofday - startTime)/10] seconds.")

/proc/build_reagent_cache()
	var/startTime = world.timeofday
	reagents_cache.Cut()
	for (var/R in (subtypesof(/reagent)))
		var/reagent/Rinstance = new R()
		//If R is not a reagent then I don't think anything I can do will help here.
		reagents_cache[Rinstance.id] = Rinstance

	logTheThing("debug", null, null, "<strong>SpyGuy/reagents_cache</strong> Finished building reagents cache. Took [(world.timeofday - startTime)/10] seconds.")
