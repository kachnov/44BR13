		...
		reaction_temperature(exposed_temperature, exposed_volume) //ugh fuck this proc seriously
			if (exposed_temperature > T0C + 100 && !ignited)
				...
				if (our_user && ismob(our_user))
					logTheThing("combat", our_user, null, "Heat-triggered [name] chemical reaction [log_reagents(holder.my_atom)] at [T ? "[log_loc(T)]" : "null"].")
				else
					logTheThing("combat", our_user, null, "Heat-triggered [name] chemical reaction [log_reagents(holder.my_atom)] at [T ? "[log_loc(T)]" : "null"].[our_fingerprints ? " Container last touched by: [our_fingerprints]." : ""]")
				...
				for (var/mob/living/M in all_hearers(world.view, location))
					...
					if (no_fluff == 0)
						if (!M.ears_protected_from_sound())
							boutput(M, "<span style=\"color:red\"><strong>[hootmode ? "HOOT" : "BANG"]</strong></span>")
						else
							continue
				...
				for (var/mob/living/silicon/S in all_hearers(world.view, location))
					if (no_fluff == 0)
						if (!S.ears_protected_from_sound())
							boutput(S, "<span style=\"color:red\"><strong>[hootmode ? "HOOT" : "BANG"]</strong></span>")
						else
							continue
		...
		reaction_temperature(exposed_temperature, exposed_volume)
			if (exposed_temperature > combustion_temp)
				...
				if (holder && volume >= explosion_threshold)
					if (holder.my_atom)
						holder.my_atom.visible_message("<span style=\"color:red\"><strong>[holder.my_atom] explodes!</strong></span>")
		...
		reaction_temperature(exposed_temperature, exposed_volume)
			...
			if (exposed_temperature > T0C + 200)
				...
				spawn (rand(5,15))
					...
					switch(our_amt)
						if (0 to 20)
							holder.my_atom.visible_message("<strong>The black powder ignites!</strong>")
							...
							holder.del_reagent(id)
						if (21 to 80)
							holder.my_atom.visible_message("<strong>[holder.my_atom] flares up!</strong>")
						...
						if (81 to 160)
							...
							holder.del_reagent(id)
						if (161 to 300)
							holder.my_atom.visible_message("<span style=\"color:red\"><strong>[holder.my_atom] violently explodes!</strong></span>")
							explosion(holder.my_atom, location, 1, 3, 6, 8)
							holder.del_reagent(id)
						if (301 to INFINITY)
							holder.my_atom.visible_message("<span style=\"color:red\"><strong>[holder.my_atom] detonates in a huge blast!</strong></span>")
	...
	combustible/nitrogentriiodide
		...
		proc/bang()
			if (holder && holder.my_atom)
				holder.my_atom.visible_message("<strong>The powder detonates!</strong>")
