...
/reagent
	...
	drug/bathsalts
		...
		on_mob_life(var/mob/M) // commence bad times
			...
			if (istype(H))
				if (check < 8 && H.cust_two_state != "tramp") // M.is_hobo = very yes
					...
					boutput(M, "<span style=\"color:red\"><strong>You feel gruff!</strong></span>")
					spawn (3)
						M.visible_message("<span style=\"color:red\"><strong>[M.name]</strong> has a wild look in their eyes!</span>")
				if (check < 60)
			...
			if (check < 8)
				...
				M.visible_message("<span style=\"color:red\"><strong>[M.name]</strong> scratches at something under their skin!</span>")
				...
				switch(rand(1,2))
					if (1)
						if (prob(20))
							...
							boutput(M, "<span style=\"color:red\"><strong>OH GOD LOOK OUT!!!</strong>!</span>")
							M.emote("scream")
							M.playsound_local(M.loc, 'sound/effects/bell.ogg', 50, 1)
						else if (prob(50))
							fake_attackEx(M, 'icons/misc/critter.dmi', "mimicface", "smiling thing")
							boutput(M, "<span style=\"color:red\"><strong>The smiling thing</strong> laughs!</span>")
							...
							boutput(M, "<span style=\"color:red\"><strong>You hear something strange behind you...</strong></span>")
							var/ants = rand(1,3)
				...
				boutput(M, "<span style=\"color:red\"><strong>They're coming for you!</strong></span>")
			else if (check < 28)
				boutput(M, "<span style=\"color:red\"><strong>THEY'RE GONNA GET YOU!</strong></span>")
			..(M)
			return
		...
		do_overdose(var/severity, var/mob/M)
			...
			if (severity == 1)
				if (effect <= 2)
					M.visible_message("<span style=\"color:red\"><strong>[M.name]</strong> flails around like a lunatic!</span>")
					M.change_misstep_chance(25)
					M.make_jittery(10)
					M.emote("scream")
					M.reagents.add_reagent("salts1", 5)
				else if (effect <= 4)
					M.visible_message("<span style=\"color:red\"><strong>[M.name]'s</strong> eyes dilate!</span>")
					M.emote("twitch_s")
				...
				if (effect <= 2)
					...
					M.visible_message("<span style=\"color:red\"><strong>[M.name]</strong> convulses violently and falls to the floor!</span>")
					M.make_jittery(50)
					M.take_toxin_damage(2)
					M.take_brain_damage(1)
					M.updatehealth()
	...
	drug/crank
		...
		on_mob_life(var/mob/M)
			...
			if (prob(8))
				boutput(M, "<span style=\"color:blue\"><strong>You feel great!</strong></span>")
				M.reagents.add_reagent("methamphetamine", rand(1,2))
				M.emote(pick("laugh", "giggle"))
			if (prob(6))
				boutput(M, "<span style=\"color:blue\"><strong>You feel warm.</strong></span>")
		...
		do_overdose(var/severity, var/mob/M)
			...
			if (severity == 1)
				if (effect <= 2)
					M.visible_message("<span style=\"color:red\"><strong>[M.name]</strong> looks confused!</span>")
					M.change_misstep_chance(20)
					M.make_jittery(20)
					M.emote("scream")
				else if (effect <= 4)
					M.visible_message("<span style=\"color:red\"><strong>[M.name]</strong> is all sweaty!</span>")
				...
				if (effect <= 2)
					M.visible_message("<span style=\"color:red\"><strong>[M.name]</strong> is sweating like a pig!</span>")
					M.bodytemperature += rand(20,100)
					M.take_toxin_damage(5)
					M.updatehealth()
					M.stunned += 3
				else if (effect <= 4)
					M.visible_message("<span style=\"color:red\"><strong>[M.name]</strong> starts tweaking the hell out!</span>")
					M.make_jittery(100)
					M.take_toxin_damage(2)
					...
					M.emote("scream")
					M.visible_message("<span style=\"color:red\"><strong>[M.name]</strong> nervously scratches at their skin!</span>")
					M.make_jittery(10)
					random_brute_damage(M, 5)
					M.emote("twitch")
...
//				if (prob(7)) M.emote(pick("twitch","drool","moan","giggle"))
			...
			if (prob(8))
				boutput(M, "<strong>You hear a voice in your head... <em>[pick(loggedsay)]</em></strong>")
			..(M)
			return
		reaction_mob(var/mob/M, var/method=TOUCH, var/volume)
			if (method == INGEST)
	...
	drug/THC
		...
		on_mob_life(var/mob/M)
			...
			if (prob(4))
				M.change_misstep_chance(10)
			if (holder.get_reagent_amount(id) >= 50 && prob(25))
				if (prob(10))
					M.drowsyness = 10
			..(M)
			return
	...
	drug/nicotine
		...
		do_overdose(var/severity, var/mob/M)
			...
			if (severity == 1)
				if (effect <= 2)
					M.visible_message("<span style=\"color:red\"><strong>[M.name]</strong> looks nervous!</span>")
				...
				if (effect <= 2)
					...
					boutput(M, "<span style=\"color:red\"><strong>You feel terrible!</strong></span>")
					M.emote("drool")
					M.make_jittery(10)
					M.take_toxin_damage(5)
					M.updatehealth()
					...
					M.emote("collapse")
					boutput(M, "<span style=\"color:red\"><strong>Your heart is pounding!</strong></span>")
	...
	drug/psilocybin
		...
		on_mob_life(var/mob/M)
			...
			if (prob(3))
				switch (rand(1,3))
					if (1)
						boutput(M, "<strong>The Emergency Shuttle has docked with the station! You have 3 minutes to board the Emergency Shuttle.</strong>")
					if (2)
						boutput(M, "<span style=\"color:red\"><strong>Restarting world!</strong> </span><span style=\"color:blue\">Initiated by Administrator!</span>")
					...
					if (3)
						switch (rand(1,4))
							if (1)
								boutput(M, "<span style=\"color:red\"><strong>Unknown fires the revolver at [M]!</strong></span>")
								M.playsound_local(M.loc, 'sound/weapons/Gunshot.ogg', 50, 1)
							if (2)
								boutput(M, "<span style=\"color:red\"><strong>[M] has been attacked with the fire extinguisher by Unknown</strong></span>")
								M.playsound_local(M.loc, 'sound/weapons/smash.ogg', 50, 1)
							if (3)
								boutput(M, "<span style=\"color:red\"><strong>Unknown has punched [M]</strong></span>")
								boutput(M, "<span style=\"color:red\"><strong>Unknown has weakened [M]</strong></span>")
								M.weakened += 10
								M.playsound_local(M.loc, 'sound/weapons/punch1.ogg', 50, 1)
							if (4)
								boutput(M, "<span style=\"color:red\"><strong>[M] has been attacked with the taser gun by Unknown</strong></span>")
								boutput(M, "<em>You can almost hear someone talking...</em>")
	...
	drug/krokodil
		...
		on_mob_life(var/mob/M)
			...
			if (prob(10))
				boutput(M, "<span style=\"color:blue\"><strong>You feel pretty chill.</strong></span>")
				M.bodytemperature--
				M.emote("smile")
			if (prob(5))
				boutput(M, "<span style=\"color:red\"><strong>You feel too chill!</strong></span>")
				...
				M.updatehealth()
			if (prob(2))
				boutput(M, "<span style=\"color:red\"><strong>Your skin feels all rough and dry.</strong></span>")
		...
		do_overdose(var/severity, var/mob/M)
			...
			if (severity == 1)
				if (effect <= 2)
					M.visible_message("<span style=\"color:red\"><strong>[M.name]</strong> looks dazed!</span>")
					M.stunned += 3
					M.emote("drool")
				else if (effect <= 4)
					M.emote("shiver")
					...
					boutput(M, "<span style=\"color:red\"><strong>Your skin is cracking and bleeding!</strong></span>")
					random_brute_damage(M, 5)
					M.take_toxin_damage(2)
					M.take_brain_damage(1)
					M.updatehealth()
				...
				if (effect <= 2)
					M.visible_message("<span style=\"color:red\"><strong>[M.name]</strong> sways and falls over!</span>")
					M.take_toxin_damage(3)
					...
					if (ishuman(M))
						M.visible_message("<span style=\"color:red\"><strong>[M.name]'s</strong> skin is rotting away!</span>")
	...
	drug/catdrugs
		...
		on_mob_life(var/mob/M)
			...
			M.druggy = max(M.druggy, 15)
			if (prob(11))
				M.visible_message("<span style=\"color:blue\"><strong>[M.name]</strong> hisses!</span>")
				playsound(M.loc, "sound/effects/cat_hiss.ogg", 50, 1)
			if (prob(9))
				M.visible_message("<span style=\"color:blue\"><strong>[M.name]</strong> meows! What the fuck?</span>")
	...
	drug/triplemeth
		...
		on_remove()
			...
			if (hascall(holder.my_atom,"removeOverlayComposition"))
				holder.my_atom:removeOverlayComposition(/overlayComposition/triplemeth)
		...
		on_mob_life(var/mob/M)
			...
			if (hascall(holder.my_atom,"addOverlayComposition"))
				holder.my_atom:addOverlayComposition(/overlayComposition/triplemeth)
