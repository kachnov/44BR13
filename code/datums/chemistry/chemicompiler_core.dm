...
/chemicompiler_core/New(_holder)
	..()
	if (!isdatum(_holder))
		del(src)
		return
...
/chemicompiler_core/Topic(href, href_list)
...
/** Callbacks */
/chemicompiler_core/proc/topicPermissionCheck(action)
	if (topicPermissionCheckCallback)
		return call(holder, topicPermissionCheckCallback)(action)

/chemicompiler_core/proc/showMessage(message)
	message = "<span style=\"color:red\">[message]</span>"
	if (messageCallback)
		return call(holder, messageCallback)(message)

/chemicompiler_core/proc/throwError(errorCode)
	if (errorCallback)
		return call(holder, errorCallback)(errorCode)

/chemicompiler_core/proc/reservoirClick(resId)
	if (reservoirClickCallback)
		return call(holder, reservoirClickCallback)(resId)

/chemicompiler_core/proc/reservoirCheck(resId)
	if (reservoirCheckCallback)
		return call(holder, reservoirCheckCallback)(resId)

/chemicompiler_core/proc/transfer(source, target, amount)
	if (transferCallback)
		return call(holder, transferCallback)(source, target, amount)

/chemicompiler_core/proc/heat(res, temp)
	if (heatCallback)
		return call(holder, heatCallback)(res, temp)

/chemicompiler_core/proc/statusChange(oldStatus, newStatus)
	return call(holder, "statusChange")(oldStatus, newStatus)

/chemicompiler_core/proc/validateButtId(var/id as text)
	var/buttId = text2num(id)
	if (buttId < 1 || buttId > 6)
		return FALSE
	return buttId

/chemicompiler_core/proc/getReservoirStatuses()
	var/list/ret[maxReservoir]
	for (var/i=minReservoir,i<=maxReservoir,i++)
		ret[i] = reservoirCheck(i)
	return ret

/chemicompiler_core/proc/windowCall(var/function, var/data = null)
	//boutput(world, "calling window func [function] with data: '[url_encode(data)]'")
	usr << output(data, "chemicompiler.browser:[function]")

/chemicompiler_core/proc/updatePanel()
...
/chemicompiler_core/proc/parseCBF(var/string, var/button)
...
/chemicompiler_core/proc/resetVM()
	dp = 0; // data pointer
	data = list(0) // data
...
/chemicompiler_core/proc/runCBF(var/list/inst)
	resetVM()
	running = 1
...
/chemicompiler_core/proc/resumeCBF()
	while (running)
...
/chemicompiler_core/proc/transferReagents(var/source, var/target, var/amount)
...
/chemicompiler_core/proc/heatReagents(var/rid, var/temp)
...
/chemicompiler_core/proc/initHtml()
	var/i
	htmlTag = new

	var/tag/script/controlscr = new
	controlscr.setContent( {"
	var ref = '\ref[src]';
	"})
	htmlTag.addToHead(controlscr)

	var/tag/heading/h = new(1)
	...
	var/tag/button/butt_load = new
	var/tag/button/butt_save = new
	...
	row.addClass("row-fluid")
	container.addChildElement(row)
	var/tag/label/sxLabel = new
	var/tag/label/txLabel = new
	var/tag/label/axLabel = new
	sxLabel.setText("sx ")
	txLabel.setText("tx ")
	axLabel.setText("ax ")
	var/tag/input/sx = new
	var/tag/input/tx = new
	var/tag/input/ax = new
...
/testChemicompilerHolder
	var/chemicompiler_core/cc

	New()
		..()
		cc = new /chemicompiler_core/testCore(src)
	proc
		err(errorCode)
...
/chemicompiler_executor
	...
	var/datum/holder
	var/chemicompiler_core/core

/chemicompiler_executor/New(_holder, corePath = /chemicompiler_core/portableCore)
	..()
	if (!isdatum(_holder))
		del(src)
		return
	...
	holder = _holder
	core = new corePath(src)
	reservoirs.len = core.maxReservoir

	for (var/i=core.minReservoir,i<=core.maxReservoir,i++)
		reservoirs[i] = null

/chemicompiler_executor/proc/panel()
	core.panel()

/chemicompiler_executor/proc/err(errorCode)
	switch(errorCode)
		if (CC_ERROR_INVALID_SX)
			beepCode(1, 1)
		if (CC_ERROR_INVALID_TX)
...
/chemicompiler_executor/proc/reservoirCheck(resId)
	if (istype(reservoirs[resId], /obj/item/reagent_containers/glass))
		return TRUE
/chemicompiler_executor/proc/reservoirClick(resId)
	if (istype(reservoirs[resId], /obj/item/reagent_containers/glass))
		// Taking a res out
		if (!usr.equipped())
			boutput(usr, "<span style=\"color:blue\">You remove the [reservoirs[resId]] from the [holder].</span>")
		...
		if (istype(I, /obj/item/reagent_containers/glass))
			...
			boutput(usr, "<span style=\"color:blue\">You place the [I] into the [holder].</span>")
			usr.drop_item()
			I.set_loc(holder)
			reservoirs[resId] = I

/chemicompiler_executor/proc/topicPermissionCheck(action)
 ...
 *  2: code saved
 */
/chemicompiler_executor/proc/beepCode(var/number = 1, var/loud = 0)
	var/how = loud ? "loudly" : "quietly"
	var/howmany = number == 1 ? "once" : "[number] times"
	var/message = "[holder] beeps [how] [howmany]."
	showMessage(message)

/chemicompiler_executor/proc/showMessage(message)
	message = "<span style=\"color:red\">[message]</span>"
...
/chemicompiler_executor/proc/transferReagents(var/source, var/target, var/amount)
	...
	showMessage("[holder] emits a slight humming sound.")
	sleep(round(amount * 2.5))

	var/holder2 = reservoirs[source]
	var/reagents/RS = holder2:reagents
	...
	if (target == 11)
		// Generate pill
		showMessage("[holder] makes an alarming grinding noise!")
		sleep(10)
		var/obj/item/reagent_containers/pill/P = new(get_turf(holder))
		RS.trans_to(P, amount)
		showMessage("[holder] ejects a pill.")
	if (target == 12)
		// Generate vial
		var/obj/item/reagent_containers/glass/vial/V = new(get_turf(holder))
		RS.trans_to(V, amount)
		showMessage("[holder] ejects a vial of some unknown substance.")
	if (target == 13)
		RS.trans_to(get_turf(holder), amount)
		showMessage("Something drips out the side of [holder].")
		sleep(10)

/chemicompiler_executor/proc/heatReagents(var/rid, var/temp)
	if (rid < 1 || rid > 10)
	...
	showMessage("[holder] clicks.") // Relay kicking on
	var/holder2 = reservoirs[rid]
	var/reagents/R = holder2:reagents
	while (R.total_volume && abs(R.total_temperature - temp) > 3)
		R.temperature_reagents(temp, 10)
		sleep(10)

	showMessage("[holder] clicks.") // Relay kicking off

/chemicompiler_executor/proc/statusChange(oldStatus, newStatus)
