...
/reagent
	...
	fooddrink/alcoholic/beepskybeer
		...
		on_mob_life(var/mob/living/M)
			...
			if (istype(V) && V.rider == M)
				boutput(M, "<strong><font color=red face=System>DRUNK DRIVING IS A CRIME</font></strong>")
	...
	fooddrink/alcoholic/cocktail_grog
		...
		reaction_mob(var/mob/M, var/method=TOUCH, var/volume)
			...
			//		H.visible_message("<span style=\"color:red\"><strong>[H] explodes in a shower of gibs, hair and piracy!</strong></span>","<span style=\"color:red\"><strong>Oh god, too much hair!</strong></span>")
			...
			//		return
				if (H.cust_one_state != "dreads" || H.cust_two_state != "fullbeard")
					boutput(H, "<strong>You feel more piratey! Arr!</strong>")
	...
	fooddrink/alcoholic/dbreath
		...
		on_mob_life(var/mob/M)
			...
			if (prob(8))
				boutput(M, "<span style=\"color:red\"><strong>Oh god! Oh GODD!!</strong></span>")
			if (prob(50))
			...
			if (prob(2 * volume))
				boutput(M, "<span style=\"color:red\"><strong>OH GOD OH GOD PLEASE NO!!</strong></span>")
				var/mob/living/L = M
				if (istype(L) && L.burning)
					L.set_burning(99)
				if (prob(50))
	...
	fooddrink/alcoholic/squeeze
		...
		reaction_mob(var/mob/M, var/method=TOUCH, var/volume_passed)
			...
			if (method == INGEST)
				...
				M.weakened += 2
				var/mob/living/L = M
				L.contract_disease(/ailment/disease/food_poisoning, null, null, 1)
				if (prob(10))
					M.visible_message("<span style=\"color:red\">[M] horks all over \himself. Gross!</span>")
					playsound(M.loc, "sound/effects/splat.ogg", 50, 1)
					new /obj/decal/cleanable/vomit(M.loc)
	...
	fooddrink/capsaicin
		...
		on_mob_life(var/mob/M)
			if (!M) M = holder.my_atom
			if (holder.get_reagent_amount(id) >= 20)
		...
		reaction_mob(var/mob/M, var/method=TOUCH, var/volume_passed)
			...
			if (method == INGEST)
				if (volume_passed > 10)
					if (volume_passed >= 80)
						boutput(M, "<span style=\"color:red\"><strong>HOLY FUCK!!!!</strong></span>")
						M.emote("scream")
						M.stuttering += 30
						M.weakened += 5
				...
				if (iscarbon(M))
					if (!M.wear_mask)
						...
						if (prob(50))
							M.emote("scream")
							boutput(M, "<span style=\"color:red\"><strong>Your eyes hurt!</strong></span>")
	...
	fooddrink/coffee/energydrink
		...
		do_overdose(var/severity, var/mob/M)
			if (severity == 1 && prob(10))
				...
				M:contract_disease(/ailment/disease/heartfailure, null, null, 1)
	...
	fooddrink/honey_tea
		...
		on_mob_life(var/mob/living/M)
			...
			for (var/ailment_data/disease/virus in M.ailments)
				if (prob(5) && istype(virus.master,/ailment/disease/cold))
					M.cure_disease(virus)
				if (prob(3) && istype(virus.master,/ailment/disease/flu))
					M.cure_disease(virus)
				if (prob(3) && istype(virus.master,/ailment/disease/food_poisoning))
					M.cure_disease(virus)
			if (prob(11))
	...
	fooddrink/chickensoup
		...
		on_mob_life(var/mob/living/M)
			if (!M) M = holder.my_atom
			for (var/ailment_data/disease/virus in M.ailments)
				if (prob(10) && istype(virus.master,/ailment/disease/cold))
					M.cure_disease(virus)
				if (prob(10) && istype(virus.master,/ailment/disease/flu))
					M.cure_disease(virus)
				if (prob(10) && istype(virus.master,/ailment/disease/food_poisoning))
					M.cure_disease(virus)
			..(M)
			return
	...
	fooddrink/enriched_msg //Hukhukhuk brings you another culinary war crime
		...
		reaction_mob(var/mob/M, var/method=TOUCH, var/volume_passed)
			...
			if (!ishuman(M))
				return
			if (method == INGEST)
				var/ailment/addiction/AD = M.addicted_to_reagent(src)
				if (!AD)
					boutput(M, "<strong>THIS TASTES <font size=\"92\">~<font color=\"#FF0000\"> A<font color=\"#FF9900\"> M<font color=\"#FFff00\"> A<font color=\"#00FF00\"> Z<font color=\"#0000FF\"> I<font color=\"#FF00FF\"> N<font color=\"#660066\"> G<font color=\"#000000\"> ~ !</font></strong>")
		...
		do_overdose(var/severity, var/mob/M)
			...
			if (severity == 1) //lesser
				M.stuttering += 1
				if (effect <= 1)
					M.visible_message("<span style=\"color:red\"><strong>[M.name]</strong> suddenly starts salivating.</span>")
					M.emote("drool")
					M.change_misstep_chance(10)
					M.weakened = max(2, M.weakened)
				else if (effect <= 3)
					M.visible_message("<span style=\"color:red\"><strong>[M.name]</strong> begins to reminisce about food.</span>")
					M.stunned = max(2, M.stunned)
				else if (effect <= 5)
					M.visible_message("<span style=\"color:red\"><strong>[M.name]</strong> pouts and sniffles a bit.</span>")
				else if (effect <= 7)
					M.visible_message("<span style=\"color:red\"><strong>[M.name]</strong> shakes uncontrollably.</span>")
					M.make_jittery(30)
			else if (severity == 2) // greater
				if (effect <= 2)
					M.visible_message("<span style=\"color:red\"><strong>[M.name]</strong> enters a food coma!</span>")
					M.emote("faint")
					M.paralysis = max(4, M.paralysis)
				else if (effect <= 5)
					M.visible_message("<span style=\"color:red\"><strong>[M.name]</strong> wants more delicious food!</span>")
					M.emote("scream")
					M.stunned = max(4, M.stunned)
				else if (effect <= 8)
					M.visible_message("<span style=\"color:red\"><strong>[M.name]</strong> appears extremely depressed.</span>")
					M.emote("moan")
		...
		reaction_mob(var/mob/M, var/method=TOUCH, var/volume)
			if (method == TOUCH)
				...
				if (prob(60) && (holder && holder.get_reagent_amount("sugar") < (volume/3)))
					M.visible_message("<strong>[M]'s</strong> mouth puckers!","<span style=\"color:red\">Yow! Sour!</span>")
