			...
			if (!H.bioHolder.HasEffect("resist_alcohol"))
				...
				if (holder.get_reagent_amount(id) >= 225)
					...
					if (H.canmove && isturf(H.loc) && prob(8))
						step(H, pick(cardinal))
				if (holder.get_reagent_amount(id) >= 275)
					if (prob(10))
					...
					if (prob(15))
						H.make_dizzy(5)
				if (holder.get_reagent_amount(id) >= 300)
					H.change_eye_blurry(10, 50)
	...
	sugar
		...
		do_overdose(var/severity, var/mob/M)
			...
			if (M:bioHolder && M:bioHolder.HasEffect("bee"))
				...
				M.visible_message("<strong>[M]</strong> regurgitates a blob of honey! Gross!")
				...
				for (var/obj/critter/domestic_bee/responseBee in range(7, M))
					...
					responseBee.visible_message("<strong>[responseBee]</strong> [ pick("looks confused.", "appears to undergo a metaphysical crisis.  What is human?  What is space bee?<br>Or it might just have gas.", "looks perplexed.", "bumbles in a confused way.", "holds out its forelegs, staring into its little bee-palms and wondering what is real.") ]")
			...
			if (istype(M))
				if (isvampire(M))
					...
					for (var/mob/O in AIviewers(M, null))
						O.show_message(text("<span style=\"color:red\"><strong>[] begins to crisp and burn!</strong></span>", M), 1)
					...
					for (var/ailment_data/disease/V in M.ailments)
						if (prob(1))
							M.cure_disease(V)
			if (method == TOUCH)
