...
/obj/item/reagent_containers/glass/jar
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/reagent_containers/food/snacks))
			...
			user.drop_item()
			W.set_loc(src)
			user.visible_message("<strong>[user]</strong> puts [W] into [src].", "You stuff [W] into [src].")
			update_icon()
	...
	attack_self(mob/user as mob)
		if (contents.len)
			var/obj/item/yoinked_out_thing = pick(contents)
			if (istype(yoinked_out_thing))
				user.put_in_hand_or_drop(yoinked_out_thing)
				user.visible_message("<strong>[user]</strong> pulls [yoinked_out_thing] out of [src]","You pull [yoinked_out_thing] out of [src]")
				update_icon()

	on_reagent_change()
		if (!color_underlay)
			color_underlay = image(icon, "mason_jar_brine")
		...
		if (!reagents || !reagents.total_volume)
			...
			underlays = null
			color_underlay.color = reagents.get_master_color()
			underlays = list(color_underlay)

		update_icon()

	proc/update_icon()
		if (contents.len)
			icon_state = "mason_jar_green"
...
/proc/load_intraround_jars()
	...
	for (var/jar_entry in jars_encoded)
		...
		logTheThing("debug", null, null, "<strong>Pickle Jar:</strong> Jar created at \[[jarX],[jarY],[jarZ]\] containing [.]")
...
/obj/item/reagent_containers/food/snacks/pickle_holder
	...
	New(newloc, var/obj/item/reagent_containers/pickled)
		...
		if (istype(pickled))
			...
			icon = new /icon(pickled.icon, pickled.icon_state)
			overlays = pickled.overlays
			underlays = pickled.underlays

			color = rgb(63,103,24)

			desc = "A pickled version of \a [pickled], It smells of vinegar."
			name = "pickled [pickled.name]"
