...
/obj/item/chem_grenade
	...
	New()
		var/reagents/R = new/reagents(150000)
		reagents = R
		R.my_atom = src
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W,/obj/item/grenade_fuse) && !stage)
			...
			if (!S || !S:status)
				return
			boutput(user, "<span style=\"color:blue\">You attach the [name] to the [S.name]!</span>")
	...
	afterattack(atom/target as mob|obj|turf|area, mob/user as mob)
		...
		if (stage == 2 && (user.equipped() == src))
			if (!state)
				...
				logTheThing("combat", user, null, "primes a [log_reagents ? "custom grenade" : "grenade ([type])"] at [log_loc(user)].[log_reagents ? " [log_reagents]" : ""]")
	...
	proc
		explode()
			...
			if (src.reagents.total_volume) //The possible reactions didnt use up all reagents.
				...
				steam.attach(src)
				steam.start()
				var/min_dispersal = reagents.get_dispersal()
				for (var/atom/A in range(min_dispersal, get_turf(loc)))
					if ( A == src ) continue
					reagents.grenade_effects(src, A)
					reagents.reaction(A, 1, 10)

			invisibility = 100 //Why am i doing this?
			if (master) master.invisibility = 100
