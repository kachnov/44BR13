...
/obj/item/extinguisher/New()
	..()
	var/reagents/R = new/reagents(75)
	reagents = R
	R.my_atom = src
...
/obj/item/extinguisher/get_desc(dist)
	...
	return "Contains [reagents.total_volume] units."
...
/obj/item/extinguisher/afterattack(atom/target, mob/user , flag)
	...
	if (!safety && !istype(target, /obj/item/storage) && !istype(target, /obj/item/storage/secure))
		...
		if (reagents.has_reagent("infernite") && reagents.has_reagent("blackpowder")) // BAHAHAHAHA
			user.visible_message("<span style=\"color:red\">[src] violently bursts!</span>")
			user.drop_item()
			playsound(loc, "sound/effects/bang.ogg", 60, 1, -3)
			fireflash(loc, 0)
		...
		else if (reagents.has_reagent("infernite") || reagents.has_reagent("foof"))
		...
		else if (reagents.has_reagent("vomit") || reagents.has_reagent("blackpowder") || reagents.has_reagent("blood") || reagents.has_reagent("gvomit") || reagents.has_reagent("carbon") || reagents.has_reagent("cryostylane") || reagents.has_reagent("chickensoup") || reagents.has_reagent("salt"))
			boutput(user, "<span style=\"color:red\">The nozzle is clogged!</span>")
			return

		else if (reagents.has_reagent("acid") || reagents.has_reagent("pacid") || reagents.has_reagent("napalm"))
...
/obj/item/extinguisher/attack_self(mob/user as mob)
	if (safety)
		icon_state = "fire_extinguisher1"
		desc = "The safety is off."
