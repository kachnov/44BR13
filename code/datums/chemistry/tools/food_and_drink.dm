...
/obj/item/reagent_containers/food
	...
	proc/on_table()
		...
		for (var/atom/movable/M in loc) //Arguably more elegant than a million locates. I don't think locate works with derived classes.
	...
	proc/heal(var/mob/living/M)
		...
		if (!isnull(unlock_medal_when_eaten))
			M.unlock_medal(unlock_medal_when_eaten, 1)
...
/*	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/shaker))
			...
			reagents.add_reagent("[shaker.stuff]", 2)
			shaker.shakes ++
...
*/
	...
	Eat(var/mob/M as mob, var/mob/user, var/bypass_utensils = 0)
		...
		if (iscarbon(M) || istype(M, /mob/living/critter))
			if (M == user)
				if (!bypass_utensils)
					if (needfork && !user.find_in_hands(/obj/item/kitchen/utensil/fork))
						...
						return
					if (needspoon && !user.find_in_hands(/obj/item/kitchen/utensil/spoon))
				...
				if (reagents && reagents.total_volume)
					...
					reagents.trans_to(M, reagents.total_volume/(amount ? amount : 1))
				amount--
				...
				on_bite(M)
				if (festivity)
					modify_christmas_cheer(festivity)
				if (!amount)
					...
					if (istype(src, /obj/item/reagent_containers/food/snacks/plant) && prob(35))
						...
						var/doseed = 1
						var/plantgenes/SRCDNA = P.plantgenes
						...
						if (doseed)
							...
							if (istype(stored) && !stored.isgrass)
								...
								var/plantgenes/PDNA = S.plantgenes
								...
								if (stored.hybrid)
									...
									S.planttype = hybrid
								user.visible_message("<span style=\"color:blue\"><strong>[user]</strong> spits out a seed.</span>",\
...
/obj/item/reagent_containers/food/drinks
	...
	on_reagent_change()
		update_gulp_size()
		doants = reagents && reagents.total_volume > 0
...
*/
			if (reagents.total_volume)
				...
				reagents.reaction(M, INGEST, gulp_size)
				spawn (5)
					if (src && reagents && M && M.reagents)
...
/obj/item/reagent_containers/food/drinks/bowl
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/reagent_containers/food/snacks/cereal_box))
			...
			cbox.prize = 0
			newcereal.reagents = reagents

			if (newcereal.reagents)
				newcereal.reagents.my_atom = newcereal
				reagents = null
			else
				newcereal.reagents = new /reagents(50)
				newcereal.reagents.my_atom = newcereal

			newcereal.on_reagent_change()

			user.visible_message("<strong>[user]</strong> pours [cbox] into [src].", "You pour [cbox] into [src].")
			cbox.amount--
			if (cbox.amount < 1)
...
/obj/item/reagent_containers/food/drinks/bottle
	...
	proc/update_icon()
		...
		if (broken)
			...
			if (label)
				...
				overlays += bottle_image
		else
			if (!bottle_image) bottle_image = image('icons/obj/bottle.dmi')
			if (reagents && reagents.total_volume <= 0) //Fix for cannot read null/volume. Also FUCK YOU REAGENT CREATING FUCKBUG!
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/pen) && !labeled)
			var/t = input(user, "Enter label", name, null) as null|text
	...
	attack(target as mob, mob/user as mob)
		if (broken && !unbreakable)
			...
			if (shatter >= rand(2,12))
				...
				shatter++
				user.visible_message("<span style=\"color:red\"><strong>[user]</strong> [pick("shanks","stabs","attacks")] [target] with the broken [src]!</span>")
...
/obj/item/reagent_containers/food/drinks/drinkingglass
	...
	proc/update_icon()
		...
		if (in_glass)
			...
			overlays += image("icon" = in_glass_icon, "layer" = FLOAT_LAYER - 1)
		if (salted)
		...
		if (wedge)
			...
			overlays += image("icon" = wedge_icon, "layer" = FLOAT_LAYER + 1)
		return
		...
		if (istype(W, /obj/item/raw_material/ice))
			if (reagents.total_volume >= (reagents.maximum_volume - 5))
				if (user.bioHolder.HasEffect("clumsy") && prob(50))
					...
					reagents.reaction(get_turf(user), TOUCH, reagents.total_volume / 2)
					reagents.add_reagent("ice", 5, null, (T0C - 1))
					qdel(W)
					return
				else
	...
	attack_self(var/mob/user as mob)
		...
		if ((H.sims && H.sims.getValue("bladder") <= 65) || (!H.sims && H.urine >= 2))
			actions += "pee in it"
		if (in_glass)
			actions += "eat the [in_glass]"
		if (wedge)
			actions += "eat the [wedge] wedge"
		if (!actions.len)
		...
		switch (action)
			if ("pee in it")
				if ((H.sims && H.sims.getValue("bladder") <= 65) || (!H.sims && H.urine >= 2))
					H.visible_message("<span style=\"color:red\"><strong>[H] pees in [src]!</strong></span>")
					playsound(get_turf(H), "sound/misc/pourdrink.ogg", 50, 1)
					if (!H.sims)
						H.urine -= 2
					else
						H.sims.affectMotive("bladder", 100)
					reagents.add_reagent("urine", 20)
				else
		...
		H.visible_message("<strong>[H]</strong> plucks the [eat_message] of [src] and eats it.")
...
/obj/item/reagent_containers/food/drinks/pitcher
	...
	New()
		..()
		fluid_image = image(icon, "fluid-pitcher")
		update_icon()
