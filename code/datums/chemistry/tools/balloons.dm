...
/obj/item/reagent_containers/balloon
	...
	proc/update_icon()
		if (reagents && reagents.total_volume > 0)
			if (reagents.has_reagent("helium"))
				icon_state = "balloon_[balloon_color]_inflated"
				item_state = "balloon_[balloon_color]_inflated"
			else
				icon_state = "balloon_[balloon_color]_full"
				item_state = "balloon_[balloon_color]_full"
		else
			icon_state = "balloon_[balloon_color]"
			item_state = "balloon_[balloon_color]"
	...
	proc/burst_chance(mob/user as mob, var/ohshit)
		...
		if (!reagents)
			return
		if (!ohshit)
			ohshit = (reagents.total_volume / 30) * 33
		if (prob(ohshit))
			smash()
			if (user)
				user.visible_message("<span style=\"color:red\">[src] bursts in [user]'s hands!</span>", \
				"<span style=\"color:red\">[src] bursts in your hands! <strong>[curse]!</strong></span>")
			else if (usr)
				usr.visible_message("<span style=\"color:red\">[src] bursts in [usr]'s hands!</span>", \
				"<span style=\"color:red\">[src] bursts in your hands! <strong>[curse]!</strong></span>")
			else
				loc.visible_message("<span style=\"color:red\">[src] bursts!</span>")
			return
/*		if (reagents.total_volume > 30)
			if (prob(50))
				user.visible_message("<span style=\"color:red\">[src] is overfilled and bursts! <strong>[curse]</strong></span>")
				smash()
				return
*/
	attack_self(var/mob/user as mob)
		...
		switch (action)
			if ("Make balloon animal")
				if (reagents.total_volume > 0)
					user.visible_message("<strong>[user]</strong> fumbles with [src]!", \
					...
					user.visible_message("<strong>[user]</strong> blows up [src] and [fluff]twists it into a[animal == "owl" ? "n" : ""] [animal]!", \
					"You blow up [src] and [fluff]twist it into a[animal == "owl" ? "n" : ""] [animal]!")
					var/obj/item/balloon_animal/A = new /obj/item/balloon_animal(get_turf(loc))
			...
			if ("Inhale")
				H.visible_message("<span style=\"color:red\"><strong>[H] inhales the contents of [src]!</strong></span>",\
				"<span style=\"color:red\"><strong>You inhale the contents of [src]!</strong></span>")
				reagents.trans_to(H, 40)
				return

			if ("Pee in it")
				H.visible_message("<span style=\"color:red\"><strong>[H] pees in [src]!</strong></span>",\
				"<span style=\"color:red\"><strong>You pee in [src]!</strong></span>")
	...
	proc/smash(var/turf/T)
		...
		decal.icon_state = "balloon_[balloon_color]_pop"
		qdel(src)
