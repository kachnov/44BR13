...
/obj/reagent_dispensers
	...
	New()
		..()
		var/reagents/R = new/reagents(4000)
		reagents = R
		R.my_atom = src
...
/obj/reagent_dispensers/watertank/big
	...
	New()
		..()
		var/reagents/R = new/reagents(100000)
		reagents = R
		R.my_atom = src
...
/obj/reagent_dispensers/watertank/fountain
	...
	get_desc(dist, mob/user) // this shit refused to show the parent get_desc() info even if I added a ..() so I'M JUST COPYING THE CODE NOW LIKE SOME KIND OF GIGANTIC ASSHOLE
		. += "There's [cup_amount] paper cup[s_es(cup_amount)] in [src]'s cup dispenser."
		if (dist <= 2 && reagents)
			. += "<br><span style=\"color:blue\">[reagents.get_description(user,RC_SCALE)]</span>"
	...
	attack_hand(mob/user as mob)
		if (cup_amount <= 0)
			...
			visible_message("<strong>[user]</strong> grabs a paper cup from [src].",\
...
/obj/reagent_dispensers/fueltank
	...
	suicide(var/mob/user as mob)
		if (!reagents.has_reagent("fuel",20))
			return FALSE
		user.visible_message("<span style=\"color:red\"><strong>[user] drinks deeply from the [name]. \He then pulls out a match from somewhere, strikes it and swallows it!</strong></span>")
		reagents.remove_any(20)
		playsound(loc, "sound/items/drink.ogg", 50, 1, -6)
...
/obj/reagent_dispensers/compostbin
	...
	attackby(obj/item/W as obj, mob/user as mob)
		var/load = 1
		if (istype(W,/obj/item/reagent_containers/food/snacks/plant)) reagents.add_reagent("poo", 20)
		else if (istype(W,/obj/item/reagent_containers/food/snacks/mushroom)) reagents.add_reagent("poo", 25)
		else if (istype(W,/obj/item/seed)) reagents.add_reagent("poo", 2)
		else if (istype(W,/obj/item/plant)) reagents.add_reagent("poo", 15)
		else load = 0
		...
		if (istype(O, /obj/item/reagent_containers/food/snacks/plant/) || istype(O, /obj/item/reagent_containers/food/snacks/mushroom/) || istype(O, /obj/item/seed/) || istype(O, /obj/item/plant))
			...
			for (var/obj/item/P in view(1,user))
				...
				if (istype(P,/obj/item/reagent_containers/food/snacks/mushroom))
					...
					amount = 15
				playsound(loc, "sound/effects/blobattack.ogg", 50, 1)
				reagents.add_reagent("poo", amount)
				qdel( P )
				sleep(3)
...
/obj/reagent_dispensers/still
	...
	proc/brew(var/obj/item/W as obj)
		...
		if (islist(W:brew_result) && W:brew_result:len)
			for (var/i in W:brew_result)
				...
				reagents.add_reagent(i, 10)
			//brewed_name = copytext(brewed_name, 3)
		else
			reagents.add_reagent(W:brew_result, 20)
			//brewed_name = reagent_id_to_name(W:brew_result)

		visible_message("<span style=\"color:blue\">[src] brews up [W]!</span>")// into [brewed_name]!")
		//qdel(W)
		return TRUE
	...
	MouseDrop_T(atom/movable/O as mob|obj, mob/user as mob)
		...
		if (istype(O, /obj/storage/crate))
			...
			user.visible_message("<span style=\"color:blue\"><strong>[user]</strong> begins quickly stuffing items into [src]!</span>",\
			...
			for (O in view(1,user))
				...
				if (brew(O))
					...
					continue
			user.visible_message("<span style=\"color:blue\"><strong>[user]</strong> finishes stuffing items into [src].</span>",\
