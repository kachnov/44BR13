...
/obj/item/reagent_containers/syringe
	...
	on_reagent_change()
		if (reagents.is_full() && mode == S_DRAW)
			mode = S_INJECT
		else if (!reagents.total_volume && mode == S_INJECT)
			mode = S_DRAW
		update_icon()
		...
		switch(mode)
			if (S_DRAW)
				if (istype(target, /mob/living))//Blood!
					...
					if (target != user)
						L.visible_message("<span style=\"color:red\"><strong>[user] is trying to draw blood from [L]!</strong></span>")
			...
			if (S_INJECT)
				...
				if (istype(target, /obj/item/bloodslide))
					...
					if (BL.reagents.total_volume)
						...
						return
					var/transferred = reagents.trans_to(target, 5)
				...
				if (iscarbon(target) || iscritter(target))
					if (target != user)
						for (var/mob/O in AIviewers(world.view, user))
							O.show_message(text("<span style=\"color:red\"><strong>[] is trying to inject []!</strong></span>", user, target), 1)
				...
				spawn (5)
					if (src && reagents && target && target.reagents)
						...
						boutput(user, "<span style=\"color:blue\">You inject 5 units of the solution. The syringe now contains [reagents.total_volume] units.</span>")
		return
	...
	proc/update_icon()
		...
		underlays = null
		if (ismob(loc))
			overlays += mode == S_INJECT ? "inject" : "draw"
		fluid_image.icon_state = "f[rounded_vol]"
