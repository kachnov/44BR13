...
/obj/item/reagent_containers/patch
	...
	New()
		..()
		fluid_image = image('icons/obj/chemical.dmi', "[style]-fluid")
	...
	on_reagent_change()
		...
		if (reagents.total_volume)
			icon_state = "[style]1"
			if (medical == 1)
				icon_state = "[style]_med1"
			if (reagents.has_reagent("LSD",1))
				icon_state = "[style]_LSD"
	...
	attack(mob/M as mob, mob/user as mob)
		...
		if (borg == 1 && !issilicon(user))
			user.show_text("This item is not designed with organic users in mind.", "red")
			return

		// No reagents check here because empty patches can be used to counteract bleeding.
		in_use = 1
		...
		if (iscarbon(M) || iscritter(M))
			if (M == user)
				...
				if (medical == 0)
					user.visible_message("<span style=\"color:red\"><strong>[user]</strong> is trying to stick [src] to [M]'s arm!</span>",\
					...
					user.visible_message("<span style=\"color:red\"><strong>[user]</strong> sticks [src] to [M]'s arm.</span>",\
					"<span style=\"color:red\">You stick [src] to [M]'s arm.</span>")

				else if (borg == 1)
					user.visible_message("<span style=\"color:blue\"><strong>[user]</strong> stamps [src] on [M].</span>",\
					...
					if (user.mind && user.mind.objectives && M.health < 90) //might as well let people complete this even if they're borged
						for (var/objective/crew/medicaldoctor/heal/H in user.mind.objectives)
							H.patchesused ++

				else
					user.visible_message("<span style=\"color:blue\"><strong>[user]</strong> applies [src] to [M].</span>",\
...
/obj/item/patch_stack
	...
	proc/update_overlay()
		...
		if (patches.len)
			...
			if (P)
				overlays += image(P.icon, P.icon_state)
				name = "[initial(name)] ([P.name]; [patches.len] patches)"
		else
			name = initial(name)
