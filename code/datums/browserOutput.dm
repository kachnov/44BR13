...
/chatOutput
	...
	proc
		...
		load()
			if (owner)
				// Do this always, because the CDN doesn't have custom styles
				if (!CDN_ENABLED || config.env == "dev" || TRUE)
					...
					owner.loadResourcesFromList(chatResources)

				owner << browse(grabResource("html/browserOutput.html"), "window=browseroutput")

				if (loadAttempts < 5) //To a max of 5 load attempts
		...
		doneLoading(ua)
			if (!loaded)
				...
				if (owner.holder)
					loadAdmin()
				if (messageQueue)
					for (var/x = 1, x <= messageQueue.len, x++)
						boutput(owner, messageQueue[x])
				messageQueue = null
				if (ua)
					...
					spawn (600) //60 seconds
						if (!cookieSent) //Client has very likely futzed with their local html/js chat file
							out(owner, "<div class='fatalError'>Chat file tampering detected. Closing connection.</div>")
							del(owner)
					*/
		...
		loadAdmin()
			...
			ehjax.send(owner, "browseroutput", url_encode(data))
		...
		sendClientData()
			...
			ehjax.send(owner, "browseroutput", data)
			...
			if (cookie != "none")
				...
				if (connData && islist(connData) && connData.len > 0 && connData["connData"])
					...
					if (found.len > 0)
						...
						logTheThing("debug", owner, null, "has a cookie from a banned account! (Matched: [found["ckey"]], [found["ip"]], [found["compid"]])")
						logTheThing("diary", owner, null, "has a cookie from a banned account! (Matched: [found["ckey"]], [found["ip"]], [found["compid"]])", "debug")
		...
		//Called in New() (/admins)
		getContextFlag()
			if (!owner.holder) return
			var/level = owner.holder.level
		...
		handleContextMenu(command, target)
			if (!owner.holder) return
			var/mind/targetMind = locate(target)
			var/mob/targetMob
			if (targetMind)
			...
			switch(command)
				if ("pm")
					owner.cmd_admin_pm(targetMob)
				if ("smsg")
					owner.cmd_admin_subtle_message(targetMob)
				if ("jump")
					if (!istype(targetMob, /mob/dead/target_observer))
						owner.jumptomob(targetMob)
					else
						var/jumptarget = targetMob.eye
						if (jumptarget)
							owner.jumptoturf(get_turf(jumptarget))
				if ("get")
					owner.Getmob(targetMob)
				if ("boot")
					owner.cmd_boot(targetMob)
				if ("ban")
					owner.addBanDialog(targetMob)
				if ("gib")
					owner.cmd_admin_gib(targetMob)
					logTheThing("admin", owner, targetMob, "gibbed %target%.")
				if ("popt")
					owner.holder.playeropt(targetMob)
