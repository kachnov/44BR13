...
/tutorial/blob
	...
	New()
		...
		if (blob_tutorial_areas.len)
			...
			logTheThing("debug", usr, null, "<strong>Blob Tutorial</strong>: Got area [tutorial_area]")
			if (tutorial_area)
				...
				for (var/obj/landmark/tutorial_start/temp in tutorial_area)
					...
					break
				if (!L)
					logTheThing("debug", usr, null, "<strong>Blob Tutorial</strong>: Tutorial failed setup: missing landmark.")
					throw EXCEPTION("Okay who removed the goddamn blob tutorial landmark")
				initial_turf = get_turf(L)
				if (!initial_turf)
					logTheThing("debug", usr, null, "<strong>Blob Tutorial</strong>: Tutorial failed setup: [L], [initial_turf].")

	Start()
		if (!initial_turf)
			logTheThing("debug", usr, null, "<strong>Blob Tutorial</strong>: Failed setup.")
			boutput(usr, "<span style=\"color:red\"><strong>Error setting up tutorial!</strong></span>")
			qdel(src)
			return
		if (..())
			bowner = owner
	...
	Finish()
		if (..())
			...
			bowner.add_ability(/blob_ability/plant_nucleus)
			bowner.add_ability(/blob_ability/set_color)
			bowner.add_ability(/blob_ability/help)
...
/tutorialStep/blob
	...
	upgrades
		...
		MayAdvance()
			var/tutorial/blob/MT = tutorial
			return MT.bowner.has_upgrade(/blob_upgrade/launcher) && MT.bowner.has_upgrade(/blob_upgrade/replicator) && MT.bowner.has_upgrade(/blob_upgrade/devour_item) && MT.bowner.has_upgrade(/blob_upgrade/reflective)
...
/proc/AddBlobSteps(var/tutorial/blob/T)
	T.AddStep(new /tutorialStep/blob/deploy)
	T.AddStep(new /tutorialStep/blob/spread)
	T.AddStep(new /tutorialStep/blob/attack)
	T.AddStep(new /tutorialStep/blob/spread2)
	T.AddStep(new /tutorialStep/blob/cutscene)
	T.AddStep(new /tutorialStep/blob/firewall)
	T.AddStep(new /tutorialStep/blob/cutscene2)
	T.AddStep(new /tutorialStep/blob/ribosomes)
	T.AddStep(new /tutorialStep/blob/clickmove)
	T.AddStep(new /tutorialStep/blob/hotkeys)
	T.AddStep(new /tutorialStep/blob/upgrades)
	T.AddStep(new /tutorialStep/blob/digestation)
	T.AddStep(new /tutorialStep/blob/deposits)
	T.AddStep(new /tutorialStep/blob/replicators)
	T.AddStep(new /tutorialStep/blob/replication)
	T.AddStep(new /tutorialStep/blob/launcher)
	T.AddStep(new /tutorialStep/blob/reagent_launcher)
	T.AddStep(new /tutorialStep/blob/cutscene3)
	T.AddStep(new /tutorialStep/blob/finished)
