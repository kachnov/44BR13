...
f_color_selector_handler
	...
	proc
		Get_Color(mob/M, default_color = null)
			if (ismob(M))
				if (!M.client)
					world.log << "<strong>ERROR:</strong> Call to f_color_selector_handler passed mob with no client."
					return
			var/client/C
			if (ismob(M))
				C = M.client
				...
				world.log << "<strong>ERROR:</strong> Call to f_color_selector_handler passed invalid value.  (Not a client or mob with client.)"
				return
