...
/client/proc/debug_variables(datum/D in world)
	...
	if (istype(D, /atom))
		var/atom/A = D
		title = "[A.name][holder.level >= LEVEL_SHITGUY ? " (\ref[A])" : ""] = [A.type]"

		#ifdef VARSICON
		if (A.icon)
			body += debug_variable("icon", new/icon(A.icon, A.icon_state, A.dir), 0)
		#endif
	title = "[D][holder.level >= LEVEL_SHITGUY ? " (\ref[D])" : ""] = [D.type]"
	...
	for (var/V in names)
		...
		if (istype(D.vars[V], /datum) && holder.level >= LEVEL_CODER)
...
*/
		...
		if ("name" in D.vars)
			dname = " (" + D.vars["name"] + ")"
		html += "<a href='byond://?src=\ref[src];Vars=\ref[value]'>[name][holder.level >= LEVEL_SHITGUY ? " \ref[value]" : ""]</a> = [D.type][dname]"

	else if (istype(value, /client))
		var/client/C = value
		html += "<a href='byond://?src=\ref[src];Vars=\ref[value]'>[name][holder.level >= LEVEL_SHITGUY ? " \ref[value]" : ""]</a> = [C] [C.type]"
...
/client/proc/modify_variable(datum/D, variable, set_global = 0)
	...
	var/list/locked_type = list(/admins) //Short list - might be good if there are more objects that oughta be paws-off
	if (!(holder.rank in list("Host", "Coder")) && D.type in locked_type )
	...
	if (!istype(D, /atom))
		original_name = "[holder.level >= LEVEL_SHITGUY ? "\ref[D] " : ""]([D])"
	else
	...
	switch(class)
		...
		if ("type")
			var/theInput = input("Enter type:","Type",D.vars[variable]) in null|subtypesof(/datum)
			if (theInput == null) return
			if (set_global)
				for (var/x in world)
					if (!istype(x, D.type)) continue
		...
		if ("reference")
			var/theInput = input("Select reference:","Reference", D.vars[variable]) as null|anything in world
			if (theInput == null) return
			if (set_global)
				for (var/x in world)
					if (!istype(x, D.type)) continue
		...
		if ("reference picker")
			...
			var/mob/M = usr
			if (istype(M))
				var/targetable/refpicker/R
				if (set_global)
					R = new /targetable/refpicker/global()
				else
					R = new()
				R.target = D
				R.varname = variable
		...
		if ("new instance of a type")
			...
			if (typename)
				var/basetype = /obj
				if (holder.rank in list("Host", "Coder", "Shit Person"))
					basetype = /datum
