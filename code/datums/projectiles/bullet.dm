...
//Any special things when it hits shit?
	on_hit(atom/hit, direction, projectile)
		if (ishuman(hit) && hit_type)
			take_bleeding_damage(hit, null, round(power / 3), hit_type) // oh god no why was the first var set to src what was I thinking
		return // BULLETS CANNOT BLEED, HAINE

/projectile/bullet/bullet_22
	name = "bullet"
	power = 35
...
/projectile/bullet/revolver_357
	name = "bullet"
...
/projectile/bullet/revolver_38/AP//traitor det revolver
	power = 50
	implanted = /obj/item/implant/projectile/bullet_38AP
	damage_type = D_PIERCING
	hit_type = DAMAGE_STAB

/projectile/bullet/rifle_3006
	name = "bullet"
	power = 125
	damage_type = D_PIERCING
	hit_type = DAMAGE_STAB
	...
	on_hit(atom/hit, dirflag)
		if (ishuman(hit))
			...
			if (hit_type)
				take_bleeding_damage(hit, null, round(power / 3), hit_type)

/projectile/bullet/tranq_dart
	name = "dart"
	power = 10
...
/projectile/bullet/revolver_38/stunners//energy bullet things so he can actually stun something
	name = "stun bullet"
	power = 20
	ks_ratio = 0.0
...
/projectile/bullet/shrapnel // cogwerks: for explosions
	name = "shrapnel"
	power = 10
	damage_type = D_PIERCING
	hit_type = DAMAGE_CUT
...
/obj/smokeDummy
	...
	proc/remove()
		for (var/mob/M in affected)
			if (M:hasOverlayComposition(/overlayComposition/smoke))
				M:removeOverlayComposition(/overlayComposition/smoke)
	...
	Crossed(atom/movable/O)
		...
		if (ismob(O) && O:client)
			if (!isobserver(O) && !iswraith(O) && !isintangible(O)) // fuck you stop affecting ghosts FUCK YOU
				if (!O:hasOverlayComposition(/overlayComposition/smoke))
					O:addOverlayComposition(/overlayComposition/smoke)
		...
		if (ismob(O) && O:client)
			if (!isobserver(O) && !iswraith(O) && !isintangible(O)) // FUCK YOU
				if (!(locate(/obj/smokeDummy) in O.loc))
					if (O:hasOverlayComposition(/overlayComposition/smoke))
						O:removeOverlayComposition(/overlayComposition/smoke)
						O:updateOverlaysClient(O:client)
				affected.Remove(O)
		return ..()

/projectile/bullet/smoke
	name = "smoke grenade"
	window_pass = 0
	icon_state = "40mmB"
	damage_type = D_KINETIC
	...
	proc/startSmoke(atom/hit, dirflag, atom/projectile)
		...
		particleMaster.SpawnSystem(new/particleSystem/areaSmoke("#ffffff", smokeLength, trgloc))
		return*/
		...
		if (T && isturf(T))
			var/effects/system/bad_smoke_spread/S = new /effects/system/bad_smoke_spread/(T)
			if (S)
				S.set_up(20, 0, T)
				S.start()
		return
...
/projectile/bullet/glitch
	...
	New()
		..()
		name = pick("weird", "puzzling", "odd", "strange", "baffling", "creepy", "unusual", "confusing", "discombobulating") + " bullet"
		name = corruptText(name, 66)
...
/projectile/bullet/rod // for the coilgun
	name = "metal rod"
	power = 50
	damage_type = D_KINETIC
	hit_type = DAMAGE_STAB
