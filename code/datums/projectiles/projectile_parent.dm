...
/obj/projectile
	...
	proc/collide(atom/A as mob|obj|turf|area)
		...
		if (immunity)
			log_shot(src, A, 1)
			A.visible_message("<strong><span style=\"color:red\">The projectile bounces off of [A] uselessly!</span></strong>")
			die()
			return
		...
		if (ismob(A))
			for (var/obj/item/cloaking_device/S in A.contents)
				if (S.active)
					S.deactivate(A)
					visible_message("<span style=\"color:blue\"><strong>[A]'s cloak is disrupted!</strong></span>")
			for (var/obj/item/device/disguiser/D in A.contents)
				if (D.on)
					D.disrupt(A)
					visible_message("<span style=\"color:blue\"><strong>[A]'s disguiser is disrupted!</strong></span>")
		...
		if (istype(A,/turf))
			...
			if (istype(A, /turf/simulated/wall) && proj_data.icon_turf_hit)
				...
				if (forensic_ID)
					W.forensic_impacts += forensic_ID
	...
	proc/set_icon()
		if (istype(proj_data))
			icon = proj_data.icon
			icon_state = proj_data.icon_state
			if (!proj_data.override_color)
				color = proj_data.color_icon
		else
	...
	proc/setup()
		if (proj_data == null || (xo == 0 && yo == 0) || proj_data.projectile_speed == 0)
			die()
			return
	...
	proc/do_step()
		...
		if (dissipation_ticker > proj_data.dissipation_delay)
			power -= proj_data.dissipation_rate
			if (power <= 0)
				die()
				return
		proj_data.tick(src)
		...
		var/dwx = proj_data.projectile_speed * xo
		var/dwy = proj_data.projectile_speed * yo
		curr_t++
		travelled += proj_data.projectile_speed
...
/projectile/laser/pred
	impact_range = 2

/projectile/laser/light
	impact_range = 2

/projectile/laser/glitter
	impact_range = 4

/projectile/laser/precursor
	impact_range = 4

/projectile/laser/precursor/sphere
	impact_range = 16

/projectile/laser/mining
	impact_range = 12

/projectile/laser/eyebeams
	impact_range = 4

/projectile/laser/drill
	impact_range = 0

/projectile/laser/drill/cutter
	impact_range = 0
...
/projectile/bullet/autocannon
	impact_range = 2

/projectile/bullet/autocannon/plasma_orb
	impact_range = 8

/projectile/bullet/autocannon/huge
	impact_range = 8

/projectile/bullet/glitch
	impact_range = 4

// for the gun, not the drone
/projectile/bullet/glitch/gun
	impact_range = 16

/projectile/bullet/rod
	impact_range = 16

/projectile/bullet/flare/ufo
	impact_range = 8
...
/projectile/energy_bolt_v
	impact_range = 4

/projectile/energy_bolt_antighost
	impact_range = 16
	hits_ghosts = 1 // do it.
...
/proc/hit_with_projectile(var/S, var/projectile/DATA, var/atom/T)
	if (!S || !T)
		return
...
/proc/hit_with_existing_projectile(var/obj/projectile/P, var/atom/T)
	...
	if (ismob(T))
		...
		if (immunity)
			log_shot(P, T, 1)
			T.visible_message("<strong><span style=\"color:red\">...but the projectile bounces off uselessly!</span></strong>")
			P.die()
			return
		P.proj_data.on_pointblank(P, T)
	P.collide(T) // The other immunity check is in there (Convair880).

/proc/shoot_projectile_ST(var/atom/movable/S, var/projectile/DATA, var/T)
	if (!S)
		return
...
/proc/shoot_projectile_ST_pixel(var/atom/movable/S, var/projectile/DATA, var/T, var/pox, var/poy)
	if (!S)
		return
	if (!isturf(S) && !isturf(S.loc))
		return null
...
/proc/shoot_projectile_ST_pixel_spread(var/atom/movable/S, var/projectile/DATA, var/T, var/pox, var/poy, var/spread_angle)
	if (!S)
		return
	if (!isturf(S) && !isturf(S.loc))
		return null
...
/proc/shoot_projectile_DIR(var/atom/movable/S, var/projectile/DATA, var/dir)
	if (!S)
		return
	if (!isturf(S) && !isturf(S.loc))
		return null
...
/proc/shoot_projectile_relay(var/atom/movable/S, var/projectile/DATA, var/T)
	if (!S)
		return
	if (!isturf(S) && !isturf(S.loc))
		return
...
/proc/shoot_projectile_relay_pixel(var/atom/movable/S, var/projectile/DATA, var/T, var/pox, var/poy)
	if (!S)
		return
	if (!isturf(S) && !isturf(S.loc))
		return
...
/proc/shoot_projectile_relay_pixel_spread(var/atom/movable/S, var/projectile/DATA, var/T, var/pox, var/poy, var/spread_angle)
	if (!S)
		return
	if (!isturf(S) && !isturf(S.loc))
		return
...
/proc/shoot_projectile_XY(var/atom/movable/S, var/projectile/DATA, var/xo, var/yo)
	if (!S)
		return
	if (!isturf(S) && !isturf(S.loc))
		return
...
/proc/shoot_projectile_XY_relay(var/atom/movable/S, var/projectile/DATA, var/xo, var/yo)
	if (!S)
		return
	if (!isturf(S) && !isturf(S.loc))
		return
...
/proc/initialize_projectile_ST(var/atom/movable/S, var/projectile/DATA, var/T)
	if (!S)
		return
	if (!isturf(S) && !isturf(S.loc))
		return
...
/proc/initialize_projectile_pixel(var/atom/movable/S, var/projectile/DATA, var/T, var/pox, var/poy)
	if (!S)
		return
	if (!isturf(S) && !isturf(S.loc))
		return
...
/proc/initialize_projectile_pixel_spread(var/atom/movable/S, var/projectile/DATA, var/T, var/pox, var/poy, var/spread_angle)
	var/obj/projectile/P = initialize_projectile_pixel(S, DATA, T, pox, poy)
	if (P && spread_angle)
		if (spread_angle < 0)
...
/proc/initialize_projectile(var/turf/S, var/projectile/DATA, var/xo, var/yo, var/shooter = null)
	if (!S)
		return
...
/proc/shoot_cardinal_normal_reflected(var/obj/projectile/P, var/obj/reflector)
	...
	if (rx == ry && rx == 0)
		logTheThing("debug", null, null, "<strong>Marquesas/Reflecting Projectiles</strong>: Reflection failed for [P.name] (incidence: [P.incidence], direction: [P.xo];[P.yo]).")
