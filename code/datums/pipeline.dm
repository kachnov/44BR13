...
//
	...
	proc/temporarily_store_air()
		...
		for (var/obj/machinery/atmospherics/pipe/member in members)
			...
			if (air.trace_gases && air.trace_gases.len)
				for (var/gas/trace_gas in air.trace_gases)
	...
	proc/network_expand(pipe_network/new_network, obj/machinery/atmospherics/pipe/reference)
	...
	proc/mingle_with_turf(turf/simulated/target, mingle_volume)
		if (!target) return
		var/gas_mixture/air_sample = air.remove_ratio(mingle_volume/air.volume)
		air_sample.volume = mingle_volume

		if (istype(target) && target.parent && target.parent.group_processing)
			//Have to consider preservation of group statuses
			var/gas_mixture/turf_copy = unpool(/gas_mixture)
			...
			var/gas_mixture/turf_air = target.return_air()
