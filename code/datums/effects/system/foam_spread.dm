...
/effects/system/foam_spread/proc/set_up(amt=5, loca, var/reagents/carry = null, var/metalfoam = 0)
	if (!carry)
		return
	...
	if (carry && !metal)
		for (var/reagent_id in carry.reagent_list)
			var/reagent/current_reagent = carry.reagent_list[reagent_id]
			carried_reagents[reagent_id] = current_reagent.volume

/effects/system/foam_spread/proc/start()
	spawn (0)
			...
			if (carried_reagents)
				for (var/id in carried_reagents)
					...
					if (banned_reagents.Find("[id]"))
						continue
					var/reagent/reagent_volume = carried_reagents[id]
