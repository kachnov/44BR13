...
/updateQueueWorker/New(var/list/objects, var/procName, var/list/arguments, var/cpuThreshold = 90)
	..()
	uq_dbg("updateQueueWorker created.")

	init(objects, procName, arguments, cpuThreshold)

/updateQueueWorker/proc/init(var/list/objects, var/procName, var/list/arguments, var/cpuThreshold = 90)
	objects = objects
	procName = procName
	arguments = arguments
...
/updateQueueWorker/proc/finished()
...
#ifdef QUEUE_STAT_DEBUG
/updateQueueWorker/proc/register_subject_time(var/datum/D, var/time)
	if (!D) return
