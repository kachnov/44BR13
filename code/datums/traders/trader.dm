/trader
	...
	New()
		current_message = pick(dialogue_greet)
		patience = rand(base_patience[1],base_patience[2])
		if (max_goods_buy > base_goods_buy.len)
			max_goods_buy = base_goods_buy.len
		if (max_goods_sell > base_goods_sell.len)
			max_goods_sell = base_goods_sell.len
		set_up_goods()
	...
	proc/set_up_goods()
		...
		var/howmanybuy = rand(1,max_goods_buy)
		while (howmanybuy > 0)
			howmanybuy--
			var/the_commodity = pick(goods_buy_temp)
			var/commodity/COM = new the_commodity(src)
			goods_buy += COM
			goods_buy_temp -= the_commodity

		var/howmanysell = rand(1,max_goods_sell)
		while (howmanysell > 0)
			howmanysell--
			var/the_commodity = pick(goods_sell_temp)
			var/commodity/COM = new the_commodity(src)
			if (COM.type == /commodity) logTheThing("debug", src, null, "<strong>SpyGuy/Traders:</strong> [src] got a /commodity when trying to set up stock with [the_commodity]")
			goods_sell += COM
			goods_sell_temp -= the_commodity

	proc/haggle(var/commodity/goods,var/askingprice,var/buying = 0)
		...
		if ((buying && askingprice > goods.price) || (!buying && askingprice < goods.price))
			if (asshole)
				current_message = pick(dialogue_wrong_haggle_accept)
				goods.price = askingprice
				return
			else
				current_message = pick(dialogue_wrong_haggle_reject)
				return

		// check if the price increase % of the haggle is more than this trader will tolerate
		var/adjustedTolerance = hiketolerance
		...
		if ((buying && hikeperc <= negatol) || (!buying && hikeperc >= adjustedTolerance))
			...
			current_message = pick(dialogue_haggle_reject)
			patience--
			if (patience == 0)
				current_message = dialogue_haggle_reject[dialogue_haggle_reject.len]
			return
		...
		if (buying)
			...
			if (middleground-goods.price <= 0.5)
				...
				goods.price = round(middleground - rand(0,negotiate))
		current_message = pick(dialogue_haggle_accept)
		patience--
		// warn the player if the trader isn't going to take any more haggling
		if (patience == 0)
			current_message = dialogue_haggle_accept[dialogue_haggle_accept.len]

	proc/wipe_cart(var/sold_stuff)
		for (var/commodity/trader/incart/COM in shopping_cart)
			if (COM.reference && istype(COM.reference,/commodity))
...
/commodity/trader/
	...
	New()
		...
		if (possible_names.len)
			listed_name = pick(possible_names)
		else
			listed_name = comname
		if (prob(alt_type_chance) && possible_alt_types.len)
			comtype = pick(possible_alt_types)
		price = rand(price_boundary[1],price_boundary[2])

/commodity/trader/incart/
	var/commodity/reference = null