...
/proc/traitCategoryAllowed(var/list/targetList, var/idToCheck)
	...
	if (C.category == null) return TRUE
	for (var/A in targetList)
		var/obj/trait/T = getTraitById(A)
		if (T.category == C.category) return FALSE
	return TRUE
...
/traitPreferences
	...
	proc/isValid()
		...
		for (var/A in traits_selected)
			var/obj/trait/T = getTraitById(A)
			if (T.unselectable) return FALSE
			if (T.category != null)
				if (categories.Find(T.category)) return FALSE
...
/obj/trait/addict
	...
	onLife(var/mob/owner) //Just to be safe.
		if (istype(owner, /mob/living/carbon/human) && prob(1))
			...
			for (var/ailment_data/addiction/A in M.ailments)
				if (istype(A, /ailment_data/addiction))
	...
	proc/addAddiction(var/mob/owner)
		var/mob/living/carbon/human/M = owner
		var/ailment_data/addiction/AD = new
...
/obj/trait/badgenes
	...
	onAdd(var/mob/owner)
		if (owner.bioHolder)
			...
			var/bioEffect/curr = owner.bioHolder.effects[curr_id]
...
*/
			...
			if (target)
				if (!actions.hasAction(owner, "otheritem") && !owner.equipped())
					...
					if (trgSlot)
						actions.start(new/action/bar/icon/otherItem( owner, target, null, trgSlot ) , owner)
			*/