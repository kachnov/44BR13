...
/stockEvent
	...
	product
		name = "product"
		var/product_name = ""
		var/article/product_article = null
		var/effect = 0
		New(var/stock/S)
			company = S
			var/mins = rand(5,20)
		...
		transition()
			...
			switch (phase_id)
				if (0)
					...
					var/article/A = company.industry.generateInCharacterProductArticle(product_name, company)
					product_article = A
	...
	bankruptcy
		...
		transition()
			switch (phase_id)
				if (0)
					next_phase = ticker.round_elapsed_ticks + rand(300, 600) * 10
					var/article/A = generateBankruptcyArticle()
					if (!A.opinion)
		...
		transition()
			switch (phase_id)
				if (0)
					...
					next_phase = ticker.round_elapsed_ticks + rand(300, 600) * 10
					var/article/A = generateArrestArticle()
					if (!A.opinion)
						effect = rand(5) * (prob(50) ? -1 : 1)
					else
...
/stockMarket
	...
	proc/process()
		for (var/stock/S in stocks)
			S.process()


var/global/stockMarket/stockExchange = new
...
/industry
	...
	proc/generateInCharacterProductArticle(var/product_name, var/stock/S)
		var/article/A = new
...
/stock
	...
	proc/addEvent(var/stockEvent/E)
		if (!(E in events))
			events += E

	proc/addArticle(var/article/A)
		if (!(A in articles))
			articles.Insert(1, A)
		A.ticks = ticker.round_elapsed_ticks

	proc/generateEvents()
		var/list/types = subtypesof(/stockEvent)
		for (var/T in types)
			generateEvent(T)

	proc/generateEvent(var/T)
		var/stockEvent/E = new T(src)
		addEvent(E)
	...
	proc/generateIndustry()
		if (findtext(name, "Farms"))
			industry = new /industry/agriculture
			...
			industry = new /industry/communications
		else if (findtext(name, "Pharmaceuticals") || findtext(name, "Health"))
			industry = new /industry/health
		else if (findtext(name, "Wholesale") || findtext(name, "Stores"))
			industry = new /industry/consumer
		else
			var/ts = subtypesof(/industry)
			var/in_t = pick(ts)
			industry = new in_t
...
//			splitStock()
	...
	proc/process()
		for (var/borrow/borrow in borrows)
		...
		if (bankrupt)
			return
		for (var/borrow/borrow in borrow_brokers)
		...
		if (fluctuation_counter >= fluctuation_rate)
			for (var/stockEvent/E in events)
				E.process()
	...
	proc/modifyAccount(whose, by, force=0)
		var/data/record/B = FindBankAccountByName(whose)
		if (B)
	...
	proc/borrow(var/borrow/B, var/who)
		if (B.lease_expires)
			return FALSE
