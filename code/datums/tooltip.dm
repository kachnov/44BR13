...
- Attach the datum to the user client on login, e.g.
	/client/New()
		tooltips = new /tooltip(src)

Usage:
- Define mouse event procs on your (probably HUD) object and simply call the show and hide procs respectively:
	/obj/screen/hud
		MouseEntered(location, control, params)
			usr.client.tooltip.show(src, params, title = name, content = desc)
...
- You may use flags defined below this comment block to tweak the tooltip. For example to align it centered:
	usr.client.tooltip.show(src, params, title = name, content = desc, flags = TOOLTIP_CENTERED)
	...
	New(client/C)
		if (C)
			...
			if (!CDN_ENABLED || config.env == "dev")
				...
				owner.loadResourcesFromList(tooltipResources)

			owner << browse(grabResource(file), "window=[control]")

		..()


	proc/show(atom/movable/thing, params = null, title = null, content = null, theme = "default", special = "none", flags = 0)
		if (!thing || !params || (!title && !content) || !owner || !isnum(flags)) return FALSE
		if (!init)
		...
		if (owner.mob && (istype(owner.mob.loc, /obj/machinery/vehicle/pod_smooth) || istype(owner.mob.loc, /obj/machinery/vehicle/miniputt) || istype(owner.mob.loc, /obj/machinery/colosseum_putt)))
			special = "pod"
		...
		//out(owner, "Params: [params]. Theme: [theme]. Special: [special]") //DEBUG
