...
/arena/colosseumController
	...
	var/gauntletEvent/current_event = null
	...
	proc/finishStaging()
		...
		for (var/mob/living/M in colosseum)
			...
			if (M.client)
				moblist_names += " ([M.client.key])"
		logTheThing("debug", null, null, "<strong>Marquesas/Pod Colosseum</strong>: Starting arena game with players: [moblist_names]")
	...
	New()
		spawn (5)
			...
			for (var/PUP in subtypesof(/obj/colosseum_powerup/stat))
			...
			for (var/PUP in subtypesof(/colosseumSystem))
				var/colosseumSystem/NS = new PUP()
				if (NS.abstract)
					continue
...
var/global/arena/colosseumController/colosseum_controller = new()

/client/proc/debug_colosseum_controller()
	set category = "Debug"
	set name = "Edit Colosseum Controller"

	debug_variables(colosseum_controller)
...
/colosseumIndicator
	...
	proc/assume_weapon(var/colosseumSystem/S)
		if (!istype(S))
			tracking = null
			assembleDefault()
			return
...
/colosseumSystem
	...
	primary
		...
		splitter
			name = "Splitter"
			myProj = /projectile/laser/light/split
			ammo = 35
			abstract = 0
			icon = 'icons/obj/colosseum.dmi'
			icon_state = "splitter"
		...
		asslaser
			rarity_class = 2
			name = "Assault Laser"
			myProj = /projectile/laser/asslaser
			ammo = 20
			abstract = 0
			icon = 'icons/obj/projectiles.dmi'
			icon_state = "u_laser"
		...
		seeker
			name = "Drone Seeker"
			myProj = /projectile/bullet/autocannon/seeker
			ammo = 10
			abstract = 0
			icon = 'icons/obj/colosseum.dmi'
			icon_state = "seeker"
	...
	secondary
		...
		projectile
			...
			artillery
				name = "HE Grenade"
				myProj = /projectile/bullet/autocannon
				ammo = 3
				abstract = 0
				icon = 'icons/obj/projectiles.dmi'
				icon_state = "400mm"
			...
			hunting_rifle
				name = "30.06 Shots"
				myProj = /projectile/bullet/rifle_3006
				ammo = 6
				abstract = 0
				icon = 'icons/obj/gun.dmi'
...
/obj/machinery/colosseum_putt
	...
	var/speed = 2
	var/facing = 2
	var/projectile/laser/light/upgradeable/simple
	var/colosseumSystem/primary/primary
	var/colosseumSystem/secondary/secondary
	var/colosseumIndicator/primary/p_indicator
	var/colosseumIndicator/secondary/s_indicator
	var/colosseumIndicator/armor_indicator
	var/colosseumIndicator/fireres_indicator
	var/colosseumIndicator/shieldgen_indicator
	var/colosseumIndicator/shotcount_indicator
	var/colosseumIndicator/shotdamage_indicator
	...
	New()
		...
		help = new()
		simple = new /projectile/laser/light/upgradeable()
		p_indicator = new()
		s_indicator = new()
		armor_indicator = new()
	...
	proc/shoot_projectile(var/projectile/Pr)
	...
	proc/message_pilot(var/message, var/type = 0)
		if (piloting)
			switch (type)
				...
				if (1)
					boutput(piloting, "<span style=\"color:red\"><strong>[message]</strong></span>")
				else
					boutput(piloting, message)

	proc/activate_system(var/colosseumSystem/S)
		if (S.slot)
	...
	attackby(obj/item/W as obj, mob/living/user as mob)
		if (istype(W, /obj/item/weldingtool))
			...
			if (T.welding && T.get_fuel() >= 1)
				T.use_fuel(1)
				visible_message("<span style=\"color:blue\"><strong>[user]</strong> repairs some dents on [src]!</span>")
				message_pilot("<strong>[user]</strong> repairs some dents on [src]!")
				repair_by(10)
...
/obj/colosseum_powerup
	...
	system
		var/colosseumSystem/system

		New(var/L, var/is_template, var/systemType)
			..()
			if (!systemType)
				systemType = pick(typesof(/colosseumSystem) - list(/colosseumSystem, /colosseumSystem/primary, /colosseumSystem/secondary, /colosseumSystem/secondary/projectile))
