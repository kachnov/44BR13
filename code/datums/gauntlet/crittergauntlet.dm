...
/arena/gauntletController
	...
	var/gauntletWave/fallback/fallback
	...
	proc/beginStaging()
		...
		players = mobcount
		current_level = 1
		for (var/gauntletDrop/D in possible_drops)
			D.used = 0
		current_match_id++
	...
	proc/finishStaging()
		...
		spawn (0)
			...
			for (var/mob/living/M in gauntlet)
				...
				if (M.client)
					moblist_names += " ([M.client.key])"
			logTheThing("debug", null, null, "<strong>Marquesas/Critter Gauntlet</strong>: Starting arena game with players: [moblist_names]")
	...
	process()
		if (state == 2)
			...
			if (current_waves.len)
				if (waiting <= 0)
					var/gauntletWave/wave = current_waves[1]
	...
	New()
		spawn (5)
			...
			for (var/tp in subtypesof(/gauntletEvent))
				possible_events += new tp()

			for (var/tp in subtypesof(/gauntletDrop))
				possible_drops += new tp()

			for (var/tp in typesof(/gauntletWave) - list(/gauntletWave, /gauntletWave/fallback))
				possible_waves += new tp()

			fallback = new()

	proc/dropIsPossible(var/gauntletDrop/drop, var/points)
		if (drop.used)
			return FALSE
	...
	proc/startWave()
		...
		logTheThing("debug", null, null, "<strong>Marquesas/Critter Gauntlet:</strong> On level [current_level]. Spending [points] points, composed of 1 base, [round(current_level * 0.1) * 1.5] major and [(current_level % 10) / 20] minor.")

		var/gauntletEvent/candidate = pick(possible_events)
		...
		var/gauntletDrop/drop = pick(possible_drops)
		var/retries = 0
		...
		for (var/i = 1, i <= waves_this_level, i++)
			...
			while (choices.len)
				var/gauntletWave/wave = pick(choices)
				choices -= wave
	...
	proc/applyDifficulty(var/gauntletWave/wave)
				...
				if (current_waves.len)
					...
					if (current_level < 50)
						for (var/i = 1, i <= current_waves.len, i++)
							var/gauntletWave/W = current_waves[i]
							stat(null, "- [W.name]")
...
var/global/arena/gauntletController/gauntlet_controller = new()
...
/gauntletWave
	...
	proc/spawnIn(var/gauntletEvent/ev)
		if (count)
			var/turf/T = pick(gauntlet_controller.spawnturfs)
...
/proc/queryGauntletMatches(phase, data)
	if (phase == 1)
		...
		logTheThing("<strong>Marquesas/Gauntlet Query:</strong> Invoked (data is [data])")
		var/list/ldata = data
		for (var/userkey in ldata)
			logTheThing("debug", null, null, "<strong>Marquesas/Gauntlet Query:</strong> Got key [userkey].")
			var/matches = ldata[userkey]
			logTheThing("debug", null, null, "<strong>Marquesas/Gauntlet Query:</strong> Matches for [userkey]: [matches].")
			var/obj/item/card/id/gauntlet/G = locate("gauntlet-id-[userkey]") in world
			if (G && istype(G))
				G.SetMatchCount(text2num(matches))
			else
				logTheThing("debug", null, null, "<strong>Marquesas/Gauntlet Query:</strong> Could not locate ID 'gauntlet-id-[userkey]'.")
				return TRUE
