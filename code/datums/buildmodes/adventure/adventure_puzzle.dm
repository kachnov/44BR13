/adventure_submode/wizard
	name = "Adventure Element Wizard"
	var/puzzlewizard/wizard = null
	...
	proc/build_wizard_cache()
		if (!wizards.len)
			for (var/T in subtypesof(/puzzlewizard))
				var/puzzlewizard/W = new T()
				wizards[W.name] = T
...
If weird things happen, you know where to find me.</span>"});
	...
	settings(var/ctrl, var/alt, var/shift)
		var/kind = input(usr, "What kind of puzzle element would you like to place?", "Wizard chooser", "cancel") as null|anything in wizards
		if (!kind)
			return
		var/wizardtype = wizards[kind]
