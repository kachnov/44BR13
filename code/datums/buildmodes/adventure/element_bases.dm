/puzzlewizard
	...
	proc/build_click(var/mob/user, var/buildmode_holder/holder, pa, var/atom/object)
		return
...
/obj/adventurepuzzle/triggerable/triggerer
	...
	proc/post_trigger()
		for (var/obj/adventurepuzzle/triggerable/T in triggered)
			var/act = triggered[T]
			spawn (0)
				T.trigger(act)
...
/obj/adventurepuzzle/triggerer/twostate
	...
	proc/post_untrigger()
		for (var/obj/adventurepuzzle/triggerable/T in triggered_unpress)
			var/act = triggered_unpress[T]
			spawn (0)
				T.trigger(act)
	...
	special_trigger_set(var/obj/adventurepuzzle/triggerable/T, var/list/toset)
		if (T in triggered_unpress)
			triggered_unpress[T] = toset[1]
		else
			triggered_unpress += T
			triggered_unpress[T] = toset[1]
	...
	special_trigger_input(var/obj/adventurepuzzle/triggerable/T)
		if (T in triggered_unpress)
			...
			triggered_unpress[T] = acts[newname]
		else
			var/act = triggered_unpress[T]
			var/list/acts = T.trigger_actions()
