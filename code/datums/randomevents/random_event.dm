/random_event
	...
	proc/event_effect(var/source)
		...
		if (announce_to_admins)
			message_admins("<span style=\"color:blue\">Beginning [name] event (Source: [source]).</span>")
			logTheThing("admin", null, null, "Random event [name] was triggered. Source: [source]")
	...
	proc/is_event_available()
		...
		if (timer < required_elapsed_round_time && random_events.time_lock)
			return FALSE

		if (wont_occur_past_this_time > -1)
			if (timer > wont_occur_past_this_time && random_events.time_lock)
				return FALSE
