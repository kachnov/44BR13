	...
	event_effect(var/source)
		...
		for (var/mob/living/silicon/ai/M in mobs)
			if (M.stat != 2 && M.see_in_dark != 0)
				boutput(M, "<span style=\"color:red\"><strong>PROGRAM EXCEPTION AT 0x30FC50B</strong></span>")
				boutput(M, "<span style=\"color:red\"><strong>Law ROM data corrupted. Attempting to restore...</strong></span>")
		switch(rand(1,100))
			if (1 to 5)
