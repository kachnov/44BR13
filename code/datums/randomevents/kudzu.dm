...
/obj/spacevine/proc/Life()
	if (!src) return
	var/Vspread
	if (prob(50)) Vspread = locate(x + rand(-1,1),y,z)
	else Vspread = locate(x,y + rand(-1, 1),z)
	var/dogrowth = 1
	...
	growth += 1
	if (growth == 10)
		name = "Thick Space Kudzu"
		icon_state = pick("vine-med1", "vine-med2", "vine-med3")
		opacity = 1
		waittime = 80
	if (growth == 20)
		name = "Dense Space Kudzu"
		icon_state = pick("vine-hvy1", "vine-hvy2", "vine-hvy3")
		density = 1
	spawn (waittime)
		if (growth < 20) Life()
