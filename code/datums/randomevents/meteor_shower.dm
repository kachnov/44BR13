...
/random_event/major/meteor_shower
	name = "Meteor Shower"
	...
	admin_call(var/source)
		...
		var/amtinput = input(usr,"How many meteors?",name) as num|null
		if (!isnum(amtinput) || amtinput < 1)
			return
		var/delinput = input(usr,"Tick delay between meteors? (10 = 1 second)",name) as num|null
		if (!isnum(delinput) || delinput < 1)
			return
		var/dirinput = input(usr,"Which direction should the meteors come from?",name) as null|anything in list("north","south","east","west")
		...
		switch(dirinput)
			...
			if ("east") dirinput = 3
			if ("west") dirinput = 4
		var/timinput = input(usr,"How many ticks between the warning and the event? (10 = 1 second)",name) as num|null
		if (!isnum(timinput) || timinput < 1)
			return
		var/spdinput = input(usr,"How fast do the meteors move?",name) as num|null
...
/obj/newmeteor
	...
	Bump(atom/A)
		spawn (0)
			if (A)
				A.meteorhit(src)
				if (sound_impact)
					playsound(loc, sound_impact, 40, 1)
			if (--hits <= 0)
				if (istype(A, /obj/forcefield)) explodes = 0
				shatter()

		return

	Move(atom/NewLoc,Dir)
		if (x == world.maxx || y == world.maxy || x == 1 || y == 1)
			qdel(src)
		if (loc == target)
			shatter()
			return
	...
	proc/shatter()
		playsound(loc, sound_explode, 50, 1)
		if (explodes)
			spawn (1)
