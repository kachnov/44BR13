/mind
	...
	var/list/objective/objectives = list()
	var/is_target = 0
	var/list/purchased_traitor_items = list()
	var/list/traitor_crate_items = list()

	var/gang/gang = null //Associate a leader with their gang.

	//Ability holders.
	var/abilityHolder/changeling/is_changeling = 0
	var/abilityHolder/wizard/is_wizard = 0
	var/abilityHolder/vampire/is_vampire = 0
	...
	New(mob/M)
		...
		if (M)
			current = M
			key = M.key
			handwriting = pick(handwriting_styles)

	proc/transfer_to(mob/new_character)
		if (!new_character)
			return
			
		// this has to be here, and not new_player/Logout(), for some stupid reason
		if (isnewplayer(current) && current.client)
			current.client << sound(null, channel = LOBBY_MUSIC_CHANNEL)
	...
	proc/swap_with(mob/target)
		var/mind/other_mind = target.mind
		...
		if (other_mind)	//They have a mind so we can do this nicely
			...
			var/mob/temp = new/mob(current.loc) //We need to put whoever we're swapping with somewhere
			other_mind.transfer_to(temp)			//So now we put them there
			transfer_to(target)					//Then I go into their head
	...
	proc/show_memory(mob/recipient)
		var/output = "<strong>[current.real_name]'s Memory</strong><HR>"
		output += memory

		if (objectives.len>0)
			output += "<HR><strong>Objectives:</strong>"

			var/obj_count = 1
			for (var/objective/objective in objectives)
				output += "<strong>Objective #[obj_count]</strong>: [objective.explanation_text]<br>"
				obj_count++
		...
		if (recipient.mind.master)
			...
			if (mymaster)
				output+= "<br><strong>Your master:</strong> [mymaster.real_name]"
	...
	proc/show_miranda(mob/recipient)
		var/output = "<strong>[current.real_name]'s Miranda Rights</strong><HR>[miranda]"

		recipient << browse(output,"window=miranda")

	Del()
		logTheThing("debug", null, null, "<strong>Mind</strong> Mind for \[[key ? key : "NO KEY"]] deleted!")
		..()