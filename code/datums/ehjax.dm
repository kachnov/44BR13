...
var/global/ehjax/ehjax = new /ehjax()
...
/ehjax
	...
	var/list/allowedProcs = list("/proc/getBanApiFallback",
								...
								"/chatOutput/proc/debug",
								"/chatOutput/proc/ping",
								"/chatOutput/proc/handleContextMenu",
								"/chatOutput/proc/analyzeClientData",
							)
	...
	proc
		...
		topic(type, href_list, client/C)
			switch(type)
				 ...
				 */
				if ("main")
					if (!href_list["proc"]) return FALSE
					var/windowName
					if (href_list["window"])
					...
					var/fullPath = ""
					if (href_list["datum"])
						fullPath = "/[href_list["datum"]]"
					fullPath += "/proc/[theProc]"

					if (!(fullPath in allowedProcs))
