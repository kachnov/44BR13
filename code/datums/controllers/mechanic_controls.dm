var/mechanic_controller/mechanic_controls = null
...
/mechanic_controller
	...
	proc
		scan_in(var/N,var/T,var/M)
			var/electronics/scanned_item/S = new/electronics/scanned_item
			S.name = N
			S.item_type = T
			S.item_mats = M
...
/electronics/scanned_item
	var
		name = "Unknown"
		item_type = ""
		var/list/item_mats
		var/finish_time = 0
		var/manufacture/mechanics/blueprint = null
	...
	proc
		...
		create_blueprint(var/mats_number = 10)
			if (istype(blueprint,/manufacture/mechanics))
				return
			if (get_schematic_from_name_in_custom(name))
				return
			if (!isnum(mats_number))
				mats_number = 10

			var/manufacture/mechanics/M = new /manufacture/mechanics(manuf_controls)
			manuf_controls.custom_schematics += M
			M.name = name
			M.time = mats_number * 2
			M.frame_path = item_type
