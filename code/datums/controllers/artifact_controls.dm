var/artifact_controller/artifact_controls = null
...
/artifact_controller
	...
	New()
		..()
		for (var/X in subtypesof(/artifact) - /artifact/art)
			var/artifact/A = new X
			artifact_types += A

		for (var/X in subtypesof(/artifact_origin))
			var/artifact_origin/AO = new X
			artifact_origins += AO

	proc/get_origin_from_string(var/string)
		if (!istext(string))
			return
		for (var/artifact_origin/AO in artifact_origins)
			if (AO.name == string)
				return AO
		return null
	...
	proc/config()
		...
		dat += "<strong><u>Artifact Controls</u></strong><HR><small>"
		dat += "<a href='byond://?src=\ref[src];Spawnnew=1'>Spawn a new Artifact on your current tile</strong></a><br>"
	...
	Topic(href, href_list[])
		if (href_list["Activate"])
			var/obj/O = locate(href_list["Activate"]) in artifacts
			if (!istype(O,/obj))
				return
			if (!istype(O.artifact,/artifact))
				return
			...
			log_me(usr, O, A.activated ? "activates" : "deactivates", 1)

		else if (href_list["Jumpto"])
			var/obj/O = locate(href_list["Jumpto"]) in artifacts
			if (!istype(O,/obj))
				return
			var/turf/T = O.loc
			usr.set_loc(T)

			log_me(usr, O, "jumps to", 0)

		else if (href_list["Get"])
			var/obj/O = locate(href_list["Get"]) in artifacts
			if (!istype(O,/obj))
				return
			var/turf/T = usr.loc
			O.set_loc(T)

			log_me(usr, O, "teleports", 0)

		else if (href_list["Destroy"])
			var/obj/O = locate(href_list["Destroy"]) in artifacts
			if (!istype(O,/obj))
				return
			if (!istype(O.artifact,/artifact))
				return

			log_me(usr, O, "destroys", 1)
			O.ArtifactDestroyed()
...
/artifact_origin
	...
	New()
		...
		if ("all" in fault_types)
			fault_types += subtypesof(/artifact_fault)

	proc/generate_name()
		return "unknown object"

/artifact_origin/ancient
	name = "ancient"
	fault_types = list(/artifact_fault/burn,/artifact_fault/irradiate,/artifact_fault/shutdown,
	/artifact_fault/murder,/artifact_fault/zap)
...
/artifact_origin/martian
	name = "martian"
	fault_types = list(/artifact_fault/shutdown,/artifact_fault/zap,/artifact_fault/poison)
...
/artifact_origin/wizard
	name = "wizard"
	fault_types = list(/artifact_fault/irradiate,/artifact_fault/shutdown,/artifact_fault/murder,
	/artifact_fault/warp,/artifact_fault/zap,/artifact_fault/messager/creepy_whispers)
