var/mining_controller/mining_controls = null
...
/mining_controller
	...
	New()
		..()
		for (var/X in subtypesof(/ore) - /ore/event)
			var/ore/O = new X
			ore_types_common += O

		for (var/X in subtypesof(/mining_encounter))
			var/mining_encounter/MC = new X
			mining_encounters_common += MC

		for (var/ore/O in ore_types_common)
			if (istype(O, /ore/event))
				events += O
				ore_types_common -= O
			if (O.rarity_tier == 2)
		...
		for (var/mining_encounter/MC in mining_encounters_common)
			if (MC.rarity_tier == 3)
	...
	proc/get_ore_from_string(var/string)
		if (!istext(string))
			return
		for (var/ore/O in ore_types_common + ore_types_uncommon + ore_types_rare)
			if (O.name == string)
				return O
		return null
...
/area/mining/magnet
	...
	proc/check_for_unacceptable_content()
		for (var/mob/living/L in contents)
			return TRUE
		for (var/obj/machinery/vehicle in contents)
			return TRUE
		return FALSE
