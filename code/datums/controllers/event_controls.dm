var/event_controller/random_events = null
...
/event_controller/New()
	for (var/X in subtypesof(/random_event/major))
		var/random_event/RE = new X
		events += RE

	for (var/X in subtypesof(/random_event/minor))
		var/random_event/RE = new X
		minor_events += RE

	for (var/X in subtypesof(/random_event/special))
		var/random_event/RE = new X
		special_events += RE

/event_controller/proc/event_cycle()
	event_cycle_count++
	var/num_players = 0
...
/event_controller/proc/minor_event_cycle()
...
/event_controller/proc/do_random_event(var/list/event_bank)
	if (!event_bank || event_bank.len < 1)
		logTheThing("debug", null, null, "<strong>Random Events:</strong> do_random_event proc was passed a bad event bank")
		return
	var/list/eligible = list()
	for (var/random_event/RE in event_bank)
		if (!RE.is_event_available())
			continue
		eligible += RE
	if (eligible.len > 0)
		var/random_event/this = pick(eligible)
		this.event_effect()
	else
		logTheThing("debug", null, null, "<strong>Random Events:</strong> do_random_event couldn't find any eligible events")

/event_controller/proc/force_event(var/string,var/reason)
	if (!string)
		return
	if (!reason)
	...
	for (var/random_event/RE in allevents)
		if (RE.name == string)
...
/event_controller/proc/event_config()
	var/dat = "<html><body><title>Random Events Controller</title>"
	dat += "<strong><u>Random Event Controls</u></strong><HR>"

	if (ticker.current_state == GAME_STATE_PREGAME)
		dat += "<strong>Random Events begin at: <a href='byond://?src=\ref[src];EventBegin=1'>[round(events_begin / 600)] minutes</a><br>"
		dat += "<strong>Minor Events begin at: <a href='byond://?src=\ref[src];MEventBegin=1'>[round(minor_events_begin / 600)] minutes</a><br>"
	else
	...
	dat += "<strong><a href='byond://?src=\ref[src];EnableEvents=1'>Random Events Enabled:</a></strong> [events_enabled ? "Yes" : "No"]<br>"
	dat += "<strong><a href='byond://?src=\ref[src];EnableMEvents=1'>Minor Events Enabled:</a></strong> [minor_events_enabled ? "Yes" : "No"]<br>"
	dat += "<strong><a href='byond://?src=\ref[src];AnnounceEvents=1'>Announce Events to Station:</a></strong> [announce_events ? "Yes" : "No"]<br>"
	dat += "<strong><a href='byond://?src=\ref[src];TimeLocks=1'>Time Locking:</a></strong> [time_lock ? "Yes" : "No"]<br>"
	dat += "<strong>Minimum Population for Events: <a href='byond://?src=\ref[src];MinPop=1'>[minimum_population] players</a><br>"
	dat += "<strong>Time Between Events:</strong> <a href='byond://?src=\ref[src];TimeLower=1'>[round(time_between_events_lower / 600)]m</a> /"
	dat += " <a href='byond://?src=\ref[src];TimeUpper=1'>[round(time_between_events_upper / 600)]m</a><br>"
	dat += "<strong>Time Between Minor Events:</strong> <a href='byond://?src=\ref[src];MTimeLower=1'>[round(time_between_minor_events_lower / 600)]m</a> /"
	dat += " <a href='byond://?src=\ref[src];MTimeUpper=1'>[round(time_between_minor_events_upper / 600)]m</a>"
	dat += "<HR>"

	dat += "<strong><u>Normal Random Events</u></strong><BR>"
	for (var/random_event/RE in events)
		dat += "<a href='byond://?src=\ref[src];TriggerEvent=\ref[RE]'><strong>[RE.name]</strong></a>"
	...
	dat += "<strong><u>Minor Random Events</u></strong><BR>"
	for (var/random_event/RE in minor_events)
		dat += "<a href='byond://?src=\ref[src];TriggerMEvent=\ref[RE]'><strong>[RE.name]</strong></a>"
	...
	dat += "<strong><u>Gimmick Events</u></strong><BR>"
	for (var/random_event/RE in special_events)
		dat += "<a href='byond://?src=\ref[src];TriggerSEvent=\ref[RE]'><strong>[RE.name]</strong></a><br>"

	dat += "<HR>"
	dat += "</body></html>"
	usr << browse(dat,"window=reconfig;size=450x450")

/event_controller/Topic(href, href_list[])

	if (href_list["TriggerEvent"])
		var/random_event/RE = locate(href_list["TriggerEvent"]) in events
		if (!istype(RE,/random_event))
			return
		...
		var/random_event/RE = locate(href_list["TriggerMEvent"]) in minor_events
		if (!istype(RE,/random_event))
			return
		var/choice = alert("Trigger a [RE.name] event?","Random Events","Yes","No")
		if (choice == "Yes")
		...
		var/random_event/RE = locate(href_list["TriggerSEvent"]) in special_events
		if (!istype(RE,/random_event))
			return
		var/choice = alert("Trigger a [RE.name] event?","Random Events","Yes","No")
		if (choice == "Yes")
		...
		var/random_event/RE = locate(href_list["DisableEvent"]) in events
		if (!istype(RE,/random_event))
			return
		...
		var/random_event/RE = locate(href_list["DisableMEvent"]) in minor_events
		if (!istype(RE,/random_event))
			return
		RE.disabled = !RE.disabled
		message_admins("Admin [key_name(usr)] switched [RE.name] event [RE.disabled ? "Off" : "On"]")
