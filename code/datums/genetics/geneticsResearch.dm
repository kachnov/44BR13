var/geneticsResearchManager/genResearch = new()

/geneticsResearchManager
	...
	proc/setup()
		researchTree = (subtypesof(/geneticsResearchEntry)) - /geneticsResearchEntry/mutation

		for (var/entry in researchTree)
			researchTree[entry] = new entry()
			var/geneticsResearchEntry/newEntry = researchTree[entry]
		...
		combinationrecipes = (subtypesof(/geneticsrecipe))
		for (var/X in combinationrecipes)
			var/geneticsrecipe/GR = new X (src)
	...
	proc/isResearched(var/type)
		...
		if (researchTree.Find(type))
			var/geneticsResearchEntry/E = researchTree[type]
			if (E.isResearched == 1)
				return TRUE
		return FALSE
	...
	proc/progress()
		...
		for (var/geneticsResearchEntry/entry in currentResearch)
			entry.onTick()
	...
	proc/addResearch(var/datum/D)
		if (istype(D, /bioEffect))
			var/geneticsResearchEntry/mutation/M = new()
		...
		else if (istype(D, /geneticsResearchEntry))
			var/geneticsResearchEntry/R = D
	...
	proc/checkClonepodBonus()
		...
		for (var/obj/machinery/clonepod/CP in clonepods)
...
/geneticsResearchEntry
	...
	proc/meetsRequirements()
		...
		if (genResearch.mutations_researched < requiredTotalMutRes)
			// Do we have the neccecary # of muts researched?
			return FALSE

		return TRUE

/geneticsResearchEntry/mutation
	var/mutation_id = null
	var/bioEffect/global_instance = null
...
/geneticsResearchEntry/rademitter
...
/geneticsResearchEntry/improvedmutres
...
/geneticsResearchEntry/improvedcooldowns
...
/geneticsResearchEntry/fqresearch_complex
...
/geneticsResearchEntry/complex_saver
...
/geneticsResearchEntry/complex_max_materials
...
/geneticsResearchEntry/reclaimer
	name = "DNA Reclaimer"
	...
	researchCost = 100
	tier = 2
	requiredResearch = list(/geneticsResearchEntry/checker)

/geneticsResearchEntry/injector
	name = "DNA Injectors"
...
/geneticsResearchEntry/saver
	...
	researchCost = 100
	tier = 3
	requiredResearch = list(/geneticsResearchEntry/reclaimer)

	onFinish()
		..()
		genResearch.max_save_slots += 3

/geneticsResearchEntry/rad_precision
	...
	researchCost = 150
	tier = 3
	requiredResearch = list(/geneticsResearchEntry/rad_dampers,/geneticsResearchEntry/rad_coolant)

/geneticsResearchEntry/extra_max_materials
...
/geneticsResearchEntry/bio_rad_dampers
	...
	researchCost = 100
	tier = 3
	requiredResearch = list(/geneticsResearchEntry/rad_dampers)
...
/geneticsResearchEntry/saver_slots
	name = "Expanded Mutation Storage"
	desc = "Adds two additional saving slots for mutations."
	researchTime = 4500
	researchCost = 120
	tier = 4
	requiredResearch = list(/geneticsResearchEntry/saver)
...
/proc/create_new_dna_sample_file(var/mob/living/carbon/C)
	...
	var/computer/file/genetics_scan/scan = new /computer/file/genetics_scan()
	scan.subject_name = C.real_name
	scan.subject_uID = C.bioHolder.Uid

	for (var/ID in C.bioHolder.effectPool)
		var/bioEffect/BE = C.bioHolder.GetEffectFromPool(ID)
		var/bioEffect/MUT = new BE.type(scan)
