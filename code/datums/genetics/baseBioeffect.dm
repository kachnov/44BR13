...
/bioEffect
	...
	var/bioEffect/global_instance = null // bioeffectlist version of this effect
	...
	var/bioHolder/holder = null //Holder that contains this effect.
	...
	New(var/for_global_list = 0)
		if (!for_global_list)
			global_instance = bioEffectList[id]
		dnaBlocks = new/dnaBlocks(src)
		return ..()
...
/dnaBlocks
	...
	proc/sequenceCorrect()
		...
		for (var/i=0, i < blockList.len, i++)
			var/basePair/correct = blockList[i+1]
			var/basePair/current = blockListCurr[i+1]
	...
	proc/pairCorrect(var/pair_index)
		if (blockList.len != blockListCurr.len || !pair_index)
			return FALSE
		var/basePair/correct = blockList[pair_index]
		var/basePair/current = blockListCurr[pair_index]
		if (correct.bpp1 != current.bpp1 || correct.bpp2 != current.bpp2) //NOPE
			return FALSE
		return TRUE

	proc/ModBlocks() //Gets the normal sequence for this mutation and then "corrupts" it locally.
		for (var/basePair/bp in blockList)
			var/basePair/bpNew = new()
			bpNew.bpp1 = bp.bpp1
			bpNew.bpp2 = bp.bpp2
			blockListCurr.Add(bpNew)

		for (var/basePair/bp in blockListCurr)
			if (prob(33))
				if (prob(50))
					bp.bpp1 = "X"
				else
		...
		for (var/i=0, i<owner.blockGaps, i++)
			var/basePair/bp = pick(blockListCurr - gapList)
			gapList.Add(bp)
			bp.bpp1 = "X"
			bp.bpp2 = "X"
...
/obj/screen/ability/topBar/genetics
	...
	get_controlling_mob()
		if (!istype(owner,/targetable/geneticsAbility))
			return null
		var/targetable/geneticsAbility/GA = owner
		var/mob/M = GA.owner
		if (!istype(M) || !M.client)
			return null
		return M

/targetable/geneticsAbility
	...
	target_anything = 1
	var/can_act_check = 1
	var/bioEffect/power/linked_power = null
