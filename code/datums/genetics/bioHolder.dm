...
var/list/bioEffect/bioEffectList = list()
...
/appearanceHolder
	...
	proc/CopyOther(var/appearanceHolder/toCopy)
	...
	// I'm help -Spy
	proc/StaggeredCopyOther(var/appearanceHolder/toCopy, var/progress = 1)
...
/bioHolder
	...
	var/appearanceHolder/mobAppearance = null
	...
	New(var/mob/owneri)
		...
		bioUids.Add(Uid)
		bioUids[Uid] = owner
		mobAppearance = new/appearanceHolder()
	...
	proc/ActivatePoolEffect(var/bioEffect/E, var/overrideDNA = 0, var/grant_research = 1)
	...
	proc/AddNewPoolEffect(var/idToAdd)
		...
		var/bioEffect/newEffect = bioEffectList[idToAdd]
	...
	proc/AddRandomNewPoolEffect()
		...
		if (!bioEffectList || !bioEffectList.len)
			logTheThing("debug", null, null, {"<strong>Genetics:</strong> Tried to add new random effect to pool for
			 [owner ? "\ref[owner] [owner.name]" : "*NULL*"], but bioEffectList is empty!"})
			return FALSE

		for (var/T in bioEffectList)
			var/bioEffect/instance = bioEffectList[T]
		...
		if (!filteredList.len)
			logTheThing("debug", null, null, {"<strong>Genetics:</strong> Unable to get effects for new random effect for
			 [owner ? "\ref[owner] [owner.name]" : "*NULL*"]. (filteredList.len = [filteredList.len])"})
			return FALSE

		var/bioEffect/selectedG = pickweight(filteredList)
		var/bioEffect/selectedNew = selectedG.GetCopy()
		selectedNew.dnaBlocks.ModBlocks() //Corrupt the local copy
		selectedNew.holder = src
		selectedNew.owner = owner
		effectPool[selectedNew.id] = selectedNew
		return TRUE

	proc/RemovePoolEffect(var/bioEffect/E)
		if (!effectPool[E.id]) return FALSE
	...
	proc/BuildEffectPool()
		...
		if (!bioEffectList || !bioEffectList.len)
			logTheThing("debug", null, null, {"<strong>Genetics:</strong> Tried to build effect pool for
			 [owner ? "\ref[owner] [owner.name]" : "*NULL*"], but bioEffectList is empty!"})
		...
		if (!filteredGood.len || !filteredBad.len)
			logTheThing("debug", null, null, {"<strong>Genetics:</strong> Unable to build effect pool for
			 [owner ? "\ref[owner] [owner.name]" : "*NULL*"]. (filteredGood.len = [filteredGood.len],
			  filteredBad.len = [filteredBad.len])"})
			return

		for (var/g=0, g<5, g++)
			var/bioEffect/selectedG = pickweight(filteredGood)
			if (selectedG)
				var/bioEffect/selectedNew = selectedG.GetCopy()
				selectedNew.dnaBlocks.ModBlocks() //Corrupt the local copy
				selectedNew.holder = src
		...
		for (var/b=0, b<5, b++)
			var/bioEffect/selectedB = pickweight(filteredBad)
			if (selectedB)
				var/bioEffect/selectedNew = selectedB.GetCopy()
				selectedNew.dnaBlocks.ModBlocks() //Corrupt the local copy
				selectedNew.holder = src
				selectedNew.owner = owner
				effectPool[selectedNew.id] = selectedNew
		...
		if (filteredSecret.len)
			var/bioEffect/selectedS = pickweight(filteredSecret)
			var/bioEffect/selectedNew = selectedS.GetCopy()
			selectedNew.dnaBlocks.ModBlocks() //Corrupt the local copy
			selectedNew.holder = src
			selectedNew.owner = owner
			effectPool[selectedNew.id] = selectedNew
	...
	proc/CopyOther(var/bioHolder/toCopy, var/copyAppearance = 1, var/copyPool = 1, var/copyEffectBlocks = 0, var/copyActiveEffects = 1)
		...
		if (copyActiveEffects)
			...
			for (var/curr in toCopy.effects)
				...
				if (HasEffect(BE.id))
					var/bioEffect/newCopy = GetEffect(BE.id)
					if (!newCopy) continue
					...
					var/bioEffect/newCopy = AddEffect(BE.id)
					if (!newCopy) continue
	...
	proc/StaggeredCopyOther(var/bioHolder/toCopy, progress = 1)
		if (progress >= 10)
	...
	proc/AddEffect(var/idToAdd, var/variant = 0, var/timeleft = 0)
		...
		if (istype(newEffect))
			for (var/bioEffect/curr_id in effects)
				var/bioEffect/curr = effects[curr_id]
			...
			newEffect.holder = src
			newEffect.OnAdd()
			genetic_stability -= newEffect.stability_loss
			genetic_stability = max(0,min(genetic_stability,100))
	...
	proc/AddEffectInstance(var/bioEffect/BE,var/do_delay = 0)
		...
		BE.holder = src
		BE.OnAdd()
		genetic_stability -= BE.stability_loss
		genetic_stability = max(0,min(genetic_stability,100))
	...
	proc/RemoveEffect(var/id)
		...
		var/bioEffect/D = effects[id]
		if (D)
			D.OnRemove()
			if (!D.activated_from_pool)
				genetic_stability += D.stability_loss
				genetic_stability = max(0,min(genetic_stability,100))
	...
	proc/HasOneOfTheseEffects() //HasAnyEffect() was already taken :I
		...
		if (temp.len)
			var/bioEffect/BE = effects[temp[1]]
	...
	proc/HasAllOfTheseEffects()
		...
		for (var/bioEffect/D in effects)
...
/proc/GetBioeffectResearchLevelFromGlobalListByID(var/id)
	if (!istext(id))
		return FALSE
	var/bioEffect/BE = bioEffectList[id]
