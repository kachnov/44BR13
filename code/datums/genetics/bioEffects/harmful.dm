...
/bioEffect/epilepsy
	...
	OnLife()
		...
		if (prob(1) && owner:paralysis < 1)
			owner:visible_message("<span style=\"color:red\"><strong>[owner] starts having a seizure!</span>", "<span style=\"color:red\">You have a seizure!</span>")
...
/bioEffect/funky_limb
	...
	OnAdd()
		...
		limb = pick(possible_limbs)

		if (istype(limb, /obj/item/parts/human_parts/arm) || istype(limb, /obj/item/parts/robot_parts/arm))
			limb_type = LIMB_IS_ARM
			return
		else if (istype(limb, /obj/item/parts/human_parts/leg) || istype(limb, /obj/item/parts/robot_parts/leg))
			limb_type = LIMB_IS_LEG
			return

	OnLife()
		..()
		if (!limb || (limb.loc != owner))
			return
		if (owner.stat)
			return

		if (limb_type == LIMB_IS_ARM)
			if (prob(5))
				owner.visible_message("<span style=\"color:red\">[owner.name]'s [limb] makes a [pick("rude", "funny", "weird", "lewd", "strange", "offensive", "cruel", "furious")] gesture!</span>")
			else if (prob(2))
				owner.emote("slap")
			else if (prob(2))
				owner.visible_message("<span style=\"color:red\"><strong>[owner.name]'s [limb] punches [him_or_her(owner)] in the face!</strong></span>")
				owner.TakeDamage("head", rand(2,5), 0, 0, DAMAGE_BLUNT)
			else if (prob(1))
				owner.visible_message("<span style=\"color:red\">[owner.name]'s [limb] tries to strangle [him_or_her(owner)]!</span>")
				while (prob(80) && owner.bioHolder.HasEffect("funky_limb"))
					owner.losebreath = max(owner.losebreath, 2)
					sleep(10)
				owner.visible_message("<span style=\"color:red\">[owner.name]'s [limb] stops trying to strangle [him_or_her(owner)].</span>")
			return

		else if (limb_type == LIMB_IS_LEG)
			if (prob(5))
				owner.visible_message("<span style=\"color:red\">[owner.name]'s [limb] twitches [pick("rudely", "awkwardly", "weirdly", "lewdly", "strangely", "offensively", "cruelly", "furiously")]!</span>")
			else if (prob(3))
				owner.visible_message("<span style=\"color:red\"><strong>[owner.name] trips over [his_or_her(owner)] own [limb]!</strong></span>")
				owner.weakened += 2
			else if (prob(2))
				owner.visible_message("<span style=\"color:red\"><strong>[owner.name]'s [limb] kicks [him_or_her(owner)] in the head somehow!</strong></span>")
				owner.paralysis += 5
				owner.TakeDamage("head", rand(5,10), 0, 0, DAMAGE_BLUNT)
			else if (prob(2))
				owner.visible_message("<span style=\"color:red\"><strong>[owner.name] can't seem to control [his_or_her(owner)] [limb]!</strong></span>")
...
/bioEffect/involuntary_teleporting
	...
	OnLife()
		if (prob(prob_per_tick))
			...
			switch(picker)
				...
				if (5)
					L.visible_message("<span style=\"color:red\"><strong>[L.name]</strong> makes a weird noise!</span>")
