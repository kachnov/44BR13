...
/bioEffect/hidden/trainingchaplain
...
//	OnAdd()
//		if (ishuman(owner))
//			owner.add_ability_holder(/abilityHolder/religious)

/bioEffect/hidden/trainingmedical
	name = "Medical Training"
	desc = "Subject is a proficient surgeon."
	id = "training_medical"

/bioEffect/hidden/arcaneshame
...
/bioEffect/hidden/consumed
	name = "Consumed"
	desc = "Most of their flesh has been chewed off."
	id = "consumed"
	effectType = effectTypeDisability

/bioEffect/hidden/zombie
	...
	OnAdd()
		owner.set_mutantrace(/mutantrace/zombie)
		return

	OnRemove()
		if (istype(owner:mutantrace, /mutantrace/zombie))
...
/bioEffect/hidden/premature_clone
		...
		if (prob(10))
			for (var/mob/living/carbon/C in view(6,get_turf(owner)))
				if (C == owner)
					continue
				if (variant == 2)
					boutput(C, "<span style=\"color:red\">[personalized_stink]</span>")
				else
