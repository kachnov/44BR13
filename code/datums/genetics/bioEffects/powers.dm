/bioEffect/power
	...
	var/safety = 0
	var/power = 0
	var/ability_path = /targetable/geneticsAbility/cryokinesis
	var/targetable/geneticsAbility/ability = /targetable/geneticsAbility/cryokinesis
	...
	proc/check_ability_owner()
		if (ispath(ability_path))
			var/targetable/geneticsAbility/AB = new ability_path(src)
			ability = AB
			AB.linked_power = src
			spawn (0)
				AB.owner = owner

/targetable/geneticsAbility/cryokinesis
	name = "Cryokinesis"
	...
	cast(atom/target)
		...
		target.visible_message("<span style=\"color:red\"><strong>[owner]</strong> points at [target]!</span>")
		playsound(target.loc, "sound/effects/bamf.ogg", 50, 0)
		particleMaster.SpawnSystem(new /particleSystem/tele_wand(get_turf(target),"8x8snowflake","#88FFFF"))
...
/bioEffect/power/mattereater
	...
	blockGaps = 2
	stability_loss = 5
	ability_path = /targetable/geneticsAbility/mattereater
	var/target_path = "/obj/item/"

/targetable/geneticsAbility/mattereater
	name = "Matter Eater"
	...
	cast()
		...
		var/bioEffect/power/mattereater/ME = linked_power
...
/bioEffect/power/jumpy
	...
	blockGaps = 2
	stability_loss = 5
	ability_path = /targetable/geneticsAbility/jumpy

/targetable/geneticsAbility/jumpy
	name = "Jumpy"
	...
	cast()
		...
		if (istype(owner.loc,/turf))
			usr.visible_message("<span style=\"color:red\"><strong>[owner]</strong> takes a huge leap!</span>")
			...
			if (owner.bioHolder.HasEffect("fat") && prob(66) && !linked_power.safety)
				owner.visible_message("<span style=\"color:red\"><strong>[owner]</strong> crashes due to their heavy weight!</span>")
		...
		if (istype(owner.loc,/obj))
			...
			owner.paralysis += 3
			owner.weakened += 5
			container.visible_message("<span style=\"color:red\"><strong>[owner.loc]</strong> emits a loud thump and rattles a bit.</span>")
...
/bioEffect/power/polymorphism
	...
	blockGaps = 4
	stability_loss = 15
	ability_path = /targetable/geneticsAbility/polymorphism

/targetable/geneticsAbility/polymorphism
	name = "Polymorphism"
	...
	cast(atom/target)
		...
		owner.visible_message("<span style=\"color:red\"><strong>[owner]</strong> touches [target], then begins to shifts and contort!</span>")
...
/bioEffect/power/telepathy
	...
	blockGaps = 2
	stability_loss = 5
	ability_path = /targetable/geneticsAbility/telepathy

/targetable/geneticsAbility/telepathy
	name = "Telepathy"
	...
	cast(atom/target)
		...
		boutput(recipient, "<span style='color: #BD33D9'><strong>[psyname]</strong> echoes, \"<em>[msg]</em>\"</span>")
		boutput(owner, "<span style='color: #BD33D9'>You echo \"<em>[msg]</em>\" to <strong>[recipient.name]</strong>.</span>")
...
/bioEffect/power/empath
	...
	blockGaps = 2
	stability_loss = 5
	ability_path = /targetable/geneticsAbility/empath

/targetable/geneticsAbility/empath
	name = "Mind Reader"
	...
	cast(atom/target)
		...
		boutput(usr, "<span style=\"color:blue\">Mind Reading of [read.name]:</strong></span>")
		...
		switch(pain_condition)
			if (81 to INFINITY)
				boutput(owner, "<span style=\"color:blue\"><strong>Condition</strong>: [read.name] feels good.</span>")
			if (61 to 80)
				boutput(owner, "<span style=\"color:blue\"><strong>Condition</strong>: [read.name] is suffering mild pain.</span>")
			if (41 to 60)
				boutput(owner, "<span style=\"color:blue\"><strong>Condition</strong>: [read.name] is suffering significant pain.</span>")
			if (21 to 40)
				boutput(owner, "<span style=\"color:blue\"><strong>Condition</strong>: [read.name] is suffering severe pain.</span>")
			else
				boutput(owner, "<span style=\"color:blue\"><strong>Condition</strong>: [read.name] is suffering excruciating pain.</span>")
				thoughts = "haunted by their own mortality"

		switch(read.a_intent)
			if (INTENT_HELP)
				boutput(owner, "<span style=\"color:blue\"><strong>Mood</strong>: You sense benevolent thoughts from [read.name].</span>")
			if (INTENT_DISARM)
				boutput(owner, "<span style=\"color:blue\"><strong>Mood</strong>: You sense cautious thoughts from [read.name].</span>")
			if (INTENT_GRAB)
				boutput(owner, "<span style=\"color:blue\"><strong>Mood</strong>: You sense hostile thoughts from [read.name].</span>")
			if (INTENT_HARM)
				boutput(owner, "<span style=\"color:blue\"><strong>Mood</strong>: You sense cruel thoughts from [read.name].</span>")
				...
				boutput(owner, "<span style=\"color:blue\"><strong>Mood</strong>: You sense strange thoughts from [read.name].</span>")

		if (istype(target,/mob/living/carbon/human))
			var/mob/living/carbon/human/H = read
			boutput(owner, "<span style=\"color:blue\"><strong>Numbers</strong>: You sense the number [H.pin] is important to [H.name].</span>")
		boutput(owner, "<span style=\"color:blue\"><strong>Thoughts</strong>: [read.name] is currently [thoughts].</span>")
...
/bioEffect/power/immolate
	...
	blockGaps = 2
	stability_loss = 10
	ability_path = /targetable/geneticsAbility/immolate

/targetable/geneticsAbility/immolate
	name = "Immolate"
	...
	cast()
		...
		if (linked_power.power)
			owner.visible_message("<span style=\"color:red\"><strong>[owner.name]</strong> erupts into a huge column of flames! Holy shit!</span>")
			fireflash_sm(get_turf(owner), 3, 7000, 2000)
		else
			owner.visible_message("<span style=\"color:red\"><strong>[owner.name]</strong> suddenly bursts into flames!</span>")
...
/bioEffect/power/melt
	...
	blockGaps = 2
	stability_loss = 5
	ability_path = /targetable/geneticsAbility/melt

/targetable/geneticsAbility/melt
	name = "Dissolve"
	...
	cast()
		...
		if (linked_power.safety && spell_invisibility(owner, 1, 0, 0, 1) == 1) // Dry run. Can we phaseshift?
			...
			owner.visible_message("<span style=\"color:red\"><strong>[owner.name]'s flesh melts right off! Holy shit!</strong></span>")
...
/bioEffect/power/superfart
	...
	blockGaps = 3
	stability_loss = 15
	ability_path = /targetable/geneticsAbility/superfart
	var/farting = 0

/targetable/geneticsAbility/superfart
	name = "Superfart"
	...
	cast()
		...
		if (!farting_allowed)
			...
			return TRUE
		var/bioEffect/power/superfart/SF = linked_power

		if (SF.farting)
			boutput(owner, "<span style=\"color:red\">You're already farting! Be patient!</span>")
			return TRUE

		owner.visible_message("<span style=\"color:red\"><strong>[owner.name]</strong> hunches down and grits their teeth!</span>")
		SF.farting = 1
		var/stun_time = 3
		var/fart_range = 6
		var/gib_user = 0
		...
		sleep(30)
		if (can_act(owner))
			owner.visible_message("<span style=\"color:red\"><strong>[owner.name]</strong>[fart_string]</span>")
...
/bioEffect/power/eyebeams
	...
	blockGaps = 5
	stability_loss = 10
	ability_path = /targetable/geneticsAbility/eyebeams
	var/projectile_path = "/projectile/laser/eyebeams"

/targetable/geneticsAbility/eyebeams
	name = "Eyebeams"
	...
	cast(atom/target)
		...
		var/bioEffect/power/eyebeams/EB = linked_power
		var/projectile_path = text2path("[EB.projectile_path]")
		if (linked_power.power)
			projectile_path = /projectile/laser

		owner.visible_message("<span style=\"color:red\"><strong>[owner.name]</strong> shoots eye beams!</span>")
		var/projectile/laser/eyebeams/PJ = new projectile_path
		shoot_projectile_ST(owner, PJ, T)

/projectile/laser/eyebeams
	name = "optic laser"
	icon_state = "eyebeam"
	power = 20
	cost = 20
...
/bioEffect/power/adrenaline
	...
	stability_loss = 5
	ability_path = /targetable/geneticsAbility/adrenaline

/targetable/geneticsAbility/adrenaline
	name = "Adrenaline Rush"
...
/bioEffect/power/midas
	...
	blockGaps = 4
	stability_loss = 5
	ability_path = /targetable/geneticsAbility/midas
	var/transmute_material = "gold"

/targetable/geneticsAbility/midas
	name = "Midas Touch"
	...
	cast()
		...
		if (!linked_power)
			...
			if (istype(linked_power,/bioEffect/power/midas))
				var/bioEffect/power/midas/linked = linked_power
...
/bioEffect/power/healing_touch
	...
	stability_loss = 10
	ability_path = /targetable/geneticsAbility/healing_touch

/targetable/geneticsAbility/healing_touch
	name = "Healing Touch"
	...
	cast(atom/target)
		...
		owner.visible_message("<span style=\"color:red\"><strong>[owner] touches [C], enveloping them in a soft glow!</strong></span>")
...
/bioEffect/power/dimension_shift
	...
	occur_in_genepools = 0
	stability_loss = 20
	ability_path = /targetable/geneticsAbility/dimension_shift
	var/active = 0
	var/processing = 0
	var/atom/last_loc = null

/targetable/geneticsAbility/dimension_shift
	name = "Dimension Shift"
	...
	cast()
		...
		if (!istype(linked_power,/bioEffect/power/dimension_shift))
			return TRUE
		var/bioEffect/power/dimension_shift/P = linked_power
		...
		if (!P.active)
			...
			owner.visible_message("<span style=\"color:red\"><strong>[owner] vanishes in a burst of blue light!</strong></span>")
			...
			owner.visible_message("<span style=\"color:red\"><strong>[owner] appears in a burst of blue light!</strong></span>")
			playsound(owner.loc, "sound/effects/ghost2.ogg", 50, 0)
			spawn (7)
...
/bioEffect/power/photokinesis
	...
	occur_in_genepools = 0
	stability_loss = 5
	ability_path = /targetable/geneticsAbility/photokinesis
	var/red = 0
	var/green = 0
	var/blue = 0
...
/targetable/geneticsAbility/photokinesis
	name = "Photokinesis"
	...
	cast(atom/target)
		if (..())
			return TRUE
		if (!istype(linked_power,/bioEffect/power/photokinesis))
			return TRUE
		var/bioEffect/power/photokinesis/P = linked_power

		var/turf/T = get_turf(target)
		owner.visible_message("<span style=\"color:red\"><strong>[owner]</strong> raises [his_or_her(owner)] hands into the air!</span>")
...
/bioEffect/power/erebokinesis
	...
	occur_in_genepools = 0
	stability_loss = 5
	ability_path = /targetable/geneticsAbility/erebokinesis
	var/time = 0
	var/size = 0
...
/targetable/geneticsAbility/erebokinesis
	name = "Erebokinesis"
	...
	cast(atom/target)
		if (..())
			return TRUE
		if (!istype(linked_power,/bioEffect/power/erebokinesis))
			return TRUE
		var/bioEffect/power/erebokinesis/P = linked_power
...
/bioEffect/power/fire_breath
	...
	occur_in_genepools = 0
	stability_loss = 10
	ability_path = /targetable/geneticsAbility/fire_breath
	var/temperature = 1200
	var/range = 4

/targetable/geneticsAbility/fire_breath
	name = "Fire Breath"
	...
	cast(atom/target)
		...
		var/bioEffect/power/fire_breath/FB = linked_power
		var/range = FB.range
		var/temp = FB.temperature
		if (FB.power)
			range *= 2
			temp *= 4
		owner.visible_message("<span style=\"color:red\"><strong>[owner] breathes fire!</strong></span>")
		playsound(owner.loc, "sound/effects/mag_fireballlaunch.ogg", 50, 0)
...
/bioEffect/power/brown_note
	...
	blockGaps = 3
	stability_loss = 15
	ability_path = /targetable/geneticsAbility/brown_note

/targetable/geneticsAbility/brown_note
	name = "Brown Note"
	...
	cast()
		if (..())
			return TRUE
		owner.visible_message("<span style=\"color:red\"><strong>[owner.name] makes a weird noise!</strong></span>")
		...
		for (var/mob/living/L in range(7,owner))
			if (L.hearing_check(1))
				if (locate(/obj/item/storage/bible) in get_turf(L))
					owner.visible_message("<span style=\"color:red\"><strong>A mysterious force smites [owner.name] for inciting blasphemy!</strong></span>")
					owner.gib()
				else
					L.emote("fart")
...
/bioEffect/power/telekinesis_drag
	...
	stability_loss = 15
	ability_path = /targetable/geneticsAbility/telekinesis
...
/targetable/geneticsAbility/telekinesis
	...
	cast(atom/T)
		...
		owner.visible_message("<span style=\"color:red\"><strong>[owner.name]</strong> makes a gesture at [T.name]!</span>")
...
/bioEffect/power/darkcloak
	...
	cooldown = 0
	var/active = 0
	ability_path = /targetable/geneticsAbility/darkcloak
		...
		if (isliving(owner))
			...
			if (T && isturf(T))
				...
				if (istype(T, /turf/space) || (A && (istype(A, /area/shuttle) || istype(A, /area/shuttle_transit_space) || A.name == "Space")))
					...
					if (T.RL_GetBrightness() < 0.2 && can_act(owner) && active)
						cloak_decloak(1)
					else
						cloak_decloak(2)
			else
...
/targetable/geneticsAbility/darkcloak
	name = "Cloak of Darkness"
	...
	cast(atom/T)
		var/bioEffect/power/darkcloak/DC = linked_power
		if (DC.active)
...
/bioEffect/power/chameleon
	...
	cooldown = 0
	var/active = 0
	ability_path = /targetable/geneticsAbility/chameleon
	...
	OnLife()
		if (isliving(owner))
			var/mob/living/L = owner
			if ((world.timeofday - owner.l_move_time) >= 30 && can_act(owner) && active)
				L.UpdateOverlays(overlay_image, id)
				L.invisibility = 1
			else
				L.UpdateOverlays(null, id)
...
/targetable/geneticsAbility/chameleon
	name = "Chameleon"
	...
	cast(atom/T)
		var/bioEffect/power/chameleon/CH = linked_power
		if (CH.active)
...
/bioEffect/power/bigpuke
	...
	occur_in_genepools = 0
	stability_loss = 10
	ability_path = /targetable/geneticsAbility/bigpuke
	var/range = 3

/targetable/geneticsAbility/bigpuke
	name = "Mass Emesis"
	...
	cast(atom/target)
		...
		var/turf/T = get_turf(target)
		var/list/affected_turfs = getline(owner, T)
		var/bioEffect/power/bigpuke/BP = linked_power
		var/range = BP.range
		if (BP.power)
			range *= 2
		owner.visible_message("<span style=\"color:red\"><strong>[owner] horfs up a huge stream of puke!</strong></span>")
...
/bioEffect/power/ink
	...
	occur_in_genepools = 0
	stability_loss = 2
	ability_path = /targetable/geneticsAbility/ink
	var/color = "#888888"
...
/targetable/geneticsAbility/ink
	name = "Ink Glands"
	...
	cast()
		...
		var/bioEffect/power/ink/I = linked_power
		if (!linked_power)
...
/bioEffect/power/fade_out
	...
	ability_path = /targetable/geneticsAbility/fade

/targetable/geneticsAbility/fade
	name = "Fade"
