...
/turf
	...
	onMaterialChanged()
		...
		if (istype(material))
			if (initial(opacity))
				opacity = (material.alpha <= MATERIAL_ALPHA_OPACITY ? 0 : 1)
...
/turf/space/New()
	...
	if (derelict_mode == 1)
		...
		icon_state = "darkvoid"
		name = "void"
	if (buzztile == null && prob(1) && prob(1) && z == 1) //Dumb shit to trick nerds.
		buzztile = src
		icon_state = "wiggle"
		desc = "There appears to be a spatial disturbance in this area of space."
...
/turf/simulated
	...
	onMaterialChanged()
		..()
		if (material && istype(material, /material/metal/steel)) //hack hack hack
			color = null
...
/turf/simulated/wall/supernorn
	...
	proc/update_icon()
		...
		for (var/dir in cardinal) // we don't need to remove every overlay, just the relevant ones
			UpdateOverlays(null, "connect-[dir]")
		for (var/dir in connect_dirs)
			if (!connect_images["connect-[dir]"])
				connect_images["connect-[dir]"] = image(icon, "connect-[dir]")
			UpdateOverlays(connect_images["connect-[dir]"], "connect-[dir]")
...
/turf/simulated/floor/blob
	...
	attackby(var/obj/item/W, var/mob/user)
		if (istype(W, /obj/item/weldingtool))
			visible_message("<strong>[user] hits [src] with [W]!</strong>")
			if (prob(25))
				ReplaceWithSpace()
...
/turf/simulated/wall
	...
	get_desc()
		if (islist(proj_impacts) && proj_impacts.len)
			var/shots_taken = 0
			for (var/i in proj_impacts)
				shots_taken ++
	...
	proc/update_projectile_image(var/update_time)
		if (last_proj_update_time && (last_proj_update_time + 1) < ticker.round_elapsed_ticks)
			return
		if (!proj_image)
			proj_image = image('icons/obj/projectiles.dmi', "blank")
		//overlays -= proj_image
		proj_image.overlays = null
		for (var/image/i in proj_impacts)
			proj_image.overlays += i
		UpdateOverlays(proj_image, "projectiles")
		//overlays += proj_image
...
/turf/simulated/shuttle/wall
	...
	New()
		...
		spawn (60) // patch up some ugly corners in derelict mode
			if (derelict_mode)
				if (icon_state == "[icon_style]_space")
					icon_state = "[icon_style]_void"
...
/turf/Enter(atom/movable/mover as mob|obj, atom/forget as mob|obj|turf|area)
	...
	// the invincible noclipping mutt 
	if (ismutt(mover) && mover:nodamage)
		return TRUE 
...
/turf/Entered(atom/movable/M as mob|obj, atom/OldLoc)
	...
	///////////////////////////////////////////////////////////////////////////////////
	..()
	if (material) material.triggerOnEntered(src, M)
...
/turf/proc/ReplaceWith(var/what)
	...
	if (!RL_Ignore)
		var/area/old_loc = loc
		if (old_loc)
			old_loc.contents -= loc

	var/new_type = text2path(what)
	if (new_type)
		new_turf = new new_type( locate(x, y, z) )
		if (!isturf(new_turf))
			new_turf = new /turf/space( locate(x, y, z) )

	else switch(what)
		if ("Floor")
			new_turf = new /turf/simulated/floor( locate(x, y, z) )
		if ("MetalFoam")
			new_turf = new /turf/simulated/floor/metalfoam( locate(x, y, z) )
		if ("EngineFloor")
			new_turf = new /turf/simulated/floor/engine( locate(x, y, z) )
		if ("Grid")
			new_turf = new /turf/simulated/floor/grid( locate(x, y, z) )
		if ("RWall")
			if (map_setting)
				if (map_setting == "COG2")
					new_turf = new /turf/simulated/wall/auto/reinforced/supernorn( locate(x, y, z) )
				else if (map_setting == "DESTINY")
					new_turf = new /turf/simulated/wall/auto/reinforced/gannets( locate(x, y, z) )
				else
					new_turf = new /turf/simulated/wall/r_wall( locate(x, y, z) )
			else
				new_turf = new /turf/simulated/wall/r_wall( locate(x, y, z) )
		if ("Wall")
			if (map_setting)
				if (map_setting == "COG2")
					new_turf = new /turf/simulated/wall/auto/supernorn( locate(x, y, z) )
				else if (map_setting == "DESTINY")
					new_turf = new /turf/simulated/wall/auto/gannets( locate(x, y, z) )
				else
					new_turf = new /turf/simulated/wall( locate(x, y, z) )
			else
				new_turf = new /turf/simulated/wall( locate(x, y, z) )
		else
			new_turf = new /turf/space( locate(x, y, z) )
...
/turf/simulated/wall/New()
	..()
	if (!ticker && istype(loc, /area/station/maintenance) && prob(7))
...
/turf/simulated/wall/proc/take_hit(var/obj/item/I)
	if (material)
		if (I.material)
			if ((I.material.getProperty(PROP_HARDNESS) ? I.material.getProperty(PROP_HARDNESS) : (I.throwing ? I.throwforce : I.force)) >= (src.material.getProperty(PROP_HARDNESS) ? src.material.getProperty(PROP_HARDNESS) : 60))
				health -= round((I.throwing ? I.throwforce : I.force) / 10)
				visible_message("<span style=\"color:red\">[usr ? usr : "Someone"] hits [src] with [I]!</span>", "<span style=\"color:red\">You hit [src] with [I]!</span>")
			else
				visible_message("<span style=\"color:red\">[usr ? usr : "Someone"] uselessly hits [src] with [I].</span>", "<span style=\"color:red\">You hit [src] with [I] but it takes no damage.</span>")
		else
	...
	if (health <= 0)
		visible_message("<span style=\"color:red\">[usr ? usr : "Someone"] destroys [src]!</span>", "<span style=\"color:red\">You destroy [src]!</span>")
		dismantle_wall(1)
	return
...
/turf/simulated/wall/attack_hand(mob/user as mob)
	if (user.bioHolder.HasEffect("hulk"))
		if (prob(40))
			boutput(usr, text("<span style=\"color:blue\">You smash through the [name].</span>"))
			dismantle_wall(1)
			return
		else
			boutput(usr, text("<span style=\"color:blue\">You punch the [name].</span>"))
			return

	if (material)
		var/fail = 0
		if (prob(material.getProperty(PROP_INSTABILITY))) fail = 1
		if (material.quality < 0) if (prob(abs(material.quality))) fail = 1
	...
	boutput(user, "<span style=\"color:blue\">You hit the [name] but nothing happens!</span>")
	playsound(loc, "sound/weapons/Genhit.ogg", 25, 1)
	return
...
/turf/simulated/wall/attackby(obj/item/W as obj, mob/user as mob)
	if (istype(W, /obj/item/pen))
		...
		attach_light_fixture_parts(user, W) // Made this a proc to avoid duplicate code (Convair880).
		return
...
/turf/simulated/wall/r_wall/attackby(obj/item/W as obj, mob/user as mob)
	...
	//More spooky halloween key
	...
	else if ((istype(W, /obj/item/sheet)) && (d_state))
		...
		if ((user.loc == T && user.equipped() == S))
			...
			d_state = 0
			icon_state = initial(icon_state)
			if (W.material) setMaterial(S.material)
			boutput(user, "<span style=\"color:blue\">You repaired the wall.</span>")
			if (S.amount > 1)
				S.amount--
			else
...
//grabsmash
	...
	if (istype(src, /turf/simulated/wall/r_wall) && d_state > 0)
...
/turf/simulated/floor/CanPass(atom/movable/mover, turf/target, height=0, air_group=0)
	if (!allows_vehicles && (istype(mover, /obj/machinery/vehicle)))
...
/turf/simulated/floor/attackby(obj/item/C as obj, mob/user as mob)
	...
	if (istype(C, /obj/item/crowbar) && intact)
		...
		to_plating()
		playsound(loc, "sound/items/Crowbar.ogg", 80, 1)
	...
	if (istype(C, /obj/item/seed/grass) && intact)
		...
		else if (icon_state == "grass1" || icon_state == "grass2" || icon_state == "grass3" || icon_state == "grass4")
		...
		if (!intact)
			if (C:amount >= 2)
				...
				if (do_after(user, 30))
					...
					playsound(loc, "sound/items/Deconstruct.ogg", 80, 1)
			else
	...
	if (istype(C, /obj/item/tile) && !intact)
		...
		if (!istype(material, /material/metal/steel))
			logTheThing("station", user, null, "constructs a floor (<strong>Material:</strong>: [material && material.name ? "[material.name]" : "*UNKNOWN*"]) at [log_loc(src)].")
...
//		return TRUE
	...
	if ((istype(A, /mob) && x > 2 && x < (world.maxx - 1)))
		var/mob/M = A
		...
		if ((!( M.handcuffed) && M.canmove))
			...
			if (prob_slip < 5) //next to something, but they might slip off
				if (prob(prob_slip) )
					boutput(M, "<span style=\"color:blue\"><strong>You slipped!</strong></span>")
...
/turf/simulated/shuttle/wall/destroyable/meteorhit()
	health -= 20 // i guess this is good enough? maybe a rand argument would be better
	...
	bluewall_glowing
		...
		attackby(obj/item/W as obj, mob/user as mob)
			if (istype(W, /obj/item/device/key))
				playsound(loc, "sound/effects/mag_warp.ogg", 50, 1)
				visible_message("<span style=\"color:blue\"><strong>[src] slides away!</strong></span>")
				ReplaceWithSpace() // make sure the area override says otherwise - maybe this sucks
...
/turf/unsimulated/dirt
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/shovel))
			...
			user.visible_message("<strong>[user]</strong> begins to dig!", "You begin to dig!")
			//todo: A digging sound effect.
			if (do_after(user, 40) && icon_state != "dirt-dug")
				icon_state = "dirt-dug"
				user.visible_message("<strong>[user]</strong> finishes digging.", "You finish digging.")
