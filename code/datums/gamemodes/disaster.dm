...
/game_mode/disaster/announce()
	if (derelict_mode)
		boutput(world, "<tt>BUG: MEM ERR 0000FF88 00F90045</tt>")
		world << sound('sound/machines/glitch1.ogg')
		boutput(world, "<strong>We are experiencing technical difficulties. Please remain calm. Help is on the way.</strong>")
		boutput(world, "<strong>Report to your station's emergency rally point: CHAPEL.</strong>")
	else
		boutput(world, "<strong>The current game mode is - Disaster!</strong>")
		boutput(world, "<strong>The station is in the middle of a cosmic disaster! You must escape or at least live!</strong>")


/game_mode/disaster/post_setup()
...
/game_mode/disaster/declare_completion()
	...
	if (survivors.len)
		boutput(world, "<span style=\"color:blue\"><strong>The following survived the [disaster_name] event!</strong></span>")
		for (var/survivor in survivors)
			var/condition = survivors[survivor]
			switch(condition)
				if ("shuttle")
					boutput(world, "&emsp; <strong><FONT size = 2>[survivor] escaped on the shuttle!</FONT></strong>")
				if ("alive")
					boutput(world, "&emsp; <FONT size = 1>[survivor] stayed alive. Whereabouts unknown.</FONT>")

	else
		boutput(world, "<span style=\"color:blue\"><strong>No one survived the [disaster_name] event!</strong></span>")
...
/game_mode/disaster/proc/start_disaster()
