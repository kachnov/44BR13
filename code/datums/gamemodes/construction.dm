/game_mode/construction
	...
	var/construction_controller/events = null
...
/game_mode/construction/announce()
	boutput(world, "<strong>The current game mode is - Construction!</strong>")
	boutput(world, "<strong>You have been assigned to construct the next generation space station.</strong>")
	boutput(world, "<strong>The map will reset in 12 hours and 30 minutes.</strong>")

/game_mode/construction/pre_setup()
	in_setup = 1
	for (var/mob/new_player/M in mobs)
		if (M.ready)
			M.close_spawn_windows()
	var/job/engineering/construction_worker/C = new /job/engineering/construction_worker()
	C.limit = -1
	enabled_jobs += C
	boutput(world, "<strong><font color='red'>Setting up map for construction. This will not take a very long time.</font></strong>")
	...
	boutput(world, "<strong>A supply post is located to the [supply_loc]. Establish your station using the supplies you might find in there.</strong>")
...
/proc/spawn_edge_asteroid(var/grade)
	...
	switch (grade)
		if (1)
			common_ores = list(/ore/pharosium, /ore/molitz, /ore/mauxite)
			rare_ores = list(/ore/claretine, /ore/bohrum)

		if (2)
			common_ores = list(/ore/claretine, /ore/bohrum, /ore/viscerite, /ore/koshmarite)
			rare_ores = list(/ore/plasmastone, /ore/cobryl)
	...
	switch (rand(1,4))
		...
		if (4)
			minor_ores += mining_controls.get_ore_from_path(pick(common_ores))
			minor_ores += mining_controls.get_ore_from_path(pick(rare_ores))
	var/generation_marker/initial = new
	logTheThing("debug", null, null, "<strong>Marquesas/Construction:</strong> Generating asteroid at [showCoords(ax, ay, 1)].")
	initial.x = ax
	initial.y = ay
	initial.z = 1
	...
	while (tiles.len)
		var/generation_marker/marker = tiles[1]
		tiles -= marker
		...
		if (ORE)
			...
			if (gems && prob(25))
				gems--
				var/ore/event/gem/gem_event = new()
...
/game_mode/construction/process()
	...
	for (var/demand_control/D in special_demand_control)
	...
	if (ticker.round_elapsed_ticks > next_progress_check_at)
		for (var/progress/P in milestones)
...
/game_mode/construction/check_finished()
	if (ticker.round_elapsed_ticks - starttime > round_length)
		return TRUE
	return FALSE

/game_mode/construction/declare_completion()
	boutput(world, "<strong><span color='red'>The construction is over. There will be some obscure scoring shit here.</span></strong>")

/game_mode/construction/post_setup()
	...
	for (var/tp in subtypesof(/supply_control))
		var/supply_control/S = new tp()
	...
	for (var/tp in subtypesof(/demand_control))
		var/demand_control/D = new tp()
		D.last_demand_change = ticker.round_elapsed_ticks
		special_demand_control += D
	for (var/pt in subtypesof(/progress))
		var/progress/P = new pt()
		if (P.is_abstract)
			qdel(P)
			continue
		milestones += P
	for (var/progress/P in milestones)
		if (P.parent)
			var/progress/Q = locate(P.parent) in milestones
			if (Q)
				P.parent = Q
			else
				P.parent = null
...
/game_mode/construction/post_post_setup()
	abandon_allowed = 1
...
/proc/debug_supply_pack()
	var/thepath = input("Path", "Path", "/supply_packs") as text
	if (!lentext(thepath))
		return
	var/therealpath = text2path(thepath)
	if (therealpath)
		var/supply_packs/S = new therealpath()
		if (istype(S))
