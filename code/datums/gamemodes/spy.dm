...
/game_mode/spy/announce()
	boutput(world, "<strong>The current game mode is - Conspiracy!</strong>")
	boutput(world, "<strong>The Syndicate is using the station as a battleground to train elite operatives!</strong>")

/game_mode/spy/pre_setup()
	...
	for (var/j = 0, j < num_teams, j++)
		var/mind/leader = pick(leaders_possible)
		leaders += leader
...
/game_mode/spy/post_setup()

	for (var/mind/leaderMind in leaders)
		if (!leaderMind.current)
			continue

		var/objective/specialist/conspiracy/spyObjective = bestow_objective(leaderMind,/objective/specialist/conspiracy)
		spyObjective = bestow_objective(leaderMind, pick(/objective/escape, /objective/escape/survive)) // They have to stay alive, dunno why dying a glorious death was a possible objective.
		...
		switch(rand(1, ((leaderMind.assigned_role in list("Captain","Head of Personnel","Head of Security","Chief Engineer","Research Director")) ? 3 : 4) ))
			if (1)
				spyObjective = bestow_objective(leaderMind,/objective/regular/assassinate)
			if (2)
				spyObjective = bestow_objective(leaderMind,/objective/regular/aikill)
			if (3)
				spyObjective = bestow_objective(leaderMind,/objective/regular/borgdeath)
			if (4)
				spyObjective = bestow_objective(leaderMind,/objective/regular/steal)
		...
		boutput(leaderMind.current, "<span style=\"color:red\">Oh yes, and <strong>one more thing:</strong> <strong>[spyObjective.explanation_text]</strong> That is, if you <em>really</em> want that new position.</span>")
...
/game_mode/spy/proc/equip_leader(mob/living/carbon/human/leader)
	if (!istype(leader))
		return
	...
	boutput(leader, "<span style=\"color:blue\">You've been supplied with a <strong>special quad-use implanter</strong> in the spy starter kit in your [!isnull(the_slot) ? "[the_slot]" : "UNKNOWN"]. Use it to recruit some mindslaved henchmen!</span>")
	return

/game_mode/spy/proc/add_spy(mob/living/new_spy, mob/living/leader)
	if (!new_spy || !leader || !new_spy.mind || !leader.mind)
		return FALSE

	var/mind/spymind = new_spy.mind
	var/mind/leadermind = leader.mind
	if (spymind in spies)
		if (leadermind != spies[spymind])
			spies[spymind] = leadermind
			return TRUE
...
/game_mode/spy/proc/remove_spy(mob/living/spy)
	spies.Remove(spy)
	spy.mind.special_role = null
	spy.mind.master = null
	return TRUE

/game_mode/spy/proc/get_possible_leaders()
	...
	if (candidates.len < 1)
		logTheThing("debug", null, null, "<strong>Enemy Assignment</strong>: Not enough players with be_spy set to yes, so we're adding players who don't want to be spy leaders to the pool.")
		for (var/mob/new_player/player in mobs)
			if ((player.client) && (player.ready) && !(player.mind in leaders) && !(player.mind in token_players) && !candidates.Find(player.mind))
				candidates += player.mind
...
/game_mode/spy/declare_completion()

	var/text = ""

	boutput(world, "<FONT size = 2><strong>The infiltrators were: </strong></FONT>")
	var/mind/potential_winner = null
	var/survived_count = 0
	for (var/mind/leader_mind in leaders)
		text = ""
		if (leader_mind.current)
			...
			if (leader_mind.current.stat == 2)
				...
				if (T.z == 2)
					var/all_complete = 1
					for (var/objective/O in leader_mind.objectives)
						if (istype(O, /objective/specialist/conspiracy))
							continue
	...
	if (istype(potential_winner) && potential_winner.current && (survived_count == 1))
		boutput(world, "<h2><strong>[potential_winner.current.real_name] succeeded! The Syndicate has promoted them to [pick("Executive Bathroom Toilet Scrubber", "Head Office Cafeteria Worker", "Board Room Custodian")]!</strong></h2>")

		#ifdef DATALOGGER
		game_stats.Increment("traitorwin")
		#endif
	else
		boutput(world, "<h2><strong>The infiltrators did not succeed!</strong></h2>")
		#ifdef DATALOGGER
	...
	text = ""
	boutput(world, "<FONT size = 2><strong>The brainwashed conspirators were: </strong></FONT>")
	if (!spies.len)
		text += "Nobody!"
	else
		for (var/mind/spy_mind in spies)
			if (spy_mind.current)
...
/obj/item/device/spy_implanter
	...
	proc/update_icon()
		icon_state = "revimplanter[min(4, round((charges/initial(charges)), 0.25) * 4)]"
		return
	...
	attack(mob/M as mob, mob/user as mob)
		...
		if (user && (charges > 0))
			...
			new_imp.implanted = 1
			new_imp.owner = M
			user.show_message("<span style=\"color:red\">You implanted the implant into [M]. <strong>[charges-1]</strong> implants remaining!</span>")
...
/obj/item/implant/spy_implant
	...
	var/mind/leader_mind = null
	var/objective/linked_objective = null
	...
	implanted(mob/M, mob/Implanter, override=0)
		...
		var/mind/oldLeader = leader_mind
		...
		if (M.mind)
			if (!linked_objective)
				linked_objective = new /objective(  )
				M.mind.objectives += linked_objective

			linked_objective.explanation_text = "Obey [leader_name]'s every order."
	...
	on_remove(var/mob/M)
		...
		if (leader_name)
			...
			if (istype(ticker.mode, /game_mode/spy))
				...
				if (M.client && leader_mind && leader_mind.current)
					for (var/image/I in M.client.images)
						if (I.loc == leader_mind.current)
							qdel(I)
							break

				if (M.mind && linked_objective)
					M.mind.objectives -= linked_objective
					qdel(linked_objective)
