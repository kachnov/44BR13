...
/game_mode/mixed/announce()
	boutput(world, "<strong>The current game mode is - Mixed!</strong>")
	boutput(world, "<strong>Anything could happen! Be on your guard!</strong>")

/game_mode/mixed/pre_setup()
	var/num_players = 0
	...
	if (num_vampires)
		...
		for (var/j = 0, j < num_vampires, j++)
			if (!possible_vampires.len)
				break
			var/mind/vampire = pick(possible_vampires)
			traitors += vampire
	...
	if (num_blobs)
		...
		for (var/j = 0, j < num_blobs, j++)
			if (!possible_blobs.len)
				break
			var/mind/blob = pick(possible_blobs)
			traitors += blob
	...
	if (num_grinches)
		...
		for (var/j = 0, j < num_grinches, j++)
			if (!possible_grinches.len)
				break
			var/mind/grinch = pick(possible_grinches)
			traitors += grinch
			possible_grinches.Remove(grinch)
			grinch.special_role = "grinch"

	if (!traitors) return FALSE

	return TRUE

/game_mode/mixed/post_setup()
		...
		switch (traitor.special_role)
			...
			if ("wizard")
				...
				if (wizardstart.len == 0)
					boutput(traitor.current, "<strong><span style=\"color:red\">A starting location for you could not be found, please report this bug!</span></strong>")
				else
			...
			if ("predator")
				...
				traitor.current.show_text("<h2><font color=red><strong>You are a predator!</strong></font></h2>", "red")
				traitor.current.make_predator()

			if ("grinch")
				objective_set_path = /objective_set/grinch
				boutput(traitor.current, "<h2><font color=red><strong>You are a grinch!</strong></font></h2>")
				traitor.current.make_grinch()

			if ("blob")
				objective_set_path = /objective_set/blob
				spawn (0)
...
/game_mode/mixed/proc/get_possible_enemies(type,number)
	...
	if (candidates.len < number)
		if (type in list("wizard","traitor","changeling", "wraith", "blob"))
			logTheThing("debug", null, null, "<strong>Enemy Assignment</strong>: Only [candidates.len] players with be_[type] set to yes were ready. We need [number] so including players who don't want to be [type]s in the pool.")
		else
			logTheThing("debug", null, null, "<strong>Enemy Assignment</strong>: Not enough players with be_misc set to yes, including players who don't want to be misc enemies in the pool for [type] assignment.")
...
/game_mode/mixed/send_intercept()
...
/game_mode/mixed/declare_completion()
	. = ..()

/game_mode/mixed/proc/add_law_zero(mob/living/silicon/ai/killer)
...
/game_mode/mixed/proc/get_mob_list()
	var/list/mobs = list()
...
/game_mode/mixed/proc/pick_human_name_except(excluded_name)
	var/list/names = list()
	for (var/mob/living/player in mobs)
