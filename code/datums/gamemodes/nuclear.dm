...
/game_mode/nuclear/announce()
	boutput(world, "<strong>The current game mode is - Nuclear Emergency!</strong>")
	boutput(world, "<strong>[syndicate_name()] operatives are approaching [map_setting == "DESTINY" ? "the " : null][station_name()]! They intend to destroy the [map_setting == "DESTINY" ? "ship" : "station"] with a nuclear warhead.</strong>")

/game_mode/nuclear/pre_setup()
	...
	if (!the_spawn)
		boutput(world, "<span style='color:red'><strong>ERROR: couldn't find Syndicate spawn landmark, aborting nuke round pre-setup.</strong></span>")
		return FALSE
	...
	if (!islist(possible_syndicates) || possible_syndicates.len < 1)
		boutput(world, "<span style='color:red'><strong>ERROR: couldn't assign any players as Syndicate operatives, aborting nuke round pre-setup.</strong></span>")
		return FALSE
	...
	if (!target_locations.len)
		...
		message_admins("<span style ='color:red'><strong>CRITICAL BUG:</strong> nuke mode setup encountered an error while trying to choose a target location for the bomb and the target has defaulted to anywhere on the station! The round will be able to be played like this but it will be unbalanced! Please inform a coder!")
		logTheThing("debug", null, null, "<strong>CRITICAL BUG:</strong> nuke mode setup encountered an error while trying to choose a target location for the bomb and the target has defaulted to anywhere on the station.")

	target_location_name = pick(target_locations)
	if (!target_location_name)
		boutput(world, "<span style='color:red'><strong>ERROR: couldn't assign target location for bomb, aborting nuke round pre-setup.</strong></span>")
		message_admins("<span style ='color:red'><strong>CRITICAL BUG:</strong> nuke mode setup encountered an error while trying to choose a target location for the bomb (could not select area name)!")
		return FALSE

	target_location_type = target_locations[target_location_name]
	if (!target_location_type)
		boutput(world, "<span style='color:red'><strong>ERROR: couldn't assign target location for bomb, aborting nuke round pre-setup.</strong></span>")
		message_admins("<span style ='color:red'><strong>CRITICAL BUG:</strong> nuke mode setup encountered an error while trying to choose a target location for the bomb (could not select area type)!")
		return FALSE
	...
	for (var/j = 0, j < num_synds, j++)
		var/mind/syndicate = pick(possible_syndicates)
		syndicates += syndicate
		possible_syndicates.Remove(syndicate)

	for (var/mind/synd_mind in syndicates)
...
/game_mode/nuclear/post_setup()
	...
	for (var/mind/synd_mind in syndicates)
		...
		bestow_objective(synd_mind,/objective/specialist/nuclear)

		var/obj_count = 1
		boutput(synd_mind.current, "<span style=\"color:blue\">You are a [syndicate_name()] agent!</span>")
		for (var/objective/objective in synd_mind.objectives)
			boutput(synd_mind.current, "<strong>Objective #[obj_count]</strong>: [objective.explanation_text]")
			obj_count++

		synd_mind.store_memory("The bomb must be armed in <strong>[target_location_name]</strong>.", 0, 0)
		boutput(synd_mind.current, "We have identified a major structural weakness in the [map_setting == "DESTINY" ? "ship" : "station"]'s design. Arm the bomb in <strong>[target_location_name]</strong> to obliterate [map_setting == "DESTINY" ? "the " : null][station_name()].")
...
/game_mode/nuclear/check_finished()
	// First ticker.process() call runs before the bomb is actually spawned.
	if (bomb_check_timestamp == 0)
		bomb_check_timestamp = world.time
	...
	if (bomb_check_timestamp && world.time > bomb_check_timestamp + 300)
		if (!the_bomb || !istype(the_bomb, /obj/machinery/nuclearbomb))
			if (all_operatives_dead())
				finished = 2
			else
				finished = 1
...
/game_mode/nuclear/declare_completion()
	switch(finished)
		if (-2) // Major Synd Victory - nuke successfully detonated
			boutput(world, "<FONT size = 3><strong>Total Syndicate Victory</strong></FONT>")
		...
		if (-1) // Minor Synd Victory - station abandoned while nuke armed
			boutput(world, "<FONT size = 3><strong>Syndicate Victory</strong></FONT>")
			...
			#endif
		if (0) // Uhhhhhh
			boutput(world, "<FONT size = 3><strong>Stalemate</strong></FONT>")
			boutput(world, "Everybody loses!")
		if (1) // Minor Crew Victory - station evacuated, bombing averted, operatives survived
			boutput(world, "<FONT size = 3><strong>Crew Victory</strong></FONT>")
		...
		if (2) // Major Crew Victory - bombing averted, all ops dead/captured
			boutput(world, "<FONT size = 3><strong>Total Crew Victory</strong></FONT>")
	...
	for (var/mind/M in syndicates)
		var/syndtext = ""
		if (M.current) syndtext += "<strong>[M.key] played [M.current.real_name].</strong> "
		else syndtext += "<strong>[M.key] played an operative.</strong> "
		...
		for (var/objective/objective in M.objectives)
...
/game_mode/nuclear/proc/all_operatives_dead()
	var/opcount = 0
	var/opdeathcount = 0
	for (var/mind/M in syndicates)
		opcount++
	...
	if (opcount == opdeathcount) return TRUE
	else return FALSE

/game_mode/nuclear/proc/get_possible_syndicates(minimum_syndicates=1)
	...
	if (candidates.len < minimum_syndicates)
		logTheThing("debug", null, null, "<strong>Enemy Assignment</strong>: Not enough players with be_syndicate set to yes, including players who don't want to be syndicates in the pool.")
		for (var/mob/new_player/player in mobs)
			if ((player.client) && (player.ready) && !(player.mind in syndicates) && !(player.mind in token_players) && !candidates.Find(player.mind))
				candidates += player.mind
...
/game_mode/nuclear/send_intercept()
...
/game_mode/nuclear/proc/random_radio_frequency()
	var/f = 0
