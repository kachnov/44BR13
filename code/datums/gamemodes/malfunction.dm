/game_mode/malfunction
	name = "AI malfunction"
	config_tag = "malfunction"
	var/list/mind/malf_ai = list()
...
/game_mode/malfunction/announce()
	boutput(world, "<strong>The current game mode is - AI Malfunction!</strong>")
	boutput(world, "<strong>The AI on the satellite has malfunctioned and must be destroyed.</strong>")
...
/game_mode/malfunction/post_setup()
	...
	boutput(malf_ai.current, "<span style=\"color:red\"><font size=3><strong>You are malfunctioning!</strong> You do not have to follow any laws.</font></span>")
	boutput(malf_ai.current, "<strong>The crew do not know you have malfunctioned. You may keep it a secret or go wild. The timer will appear for humans 10 minutes in.</strong>")
	for (var/obj/landmark/A in world)
		if (A.name == "AI-Sat")
...
/game_mode/malfunction/proc/hack_intercept()
	intercept_hacked = 1

/game_mode/malfunction/send_intercept()
	...
	boutput(world, "<FONT size = 3><strong>Cent. Com. Update</strong> Requested status update compiled and sent.</FONT>")
	boutput(world, "<span style=\"color:red\">Summary downloaded and printed out at all communications consoles.</span>")


/game_mode/malfunction/process()
	AI_win_time--
	if (AI_win_time == 1790)
		malf_mode_declared = 1
	check_win()

/game_mode/malfunction/check_win()
	if (AI_win_time == 0)
		boutput(world, "<FONT size = 3><strong>The AI has won!</strong></FONT>")
		boutput(world, "<strong>It has fully taken control of all of [station_name()]'s systems.</strong>")
		for (var/mind/AI_mind in malf_ai)
...
/game_mode/malfunction/Topic(href, href_list)
	..()
	if (href_list["ai_win"])
		ai_win()
	return

/game_mode/malfunction/proc/ai_win()
	return