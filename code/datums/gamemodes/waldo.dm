...
/game_mode/waldo/announce()
	boutput(world, "<strong>The current game mode is - Waldo!</strong>")
	boutput(world, "<strong><span style=\"color:red\">A man named Waldo</span> is likely to be somewhere on the station. You must find him (And beware of any of his compatriots!)</strong>")

/game_mode/waldo/pre_setup()
	...
	for (var/j = 0, j < num_waldos, j++)
		var/mind/waldo = pick(possible_waldos)
		waldos += waldo
		possible_waldos.Remove(waldo)

	for (var/mind/waldo_mind in waldos)
		waldo_mind.assigned_role = "MODE" //So they aren't chosen for other jobs.
	return TRUE

/game_mode/waldo/post_setup()
	...
	for (var/mind/waldo in waldos)
		...
		if (istype(waldo))
			...
			waldo.current.resistances += list(/ailment/disease/dnaspread, /ailment/disease/clowning_around, /ailment/disease/cluwneing_around, /ailment/disease/enobola, /ailment/disease/robotic_transformation)
			if (wizardstart.len == 0)
				boutput(waldo.current, "<strong><span style=\"color:red\">A starting location for you could not be found, please report this bug!</span></strong>")
			else
			...
			switch(waldo.special_role)
				if ("waldo")
					boutput(waldo.current, "<strong><span style=\"color:red\">You are Waldo!</span></strong>")
					waldo.current.real_name = "Waldo"
				if ("odlaw")
					boutput(waldo.current, "<strong><span style=\"color:red\">You are Odlaw!</span></strong>")
					waldo.current.real_name = "Odlaw"
				if ("wizard")
					boutput(waldo.current, "<strong><span style=\"color:red\">You are Wizard Whitebeard!</span></strong>")
					waldo.current.real_name = "Wizard Whitebeard"
			equip_waldo(waldo.current)
			boutput(waldo.current, "<strong>You have come to [station_name()] to carry out the following tasks:</strong>")

			var/obj_count = 1
			for (var/objective/objective in waldo.objectives)
				boutput(waldo.current, "<strong>Objective #[obj_count]</strong>: [objective.explanation_text]")
				obj_count++

			if (waldo.special_role == "waldo")
				boutput(waldo.current, "<span style=\"color:red\"><strong><font size=3>WARNING: Being away from the station (ie. in space) will decrement your stealth points! Stay alive on the station!</font></strong></span>")
			k++
...
/game_mode/waldo/proc/get_possible_waldos()
...
/game_mode/waldo/proc/equip_waldo(mob/living/carbon/human/waldo_mob)
	if (!istype(waldo_mob))
		return
	var/mind/waldo_mind = waldo_mob.mind
	if (waldo_mind)
			...
			if ("wizard")
				...
				if (!R)
					...
					if (istype(R, /obj/item/device/radio))
						...
						waldo_mob.mind.store_memory("<strong>Radio Freq:</strong> [format_frequency(freq)] ([R.name] [loc_text]).")
						...
						waldo_mob.mind.store_memory("<strong>Uplink Passcode:</strong> [pda_pass] ([R.name] [loc_text]).")
...
/game_mode/waldo/send_intercept()
	var/intercepttext = "<FONT size = 3><strong>Cent. Com. Update</strong> Requested staus information:</FONT><HR>"
	intercepttext += "<strong> Cent. Com has recently been contacted by the following syndicate affiliated organisations in your area, please investigate any information you may have:</strong>"
...
/game_mode/waldo/declare_completion()
	var/wizcount = 0
	var/wizdeathcount = 0
	for (var/mind/W in waldos)
		wizcount++
		if (!W.current || W.current.stat == 2) wizdeathcount++
	if (wizcount == wizdeathcount)
		if (wizcount >= 2) boutput(world, "<span style=\"color:red\"><FONT size=3><strong>Waldo and friends have been killed by the crew!</strong></FONT></span>")
		else boutput(world, "<span style=\"color:red\"><FONT size=3><strong>Waldo has been killed by the crew!</strong></FONT></span>")
	else
		boutput(world, "<span style=\"color:red\"><font size=3><strong>Waldo and friends have survived their stay at [station_name()]!</strong></font></span>")
	...
	for (var/mind/W in waldos)
		...
		boutput(world, "<strong>[waldo_name]</strong>")
		var/count = 1
...
/game_mode/waldo/proc/get_mob_list()
	var/list/mobs = list()
...
/game_mode/waldo/proc/pick_human_name_except(excluded_name)
	var/list/names = list()
	for (var/mob/living/player in mobs)
...
/game_mode/waldo/check_finished()
	var/wizcount = 0
	var/wizdeathcount = 0
	for (var/mind/W in waldos)
		wizcount++
