...
/game_mode/gang/announce()
	boutput(world, "<strong>The current game mode is - Gang War!</strong>")
	boutput(world, "<strong>A number of gangs are competing for control of the station!</strong>")
	boutput(world, "<strong>Gang members are normal crew except that they can kill other gang members without being banned.</strong>")

/game_mode/gang/pre_setup()
	var/num_players = 0
...
/game_mode/gang/post_setup()
	...
	for (var/mind/leaderMind in leaders)
		...
		bestow_objective(leaderMind,/objective/specialist/gang)
		...
		boutput(leaderMind.current, "<span style=\"color:red\"><strong>Turf, cash, guns and drugs all count towards victory, and your survival gives your gang bonus points!</strong></span>")
...
/game_mode/gang/proc/force_shuttle()
...
/game_mode/gang/proc/equip_leader(mob/living/carbon/human/leader)
	if (!istype(leader))
		return
...
/game_mode/gang/proc/get_possible_leaders(minimum_leaders=1)
	...
	if (candidates.len < minimum_leaders)
		logTheThing("debug", null, null, "<strong>Enemy Assignment</strong>: Not enough players with be_gangleader set to yes, including players who don't want to be misc enemies in the pool for Gang Leader selection.")
		for (var/mob/new_player/player in mobs)
			if ((player.client) && (player.ready) && !(player.mind in leaders) && !(player.mind in token_players) && !candidates.Find(player.mind))
				candidates += player.mind
...
/game_mode/gang/check_finished()
	...
	var/leadercount = 0
	for (var/mind/L in ticker.mode:leaders)
		leadercount++
...
/game_mode/gang/declare_completion()

	var/text = ""

	boutput(world, "<FONT size = 2><strong>The gang leaders were: </strong></FONT><br>")
	for (var/mind/leader_mind in leaders)
		text = ""
		text += "<strong>[leader_mind.gang.gang_name]</strong><br>"
		if (leader_mind.current)
			text += "<strong>Leader: </strong>[leader_mind.current.real_name]"
			if (leader_mind.current.stat == 2) text += " (Dead)"
			else text += " (Survived)"
		else
			text += "<strong>Leader: </strong>[leader_mind.key] (Destroyed)"
		text += "<br><strong>Members:</strong> "
		...
		text += "<br><strong>Areas Owned:</strong> [leader_mind.gang.num_areas_controlled()]"
		text += "<br><strong>Cash Pile:</strong> $[leader_mind.gang.gang_cash_amount()]"
		text += "<br><strong>Guns Stashed:</strong> [leader_mind.gang.gang_gun_amount()]"
		text += "<br><strong>Drug Score:</strong> [leader_mind.gang.gang_drug_score()]"
		text += "<br><strong>Total Score: [leader_mind.gang.gang_score()]</strong>"
		text += "<br>"
		boutput(world, text)

	if (!check_winner())
		boutput(world, "<h2><strong>The round was a draw!</strong></h2>")

	else
		var/mind/winner = check_winner()
		if (istype(winner))
			boutput(world, "<h2><strong>[winner.gang.gang_name], led by [winner.current.real_name], won the round!</strong></h2>")

	..() // Admin-assigned antagonists or whatever.

/game_mode/gang/proc/generate_gang(mind/leaderMind)
	leaderMind.gang = new /gang
...
/game_mode/gang/proc/check_winner()
	var/mind/current_winner = null

	for (var/mind/leader_mind in leaders) // Find the highest score
		if (!current_winner)
			current_winner = leader_mind
		else if (current_winner.gang.gang_score() < leader_mind.gang.gang_score())
			current_winner = leader_mind

	for (var/mind/leader_mind in leaders) // See if two gangs have the highest score ie. it's a draw
...
/obj/ganglocker
	...
	proc/get_gear(var/mob/living/carbon/human/user)
		if (user.mind.gang == gang) // Needs new gear? Maybe!
			var/hasitem1 = 0
			var/hasitem2 = 0
			var/haspaint = 0
			var/hasheadset = 0
	...
	proc/update_icon()
		overlays = null
		if (can_be_joined())
			overlays += image('icons/obj/closet.dmi', "greenlight")
		else
			overlays += image('icons/obj/closet.dmi', "redlight")
