...
/game_mode/sandbox/announce()
	boutput(world, "<strong>The current game mode is - Sandbox!</strong>")
	boutput(world, "<strong>Build your own station with the sandbox-panel command!</strong>")

/game_mode/sandbox/pre_setup()
...
/game_mode/sandbox/check_finished()
	return FALSE
...
/hSB
	...
	proc
		update()
			var/hsbpanel = "<center><strong>h_Sandbox Panel</strong></center><hr>"
			if (admin)
				hsbpanel += "<strong>Administration Tools:</strong><br>"
				hsbpanel += "- <a href=\"?\ref[src];hsb=hsbtobj\">Toggle Object Spawning</a><br><br>"
			hsbpanel += "<strong>Regular Tools:</strong><br>"
			for (var/T in hrefs)
		...
		if (href_list["hsb"])
			switch(href_list["hsb"])
				if ("hsbtobj")
					if (!admin) return
					if (hsboxspawn)
						boutput(world, "<strong>Sandbox:  [usr.key] has disabled object spawning!</strong>")
						hsboxspawn = 0
						return
					if (!hsboxspawn)
						boutput(world, "<strong>Sandbox:  [usr.key] has enabled object spawning!</strong>")
						hsboxspawn = 1
						return
				if ("hsbsuit")
				...
				if ("hsbairlock")
					...
					hsb.set_loc(usr.loc)
					boutput(usr, "<strong>Sandbox:  Created an airlock.")
				if ("hsbcanister")
