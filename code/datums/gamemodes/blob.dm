...
/game_mode/blob/announce()
	boutput(world, "<strong>The current game mode is - <font color='green'>Blob</font>!</strong>")
	boutput(world, "<strong>A dangerous alien organism is rapidly spreading throughout the station!</strong>")
	boutput(world, "You must kill it all while minimizing the damage to the station.")

/game_mode/blob/pre_setup()
	..()
	var/num_players = 0
...
/game_mode/blob/post_setup()
	..()
	emergency_shuttle.disabled = 0
	for (var/mind/blob in traitors)
		if (istype(blob))
			bestow_objective(blob,/objective/specialist/blob)
...
/game_mode/blob/declare_completion()
	var/list/blobs = list()
	for (var/mind/M in traitors)
		if (!M)
			continue
	...
	if (!blobs.len)
		boutput(world, "<span style='font-size:20px; color:red'><strong>Station victory!</strong> - All blobs have been exterminated!")
	else
		boutput(world, "<span style='font-size:20px; color:red'><strong>Blob victory!</strong> - The crew has failed to stop the overmind! The station is lost to the blob!")

	..()

/game_mode/blob/proc/get_possible_blobs(num_blobs=1)
	...
	if (candidates.len < num_blobs)
		logTheThing("debug", null, null, "<strong>Enemy Assignment</strong>: Only [candidates.len] players with be_blob set to yes were ready. We need [num_blobs], so including players who don't want to be blobstart in the pool.")
		for (var/mob/new_player/player in mobs)
			if ((player.client) && (player.ready) && !(player.mind in traitors) && !(player.mind in token_players) && !candidates.Find(player.mind))
				candidates += player.mind
