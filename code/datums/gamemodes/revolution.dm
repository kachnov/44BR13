// To add a rev to the list of revolutionaries, make sure it's rev (with if (istype(ticker.mode, /game_mode/revolution))),
...
/game_mode/revolution
	...
	var/list/mind/head_revolutionaries = list()
	var/list/mind/revolutionaries = list()
	var/finished = 0
...
/game_mode/revolution/pre_setup()
	...
	while (rev_number > 0)
		var/mind/revP = pick(revs_possible)
...
/game_mode/revolution/post_setup()

	var/list/heads = get_living_heads()

	if (!head_revolutionaries || !heads)
		boutput(world, "<strong><span style=\"color:red\">Not enough players for revolution game mode. Restarting world in 5 seconds.</span></strong>")
		sleep(50)
		Reboot_server()
		return

	for (var/mind/rev_mind in head_revolutionaries)
		for (var/mind/head_mind in heads)
			var/objective/regular/assassinate/rev_obj = new
	...
	for (var/mind/rev_mind in head_revolutionaries)
		...
		for (var/objective/objective in rev_mind.objectives)
			boutput(rev_mind.current, "<strong>Objective #[obj_count]</strong>: [objective.explanation_text]")
			obj_count++
...
/game_mode/revolution/proc/equip_revolutionary(mob/living/carbon/human/rev_mob)
	...
	rev_mob.show_text("You've been supplied with a <strong>flash</strong> in your [the_slot] with which to convert others to the cause!", "blue")
	return

/game_mode/revolution/send_intercept()
...
/game_mode/revolution/check_win()
...
/game_mode/revolution/check_finished()
	if (finished != 0)
		return TRUE
	else
		return FALSE

/game_mode/revolution/proc/add_revolutionary(mind/rev_mind)
...
/game_mode/revolution/proc/remove_revolutionary(mind/rev_mind)
	if (!rev_mind.current)
		return
	...
	if (rev_mind in revolutionaries)
		...
		for (var/mob/living/M in view(rev_mind.current))
			M.show_text("<strong>[rev_mind.current] looks like they just remembered their real allegiance!</strong>", "red")

	return

/game_mode/revolution/proc/update_all_rev_icons()
	var/list/update_me = list()
	update_me.Add(head_revolutionaries)
	update_me.Add(revolutionaries)

	for (var/mind/M in update_me)
		if (M.current)
			M.current.antagonist_overlay_refresh(1, 0)

	return

/game_mode/revolution/proc/update_rev_icons_added(mind/rev_mind)
	var/list/update_me = list()
	update_me.Add(head_revolutionaries)
	update_me.Add(revolutionaries) // Includes rev_mind.
...
/game_mode/revolution/proc/update_rev_icons_removed(mind/rev_mind)
...
/game_mode/revolution/proc/get_possible_revolutionaries()
	...
	if (candidates.len < 1)
		logTheThing("debug", null, null, "<strong>Enemy Assignment</strong>: Not enough players with be_misc set to yes, so we're adding players who don't want to be rev leaders to the pool.")
		for (var/mob/new_player/player in mobs)
			if ((player.client) && (player.ready) && !(player.mind in head_revolutionaries) && !(player.mind in token_players) && !candidates.Find(player.mind))
				candidates += player.mind
...
/game_mode/revolution/proc/get_living_heads()
	var/list/heads = list()
...
/game_mode/revolution/proc/get_all_heads()
	var/list/heads = list()
...
/game_mode/revolution/proc/get_unconvertables()
	var/list/ucs = list()
...
/game_mode/revolution/proc/check_rev_victory()
	...
	for (var/mind/head_mind in head_check)
...
/game_mode/revolution/proc/check_heads_victory()
	if (endthisshit == 1)
		return FALSE
...
/game_mode/revolution/proc/check_centcom_victory()
...
/game_mode/revolution/declare_completion()

	var/text = ""
	if (finished == 1)
		boutput(world, "<span style=\"color:red\"><FONT size = 3><strong> The heads of staff were killed or abandoned the station! The revolutionaries win!</strong></FONT></span>")
	else if (finished == 2)
		boutput(world, "<span style=\"color:red\"><FONT size = 3><strong> The heads of staff managed to stop the revolution!</strong></FONT></span>")
	else if (finished == 3)
		boutput(world, "<span style=\"color:red\"><FONT size = 3><strong> Everyone was terminated! CentCom wins!</strong></FONT></span>")
	...
	boutput(world, "<FONT size = 2><strong>The head revolutionaries were: </strong></FONT>")
	for (var/mind/rev_mind in head_revolutionaries)
		text = ""
		if (rev_mind.current)
	...
	text = ""
	boutput(world, "<FONT size = 2><strong>The converted revolutionaries were: </strong></FONT>")
	for (var/mind/rev_nh_mind in revolutionaries)
	...
	boutput(world, "<FONT size = 2><strong>The heads of staff were: </strong></FONT>")
	var/list/heads = list()
	heads = get_all_heads()
	for (var/mind/head_mind in heads)
		text = ""
