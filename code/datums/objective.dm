...
/objective/regular/assassinate
	...
	set_up()
		...
		for (var/mind/possible_target in ticker.minds)
	...
	check_completion()
		if (target && target.current)
			...
			return TRUE
	proc/create_objective_string(var/mind/target)
		if (!(target && target.current))
...
/proc/create_fluff(var/mind/target)
...
/objective/regular/gimmick
	...
	set_up()
		...
		explanation_text = "[gimmick_text] <em>This objective is not tracked and will automatically succeed, so just have fun with it!</em>"
...
/objective/regular/bonsaitree
	...
	check_completion()
		...
		for (var/obj/shrub/captainshrub/T in cap_quarters)
			our_tree = T
		if (!our_tree)
			return TRUE  // Somebody deleted it somehow, I suppose?
		else if (our_tree && our_tree.destroyed == 1)
			return TRUE
		else
			return FALSE

/objective/regular/damage_area
	...
	set_up()
		...
		while (area_attempt < 4 && (!target_area || !istype(target_area)))
...
/objective/regular/destroy_equipment
...
/objective/regular/job_genocide
...
/objective/regular/traitor_supremacy
	...
	check_completion()
		for (var/mind/M in ticker.mode.traitors + ticker.mode.Agimmicks)
			if (!M.current)
				continue
			if (M.current.stat != 2)
				return FALSE
...
/objective/regular/force_evac_time
	var/time = 45
...
/objective/regular/destroy_outpost
	explanation_text = "Activate the computer mainframe's self-destruct charge."

	check_completion()
		if (outpost_destroyed == 1) return TRUE
		else return FALSE

/objective/regular/multigrab
...
/objective/specialist/xenomorph
/objective/specialist/xenomorph/regicide

/objective/specialist/xenomorph/regicide/set_up()
	explanation_text = "Kill at least one Xenomorph Praetorian."

/objective/specialist/xenomorph/regicide/check_completion()
	var/game_mode/_44BR13/mode = ticker.mode
	return mode.dead_xenomorph_praetorians.len > 0

/objective/specialist/nuclear
...
/objective/specialist/conspiracy
	...
	check_completion()
		...
		var/game_mode/spy/spymode = ticker.mode
		for (var/mind/mindCheck in spymode.leaders)
			if (mindCheck == owner)
				continue
...
/objective/specialist/drinkblood
...
/objective/specialist/predator/trophy
...
/objective/specialist/stealth
	var/min_score
	var/score = 0
	var/list/mind/safe_minds = list()
...
/objective/specialist/gang
	...
	check_completion()
		...
		var/game_mode/gang/gangmode = ticker.mode
		for (var/mind/mindCheck in gangmode.leaders)
			if (mindCheck == owner)
				continue
...
/objective/specialist/wraith/absorb
	var/absorbs = 0
	var/absorb_target
...
/objective/specialist/wraith/murder
	var/mind/target
	var/targetname
...
/objective/specialist/wraith/murder/absorb
	var/success = 0
	setText()
...
/objective/specialist/wraith/prevent
	var/max_escapees
...
/objective/specialist/wraith/travel
...
/objective/specialist/wraith/survive
...
/objective/specialist/wraith/flawless
...
/objective/specialist/werewolf/feed
	var/feed_count = 0
...
/objective/specialist/ruin_xmas
...
/objective_set
	var/list/objective_list = list(/objective/regular/gimmick)
	var/list/escape_choices = list(/objective/escape,
	/objective/escape/survive,
	/objective/escape/hijack,
	/objective/escape/kamikaze)
...
/objective_set/changeling
	objective_list = list(/objective/specialist/absorb)
	escape_choices = list(/objective/escape,
	/objective/escape/hijack)

/objective_set/vampire
	objective_list = list(/objective/specialist/drinkblood)
	escape_choices = list(/objective/escape,
	/objective/escape/hijack)

/objective_set/grinch
	objective_list = list(/objective/specialist/ruin_xmas)
	escape_choices = list(/objective/escape,
	/objective/escape/survive,
	/objective/escape/hijack,
	/objective/escape/kamikaze)

/objective_set/predator
	objective_list = list(/objective/specialist/predator/trophy)
	escape_choices = list(/objective/escape/survive)

/objective_set/werewolf
	objective_list = list(/objective/specialist/werewolf/feed)
	escape_choices = list(/objective/escape/survive)

/objective_set/blob
	objective_list = list(/objective/specialist/blob)
	escape_choices = list(/objective/escape/survive)
...
/objective_set/traitor/easy/triple_assassinate
	objective_list = list(/objective/regular/assassinate,
	/objective/regular/assassinate,
	/objective/regular/assassinate)
	escape_choices = list(/objective/escape,
	/objective/escape/survive)

/objective_set/traitor/easy/genocide
	objective_list = list(/objective/regular/job_genocide)
	escape_choices = list(/objective/escape,
	/objective/escape/survive)

/objective_set/traitor/easy/massacre
	objective_list = list(/objective/regular/assassinate,
	/objective/regular/assassinate,
	/objective/regular/assassinate,
	/objective/regular/assassinate,
	/objective/regular/assassinate,
	/objective/regular/assassinate)
	escape_choices = list(/objective/escape/kamikaze)

/objective_set/traitor/easy/kill_heads
	objective_list = list(/objective/regular/kill_heads)
	escape_choices = list(/objective/escape/kamikaze,
	/objective/escape/survive)

/objective_set/traitor/easy/sabotage
	objective_list = list(/objective/regular/damage_area,
	/objective/regular/destroy_equipment)
	escape_choices = list(/objective/escape)

/objective_set/traitor/easy/havoc
	objective_list = list(/objective/regular/damage_area,
	/objective/regular/destroy_equipment,
	/objective/regular/job_genocide)
	escape_choices = list(/objective/escape/survive,
...
/objective_set/traitor/easy/supremacy_and_hijack
	objective_list = list(/objective/regular/traitor_supremacy)
	escape_choices = list(/objective/escape/hijack)

/objective_set/traitor/easy/damage_and_hijack
	objective_list = list(/objective/regular/damage_area)
	escape_choices = list(/objective/escape/hijack)

/objective_set/traitor/easy/steal_and_hijack
	objective_list = list(/objective/regular/steal)
	escape_choices = list(/objective/escape/hijack)

/objective_set/traitor/easy/noclones_and_hijack
	objective_list = list(/objective/regular/no_clones)
	escape_choices = list(/objective/escape/hijack)

/objective_set/traitor/easy/steal_ai_brain
	objective_list = list(/objective/regular/aikill)
	escape_choices = list(/objective/escape)

/objective_set/traitor/easy/borg_death
	objective_list = list(/objective/regular/borgdeath)
	escape_choices = list(/objective/escape)

/objective_set/traitor/easy/dead_means_dead
	objective_list = list(/objective/regular/borgdeath,
	/objective/regular/no_clones)
	escape_choices = list(/objective/escape/survive,
	/objective/escape)

/objective_set/traitor/easy/kill_all_silicons
	objective_list = list(/objective/regular/aikill,
	/objective/regular/borgdeath)
	escape_choices = list(/objective/escape/survive)

/objective_set/traitor/easy/bonsai_tree
	objective_list = list(/objective/regular/bonsaitree,
	/objective/regular/gimmick)
	escape_choices = list(/objective/escape,
	/objective/escape/survive)
...
/objective_set/traitor/hard
	objective_list = list(/objective/regular/assassinate,
	/objective/regular/gimmick)
	escape_choices = list(/objective/escape)

/objective_set/traitor/hard/triple_assassinate_and_hijack
	objective_list = list(/objective/regular/assassinate,
	/objective/regular/assassinate,
	/objective/regular/assassinate,
	/objective/regular/no_clones)
...
/objective_set/traitor/hard/double_genocide
	objective_list = list(/objective/regular/job_genocide,
	/objective/regular/job_genocide)
	escape_choices = list(/objective/escape)

/objective_set/traitor/hard/kill_all_silicons
	objective_list = list(/objective/regular/aikill,
	/objective/regular/borgdeath)
	escape_choices = list(/objective/escape)

/objective_set/traitor/hard/rampage
	objective_list = list(/objective/regular/assassinate,
	/objective/regular/damage_area,
	/objective/regular/damage_area,
	/objective/regular/destroy_equipment,
...
/objective_set/traitor/xeno 
/objective_set/traitor/xeno/regicide
		objective_list = list(/objective/specialist/xenomorph/regicide)