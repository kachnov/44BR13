...
/wage_system
	...
	New()
		...
		for (var/occupation in occupations.assistant_jobs)
			// If its not already in the list add it
			if (!jobs.Find(occupation))
				// 0.0 is the default wage
				jobs[occupation] = 0.0
...
//		jobs["Attorney at Space-Law"] = 500
		...
		time_until_lotto = ( ticker ? ticker.round_elapsed_ticks : 0 ) + time_between_lotto
		time_until_payday = ( ticker ? ticker.round_elapsed_ticks : 0 ) + time_between_paydays

	// This returns the time left in seconds
	proc/timeleft()
		var/timeleft = time_until_payday - ticker.round_elapsed_ticks
		...
		if (timeleft <= 0)
			payday()
			time_until_payday = ticker.round_elapsed_ticks + time_between_paydays
			return FALSE
	...
	proc/checkLotteryTime()
		...
		var/timeleft = time_until_lotto - ticker.round_elapsed_ticks

		if (timeleft <= 0)
			lotteryDay()
			time_until_lotto = ticker.round_elapsed_ticks + time_between_lotto
			return FALSE
...
/obj/machinery/computer/ATM
	...
	var/data/record/accessed_record = null
	...
	attackby(var/obj/item/I as obj, user as mob)
		...
		if (istype(I, /obj/item/spacecash))
			if (accessed_record)
				...
				accessed_record.fields["current_money"] += I.amount
				I.amount = 0
				qdel(I)
		...
		if (istype(I, /obj/item/lotteryTicket))
			if (accessed_record)
				...
				if (I:winner)
					...
					accessed_record.fields["current_money"] += I:winner
		...
		switch(state)
			...
			if (STATE_LOGGEDIN)
				if (!accessed_record)
					...
					if (scan)
						dat += "<BR>Your balance is: $ [accessed_record.fields["current_money"]]."
						dat += "<BR>Your balance on your card is: $ [scan.money]"
	...
	Topic(href, href_list)
		...
		if (href_list["type"])
			if (href_list["type"] == "E")
				if (text2num(pin) == scan.pin)
					if (TryToFindRecord())
						state = STATE_LOGGEDIN
						pin = null
					else
						pin = "RECORD NOT FOUND"
				else
					pin = "ERROR PIN NOT CORRECT"
			else
				if (href_list["type"] == "R")
					pin = null
				else
					pin += href_list["type"]
					if (length(pin) > 4)
						pin = "ERROR PIN IS ONLY 4 DIGITS"
		...
		switch(href_list["operation"])
			...
			if ("withdraw")
				...
				if (amount < 1)
					...
					return
				if (amount > accessed_record.fields["current_money"])
					boutput(usr, "<span style=\"color:red\">Insufficient funds in account.</span>")
				else
					scan.money += amount
					accessed_record.fields["current_money"] -= amount
			...
			if ("withdrawcash")
				...
				if (amount > accessed_record.fields["current_money"])
					...
					accessed_record.fields["current_money"] -= amount
					new /obj/item/spacecash(loc, amount )
			...
			if (authenticated)
				switch(screen)
					if (1.0)
						...
						<BR>
						<strong>Current Payroll Cost:</strong> $[payroll]
						<BR>
		...
		if ((usr.contents.Find(src) || (in_range(src, usr) && istype(loc, /turf))) || (istype(usr, /mob/living/silicon)))
			...
			if (authenticated)
				if (href_list["list"])
					...
					var/data/record/R = locate(href_list["Fname"])
					...
					if ((!( t1 ) || !( authenticated ) || usr.stat || usr.restrained() || (!in_range(src, usr) && (!istype(usr, /mob/living/silicon))))) return
					R.fields["name"] = t1
				else if (href_list["Fjob"])
					var/data/record/R = locate(href_list["Fjob"])
					var/t1 = input("Please input name:", "Secure. records", R.fields["job"], null)  as null|text
					t1 = copytext(html_encode(t1), 1, MAX_MESSAGE_LEN)
					if ((!( t1 ) || !( authenticated ) || usr.stat || usr.restrained() || (!in_range(src, usr) && (!istype(usr, /mob/living/silicon))))) return
					R.fields["job"] = t1
				else if (href_list["Fwage"])
					var/data/record/R = locate(href_list["Fwage"])
					var/t1 = input("Please input wage:", "Secure. records", R.fields["wage"], null)  as null|num
					if ((!( authenticated ) || usr.stat || usr.restrained() || (!in_range(src, usr) && (!istype(usr, /mob/living/silicon))))) return
					if (t1 < 0)
						t1 = 0
					...
					var/data/record/R = locate(href_list["Fmoney"])
					var/avail = null
		...
		switch(state)
			...
			if (STATE_LOGGEDIN)
				if (!accessed_record)
					...
					if (scan)
						dat += "<BR><BR>Your balance is: $ [accessed_record.fields["current_money"]]."
						dat += "<BR>Your balance on your card is: $ [scan.money]"
						dat += "<BR><BR><A HREF='?src=\ref[src];operation=withdraw'>Withdraw to Card</A>"
						dat += "<BR><A HREF='?src=\ref[src];operation=withdrawcash'>Withdraw Cash</A>"
						dat += "<BR><A HREF='?src=\ref[src];operation=deposit'>Deposit from Card</A>"
		...
		switch(href_list["operation"])
			...
			if ("buy")
				if (accessed_record.fields["current_money"] >= 100)
					accessed_record.fields["current_money"] -= 100
					boutput(usr, "<span style=\"color:red\">Ticket being dispensed. Good luck!</span>")

					new /obj/item/lotteryTicket(loc)
