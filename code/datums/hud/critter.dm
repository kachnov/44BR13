...
/hud/critter
	...
	New(M)
		...
		for (var/i = 1, i <= master.hands.len, i++)
			var/curr = hand_s + i - 1
			var/handHolder/HH = master.hands[i]
		...
		for (var/i = 1, i <= master.equipment.len, i++)
			var/equipmentHolder/EH = master.equipment[i]
			var/SL = loc_left()
		...
		if (copytext(id, 1, 5) == "hand")
			...
			switch(id)
				...
				if ("health")
					...
					if (isxenomorphlarva(master))
						var/mob/living/critter/xenomorph_larva/XL = master
						boutput(master, "<span style='color:blue'>Your evolution progress: <strong>[XL.evolution_progress()]</strong></span>")
