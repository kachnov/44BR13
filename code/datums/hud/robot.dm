/hud/robot
	...
	New(M)
		master = M
		boxes = create_screen("boxes", "Storage", 'icons/mob/screen1.dmi', "block", "1, 10 to 1, 1")
		remove_screen(boxes)
		prev = create_screen("prev", "Previous Page", 'icons/mob/screen1.dmi', "up_dis", "1, 10", HUD_LAYER+1)
		remove_screen(prev)
		next = create_screen("next", "Next Page", 'icons/mob/screen1.dmi', "down", "1, 2", HUD_LAYER+1)
	...
	clicked(id)
		...
		switch (id)
			...
			if ("pulling")
				...
				update_pulling()
			if ("upgrades")
				set_show_upgrades(!show_upgrades)
			if ("upgrade1") // this is horrifying
				if (last_upgrades.len >= 1)
					master.activate_upgrade(last_upgrades[1])
			if ("upgrade2")
				if (last_upgrades.len >= 2)
					master.activate_upgrade(last_upgrades[2])
			if ("upgrade3")
				if (last_upgrades.len >= 3)
					master.activate_upgrade(last_upgrades[3])
			if ("upgrade4")
				if (last_upgrades.len >= 4)
					master.activate_upgrade(last_upgrades[4])
			if ("upgrade5")
				if (last_upgrades.len >= 5)
					master.activate_upgrade(last_upgrades[5])
			if ("upgrade6")
				if (last_upgrades.len >= 6)
					master.activate_upgrade(last_upgrades[6])
			if ("upgrade7")
				if (last_upgrades.len >= 7)
					master.activate_upgrade(last_upgrades[7])
			if ("upgrade8")
				if (last_upgrades.len >= 8)
					master.activate_upgrade(last_upgrades[8])
			if ("upgrade9")
				if (last_upgrades.len >= 9)
					master.activate_upgrade(last_upgrades[9])
			if ("upgrade10")
				if (last_upgrades.len >= 10)
					master.activate_upgrade(last_upgrades[10])
	proc
