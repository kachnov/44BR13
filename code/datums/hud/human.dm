...
/hud/human
	...
	New(M)
		...
		spawn (0)
			...
			invtoggle = create_screen("invtoggle", "toggle inventory", icon_hud, "invtoggle", "CENTER-5, SOUTH", HUD_LAYER+1)
			belt = create_screen("belt", "belt", icon_hud, "belt", ui_belt, HUD_LAYER+1)
			storage1 = create_screen("storage1", "pocket", icon_hud, "pocket", ui_storage1, HUD_LAYER+1)
			storage2 = create_screen("storage2", "pocket", icon_hud, "pocket", ui_storage2, HUD_LAYER+1)
			back = create_screen("back", "back", icon_hud, "back", ui_back, HUD_LAYER+1)
			lhand = create_screen("lhand", "left hand", icon_hud, "handl0", "CENTER, SOUTH", HUD_LAYER+1)
			rhand = create_screen("rhand", "right hand", icon_hud, "handr0", "CENTER+1, SOUTH", HUD_LAYER+1)
			throwing = create_screen("throw", "throw mode", icon_hud, "throw0", "CENTER+2, SOUTH", HUD_LAYER+1)
			intent = create_screen("intent", "action intent", icon_hud, "intent-help", "CENTER+3, SOUTH", HUD_LAYER+1)
			mintent = create_screen("mintent", "movement mode", icon_hud, "move-run", "CENTER+5, SOUTH", HUD_LAYER+1)
			resist = create_screen("resist", "resist", icon_hud, "resist", "CENTER+5, SOUTH", HUD_LAYER+1)
			pulling = create_screen("pull", "pulling", icon_hud, "pull0", "CENTER+6, SOUTH", HUD_LAYER+1)
			resting = create_screen("rest", "resting", icon_hud, "rest0", "CENTER+6, SOUTH", HUD_LAYER+1)

			inventory_bg += create_screen("shoes", "shoes", icon_hud, "shoes", ui_shoes, HUD_LAYER+1)
			inventory_bg += create_screen("gloves", "gloves", icon_hud, "gloves", ui_gloves, HUD_LAYER+1)
			inventory_bg += create_screen("id", "ID", icon_hud, "id", ui_id, HUD_LAYER+1)
			inventory_bg += create_screen("under", "clothing", icon_hud, "center", ui_clothing, HUD_LAYER+1)
			inventory_bg += create_screen("suit", "suit", icon_hud, "armor", ui_suit, HUD_LAYER+1)
			inventory_bg += create_screen("glasses", "glasses", icon_hud, "glasses", ui_glasses, HUD_LAYER+1)
			inventory_bg += create_screen("ears", "ears", icon_hud, "ears", ui_ears, HUD_LAYER+1)
			inventory_bg += create_screen("mask", "mask", icon_hud, "mask", ui_mask, HUD_LAYER+1)
			inventory_bg += create_screen("head", "head", icon_hud, "hair", ui_head, HUD_LAYER+1)

			health = create_screen("health","Health", icon_hud, "health0", "EAST, NORTH", HUD_LAYER, tooltipTheme = "healthDam healthDam0")
			health.desc = "You feel fine."

			health_brute = create_screen("mbrute","Brute Damage", icon_hud, "blank", "EAST, NORTH", HUD_LAYER, tooltipTheme = "healthDam healthDam0")
			health_burn = create_screen("mburn","Burn Damage", icon_hud, "blank", "EAST, NORTH", HUD_LAYER, tooltipTheme = "healthDam healthDam0")
			health_tox = create_screen("mtox","Toxin Damage", icon_hud, "blank", "EAST, NORTH", HUD_LAYER, tooltipTheme = "healthDam healthDam0")
			health_oxy = create_screen("moxy","Oxygen Damage", icon_hud, "blank", "EAST, NORTH", HUD_LAYER, tooltipTheme = "healthDam healthDam0")

			bleeding = create_screen("bleeding","Bleed Warning", icon_hud, "blood0", "EAST, NORTH-1", HUD_LAYER, tooltipTheme = "healthDam healthDam0")
			bleeding.desc = "This indicator warns that you are currently bleeding. You will die if the situation is not remedied."

			stamina = create_screen("stamina","Stamina", icon_hud, "stamina", "EAST-1, NORTH", HUD_LAYER, tooltipTheme = "stamina")
			if (master.stamina_bar)
				stamina.desc = master.stamina_bar.getDesc(master)

			bodytemp = create_screen("bodytemp","Temperature", icon_hud, "temp0", "EAST-2, NORTH", HUD_LAYER, tooltipTheme = "tempInd tempInd0")
			bodytemp.desc = "The temperature feels fine."

			oxygen = create_screen("oxygen","Suffocation Warning", icon_hud, "oxy0", "EAST-3, NORTH", HUD_LAYER, tooltipTheme = "statusOxy")
			...
			fire = create_screen("fire","Fire Warning", icon_hud, "fire0", "EAST-4, NORTH", HUD_LAYER, tooltipTheme = "statusFire")
			...
			toxin = create_screen("toxin","Toxic Warning",icon_hud, "toxin0", "EAST-5, NORTH", HUD_LAYER, tooltipTheme = "statusToxin")
			toxin.desc = "This indicator warns that you are poisoned. You will take toxic damage until the situation is remedied."

			rad = create_screen("rad","Radiation Warning", icon_hud, "rad0", "EAST-6, NORTH", HUD_LAYER, tooltipTheme = "statusRad")
			...
			ability_toggle = create_screen("ability", "Toggle Ability Hotbar", icon_hud, "ability-1", "CENTER-6, SOUTH", HUD_LAYER)
	...
	proc/update_ability_hotbar()
		...
		if (current_ability_set == 2) // genetics
			var/bioEffect/power/P
			for (var/ID in master.bioHolder.effects)
				P = master.bioHolder.GetEffect(ID)
				if (!istype(P, /bioEffect/power) || !istype(P.ability) || !istype(P.ability.object))
					continue
				var/targetable/geneticsAbility/POWER = P.ability
