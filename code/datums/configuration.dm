...
/configuration/New()
	var/list/L = subtypesof(/game_mode)
	for (var/T in L)
		// I wish I didn't have to instance the game modes in order to look up
		// their information, but it is the only way (at least that I know of).
		var/game_mode/M = new T()
...
/configuration/proc/load(filename)
	...
	for (var/t in CL)
		...
		switch (name)
			...
			if ("visible")
				config.visible = text2num(value)

				// world is visible by default - setting it to 1 may break things
				if (!config.visible)
					world.visibility = config.visible
...
/configuration/proc/pick_mode(mode_name)
	// I wish I didn't have to instance the game modes in order to look up
	// their information, but it is the only way (at least that I know of).
	for (var/T in (subtypesof(/game_mode)))
		var/game_mode/M = new T()
...
/configuration/proc/pick_random_mode()
	var/total = 0
	var/list/accum = list()

	for (var/M in modes)
		total += probabilities[M]
		accum[M] = total
	...
	var/mode_name = null
	for (var/M in modes)
		if (probabilities[M] > 0 && accum[M] >= r)
			mode_name = M
			break
	...
	return pick_mode(mode_name)

/configuration/proc/get_used_mode_names()
	var/list/names = list()
