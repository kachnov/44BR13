/manufacture
	...
	proc/sanity_check()
		if (item_paths.len != item_names.len || item_paths.len != item_amounts.len || item_names.len != item_amounts.len)
			logTheThing("debug", null, null, "<strong>Manufacturer:</strong> [name] schematic requirement lists not properly configured")
			qdel(src)
			return
		if (!item_outputs.len)
			logTheThing("debug", null, null, "<strong>Manufacturer:</strong> [name] schematic output list not properly configured")
			qdel(src)
			return
	...
	proc/modify_output(var/obj/machinery/manufacturer/M, var/atom/A,var/list/materials)
		...
		if (apply_material && materials.len > 0)
...
/manufacture/mechanics
	...
	modify_output(var/obj/machinery/manufacturer/M, var/atom/A)
		...
		if (istype(A,/obj/item/electronics/frame))
			...
			if (ispath(frame_path))
				F.name = "[name] frame"
				F.store_type = frame_path
				F.viewstat = 2
				F.secured = 2
				F.icon_state = "dbox"
			else
...
/manufacture/cable
	...
	modify_output(var/obj/machinery/manufacturer/M, var/atom/A,var/list/materials)
		...
		for (var/mat_id in materials)
			var/material/cand = getCachedMaterial(mat_id)
			if (!cand)
				continue
...
/manufacture/pod/armor_industrial
...
/manufacture/pod/weapon/ltlaser
