/preferences
	...
	var/traitPreferences/traitPreferences = new
	...
	proc/update_preview_icon()
		...
		preview_icon = new /icon('icons/mob/human.dmi', "body_[gender == MALE ? "m" : "f"]")

		// Skin tone
		if (AH.s_tone >= 0)
			preview_icon.Blend(rgb(AH.s_tone, AH.s_tone, AH.s_tone), ICON_ADD)
		else
			preview_icon.Blend(rgb(-AH.s_tone,  -AH.s_tone,  -AH.s_tone), ICON_SUBTRACT)
	...
	proc/ShowChoices(mob/user)
		...
		dat += "<strong>Profile Name:</strong> "
		dat += "<a href=\"byond://?src=\ref[user];preferences=1;profile_name=input\"><strong>[profile_name ? profile_name : "Unnamed"]</strong></a> "

		dat += "<hr><strong>Character Name:</strong> "
		dat += "<a href=\"byond://?src=\ref[user];preferences=1;real_name=input\"><strong>[real_name]</strong></a> "

		dat += "<br><strong>Random name? </strong>"
		dat += "(&reg; = <a href=\"byond://?src=\ref[user];preferences=1;b_random_name=1\">[be_random_name ? "Yes" : "No"]</a>)"
		dat += "<br>"

		dat += "<strong>Random appearance? </strong>"
		dat += "(&reg; = <a href=\"byond://?src=\ref[user];preferences=1;b_random_look=1\">[be_random_look ? "Yes" : "No"]</a>)"
		dat += "<br>"

		dat += "<strong>Gender:</strong> <a href=\"byond://?src=\ref[user];preferences=1;gender=input\"><strong>[gender == MALE ? "Male" : "Female"]</strong></a><br>"
		dat += "<strong>Age:</strong> <a href='byond://?src=\ref[user];preferences=1;age=input'>[age]</a><br>"
		dat += "<strong>Blood Type:</strong> <a href='byond://?src=\ref[user];preferences=1;blType=input'>[random_blood ? "Random" : blType]</a><br>"
		dat += "<strong>Bank PIN:</strong> <a href='byond://?src=\ref[user];preferences=1;pin=input'>[pin ? pin : "Random"]</a> (<a href=\"byond://?src=\ref[user];preferences=1;random_pin=1\">&reg;</a>)<br>"

		var/favoriteJob = find_job_in_controller_by_string(job_favorite)
		dat += "<strong><a href=\"byond://?src=\ref[user];preferences=1;jobswindow=1\">Occupation Choices</a>[ favoriteJob ? " (Fav: [favoriteJob])" : "" ]</strong><br>"
		dat += "<strong><a href=\"byond://?src=\ref[user];preferences=1;traitswindow=1\">Trait Choices</a></strong>"
		...
		if (AH)
			dat += "<a href='byond://?src=\ref[user];preferences=1;s_tone=input'><strong>Skin Tone:</strong></a> [-AH.s_tone + 35]/220<br>"

			dat += "<a href='byond://?src=\ref[user];preferences=1;underwear=input'><strong>Underwear:</strong></a> [AH.underwear] "
			dat += "<a href='byond://?src=\ref[user];preferences=1;underwear_color=input'><font face=\"fixedsys\" size=\"3\" color=\"[AH.u_color]\"><strong>#</strong></font></a><br>"

			dat += "<a href='byond://?src=\ref[user];preferences=1;eyes=input'><strong>Eye Color:</strong> <font face=\"fixedsys\" size=\"3\" color=\"[AH.e_color]\"><strong>#</strong></font></a><br>"

			dat += "<a href='byond://?src=\ref[user];preferences=1;customization_first=input'><strong>Bottom Detail:</strong></a> [AH.customization_first] "
			dat += "<a href='byond://?src=\ref[user];preferences=1;hair=input'><font face=\"fixedsys\" size=\"3\" color=\"[AH.customization_first_color]\"><strong>#</strong></font></a><br>"

			dat += "<a href='byond://?src=\ref[user];preferences=1;customization_second=input'><strong>Mid Detail:</strong></a> [AH.customization_second] "
			dat += "<a href='byond://?src=\ref[user];preferences=1;facial=input'><font face=\"fixedsys\" size=\"3\" color=\"[AH.customization_second_color]\"><strong>#</strong></font></a><br>"

			dat += "<a href='byond://?src=\ref[user];preferences=1;customization_third=input'><strong>Top Detail:</strong></a> [AH.customization_third] "
			dat += "<a href='byond://?src=\ref[user];preferences=1;detail=input'><font face=\"fixedsys\" size=\"3\" color=\"[AH.customization_third_color]\"><strong>#</strong></font></a><br>"
		...
		if ((user && ismob(user)) && user.client && user.client.mentor_authed) // undefined variable /client/var/client
			dat += "<strong>Display Mentorhelps?</strong> <a href=\"byond://?src=\ref[user];preferences=1;toggle_mentorhelp=1\">[(see_mentor_pms ? "Yes" : "No")]</a></strong><br>"

		dat += "<strong>HUD style: <a href =\"byond://?src=\ref[user];preferences=1;hud_style=1\"><img style=\"-ms-interpolation-mode:nearest-neighbor;\" src=hud_preview.png></a></strong> | "
		dat += "<strong>Targeting cursor: <a href =\"byond://?src=\ref[user];preferences=1;tcursor=1\"><img style=\"-ms-interpolation-mode:nearest-neighbor;\" src=tcursor.png></a></strong><br>"
		dat += "<strong>Display OOC chat?</strong> <a href=\"byond://?src=\ref[user];preferences=1;listen_ooc=1\">[(listen_ooc ? "Yes" : "No")]</a></strong><br>"
		dat += "<strong>Display LOOC chat?</strong> <a href=\"byond://?src=\ref[user];preferences=1;listen_looc=1\">[(listen_looc ? "Yes" : "No")]</a></strong><br>"
		dat += "<strong>View Changelog Automatically?: <a href =\"byond://?src=\ref[user];preferences=1;changelog=1\">[(view_changelog ? "Yes" : "No")]</a></strong><br>"
		dat += "<strong>View Score Info Automatically?: <a href =\"byond://?src=\ref[user];preferences=1;scores=1\">[(view_score ? "Yes" : "No")]</a></strong><br>"
		dat += "<strong>View Tickets/Fines Automatically?: <a href =\"byond://?src=\ref[user];preferences=1;tickets=1\">[(view_tickets ? "Yes" : "No")]</a></strong><br>"
		dat += "<strong>Use Click Buffer?: <a href =\"byond://?src=\ref[user];preferences=1;clickbuffer=1\">[(use_click_buffer ? "Yes" : "No")]</a></strong><br>"
		...
		if (!IsGuestKey(user.key))
			dat += "<strong>Save Profile:</strong>"
			for (var/i=1, i <= SAVEFILE_PROFILES_MAX, i++)
				dat += " <a href='byond://?src=\ref[user];preferences=1;save=[i]'>[i]</a>"
			dat += "<br>"
			dat += "<strong>Load Profile:</strong>"
			for (var/i=1, i <= SAVEFILE_PROFILES_MAX, i++)
				dat += " <a href='byond://?src=\ref[user];preferences=1;load=[i]'>[savefile_get_profile_name(user, i) || i]</a>"
			dat += "<br>"
		dat += "<a href='byond://?src=\ref[user];preferences=1;reset_all=1'><strong>Reset</strong></a> - "
		dat += "<a href='byond://?src=\ref[user];preferences=1;real_name=random'><strong>Randomize</strong></A><br>"
		dat += "</body></html>"
	...
	proc/SetChoices(mob/user)
		if (isnull(jobs_med_priority) || isnull(jobs_low_priority) || isnull(jobs_unwanted))
			ResetAllPrefsToLow(user)
			boutput(user, "<span style=\"color:red\"><strong>Your Job Preferences were null, and have been reset.</strong></span>")
		else if (isnull(job_favorite) && !jobs_med_priority.len && !jobs_low_priority.len && !jobs_unwanted.len)
			ResetAllPrefsToLow(user)
			boutput(user, "<span style=\"color:red\"><strong>Your Job Preferences were empty, and have been reset.</strong></span>")

		var/HTML = "<body><title>Job Preferences</title>"

		HTML += "<strong>Favorite Job:</strong>"
		if (!job_favorite)
			HTML += " None"
		else
			var/job/J_Fav = find_job_in_controller_by_string(job_favorite)
			if (!J_Fav)
				HTML += " Favorite Job not found!"
			else if (jobban_isbanned(user,J_Fav.name) || (J_Fav.requires_whitelist && !NT.Find(ckey(user.mind.key))))
				boutput(user, "<span style=\"color:red\"><strong>You are no longer allowed to play [J_Fav.name]. It has been removed from your Favorite slot.</span>")
				jobs_unwanted += J_Fav.name
				job_favorite = null
			else
		...
		HTML += "<tr>"
		HTML += "<th><strong>Medium Priority:</strong> <a href=\"byond://?src=\ref[user];preferences=1;help=medjobs\"><small>(Help)</small></a></th>"
		HTML += "<th><strong>Low Priority:</strong> <a href=\"byond://?src=\ref[user];preferences=1;help=lowjobs\"><small>(Help)</small></a></th>"
		HTML += "<th><strong>Unwanted Jobs:</strong> <a href=\"byond://?src=\ref[user];preferences=1;help=unjobs\"><small>(Help)</small></a></th>"
		HTML += "</tr><tr>"

		var/category_counter = 0
		HTML += {"<td valign="top"><center>"}
		for (var/J in jobs_med_priority)
			var/job/J_Med = find_job_in_controller_by_string(J)
			if (!J_Med) continue
			if (jobban_isbanned(user,J_Med.name))
				boutput(user, "<span style=\"color:red\"><strong>You are no longer allowed to play [J_Med.name]. It has been removed from your Medium Priority List.</span>")
				jobs_med_priority -= J_Med.name
				jobs_unwanted += J_Med.name
			else
		...
		category_counter = 0
		HTML += {"<td valign="top"><center>"}
		for (var/J in jobs_low_priority)
			var/job/J_Low = find_job_in_controller_by_string(J)
			if (!J_Low) continue
			if (J_Low.requires_whitelist && !NT.Find(ckey(user.mind.key))) continue
			if (jobban_isbanned(user,J_Low.name))
				boutput(user, "<span style=\"color:red\"><strong>You are no longer allowed to play [J_Low.name]. It has been removed from your Low Priority List.</span>")
				jobs_low_priority -= J_Low.name
				jobs_unwanted += J_Low.name
			else
		...
		category_counter = 0
		HTML += {"<td valign="top"><center>"}
		for (var/J in jobs_unwanted)
			var/job/J_Un = find_job_in_controller_by_string(J)
			if (!J_Un) continue
			if (J_Un.requires_whitelist && !NT.Find(ckey(user.mind.key))) continue
			if (J_Un.cant_allocate_unwanted)
				boutput(user, "<span style=\"color:red\"><strong>[J_Un.name] is not supposed to be in the Unwanted category. It has been moved to Low Priority.</strong></span>")
				boutput(user, "<span style=\"color:red\"><strong>You may need to refresh your job preferences page to correct the job count.</strong></span>")
				jobs_unwanted -= J_Un.name
				jobs_low_priority += J_Un.name
		...
		/*
		HTML += "<br><strong>Antagonist Roles:</strong>"
		...
		if (jobban_isbanned(user, "Syndicate"))
			...
			if (be_traitor) HTML += "<a href =\"byond://?src=\ref[user];preferences=1;b_traitor=1\"><font color=#00CC00>Traitor</font></a>"
			else HTML += "<a href =\"byond://?src=\ref[user];preferences=1;b_traitor=1\"><font color=#FF0000><strike>Traitor</strike></font></a>"

			HTML += " * "

			if (be_syndicate) HTML += "<a href =\"byond://?src=\ref[user];preferences=1;b_syndicate=1\"><font color=#00CC00>Syndicate Operative</font></a>"
			...
			if (be_gangleader) HTML += "<a href =\"byond://?src=\ref[user];preferences=1;b_gangleader=1\"><font color=#00CC00>Gang Leader</font></a>"
			else HTML += "<a href =\"byond://?src=\ref[user];preferences=1;b_gangleader=1\"><font color=#FF0000><strike>Gang Leader</strike></font></a>"

			HTML += " * "

			if (be_changeling) HTML += "<a href =\"byond://?src=\ref[user];preferences=1;b_changeling=1\"><font color=#00CC00>Changeling</font></a>"
			else HTML += "<a href =\"byond://?src=\ref[user];preferences=1;b_changeling=1\"><font color=#FF0000><strike>Changeling</strike></font></a>"

			HTML += "<br>"

			if (be_wizard) HTML += "<a href =\"byond://?src=\ref[user];preferences=1;b_wizard=1\"><font color=#00CC00>Wizard</font></a>"
			else HTML += "<a href =\"byond://?src=\ref[user];preferences=1;b_wizard=1\"><font color=#FF0000><strike>Wizard</strike></font></a>"

			HTML += " * "

			if (be_vampire) HTML += "<a href =\"byond://?src=\ref[user];preferences=1;b_vampire=1\"><font color=#00CC00>Vampire</font></a>"
			else HTML += "<a href =\"byond://?src=\ref[user];preferences=1;b_vampire=1\"><font color=#FF0000><strike>Vampire</strike></font></a>"

			HTML += " * "

			if (be_wraith) HTML += "<a href =\"byond://?src=\ref[user];preferences=1;b_wraith=1\"><font color=#00CC00>Wraith</font></a>"
			else HTML += "<a href =\"byond://?src=\ref[user];preferences=1;b_wraith=1\"><font color=#FF0000><strike>Wraith</strike></font></a>"

			HTML += " * "

			if (be_blob) HTML += "<a href =\"byond://?src=\ref[user];preferences=1;b_blob=1\"><font color=#00CC00>Blob</font></a>"
			else HTML += "<a href =\"byond://?src=\ref[user];preferences=1;b_blob=1\"><font color=#FF0000><strike>Blob</strike></font></a>"

			HTML += " * "

			if (be_misc) HTML += "<a href =\"byond://?src=\ref[user];preferences=1;b_misc=1\"><font color=#00CC00>Other Foes</font></a>"
			else HTML += "<a href =\"byond://?src=\ref[user];preferences=1;b_misc=1\"><font color=#FF0000><strike>Other Foes</strike></font></a>"
			*/
		HTML += "<hr>"
		HTML += {"<a href=\"byond://?src=\ref[user];preferences=1;help=jobs\"><strong>Help</strong></a> * "}
		HTML += {"<a href=\"byond://?src=\ref[user];preferences=1;jobswindow=1\"><strong>Refresh</strong></a> * "}
		HTML += {"<a href=\"byond://?src=\ref[user];preferences=1;resetalljobs=1\"><strong>Reset All Jobs</strong></a> * "}
		HTML += {"<a href=\"byond://?src=\ref[user];preferences=1;closejobswindow=1\"><strong>Close Window</strong></a>"}

		user << browse(null, "window=preferences")
		user << browse(HTML, "window=mob_occupation;size=550x250")
		return
	...
	proc/SetJob(mob/user, occ=1, job="Captain",var/level = 0)
		...
		if (!find_job_in_controller_by_string(job,1))
			boutput(user, "<span style=\"color:red\"><strong>The game could not find that job in the internal list of jobs.</strong></span>")
			switch(occ)
				if (1) job_favorite = null
				if (2) jobs_med_priority -= job
				if (3) jobs_low_priority -= job
				if (4) jobs_unwanted -= job
			return
		if (job=="AI" && (!config.allow_ai))
			boutput(user, "<span style=\"color:red\"><strong>Selecting the AI is not currently allowed.</strong></span>")
			if (occ != 4)
				switch(occ)
					if (1) job_favorite = null
					if (2) jobs_med_priority -= job
		...
		if (jobban_isbanned(user, job))
			boutput(user, "<span style=\"color:red\"><strong>You are banned from this job and may not select it.</strong></span>")
			if (occ != 4)
				switch(occ)
					if (1) job_favorite = null
					if (2) jobs_med_priority -= job
		...
		var/job/J = find_job_in_controller_by_string(job)
		if (J.cant_allocate_unwanted && picker == "Unwanted")
			boutput(user, "<span style=\"color:red\"><strong>[job] cannot be set to Unwanted.</strong></span>")
			antispam = 0
			return
		...
		switch(picker)
			if ("Favorite")
				if (job_favorite)
					jobs_med_priority += job_favorite
				job_favorite = job
				successful_move = 1
			if ("Medium Priority")
				jobs_med_priority += job
				if (occ == 1)
					job_favorite = null
				successful_move = 1
			if ("Low Priority")
				jobs_low_priority += job
				if (occ == 1)
					job_favorite = null
				successful_move = 1
			if ("Unwanted")
	...
	proc/process_link(mob/user, list/link_tags)
		...
		if (link_tags["help"])
			var/helptext = "<html><body><title>Jobs Help</title><strong><u>Job Preferences Help:</u></strong><br>"
		...
		if (link_tags["job"])
			SetJob(user, text2num(link_tags["occ"]), link_tags["job"], text2num(link_tags["level"]))
			SetChoices(user)
			return
		...
		if (link_tags["resetalljobs"])
			...
			switch(resetwhat)
				if ("Medium Priority")
					ResetAllPrefsToMed(user)
				if ("Low Priority")
					ResetAllPrefsToLow(user)
				if ("Unwanted")
					ResetAllPrefsToUnwanted(user)
				else
					return
			SetChoices(user)
			return
		...
		if (link_tags["pin"])
			...
			if (new_pin)
				pin = max(min(round(text2num(new_pin)), 9999), 1000)
		...
		if (link_tags["toggle_mentorhelp"])
			if (user && user.client && user.client.mentor_authed)
				see_mentor_pms = !(see_mentor_pms)
				user.client.set_mentorhelp_visibility(see_mentor_pms)

		if (link_tags["listen_ooc"])
			listen_ooc = !(listen_ooc)

		if (link_tags["listen_looc"])
			listen_looc = !(listen_looc)

		if (link_tags["volume"])
			admin_music_volume = input("Goes from 0 to 100.","Admin Music Volume", admin_music_volume) as num
			admin_music_volume = max(0,min(admin_music_volume,100))

		if (link_tags["clickbuffer"])
			use_click_buffer = !(use_click_buffer)

		if (link_tags["default_wasd"])
			default_wasd = !(default_wasd)
		...
		*/
		...
		if (!IsGuestKey(user.key))
			if (link_tags["save"])
				savefile_save(user, (isnum(text2num(link_tags["save"])) ? text2num(link_tags["save"]) : 1))
				boutput(user, "<span style=\"color:blue\"><strong>Character saved to Slot [text2num(link_tags["save"])].</strong></span>")

			else if (link_tags["load"])
				if (!savefile_load(user, (isnum(text2num(link_tags["load"])) ? text2num(link_tags["load"]) : 1)))
					alert(user, "You do not have a savefile.")
				else if (!user.client.holder)
					sanitize_name()
					boutput(user, "<span style=\"color:blue\"><strong>Character loaded from Slot [text2num(link_tags["load"])].</strong></span>")
				else
					boutput(user, "<span style=\"color:blue\"><strong>Character loaded from Slot [text2num(link_tags["load"])].</strong></span>")
	...
	proc/copy_to(mob/character,var/mob/user,ignore_randomizer = 0)
		...
		if (character.bioHolder)
			...
			character.bioHolder.mobAppearance.gender = gender
...
/proc/randomize_look(var/to_randomize, var/change_gender = 1, var/change_blood = 1, var/change_age = 1, var/change_name = 1, var/change_underwear = 1, var/remove_effects = 1)
	...
	else if (istype(to_randomize, /appearanceHolder))
		AH = to_randomize
		if (ishuman(AH.owner))
			H = AH.owner
