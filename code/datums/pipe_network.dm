var/global/list/pipe_network/pipe_networks = list()
//
/pipe_network
	var/list/gas_mixture/gases = list() //All of the gas_mixtures continuously connected in this network

	var/list/obj/machinery/atmospherics/normal_members = list()
	var/list/pipeline/line_members = list()
		//membership roster to go through for updates and what not

	var/update = 1
	var/gas_mixture/air_transient = null

	New()
		air_transient = unpool(/gas_mixture)
	...
	disposing()
		...
		normal_members = 0
		if (line_members)
			for (var/pipeline/member in line_members)
				member.network = null
			line_members.len = 0
		line_members = null
		if (air_transient)
	...
	proc/member_disposing(pipeline/line_member)
		if (gases)
	...
	proc/process()
		...
		for (var/pipeline/line_member in line_members)
			line_member.process()
	...
	proc/merge(pipe_network/giver)
		...
		for (var/pipeline/line_member in giver.line_members)
	...
	proc/reconcile_air()
		...
		for (var/gas_mixture/gas in gases)
			...
			if (gas.trace_gases && gas.trace_gases.len)
				for (var/gas/trace_gas in gas.trace_gases)
		...
		if (air_transient.volume > 0)
			...
			for (var/gas_mixture/gas in gases)
				...
				if (air_transient.trace_gases && air_transient.trace_gases.len)
					for (var/gas/trace_gas in air_transient.trace_gases)
						var/gas/corresponding
						if (gas.trace_gases && gas.trace_gases.len)
...
/proc/equalize_gases(gas_mixture/list/gases)
	//Perfectly equalize all gases members instantly
	...
	if (total_volume > 0)
		...
		for (var/gas_mixture/gas in gases)
			...
			if (total_trace_gases && total_trace_gases.len)
				for (var/gas/trace_gas in total_trace_gases)
					var/gas/corresponding
					if (gas.trace_gases && gas.trace_gases.len)
						corresponding = locate(trace_gas.type) in gas.trace_gases
					if (!corresponding)
