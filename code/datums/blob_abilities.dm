...
/blob_ability/help
	...
	onUse(var/turf/T)
		...
		button.icon_state = "blob-help[owner.help_mode]"
		owner.update_buttons()

// STARTER ABILITIES

/blob_ability/plant_nucleus
	name = "Deploy"
	...
	onUse(var/turf/T)
		...
		C.Life()
		owner.started = 1
		owner.remove_ability(/blob_ability/plant_nucleus)
		owner.remove_ability(/blob_ability/set_color)
		owner.remove_ability(/blob_ability/tutorial)
		owner.add_ability(/blob_ability/spread)
		owner.add_ability(/blob_ability/attack)
		owner.add_ability(/blob_ability/consume)
		owner.add_ability(/blob_ability/repair)
		owner.add_ability(/blob_ability/absorb)
		owner.add_ability(/blob_ability/promote)
		owner.add_ability(/blob_ability/build/ribosome)
		owner.add_ability(/blob_ability/build/lipid)
		owner.add_ability(/blob_ability/build/mitochondria)
		owner.add_ability(/blob_ability/build/wall)
		owner.add_ability(/blob_ability/build/firewall)
		owner.add_ability(/blob_ability/upgrade)
		for (var/X in subtypesof(/blob_upgrade))
			owner.add_upgrade(X, 1)

/blob_ability/set_color
	name = "Set Color"
...
/blob_ability/consume
	...
	onUse(var/turf/T)
		...
		B.visible_message("<span style=\"color:red\"><strong>The blob consumes a piece of itself!</strong></span>")
		qdel(B)
		deduct_bio_points()
		do_cooldown()
...
/blob_ability/absorb
	...
	onUse(var/turf/T)
		...
		B.visible_message("<span style=\"color:red\"><strong>The blob starts trying to absorb [H.name]!</strong></span>")
		sleep(40)
		T = get_turf(H)
		if (T.get_blob_on_this_turf())
			if (H.decomp_stage == 4)
				return
			H.decomp_stage = 4
			H.visible_message("<span style=\"color:red\"><strong>[H.name] is absorbed by the blob!</strong></span>")
			playsound(T, 'sound/effects/blobattack.ogg', 50, 1)
			owner.evo_points += 4
			H.transforming = 1
...
/blob_ability/reinforce
	...
	onUse(var/turf/T)
		...
		B.visible_message("<span style=\"color:red\"><strong>[B] reinforces using [reinforcing]!</strong></span>")
...
/blob_ability/devour_item
	...
	onUse(var/turf/T)
		...
		Bleb.visible_message("<span style=\"color:red\"><strong>The blobs starts devouring [I]!</strong></span>")
		sleep(20)
		if (!I)
			return TRUE
		if (!isturf(I.loc))
		...
		Bleb.visible_message("<span style=\"color:red\"><strong>The blob devours [I]!</strong></span>")
		...
		if (aggregated.len)
			...
			if (Bleb)
				...
				B2.setOvermind(owner)
				qdel(Bleb)
				B2.reagents = new /reagents(0)
				B2.reagents.my_atom = B2
				for (var/reagent/R in aggregated)
					if (!B2)
						deduct_bio_points()
						return
...
/blob_ability/build/mitochondria
...
/blob_ability/build/plasmaphyll
...
/blob_ability/build/ectothermid
...
/blob_upgrade
	...
	proc/deduct_evo_points()
		...
		evo_point_cost = round(evo_point_cost * scaling_cost_mult)
		evo_point_cost += scaling_cost_add
...
/blob_upgrade/devour_item
	...
	take_upgrade()
		if (..())
			return TRUE
		owner.add_ability(/blob_ability/devour_item)
		owner.add_upgrade(/blob_upgrade/reclaimer)
		owner.add_upgrade(/blob_upgrade/replicator)
		owner.add_upgrade(/blob_upgrade/reinforce)

/blob_upgrade/reinforce
	...
	take_upgrade()
		if (..())
			return TRUE
		owner.add_ability(/blob_ability/reinforce)
		owner.add_upgrade(/blob_upgrade/reinforce_spread)

/blob_upgrade/reinforce_spread
	...
	take_upgrade()
		...
		if (count < required_deposits)
			boutput(usr, "<span style=\"color:red\"><strong>You need more deposits on your screen! (Required: [required_deposits], have: [count])</strong></span>")
			return TRUE
		if (taking)
		...
		if (!total)
			return TRUE
		if (total < required_deposits)
			boutput(usr, "<span style=\"color:red\"><strong>You need more deposits on your screen! (Required: [required_deposits], have (of highest material '[max_id]'): [count])</strong></span>")
			return TRUE
		if (!mats.len)
			return TRUE
		var/material/to_merge = copyMaterial(mats[max_id])
...
/blob_upgrade/reclaimer
	...
	take_upgrade()
		if (..())
			return TRUE
		owner.add_ability(/blob_ability/reclaimer)

/blob_upgrade/replicator
	...
	take_upgrade()
		if (..())
			return TRUE
		owner.add_ability(/blob_ability/replicator)
...
/blob_upgrade/bridge
	...
	take_upgrade()
		if (..())
			return TRUE
		owner.add_ability(/blob_ability/bridge)

/blob_upgrade/launcher
	...
	take_upgrade()
		if (..())
			return TRUE
		owner.add_ability(/blob_ability/build/launcher)

/blob_upgrade/plasmaphyll
	...
	take_upgrade()
		if (..())
			return TRUE
		owner.add_ability(/blob_ability/build/plasmaphyll)

/blob_upgrade/ectothermid
	...
	take_upgrade()
		if (..())
			return TRUE
		owner.add_ability(/blob_ability/build/ectothermid)

/blob_upgrade/reflective
	...
	take_upgrade()
		if (..())
			return TRUE
		owner.add_ability(/blob_ability/build/reflective)