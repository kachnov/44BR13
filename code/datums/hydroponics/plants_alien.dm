...
/plant/artifact/pukeplant
	...
	HYPspecial_proc(var/obj/machinery/plantpot/POT)
		...
		if (POT.growth > (P.harvtime + DNA.harvtime) && prob(20))
			POT.visible_message("<span style=\"color:red\"><strong>[POT.name]</strong> vomits profusely!</span>")
...
/plant/artifact/peeker
	...
	HYPspecial_proc(var/obj/machinery/plantpot/POT)
		...
		if (POT.growth > (P.growtime + DNA.growtime) && prob(16))
			...
			if (stuffnearby.len > 1) POT.visible_message("<span style=\"color:red\"><strong>[POT.name]</strong> stares at [pick(stuffnearby)].</span>")

// harvestables

/plant/artifact/dripper
	name = "Dripper"
...
/plant/artifact/rocks
	...
	mutations = list(/plantmutation/rocks/syreline,/plantmutation/rocks/bohrum,/plantmutation/rocks/mauxite,/plantmutation/rocks/erebite)

/plant/artifact/litelotus
	name = "Light Lotus"
...
/plant/maneater
	...
	HYPspecial_proc(var/obj/machinery/plantpot/POT)
		...
		if (POT.growth > (P.growtime + DNA.growtime) && prob(4))
			...
			for (var/mob/M in hearers(POT, null)) M.show_message("<strong>Man-Eating Plant</strong> gurgles, \"[MEspeech]\"")
	...
	HYPattacked_proc(var/obj/machinery/plantpot/POT,var/mob/user)
		...
		for (var/mob/O in hearers(POT, null))
			O.show_message("<strong>Man-Eating Plant</strong> gurgles, \"[MEspeech]\"", 1)
