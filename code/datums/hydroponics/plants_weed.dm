/plant/fungus
	...
	mutations = list(/plantmutation/fungus/amanita,/plantmutation/fungus/psilocybin)
...
/plant/lasher
	...
	vending = 2
	genome = 5
	mutations = list(/plantmutation/lasher/berries)
	...
	HYPspecial_proc(var/obj/machinery/plantpot/POT)
		...
		if (POT.growth > (P.growtime + DNA.growtime) && prob(33))
			for (var/mob/living/M in range(1,POT))
				...
				if (POT.health <= P.starthealth / 2) POT.visible_message("<span style=\"color:red\"><strong>[POT.name]</strong> weakly slaps [M] with a vine!</span>")
				else POT.visible_message("<span style=\"color:red\"><strong>[POT.name]</strong> slashes [M] with thorny vines!</span>")
	...
	HYPattacked_proc(var/obj/machinery/plantpot/POT,var/mob/user,var/obj/item/W)
		...
		if (POT.growth < (P.growtime + DNA.growtime)) return FALSE
		// It's not big enough to be violent yet, so nothing happens

		POT.visible_message("<span style=\"color:red\"><strong>[POT.name]</strong> violently retaliates against [user.name]!</span>")
	...
	HYPspecial_proc(var/obj/machinery/plantpot/POT)
		...
		if (POT.growth > (P.growtime + DNA.growtime) && POT.health > P.starthealth / 2 && prob(33))
			for (var/obj/machinery/plantpot/C in range(1,POT))
				...
				if (!C.dead && C.current && !istype(growing,/plant/crystal) && !istype(growing,/plant/creeper)) C.health -= 10
...
/plant/radweed
	...
	mutations = list(/plantmutation/radweed/redweed,/plantmutation/radweed/safeweed)
...
/plant/slurrypod
	...
	mutations = list(/plantmutation/slurrypod/omega)
	...
	HYPspecial_proc(var/obj/machinery/plantpot/POT)
		...
		if (POT.growth >= (P.harvtime + DNA.harvtime + 50) && prob(10) && !exploding)
			exploding = 1
			POT.visible_message("<span style=\"color:red\"><strong>[POT]</strong> begins to bubble and expand!</span>")
			playsound(POT.loc, "sound/effects/bubbles.ogg", 50, 1)

			spawn (50)
				POT.visible_message("<span style=\"color:red\"><strong>[POT]</strong> bursts, sending toxic goop everywhere!</span>")
