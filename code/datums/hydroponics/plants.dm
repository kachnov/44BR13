...
/plant/
	...
	// hyp procs now return FALSE for success and continue, any other number for error codes
	...
	*/
	...
	proc/HYPharvested_proc(var/obj/machinery/plantpot/POT,var/mob/user)
		...
		if (!harvestable || !crop) return 303
		if (lasterr)
			logTheThing("debug", null, null, "<strong>Plant HYP</strong> [src] in pot [POT] failed with error [.]")
			harvested_proc = 0
		return lasterr
	...
	proc/HYPinfusionP(var/obj/item/seed/S,var/reagent)
		...
		var/damage_prob = 100 - (endurance + DNA.endurance)
		...
		switch (reagent)
			...
			if ("weedkiller")
				if (!HYPCheckCommut(DNA,/plant_gene_strain/immunity_toxin) && growthmode == "weed")
					damage_amt = rand(50,60)
			if ("toxin","mercury","chlorine","fluorine","fuel","oil","cleaner")
				if (!HYPCheckCommut(DNA,/plant_gene_strain/immunity_toxin))
					damage_amt = rand(15,30)
			if ("plasma")
				if (!HYPCheckCommut(DNA,/plant_gene_strain/immunity_toxin))
					damage_amt = rand(15,30)
			if ("blood","bloodc")
				if (growthmode == "carnivore")
			...
			if ("dna_mutagen")
				...
				HYPnewmutationcheck(src,DNA)
				if (prob(2))
					HYPaddCommut(DNA,/plant_gene_strain/unstable)
			if ("mutagen")
				HYPmutateDNA(DNA,2)
				HYPnewcommutcheck(src,DNA)
				HYPnewmutationcheck(src,DNA)
			...
			if ("ammonia")
				...
				if (prob(5))
					HYPaddCommut(DNA,/plant_gene_strain/accelerator)
			if ("potash")
			...
			if ("space_fungus")
				...
				if (prob(3))
					HYPaddCommut(DNA,/plant_gene_strain/damage_res)
			if ("mutadone")
				if (DNA.growtime > 0)
					DNA.growtime--
				if (DNA.harvtime > 0)
...
/plantgenes/
	...
	var/harvtime = 0 // same vars found in /plant honestly. They go largely towards
	...
	var/plantmutation/mutation = null // is it mutated? if so which variation?
