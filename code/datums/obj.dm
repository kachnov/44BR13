...
//heh no not really
	...
	proc/updateHealth(var/prevHealth)
		...
		*/
		return
	proc/onDestroy()
		visible_message("<span style=\"color:red\"><strong>[src] is destroyed.</strong></span>")
		qdel(src)
		return

	ex_act(severity)
		if (material)
			material.triggerExp(src, severity)
		switch(severity)
			if (1.0)
				changeHealth(-95)
				return
	...
	onMaterialChanged()
		...
		if (istype(material))
			...
			if (initial(opacity) && material.alpha <= MATERIAL_ALPHA_OPACITY)
				opacity = 0
			else if (initial(opacity) && !opacity && material.alpha > MATERIAL_ALPHA_OPACITY)
				opacity = 1
		return
	...
	proc/handle_internal_lifeform(mob/lifeform_inside_me, breath_request)
		...
		//		air_group to tell lifeform to process using that breath return
...
// grabsmash
	...
	MouseDrop(atom/over_object as mob|obj|turf)
		..()
		if (usr.bioHolder && usr.bioHolder.HasEffect("telekinesis_drag") && istype(src, /obj) && isturf(loc) && usr.stat == 0  && usr.canmove && get_dist(src,usr) <= 7 )
			var/bioEffect/TK = usr.bioHolder.GetEffect("telekinesis_drag")

			if (!anchored && (istype(src, /obj/item) || TK.variant == 2))
				throw_at(over_object, 7, 1)
...
/obj/bedsheetbin
	...
	attack_hand(mob/user as mob)
		if (amount >= 1)
			amount--
			new /obj/item/clothing/suit/bedsheet(loc)
			add_fingerprint(user)

	get_desc()
		. += "There's [amount ? amount : "no"] bedsheet[s_es(amount)] in [src]."
...
/obj/towelbin
	...
	attack_hand(mob/user as mob)
		if (amount >= 1)
			amount--
			new /obj/item/clothing/under/towel(loc)
			add_fingerprint(user)

	get_desc()
		. += "There's [amount ? amount : "no"] towel[s_es(amount)] in [src]."
...
/obj/lattice/barricade
	...
	proc/barricade_damage(var/hitstrength)
		...
		if (strength < 1)
			visible_message("The barricade breaks!")
			if (prob(50)) new /obj/item/rods/steel(loc)
			qdel(src)
			return
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/weldingtool))
			...
			if (WELD.welding)
				...
				WELD.eyecheck(user)
				var/obj/item/rods/R = new /obj/item/rods/steel(loc)
				R.amount = strength
				qdel(src)
				return
		else if (istype(W,/obj/item/rods))
			var/obj/item/rods/R = W
			var/difference = 5 - strength
			if (difference <= 0)
			...
			if (R.amount > difference)
				...
				if (strength >= 5) boutput(user, "<span style=\"color:blue\">The barricade is now fully reinforced!</span>")
				if (R.amount < 1)
					user.u_equip(W)
					qdel(W)
				return
	...
	ex_act(severity)
		switch(severity)
			if (1.0)
				qdel(src)
				return
			if (2.0) barricade_damage(3)
			if (3.0) barricade_damage(1)
		return

	blob_act(var/power)
		barricade_damage(2 * power / 20)
