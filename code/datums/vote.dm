...
/vote/proc/timetext(var/interval)
...
/vote/proc/endvote()
	...
	boutput(world, "<span style=\"color:red\"><strong>***Voting has closed.</strong></span>")
	...
	if (mode)
		...
		if (ticker)
			boutput(world, "<span style=\"color:red\"><strong>World will reboot in 10 seconds</strong></span>")
		...
		boutput(world, "<span style=\"color:red\"><strong>World will reboot in 5 seconds</strong></span>")
...
/mob/verb/vote()
	...
	if (vote.voting)
		...
		if (vote.mode)		// true if changing mode
			...
			text += "Current game mode is: <strong>[master_mode]</strong>.<BR>Select the mode to change to:<UL>"
			...
			for (var/md in config.votable_modes)
				...
				//boutput(world, "[md]|[disp]|[client.vote]|[votes[md]]")

				if (client.vote == md)
					text += "<LI><strong>[disp]</strong>"
				else
			...
			text +="<p>Current winner: <strong>[vote.calcwin()]</strong><BR>"
...
/vote/Topic(href, href_list)
	...
	if (href_list["vmode"])
		...
		boutput(world, "<span style=\"color:red\"><strong>*** A vote to [vote.mode?"change game mode":"restart"] has been initiated by [M.key].</strong></span>")
