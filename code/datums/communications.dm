...
var/global/controller/radio/radio_controller = null

/controller/radio
	var/list/radio_frequency/frequencies = list()
	var/list/active_jammers = list()

	proc/add_object(obj/device, new_frequency)
		var/radio_frequency/frequency = frequencies[new_frequency]
	...
	proc/remove_object(obj/device, old_frequency)
		var/radio_frequency/frequency = frequencies[old_frequency]
...
/mob/verb/listfreq()
	...
	for (var/fn in radio_controller.frequencies)
		var/radio_frequency/f = radio_controller.return_frequency(fn)
...
var/global/list/signal/reusable_signals = list()
...
/radio_frequency
	...
	proc
		...
		post_signal(obj/source, signal/signal, range)
			...
			signal.channels_passed += "[frequency];"
...
/signal
	...
	var/computer/file/data_file

	proc/copy_from(signal/model)
		source = model.source
