...
/ailment/disease/kuru/stage_act(var/mob/living/affected_mob,var/ailment_data/D)
	if (..())
		return
	switch(D.stage)
		if (1)
		...
		if (2)
			if (prob(50))
				for (var/mob/O in viewers(affected_mob, null))
					O.show_message(text("<span style=\"color:red\"><strong>[]</strong> laughs uncontrollably!</span>", affected_mob), 1)
