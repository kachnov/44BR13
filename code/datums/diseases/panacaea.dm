...
//
/ailment/disease/panacaea/stage_act(var/mob/living/affected_mob,var/ailment_data/D)
	if (..())
		return
	switch(D.stage)
		if (1)
		...
		if (2)
			...
			if (prob(8))
				...
				affected_mob.take_toxin_damage(-2)
			if (prob(10))
				for (var/ailment_data/disease/V in affected_mob.ailments)
					if (istype(V.master, /ailment/disease/panacaea))
						continue
