...
/ailment/parasite/spidereggs/surgery(var/mob/living/surgeon, var/mob/living/affected_mob, var/ailment_data/D)
	if (D.disposed)
		return FALSE
	...
	switch (outcome)
		...
		if (16 to 60)
			...
			var/success = 0
			if (prob(50))
				around_msg = "<span style=\"color:blue\"><strong>[surgeon] cuts open [affected_mob] and removes some [name].</strong></span>"
				self_msg = "<span style=\"color:blue\">You remove some [name] from [affected_mob]. You can still see some of it in there, though.</span>"
			else
				around_msg = "<span style=\"color:blue\"><strong>[surgeon] cuts open [affected_mob] and removes the remaining [name].</strong></span>"
			...
			return success
		if (61 to INFINITY)
			surgeon.visible_message("<span style=\"color:blue\"><strong>[surgeon] cuts open [affected_mob] and removes all traces of [name]</strong></span>", "<span style=\"color:blue\">You masterfully remove the [name] from [affected_mob].</span>")
			if (!numb)
				affected_mob.show_message("<span style=\"color:red\"><strong>You feel a mild stabbing pain in your chest!</strong></span>")
				affected_mob.TakeDamage("chest", 10, 0, 0, DAMAGE_STAB)
				affected_mob.updatehealth()
...
/ailment/parasite/spidereggs/stage_act(var/mob/living/affected_mob,var/ailment_data/D)
	if (..())
		return
	switch(D.stage)
		if (2)
		...
		if (5)
			...
			if (prob(30))
				...
				affected_mob.visible_message("<span style=\"color:red\"><strong>[affected_mob] bursts open! Holy fuck!</strong></span>")
				affected_mob:gib()
				return


//IBM note: haha this is the dumbest thing I have ever coded
/ailment/parasite/bee_larva
...
/ailment/parasite/bee_larva/stage_act(var/mob/living/affected_mob,var/ailment_data/D)
	if (..())
		return
	switch(D.stage)
		if (2, 3)
		...
		if (5)
			...
			if (prob(40))
				...
				affected_mob.visible_message("<span style=\"color:red\"><strong>[affected_mob] horks up a bee larva!  Grody!</strong></span>", "<span style=\"color:red\"><strong>You cough up...a bee larva. Uhhhhh</strong></span>")
