...
/ailment/disease/space_aids/stage_act(var/mob/living/affected_mob,var/ailment_data/D)
	if (..())
		return
	switch(D.stage)
		if (1)
		...
		if (2)
			if (prob(10))
				for (var/ailment/A in affected_mob.ailments)
		...
		if (3)
			...
			if (prob(8))
				...
				boutput(affected_mob, "<span style=\"color:red\">[procmessage]</span>")
			if (prob(5))
				for (var/ailment/disease/V in disease_controls.standard_diseases)
					if (prob(10))
