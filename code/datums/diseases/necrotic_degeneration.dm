/ailment/disease/necrotic_degeneration
...
/ailment/disease/necrotic_degeneration/stage_act(var/mob/living/affected_mob,var/ailment_data/D)
	if (..())
		return
	...
	switch(D.stage)
		...
		if (5)
			...
			affected_mob.set_mutantrace(/mutantrace/zombie)
			...
			D.stage++
		if (6)
			if (!istype(affected_mob:mutantrace, /mutantrace/zombie))
				affected_mob.set_mutantrace(/mutantrace/zombie)
				if (ishuman(affected_mob))
					affected_mob:update_face()
					affected_mob:update_body()
