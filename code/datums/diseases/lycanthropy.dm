...
/ailment/disease/lycanthropy/stage_act(var/mob/living/affected_mob, var/ailment_data/D)
	if (..())
		return
	if (ishuman(affected_mob))
		var/mob/living/carbon/human/H = affected_mob
		if (!istype(H.mutantrace, /mutantrace/werewolf))
			switch (D.stage)
				if (2)
					if (prob(1))
						H.emote("sneeze")
				...
				if (5)
					...
					if (prob(50) && triggered_transformation == 0)
						H.visible_message("<span style=\"color:red\"><strong>[H] starts having a seizure!</strong></span>")
						H.weakened = max(15, H.weakened)
						H.stuttering = max(10, H.stuttering)
						H.make_jittery(1000)
						triggered_transformation = 1
						...
						spawn (rand(100, 300))
							if (H && D)
								...
								D.stage_prob = 0
								D.stage = 1
							triggered_transformation = 0 // Necessary. Disease datums seem to be pooled or something, dunno.
