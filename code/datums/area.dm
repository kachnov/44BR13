...
/area
	...
	var/gang/gang_owners = null // gang that owns this area in gang mode
			...
			if (enteringMobs.len > 0)
				for (var/mob/enteringM in enteringMobs) //each dumb mob
					...
					if (enteringM.ckey && enteringM.client && enteringM.mind)
						if (name != "Space") //Who cares about making space active gosh
							if (!(enteringM.mind in population))
								population += enteringM.mind
							if (!active)
								active = 1

						//Dumb fucking medal fuck
						if (name == "Space" && istype(A, /obj/vehicle/segway))
			...
			if (exitingMobs.len > 0)
				for (var/mob/exitingM in exitingMobs)
					if (exitingM.ckey && exitingM.client && exitingM.mind)
						if (name != "Space")
							if (exitingM.mind in population)
								population -= exitingM.mind
							if (active && population.len == 0) //Only if this area is now empty
								active = 0
	...
	proc/wake_critters()
		...
		waking_critters = 1
		for (var/obj/critter/C in registered_critters)
	...
	proc/calculate_area_value()
		...
		for (var/obj/machinery/M in contents)
...
/area/shuttle/escape/transit
	...
	Entered(atom/movable/Obj,atom/OldLoc)
		...
		if (ismob(Obj))
			var/mob/M = Obj
			if (warp_dir == "north" || warp_dir == "south")
				M.addOverlayComposition(/overlayComposition/shuttle_warp)
			else
				M.addOverlayComposition(/overlayComposition/shuttle_warp/ew)
...
/area/shuttle_transit_space
	...
	Entered(atom/movable/Obj,atom/OldLoc)
		...
		if (ismob(Obj))
			var/mob/M = Obj
			if (throw_dir == "north" || throw_dir == "south")
				M.addOverlayComposition(/overlayComposition/shuttle_warp)
			else
				M.addOverlayComposition(/overlayComposition/shuttle_warp/ew)
...
/area/someplacehot
	...
	Entered(atom/movable/Obj,atom/OldLoc)
		..()
		if (ismob(Obj))
			Obj:addOverlayComposition(/overlayComposition/heat)
	...
	Exited(atom/movable/Obj)
		..()
		if (ismob(Obj))
			Obj:removeOverlayComposition(/overlayComposition/heat)
...
/area/station/turret_protected/proc/popDownTurrets()
	for (var/obj/machinery/turret/aTurret in turret_list)
		aTurret.popDown()
