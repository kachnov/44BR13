...
// -------------------- GLOBAL VARS -------------------- (LEAVE THIS HERE BECAUSE scripts/code-standardization-srcdot.py CHECKS FOR IT)
...
//	airtunnel_bottom = 72 // default
	...
	// Controllers
	research/disease/disease_research = new()
	research/artifact/artifact_research = new()
	research/robotics/robotics_research = new()
	wage_system/wagesystem
	shipping_market/shippingmarket

	station_state/start_state = null
	configuration/config = null
	vote/vote = null
	sun/sun = null

	changelog/changelog = null
	admin_changelog/admin_changelog = null
	...
	list/supply_packs/qm_supply_cache = list()

	//Okay, I guess this was getting constructed every time someone wanted something from it
	list/syndicate_buylist/syndi_buylist_cache = list()
	...
	list/particleSystem/mechanic/camera_path_list = list() //List of particlesystems that the connection display proc creates. I dunno where else to put it. :(
	...
	list/ai_camera_tracker/tracking_list = list()
	...
	// battle royale
	list/mutt_noclip_locations = list()

// end global variable definitions (LEAVE THIS HERE BECAUSE scripts/code-standardization-srcdot.py CHECKS FOR IT)
