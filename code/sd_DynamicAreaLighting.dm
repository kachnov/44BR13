...
/atom
	...
	proc/sd_ApplyLum(updateMode = 0)
		...
		for (var/turf/T in view(min(20, luminosity), sd_Center))
			var/intensity = sd_Intensity(T, loc)
...
//		..()
	...
	proc/sd_LumUpdate()
		...
		if (nRGB[4] > 2)
			...
			if (additiveDelta > 0.05)
				...
				if (!effect_overlay_additive)
					effect_overlay_additive = unpool(/obj/overlay/tile_effect)
					effect_overlay_additive.set_loc(src)
					effect_overlay_additive.blend_mode = BLEND_ADD
					// Maybe scale this based on nRGB[4] (coefficient)
					effect_overlay_additive.alpha = 60
				effect_overlay_additive.color = rgb(_rgb[1], _rgb[2], _rgb[3])
		else if (effect_overlay_additive) //If there's no additive remainder, pool the effect
			sd_LastRedAdditiveApplied = 0
			sd_LastGreenAdditiveApplied = 0
			sd_LastBlueAdditiveApplied = 0
			pool(effect_overlay_additive)
			effect_overlay_additive.set_loc(null)
			effect_overlay_additive.alpha = 255
			effect_overlay_additive = null
		...
		if (!effect_overlay)
			effect_overlay = unpool(/obj/overlay/tile_effect)
			effect_overlay.set_loc(src)
			effect_overlay.blend_mode = BLEND_MULTIPLY
		effect_overlay.color = rgb(min(255, r * 255), min(255, g * 255), min(255, b * 255))

		for (var/obj/overlay/tile_effect/secondary/S in src)
			S.color = effect_overlay.color

	proc/sd_StripEffectOverlayLighting()
		if (effect_overlay)
			pool(effect_overlay)
			effect_overlay.set_loc(null)
			effect_overlay = null
