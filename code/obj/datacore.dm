...
/obj/datacore/proc/addManifest(var/mob/living/carbon/human/H as mob)
	...
	var/data/record/M = new /data/record(  )
	var/data/record/S = new /data/record(  )
	var/data/record/B = new /data/record(  )
	...
	// Otherwise give them a default wage
		...
		var/job/J = find_job_in_controller_by_string(G.fields["rank"])
		if (J && J.wages)
	...
	if ("[H.mind.assigned_role]" in job_mailgroup_list)
		...
		if (!mainframe || !mainframe.hd || !(mainframe.hd in mainframe))
		...
		if (mainframe)
			for (var/computer/folder/folder in mainframe.hd.root.contents)
				if (ckey(folder.name) == "etc")
					for (var/computer/folder/folder2 in folder.contents)
						if (ckey(folder2.name) == "mail")
							for (var/computer/file/record/groups in folder2.contents)
...
/fine
	...
	var/paid_amount = 0
	var/paid = 0
	var/data/record/bank_record = null
	var/target_byond_key = null
	var/issuer_byond_key = null
...
/fine/proc/approve(var/approved_by,var/their_job)
