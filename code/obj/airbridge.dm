...
/obj/airbridge_controler
	...
	proc/is_working()
		...
		if (linked.working || working) return TRUE
		else return FALSE
...
/obj/machinery/computer/airbr
	...
	New()
		..()
		if (emergency && emergency_shuttle) // emergency_shuttle is the controller datum
	...
	proc/do_initial_extend()
		var/obj/airbridge_controler/C = pick_controller()
		if (!istype(C))
			return
	...
	attack_hand(var/mob/user as mob)
		...
		var/dat = ""
		dat += "<strong>Controller Status:</strong><BR>"
		dat += "[state_str]<BR>"
		dat += "[working ? "Working..." : "Idle..."]<BR><BR>"
		dat += "<strong>Airbridge Control:</strong><BR>"
	...
	Topic(href, href_list)
		...
		if (href_list["create"])
			if (emergency && emergency_shuttle)
