...
/artifact_fault/irradiate
	...
	deploy(var/obj/O,var/mob/living/user)
		...
		user.irradiate(rads_amount)

/artifact_fault/shutdown
	...
	deploy(var/obj/O,var/mob/living/user)
		...
		if (istype(src,/obj/item))
			...
			if (I.loc == user)
				...
				I.dropped()
		var/turf/T = get_turf(O)
		T.visible_message("<span style=\"color:red\"><strong>The [O.name] utterly annihilates [user.name]!</strong></span>")
		playsound(T, "sound/effects/elec_bigzap.ogg", 100, 1)
		user.elecgib()

/artifact_fault/explode
...
/artifact_fault/zap
	...
	deploy(var/obj/O,var/mob/living/user)
		if (..())
			return
		var/turf/T = get_turf(O)
		T.visible_message("<span style=\"color:red\"><strong>[user.name]</strong> is shocked by a surge of energy from [O.name]!</span>")
...
/artifact_fault/messager/creepy_whispers
	text_style = "small"
