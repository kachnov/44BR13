/obj/item/artifact/teleport_wand
	...
	artifact = 1
	associated_datum = /artifact/telewand
	...
	afterattack(atom/target as mob|obj|turf|area, mob/user as mob, flag)
		if (!ArtifactSanityCheck())
			return
		var/artifact/telewand/A = artifact
		if (!istype(A))
			return
		...
		if (A.activated)
			if (A.can_teleport_here(U))
				...
				boutput(user, "<strong>[src]</strong> [A.error_phrase]")
...
/artifact/telewand
	...
	effect_click_tile(var/obj/O,var/mob/living/user,var/turf/T)
		...
		particleMaster.SpawnSystem(new /particleSystem/tele_wand(T,particle_sprite,particle_color))
		return
