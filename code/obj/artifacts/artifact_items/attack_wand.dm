/obj/item/artifact/attack_wand
	...
	associated_datum = /artifact/attack_wand
	...
	afterattack(atom/target as mob|obj|turf|area, mob/user as mob, flag)
		if (user.equipped() == src)
			if (!ArtifactSanityCheck())
				return
			var/artifact/attack_wand/A = artifact
			if (!istype(A))
				return
			if (!A.activated)
				return
...
/artifact/attack_wand
	...
	/artifact_trigger/radiation,/artifact_trigger/force)
	...
	effect_click_tile(var/obj/O,var/mob/living/user,var/turf/T)
		...
		spawn (cooldown)
			if (O.loc == user)
				boutput(user, "<strong>[O]</strong> [recharge_phrase]")
			ready = 1
		...
		switch(attack_type)
			...
			if ("lightning")
				...
				if (attack_amt)
					...
					boutput(user, "<span style=\"color:red\"><strong>[O]</strong> crackles with electricity for a moment. Perhaps it couldn't find a target?</span>")

			if ("sonic")
				playsound(T, "sound/effects/screech.ogg", 100, 1, 0)
				particleMaster.SpawnSystem(new /particleSystem/sonic_burst(T))
