/obj/artifact/power_giver
	...
	associated_datum = /artifact/power_giver

/artifact/power_giver
	...
	effect_touch(var/obj/O,var/mob/living/user)
		...
		if (user.bioHolder && ready)
			...
			T.visible_message("<strong>[O]</strong> envelops [user] in a strange light!")
			user.bioHolder.AddEffect(power_granted,0,power_time)
			if (recharge_time > 0)
				ready = 0
				spawn (recharge_time)
					T.visible_message("<strong>[O]</strong> begins to glow again.")
					ready = 1