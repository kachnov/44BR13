...
/artifact/bomb
	...
	/artifact_trigger/cold,/artifact_trigger/radiation)
	...
	effect_activate(var/obj/O)
		...
		if (warning_initial)
			T.visible_message("<strong>[O] [warning_initial]</strong>")
		if (alarm_initial)
			playsound(T, alarm_initial, 50, 1, -1)

		spawn (explode_delay)
			if (warning_final)
				T.visible_message("<strong>[O] [warning_final]</strong>")
			if (alarm_final)
			...
			spawn (30)
				if (activated)
					...
					T.visible_message("<strong>[O] [text_disarmed]")
	...
	proc/deploy_payload(var/obj/O)
		...
		if (dud)
			var/turf/T = get_turf(O)
			T.visible_message("<strong>[O] [text_dud]")
...
/obj/artifact/bomb
	name = "artifact bomb"
	associated_datum = /artifact/bomb/explosive

/artifact/bomb/explosive
	...
	deploy_payload(var/obj/O)
		if (..())
			return
		explosion(O, O.loc, exp_deva, exp_hevy, exp_lite, exp_lite * 2)

		O.ArtifactDestroyed()

/obj/artifact/bomb/devastating
	name = "artifact devastating bomb"
	associated_datum = /artifact/bomb/explosive/devastating

/artifact/bomb/explosive/devastating
...
/obj/artifact/bomb/blackhole
	...
	associated_datum = /artifact/bomb/blackhole

/artifact/bomb/blackhole
...
/obj/artifact/bomb/chemical
	...
	associated_datum = /artifact/bomb/chemical

	New()
		..()
		var/reagents/R = new/reagents(10000)
		reagents = R
		R.my_atom = src

/artifact/bomb/chemical
	explode_delay = 0
	react_xray = list(5,65,20,11,"HOLLOW")
	validtypes = list("ancient","martian","eldritch","precursor")
	validtriggers = list(/artifact_trigger/force,/artifact_trigger/heat,/artifact_trigger/carbon_touch)
