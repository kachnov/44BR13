...
/obj/item/wrapping_paper
	...
	New()
		..()
		style = pick("r", "rs", "g", "gs")
		icon_state = "wrap_paper-[style]"

/obj/item/wrapping_paper/attackby(obj/item/W as obj, mob/user as mob)
	if (!( locate(/obj/table, loc) ))
	...
	if (W.w_class < 4)
		if (user.find_type_in_hand(/obj/item/wirecutters) || user.find_type_in_hand(/obj/item/scissors))
			...
			if (amount < a_used)
				...
				amount -= a_used
				user.drop_item()
				var/obj/item/gift/G = new /obj/item/gift(loc)
				G.size = W.w_class
				G.w_class = G.size + 1
				G.icon_state = "gift[minmax(G.size, 1, 3)]-[style]"
				G.gift = W
				W.set_loc(G)
			...
			if (amount <= 0)
				user.u_equip(src)
				var/obj/item/c_tube/C = new /obj/item/c_tube(loc)
...
/obj/item/wrapping_paper/attack(mob/target as mob, mob/user as mob)
	...
	if (target.stat == 2)
		if (amount > 2)
			...
			present.icon_state = "strange-[style]"
			amount -= 2
...
/obj/item/a_gift
	...
	New()
		..()
		icon_state = "gift[rand(1,3)]-[pick("r", "rs", "g", "gs")]"
