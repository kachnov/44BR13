...
/obj/item/coin/throw_impact(atom/hit_atom)
	..(hit_atom)
	if (prob(49))
		visible_message("<span style=\"color:blue\">The coin comes up heads</span>")

	else if (prob(49))
		visible_message("<span style=\"color:blue\">The coin comes up tails</span>")
	else
		visible_message("<span style=\"color:blue\">The coin lands on its side</span>")
...
/obj/item/coin/suicide(var/mob/user as mob)
	user.visible_message("<span style=\"color:red\"><strong>[user] swallows the [name] and begins to choke!</strong></span>")
