/obj/item/handcuffs
	...
	examine()
		..()
		if (delete_on_last_use)
			boutput(usr, "There are [amount] lengths of [istype(src, /obj/item/handcuffs/tape_roll) ? "tape" : "ziptie"] left!")
		return

	suicide(var/mob/user as mob) //brutal
		if (!hasvar(user,"organHolder") || istype(src,/obj/item/handcuffs/tape_roll) || istype(src,/obj/item/handcuffs/tape)) return FALSE
		user.canmove = 0
		user.visible_message("<span style=\"color:red\"><strong>[user] jams one end of the [name] into one of \his eye sockets, closing the loop through the other!")
		playsound(loc, "sound/effects/bloody_stab.ogg", 50, 1)
		user.emote("scream")
		spawn (10)
			user.visible_message("<span style=\"color:red\"><strong>[user] yanks the other end of the [name] as hard as \he can, ripping \his skull clean out of \his head! [pick("Jesus christ!","Holy shit!","What the fuck!?","Oh my god!")]</strong></span>")
			...
			spawn (5)
				...
				brain.visible_message("<span style=\"color:red\"><strong>[brain.name] falls out of the bottom of [skull.name]</strong></span>")
...
/obj/item/handcuffs/attack(mob/M as mob, mob/user as mob)
	...
	if (ishuman(M))
		...
		actions.start(new/action/bar/icon/handcuffSet(H, src), user)
		return
