/obj/item/gun/energy
	...
	examine()
		set src in usr
		if (cell)
			desc = "[projectiles ? "It is set to [current_projectile.sname]. " : ""]There are [cell.charge]/[cell.max_charge] PUs left!"
		else
	...
	process()
		wait_cycle = !wait_cycle // Self-charging cells recharge every other tick (Convair880).
		if (wait_cycle)
			return

		if (!(src in processing_items))
			logTheThing("debug", null, null, "<strong>Convair880</strong>: Process() was called for an egun ([src]) that wasn't in the item loop. Last touched by: [fingerprintslast]")
			processing_items.Add(src)
			return
		if (!cell)
			processing_items.Remove(src)
			return
		if (!istype(cell, /obj/item/ammo/power_cell/self_charging)) // Plain cell? No need for dynamic updates then (Convair880).
			processing_items.Remove(src)
			return
		if (cell.charge == cell.max_charge) // Keep them in the loop, as we might fire the gun later (Convair880).
			return
	...
	attack_hand(mob/user as mob)
		if ((user.r_hand == src || user.l_hand == src) && contents && contents.len)
			if (cell&&!rechargeable)
				var/obj/item/ammo/power_cell/W = cell
...
/obj/item/gun/energy/taser_gun
	...
	New()
		...
		projectiles = list(current_projectile,new/projectile/energy_bolt/burst)
		..()

	update_icon()
		if (cell)
			var/ratio = min(1, cell.charge / cell.max_charge)
			ratio = round(ratio, 0.25) * 100
			icon_state = "taser[ratio]"
			return
...
/obj/item/gun/energy/laser_gun
	...
	virtual
		...
		New()
			..()
			current_projectile = new /projectile/laser/virtual
			projectiles.len = 0
	...
	update_icon()
		if (cell)
			...
			var/maxCharge = (cell.max_charge > 0 ? cell.max_charge : 0)
			var/ratio = min(1, cell.charge / maxCharge)
			ratio = round(ratio, 0.25) * 100
			icon_state = "laser[ratio]"
			return
...
/obj/item/gun/energy/phaser_gun
	...
	New()
		cell = new/obj/item/ammo/power_cell/med_power
		current_projectile = new/projectile/laser/light
		projectiles = list(current_projectile)
		..()
...
/obj/item/gun/energy/crossbow
	...
	cell = new/obj/item/ammo/power_cell/self_charging/slowcharge
	current_projectile = new/projectile/rad_bolt
	projectiles = null
	is_syndicate = 1
...
/obj/item/gun/energy/egun
	...
	update_icon()
		if (cell)
			var/ratio = min(1, cell.charge / cell.max_charge)
			ratio = round(ratio, 0.25) * 100
			if (current_projectile.type == /projectile/energy_bolt)
				icon_state = "energystun[ratio]"
			else if (current_projectile.type == /projectile/laser)
				icon_state = "energykill[ratio]"
			else
...
/obj/item/gun/energy/vuvuzela_gun
	...
	New()
		cell = new/obj/item/ammo/power_cell/med_power
		current_projectile = new/projectile/energy_bolt_v
		projectiles = list(current_projectile)
		..()
	update_icon()
		if (cell)
...
/obj/item/gun/energy/disruptor
	...
	New()
		...
		current_projectile = new/projectile/disruptor
		projectiles = list(current_projectile,new/projectile/disruptor/burst,new/projectile/disruptor/high)
		update_icon()
		..()
...
/obj/item/gun/energy/wavegun
	...
	New()
		cell = new/obj/item/ammo/power_cell/med_power
		current_projectile = new/projectile/wavegun
		projectiles = list(current_projectile,new/projectile/wavegun/burst,new/projectile/wavegun/paralyze)
		..()
...
/obj/item/gun/energy/bfg
	...
	New()
		...
		current_projectile = new/projectile/bfg
		projectiles = list(new/projectile/bfg)
		..()
...
/obj/item/gun/energy/teleport
	...
	New()
		cell = new/obj/item/ammo/power_cell/med_power
		current_projectile = new /projectile/tele_bolt
		projectiles = list(current_projectile)
		..()
	...
	update_icon()
		...
		icon_state = "teleport[round((cell.charge / cell.max_charge), 0.25) * 100]"
		return
	...
	attack(mob/M as mob, mob/user as mob)
		if (!our_target)
			...
			return
		if (!our_teleporter || (our_teleporter.check_teleporter() != 1))
			user.show_text("Error: linked teleporter is out of order.", "red")
			return

		var/projectile/tele_bolt/TB = current_projectile
...
/obj/item/gun/energy/ghost
	...
	New()
		cell = new/obj/item/ammo/power_cell/med_power
		current_projectile = new /projectile/energy_bolt_antighost
		projectiles = list(current_projectile)
		..()
...
/obj/item/gun/energy/mod
	...
	New()
		...
		if (!current_projectile)
			current_projectile = emitter.projectile
		projectiles = list(current_projectile)
		..()
	...
	examine()
		...
		if (emitter)
			boutput(usr, "<span style=\"color:blue\">[emitter.name]</span>")
		if (cell)
			boutput(usr, "<span style=\"color:blue\">[cell.name]</span>")
		if (back)
			boutput(usr, "<span style=\"color:blue\">[back.name]</span>")
		if (top_rail)
			boutput(usr, "<span style=\"color:blue\">[top_rail.name]</span>")
		if (bottom_rail)
			boutput(usr, "<span style=\"color:blue\">[bottom_rail.name]</span>")
		..()

	/*proc/generate_overlays()
		overlays = null
		if (extension_mod)
			overlays += icon('icons/obj/gun_mod.dmi',extension_mod.overlay_name)
		if (converter_mod)
			overlays += icon('icons/obj/gun_mod.dmi',converter_mod.overlay_name)*/
	/*proc/update_icon()
		var/ratio = charges / maximum_charges
		ratio = round(ratio, 0.25) * 100
		icon_state = text("phaser[]", ratio)*/
...
/obj/item/gun_parts/emitter
	...
	icon_state = "emitter"
	var/projectile/laser/light/projectile = new/projectile/laser/light
...
/obj/item/gun/energy/owl
	...
	New()
		cell = new/obj/item/ammo/power_cell/med_power
		current_projectile = new/projectile/owl
		projectiles = list(current_projectile,new/projectile/owl/owlate)
		..()
	update_icon()
		if (cell)
			var/ratio = min(1, cell.charge / cell.max_charge)
...
/obj/item/gun/energy/shrinkray
	...
	New()
		cell = new/obj/item/ammo/power_cell/med_power
		current_projectile = new/projectile/shrink_beam
		projectiles = list(current_projectile)
		..()
	update_icon()
		if (cell)
...
/obj/item/gun/energy/glitch_gun
	...
	New()
		cell = new/obj/item/ammo/power_cell/high_power
		current_projectile = new/projectile/bullet/glitch/gun
		projectiles = list(new/projectile/bullet/glitch/gun)
		..()
...
/obj/item/gun/energy/laser_gun/pred // Made use of a spare sprite here (Convair880).
	...
	New()
		...
		current_projectile = new/projectile/laser/pred
		projectiles = list(new/projectile/laser/pred)

	update_icon()
		if (cell)
			var/ratio = min(1, cell.charge / cell.max_charge)
			ratio = round(ratio, 0.25) * 100
			icon_state = "bullpup[ratio]"
			return
