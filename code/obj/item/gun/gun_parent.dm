...
/obj/item/gun
	...
	New()
		spawn (20)
			forensic_ID = CreateID()
			forensic_IDs.Add(forensic_ID)
		return
...
/obj/item/gun/proc/shoot_point_blank(var/mob/M as mob, var/mob/user as mob)
	...
	if (!canshoot())
		if (!silenced)
			M.visible_message("<span style=\"color:red\"><strong>[user] tries to shoot [M] with [src] point-blank, but it was empty!</strong></span>")
		else
			user.show_text("*click* *click*", "red")
		return

	if (ishuman(user) && add_residue) // Additional forensic evidence for kinetic firearms (Convair880).
	...
	if (!silenced)
		for (var/mob/O in AIviewers(M, null))
			if (O.client)
				O.show_message("<span style=\"color:red\"><strong>[user] shoots [M] point-blank with [src]!</strong></span>")
	else
	...
	alter_projectile(P)
	P.forensic_ID = forensic_ID // Was missing (Convair880).
	P.was_pointblank = 1
...
/obj/item/gun/proc/alter_projectile(var/obj/projectile/P)
	if (resin)
		...
		P.resin = TRUE
		if (P.proj_data)
			P.proj_data = new P.proj_data.type
			//P.proj_data.damage_type = D_BURNING // it turns out this sets people on fire which is bad
			P.proj_data.power *= 0.66
...
/obj/item/gun/proc/shoot(var/target,var/start,var/mob/user,var/POX,var/POY)
	...
	if (!istype(target, /turf) || !istype(start, /turf))
		return
	if (!istype(current_projectile,/projectile))
		return
	...
	if (user)
		if (!silenced)
			for (var/mob/O in AIviewers(user, null))
				O.show_message("<span style=\"color:red\"><strong>[user] fires [src] at [target]!</strong></span>", 1, "<span style=\"color:red\">You hear a gunshot</span>", 2)
		else
		...
		var/turf/T = target
		logTheThing("combat", user, null, "fires \a [src] from [log_loc(user)], vector: ([T.x - user.x], [T.y - user.y]), dir: <em>[dir2text(get_dir(user, target))]</em>, projectile: <em>[P.name]</em>[P.proj_data && P.proj_data.type ? ", [P.proj_data.type]" : null]")

	if (ismob(user))
		var/mob/M = user
		if (ishuman(M) && add_residue) // Additional forensic evidence for kinetic firearms (Convair880).
			var/mob/living/carbon/human/H = user
			H.gunshot_residue = 1
...
/obj/item/gun/proc/logme_temp(mob/user as mob, obj/item/gun/G as obj, obj/item/ammo/A as obj)
	if (!user || !G || !A)
		...
		logTheThing("combat", user, null, "reloads [G] (<strong>Ammo type:</strong> <em>[G.current_projectile.type]</em>) at [log_loc(user)].")
		return

	else if (istype(G, /obj/item/gun/energy) && istype(A, /obj/item/ammo/power_cell))
		logTheThing("combat", user, null, "reloads [G] (<strong>Cell type:</strong> <em>[A.type]</em>) at [log_loc(user)].")
		return
...
/obj/item/gun/on_spin_emote(var/mob/living/carbon/human/user as mob)
	if ((user.bioHolder && user.bioHolder.HasEffect("clumsy") && prob(50)) || (user.reagents && prob(user.reagents.get_reagent_amount("ethanol") / 2)) || prob(5))
		user.visible_message("<span style=\"color:red\"><strong>[user] accidentally shoots [him_or_her(user)]self with [src]!</strong></span>")
		shoot_point_blank(user, user)
		
/obj/item/gun/resinize()
	..()
	resin = TRUE
