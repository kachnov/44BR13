	...
	*/
	...
	proc/fire_gun(mob/user as mob)
		if (shotsLeft > 1)
			...
			for (var/mob/O in AIviewers(user, null))
				if (O.client)	O.show_message("<span style=\"color:red\"><strong>BOOM!</strong> [user]'s head explodes.</span>", 1, "<span style=\"color:red\">You hear someone's head explode.</span>", 2)
