/obj/item/gun/kinetic
	...
	examine()
		set src in usr
		if (ammo && (ammo.amount_left > 0))
			desc = "There are [ammo.amount_left] bullets of [ammo.sname] left!"
		else
			desc = "There are 0 bullets left!"
		if (current_projectile)
			desc += "<br>Each shot will currently use [current_projectile.cost] bullets!"
		else
			desc += "<br><span style=\"color:red\">*ERROR* No output selected!</span>"
		..()
	...
	canshoot()
		if (ammo && current_projectile)
			if (ammo:amount_left >= current_projectile:cost)
				return TRUE
		return FALSE

	process_ammo(var/mob/user)
		if (ammo && current_projectile)
			if (ammo.use(current_projectile.cost))
				return TRUE
	...
	attackby(obj/item/ammo/bullets/b as obj, mob/user as mob)
		...
		// reload resin guns with resin
		if (resin && istype(b, /obj/item/resin))
			boutput(user, "You reload \the [src] with the piece of resin.")
			ammo.amount_left = ammo.max_amount
			var/obj/item/resin/R = b 
			R.consume(1)
			
		else if (istype(b, /obj/item/ammo/bullets))
			switch (src.ammo.loadammo(b,src))
				...
				if (4)
					...
					logme_temp(user, src, b) // Might be useful (Convair880).
					return
				if (5)
	...
	// Added this to make manual reloads possible (Convair880).
		...
		if ((loc == user) && user.find_in_hand(src)) // Make sure it's not on the belt or in a backpack.
			add_fingerprint(user)
			if (sanitycheck(0, 1) == 0)
				user.show_text("You can't unload this gun.", "red")
				return
			if (ammo.amount_left <= 0)
				// The gun may have been fired; eject casings if so.
				if ((casings_to_eject > 0) && current_projectile.casing)
					if (sanitycheck(1, 0) == 0)
						logTheThing("debug", usr, null, "<strong>Convair880</strong>: [usr]'s gun ([src]) ran into the casings_to_eject cap, aborting.")
						casings_to_eject = 0
						return
					else
						user.show_text("You eject [casings_to_eject] casings from [src].", "red")
						ejectcasings()
						return
				else
	...
	// Finished Cogwerks' former WIP system (Convair880).
		if (canshoot() && user.a_intent != "help")
			if (auto_eject)
				var/turf/T = get_turf(src)
				if (T)
					if (current_projectile.casing && (sanitycheck(1, 0) == 1))
						var/number_of_casings = max(1, current_projectile.shot_number)
						//DEBUG("Ejected [number_of_casings] casings from [src].")
						for (var/i = 1, i <= number_of_casings, i++)
							var/obj/item/casing/C = new current_projectile.casing(T)
							C.forensic_ID = forensic_ID
							C.loc = T
			else
				if (casings_to_eject < 0)
					casings_to_eject = 0
				casings_to_eject += current_projectile.shot_number
		..()
	...
	proc/ejectcasings()
		if ((casings_to_eject > 0) && current_projectile.casing && (sanitycheck(1, 0) == 1))
			var/turf/T = get_turf(src)
			if (T)
				//DEBUG("Ejected [casings_to_eject] [current_projectile.casing] from [src].")
				var/obj/item/casing/C = null
				while (casings_to_eject > 0)
					C = new current_projectile.casing(T)
					C.forensic_ID = forensic_ID
					C.loc = T
					casings_to_eject--
		return
	...
	proc/sanitycheck(var/casings = 0, var/ammo = 1)
		if (casings && (casings_to_eject > 30 || current_projectile.shot_number > 30))
			logTheThing("debug", usr, null, "<strong>Convair880</strong>: [usr]'s gun ([src]) ran into the casings_to_eject cap, aborting.")
			if (casings_to_eject > 0)
				casings_to_eject = 0
			return FALSE
		if (ammo && (src.max_ammo_capacity > 200 || src.ammo.amount_left > 200))
			logTheThing("debug", usr, null, "<strong>Convair880</strong>: [usr]'s gun ([src]) ran into the magazine cap, aborting.")
			return FALSE
		return TRUE
...
/obj/item/gun/kinetic/revolver
	...
	New()
		...
		current_projectile = new/projectile/bullet/revolver_357
		..()
...
/obj/item/gun/kinetic/derringer
	...
	New()
		...
		current_projectile = new/projectile/bullet/derringer
		..()
...
/obj/item/gun/kinetic/detectiverevolver
	...
	New()
		...
		current_projectile = new/projectile/bullet/revolver_38/stunners
		..()
...
/obj/item/gun/kinetic/spacker
	...
	New()
		...
		current_projectile = new/projectile/bullet/a12
		..()

	suicide(var/mob/usr as mob)
		if (!canshoot() || !hasvar(usr,"organHolder")) return FALSE

		process_ammo(usr)
		usr.visible_message("<span style=\"color:red\"><strong>[usr] places the [name]'s barrel in \his mouth and pulls the trigger with \his foot!</strong></span>")
...
/obj/item/gun/kinetic/riotgun
	...
	New()
		...
		current_projectile = new/projectile/bullet/abg
		..()
...
/obj/item/gun/kinetic/ak47
	...
	New()
		...
		current_projectile = new/projectile/bullet/ak47
		..()
...
/obj/item/gun/kinetic/hunting_rifle
	...
	New()
		...
		current_projectile = new/projectile/bullet/rifle_3006
		..()
...
/obj/item/gun/kinetic/dart_rifle
	...
	New()
		...
		current_projectile = new/projectile/bullet/tranq_dart
		..()
...
/obj/item/gun/kinetic/silenced_22
	...
	New()
		...
		current_projectile = new/projectile/bullet/bullet_22
		..()
...
/obj/item/gun/kinetic/vgun
	...
	New()
		...
		current_projectile = new/projectile/bullet/vbullet
		..()
...
/obj/item/gun/kinetic/flaregun
	...
	New()
		...
		current_projectile = new/projectile/bullet/flare
		..()
...
/obj/item/gun/kinetic/riot40mm
	...
	New()
		...
		current_projectile = new/projectile/bullet/smoke
		..()
...
/obj/item/gun/kinetic/rpg7
	...
	New()
		...
		current_projectile = new /projectile/bullet/rpg
		..()
		return

	update_icon()
		if (ammo.amount_left < 1)
			icon_state = "rpg7_empty"
			item_state = "rpg7_empty"
		else
			icon_state = "rpg7"
...
/obj/item/gun/kinetic/coilgun_TEST
	...
	New()
		...
		current_projectile = new/projectile/bullet/rod
		..()
