...
/obj/item/teleportation_scroll/attack_self(mob/user as mob)
	...
	if (!iswizard(user))
		user.machine = null
		boutput(user, "<span style=\"color:red\"><strong>The text is illegible!</strong></span>")
		return
	if (!uses)
		boutput(user, "<span style=\"color:blue\"><strong>The depleted scroll vanishes in a puff of smoke!</strong></span>")
		user.machine = null
		user << browse(null,"window=scroll")
		qdel(src)
		return
	dat += "<strong>Teleportation Scroll:</strong><br><br>"
	dat += "Charges left: [uses]<br><hr><br>"
...
/obj/item/teleportation_scroll/Topic(href, href_list)
	...
	if ((usr.contents.Find(src) || (in_range(src, usr) && istype(loc, /turf))))
		...
		if (href_list["spell_teleport"])
			if (uses >= 1 && usr.teleportscroll(0, 1, src) == 1)
				uses -= 1
		if (istype(loc, /mob))
			attack_self(loc)
		else
...
/obj/item/staff
	...
	// Used by /targetable/spell/summon_staff. Exposed for convenience (Convair880).
	proc/send_staff_to_target_mob(var/mob/living/M)
		if (!src || !istype(src) || !M || !istype(M))
			return

		visible_message("<span style=\"color:red\"><strong>The [name] is suddenly warped away!</strong></span>")
		var/effects/system/spark_spread/s = unpool(/effects/system/spark_spread)
		...
		if (istype(loc, /obj/item/storage))
			var/obj/item/storage/S_temp = loc
...
/obj/item/staff/cthulhu
	...
	attack_hand(var/mob/user as mob)
		if (user.mind)
			if (iswizard(user))
				if (user.mind.key != wizard_key)
					boutput(user, "<span style=\"color:red\">The [name] is magically attuned to another wizard! You can use it, but the staff will refuse your attempts to control or summon it.</span>")
				..()
				return
			else
				do_brainmelt(user, 2)
...
/obj/magicmirror
	...
	get_desc(dist)
		...
		var/T = null
		var/W_count = 0
		T = "<strong>Dark ritual of all wizards:</strong>"
		...
		for (var/mind/M in ticker.minds)
			if (M && M.special_role == "wizard" && M.current)
				W_count++
				T += "<hr>"
				T += "<strong>[M.current.real_name]'s objectives:</strong>"
				var/i = 1
				for (var/objective/O in M.objectives)
					if (istype(O, /objective/crew) || istype(O, /objective/miscreant))
						continue
		...
		if (corrupt >= 2100)
			. += "<br><span style=\"color:green\"><strong>The Corruption</strong> is at [percentage]%!</span>"
		else
			. += "<br><span style=\"color:red\"><strong>The Corruption</strong> is at [percentage]%!</span>"*/