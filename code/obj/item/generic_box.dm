...
/obj/item/item_box // for when you want something that "contains" a certain amount of an item
	...
	New()
		..()
		spawn (10)
			if (!ispath(contained_item))
				logTheThing("debug", src, null, "has a non-path contained_item, \"[contained_item]\", and is being disposed of to prevent errors")
				qdel(src)
				return
	...
	get_desc(dist)
		if (item_amount == -1)
			...
			. += "There's [item_amount] thing[item_amount == 1 ? "" : "s"] inside."
		else
			. += "It's empty."
	...
	proc/take_from()
		var/obj/item/myItem = locate(contained_item) in src
		if (myItem)
			if (item_amount >= 1)
				item_amount--
			update_icon()
			return myItem
		else if (item_amount != 0) // should be either a positive number or -1
			if (item_amount >= 1)
				item_amount--
			var/obj/item/newItem = new contained_item(src)
			update_icon()
			return newItem
		else
			return FALSE
	...
	proc/add_to(var/obj/item/I, var/mob/user)
		...
		if (!user && usr)
			user = usr
		if (!istype(I, contained_item))
			if (user)
				boutput(user, "<span style=\"color:red\">[I] doesn't fit in [src]!</span>")
			return FALSE
		if (reusable && !(item_amount >= max_item_amount || max_item_amount == -1))
			if (!open)
				if (user)
