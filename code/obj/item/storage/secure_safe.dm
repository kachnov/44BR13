...
/obj/item/storage/secure/get_desc()
	return "The service panel is [open ? "open" : "closed"]."
...
/obj/item/storage/secure/attackby(obj/item/W as obj, mob/user as mob)
	...
	//Waluigi hates this
	if (hackable)
		if ((istype(W, /obj/item/screwdriver)) && (locked == 1))
			sleep(6)
			open =! open
			user.show_message("<span style=\"color:blue\">You [open ? "open" : "close"] the service panel.</span>")
			return

		if ((istype(W, /obj/item/device/multitool)) && (open == 1) && (!locked) && (!l_hacking))
...
/obj/item/storage/secure/MouseDrop(atom/over_object, src_location, over_location)
	if ((usr.is_in_hands(src) || over_object == usr) && locked == 1)
		boutput(usr, "<span style=\"color:red\">[src] is locked and cannot be opened!</span>")
		return
	return ..()
...
<body bgcolor=#2F2F2F>
	<table border = 2 bgcolor=#7F3030 width = 150px>
		<tr><td><font face='system' size = 6 color=#FF0000 [emagged ? ">ERR" : "id = \"readout\">&nbsp;"]</font></td></tr>
	</table>
	<br>
	<table class = "keypad">
...
/*
	var/dat = "<TT><strong>[name]</strong><BR><br><br>Lock Status: [locked ? "LOCKED" : "UNLOCKED"]"
	var/message = "Code"
	if ((l_set == 0) && (!emagged) && (!l_setshort))
		dat += "<p><br><strong>5-DIGIT PASSCODE NOT SET.<br>ENTER NEW PASSCODE.</strong>"
	if (emagged)
		dat += "<p><br><font color=red><strong>LOCKING SYSTEM ERROR - 1701</strong></font>"
	if (l_setshort)
		dat += "<p><br><font color=red><strong>ALERT: MEMORY SYSTEM ERROR - 6040 201</strong></font>"
	message = "[code]"
	if (!locked)
		message = "*****"
		...
		if (configure_mode)
			...
			if (uppertext(href_list["enter"]) == code)
				...
				if (length(code_attempt) == 4)
					...
					if (desctext)
						visible_message("<span style=\"color:red\">[src]'s lock panel emits [desctext].</span>")
						playsound(loc, "sound/machines/twobeep.ogg", 55, 1) // set this to play proper beeps later
...
/obj/item/storage/secure/sbriefcase/attack(mob/M as mob, mob/user as mob)
	if (usr.bioHolder.HasEffect("clumsy") && prob(50))
		user.visible_message("<span style=\"color:red\"><strong>[usr]</strong> swings [src] too hard and nails \himself in the face.</span>")
	...
	if (t == "head")
		if (M.stat < 2 && M.health < 50 && prob(90))
			...
			M.visible_message("<span style=\"color:red\"><strong>[M] has been knocked unconscious!</strong></span>")
		else
...
/obj/item/paper/IOU
	...
	New()
		...
		desc = "Looks like \"[iou_name]\" got here first. Hope you didn't want that [iou_thing] too bad, cause unless you find whoever that is, you're probably never gunna see that thing."
		info = {"I owe you one (1):
		<u>[iou_thing]</u>
		- <em>[iou_name]</em>"}
		return
