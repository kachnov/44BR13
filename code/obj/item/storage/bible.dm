...
/obj/item/storage/bible
	...
	proc/bless(mob/M as mob)
		if (isvampire(M) || istype(M, /mob/wraith) || M.bioHolder.HasEffect("revenant"))
			M.visible_message("<span style=\"color:red\"><strong>[M] burns!</span>", 1)
			var/zone = "chest"
			if (usr.zone_sel)
	...
	attack(mob/M as mob, mob/user as mob)
		...
		if (user.bioHolder && user.bioHolder.HasEffect("clumsy") && prob(50))
			user.visible_message("<span style=\"color:red\"><strong>[user]</strong> fumbles and drops [src] on \his foot.</span>")
	...
	//		return
		...
		if (istype(M, /mob/wraith) || (M.bioHolder && M.bioHolder.HasEffect("revenant")))
			M.visible_message("<span style=\"color:red\"><strong>[user] smites [M] with the [src]!</strong></span>")
			bless(M)
			boutput(M, "<span_style=\"color:red\"><strong>IT BURNS!</strong></span>")
			if (narrator_mode)
				playsound(loc, "sound/vox/hit.ogg", 25, 1, -1)
			else
			...
			if ((istype(H, /mob/living/carbon/human) && prob(60)))
				bless(M)
				M.visible_message("<span style=\"color:red\"><strong>[user] heals [M] with the power of Christ!</strong></span>")
				...
				M.visible_message("<span style=\"color:red\"><strong>[user] beats [M] over the head with [src]!</strong></span>")
				if (narrator_mode)
					playsound(loc, "sound/vox/hit.ogg", 25, 1, -1)
				else
					playsound(loc, "punch", 25, 1, -1)
		else if (M.stat == 2)
			M.visible_message("<span style=\"color:red\"><strong>[user] smacks [M]'s lifeless corpse with [src].</strong></span>")
			if (narrator_mode)
				playsound(loc, "sound/vox/hit.ogg", 25, 1, -1)
			else
				playsound(loc, "punch", 25, 1, -1)
	...
	attack_hand(var/mob/user as mob)
		if (isvampire(user) || user.bioHolder.HasEffect("revenant"))
			user.visible_message("<span style=\"color:red\"><strong>[user] tries to take the [src], but their hand bursts into flames!</strong></span>", "<span style=\"color:red\"><strong>Your hand bursts into flames as you try to take the [src]! It burns!</strong></span>")
	...
	suicide(var/mob/user as mob)
		if (!farting_allowed)
			return FALSE
		user.visible_message("<span style=\"color:red\">[user] farts on the bible.<br><strong>A mysterious force smites [user]!</strong></span>")
		user.u_equip(src)
		layer = initial(layer)
		set_loc(user.loc)
		user.gib()
