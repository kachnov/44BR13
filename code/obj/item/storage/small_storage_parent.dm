...
/obj/item/storage
	...
	attackby(obj/item/W as obj, mob/user as mob)
		...
		var/atom/checkloc = loc // no infinite loops for you
		while (checkloc && !isturf(loc))
			if (checkloc == W) // nope
		...
		if (!sneaky && !istype(W, /obj/item/gun/energy/crossbow))
	...
	proc/mousetrap_check(mob/user)
		...
		for (var/obj/item/mousetrap/MT in src)
			if (MT.armed)
				user.visible_message("<span style=\"color:red\"><strong>[user] reaches into \the [src] and sets off a mousetrap!</strong></span>",\
				"<span style=\"color:red\"><strong>You reach into \the [src], but there was a live mousetrap in there!</strong></span>")
				MT.triggered(user, user.hand ? "l_hand" : "r_hand")
				. = 1
		for (var/obj/item/mine/M in src)
			if (M.armed && M.used_up != 1)
				user.visible_message("<span style=\"color:red\"><strong>[user] reaches into \the [src] and sets off a [M.name]!</strong></span>",\
				"<span style=\"color:red\"><strong>You reach into \the [src], but there was a live [M.name] in there!</strong></span>")
				M.triggered(user)
				. = 1
	...
	MouseDrop(atom/over_object, src_location, over_location)
		...
		if (istype(S))
			...
			if (!usr.restrained() && !usr.stat && loc == usr)
				if (S.id == "rhand")
					if (!usr.r_hand)
						usr.u_equip(src)
	...
	attack_hand(mob/user as mob)
		if (!sneaky)
			playsound(loc, "rustle", 50, 1, -5)
		if (loc == user && (!does_not_open_in_pocket || src == user.l_hand || src == user.r_hand))
			if (ishuman(user))
...
/obj/item/storage/pill_bottle/attack_hand(mob/user as mob)
	...
	if (ishuman(user) && loc == user)
		var/mob/living/carbon/human/H = user
		if (H.stats && H.stats.getStat(STAT_IQ) < PILL_BOTTLE_IQ_REQUIREMENT)
			boutput(H, "<span style = \"color:red\">Your IQ is too low to open \the [src].</span>")
			return FALSE
