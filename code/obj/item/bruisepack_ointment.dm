/obj/item/medical
	...
	examine()
		...
		boutput(usr, "[bicon(src)] <span style=\"color:blue\">There [amount == 1 ? "is" : "are"] [amount] [name]\s left on the stack!</span>")
		return

	attack_hand(mob/user as mob)
		if (user.r_hand == src || user.l_hand == src)
			add_fingerprint(user)
			var/obj/item/medical/split = new type(user)
			split.amount = 1
	...
	attackby(obj/item/medical/W as obj, mob/user as mob)
		...
		if (W.amount + amount > 5)
			amount = (W.amount + amount) - 5
			W.amount = 5
		else
			W.amount += amount
			qdel(src)
	...
	attack(mob/M as mob, mob/user as mob)
		...
		if (ishuman(M))
			...
			M.HealDamage("All", heal_brute, heal_burn)
