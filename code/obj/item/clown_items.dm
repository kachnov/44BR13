	...
	if (spam_flag == 0)
		if (ishuman(user))
			...
			if (H.sims)
				H.sims.affectMotive("fun", 5)
		spam_flag = 1
		user.visible_message("<strong>[user]</strong> plays a [pick("delightful", "chilling", "upbeat")] tune with \his harmonica!")
		playsound(loc, pick(sounds_harmonica), 50, 1)
	...
	if (spam_flag == 0)
		if (ishuman(user))
			...
			if (H.sims)
				H.sims.affectMotive("fun", 5)
		spam_flag = 1
		user.visible_message("<span style=\"color:red\"><strong>[user]</strong> blows [src]!</span>")
		playsound(loc, sound_whistle, 35, 1)
		for (var/obj/critter/dog/george/G in range(user,6))
			if (prob(60))
				G.howl()
		add_fingerprint(user)
...
/obj/item/whistle/suicide(var/mob/user as mob)
	user.visible_message("<span style=\"color:red\"><strong>[user] swallows the [name]. \He begins to choke, the [name] sounding shrilly!</strong></span>")
	user.take_oxygen_deprivation(175)
	user.updatehealth()

	//fuck it that'll do
	var/whistlesound = sound_whistle //so we can still use it when the whistle is deleted
...
/obj/item/vuvuzela/attack_self(mob/user as mob)
	if (spam_flag == 0)
		...
		playsound(loc, sound_vuvuzela, 80, 1)
		for (var/obj/critter/dog/george/G in range(user,6))
			if (prob(60))
				G.howl()
		add_fingerprint(user)
