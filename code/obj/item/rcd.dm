...
/obj/item/rcd/proc/create_door(var/turf/A, mob/user as mob)
	...
	if (do_after(user, 50))
		if (rcd_ammocheck(user, matter_create_door))
			...
			playsound(loc, "sound/effects/sparks2.ogg", 50, 1)
...
/* /obj/item/rcd/attack(mob/M as mob, mob/user as mob, def_zone)
	if (ishuman(M) && matter >= 3)
		...
		if (H.stat != 2 && H.health > 0)
			...
			return
		boutput(user, "<span style=\"color:red\"><strong>You shove the RCD down [H]'s mouth and pull the trigger!</strong></span>")
		H.show_message("<span style=\"color:red\"><strong>[user] is shoving an RCD down your throat!</strong></span>", 1)
		for (var/mob/N in viewers(user, 3))
			if (N.client && N != user && N != H)
				N.show_message(text("<span style=\"color:red\"><strong>[] shoves the RCD down []'s throat!</strong></span>", user, H), 1)
		playsound(loc, "sound/machines/click.ogg", 50, 1)
		if (do_after(user, 20))
			spark_system.set_up(5, 0, src)
			spark_system.start()
...
/obj/item/rcd/proc/rcd_ammocheck(mob/user as mob, var/checkamt = 0)
	if (istype(user,/mob/living/silicon/robot))
		...
		if (R.cell.charge >= checkamt * 5) return TRUE
		else return FALSE
		...
		if (matter >= checkamt) return TRUE
		else return FALSE
