...
/obj/item/spraybottle/New()
	var/reagents/R = new/reagents(100) // cogwerks - lowered from 1000 (what the hell) to 100
	reagents = R
	R.my_atom = src
...
/obj/item/spraybottle/detective
	...
	examine()
		set src in usr
		boutput(usr, "[bicon(src)] [reagents.total_volume] units of luminol left!")
		..()
		return
...
/obj/item/mop/afterattack(atom/A, mob/user as mob)
	...
	if (istype(A, /turf/simulated) || istype(A, /obj/decal/cleanable))
		user.visible_message("<span style=\"color:red\"><strong>[user] begins to clean [A]</strong></span>")
		...
		if (reagents.total_volume && (!reagents.has_reagent("water") || (reagents.has_reagent("water") && reagents.reagent_list.len > 1)))
			logTheThing("combat", user, null, "mops [U && isturf(U) ? "[U]" : "[A]"] with chemicals [log_reagents(src)] at [log_loc(user)].")

		if (U && isturf(U))
			reagents.reaction(U,1,10)
			mopcount++
...
/obj/item/sponge/attack_self(mob/user as mob)
	...
	if (location)
		reagents.reaction(location,1,1)
...
/obj/item/sponge/throw_impact(atom/hit)
	if (hit && ishuman(hit))
		if (prob(hit_face_prob))
			...
			hit.visible_message("<span style=\"color:red\"><strong>[src] hits [DUDE] squarely in the face!</strong></span>")
...
/obj/item/sponge/afterattack(atom/target, mob/user as mob)
	...
	else if (istype (target, /obj/item/reagent_containers/glass/bucket) && reagents.total_volume) // popup "dry/wet" interactions with these
		user.visible_message("<span style=\"color:red\">[user] wrings [src] out into [target].</span>")
		reagents.trans_to(target, 10)
		return
