/obj/item/pinpointer
	...
	proc/work()
		...
		if (!target)
			...
			if (!target)
				active = 0
				icon_state = "[icon_type]_pinonnull"
				return
		dir = get_dir(src,target)
		switch(get_dist(src,target))
			if (0)
				icon_state = "[icon_type]_pinondirect"
			if (1 to 8)
				icon_state = "[icon_type]_pinonclose"
			if (9 to 16)
				icon_state = "[icon_type]_pinonmedium"
			if (16 to INFINITY)
				icon_state = "[icon_type]_pinonfar"
		spawn (5) .()
	...
	attack_self()
		if (!active)
			...
			for (var/obj/item/card/id/I in world)
				for (var/objective/regular/assassinate/A in owner.mind.objectives)
