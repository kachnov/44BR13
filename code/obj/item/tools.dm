...
/obj/item/wirecutters
	...
	attack_self(mob/user as mob)
		...
		if (prob(fuckup_chance))
			user.visible_message("<span style=\"color:red\"><strong>[user.name]</strong> accidentally cuts [himself_or_herself(user)] while fooling around with [src] and drops them!</span>")
			playsound(loc, "sound/effects/bloody_stab.ogg", 50, 1, -6)
			user.TakeDamage(user.zone_sel.selecting, 3, 0)
			take_bleeding_damage(user, user, 3, DAMAGE_CUT)
			user.drop_item() // Damn message spam.
			return
		else
			user.visible_message("<strong>[user.name]</strong> snips [src].")
			playsound(loc, "sound/items/Wirecutter.ogg", 50, 1, -6)
			sleep(3)
			playsound(loc, "sound/items/Wirecutter.ogg", 50, 1, -6)
		return
...
/obj/item/screwdriver/suicide(var/mob/user as mob)
	user.visible_message("<span style=\"color:red\"><strong>[user] jams the screwdriver into \his eye over and over and over.</strong></span>")
...
/obj/item/weldingtool/New()
	var/reagents/R = new/reagents(20)
	reagents = R
	R.my_atom = src
...
/obj/item/weldingtool/examine()
	set src in usr
	set category = "Local"
	boutput(usr, "[bicon(src)] [name] contains [get_fuel()] units of fuel left!")
	return
...
/obj/item/weldingtool/afterattack(obj/O as obj, mob/user as mob)
	if (istype(O, /obj/reagent_dispensers/fueltank) && get_dist(src,O) <= 1)
		...
		playsound(loc, "sound/effects/zzzt.ogg", 50, 1, -6)
...
/*	L:head:up broke for no reason so I had to rewrite it.
	...
	switch(safety)
		...
		if (-1)
			boutput(usr, "<span style=\"color:red\"><strong>Your thermals intensify the welder's glow. Your eyes itch and burn severely.</strong></span>")
...
/obj/item/weldingtool/temperature_expose(gas_mixture/air, exposed_temperature, exposed_volume)
...
/obj/item/weldingtool/attack(mob/living/carbon/M as mob, mob/living/carbon/user as mob)
	if (!welding)
		if (!cautery_surgery(M, user, 0, welding))
			return ..()
	...
	if (ishuman(M))
		...
		if (H.bleeding || (H.butt_op_stage == 4 && user.zone_sel.selecting == "chest"))
			if (!cautery_surgery(H, user, 15, welding))
				return ..()
