...
/obj/item/table_parts/attackby(obj/item/W as obj, mob/user as mob)
	if (istype(W, /obj/item/wrench))
		var/atom/A = new /obj/item/sheet( loc )
		if (material)
...
/obj/item/table_parts/attack_self(mob/user as mob)
	...
	if (material)
		newTable.setMaterial(material)

	newTable.add_fingerprint(user)
	logTheThing("station", user, null, "builds a table (<strong>Material:</strong> [newTable.material && newTable.material.mat_id ? "[newTable.material.mat_id]" : "*UNKNOWN*"]) at [log_loc(user)].")
	user.u_equip(src)
	qdel(src)
	return
/*
...
/obj/item/table_parts/Topic(href, href_list)
	...
	if (loc == usr)
		if (href_list["dir"])
			...
			logTheThing("station", usr, null, "builds a table (<strong>Material:</strong> [newTable.material && newTable.material.mat_id ? "[newTable.material.mat_id]" : "*UNKNOWN*"]) at [log_loc(usr)].")
...
/*
	...
	logTheThing("station", user, null, "builds a table (<strong>Material:</strong> [T.material && T.material.mat_id ? : "[T.material.mat_id]" : "*UNKNOWN*"]) at [log_loc(user)].")
...
/obj/item/rack_parts/attack_self(mob/user as mob)
	...
	if (material)
		...
		var/material/M = getCachedMaterial("steel")
		R.setMaterial(M)
	logTheThing("station", user, null, "builds a rack (<strong>Material:</strong> [R.material && R.material.mat_id ? "[R.material.mat_id]" : "*UNKNOWN*"]) at [log_loc(user)].")
