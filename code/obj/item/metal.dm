...
/obj/item/rods
	...
	attack_hand(mob/user as mob)
		if ((user.r_hand == src || user.l_hand == src) && amount > 1)
			...
			if (material) new_stack.setMaterial(material)
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/weldingtool))
			...
			if (WELD.welding)
				...
				user.visible_message("<span style=\"color:red\"><strong>[user]</strong> welds the rods together into metal.</span>")
...
/obj/item/sheet/metal
	...
	metal/examine()
		...
		..()
		boutput(usr, text("There are [] metal sheet\s on the stack.", amount))
		return
	...
	attack_hand(mob/user as mob)
		if ((user.r_hand == src || user.l_hand == src) && amount > 1)
			...
			amount = diff
			var/obj/item/sheet/metal/new_stack = new type(usr.loc, diff)
			if (material) new_stack.setMaterial(material)
			new_stack.amount = splitnum
			new_stack.attack_hand(user)
			new_stack.add_fingerprint(user)
...
/obj/item/sheet/r_metal
	...
	attack_hand(mob/user as mob)
		if ((user.r_hand == src || user.l_hand == src) && amount > 1)
			...
			boutput(usr, "<span style=\"color:blue\">You take [splitnum] sheets from the stack, leaving [diff] sheets behind.</span>")
			amount = diff
			var/obj/item/sheet/r_metal/new_stack = new type(usr.loc, diff)
			if (material) new_stack.setMaterial(material)
			new_stack.amount = splitnum
			new_stack.attack_hand(user)
			new_stack.add_fingerprint(user)
