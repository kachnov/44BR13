...
/obj/item/paper/New()
	...
	if (!offset)
		...
		pixel_x = rand(-9, 9)
	spawn (0)
		if (info && icon_state == "paper_blank")
			icon_state = "paper"
	return
...
/obj/item/paper/examine()
	...
	..()
	. = info
	if (form_startpoints && form_endpoints)
		for (var/x = form_startpoints.len, x > 0, x--)
			. = copytext(., 1, form_startpoints[form_startpoints[x]]) + "<a href='byond://?src=\ref[src];form=[form_startpoints[x]]'>" + copytext(., form_startpoints[form_startpoints[x]], form_endpoints[form_endpoints[x]]) + "</a>" + copytext(., form_endpoints[form_endpoints[x]])

	var/font_junk = ""
	for (var/i in fonts)
		font_junk += "<link href='http://fonts.googleapis.com/css?family=[i]' rel='stylesheet' type='text/css'>"

	usr << browse("<HTML><HEAD><TITLE>[name]</TITLE>[font_junk]</HEAD><BODY><TT>[.]</TT></BODY></HTML>", "window=[name][(sizex || sizey) ? {";size=[sizex]x[sizey]"} : ""]")
	onclose(usr, "[name]")
	return null
...
/obj/item/paper/Map/examine()
	...
	if (!( ishuman(usr) || isobserver(usr) || issilicon(usr) ))
		usr << browse("<HTML><HEAD><TITLE>[name]</TITLE></HEAD><BODY><TT>[stars(info)]</TT></BODY></HTML>", "window=[name]")
		onclose(usr, "[name]")
	else
		usr << browse("<HTML><HEAD><TITLE>[name]</TITLE></HEAD><BODY><TT>[info]</TT></BODY></HTML>", "window=[name]")
		onclose(usr, "[name]")
	return

/obj/item/paper/suicide(var/mob/user as mob)
	user.visible_message("<span style=\"color:red\"><strong>[user] cuts \himself over and over with the paper.</strong></span>")
...
/obj/item/paper/attack_ai(var/mob/living/silicon/ai/user as mob)
	if (!isAI(user) || (user.current && get_dist(src, user.current) < 2)) //Wire: fix for undefined variable /mob/living/silicon/robot/var/current
		...
		for (var/i in fonts)
			font_junk += "<link href='http://fonts.googleapis.com/css?family=[i]' rel='stylesheet' type='text/css'>"
		usr << browse("<HTML><HEAD><TITLE>[name]</TITLE>[font_junk]</HEAD><BODY><TT>[info]</TT></BODY></HTML>", "window=[name]")
		onclose(usr, "[name]")
	return
...
/obj/item/paper/Topic(href, href_list)
	...
	if (href_list["form"] && istype(usr.equipped(), /obj/item/pen))
		...
		if (. in form_startpoints)
			var/t = input(usr, "What text do you wish to add?", "[name]", null) as null|text
			if (!t)
				return
			...
			info = copytext(info, 1, form_startpoints[.]) + "" + t + "" + copytext(info, form_startpoints[.] + length(t))
...
/obj/item/paper/attackby(obj/item/P as obj, mob/user as mob)
	...
	if (istype(P, /obj/item/pen))
		...
		if (pen.uses_handwriting)
			...
			if (islist(fonts) && !fonts[custom_font])
				fonts[custom_font] = 1
			custom_font += ", cursive"
			custom_size += rand(0,4)

		else if (pen.webfont && islist(fonts) && !fonts[pen.webfont])
			fonts[pen.webfont] = 1

		var/t = input(user, "What text do you wish to add?", "[name]", null) as null|message
		if (!t)
			return
		...
		t = replacetext(t, "\n", "<BR>")
		t = replacetext(t, "\[b\]", "<strong>")
		t = replacetext(t, "\[/b\]", "</strong>")
		t = replacetext(t, "\[i\]", "<em>")
		t = replacetext(t, "\[/i\]", "</em>")
		t = replacetext(t, "\[u\]", "<U>")
		t = replacetext(t, "\[/u\]", "</U>")
		...
		if (findtext(t, "\[sign\]") || findtext(t, "\[signature\]"))
			...
			if (islist(fonts) && !fonts[writing_style])
				fonts[writing_style] = 1
		...
		info += "<span style='font-family: [custom_font]; color: [custom_color]; font-size: [custom_size]px'> [t] </span>"

		//info += "<font face=[custom_font] color=[custom_color] size='3'> [t] </font>" // shit's hard to read at size 2 goddamn
		// bad font arguments don't seem to do much

		build_formpoints()

		if (icon_state == "paper_blank" && info)
			icon_state = "paper"
	else
		if (istype(P, /obj/item/stamp) && stampable)
			if ((!in_range(src, usr) && src.loc != user && !( istype(src.loc, /obj/item/clipboard) ) && src.loc.loc != user && user.equipped() != P))
				return
			info += "<BR><em>This paper has been stamped with the [P.name].</em><BR>"
			icon_state = "paper_stamped"
			...
			var/obj/item/paper_mask/M = new /obj/item/paper_mask(loc)
...
/obj/item/paper/proc/build_formpoints()
	...
	. = 0
	while (formStart)
		formStart = findtext(info, "__", formStart)
		if (formStart)
			formEnd = formStart + 1
			while (copytext(info, formEnd, formEnd+1) == "_")
				formEnd++
...
/obj/item/paper/Cloning
	...
	<em>Notice, cloning system cannot scan inorganic life or small primates.  Scan may fail if subject has suffered extreme brain damage.</em><br>
	...
	<em>Please note that resulting clones may have a small DEVELOPMENTAL DEFECT as a result of genetic drift.</em><br>
	<h4>Profile Management</h4>
	<p>The H-87 (as well as your station's standard genetics machine) can accept STANDARD DATA DISKETTES.
	These diskettes are used to transfer genetic information between machines and profiles.
	A load/save dialog will become available in each profile if a disk is inserted.</p><br>
	<em>A good diskette is a great way to counter aforementioned genetic drift!</em><br>
	<br>
...
/obj/item/paper/Wizardry101
	...
	<li>Wizard's hat</li><dd><em>- Required for spellcasting, snazzy. Don't let others remove it from you!</em></dd>
	<li>Wizard's robe</li><dd><em>- Required for spellcasting, comfy. Don't let others remove it from you!</em></dd>
	<li>Magic sandals</li><dd><em>- Keeps you from slipping on ice and from falling down after being hit by a runaway segway. They also double as galoshes.</em></dd>
	<li>Wizard's staff</li><dd><em>- Your spells will be greatly weakened, not last as long and take longer to recharge if you cast them without one of these. The staff can be easily lost if you are knocked down!</em></dd>
	<li>Teleportation scroll</li><dd><em>- Allows instant teleportation to an area of your choice. The scroll has four charges. Don't lose it though, or you can't get back to the shuttle without knowing the <strong><em>teleport</strong></em> spell, or dying while <strong><em>soulguard</strong></em> is active!</em></dd>
	<li>Spellbook</li><dd><em>- This is your personal spellbook that gives you access to the Wizarding Archives, allowing you to choose 4 spells with which to complete your objectives. The spellbook only works for you, and can be discarded after its uses are expended.</em></dd>
	<br><br><br><hr>Spells every wizard starts with:<br><br>
	<li>Magic missile (20 seconds)</li><dd><em>- This spell fires several slow-moving projectiles at nearby targets. If they hit a target, it is stunned and takes minor damage.</em></dd>
	<li>Phase shift (30 seconds)</li><dd><em>- This spell briefly turns your form ethereal, allowing you to pass invisibly through anything.</em></dd>
	<li>Clairvoyance (60 seconds)</li><dd><em>- This spell will tell you the location of those you target with it. It will also inform you if they are hiding inside something, or are dead.</em></dd>
	<br><br><br>Click the question mark in your <strong>spellbook</strong> to learn more about certain spells.<br>Recommended loadout for beginners: <strong><em>ice burst, blink, shocking touch, blind</em></strong>
	<br><br><br><center>Remember, the wizard shuttle is your home base.<br>There is a vendor and wardrobe here to dispense backup wizardly apparel and staves, a <strong>Magix System IV</strong> computer to teleport you into the station, and this is your safe point of return if you are killed while the <strong><em>soulguard enchantment</strong></em> is active.
...
/obj/item/paper/Map
	...
	New()
		..()
		info = {"<IMG SRC="[resource("images/map.png")]">
<BR>
CQ: Crew Quarters<BR>
L: Lounge<BR>
CH: Chapel<BR>
...
/obj/item/paper/engine
	...
	info = {"<strong>Startup Procedure for Mark II Thermo-Electric Generators</strong><BR>
Standard checklist for thermo-electric generator cold-start:
<HR>
<ol>
<li>Perform visual inspection of the <strong>HOT (left)</strong> and <strong>COLD (right)</strong> coolant-exchange pipe loops. Weld any breaks or cracks in the pipes before continuing.
...
<em>Note:</em> Observe standard canister safety procedures. Additional canisters may be utilized or mixed together for various thermodynamic effects. CO2 and N2 can be effective moderators.
...
<em>If you wish to use the supplemental combustion chamber instead of or in addition to the furnaces, close the bypass and open the inlet and outlet valves above it.</em><BR>
...
<strong>*Direct combustion of internal coolant may void your engine warranty and result in: fire, explosion, death, and/or property damage.</strong><BR>
...
<HR>
<em>Warning!</em> Improper engine and generator operation may cause exposure to hazardous gasses, extremes of heat and cold, and dangerous electrical voltages.
...
/obj/item/paper/zeta_boot_kit
	...
	info = {"<center><strong>RECOVERY INSTRUCTIONS:</strong></center><ul>
			...
			<strong>DEVICES MAY NEED TO BE RESET BEFORE THEY ARE SEEN BY THE OPERATING SYSTEM</strong>"}

/obj/item/paper/note_from_mom
	name = "note from mom"
	desc = "Aw dang, mooom!"
	info = "Good luck on your adventure, sweetie! Love, Mom.<br><em>Whose mom? Yours? Who knows.</em>"
...
/obj/item/paper/printout
	...
	New()
		..()
		info = {"<IMG SRC="sstv_cachedimage.png">"}
		return
...
/obj/item/paper_bin/proc/update()
	icon_state = "paper_bin[(amount || locate(/obj/item/paper, src)) ? "1" : null]"
	return
...
/obj/item/paper_bin/attack_hand(mob/user as mob, unused, flag)
	...
	if (paper)
		...
		if (amount >= 1 && usr) //Wire: Fix for Cannot read null.loc (&& usr)
			amount--
...
/obj/item/stamp
	...
	suicide(var/mob/user as mob)
		user.visible_message("<span style=\"color:red\"><strong>[user] stamps 'VOID' on \his forehead!</strong></span>")
...
/obj/item/stamp/random
	New()
		..()
		icon_state = "stamp-[pick("hos", "cap", "qm", "hop")]"
