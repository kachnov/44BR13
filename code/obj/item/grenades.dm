...
/obj/item/old_grenade
	...
	attack_self(mob/user as mob)
		...
		if (!state)
			...
			logTheThing("combat", user, null, "primes a grenade ([type]) at [log_loc(user)].")
			if (user && user.bioHolder.HasEffect("clumsy"))
				...
				playsound(loc, sound_armed, 75, 1, -3)
		...
		if (user.equipped() == src)
			if (!state)
				...
				spawn (det_time)
					...
					return
			user.drop_item()
			throw_at(get_turf(target), 10, 3)
			add_fingerprint(user)
		return

	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/screwdriver))
			if (det_time == org_det_time)
				det_time = alt_det_time
				user.show_message("<span style=\"color:blue\">You set [src] for a [det_time/10] second detonation time.</span>")
				desc = "It is set to detonate in [det_time/10] seconds."
			else
				det_time = org_det_time
				user.show_message("<span style=\"color:blue\">You set [src] for a [det_time/10] second detonation time.</span>")
				desc = "It is set to detonate in [det_time/10] seconds."
			add_fingerprint(user)
		return
	...
	prime()
		...
		if (T)
			if (isrestrictedz(T.z))
				visible_message("<span style=\"color:red\">[src] buzzes for a moment, then self-destructs.</span>")
...
/obj/item/old_grenade/smoke
	...
	var/effects/system/bad_smoke_spread/smoke

	New()
		..()
		if (usr && usr.loc) //Wire: Fix for Cannot read null.loc
			smoke = new /effects/system/bad_smoke_spread/
			smoke.attach(src)
...
/obj/item/old_grenade/smoke/mustard
	...
	var/effects/system/mustard_gas_spread/mustard_gas

	New()
		..()
		if (usr && usr.loc) //Wire: Fix for Cannot read null.loc
			mustard_gas = new /effects/system/mustard_gas_spread/
			mustard_gas.attach(src)
			mustard_gas.set_up(5, 0, usr.loc)
...
/obj/item/old_grenade/moustache
	...
	prime()
		...
		if (T)
			...
			E.fingerprintslast = fingerprintslast
...
/obj/item/gimmickbomb/
	...
	proc/detonate()
		playsound(loc, sound_explode, 100, 1)

		var/obj/effects/explosion/E = new /obj/effects/explosion(loc)
		E.fingerprintslast = fingerprintslast
	...
	proc/arm(mob/usr as mob)
		usr.show_message("<span style=\"color:red\"><strong>You have armed the [name]!</span>")
		for (var/mob/O in viewers(usr))
			if (O.client)
				O.show_message("<span style=\"color:red\"><strong>[usr] has armed the [name]! Run!</strong></span>", 1)
...
/obj/item/breaching_charge
	...
	proc/boom()
		...
		if (location && istype(location))
			...
			explosion(src, location, expl_devas, expl_heavy, expl_light, expl_flash)

			// Breaching charges should be, you know, actually be decent at breaching walls and windows (Convair880).
			for (var/turf/simulated/wall/W in range(expl_range, location))
				if (W && istype(W))
					W.ReplaceWithFloor()
			for (var/obj/structure/girder/G in range(expl_range, location))
				if (G && istype(G))
					qdel(G)
			for (var/obj/window/WD in range(expl_range, location))
				if (WD && istype(WD) && prob(max(0, 100 - (WD.health / 3))))
					WD.smash()
			for (var/obj/grille/GR in range(expl_range, location))
...
/obj/item/breaching_charge/thermite
	...
	boom()
		...
		if (location && istype(location))
			...
			for (var/turf/T in range(expl_range, location))
			...
			for (var/obj/structure/girder/G in range(expl_range, location))
				if (G && istype(G))
					qdel(G)
			for (var/obj/window/W in range(expl_range, location))
				if (W && istype(W))
					W.damage_heat(500)
			for (var/obj/grille/GR in range(expl_range, location))
				if (GR && istype(GR) && GR.ruined != 1)
			...
			for (var/mob/living/M in range(expl_range, location))
			...
			spawn (100)
				if (src)
					for (var/obj/overlay/O in range(expl_range, location))
						if (O.name == "Thermite")
							qdel(O)
					qdel(src)
		else
	...
	attackby(obj/item/W, mob/user)
		if (istype(W, /obj/item/weldingtool) && state == 1)
			if (W:welding == 1)
				...
				if (material)
					name = "hollow [material.name] pipe frame"
				else
		...
		if (istype(W, /obj/item/reagent_containers) && state == 2)
			...
			reagents = new /reagents(20)
			reagents.my_atom = src
			W.reagents.trans_to(src, 20)
			for (var/id in reagents.reagent_list)
				var/reagent/R = reagents.reagent_list[id]
				avg_volatility += R.volatility * R.volume / reagents.total_volume

			qdel(reagents)
			if (avg_volatility < 1) // B A D.
				strength = 0
			else
				strength *= avg_volatility
			...
			if (material)
				name = "filled [material.name] pipe frame"
			else
		...
		if (istype(W, /obj/item/cable_coil) && state == 3)
			...
			if (material)
				name = "[material.name] pipe bomb frame"
			else
...
/obj/proc/on_blowthefuckup(strength)
	if (material)
		material.triggerTemp(src, T0C + strength * 100)
		material.triggerExp(src, 1)
...
/obj/proc/blowthefuckup(var/strength = 1) // dropping this to object-level so that I can use it for other things
	var/T = get_turf(src)
	visible_message("<span style=\"color:red\">[src] explodes!</span>")
	...
	for (var/mob/living/carbon/human/M in view(T, shrapnel_range))
		if (istype(M.wear_suit, /obj/item/clothing/suit/armor))
			boutput(M, "<span style=\"color:red\"><strong>Your armor blocks the shrapnel!</strong></span>")
			...
			boutput(M, "<span style=\"color:red\"><strong>You are struck by shrapnel!</strong></span>")
			if (!M.stat)
				M.emote("scream")
...
/mob/proc/blowthefuckup(var/strength = 1,var/visible_message = 1) // similar proc for mobs
	var/T = get_turf(src)
	if (visible_message) visible_message("<span style=\"color:red\">[src] explodes!</span>")
	var/sqstrength = sqrt(strength)
	var/shrapnel_range = 3 + sqstrength
	for (var/mob/living/carbon/human/M in view(T, shrapnel_range))
		if (M != src && istype(M.wear_suit, /obj/item/clothing/suit/armor))
			boutput(M, "<span style=\"color:red\"><strong>Your armor blocks the chunks of [name]!</strong></span>")
			M.TakeDamage("chest", 10, 0)
		else if (M != src)
			M.TakeDamage("chest", 15, 0)
			var/obj/item/implant/projectile/shrapnel/implanted = new /obj/item/implant/projectile/shrapnel(M)
			...
			M.implant += implanted
			implanted.implanted(M, null, 25 * sqstrength)
			boutput(M, "<span style=\"color:red\"><strong>You are struck by chunks of [name]!</strong></span>")
			if (!M.stat)
				M.emote("scream")
