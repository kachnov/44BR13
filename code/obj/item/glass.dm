...
/obj/item/sheet/glass
	...
	attack_hand(mob/user as mob)
		if ((user.r_hand == src || user.l_hand == src) && amount > 1)
			...
			amount = diff
			var/obj/item/sheet/glass/new_stack = new type(usr.loc, diff)
	...
	attackby(obj/item/W, mob/user)
		...
		if ( W.type == type )
			...
			if (G.amount >= max_stack)
				boutput(user, "<span style=\"color:red\">You can't put any more sheets in this stack!</span>")
				return
			if (G.amount + amount > max_stack)
				amount = min(G.amount + amount, max_stack)
				G.amount = 5
	...
	examine()
		...
		..()
		boutput(usr, text("There are [] glass sheet\s on the stack.", amount))
		return
	...
	if (ismob(AM))
		...
		if (ishuman(M))
			...
			if (istype(H.mutantrace, /mutantrace/abomination))
				return
			if (!H.shoes)
				boutput(H, "<span style=\"color:red\"><strong>You step in the broken glass!</strong></span>")
				playsound(loc, "sound/misc/glass_step.ogg", 50, 1)
...
/obj/item/shard/crystal
	...
	g_amt = 0
	New()
		icon_state = pick("clarge", "cmedium", "csmall")
		switch(icon_state)
			if ("csmall")
				pixel_x = rand(1, 18)
				pixel_y = rand(1, 18)
	...
	HasEntered(AM as mob|obj)
		if (ismob(AM))
			var/mob/M = AM
			boutput(M, "<span style=\"color:red\"><strong>You step on the crystal shard!</strong></span>")
			playsound(loc, "sound/misc/glass_step.ogg", 50, 1)
			if (ishuman(M))
				var/mob/living/carbon/human/H = M
				var/obj/item/affecting = H.organs[pick("l_leg", "r_leg")]
