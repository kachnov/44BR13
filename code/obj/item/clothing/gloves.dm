...
/obj/item/clothing/gloves
	...
	New()
		...
		spawn (20)
			...
			if (glove_IDs) // fix for Cannot execute null.Add(), maybe??
				glove_IDs.Add(glove_ID)
		return

	examine()
		..()
		if (stunready)
			boutput(usr, "It seems to have some wires attached to it.[max_uses > 0 ? " There are [uses]/[max_uses] charges left!" : ""]")
		return
	...
	attackby(obj/item/W, mob/user)
		if (istype(W, /obj/item/cable_coil))
			if (!can_be_charged)
				user.show_text("The [name] cannot be electrically charged.", "red")
				return
			if (stunready)
				user.show_text("You don't need to add more wiring to the [name].", "red")
				return

			boutput(user, "<span style=\"color:blue\">You attach the wires to the [name].</span>")
			stunready = 1
			material_prints += ", electrically charged"
			W:amount--
			return
		...
		if (istype(W, /obj/item/cell)) // Moved from cell.dm (Convair880).
			...
			if (C.charge < 2500)
				...
				return
			if (!stunready)
				user.visible_message("<span style=\"color:red\"><strong>[user]</strong> shocks themselves while fumbling around with [C]!</span>", "<span style=\"color:red\">You shock yourself while fumbling around with [C]!</span>")
				C.zap(user)
				return
			...
			if (can_be_charged)
				...
				user.visible_message("<span style=\"color:red\"><strong>[user]</strong> charges [his_or_her(user)] stun gloves.</span>", "<span style=\"color:blue\">The stun gloves now hold [uses]/[max_uses] charges!</span>")
			else
				user.visible_message("<span style=\"color:red\"><strong>[user]</strong> shocks themselves while fumbling around with [C]!</span>", "<span style=\"color:red\">You shock yourself while fumbling around with [C]!</span>")
				C.zap(user)
			return
	...
	proc/distort_prints(var/prints as text, var/get_glove_ID = 1) // Ditto (Convair880).
		...
		if (!hide_prints)
			...
			// Known bug: occasionally breaks the format of the forensic scanner's readout. Dunno how to account for that yet (Convair880).
				...
				if (material_prints)
					data += material_prints
				else
					data += "unknown fiber material"

		if (get_glove_ID)
			data += " (Glove ID: [glove_ID])" // Space is required for formatting (Convair880).
			...
			for (var/count=0, count<4, count++)
				...
				if (istype(target_r, /obj/machinery/power/generatorTemp))
					...
					switch(user:a_intent)
						if ("harm")
							electrocute(target_r, 100, netnum)
							break
						if ("disarm")
							target_r:weakened += 3
							break
