...
/obj/item/clothing/mask/predator
	...
	New()
		..()
		vchange = new(src) // Built-in voice changer (Convair880).
	...
	suicide(var/mob/user as mob)
		if (ishuman(user))
			...
			if (istype(H.w_uniform, /obj/item/clothing/under/gimmick/owl) && !(user.stat || user.paralysis))
				user.visible_message("<span style=\"color:red\"><strong>[user] hoots loudly!</strong></span>")
				user.owlgib()
				return TRUE
			else
...
/obj/item/clothing/mask/cursedclown_hat/equipped(var/mob/user, var/slot)
	...
	if (istype(Victim) && slot == "mask")
		boutput(user, "<span style=\"color:red\"><strong> The mask grips your face!</strong></span>")
...
/obj/item/clothing/mask/cursedclown_hat/suicide(var/mob/user, var/slot)
	user.visible_message("<span style=\"color:red\"><strong>[user] gazes into the eyes of the [name]. The [name] gazes back!</strong></span>") //And when you gaze long into an abyss, the abyss also gazes into you.
	spawn (10)
		playsound(loc, "sound/effects/chanting.ogg", 25, 0, 0)
		playsound(loc, pick("sound/voice/cluwnelaugh1.ogg","sound/voice/cluwnelaugh2.ogg","sound/voice/cluwnelaugh3.ogg"), 25, 0, 0)
		spawn (15)
			user.emote("scream")
			spawn (15)
				user.implode()
...
/obj/item/clothing/under/gimmick/sbahj
	...
	red
		name = "<font face='Comic Sans MS' size='3'><span style=\"color:red\"><strong><u>read</u></strong></span><span style=\"color:blue\"> jumsut</span></font>"
		desc = "<font face='Comic Sans MS' size='3'>\"samething to ware for <span style=\"color:red\"><em><u>studid fuckasses</u></em></span></font>"
		icon_state = "sbahjR"
		item_state = "sbahjR"

	yellow
		name = "<font face='Comic Sans MS' size='3'><span style=\"color:yellow\"><strike>yello  jamsuuit</strike><strong><u><em>GEROMY</em></u></strong></span></font>"
...
/obj/item/clothing/under/gimmick/dawson
	...
	equipped(var/mob/user, var/slot)
		if (slot == "i_clothing" && ishuman(user))
			...
			boutput(H, "<span style=\"color:red\"><strong>You suddenly feel whiny and ineffectual.</strong></span>")
...
/obj/item/clothing/suit/bio_suit/beekeeper
	...
	desc = "A suit that protects against bees. Not space bees, but like the tiny, regular kind. This thing doesn't do <em>shit</em> to protect you from space bees."
