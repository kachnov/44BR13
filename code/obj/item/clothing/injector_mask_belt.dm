...
/obj/item/injector_belt
	...
	var/injector_belt_condition/condition = null
	var/min_time = 10
	var/inj_amount = -1
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W,/obj/item/reagent_containers/glass))
			...
			user.drop_item()
			W.set_loc(src)
			if (is_equipped() && owner == user && condition)
				active = 1
				check()
				user.show_text("You attach the [W] to the [name]! The injector system is now operational.", "blue")
			else
				user.show_text("You attach the [W] to the [name]! Please select a condition and re-equip [src] to initialize injector system.", "blue")
	...
	Topic(href, href_list)
		...
		if (href_list["sel_cond"])
			var/list/cond_types = (subtypesof(/injector_belt_condition))
			var/list/filtered = new/list()

			for (var/A in cond_types)
				var/injector_belt_condition/C = new A()
				filtered += C.name
				filtered[C.name] = A
...
/injector_belt_condition/health
	...
	check_trigger(mob/M)
		if (M.health < threshold) return TRUE
		else return FALSE

/injector_belt_condition/damage
	...
	check_trigger(mob/M)
		switch(damagetype)
			if ("brute")
				if (M.get_brute_damage() > threshold) return TRUE
			if ("burn")
				if (M.get_burn_damage() > threshold) return TRUE
			if ("toxin")
				if (M.get_toxin_damage() > threshold) return TRUE
			if ("oxygen")
				if (M.get_oxygen_deprivation() > threshold) return TRUE
		return FALSE

/injector_belt_condition/tempdiff
	...
	check_trigger(mob/M)
		if (M.bodytemperature > threshold)
			if ((M.bodytemperature - threshold) > 20) return TRUE
		else
			if ((threshold - M.bodytemperature) > 20) return TRUE
		return FALSE

/injector_belt_condition/tempover
	name = "Condition: Temperature >"
	...
	check_trigger(mob/M)
		if (M.bodytemperature > threshold) return TRUE
		else return FALSE

/injector_belt_condition/tempunder
	name = "Condition: Temperature <"
...
/injector_belt_condition/incapacitated
	...
	check_trigger(mob/M)
		if (M.stunned || M.paralysis || M.weakened || M.stat == 1) return TRUE
		else return FALSE

/injector_belt_condition/life
