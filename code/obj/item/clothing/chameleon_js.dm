/obj/item/clothing/under/chameleon
	...
	New()
		..()
		for (var/U in (typesof(/chameleon_jumpsuit_pattern)))
			var/chameleon_jumpsuit_pattern/P = new U
			clothing_choices += P
		return
	...
	attackby(obj/item/clothing/under/U as obj, mob/user as mob)
		...
		if (istype(U, /obj/item/clothing/under))
			for (var/chameleon_jumpsuit_pattern/P in clothing_choices)
				if (P.name == U.name)
					boutput(user, "<span style=\"color:red\">That appearance is already saved in the chameleon pattern banks!</span>")
					return

			var/chameleon_jumpsuit_pattern/P = new /chameleon_jumpsuit_pattern(src)
			P.name = U.name
			P.desc = U.desc
	...
	emp_act()
		if (ishuman(loc))
			...
			boutput(M, "<span style=\"color:red\"><strong>Your chameleon jumpsuit malfunctions!</strong></span>")
	...
	verb/change()
		...
		var/chameleon_jumpsuit_pattern/which = input("Change the jumpsuit to which pattern?", "Chameleon Jumpsuit") as null|anything in clothing_choices
		...
		name = which.name
		desc = which.desc
		icon_state = which.icon_state
		item_state = which.item_state
		icon = which.sprite_item
		wear_image_icon = which.sprite_worn
		inhand_image_icon = which.sprite_hand
		wear_image = image(wear_image_icon)
		inhand_image = image(inhand_image_icon)
		usr.set_clothing_icon_dirty()

/chameleon_jumpsuit_pattern
