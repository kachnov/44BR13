...
/obj/item/clothing/suit/hoodie
	...
	attack_self(mob/user as mob)
		hood = !(hood)
		user.show_text("You flip [src]'s hood [hood ? "up" : "down"].")
		if (hood)
			over_hair = 1
			icon_state = "hoodie-up"
			item_state = "hoodie-up"
		else
			over_hair = 0
			icon_state = "hoodie"
			item_state = "hoodie"
...
/obj/item/clothing/suit/bedsheet
	...
	New()
		..()
		update_icon()
		setMaterial(getCachedMaterial("cotton"))

	attack_hand(mob/user as mob)
		if (Bed)
			Bed.untuck_sheet(user)
		Bed = null
		return ..()

	ex_act(severity)
		if (severity <= 2)
			if (Bed && Bed.Sheet == src)
				Bed.Sheet = null
			qdel(src)
			return
		return
			...
			switch (action)
				if ("Make bandages")
					...
					if (!do_after(user, 30))
						...
						playsound(loc, "sound/items/Scissor.ogg", 100, 1)
	...
	proc/update_icon()
		if (cape)
			icon_state = "bedcape[bcolor ? "-[bcolor]" : null]"
			item_state = icon_state
			see_face = 1
			over_hair = 0
			over_all = 0
		else
			icon_state = "bedsheet[bcolor ? "-[bcolor]" : null][eyeholes ? "1" : null]"
			item_state = icon_state
			see_face = 0
			over_hair = 1
			over_all = 1
...
/obj/item/clothing/suit/bedsheet/random
	New()
		..()
		bcolor = pick("", "red", "orange", "yellow", "green", "blue", "pink", "black")
		update_icon()
...
/obj/item/clothing/suit/sweater
	...
	grandma
		...
		New()
			...
			spawn (20)
				name = initial(name)
				material = new /material/fabric/cloth/cotton ()
...
/obj/item/clothing/suit/cardboard_box
	...
	attackby(obj/item/W, mob/user)
		if (istype(W, /obj/item/wirecutters) || istype(W, /obj/item/scissors))
			...
			moustache = 1
			UpdateOverlays(image(icon, "c_box-moustache"), "moustache")
			if (wear_image)
				wear_image.overlays += image(wear_image_icon, "c_box-moustache")
...
/obj/item/clothing/suit/cardboard_box/head_surgeon
	...
	proc/speak(var/message)
		if (!message)
			return
		audible_message("<span class='game say'><span class='name'>[src]</span> [pick("rustles", "folds", "womps", "boxes", "foffs", "flaps")], \"[message]\"")
		if (text2speech)
