...
/obj/item/clothing/mask
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/voice_changer))
			...
			if (vchange)
				user.show_text("You begin removing [vchange] from [src].", "blue")
				if (!do_after(user, 20))
					user.show_text("You were interrupted!", "red")
					return
				user.show_text("You remove [vchange] from [src].", "green")
				user.put_in_hand_or_drop(vchange)
				vchange = null
				return
			else
				return ..()
...
/obj/item/clothing/mask/surgical_shield
	...
	desc = "For those really, <em>really</em> messy surgeries."
...
/obj/item/paper_mask
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/pen))
			...
			C.use(1)
			var/obj/item/clothing/mask/paper/M = new /obj/item/clothing/mask/paper(loc)
...
/obj/item/clothing/mask/wrestling
	name = "wrestling mask"
	desc = "A mask that will greatly enhance your wrestling prowess! Not, like, <em>physically</em>, but mentally. In your heart. In your soul. Something like that."
