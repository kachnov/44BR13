...
/obj/item/clothing/glasses/sunglasses/equipped(var/mob/user, var/slot)
	...
	if (istype(H) && slot == "eyes")
		if (H.mind)
			if (H.mind.assigned_role == "Detective" && !already_worn)
				already_worn = 1
				playsound(user.loc, "sound/effects/yeaaahhh.ogg", 100, 0)
				user.visible_message("<span style=\"color:red\"><strong><font size=3>YEAAAAAAAAAAAAAAAH!</font></strong></span>")
	..()
	return
	...
	emp_act()
		if (ishuman(loc))
			...
			if (istype(H.glasses, /obj/item/clothing/glasses/thermal))
				boutput(H, "<span style=\"color:red\"><strong>Your thermals malfunction!</strong></span>")
...
/obj/item/clothing/glasses/vr
	...
	New()
		spawn (20)
			if (src)
				name += " - '[network]'" // They otherwise all look the same (Convair880).
		..()
	...
	equipped(var/mob/user, var/slot)
		...
		if (istype(H) && slot == "eyes" && !H.network_device)
			...
			Station_VNet.Enter_Vspace(H, src,network)
		return
...
/obj/item/clothing/glasses/healthgoggles
	...
	attack_self(mob/user as mob)
		if (!scan_upgrade)
			...
			boutput(user, "<span style=\"color:blue\">Health scanner [health_scan ? "enabled" : "disabled"].</span>")
			return
