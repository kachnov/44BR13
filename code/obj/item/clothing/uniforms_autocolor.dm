...
/obj/item/clothing/under/autocolor
	...
	proc/autojump_updateworldicon()
		if (overlaid)
			var/icon/coloroverlay1 = icon("icon" = 'icons/obj/clothing/uniforms/item_js.dmi', "icon_state" = text("[]-fx1", icon_state))
			coloroverlay1.Blend(rgb(base_r, base_g, base_b), ICON_ADD)
			overlays += image("icon" = coloroverlay1)
			if (overlaid >= 2)
				var/icon/coloroverlay2 = icon("icon" = 'icons/obj/clothing/uniforms/item_js.dmi', "icon_state" = text("[]-fx2", icon_state))
				coloroverlay2.Blend(rgb(detail_r, detail_g, detail_b), ICON_ADD)
				overlays += image("icon" = coloroverlay2)

	New()
		..()
		autojump_updateworldicon()
