...
/obj/item/storage/toilet/MouseDrop_T(mob/M as mob, mob/user as mob)
	if (!ticker)
		...
		return
	if ((!( istype(M, /mob) ) || get_dist(src, user) > 1 || M.loc != loc || user.restrained() || usr.stat))
		return
	if (M == usr)
...
//			boutput(world, "[M] is no longer buckled to [src]")
			...
			M.buckled = null
			add_fingerprint(user)
	if ((clogged < 1) || (contents.len < 7) || (user.loc != loc))
		user.visible_message("<span style=\"color:blue\">[user] flushes the toilet.</span>", "<span style=\"color:blue\">You flush the toilet.</span>")
		clogged = 0
		contents.len = 0
	else if ((clogged >= 1) || (contents.len >= 7) || (user.buckled != loc))
		visible_message("<span style=\"color:blue\">The toilet is clogged!</span>")
