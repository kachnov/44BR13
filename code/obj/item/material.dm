/obj/item/raw_material/
	...
	New()
		..()
		pixel_x = rand(0 - wiggle, wiggle)
		pixel_y = rand(0 - wiggle, wiggle)
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/satchel/mining))
			if (W:contents.len < W:maxitems)
				...
				desc = "A leather bag. It holds [oreamt]/[W:maxitems] [W:itemstring]."
	...
	HasEntered(AM as mob|obj)
		if (istype(AM,/mob/dead))
			...
			if (S && S.satchel && S.satchel.contents.len < S.satchel.maxitems && scoopable)
				loc = S.satchel
			...
			if (istype(V.sec_system,/obj/item/shipcomponent/secondary_system/orescoop))
				...
				if (SCOOP.contents.len >= SCOOP.capacity || !scoopable)
					return
				loc = SCOOP
...
/obj/item/raw_material/mauxite
	...
	New()
		material = getCachedMaterial("mauxite")
		return ..()
...
/obj/item/raw_material/molitz
	...
	New()
		material = getCachedMaterial("molitz")
		return ..()
...
/obj/item/raw_material/pharosium
	...
	New()
		material = getCachedMaterial("pharosium")
		return ..()
...
/obj/item/raw_material/cobryl // relate this to precursors
	...
	New()
		material = getCachedMaterial("cobryl")
		return ..()
...
/obj/item/raw_material/char
	...
	New()
		material = getCachedMaterial("char")
		return ..()
...
/obj/item/raw_material/claretine // relate this to wizardry somehow
	...
	New()
		material = getCachedMaterial("claretine")
		return ..()
...
/obj/item/raw_material/syreline
	...
	New()
		material = getCachedMaterial("syreline")
		return ..()
...
/obj/item/raw_material/erebite
	...
	New()
		material = getCachedMaterial("erebite")
		return ..()
	...
	ex_act(severity)
		...
		switch(severity)
			if (1)
				explosion(src, loc, 1, 2, 3, 4, 1)
			if (2)
				explosion(src, loc, 0, 1, 2, 3, 1)
			if (3)
				explosion(src, loc, 0, 0, 1, 2, 1)
			else
				return
		...
		if (z != MINING_Z)
			...
			if (bombturf)
				...
				logTheThing("combat", null, null, "Erebite detonated by an explosion in [bombarea] ([showCoords(bombturf.x, bombturf.y, bombturf.z)]). Last touched by: [fingerprintslast]")
				message_admins("Erebite detonated by an explosion in [bombarea] ([showCoords(bombturf.x, bombturf.y, bombturf.z)]). Last touched by: [fingerprintslast]")
	...
	temperature_expose(null, temp, volume)
		...
		if (z != MINING_Z)
			...
			logTheThing("combat", null, null, "Erebite detonated by heat in [bombarea]. Last touched by: [fingerprintslast]")
			message_admins("Erebite detonated by heat in [bombarea]. Last touched by: [fingerprintslast]")
...
/obj/item/raw_material/cerenkite
	...
	New()
		material = getCachedMaterial("cerenkite")
		return ..()
...
/obj/item/raw_material/plasmastone
	...
	New()
		material = getCachedMaterial("plasmastone")
		return ..()
...
/obj/item/raw_material/gemstone
	...
	New()
		...
		var/material/M = getCachedMaterial(pick(picklist))
		setMaterial(M)
...
/obj/item/raw_material/uqill // relate this to ancients
	...
	New()
		material = getCachedMaterial("uqill")
		return ..()
...
/obj/item/raw_material/fibrilith
	...
	New()
		material = getCachedMaterial("fibrilith")
		return ..()
...
/obj/item/raw_material/telecrystal
	...
	New()
		material = getCachedMaterial("telecrystal")
		return ..()
...
/obj/item/raw_material/miracle
	...
	New()
		material = getCachedMaterial("miracle")
		return ..()
...
/obj/item/raw_material/starstone
	...
	New()
		material = getCachedMaterial("starstone")
		return ..()
...
/obj/item/raw_material/eldritch
	...
	New()
		material = getCachedMaterial("koshmarite")
		return ..()
...
/obj/item/raw_material/martian
	...
	New()
		material = getCachedMaterial("viscerite")
		var/reagents/R = new/reagents(25)
		reagents = R
		R.my_atom = src
		reagents.add_reagent("synthflesh", 25)
		return ..()
...
/obj/item/raw_material/ice
	...
	New()
		material = getCachedMaterial("ice")
		return ..()
...
/obj/item/raw_material/shard
		...
		if (ismob(AM))
			...
			if (ishuman(M))
				...
				if (istype(H.mutantrace, /mutantrace/abomination))
					return
				if (!H.shoes)
					boutput(H, "<span style=\"color:red\"><strong>You step on [src] barefoot! Ouch!</strong></span>")
					playsound(loc, sound_stepped, 50, 1)
	...
	suicide(var/mob/usr as mob)
		usr.visible_message("<span style=\"color:red\"><strong>[usr] slashes \his own throat with [src]!</strong></span>")
		blood_slash(usr, 25)
...
/obj/machinery/portable_reclaimer
	...
	attack_hand(var/mob/user as mob)
		...
		if (contents.len < 1)
			...
			return
		user.visible_message("<strong>[user.name]</strong> switches on [src].")
		active = 1
		anchored = 1
		...
		if (reject_counter > 0)
			visible_message("<strong>[src]</strong> emits an angry buzz and rejects some unsuitable materials!")
			playsound(loc, sound_grump, 40, 1)

		for (var/obj/item/raw_material/M in contents)
		...
		for (var/obj/item/rods/R in contents)
			while (R.amount >= 20)
		...
		for (var/obj/item/tile/T in contents)
			while (T.amount >= 40)
		...
		for (var/obj/item/wizard_crystal/W in contents)
			W.create_bar(src)
			qdel(W)
			sleep(smelt_interval)
		...
		for (var/obj/item/cable_coil/C in contents)
		...
		if (bad_flag)
			visible_message("<strong>[src]</strong> emits a grumpy buzz.")
			playsound(loc, sound_grump, 40, 1)

		if (insufficient_counter > 0)
			visible_message("<strong>[src]</strong> emits a grumpy buzz and ejects some leftovers.")
			playsound(loc, sound_grump, 40, 1)

		active = 0
		anchored = 0
		icon_state = "reclaimer"
		visible_message("<strong>[src]</strong> finishes working and shuts down.")

	proc/output_bar_with_quality(var/quality,var/default_material)
		var/material/MAT = null
		...
		BAR.setMaterial(MAT)
		playsound(loc, sound_process, 40, 1)
	...
	proc/get_output_location()
		...
		if (get_dist(output_location,src) > 1)
			output_location = null
			return loc
