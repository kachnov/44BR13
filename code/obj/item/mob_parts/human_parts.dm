	...
	take_damage(brute, burn, tox, damage_type, disallow_limb_loss)
		...
		if (ishuman(holder))
			...
			if (brute > 30 && prob(brute - 30) && !disallow_limb_loss)
				sever()
			else if (bone_system && bones && brute && prob(brute * 2))
...
*/
	New(mob/new_holder)
		...
		bones = new /bone(src)
		bones.donor = new_holder
		bones.parent_organ = "[name]"
		spawn (20)
	...
	proc/set_skin_tone()
		...
		if (lyingImage)
			lyingImage.color = newrgb
		if (standImage)
			standImage.color = newrgb
	...
	surgery(var/obj/item/tool)
		...
		switch(remove_stage)
			if (0)
				...
				logTheThing("combat", tool.the_mob, holder, "staples %target%'s [name] back on")
				logTheThing("diary", tool.the_mob, holder, "staples %target%'s [name] back on", "combat")
			if (1)
				tool.the_mob.visible_message("<span style=\"color:red\">[tool.the_mob] slices through the skin and flesh of [holder.name]'s [name] with [tool].</span>", "<span style=\"color:red\">You slice through the skin and flesh of [holder.name]'s [name] with [tool].</span>")
			if (2)
				tool.the_mob.visible_message("<span style=\"color:red\">[tool.the_mob] saws through the bone of [holder.name]'s [name] with [tool].</span>", "<span style=\"color:red\">You saw through the bone of [holder.name]'s [name] with [tool].</span>")

				spawn (rand(150,200))
					if (remove_stage == 2)
						remove(0)
			if (3)
				tool.the_mob.visible_message("<span style=\"color:red\">[tool.the_mob] cuts through the remaining strips of skin holding [holder.name]'s [name] on with [tool].</span>", "<span style=\"color:red\">You cut through the remaining strips of skin holding [holder.name]'s [name] on with [tool].</span>")
				logTheThing("combat", tool.the_mob, holder, "removes %target%'s [name]")
				logTheThing("diary", tool.the_mob, holder, "removes %target%'s [name]", "combat")
				remove(0)
...
/obj/item/parts/human_parts/arm/left/wendigo
	...
	getMobIcon(var/lying, var/decomp_stage = 0)
		...
		standImage = image('icons/mob/human.dmi', "[slot]_wendigo")
		return standImage
...
/obj/item/parts/human_parts/arm/left/bear
	...
	getMobIcon(var/lying, var/decomp_stage = 0)
		...
		if (standImage && ((decomp_affected && current_decomp_stage_s == decomp_stage) || !decomp_affected))
			return standImage
		current_decomp_stage_s = decomp_stage
		standImage = image('icons/mob/human.dmi', "[slot]_bear")
		return standImage
...
/obj/item/parts/human_parts/arm/left/synth
	...
	getMobIcon(var/lying, var/decomp_stage = 0)
		if (standImage && ((decomp_affected && current_decomp_stage_s == decomp_stage) || !decomp_affected))
			return standImage
		current_decomp_stage_s = decomp_stage
		standImage = image('icons/mob/human.dmi', "[slot]_[name_thing]")
		return standImage
...
/obj/item/parts/human_parts/arm/left/abomination
	...
	getMobIcon(var/lying, var/decomp_stage = 0)
		if (standImage && ((decomp_affected && current_decomp_stage_s == decomp_stage) || !decomp_affected))
			return standImage
		current_decomp_stage_s = decomp_stage
		standImage = image('icons/mob/human.dmi', "[slot]_abomination")
		return standImage
...
/obj/item/parts/human_parts/arm/left/werewolf
	...
	skintoned = 0
	override_attack_hand = 1
	limb_type = /limb/abomination/werewolf
	...
	getMobIcon(var/lying, var/decomp_stage = 0)
		if (standImage && ((decomp_affected && current_decomp_stage_s == decomp_stage) || !decomp_affected))
			return standImage
		current_decomp_stage_s = decomp_stage
		standImage = image('icons/mob/human.dmi', "[slot]_werewolf")
		return standImage
...
/obj/item/parts/human_parts/arm/left/predator
	...
	getMobIcon(var/lying, var/decomp_stage = 0)
		if (standImage && ((decomp_affected && current_decomp_stage_s == decomp_stage) || !decomp_affected))
			return standImage
		current_decomp_stage_s = decomp_stage
		standImage = image('icons/mob/human.dmi', "[slot]_predator")
		return standImage
