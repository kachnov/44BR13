...
/obj/item/card/id
	...
	var/computer/file/cardfile = null
...
/obj/item/card/id/attack_self(mob/user as mob)
	user.visible_message("[user] shows you: [bicon(src)] [name]: assignment: [assignment]", "You show off your card: [bicon(src)] [name]: assignment: [assignment]")
...
/obj/item/card/id/emag_act(var/mob/user, var/obj/item/card/emag/E)
	if (emagged)
		...
		return
	access = list() // clear what used to be there
	...
	for (var/i = rand(2,25), i > 0, i--)
		...
		if (istype(src, /obj/item/card/id/syndicate)) // Nuke ops unable to exit their station (Convair880).
			access += access_syndicate_shuttle
...
/obj/item/card/id/verb/read()
	...
	boutput(usr, "[bicon(src)] [name]: The current assignment on the card is [assignment].")
	return
...
/obj/item/card/id/syndicate/attack_self(mob/user as mob)
	if (!registered)
		var/reg = copytext(sanitize_name(input(user, "What name would you like to put on this card?", "Agent card name", ishuman(user) ? user.real_name : user.name)), 1, 100)
		var/ass = copytext(sanitize_name(input(user, "What occupation would you like to put on this card?\n Note: This will not grant any access levels other than Maintenance.", "Agent card job assignment", "Staff Assistant"), 1), 1, 100)
