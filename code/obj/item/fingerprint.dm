// FINGERPRINT HOLDER

// All of this was phased out with the forensic scanner overhaul. Was useless anyway (Convair880).

/obj/item/fcardholder
	name = "Finger Print Case"
	icon = 'icons/obj/items.dmi'
	icon_state = "fcardholder0"
	item_state = "clipboard"
	stamina_damage = 2
	stamina_cost = 2
	stamina_crit_chance = 1

/obj/item/fcardholder/attack_self(mob/user as mob)
	var/dat = "<strong>Clipboard</strong><BR>"
	for (var/obj/item/f_card/P in src)
		dat += text("<A href='?src=\ref[];read=\ref[]'>[]</A> <A href='?src=\ref[];remove=\ref[]'>Remove</A><BR>", src, P, P.name, src, P)
	user << browse(dat, "window=fcardholder")
	onclose(user, "fcardholder")
	return

/obj/item/fcardholder/Topic(href, href_list)
	..()
	if ((usr.stat || usr.restrained()))
		return
	if (usr.contents.Find(src))
		usr.machine = src
		if (href_list["remove"])
			var/obj/item/P = locate(href_list["remove"])
			if (P && P.loc == src)
				usr.put_in_hand_or_drop(P)
				add_fingerprint(usr)
			update()
		if (href_list["read"])
			var/obj/item/f_card/P = locate(href_list["read"])
			if ((P && P.loc == src))
				if (!( istype(usr, /mob/living/carbon/human) ))
					usr << browse(text("<HTML><HEAD><TITLE>[]</TITLE></HEAD><BODY><TT>[]</TT></BODY></HTML>", P.name, P.display()), text("window=[]", P.name))
					onclose(usr, "[P.name]")
				else
					usr << browse(text("<HTML><HEAD><TITLE>[]</TITLE></HEAD><BODY><TT>[]</TT></BODY></HTML>", P.name, P.display()), text("window=[]", P.name))
					onclose(usr, "[P.name]")
			add_fingerprint(usr)
		if (ismob(loc))
			var/mob/M = loc
			if (M.machine == src)
				spawn ( 0 )
					attack_self(M)
					return
	return

/obj/item/fcardholder/attack_hand(mob/user as mob)
	if (user.contents.Find(src))
		spawn ( 0 )
			attack_self(user)
			return
		add_fingerprint(user)
	else
		return ..()
	return

/obj/item/fcardholder/attackby(obj/item/P as obj, mob/user as mob)
	if (istype(P, /obj/item/f_card))
		if (contents.len < 30)
			user.drop_item()
			P.set_loc(src)
			add_fingerprint(user)
			add_fingerprint(user)
		else
			boutput(user, "<span style=\"color:blue\">Not enough space!!!</span>")
	else
		if (istype(P, /obj/item/pen))
			var/t = input(user, "Holder Label:", text("[]", name), null)  as text
			if (user.equipped() != P)
				return
			if ((!in_range(src, usr) && loc != user))
				return
			t = copytext(sanitize(t),1,MAX_MESSAGE_LEN)
			if (t)
				name = text("FPCase- '[]'", t)
			else
				name = "Finger Print Case"
		else
			return
	update()
	spawn ( 0 )
		attack_self(user)
		return
	return

/obj/item/fcardholder/proc/update()
	var/i = 0
	for (var/obj/item/f_card/F in src)
		i = 1
		break
	icon_state = text("fcardholder[]", (i ? "1" : "0"))
	return





// FINGERPRINT CARD

/obj/item/f_card
	name = "Finger Print Card"
	icon = 'icons/obj/card.dmi'
	icon_state = "fingerprint0"
	amount = 10.0
	item_state = "paper"
	throwforce = 1
	w_class = 1.0
	throw_speed = 3
	throw_range = 5
	max_stack = 10


/obj/item/f_card/examine()
	set src in view(2)
	set category = "Local"

	..()
	boutput(usr, text("<span style=\"color:blue\">There are [] on the stack!</span>", amount))
	usr << browse(text("<HTML><HEAD><TITLE>[]</TITLE></HEAD><BODY><TT>[]</TT></BODY></HTML>", name, display()), text("window=[]", name))
	onclose(usr, "[name]")
	return

/obj/item/f_card/proc/display()

	if (fingerprints)
		var/dat = "<strong>Fingerprints on Card</strong><HR>"
		var/L = params2list(fingerprints)
		for (var/i in L)
			dat += text("[]<BR>", i)
			//Foreach goto(41)
		return dat
	else
		return "<strong>There are no fingerprints on this card.</strong>"
	return

/obj/item/f_card/attack_hand(mob/user as mob)

	if ((user.r_hand == src || user.l_hand == src))
		add_fingerprint(user)
		var/obj/item/f_card/F = new /obj/item/f_card( user )
		F.amount = 1
		amount--
		user.put_in_hand_or_drop(F)
		if (ishuman(user) && !user:gloves)
			F.add_fingerprint(user)
		if (amount < 1)
			//SN src = null
			qdel(src)
			return
	else
		..()
	return

/obj/item/f_card/attackby(obj/item/W as obj, mob/user as mob)

	if (istype(W, /obj/item/f_card))
		if ((fingerprints || W.fingerprints))
			return
		if (amount == 10)
			return
		if (W:amount + amount > 10)
			amount = 10
			W:amount = W:amount + amount - 10
		else
			amount += W:amount
			//W = null
			qdel(W)
		add_fingerprint(user)
		if (W)
			W.add_fingerprint(user)
	else
		if (istype(W, /obj/item/pen))
			var/t = input(user, "Card Label:", text("[]", name), null)  as text
			if (user.equipped() != W)
				return
			if ((!in_range(src, usr) && loc != user))
				return
			t = copytext(sanitize(t),1,MAX_MESSAGE_LEN)
			if (t)
				name = text("FPrintC- '[]'", t)
			else
				name = "Finger Print Card"
			W.add_fingerprint(user)
			add_fingerprint(user)
	return

/obj/item/f_card/add_fingerprint()

	..()
	if (!istype(usr, /mob/living/silicon))
		if (fingerprints)
			if (amount > 1)
				var/obj/item/f_card/F = new /obj/item/f_card( (ismob(src.loc) ? src.loc.loc : src.loc) )
				F.amount = --amount
				amount = 1
			icon_state = "fingerprint1"
	return