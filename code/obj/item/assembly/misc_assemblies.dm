...
/obj/item/assembly/time_ignite/receive_signal()
	...
	part2.ignite()
	if (part3)
		part3.reagents.temperature_reagents(4000, 400)
		part3.reagents.temperature_reagents(4000, 400)
	return
...
/obj/item/assembly/time_ignite/attackby(obj/item/W as obj, mob/user as mob)
	...
	if ((istype(W, /obj/item/reagent_containers/glass/beaker) && !( status )))
		if (!part3)
			part3 = W
			W.master = src
...
/obj/item/assembly/time_ignite/c_state(n)
	if (!part3)
		icon = 'icons/obj/assemblies.dmi'
		icon_state = text("timer-igniter[n]")
		overlays = null
		underlays = null
		name = "Timer/Igniter Assembly"
	else
		icon = part3.icon
		icon_state = part3.icon_state
		overlays = null
		underlays = null
		overlays += image('icons/obj/assemblies.dmi', "timeignite_overlay[n]", layer = FLOAT_LAYER)
		underlays += part3.underlays
		name = "Timer/Igniter/Beaker Assembly"
	return
...
/obj/item/assembly/time_ignite/verb/removebeaker()
	...
	if (part3)
		part3.master = null
		part3.attack_hand(usr)
		part3 = null
		c_state(part1.timing)
...
/obj/item/assembly/prox_ignite/c_state(n)
	if (!part3)
		icon = 'icons/obj/assemblies.dmi'
		icon_state = text("prox-igniter[n]")
		overlays = null
		underlays = null
		name = "Proximity/Igniter Assembly"
	else
		...
		overlays = null
		underlays = null
		overlays += image('icons/obj/assemblies.dmi', "proxignite_overlay[n]", layer = FLOAT_LAYER)
		underlays += part3.underlays
		name = "Proximity/Igniter/Beaker Assembly"
	return
...
/obj/item/assembly/rad_ignite/c_state()
	if (!part3)
		...
		overlays = null
		underlays = null
		overlays += image('icons/obj/assemblies.dmi', "radignite_overlay", layer = FLOAT_LAYER)
		underlays += part3.underlays
		name = "Radio/Igniter/Beaker Assembly"
	return
...
/obj/item/assembly/anal_ignite/New()
	...
	spawn (5)
		if (src && !part1)
			part1 = new /obj/item/device/healthanalyzer(src)
			part1.master = src
		if (src && !part2)
			part2 = new /obj/item/device/igniter(src)
			part2.master = src
	return
...
/obj/item/assembly/radio_horn/receive_signal()
	if (part2.spam_flag == 0)
		part2.spam_flag = 1
		playsound(loc, part2.sound_horn, part2.volume, part2.randomized_pitch)
...
/obj/item/assembly/rad_time/receive_signal(signal/signal)
	// drsingh for cannot read null.source
	if (signal && signal.source == part2)
...
/obj/item/assembly/rad_prox/receive_signal(signal/signal)
	// drsingh for Cannot read null.source
	if (signal && signal.source == part2)
		part1.send_signal("ACTIVATE")
	return
