/obj/item/tank
	...
	var/gas_mixture/air_contents = null
	...
	attack_self(mob/user as mob)
		...
		<FONT color='blue'><strong>Tank Pressure:</strong> [air_contents.return_pressure()]</FONT><BR>
		<BR>
		<strong>Mask Release Pressure:</strong> <A href='?src=\ref[src];dist_p=-10'>-</A> <A href='?src=\ref[src];dist_p=-1'>-</A> <A href='?src=\ref[src];setpressure=1'>[distribute_pressure]</A> <A href='?src=\ref[src];dist_p=1'>+</A> <A href='?src=\ref[src];dist_p=10'>+</A><BR>
		<strong>Mask Release Valve:</strong> <A href='?src=\ref[src];stat=1'>[using_internal?("Open"):("Closed")]</A>
		"}
	...
	Topic(href, href_list)
		...
		if (loc == usr)
			...
			if (href_list["dist_p"])
				...
				distribute_pressure = min(max(round(distribute_pressure), 0), 3*ONE_ATMOSPHERE)
			if (href_list["stat"])
		...
		if (pressure > TANK_FRAGMENT_PRESSURE) // 50 atmospheres, or: 5066.25 kpa under current _setup.dm conditions
			...
			if (src in bible_contents)
				for (var/obj/item/storage/bible/B in world)
					...
					if (T)
						logTheThing("bombing", src, null, "exploded at [showCoords(T.x, T.y, T.z)], range: [range], last touched by: [fingerprintslast]")
			...
			logTheThing("bombing", src, null, "exploded at [showCoords(epicenter.x, epicenter.y, epicenter.z)], , range: [range], last touched by: [fingerprintslast]")
			...
			if (integrity <= 0)
				...
				air_contents = null
				//TODO: make pop sound
				playsound(loc, "sound/effects/bang.ogg", 60, 1) // consider it TO DID
				qdel(src)
			else
				integrity--
			...
			if (integrity <= 0)
				var/gas_mixture/leaked_gas = air_contents.remove_ratio(0.25)
...
/obj/item/tank/jetpack
	...
	w_class = 4.0
	item_state = "jetpack"
	var/effects/system/ion_trail_follow/ion_trail
	mats = 16
	force = 8
	...
	New()
		...
		air_contents.oxygen = (6*ONE_ATMOSPHERE)*70/(R_IDEAL_GAS_EQUATION*T20C)
		return

	verb/toggle()
		on = !( on )
		icon_state = text("jetpack[]", on)
		if (on)
	...
	proc/allow_thrust(num, mob/user as mob)
		if (!( on ))
			return FALSE
		if ((num < 0.01 || air_contents.total_moles() < num))
			ion_trail.stop()
			return FALSE

		var/gas_mixture/G = air_contents.remove(num)
...
/obj/item/tank/emergency_oxygen
	...
	New()
		..()
		air_contents.oxygen = (ONE_ATMOSPHERE / 2.5)*70/(R_IDEAL_GAS_EQUATION*T20C) // cogwerks: drastically reduced capacity of emerg tanks
		return
...
/obj/item/tank/air
	...
	New()
		..()
		air_contents.oxygen = (6*ONE_ATMOSPHERE)*70/(R_IDEAL_GAS_EQUATION*T20C) * O2STANDARD
		air_contents.nitrogen = (6*ONE_ATMOSPHERE)*70/(R_IDEAL_GAS_EQUATION*T20C) * N2STANDARD
		return
...
/obj/item/tank/plasma
	...
	proc/release()
		var/gas_mixture/removed = air_contents.remove(air_contents.total_moles())
