...
/obj/item/sticker
	...
	New()
		if (islist(random_icons) && random_icons.len)
	...
	afterattack(var/atom/A as mob|obj|turf, var/mob/user as mob, reach, params)
		...
		if (isarea(A) || istype(A, /obj/item/item_box))
			return
		user.tri_message("<strong>[user]</strong> sticks [src] to [A]!",\
		user, "You stick [src] to [user == A ? "yourself" : "[A]"]!",\
		A, "[user == A ? "You stick" : "<strong>[user]</strong> sticks"] [src] to you[user == A ? "rself" : null]!")
		var/image/sticker = image('icons/misc/stickers.dmi', icon_state)
		sticker.layer = EFFECTS_LAYER_BASE // I swear to fuckin god stop being under CLOTHES you SHIT
		sticker.icon_state = icon_state
		var/pox = pixel_x
		var/poy = pixel_y
