...
/obj/item/cable_coil
	...
	suicide(var/mob/user as mob)
		user.visible_message("<span style=\"color:red\"><strong>[user] wraps the cable around \his neck and tightens it.</strong></span>")
	...
	proc/setInsulator(var/material/M)
		if (!M)
			return
		insulator = M
		applyCableMaterials(src, M, conductor)

	proc/setConductor(var/material/M)
		if (!M)
			return
		conductor = M
...
/obj/item/cable_coil/attackby(obj/item/W, mob/user)
	if (istype(W, /obj/item/wirecutters) && amount > 1)
		amount--
		take(1, usr.loc)
		...
		if ((C.amount + amount <= MAXCOIL))
			...
			boutput(user, "You transfer [MAXCOIL - amount ] length\s of cable from one coil to the other.")
			amount -= (MAXCOIL-C.amount)
			updateicon()
			C.amount = MAXCOIL
			C.updateicon()
			return
...
/obj/item/cable_coil/proc/turf_place_between(turf/simulated/floor/A, turf/simulated/floor/B)
	...
	applyCableMaterials(NC, insulator, conductor)
	NC.d1 = 0
	NC.d2 = dirn
	NC.updateicon()
	NC.update_network()
...
/obj/item/cable_coil/proc/turf_place(turf/simulated/floor/F, mob/user)
	...
	if (F.intact)		// if floor is intact, complain
		...
		C.updateicon()
		C.update_network()
		applyCableMaterials(C, insulator, conductor)
