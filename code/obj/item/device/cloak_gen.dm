...
/obj/item/cloak_gen
	...
	New()
		var/obj/item/remote/cloak_gen/remote = new /obj/item/remote/cloak_gen(loc)
		spawn (0)
			remote.my_gen = src
	...
	attackby(obj/item/W as obj, mob/user as mob)
		...
		if (istype(W, /obj/item/remote/cloak_gen))
			...
			if (!R.my_gen)
				user.show_text("Connection to [name] established", "blue")
				R.my_gen = src
			else
		...
		if (istype(usr,/mob/living))
			if (my_gen)
				if (my_gen.active)
					...
					if (anti_spam && world.time < anti_spam + 100)
