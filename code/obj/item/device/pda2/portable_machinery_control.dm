...
/computer/file/pda_program/portable_machinery_control
	...
	proc/teleport_sanity_check(var/obj/machinery/test_machinery, var/mob/test_mob, var/turf/test_turf, var/no_zlevel_check = 0)
		...
		if (!test_machinery || !master || (test_turf && !isturf(test_turf)))
			return FALSE
		...
		var/turf/our_loc = get_turf(master)
	...
	return_text()
		...
		. += "<h4>[machinery_name] Interlink</h4>"

		if (!active) // Show us the list.
			if (!machinerylist || (machinerylist && machinerylist.len == 0))
				. += "No linkable machinery found.<BR>"

			else
				for (our_machinery in machinerylist)
					. += "<A href='byond://?src=\ref[src];op=control;machinery=\ref[our_machinery]'>[our_machinery] at [get_area(our_machinery)]</A><BR>"
			...
			. += "<strong>[active]</strong><BR> Status: (<A href='byond://?src=\ref[src];op=control;machinery=\ref[active]'><em>refresh</em></A>)<BR>"

			if (istype(active, /obj/machinery/port_a_brig))
				var/obj/machinery/port_a_brig/P2 = active
			...
			else if (istype(active, /obj/machinery/port_a_medbay))
				var/obj/machinery/port_a_medbay/P2 = active
			...
			else if (istype(active, /obj/machinery/vending/port_a_nanomed))
				var/obj/machinery/vending/port_a_nanomed/P2 = active
			...
			else if (istype(active, /obj/storage/closet/port_a_sci))
				var/obj/storage/closet/port_a_sci/P2 = active
	...
	Topic(href, href_list)
		...
		switch (href_list["op"])
			...
			if ("lock")
				if (istype(active, /obj/machinery/port_a_brig))
					var/obj/machinery/port_a_brig/P3 = active
					if (P3.locked)
						P3.locked = 0
					else
						P3.locked = 1
					...
					PDA.display_alert("<span style=\"color:blue\">The [machinery_name] is now [P3.locked ? "locked" : "unlocked"].</span>")
			...
			if ("summon")
				...
				switch (teleport_sanity_check(P4, usr, null, zlevel_check_bypass))
					if (0)
						PDA.display_alert("<span style=\"color:red\">Teleportation failed due to unknown interference!</span>")
					if (2)
						PDA.display_alert("<span style=\"color:red\">The [machinery_name] is recharging!</span>")
					if (3)
						PDA.display_alert("<span style=\"color:red\">Cannot teleport unlocked [machinery_name] with someone inside!</span>")
					if (4)
			...
			if ("return")
				...
				if (!dest_loc || !isturf(dest_loc))
					PDA.display_alert("<span style=\"color:red\">No home turf assigned to [machinery_name], can't teleport!</span>")
					return
				...
				switch (teleport_sanity_check(P5, usr, dest_loc, zlevel_check_bypass))
					if (0)
						PDA.display_alert("<span style=\"color:red\">Teleportation failed due to unknown interference!</span>")
					if (2)
						PDA.display_alert("<span style=\"color:red\">The [machinery_name] is recharging!</span>")
...
/computer/file/pda_program/portable_machinery_control/portabrig
...
/computer/file/pda_program/portable_machinery_control/portamedbay
...
/computer/file/pda_program/portable_machinery_control/portananomed
...
/computer/file/pda_program/portable_machinery_control/portasci
