...
//Banking
/computer/file/pda_program/banking
	name = "BankBuddy"
	size = 8

	var/tmp/data/record/bank_record = null
...
//Manifest
/computer/file/pda_program/manifest
	name = "Manifest"
...
//Status Display
/computer/file/pda_program/status_display
...
//Signaler
/computer/file/pda_program/signaler
	name = "Signalix 5"
	size = 8
...
<a href='byond://?src=\ref[src];adj_code=5'>+</a><br>"}
	...
	Topic(href, href_list)
		...
		if (href_list["send"])
			...
			spawn ( 0 )
				logTheThing("signalers", usr, null, "used [master] @ location ([showCoords(master.loc.x, master.loc.y, master.loc.z)]) <strong>:</strong> [format_frequency(send_freq)]/[send_code]")
...
/computer/file/pda_program/qm_records
	name = "Supply Records"
	size = 8
...
/computer/file/pda_program/honk_synth
	...
	proc/honk()
		if (!master || master.active_program != src)
			return
		if (last_honk && world.time < last_honk + 20)
			return
		playsound(master.loc, "sound/items/bikehorn.ogg", (honk_volume * 25), 1)
		last_honk = world.time

		return

//TO-DO: Change to use radio system I guess
/computer/file/pda_program/mopfinder
	name = "Mop Locator"
	size = 8
	...
	return_text()
		...
		var/turf/cl = get_turf(master)
		if (cl)
			dat += "Current Orbital Location: <strong>\[[cl.x],[cl.y]\]</strong>"
			...
			for (var/obj/item/mop/M in world)
				...
				ldat += "Mop - <strong>\[[ml.x],[ml.y] ([get_area(ml)])\]</strong> - [M.reagents.total_volume ? "Wet" : "Dry"]<br>"
			...
			for (var/obj/mopbucket/B in world)
				...
				ldat += "Bucket - <strong>\[[bl.x],[bl.y] ([get_area(bl)])\]</strong> - Water level: [B.reagents.total_volume]/50<br>"
			...
			for (var/obj/machinery/bot/cleanbot/B in machines)
				...
				ldat += "Cleanbot - <strong>\[[cb.x],[cb.y] ([get_area(cb)])\]</strong> - [B.on ? "Online" : "Offline"]<br>"
...
/computer/file/pda_program/door_control
	name = "DoorMaster"
	size = 8
	var/id = 1.0
	var/last_toggle = 0
	...
	return_text()
		...
		dat += "<font size=1><em>Like this program? Send $9.95 to SPACETREND MICROSYSTEMS in Neo Toronto, Ontario for more bargain software!</em></font>"
...
/computer/file/pda_program/atmos_alerts
	name = "AtmosAlerter"
	size = 8
	...
	return_text()
		...
		if (severe_alerts.len)
			for (var/zone in severe_alerts)
				severe_text += "<font color='red'><strong>[zone]</strong></font><br>"
		else
			severe_text = "No priority alerts detected.<br>"

		if (minor_alerts.len)
			for (var/zone in minor_alerts)
				minor_text += "<strong>[zone]</strong><br>"
		else
	...
	receive_signal(signal/signal)
		...
		if (signal.data["command"] == "reply_alerts")
			...
			if (signal.data["severe_list"])
				severe_alerts = splittext(signal.data["severe_list"], ";")
			if (signal.data["minor_list"])
				minor_alerts = splittext(signal.data["minor_list"], ";")
...
//Hydroponics plant monitor.
/computer/file/pda_program/hydro_monitor
	name = "Plant Monitor"
	size = 8
	...
	proc/post_status(var/key, var/value, var/key2, var/value2, var/key3, var/value3)
		...
		post_signal(signal, report_freq)
	...
	return_text()
		...
		dat += "<strong>Status Reports:</strong><br>"

		if (status_reports.len)
			for (var/i in status_reports)
				status_text += "<strong>[i]</strong><br>"
		else
	...
	Topic(href, href_list)
		...
		if (href_list["scan"] && (world.time >= last_scan + 20))
			src.temp = "Waiting for reply, please hold..."
			status_reports.len = 0

			post_status("command","status_req")
...
//Emergency alert program
/computer/file/pda_program/emergency_alert
	name = "Crisis Alert"
	size = 4
	...
	return_text()
		...
		if (!confirm_menu)
			...
			dat += "<center><strong>Please confirm: <a href='?src=\ref[src];confirm=y'>Y</a> / <a href='?src=\ref[src];confirm=n'>N</a></strong></center>"
	...
	proc/send_alert(var/mailgroupNum=0)
		if (!master || !isnum(mailgroupNum) || (last_transmission && (last_transmission + 3000 > ticker.round_elapsed_ticks)))
			return
		...
		var/area/an_area = get_area(master)
		signal.data["message"] = "<strong><span style=\"color:red\">***CRISIS ALERT*** Location: [an_area ? an_area.name : "nowhere"]!</span></strong>"

		post_signal(signal)

//Whoever runs this gets to explode.
/computer/file/pda_program/bomb
	name = "BOMB"
	size = 8
	var/detonating = 0
	...
	return_text()
		...
		dat += "<center><strong><font color='red'>!!! NOW OVERLOADING BATTERY !!!</font></strong></center>"
		dat += "<font size=1><em>Thank you for choosing the Syndicate Robust Electronic Program Bureau</em></font>"
...
/computer/file/pda_program/missile
	name = "MISSILE"
	size = 8
	...
	return_text()
		...
		dat += "<strong>Charges remaining:</strong> [charges]<br>"

		dat += "<font size=2><a href='byond://?src=\ref[src];import=1'>Import PDA List</a></font><br>"
		dat += "<strong>Known PDAs</strong><br>"

		dat += "<ul>"
		var/count = 0
		for (var/P_id in pdas)
			var/P_name = pdas[P_id]
			if (!P_name)
				pdas -= P_id
				continue
	...
	Topic(href, href_list)
		...
		if (href_list["import"])
			if (master.host_program && istype(master.host_program, /computer/file/pda_program/os/main_os))
				pdas = master.host_program:detected_pdas

		else if (href_list["detonate"])
			if (charges <= 0)
				return
			var/target_id = href_list["detonate"]
			if (!target_id in pdas)
				return
			var/signal/signal = get_free_signal()
			signal.data["command"] = "text_message"
...
/computer/file/text/bomb_manual
	name = "Bomb Readme"
...
<li>Simply rename new BOMB.PPROG to unassuming, safe name such as <em>"PRETTY PLANTS.PPROG"</em></li>
...
<br>
<strong>Caution: </strong>Do not run BOMB.PPROG on your own system!  It will explode!
"}

//Security ticket writer - not really a small prog any more but oh well
/computer/file/pda_program/security_ticket
	name = "Ticket Master"
	size = 4
	var/mode = 0
	var/message = null
	...
	Topic(href, href_list)
		...
		if (href_list["ticket"])
			...
			var/ticket_target = input(usr, "Ticket recipient:",name) as text
			if (!ticket_target) return
			ticket_target = copytext(sanitize(html_encode(ticket_target)), 1, MAX_MESSAGE_LEN)
			var/ticket_reason = input(usr, "Ticket reason:",name) as text
			...
			spawn (30)
				var/obj/item/paper/p = new (get_turf(master))
...
/*			for (var/data/record/S in data_core.security) //there is probably a better way of doing this too
			...
			var/ticket_target = input(usr, "Fine recipient:",name) as text
			if (!ticket_target) return
			...
			if (!has_bank_record)
				...
				master.updateSelfDialog()
				return
			var/ticket_reason = input(usr, "Fine reason:",name) as text
			if (!ticket_reason) return
			ticket_reason = copytext(strip_html(ticket_reason), 1, MAX_MESSAGE_LEN)
			var/fine_amount = input(usr, "Fine amount (1-1000):",name, 0) as num
			...
			var/fine/F = locate(href_list["approve"])
...
//cargo request
/computer/file/pda_program/cargo_request
	name = "Cargo Request"
	size = 2
	var/tmp/temp = null
	var/tmp/antispam = 0
	...
	return_text()
		...
		if (temp)
			...
			dat += {"<br><strong>Supply Ordering Program</strong><HR>
	...
	Topic(href, href_list)
		...
		if (href_list["order"])
			...
			for (var/S in (subtypesof(/supply_packs)) )
				var/supply_packs/N = new S()
				...
				temp += {"<A href='?src=\ref[src];doorder=[N.type]'><strong><U>[N.name]</U></strong></A><BR>
				<strong>Cost:</strong> [N.cost] Credits<BR>
				<strong>Contents:</strong> [N.desc]<BR><BR>"}
			temp += "<BR><A href='?src=\ref[src];mainmenu=1'>OK</A>"

		else if (href_list["doorder"])
			var/supply_order/O = new/supply_order ()
			var/supplytype = href_list["doorder"]
			if (!dd_hasprefix(supplytype, "/supply_packs"))
				qdel(O)
				return
			var/supply_packs/P = new supplytype ()
			O.object = P
			O.orderedby = master.owner
			O.console_location = get_area(master)
