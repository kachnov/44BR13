...
/computer/file/pda_program/pingtool
	name = "Ping Tool"
	size = 8
...
<a href='byond://?src=\ref[src];adj_freq=100'>+</a></tt> | "}
		...
		if (mode == 0)
			...
			if (result && result.len > 0)
				...
				dat += "<em>No devices found</em><br>"

		else
			dat += "<em>Listening...</em><br>"
...
/computer/file/pda_program/packet_sniffer
	name = "Packet Sniffer"
	size = 16
	var/scan_freq = 1149
	var/range = 32
...
<a href='byond://?src=\ref[src];adj_freq=100'>+</a></tt> | "}
	...
	network_hook(signal/signal, rx_method, rx_freq)
...
/computer/file/pda_program/packet_sender
	name = "Packet Sender"
	size = 8
	var/send_freq = 1149
	var/range = 32
		...
		if (href_list["adj_freq"])
			...
			spawn ( 0 )
				...
				if ((send_freq == 1149) && (!isnull(signal.data["message"])) && (signal.data["command"] == "text_message"))
					logTheThing("pdamsg", null, null, "<em><strong>[master.owner]'s PDA used by [master.loc.name] ([master.fingerprintslast]) (as [isnull(signal.data["sender_name"]) ? "Nobody" : signal.data["sender_name"]]) &rarr; [isnull(signal.data["address_1"]) ? "Everybody" : "[signal.data["address_1"]]"]:</strong></em> [signal.data["message"]]")
