/obj/item/device/transfer_valve
	...
	attack_self(mob/user as mob)
		user.machine = src
		var/dat = {"<strong> Valve properties: </strong>
		<BR> <strong> Attachment one:</strong> [tank_one] [tank_one ? "<A href='?src=\ref[src];tankone=1'>Remove</A>" : ""]
		<BR> <strong> Attachment two:</strong> [tank_two] [tank_two ? "<A href='?src=\ref[src];tanktwo=1'>Remove</A>" : ""]
		<BR> <strong> Valve attachment:</strong> [attached_device ? "<A href='?src=\ref[src];device=1'>[attached_device]</A>" : "None"] [attached_device ? "<A href='?src=\ref[src];rem_device=1'>Remove</A>" : ""]
		<BR> <strong> Valve status: </strong> [ valve_open ? "<A href='?src=\ref[src];open=1'>Closed</A> <strong>Open</strong>" : "<strong>Closed</strong> <A href='?src=\ref[src];open=1'>Open</A>"]"}
	...
	proc
		update_icon()
			...
			if (tank_one)
				var/image/I = new(icon, icon_state = "[tank_one_icon]")
				//var/obj/overlay/tank_one_overlay = new
				//tank_one_overlay.icon = icon
				//tank_one_overlay.icon_state = tank_one_icon
				underlays += I
			if (tank_two)
				var/image/J = new(icon, icon_state = "[tank_two_icon]")
				...
				underlays += J
			if (attached_device)
				var/image/K = new(icon, icon_state = "device")
				//var/obj/overlay/device_overlay = new
				//device_overlay.icon = icon
		...
		*/
		...
		toggle_valve()
			...
			if (valve_open && force_dud)
				message_admins("A bomb valve would have opened at [log_loc(src)] but was forced to dud! Last touched by: [fingerprintslast ? "[fingerprintslast]" : "*null*"]")
				logTheThing("bombing", null, null, "A bomb valve would have opened at [log_loc(src)] but was forced to dud! Last touched by: [fingerprintslast ? "[fingerprintslast]" : "*null*"]")
				return
			...
			if (valve_open && (tank_one && tank_two))
				...
				logTheThing("bombing", null, null, "Bomb valve opened in [bombarea] ([showCoords(bombturf.x, bombturf.y, bombturf.z)]). Last touched by [fingerprintslast]")
				message_admins("Bomb valve opened in [bombarea] ([showCoords(bombturf.x, bombturf.y, bombturf.z)]). Last touched by [fingerprintslast]")

				var/gas_mixture/temp
...
//Prox sensor handling.
	...
	suicide(var/mob/user as mob)
		user.visible_message("<span style=\"color:red\"><strong>[user] drops the [name], takes a short run up and kicks the valve as hard as \he can, knocking it [valve_open ? "closed" : "open"]!</strong></span>")
		user.u_equip(src)
		set_loc(user.loc)
		toggle_valve()
		spawn (20)
			if (user)
				...
				if (user.stat == 0 && src && get_dist(user,src) <= 7)
					user.visible_message("<span style=\"color:red\">[user] stares at the [name], a confused expression on \his face.</span>") //It didn't blow up!
		return TRUE
...
/obj/item/device/transfer_valve/vr
	...
	disposing()
		...
		if (ismob(loc))
			boutput(loc, "<span style=\"color:red\">[src] fades away!</span>")
		else
			visible_message("<span style=\"color:red\">[src] fades away!</span>")
		..()
