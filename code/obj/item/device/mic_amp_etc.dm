...
/obj/item/device/microphone
	...
	get_desc()
		..()
		. += "It's currently [on ? "on" : "off"]."

	attack_self(mob/user as mob)
		on = !(on)
		user.show_text("You switch [src] [on ? "on" : "off"].")
		if (on && prob(5))
	...
	hear_talk(mob/M as mob, msg, real_name, lang_id)
		...
		if (M in range(1, T))
			talk_into(M, msg, null, real_name, lang_id)
	...
	talk_into(mob/M as mob, messages, param, real_name, lang_id)
		...
		for (var/obj/loudspeaker/S in range(7, T))
			for (var/mob/H in hearers(S, null))
				...
				H.show_text("<font size=[min(max_font, max(0, speakers - round(get_dist(H, S) / 2), 1))]><strong>[M.get_heard_name()]</strong> [U ? stuff : stuff_b]</font>")
				mobs_messaged += H
