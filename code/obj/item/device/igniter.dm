...
/obj/item/device/igniter/attack(mob/M as mob, mob/user as mob)
	if (ishuman(M))
		if (M:bleeding || (M:butt_op_stage == 4 && user.zone_sel.selecting == "chest"))
			if (!cautery_surgery(M, user, 15))
				return ..()
		else return ..()
	else return ..()

/obj/item/device/igniter/attackby(obj/item/W as obj, mob/user as mob)
	if ((istype(W, /obj/item/device/radio/signaler) && !( status )))
	...
	else if ((istype(W, /obj/item/device/prox_sensor) && !( status )))
	...
	else if ((istype(W, /obj/item/device/timer) && !( status )))
		...
		R.part2 = src
		add_fingerprint(user)
	else if ((istype(W, /obj/item/device/healthanalyzer) && !( status )))
		...
		R.part2 = src
		add_fingerprint(user)
	else if ((istype(W, /obj/item/device/multitool) && !(status)))
