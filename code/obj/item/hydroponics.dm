...
/obj/item/saw
	...
	proc/update_icon()
		icon_state = "[base_state][active ? null : "_off"]"
		return
	...
	attack(mob/target as mob, mob/user as mob)
		...
		if (active)
			...
			switch (how_dangerous_is_this_thing)
				if (2) // Red chainsaw.
					if (iscarbon(target))
						...
						if (C.stat != 2)
							...
							logTheThing("combat", user, C, "butchers [C]'s corpse with the [name] at [log_loc(C)].")
	...
	attack_self(mob/user as mob)
		if (user.bioHolder.HasEffect("clumsy") && prob(50))
			user.visible_message("<span style=\"color:red\"><strong>[user]</strong> accidentally grabs the blade of [src].</span>")
	...
	suicide(var/mob/user as mob)
		user.visible_message("<span style=\"color:red\"><strong>[user] shoves the chainsaw into \his chest!</strong></span>")
		user.u_equip(src)
		set_loc(user.loc)
		user.gib()
		return TRUE
...
*/
		...
		var/plant/pick = input(usr, "Which seed do you want?", "Portable Seed Fabricator", null) in usable
		selected = pick
	...
	afterattack(atom/target as obj|mob|turf, mob/user as mob, flag)
		if (isturf(target) && selected)
			...
			if (selected.unique_seed)
				S = new selected.unique_seed(loc)
			else
