...
/*	var/mob/living/silicon/ai/AI = null
	for (var/mob/living/silicon/ai/M in mobs)
		...
		break
	if (!AI)
		boutput(user, "<span style=\"color:red\"><strong>Unable to detect AI unit.</strong></span>")
		return
*/
	var/ai_laws/LAWS = ticker.centralized_ai_laws
	if (!LAWS)
		// YOU BETRAYED THE LAW!!!!!!
		boutput(user, "<span style=\"color:red\">Unable to detect AI unit's Law software. It may be corrupt.</span>")
		return
	boutput(user, "<strong>The AI's current laws are:</strong>")
...
/obj/machinery/computer/aiupload/attackby(obj/item/aiModule/module as obj, mob/user as mob)
	if (istype(module, /obj/item/aiModule))
		...
		for (var/mob/living/silicon/ai/M in mobs)
			M.moustache_mode = 1
			user.visible_message("<span style=\"color:red\"><strong>[user.name]</strong> uploads a moustache to [M.name]!</span>")
			M.update_appearance()
	else
		return ..()
...
/obj/item/aiModule/rename/transmitInstructions(var/mob/sender, var/law)
	law = "<span color='blue'>Your name has been changed. From now on, you will be known as <strong>[targetName]</strong></span>"
	for (var/mob/living/silicon/AI in world)
		if (!isAI(AI))
			continue
		boutput(AI, law)
