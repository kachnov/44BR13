...
/obj/item/reagent_containers/food/snacks/plant/
	...
	New()
		..()
		var/plant/species = HY_get_species_from_path(planttype)
		if (species)
			planttype = species
		else
			if (ispath(planttype))
				planttype = new planttype(src)
			else
				qdel(src)
				return
...
/obj/item/reagent_containers/food/snacks/plant/tomato/explosive
	...
	proc/ignite()
		if (lit) return
		lit = 1
		visible_message("<span style=\"color:red\">[src] catches fire!</span>")
		icon_state = "tomato-fire"
		sleep(rand(30,60))
		visible_message("<span style=\"color:red\">[src] explodes violently!</span>")
		playsound(loc, "sound/effects/splat.ogg", 100, 1)
...
/obj/item/reagent_containers/food/snacks/plant/corn
	...
	proc/pop() //Pop that corn!!
		...
		popping = 1
		visible_message("<span style=\"color:red\">[src] pops violently!</span>")
...
/obj/item/reagent_containers/food/snacks/plant/orange/
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/reagent_containers/food/snacks/ingredient/meat/synthmeat))
			...
			var/plantgenes/PDNA = P.plantgenes
...
/obj/item/reagent_containers/food/snacks/plant/apple/
	...
	heal(var/mob/M)
		...
		M.take_toxin_damage(0 - heal_amt)
		M.take_oxygen_deprivation(0 - heal_amt)
		M.take_brain_damage(0 - heal_amt)
	...
	attack_self(var/mob/user as mob)
		if (icon_state == "banana")
			if (user.bioHolder.HasEffect("clumsy") && prob(50))
				user.visible_message("<span style=\"color:red\"><strong>[user]</strong> fumbles and pokes \himself in the eye with [src].</span>")
...
/obj/item/reagent_containers/food/snacks/plant/peanuts
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/kitchen/utensil/knife) || istype(W,/obj/item/knife_butcher))
			if (icon_state == "potato")
				user.visible_message("[user] peels [src].", "You peel [src].")
				icon_state = "potato-peeled"
				desc = "It needs to be cooked."
			else if (icon_state == "potato-peeled")
...
/obj/item/reagent_containers/food/snacks/plant/purplegoop
	...
	planttype = /plant/artifact/dripper
	amount = 1
	heal_amt = 0
	food_color = "#9865c5"
	initial_volume = 25
...
/obj/item/reagent_containers/food/snacks/plant/glowfruit
	...
	planttype = /plant/artifact/litelotus
	amount = 4
	heal_amt = 1
	luminosity = 3
	food_color = "#ccccff"
