	...
	afterattack(obj/O as obj, mob/user as mob)
		if (istype(O, /obj/machinery/vehicle) || istype(O, /obj/vehicle) && user.a_intent == "harm")
			...
			if (broken)
				...
				return
			if (prob(50))
				user.visible_message("<span style=\"color:red\"><strong>[user]</strong> hits [O] with [src], shattering it open!</span>")
...
/obj/item/cocktail_stuff
	...
	attack(mob/M as mob, mob/user as mob)
		...
		if (user == M)
			...
			user.visible_message("<strong>[user]</strong> eats [src].")
		else
			boutput(M, "<span style=\"color:red\">You eat [src]. Yum!</span>")
			user.visible_message("<span style=\"color:red\"><strong>[user]</strong> sticks [src] into [M]'s mouth.</span>")
