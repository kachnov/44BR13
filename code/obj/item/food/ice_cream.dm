...
/obj/item/reagent_containers/food/snacks/ice_cream
	...
	New()
		..()
		spawn (12)
			var/icecream_name = reagents.get_master_reagent_name()
			food_color = reagents.get_master_color()
			if (icecream_name)
				name ="[icecream_name]-flavored ice cream"
			update_cone()
		return
	...
	suicide(var/mob/user as mob)
		...
		if (istype(user.r_hand,/obj/item/reagent_containers/food/snacks/ice_cream) && user.r_hand.amount)
			...
			I.update_cone()
		if (!icecount) return
		user.visible_message("<span style=\"color:red\"><strong>[user] eats the ice cream in one bite and collapses from brainfreeze.</strong></span>")
