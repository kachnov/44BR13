...
/obj/item/reagent_containers/food/drinks/bottle/grones
	...
	New()
		...
		switch(flavor)
			if (1)
				desc = "Wicked Sick Pumpkin Prolapse flavor."
				reagents.add_reagent("diarrhea", 10)
			if (2)
				desc = "Ballin' Banana Testicular Torsion flavor."
				reagents.add_reagent("urine", 10)
			if (3)
				desc = "Radical Roadkill Rampage flavor."
				reagents.add_reagent("bloodc", 10) // heh
			if (4)
				desc = "Sweet Cherry Brain Haemorrhage flavor."
				reagents.add_reagent("impedrezine", 10)
			if (5)
				desc = "Awesome Asbestos Candy Apple flavor."
				reagents.add_reagent("lithium", 10)
			if (6)
				desc = "Salt-Free Senile Dementia flavor."
				reagents.add_reagent("mercury", 10)
			if (7)
				desc = "High Fructose Traumatic Stress Disorder flavor."
				reagents.add_reagent("atropine", 10)
			if (8)
				desc = "Tangy Dismembered Orphan Tears flavor."
				reagents.add_reagent("epinephrine", 10)
			if (9)
				desc = "Chunky Infected Laceration Salsa flavor."
				reagents.add_reagent("charcoal", 10)
			if (10)
				desc = "Manic Depressive Multivitamin Dewberry flavor."
				reagents.add_reagent("ephedrine", 10)
			if (11)
				desc = "Anti-Bacterial Air Freshener flavor."
				reagents.add_reagent("spaceacillin", 10)
			if (12)
				desc = "Old Country Hay Fever flavor."
				reagents.add_reagent("antihistamine", 10)
			if (13)
				desc = "Minty Restraining Order Pepper Spray flavor."
				reagents.add_reagent("capsaicin", 10)
			if (14)
				desc = "Cool Keratin Rush flavor."
