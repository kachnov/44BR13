...
/obj/item/reagent_containers/food/snacks/pizza
	...
	New()
		...
		if (prob(1))
			spawn ( rand(300, 900) )
				visible_message("<strong>[src]</strong> <em>says, \"I'm pizza.\"</em>")
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/axe) || istype(W, /obj/item/circular_saw) || istype(W, /obj/item/kitchen/utensil/knife) || istype(W, /obj/item/scalpel) || istype(W, /obj/item/sword) || istype(W,/obj/item/saw) || istype(W,/obj/item/knife_butcher))
			...
			if (name == "cheese keyzza")
				boutput(user, "<em>You feel as though something of value has been lost...</em>")
			var/makeslices = amount
			while (makeslices > 0)
				var/obj/item/reagent_containers/food/snacks/pizza/P = new type(get_turf(src))
				P.sliced = 1
				P.amount = 1
				P.icon_state = "pslice"
				P.quality = quality
				if (topping)
					...
					P.desc = desc
					P.topping = 1
					P.topping_color = topping_color
					P.add_topping(1)
				reagents.trans_to(P, reagents.total_volume/makeslices)
				P.pixel_x = rand(-6, 6)
				P.pixel_y = rand(-6, 6)
				makeslices--
			qdel (src)
	...
	attack(mob/M as mob, mob/user as mob, def_zone)
		if (!sliced)
			if (user == M)
				...
				user.visible_message("<strong>[user]</strong> stares at [src] in a confused manner.")
				return
			else
				user.visible_message("<span style=\"color:red\"><strong>[user]</strong> futilely attempts to shove [src] into [M]'s mouth!</span>")
				return
		else
			..()
...
/obj/item/reagent_containers/food/snacks/cookie
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (!frosted && istype(W, /obj/item/reagent_containers/food/snacks/condiment/cream))
			...
			var/icon/frosticon = icon('icons/obj/foodNdrink/food_snacks.dmi', "frosting-cookie", dir, 1)
...
/obj/item/reagent_containers/food/snacks/moon_pie
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (!frosted && istype(W, /obj/item/reagent_containers/food/snacks/condiment/cream))
			...
			var/list/frosting_colors = list(rgb(0,0,255),rgb(204,0,102),rgb(255,255,0),rgb(51,153,0))
			var/icon/frosticon = icon('icons/obj/foodNdrink/food_snacks.dmi', "frosting-moonpie", dir, 1)
			frosticon.Blend(pick(frosting_colors) )
			overlays += frosticon
...
/obj/item/reagent_containers/food/snacks/soup
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/reagent_containers/food/snacks/tortilla_chip))
			...
			if (reagents)
				reagents.trans_to(W, reagents.total_volume/amount)
...
/obj/item/reagent_containers/food/snacks/soup/oatmeal
	...
	New()
		..()
		if (randomized)
			name = "[pick("cranberry", "apple cinnamon", "maple", "cran-apple";5, "blueberry-maple";5, "peaches and cream", "bananas and cream", "strawberries and cream", "plain", "cinnamon", "raisins, dates, and walnuts";5)] oatmeal"
		return
...
/obj/item/reagent_containers/food/snacks/cereal_box
	...
	attack(mob/M as mob, mob/user as mob, def_zone)
		if (user == M)
			user.visible_message("<strong>[user]</strong> pours [src] directly into their mouth!", "You eat straight from the box!")
		else
			user.visible_message("<span style=\"color:red\"><strong>[user]</strong> pours [src] into [M]'s mouth!</span>")
		...
		if (name_len > 14)
			var/tempname = name
			name = copytext(name, 14, name_len)
			..()
			name = tempname
		else
			..()
...
/obj/item/reagent_containers/food/snacks/donkpocket
	...
	temperature_expose(gas_mixture/air, exposed_temperature, exposed_volume)
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/reagent_containers/food/snacks/condiment)) amount += 1
...
/obj/item/reagent_containers/food/snacks/surstromming
	...
	process()
		if (prob(30) && icon_state == "surs_open")
	...
	heal(var/mob/M)
		if (M.bioHolder.HasEffect("accent_swedish"))
			...
			switch(effect)
				if (1 to 5)
					boutput(M, "<span style=\"color:red\">aaaaaAAAAA<strong>AAAAAAAA</strong></span>")
				...
				if (16 to 20)
					...
					if (ishuman(M))
						...
						if (prob(5) && H.organHolder && H.organHolder.heart)
							...
							H.visible_message("<span style=\"color:red\"><strong>Wait, is that their heart!?</strong></span>")
				if (21)
					if (!M.bioHolder.HasEffect("stinky"))
						boutput(M, "<span style=\"color:red\">Oh God, the stink is <strong>inside</strong> you now!</span>")
	...
	examine()
		...
		if (usr.bioHolder.HasEffect("accent_swedish"))
			...
			if (icon_state == "surs_closed")
				...
				boutput(usr, "<strong>AAAAAAAAAAAAAAAAUGH AAAAAAAAAAAUGH IT SMELLS LIKE FERMENTED SKUNK EGG BUTTS MAKE IT STOP</strong>")
			return

	attack_self(var/mob/user as mob)
		if (icon_state == "surs_closed")
			boutput(user, "<span style=\"color:blue\">You pop the lid off the [src].</span>")
			icon_state = "surs_open" //todo: get real sprite
			for (var/mob/living/carbon/M in viewers(user, null))
				if (M == user)
					if (user.bioHolder.HasEffect("accent_swedish"))
						...
						boutput(user, "<span style=\"color:red\"><font size=4><strong>HOLY FUCK THAT REEKS!!!!!</strong></font></span>")
						user.weakened+= 8
					...
					if (M.bioHolder.HasEffect("accent_swedish"))
						...
						boutput(M, "<span style=\"color:red\"><font size=4><strong>WHAT THE FUCK IS THAT SMELL!?</strong></font></span>")
						M.weakened+= 4
						M.visible_message("<span style=\"color:red\">[M] suddenly and violently vomits!</span>")
						playsound(M.loc, "sound/effects/splat.ogg", 50, 1)
						new /obj/decal/cleanable/vomit(M.loc)
...
/obj/item/reagent_containers/food/snacks/corndog
	...
	proc/update_icon()
		overlays.len = 0
		if (reagents.has_reagent("juice_tomato"))
			overlays += image(icon, "corndog-k")
			//to-do: mustard
		return
...
/obj/item/reagent_containers/food/snacks/hotdog
	...
	heal(var/mob/M)
		if (bun == 4) M.bioHolder.AddEffect("accent_elvis", timeleft = 180)
		..()
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/reagent_containers/food/snacks/breadslice))
			...
			if (istype(W, /obj/item/reagent_containers/food/snacks/breadslice/banana))
				...
				bun = 3
				desc = "A hotdog in some manner of meat-bread bun."
				heal_amt += 2
				name = "braindog"
				if (herb)
					name = "herbal " + name
				...
				bun = 5
				desc = "A very haunted hotdog. A hauntdog, perhaps."
				heal_amt += 1
				name = "frankenstein's beef frank"
				if (reagents)
					reagents.add_reagent("ectoplasm", 10)
				if (herb)
					name = "herbal " + name
				...
				bun = 1
				desc = "A hotdog! A staple of both sporting events and space stations."
		...
		else if (istype(W,/obj/item/plant/herb) && !herb)
			if (bun)
	...
	proc/update_icon()
		...
		switch(bun)
			if (1)
				overlays += image(icon, "hotdog-bun")
			if (2)
				overlays += image(icon, "hotdog-bunb")
			if (3)
				overlays += image(icon, "hotdog-bunbr")
			if (4)
				overlays += image(icon, "elvisdog")
			if (5)
				icon_state = "hauntdog"
		if (reagents.has_reagent("juice_tomato"))
			overlays += image(icon, "hotdog-k")
			//to-do: mustard
		return
		...
		if (istype(W, /obj/item/reagent_containers/food/snacks/ingredient/meat))
			...
			switch(stage)
				...
				if (1)
					...
					heal_amt++
					icon_state = "taco2"
					desc = "A complete taco.  Looks pretty good."
				if (2)
					boutput(user, "<span style=\"color:red\">It can't hold any more!</span>")
			return
		return
...
/obj/item/reagent_containers/food/snacks/tortilla_chip_spawner
	...
	New()
		...
		spawn (5)
			if (isturf(loc))
				for (var/x = 1, x <= 4, x++)
					new /obj/item/reagent_containers/food/snacks/tortilla_chip(loc)
	...
	on_reagent_change()
		if (reagents && reagents.total_volume)
			...
			for (var/current_id in reagents.reagent_list)
				...
				overlays += image("icon" = I, "layer" = FLOAT_LAYER)
		return
...
/obj/item/reagent_containers/food/snacks/wonton_spawner
	...
	New()
		...
		spawn (5)
			if (isturf(loc))
				for (var/x = 1, x <= 4, x++)
					new /obj/item/reagent_containers/food/snacks/wonton_wrapper(loc)
...
/obj/item/reagent_containers/food/snacks/wonton_wrapper
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (wrapped)
			...
			if (W.w_class > maximum_wrapped_size || istype(W, /obj/item/storage) || istype(W, /obj/item/storage/secure))
			...
			if (W.w_class > (maximum_wrapped_size / 2))
				name = "[W.name] eggroll"
				desc = "A rolled appetizer with a wonton wrapper skin. It really should be fried before you eat it."
				icon_state = "eggroll"
			else
				name = "[W.name] rangoon"
				desc = "A dumpling made from a wonton wrapper wrapped in a flower configuration. It really should be fried before you eat it."
				icon_state = "rangoon"
...
var/list/valid_jellybean_reagents = (subtypesof(/reagent))
...
/obj/item/reagent_containers/food/snacks/candy/everyflavor
	...
	New()
		...
		if (prob(75))
			...
			reagents.add_reagent(flavor, 50)
			...
			if (prob(10))
				...
				reagents.add_reagent("bilk", 10)
				heal_amt = 0
			else if (prob(10))
				reagents.add_reagent("gravy", 10)
			else if (prob(10))
				reagents.add_reagent("beff", 10)
				heal_amt = 0
			else if (prob(10))
				reagents.add_reagent("vomit", 10)
				heal_amt = 0
			else if (prob(10))
				reagents.add_reagent("gvomit", 10)
				heal_amt = 0
			else if (prob(10))
				reagents.add_reagent("fakecheese", 10)
			else if (prob(10))
				reagents.add_reagent("gravy", 10)
			else if (prob(10))
				reagents.add_reagent("porktonium", 10)
				...
				reagents.add_reagent("badgrease", 10)
				heal_amt = 0
		icon += reagents.get_master_color()
		food_color = reagents.get_master_color()
...
/obj/item/reagent_containers/food/snacks/lollipop
	...
	New()
		..()
		icon_state = "lpop-[rand(1,6)]"
		if (reagents && islist(flavors) && flavors.len)
			for (var/i=5, i>0, i--)
				reagents.add_reagent(pick(flavors), 1)
		return
