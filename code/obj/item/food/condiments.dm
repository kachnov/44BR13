...
/obj/item/reagent_containers/food/snacks/condiment/chocchips
	...
	afterattack(atom/target, mob/user, flag)
		if (istype(target, /obj/item/reagent_containers/food/snacks) && reagents) //Wire: fix for Cannot execute null.trans to()
...
/obj/item/shaker
	...
	afterattack(atom/A, mob/user as mob)
		if (istype(A, /obj/item/reagent_containers/food))
			A.reagents.add_reagent("[stuff]", 2)
			shakes ++
			user.show_text("You put some [stuff] onto [A].")
		else
			return ..()
	...
	attack(mob/M as mob, mob/user as mob)
		...
		if (ishuman(M))
			...
			if ((H.head && H.head.c_flags & COVERSEYES) || (H.wear_mask && H.wear_mask.c_flags & COVERSEYES) || (H.glasses && H.glasses.c_flags & COVERSEYES))
				H.tri_message("<span style=\"color:red\"><strong>[user]</strong> uselessly [myVerb]s some [stuff] onto [H]'s headgear!</span>",\
				H, "<span style=\"color:red\">[H == user ? "You uselessly [myVerb]" : "[user] uselessly [myVerb]s"] some [stuff] onto your headgear! Okay then.</span>",\
				user, "<span style=\"color:red\">You uselessly [myVerb] some [stuff] onto [user == H ? "your" : "[H]'s"] headgear![user == H ? " Okay then." : null]</span>")
				shakes ++
				return
			else if (stuff == "salt")
				H.tri_message("<span style=\"color:red\"><strong>[user]</strong> [myVerb]s something into [H]'s eyes!</span>",\
				H, "<span style=\"color:red\">[H == user ? "You [myVerb]" : "[user] [myVerb]s"] some salt into your eyes! <strong>FUCK!</strong></span>",\
				user, "<span style=\"color:red\">You [myVerb] some salt into [user == H ? "your" : "[H]'s"] eyes![user == H ? " <strong>FUCK!</strong>" : null]</span>")
				H.emote("scream")
				random_brute_damage(user, 1)
				shakes ++
				return
			else if (stuff == "pepper")
				H.tri_message("<span style=\"color:red\"><strong>[user]</strong> [myVerb]s something onto [H]'s nose!</span>",\
				H, "<span style=\"color:red\">[H == user ? "You [myVerb]" : "[user] [myVerb]s"] some pepper onto your nose! <strong>Why?!</strong></span>",\
				user, "<span style=\"color:red\">You [myVerb] some pepper onto [user == H ? "your" : "[H]'s"] nose![user == H ? " <strong>Why?!</strong>" : null]</span>")
				H.emote("sneeze")
				shakes ++
				for (var/i = 1, i <= 30, i++)
					spawn (50*i)
				...
				H.tri_message("<span style=\"color:red\"><strong>[user]</strong> [myVerb]s some [stuff] at [H]'s head!</span>",\
				H, "<span style=\"color:red\">[H == user ? "You [myVerb]" : "[user] [myVerb]s"] some [stuff] at your head! Fuck!</span>",\
				user, "<span style=\"color:red\">You [myVerb] some [stuff] at [user == H ? "your" : "[H]'s"] head![user == H ? " Fuck!" : null]</span>")
				shakes ++
				return
		else
			return ..()
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/reagent_containers))
			if (W.reagents.has_reagent("[stuff]") && W.reagents.get_reagent_amount("[stuff]") >= 15)
				user.show_text("You refill [src].", "blue")
				W.reagents.remove_reagent("[stuff]", 15)
				shakes = 0
				return
			else
				user.show_text("There isn't enough [stuff] in here to refill [src]!", "red")
				return
		else
			return ..()
