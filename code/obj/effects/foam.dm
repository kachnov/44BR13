...
/obj/effects/foam/proc/set_up(loc, var/ismetal)
	...
	if (ismetal == 1)
		...
		if (reagents)
			overlays.len = 0
			icon_state = "foam"
			foamcolor = reagents.get_master_color()
			var/icon/I = new /icon('icons/effects/effects.dmi',"foam_overlay")
			I.Blend(foamcolor, ICON_ADD)
			overlays += I
	metal = ismetal
...
// except metal foams
/obj/effects/foam/temperature_expose(gas_mixture/air, exposed_temperature, exposed_volume)
