...
/obj/machinery/the_singularitygen/process()
	...
	if (checkpointC >= 20)
		...
		logTheThing("bombing", fingerprintslast, null, "A [name] was activated, spawning a singularity at [log_loc(src)]. Last touched by: [fingerprintslast ? "[fingerprintslast]" : "*null*"]")
		message_admins("A [name] was activated, spawning a singularity at [log_loc(src)]. Last touched by: [fingerprintslast ? "[fingerprintslast]" : "*null*"]")
...
/obj/machinery/the_singularitygen/attackby(obj/item/W, mob/user)
	...
	if (istype(W, /obj/item/wrench))
		if (!anchored)
			...
			boutput(user, "You secure the [name] to the floor.")
			anchored = 1
		else if (anchored)
			anchored = 0
			playsound(loc, "sound/items/Ratchet.ogg", 75, 1)
			boutput(user, "You unsecure the [name].")
			anchored = 0

		logTheThing("station", user, null, "[anchored ? "bolts" : "unbolts"] a [name] [anchored ? "to" : "from"] the floor at [log_loc(src)].") // Ditto (Convair880).
		return
	else
		return ..()
...
/obj/machinery/the_singularity/process()
	...
	if (Dtime)//If its a temp singularity IE: an event
		if (Wtime != 0)
			if ((Wtime + Dtime) <= world.time)
				Wtime = 0
				qdel (src)
		else
			Wtime = world.time
...
/obj/machinery/the_singularity/attackby(var/obj/item/I as obj, var/mob/user as mob)
	if (istype(I, /obj/item/clothing/mask/cigarette))
		...
		if (C.lit == 0)
			C.light(user, "<span style=\"color:red\"><strong>[user]</strong> lights [C] on [src]. Holy fucking shit!</span>")
		else
			return ..()
	else
		return ..()
	...
	for (var/mob/living/carbon/M in oviewers(8, src))
		if (ishuman(M))
			...
			if (istype(H.glasses,/obj/item/clothing/glasses/meson))
				M.show_text("You look directly into [name], good thing you had your protective eyewear on!", "green")
				return
		if (M.stunned < 3)
			M.stunned = 3
		M.visible_message("<span style=\"color:red\"><strong>[M] stares blankly at [src]!</strong></span>",\
		"<strong>You look directly into [src]!<br><span style=\"color:red\">You feel weak!</span></strong>")
...
/obj/machinery/field_generator/attack_hand(mob/user as mob)
	...
	//			cleanup()
				...
				logTheThing("station", user, null, "activated a [name] at [log_loc(src)].") // Hmm (Convair880).
		else
...
/obj/machinery/field_generator/process()
	...
	if (active >= 1)
		...
		if (Varpower == 0)
			if (power <= 0)
				visible_message("<span style=\"color:red\">The [name] shuts down due to lack of power!</span>")
				icon_state = "Field_Gen"
				active = 0
				spawn (1)
					cleanup(1)
...
/obj/machinery/field_generator/attackby(obj/item/W, mob/user)
	...
	if (istype(W, /obj/item/weldingtool) && W:welding)
		...
		if (state == 1)
			...
			if ((user.loc == T && user.equipped() == W))
				...
				get_link() //Set up a link, now that we're secure!
		...
		if (state == 3)
			...
			if ((user.loc == T && user.equipped() == W))
				state = 1
				if (data_link) //Clear active link.
					data_link.master = null
					data_link = null
				W:eyecheck(user)
	...
	if (istype(W, /obj/item/card/id))
		...
		add_fingerprint(user)
		boutput(user, "<span style=\"color:red\">You hit the [name] with your [W.name]!</span>")
		for (var/mob/M in AIviewers(src))
			if (M == user)	continue
			M.show_message("<span style=\"color:red\">The [name] has been hit with the [W.name] by [user.name]!</span>")
...
/obj/machinery/field_generator/receive_signal(signal/signal)
	if (!data_link)
		return
	...
	if (signal.data["address_1"] != net_id)
		if ((signal.data["address_1"] == "ping") && signal.data["sender"])
			spawn (5) //Send a reply for those curious jerks
				post_status(target, "command", "ping_reply", "device", "PNET_ENG_FIELD", "netid", net_id)
...
/obj/machinery/containment_field/proc/shock(mob/user as mob)
	...
	s.start()
	power = max(gen_primary.power,gen_secondary.power)
	var/prot = 1
	var/shock_damage = 0
	if (power > 200)
...
/obj/machinery/emitter
	...
	var/net_id = null
	var/obj/machinery/power/data_terminal/data_link = null
	var/projectile/current_projectile = new/projectile/laser/heavy
	mats = 10
...
/obj/machinery/emitter/process()
	...
	if (((last_shot + fire_delay) <= world.time) && (active == 1))
		...
		if ((dir - 1) & dir) // Not cardinal (not power of 2)
			dir &= 12 // Cardinalize
...
/obj/machinery/emitter/attackby(obj/item/W, mob/user)
	...
	if (istype(W, /obj/item/weldingtool) && W:welding)
		...
		if (state == 3)
			...
			if ((user.loc == T && user.equipped() == W))
				state = 1
				if (data_link) //Time to clear our link.
					data_link.master = null
					data_link = null
...
//What do we do with an incoming command?
/obj/machinery/emitter/receive_signal(signal/signal)
	if (!data_link)
		return
	if (!signal || !net_id || signal.encryption)
		return
	...
	if (signal.data["address_1"] != net_id)
		if ((signal.data["address_1"] == "ping") && signal.data["sender"])
			spawn (5) //Send a reply for those curious jerks
				post_status(target, "command", "ping_reply", "device", "PNET_ENG_EMITR", "netid", net_id)
	...
	else if (sigcommand == "deactivate" && active)
		active = 0
		icon_state = "Emitter"
...
/obj/machinery/the_singularitybomb/attackby(obj/item/W, mob/user)
	...
	if (istype(W, /obj/item/weldingtool) && W:welding)
		...
		if (state == 1)
			...
			logTheThing("station", user, null, "welds a [name] to the floor at [log_loc(src)].") // Like here (Convair880).
		...
		if (state == 3)
			...
			logTheThing("station", user, null, "cuts a [name] from the floor at [log_loc(src)].") // Hmm (Convair880).
			if (activator)
				activator = null
...
/obj/machinery/the_singularitybomb/Topic(href, href_list)
	...
	if ((in_range(src, usr) && istype(loc, /turf)))
		...
		switch(href_list["action"]) //Yeah, this is weirdly set up. Planning to expand it later.
				...
				switch(href_list["spec"])
					if ("prime")
						if (!timing)
							...
							// And here (Convair880).
							logTheThing("bombing", usr, null, "activated [name] ([time] seconds) at [log_loc(src)].")
							message_admins("[key_name(usr)] activated [name] ([time] seconds) at [log_loc(src)].")
							if (ismob(usr))
								activator = usr
					...
					if ("abort")
						if (timing)
							...
							// And here (Convair880).
							logTheThing("bombing", usr, activator, "deactivated [name][activator ? " (primed by %target%" : ""] at [log_loc(src)].")
							message_admins("[key_name(usr)] deactivated [name][activator ? " (primed by [key_name(activator)])" : ""] at [log_loc(src)].")
			...
			if ("timer")
				if (!timing)
					...
					time += tp
					time = min(max(round(time), 30), 600)
				else
		...
		if (href_list["tp"])
			var/tp = text2num(href_list["tp"])
			time += tp
			time = min(max(round(time), 60), 600)
...
/obj/machinery/the_singularitybomb/attack_hand(mob/user as mob)
	...
	if (user.stat || user.restrained() || user.lying)
		return
	if ((get_dist(src, user) <= 1 && istype(loc, /turf)))
		user.machine = src
		/*
		var/dat = text("<TT><strong>Timing Unit</strong><br>[] []:[]<br><A href='?src=\ref[];tp=-30'>-</A> <A href='?src=\ref[];tp=-1'>-</A> <A href='?src=\ref[];tp=1'>+</A> <A href='?src=\ref[];tp=30'>+</A><br></TT>", (timing ? text("<A href='?src=\ref[];time=0'>Timing</A>", src) : text("<A href='?src=\ref[];time=1'>Not Timing</A>", src)), minute, second, src, src, src, src)
		dat += "<BR><BR><A href='?src=\ref[src];close=1'>Close</A>"
		*/
		user << browse(get_interface(), "window=timer")
		onclose(user, "timer")
	else
...
/obj/machinery/the_singularitybomb/proc/time()
	...
	particleMaster.SpawnSystem(new /particleSystem/bhole_warning(T))
	...
	spawn (30)
		...
		logTheThing("bombing", activator, null, "A [name] (primed by [activator ? "[activator]" : "*unknown*"]) detonates at [log_loc(src)].")
		message_admins("A [name] (primed by [activator ? "[key_name(activator)]" : "*unknown*"]) detonates at [log_loc(src)].")

		playsound(T, 'sound/machines/satcrash.ogg', 100, 0, 5, 0.5)
		if (bhole)
			var/obj/B = new /obj/bhole(get_turf(loc), rand(1600, 2400), rand(75, 100))
			B.name = "gravitational singularity"
			B.color = "#FF00FF"
		else
			new /obj/machinery/the_singularity(get_turf(loc), rand(1600, 2400))
				...
				<body bgcolor=#555>
					<div class="timing_div top_level">
						<table class="timer_table">
							<tr>
								<td class="timer[timing ? " active" : ""]" colspan=4>[get_time()]</td>
							</tr>
