/obj/machinery/portable_atmospherics
	...
	onMaterialChanged()
		..()
		if (istype(material))
			maximum_pressure = max((90 + (((material.getProperty(PROP_TENSILE) + material.getProperty(PROP_COMPRESSIVE)) - 30) / 2)) * ONE_ATMOSPHERE, ONE_ATMOSPHERE * 2)
		return
	...
	proc
		...
		connect(obj/machinery/atmospherics/portables_connector/new_port)
			...
			var/pipe_network/network = connected_port.return_network(src)
...
/obj/machinery/portable_atmospherics/attackby(var/obj/item/W as obj, var/mob/user as mob)
	if (istype(W, /obj/item/tank))
		if (!holding)
			boutput(user, "<span style=\"color:blue\">You attach the [W.name] to the the [name]</span>")
			user.drop_item()
			W.set_loc(src)
			holding = W
			update_icon()
