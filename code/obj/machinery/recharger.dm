...
/obj/machinery/recharger/attackby(obj/item/G as obj, mob/user as mob)
	...
	if (charging || charging2 || charging3 || charging4)
		return
...
/obj/machinery/recharger/attack_hand(mob/user as mob)
	...
	if (charging)
		...
		charging.set_loc(loc)
		charging = null
	if (charging2)
		charging2.update_icon()
		charging2.set_loc(loc)
		charging2 = null
	if (charging3)
		charging3.set_loc(loc)
		charging3 = null
	if (charging4)
		charging4.update_icon()
		charging4.set_loc(loc)
		charging4 = null
...
/obj/machinery/recharger/process()
	...
	if ((charging) && ! (stat & NOPOWER) )
		if (charging.cell)
			if (charging.cell.charge(20))
				icon_state = "recharger1"
				use_power(600)
			else
				icon_state = "recharger2"

	else if ((charging2) && ! (stat & NOPOWER) )
		if (charging2.cell)
			if (charging2.cell.charge(15))
				icon_state = "recharger1"
				use_power(500)
			else
				icon_state = "recharger2"

	else if ((charging3) && ! (stat & NOPOWER) )
		if (charging3.charges < charging3.maximum_charges)
			charging3.charges++
			icon_state = "recharger1"
			use_power(250)
		else
	...
	else if ((charging4) && ! (stat & NOPOWER) )
		if (charging4.charge(15))
			icon_state = "recharger1"
			use_power(500)
		else
			icon_state = "recharger2"

	else if (!(charging || charging2 || charging3 || charging4))
		icon_state = "recharger0"
