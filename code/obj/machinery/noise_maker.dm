...
/obj/machinery/noise_switch/attackby(obj/item/W, mob/user)
	user.visible_message("<span style=\"color:red\">The [name] has been hit with the [W.name] by [user.name]!</span>", "<span style=\"color:red\">You hit the [name] with your [W.name]!</span>")
...
/obj/machinery/noise_maker/attackby(obj/item/W, mob/user)
	if (istype(W, /obj/item/wirecutters))
		playsound(loc, "sound/items/Wirecutter.ogg", 60, 1)
		if (broken)
			broken = 0
			icon_state = "nm n +o"
			user.visible_message("<span style=\"color:red\">The [name] has been connected by [user.name]!</span>", "<span style=\"color:red\">You connect the [name]!</span>")
		else
			broken = 1
			icon_state = "nm n -o"
			user.visible_message("<span style=\"color:red\">The [name] has been disconnected by [user.name]!</span>", "<span style=\"color:red\">You disconnect the [name]!</span>")
...
//			last_shot = world.time
		if (sound == 0)
			playsound(loc, "sound/misc/null.ogg", 100, 1)
		else if (sound == 1)
			playsound(loc, "sound/effects/screech.ogg", 100, 1)
		else if (sound == 2)
			playsound(loc, "sound/misc/burp.ogg", 100, 1)
		else if (sound == 3)
			playsound(loc, "sound/effects/screech2.ogg", 100, 5,0)
