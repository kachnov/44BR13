

/obj/machinery/bot/goosebot
	name = "THE GOOSE"
	desc = "How did this manage to pass Nanotrasen's safety regulations?"
	icon = 'icons/obj/aibots.dmi'
	icon_state = "goosebot"
	layer = 5.0 //TODO LAYER
	density = 0
	anchored = 0
	on = 1
	health = 10
	no_camera = 1

/obj/machinery/bot/goosebot/proc/quack(var/message)
	if (!on || !message || muted)
		return
	visible_message("<span class='game say'><span class='name'>[src]</span> blares, \"[message]\"")
	return

/obj/machinery/bot/goosebot/proc/wakka_wakka()
		var/turf/moveto = locate(x + rand(-1,1),y + rand(-1, 1),z)
		if (isturf(moveto) && !moveto.density) step_towards(src, moveto)

/obj/machinery/bot/goosebot/process()
	if (prob(50) && on == 1)
		spawn (0)
			var/message = pick("HONK", "HOOOOOOONK","WACK WACK","GOWGOW","SCREEEEEE")
			quack(message)
			wakka_wakka()
			if (prob(50))
				playsound(loc, "sound/misc/thegoose_honk.ogg", 100, 0)
				throw_egg_is_true()
			else
				playsound(loc, "sound/misc/thegoose_song.ogg", 100, 0)



/obj/machinery/bot/goosebot/attack_hand(mob/user as mob)
	var/dat
	dat += "<TT><em>YOU CHOICE</em></TT><BR>"
	dat += "<TT><strong>THE GOOSE</strong></TT><BR>"
	dat += "NEW EDITION!<BR><BR>"
	dat += "LATEST TECHNOLOGY SPECIAL STYLE<BR><BR>"
	dat += "SPECIFICATIONS COLOURS AND CONTENTS MAY VARY FROM ILLUSTRATIONS<BR>"
	dat += "THE HEAD CAN TURN<BR>"
	dat += "Lamplight beautiful melody THE WHOLE BODY WILL SWING<BR>"
	dat += "WILL LAY EGG<BR>"
	dat += "USE 3 AA BATTERIES (BOT INCLUDED)<BR>"
	dat += "BUMP THE SHOT WILL TURN A CORNER<BR>"
	dat += "INSTALL THE EGG<BR>"
	user << browse("<HEAD><TITLE>THE GOOSE</TITLE></HEAD>[dat]", "window=GOOSE")
	onclose(user, "GOOSE")
	return

/obj/machinery/bot/goosebot/attackby(obj/item/W as obj, mob/user as mob)
	visible_message("<span class='combat'>[user] hits [src] with [W]!</span>")
	switch(W.damtype)
		if ("fire")
			health -= W.force * 0.5
		if ("brute")
			health -= W.force * 0.5
	if (health <= 0)
		explode()

/obj/machinery/bot/goosebot/gib()
	return explode()

/obj/machinery/bot/goosebot/explode()
	on = 0
	for (var/mob/O in hearers(src, null))
		O.show_message("<span class='combat'><strong>[src] blows apart!</strong></span>", 1)
	explosion(src, loc , 0, 0, 1, 1)
	qdel(src)
	return



/obj/machinery/bot/goosebot/proc/throw_egg_is_true()
	var/mob/living/target = locate() in view(7,src)
	if (target && !target.lying)
		var/obj/item/a_gift/easter/duck/E = new /obj/item/a_gift/easter/duck(loc)
		E.throwforce = 40
		E.name = "Goose egg"
		E.desc = "A goose's egg, apparently."
		E.throw_at(target, 16, 3)

		icon_state = "goosebot-spaz"
		visible_message("<span class='combat'><strong>[src] fires an egg at [target.name]!</strong></span>")
		playsound(loc, "sound/effects/pump.ogg", 50, 1)
		spawn (10)
			E.throwforce = 1
		spawn (50)
			icon_state = "goosebot"

	return