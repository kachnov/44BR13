...
/obj/machinery/computer/arcade/proc/arcade_action()
	...
	if ((enemy_mp <= 0) || (enemy_hp <= 0))
		...
		switch(prizeselect)
			if (1)
				prize = new /obj/item/spacecash(loc)
			...
			if (2)
				if (isrestrictedz(z))
					...
					prize = new /obj/item/device/radio/beacon(loc)
					prize.name = "electronic blink toy game"
					prize.desc = "Blink.  Blink.  Blink."
			if (3)
				prize = new /obj/item/zippo(loc)
				prize.name = "Burno Lighter"
				prize.desc = "Almost like a decent lighter!"
			if (4)
				if (prob(5))
					prize = new /obj/item/device/key/virtual(loc)
				else
					prize = new /obj/item/toy/sword(loc)
			if (5)
				prize = new /obj/item/harmonica(loc)
