...
/obj/machinery/computer/shuttle/emag_act(var/mob/user, var/obj/item/card/emag/E)
	...
	if (user)
		...
		switch(choice)
			if ("Launch")
				boutput(world, "<span style=\"color:blue\"><strong>Alert: Shuttle launch time shortened to 10 seconds!</strong></span>")
		...
		switch(choice)
			if ("Authorize")
				if (emergency_shuttle.timeleft() < 60)
					...
					return
				authorized -= W:registered
				authorized += W:registered
				if (auth_need - authorized.len > 0)
					boutput(world, text("<span style=\"color:blue\"><strong>Alert: [] authorizations needed until shuttle is launched early</strong></span>", auth_need - authorized.len))
				else
					boutput(world, "<span style=\"color:blue\"><strong>Alert: Shuttle launch time shortened to 60 seconds!</strong></span>")
			...
			if ("Abort")
				boutput(world, "<span style=\"color:blue\"><strong>All authorizations to shorting time for shuttle launch have been revoked!</strong></span>")
				authorized.len = 0
				authorized = list(  )
	return
