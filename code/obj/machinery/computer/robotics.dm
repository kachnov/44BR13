					...
					if (check_access(I))
						if (istype(R))
							if (R.client)
								...
								boutput(R, "<span style=\"color:red\"><strong>Killswitch process activated.</strong></span>")
							R.killswitch = 1
							...
							if (A.client)
								...
								boutput(A, "<span style=\"color:red\"><strong>AI Killswitch process activated.</strong></span>")
								boutput(A, "<span style=\"color:red\"><strong>Killswitch will engage in 60 seconds.</strong></span>") // more like 180 really but whatever
							A.killswitch = 1
			...
			if ("2")
				if (istype(R))
					...
					if (R.client)
						...
						boutput(R, "<span style=\"color:blue\"><strong>Killswitch process deactivated.</strong></span>")
				else if (istype(A))
					A.killswitch_time = 60
					A.killswitch = 0
					if (A.client)
						...
						boutput(A, "<span style=\"color:blue\"><strong>Killswitch process deactivated.</strong></span>")
		...
		switch(href_list["lock"])
			if ("1")
				if (istype(R))
					if (R.client)
						if (R.emagged)
							boutput(R, "<span style=\"color:blue\"><strong>Weapon Lock signal blocked!</strong></span>")
							return
						boutput(R, "<span style=\"color:red\"><strong>Weapon Lock activated!</strong></span>")
					R.weapon_lock = 1
					...
					for (var/obj/item/roboupgrade/X in R.contents)
						if (X.activated)
							X.activated = 0
							boutput(R, "<strong><span style=\"color:red\">[X] was shut down by the Weapon Lock!</span></strong>")
					...
					if (istype(I))
						if (check_access(I))
							if (A.client)
								boutput(A, "<span style=\"color:red\"><strong>Emergency lockout activated!</strong></span>")
								A.weapon_lock = 1
