...
/obj/machinery/plantpot
	...
	mats = 2
	flags = NOSPLASH
	var/plant/current = null // What is currently growing in the plant pot
	var/plantgenes/plantgenes = null // Set this up in New
	...
	New()
		...
		var/reagents/R = new/reagents(400)
		reagents = R
		...
		water_display = image('icons/obj/hydroponics/hydroponics.dmi', "wat-[water_level]")
		plant_sprite = image('icons/obj/hydroponics/hydroponics.dmi', "")
		update_icon()
	...
	proc/post_alert(var/alert_msg)
		...
		var/radio_frequency/frequency = radio_controller.return_frequency("[report_freq]")
	...
	process()
		...
		var/plant/growing = current
		var/plantgenes/DNA = plantgenes
		...
		if (growing.simplegrowth)
			...
			var/current_water_level = reagents.get_reagent_amount("water")
			...
			if (current_water_level)
				if (current_water_level < 200) // max water limit!!
					if (HYPCheckCommut(DNA,/plant_gene_strain/metabolism_slow) && prob(50))
						growth++
						if (drink_rate)
							drink_rate /= 2
					...
					if (HYPCheckCommut(DNA,/plant_gene_strain/metabolism_fast))
						drink_rate *= 2
						growth += growth_rate
			...
			for (var/X in DNA.commuts)
				switch(X)
					...
					if ("Stunted Growth")
						if (growth > 1)
							growth--
						// Slow down growth. We don't want this to reduce growth to zero in
						// any case, because that means the plant would die.
			*/

			for (var/plant_gene_strain/X in DNA.commuts)
				X.on_process(src)

		reagents.remove_any_except(drink_rate, "nectar")
		...
		if (growing.nectarlevel)
			var/current_level = reagents.get_reagent_amount("nectar")
			if (current_level < growing.nectarlevel)
				reagents.add_reagent("nectar", rand(growing.nectarlevel * 0.2, growing.nectarlevel * 0.5) )
		...
		if (growing.special_proc)
			if (plantgenes.mutation)
				...
				var/plantmutation/MUT = plantgenes.mutation
		...
		if (growth >= growing.harvtime + DNA.harvtime && !growing.isgrass)
			current_growth_level = 3
		else if (growth >= growing.growtime + DNA.growtime)
			current_growth_level = 2
		else
			current_growth_level = 1

		if (current_growth_level != grow_level)
			grow_level = current_growth_level
			do_update_icon = 1
		...
		if (!health_warning && health <= growing.starthealth / 2)
			health_warning = 1
			do_update_icon = 1
		else if (health_warning && health > growing.starthealth / 2)
			health_warning = 0
			do_update_icon = 1
		...
		if ((health < 1 || growth < 0) || (growing.harvestable && harvests < 1))
			HYPkillplant()
			return
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (current)
			...
			if (istype(current,/plant/maneater))
				// We want to be able to feed stuff to maneaters, such as meat, people, etc.
				if (istype(W, /obj/item/grab) && iscarbon(W:affecting) && istype(current,/plant/maneater))
					if (growth < 60)
					...
					if (do_after(user, 30)) // Same as the gibber and reclaimer. Was 20 (Convair880).
						if (src && W && W.loc == user && W:affecting && iscarbon(W:affecting))
							user.visible_message("<span style=\"color:red\">[name] grabs [W:affecting] and devours them ravenously!</span>")
							...
							reagents.add_reagent("blood", 120)
							spawn (25)
								if (src)
		...
		if (istype(W, /obj/item/wrench) || istype(W, /obj/item/screwdriver))
			...
			if (anchored == 1)
				user.visible_message("<strong>[user]</strong> unbolts the [src] from the floor.")
				playsound(loc, "sound/items/Screwdriver.ogg", 100, 1)
				anchored = 0
			else
				user.visible_message("<strong>[user]</strong> secures the [src] to the floor.")
				playsound(loc, "sound/items/Screwdriver.ogg", 100, 1)
				anchored = 1
			...
			if (current)
				...
				if (dead)
					visible_message("<span style=\"color:red\">[src] goes up in flames!</span>")
					reagents.add_reagent("ash", growth)
					HYPdestroyplant()
					// Ashes in the plantpot I guess.
				else
					if (!HYPdamageplant("fire",150)) visible_message("<span style=\"color:red\">[src] resists the fire!</span>")
			...
			if (current)
				...
				if (dead)
					visible_message("<span style=\"color:red\">[src] is is destroyed by [user.name]'s [W]!</span>")
					HYPdestroyplant()
					return
				else
					HYPdamageplant("physical",150,1)
					visible_message("<span style=\"color:red\">[user.name] cuts at [src] with [W]!</span>")
			...
			if (SEED.planttype)
				HYPnewplant(SEED)
				if (istype(SEED.planttype,/plant/maneater)) // Logging for man-eaters, since they can't be harvested (Convair880).
	...
	attack_ai(mob/user as mob)
		if (istype(user, /mob/living/silicon/robot) && get_dist(src, user) <= 1) return attack_hand(user)
	...
	attack_hand(var/mob/user as mob)
		...
		if (current)
			...
			if (HYPcheck_if_harvestable())
				...
				boutput(user, "You check [name] and the tray.")

				if (recently_harvested) boutput(user, "This plant has been harvested recently. It needs some time to regenerate.")
				if (!reagents.has_reagent("water")) boutput(user, "<span style=\"color:red\">The tray is completely dry.</span>")
				else
					if (reagents.get_reagent_amount("water") > 200)  boutput(user, "<span style=\"color:red\">The tray has too much water.</span>")
					if (reagents.get_reagent_amount("water") < 40) boutput(user, "<span style=\"color:red\">The tray's water level looks a little low.</span>")
				if (health >= growing.starthealth * 4) boutput(user, "<span style=\"color:blue\">The plant is flourishing!</span>")
				else if (health >= growing.starthealth * 2) boutput(user, "<span style=\"color:blue\">The plant looks very healthy.</span>")
				else if (health <= growing.starthealth / 2) boutput(user, "<span style=\"color:red\">The plant is in poor condition.</span>")
				...
				boutput(user, "There is a total of [reagents.total_volume] units of solution.")
				boutput(user, "The solution seems to contain [reag_list].")
		else
		// If there's no plant, just check what reagents are in there.
			boutput(user, "You check the solution in [name].")
			var/reag_list = ""
			for (var/current_id in reagents.reagent_list)
				var/reagent/current_reagent = reagents.reagent_list[current_id]
				reag_list += "[reag_list ? ", " : " "][current_reagent.name]"
		...
		if (current)
			...
			if (growing.growthmode == "weed")
				if (alert("Clear this tray?",,"Yes","No") == "Yes")
					usr.visible_message("<strong>[usr.name]</strong> dumps out the tray's contents.")
	...
	MouseDrop_T(atom/over_object as obj, mob/user as mob) // ty to Razage for the initial code
		if (istype(over_object, /obj/item/seed))  // Checks to make sure it's a seed being dragged onto the tray.
			...
			if (get_dist(user, over_object) > 1)
				...
				return
			attackby(over_object, user)  // Activates the same command as would be used with a seed in hand on the tray.
			return
	...
	temperature_expose(null, temp, volume)
		...
		if (temp >= 360)
			if (current)
				if (dead)
					reagents.add_reagent("saltpetre", growth)
					HYPdestroyplant()
	...
	proc/update_icon()
		water_display.icon_state = "wat-[water_level]"
		...
		var/planticon = null
		if (growing.sprite)
			planticon = "[growing.sprite]-G[grow_level]"
		else if (MUT && MUT.iconmod)
			planticon = "[MUT.iconmod]-G[grow_level]"
		else if (growing.special_icon)
			planticon = "[growing.special_icon]-G[grow_level]"
		else
			planticon = "[growing.name]-G[grow_level]"

		plant_sprite.icon = iconname
		plant_sprite.icon_state = planticon
	...
	proc/update_name()
		...
		if (growing && growing.cantscan)
			...
			if (istype(MUT,/plantmutation) && MUT.name)
				name = "[MUT.name] plant"
			else
				name = "[growing.name] plant"
		if (dead)
			name = "Dead " + name
	...
	proc/HYPcheck_if_harvestable()
		...
		if (plantgenes.mutation)
			...
			if (MUT.harvest_override && MUT.crop)
				if (growth >= current.harvtime + plantgenes.harvtime) return TRUE
				else return FALSE
	...
	proc/HYPharvesting(var/mob/living/user,var/obj/item/satchel/SA)
		...
		if (SA)
			if (SA.contents.len >= SA.maxitems)
				...
				satchelpick = input(user, "What do you want to harvest into the satchel?", "[name]", 0) in list("Everything","Produce Only","Seeds Only","Never Mind")
		...
		var/plantmutation/MUT = DNA.mutation
		if (!growing)
			logTheThing("debug", null, null, "<strong>Hydro Controls</strong>: Plant pot at \[[x],[y],[z]] used by ([user]) attempted a harvest without having a current plant.")
			return
		...
		if (hydro_controls)
			recently_harvested = 1
			harvest_warning = 0
			spawn (hydro_controls.delay_between_harvests)
				recently_harvested = 0
		else
			logTheThing("debug", null, null, "<strong>Hydro Controls</strong>: Could not access Hydroponics Controller to get Delay cap.")
		...
		if (hydro_controls)
			...
			logTheThing("debug", null, null, "<strong>Hydro Controls</strong>: Could not access Hydroponics Controller to get Harvest cap.")

		growth = growing.growtime + DNA.growtime
		...
		if (health >= growing.starthealth * 2 && prob(30))
		...
		if (health >= growing.starthealth * 4 && prob(30))
		...
		if (health <= growing.starthealth / 2 && prob(70))
		...
		for (var/plant_gene_strain/quality/Q in DNA.commuts)
			if (Q.negative)
		...
		if (growing.crop)
			if (MUT)
				if (MUT.crop)
					...
					logTheThing("debug", null, null, "<strong>I Said No/Hydroponics:</strong> Plant mutation [MUT] crop is not properly configured")
		...
		for (var/plant_gene_strain/yield/Y in DNA.commuts)
			if (Y.negative)
		...
		if (getamount < 1)
			...
			while (getamount > 0)
				...
				if (HYPCheckCommut(DNA,/plant_gene_strain/unstable))
				...
				if (istype(CROP,/obj/item/reagent_containers/food/snacks/plant))
					...
					var/plantgenes/FDNA = F.plantgenes
					...
					if (HYPCheckCommut(DNA,/plant_gene_strain/unstable) && prob(33))
					...
					C.friends = C.friends | contributors
					...
					var/plantgenes/HDNA = plantgenes
				...
				if (((growing.isgrass || growing.force_seed_on_harvest) && prob(80)) && !istype(CROP,/obj/item/seed) && !HYPCheckCommut(DNA,/plant_gene_strain/seedless))
	...
	proc/HYPnewplant(var/obj/item/seed/SEED)
		...
		// deletes the seed.
		current = SEED.planttype
		var/plant/growing = current
		var/plantgenes/DNA = plantgenes
		var/plantgenes/SDNA = SEED.plantgenes

		health = growing.starthealth

		// Now we deal with various health bonuses and penalties for the plant.

		if (growing.isgrass)
			health += plantgenes.harvests * 2
		...
		if (growing.harvestable) harvests = growing.harvests + DNA.harvests
	...
	proc/HYPdamageplant(var/damage_source, var/damage_amount, var/bypass_resistance = 0)
		...
		if (!damage_source || damage_amount < 1 || !damage_amount) return FALSE
		var/plant/growing = current
		var/plantgenes/DNA = plantgenes
		if (!growing || !DNA) return FALSE
		var/damage_prob = 100

		if (!bypass_resistance)
			switch(damage_source)
				if ("poison")
					if (HYPCheckCommut(DNA,/plant_gene_strain/immunity_toxin)) return FALSE
				if ("radiation")
					if (HYPCheckCommut(DNA,/plant_gene_strain/immunity_radiation)) return FALSE
				if ("drought")
					if (HYPCheckCommut(DNA,/plant_gene_strain/resistance_drought) && damage_prob > 0) damage_prob /= 2
					if (HYPCheckCommut(DNA,/plant_gene_strain/metabolism_fast)) damage_amount *= 2
					if (HYPCheckCommut(DNA,/plant_gene_strain/metabolism_slow) && damage_amount > 0) damage_amount /= 2
			...
			for (var/plant_gene_strain/damage_res/D in DNA.commuts)
				if (D.negative)
...
/proc/HYPadd_harvest_reagents(var/obj/item/I,var/plant/growing,var/plantgenes/DNA,var/special_condition = null)
	...
	for (var/plant_gene_strain/reagent_adder/R in DNA.commuts)
...
/proc/HYPpassplantgenes(var/plantgenes/PARENT,var/plantgenes/CHILD)
	...
	var/plantmutation/MUT = PARENT.mutation
...
/proc/HYPgeneticanalysis(var/mob/user as mob,var/obj/scanned,var/plant/P,var/plantgenes/DNA)
	...
	if (P.cantscan)
		boutput(user, "<span style=\"color:red\"><strong>ERROR:</strong> Genetic structure not recognized. Cannot scan.</span>")
		return
	...
	var/message = "--------------------------------<br>\
	<font color='blue'>Analysis of <strong>[scanned.name]</strong><br>\
	<br>\
	...
	[MUT ? "<br><font color='red'><strong>Alert:</strong> Abnormal genetic patterns detected.</font><br>" : ""]"
	...
	if (gene_strains != "")
		...
		message += "[MUT ? "" : "<br>"]<font color='red'><strong>Gene strains detected:</strong> [final]</font>"

	boutput(user, message)
	return

/proc/HYPnewmutationcheck(var/plant/P,var/plantgenes/DNA,var/obj/machinery/plantpot/PP)
	...
	if (!P || !DNA)
		return
	if (HYPCheckCommut(DNA,/plant_gene_strain/stabilizer))
		return
	if (P.mutations.len)
		for (var/plantmutation/MUT in P.mutations)
			var/chance = MUT.chance
			for (var/plant_gene_strain/mutations/M in DNA.commuts)
				if (M.negative)
					chance -= M.chance_mod
				else
					chance += M.chance_mod
...
/proc/HYPCheckCommut(var/plantgenes/DNA,var/searchtype)
	// This just checks to see if we have a paticular gene strain active.
	if (!DNA || !searchtype) return FALSE
	for (var/plant_gene_strain/X in DNA.commuts)
		if (X.type == searchtype) return TRUE
	return FALSE

/proc/HYPnewcommutcheck(var/plant/P,var/plantgenes/DNA)
	// This is the proc for checking if a new random gene strain will appear in the plant.
	if (!P || !DNA) return
	if (HYPCheckCommut(DNA,/plant_gene_strain/stabilizer))
		return
	if (P.commuts.len > 0)
		var/plant_gene_strain/MUT = null
		for (var/plant_gene_strain/X in P.commuts)
...
/proc/HYPaddCommut(var/plant/P,var/plantgenes/DNA,var/commut)
...
/proc/HYPmutateDNA(var/plantgenes/DNA,var/severity = 1)
...
/proc/HYPmutationcheck_full(var/plant/growing,var/plantgenes/DNA,var/plantmutation/MUT)
...
/proc/HYPmutationcheck_sub(var/lowerbound,var/upperbound,var/checkedvariable)
	...
	if (lowerbound || upperbound)
		if (lowerbound && checkedvariable < lowerbound) return FALSE
		if (upperbound && checkedvariable > upperbound) return FALSE
		return TRUE
	else return TRUE
		...
		if (active)
			for (var/obj/machinery/plantpot/P in view(1,src))
				if (!P.current || P.dead)
					continue
				P.growth += 2
				if (istype(P.plantgenes,/plantgenes))
					var/plantgenes/DNA = P.plantgenes
					if (HYPCheckCommut(DNA,/plant_gene_strain/photosynthesis))
						P.growth += 4

	attack_hand(var/mob/user as mob)
		add_fingerprint(user)
		active = !active
		user.visible_message("<strong>[user]</strong> switches [name] [active ? "on" : "off"].")
		icon_state = "growlamp[active]"
		...
		if (active)
			for (var/obj/machinery/plantpot/P in view(1,src))
				if (P.reagents.total_volume >= 200)
					continue
				reagents.trans_to(P, 1)

	attack_hand(var/mob/user as mob)
		add_fingerprint(user)
		active = !active
		user.visible_message("<strong>[user]</strong> switches [name] [active ? "on" : "off"].")
		icon_state = "fogmachine[active]"
