...
/obj/machinery/camera/disposing()
	...
	//logTheThing("debug", null, null, "<strong>SpyGuy/Camnet:</strong> Camera destroyed. Camera network needs a rebuild! Number of dirty cameras: [dirty_cameras.len]")
...
/obj/machinery/camera/attack_ai(var/mob/living/silicon/ai/user as mob)
	if (!istype(user)) //Other silicon mobs shouldn't try to encroach on the AI's "view through all cameras" schtick.  Mostly because it generates runtime errors.
		return
	if (network != user.network || !(status))
		return
...
/obj/machinery/camera/attackby(obj/W as obj, mob/user as mob)
	...
	if (istype(W, /obj/item/wirecutters))
		...
		if (!( status ))
			...
			logTheThing("station", null, null, "[key_name(user)] deactivated a security camera ([showCoords(loc.x, loc.y, loc.z)])")
		...
		for (var/mob/O in mobs)
			if (istype(O, /mob/living/silicon/ai))
				...
				logTheThing("station", user, O, "holds up a paper to a camera at [log_loc(src)], forcing %target% to read it. <strong>Title:</strong> [X.name]. <strong>Text:</strong> [adminscrub(X.info)]")
			else if (istype(O.machine, /obj/machinery/computer/security))
				var/obj/machinery/computer/security/S = O.machine
				if (S.current == src)
...
/proc/connect_camera_list(var/list/obj/machinery/camera/camlist, var/force_connection=0)
	if ( camlist.len < 1)  return TRUE

	logTheThing("debug", null, null, "<strong>SpyGuy/Camnet:</strong> Starting to connect cameras")
	var/count = 0
	for (var/obj/machinery/camera/C in camlist)
		...
		if (!(C.c_north || C.c_east || C.c_south || C.c_west))
			logTheThing("debug", null, null, "<strong>SpyGuy/Camnet:</strong> Camera at [showCoords(C.x, C.y, C.z)] failed to receive cardinal directions during initialization.")

	logTheThing("debug", null, null, "<strong>SpyGuy/Camnet:</strong> Done. Connected [count] cameras.")
...
/proc/connect_camera_neighbours(var/obj/machinery/camera/C, var/direction, var/force_connection=0)
	...
	if (!C.vars[dir_var] || force_connection)
		...
		/*
		if (!candidate)
			logTheThing("debug", null, null, "<strong>SpyGuy/Camnet:</strong> Camera at [showCoords(C.x, C.y, C.z)] didn't get a candidate when heading [dir2text(direction)].")
			return
		*/
...
/*
			...
			logTheThing("debug", null, null, "<strong>SpyGuy/Camnet:</strong> Camera at [showCoords(C.x, C.y, C.z)] rejected. cand z = [candidate.z], C z = [C.z]; cand net = [candidate.network], C net = [C.network]; reciprocity = [camera_network_reciprocity], rec_var:[rec_var] ( [isnull(candidate.vars[rec_var]) ? "null" : "not null"] )")
	else
		logTheThing("debug", null, null, "<strong>SpyGuy/Camnet:</strong> Camera at [showCoords(C.x, C.y, C.z)] rejected because [dir_var] was already set.")
		*/
