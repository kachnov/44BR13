...
/obj/machinery/teleport/portal_ring
	...
	process()
		if (icon_state == "tele1") // REALLY. really. really? r e a l l y?
			power_usage = 5000
		else
			power_usage = 0
		..()
	...
	proc/teleport(atom/movable/M as mob|obj)
		if (find_links() < 2)
			visible_message("<strong>[src]</strong> intones, \"System error. Cannot find required equipment links.\"")
			return
		if (!linked_computer.locked)
			visible_message("<strong>[src]</strong> intones, \"System error. Cannot verify locked co-ordinates.\"")
			return
...
/obj/machinery/teleport/portal_generator
	...
	proc/engage()
		...
		for (var/obj/machinery/teleport/portal_ring/R in linked_rings)
			...
			R.light.enable()
		use_power(5000)
		visible_message("<strong>[src]</strong> intones, \"Teleporter engaged.\"")
		add_fingerprint(usr)
		engaged = 1
		return
	...
	proc/disengage()
		...
		for (var/obj/machinery/teleport/portal_ring/R in linked_rings)
			R.icon_state = "tele0"
			R.light.disable()
		visible_message("<strong>[src]</strong> intones, \"Teleporter disengaged.\"")
		add_fingerprint(usr)
		engaged = 0
		return
	...
	if ((istype(tmploc.loc, /area) && tmploc.loc:teleport_blocked) || m_blocked)
		...
		if ((!restr && use_teleblocks) || restr)
			if (istype(M,/mob/living))
				boutput(M, "<span style=\"color:red\"><strong>Teleportation failed!</strong></span>")
				return
