...
/obj/machinery/light/New()
	...
	light.set_color(initial(light_type.color_r), initial(light_type.color_g), initial(light_type.color_b))
		...
		if (status == LIGHT_OK)
			if (on && rigged)
				if (rigger)
					...
					logTheThing("combat", rigger, null, "'s rigged bulb exploded in [rigger.loc.loc] ([showCoords(x, y, z)])")
				explode()
...
#define LIGHTING_POWER_FACTOR 40 // Used to be 20 with the old brightness values (reduced drastically for the new lighting system).

/obj/machinery/light/process()
	if (on)
		..()
		var/thepower = brightness * LIGHTING_POWER_FACTOR
		...
		if (rigged)
			if (prob(1))
				...
				if (rigger)
					...
					logTheThing("combat", rigger, null, "'s rigged bulb tried to explode but failed in [rigger.loc.loc] ([showCoords(x, y, z)])")
				rigged = 0
				rigger = null
// called when area power state changes

/obj/machinery/light/power_change()
	if (loc) //TODO fix the dispose proc for this so that when it is sent into the delete queue it doesn't try and exec this
		var/area/A = get_area(src)
		seton(A.lightswitch && A.power_light)

// called when on fire

/obj/machinery/light/temperature_expose(gas_mixture/air, exposed_temperature, exposed_volume)
