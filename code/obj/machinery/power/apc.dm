...
/obj/machinery/power/apc/New()
	...
	if (!isnull(areastring) && !isnull(get_area_name(areastring)))
		...
		if (autoname_on_spawn == 1 || (name == "N APC" || name == "E APC" || name == "S APC" || name == "W APC"))
			name = "[area.name] APC"
	...
	if (setup_networkapc)
		terminal = new /obj/machinery/power/terminal/netlink(loc)
		...
		if (istype(W, /obj/item/screwdriver))
			switch (repair_status)
				...
				if (4)
					...
					if (!terminal)
						var/obj/machinery/power/terminal/newTerm = locate(/obj/machinery/power/terminal) in loc
						if (istype(newTerm) && !newTerm.master)
							terminal = newTerm
							newTerm.master = src
							newTerm.dir = initial(dir) //Can't use CURRENT dir because it is set to south on spawn.
						else
							if (setup_networkapc)
								terminal = new /obj/machinery/power/terminal/netlink(loc)
							else
			...
			switch (repair_status)
				...
				if (1) //Repair the transformer with a cable.
					...
					if (theCoil.amount >= 4)
						...
						boutput(user, "<span style=\"color:red\">Not enough cable! <em>(Requires four pieces)</em></span>")
						return
...
/obj/machinery/power/apc/proc/interact(mob/user)
	...
	if (wiresexposed && (!istype(user, /mob/living/silicon/ai)))
		...
		for (var/wiredesc in apcwires)
			var/is_uncut = apcwires & APCWireColorToFlag[apcwires[wiredesc]]
			t1 += "[wiredesc] wire: "
			if (!is_uncut)
			...
			t1 += "<br>"
		t1 += text("<br><br>[(locked ? "The APC is locked." : "The APC is unlocked.")]<br><br>[(shorted ? "The APCs power has been shorted." : "The APC is working properly!")]<br><br>[(aidisabled ? "The 'AI control allowed' light is off." : "The 'AI control allowed' light is on.")]")
		t1 += text("<p><a href='?src=\ref[src];close2=1'>Close</a></p><br>")
		user << browse(t1, "window=apcwires")
		onclose(user, "apcwires")

	user.machine = src
	var/t = "<TT><strong>Area Power Controller</strong> ([area.name])<HR>"

	if ((locked || (setup_networkapc > 1)) && (!istype(user, /mob/living/silicon)))
		if (setup_networkapc < 2)
			t += "<em>(Swipe ID card to unlock inteface.)</em><BR>"
		else
			t += "Host Connection: <strong>[src.host_id ? "<font color=green>OK</font>" : "<font color=red>NONE</font>"]</strong><BR>"
		t += "Main breaker : <strong>[operating ? "On" : "Off"]</strong><BR>"
		t += "External power : <strong>[ main_status ? (main_status ==2 ? "<FONT COLOR=#004000>Good</FONT>" : "<FONT COLOR=#D09000>Low</FONT>") : "<FONT COLOR=#F00000>None</FONT>"]</strong><BR>"
		t += "Power cell: <strong>[cell ? "[round(cell.percent())]%" : "<FONT COLOR=red>Not connected.</FONT>"]</strong>"
		if (cell)
		...
		t += "Equipment:    [add_lspace(lastused_equip, 6)] W : <strong>[L[equipment+1]]</strong><BR>"
		t += "Lighting:     [add_lspace(lastused_light, 6)] W : <strong>[L[lighting+1]]</strong><BR>"
		t += "Environmental:[add_lspace(lastused_environ, 6)] W : <strong>[L[environ+1]]</strong><BR>"

		t += "<BR>Total load: [lastused_light + lastused_equip + lastused_environ] W</PRE>"
		t += "<HR>Cover lock: <strong>[coverlocked ? "Engaged" : "Disengaged"]</strong>"

	else
		if (!istype(user, /mob/living/silicon))
			t += "<em>(Swipe ID card to lock interface.)</em><BR>"
		t += "Main breaker: [operating ? "<strong>On</strong> <A href='?src=\ref[src];breaker=1'>Off</A>" : "<A href='?src=\ref[src];breaker=1'>On</A> <strong>Off</strong>" ]<BR>"
		t += "External power : <strong>[ main_status ? (main_status ==2 ? "<FONT COLOR=#004000>Good</FONT>" : "<FONT COLOR=#D09000>Low</FONT>") : "<FONT COLOR=#F00000>None</FONT>"]</strong><BR>"
		if (cell)
			t += "Power cell: <strong>[round(cell.percent())]%</strong>"
			t += " ([charging ? ( charging == 1 ? "Charging" : "Fully charged" ) : "Not charging"])"
			t += " ([chargemode ? "<A href='?src=\ref[src];cmode=1'>Off</A> <strong>Auto</strong>" : "<strong>Off</strong> <A href='?src=\ref[src];cmode=1'>Auto</A>"])"

		else
			t += "Power cell: <strong><FONT COLOR=red>Not connected.</FONT></strong>"
		...
		switch(equipment)
			if (0)
				t += "<strong>Off</strong> <A href='?src=\ref[src];eqp=2'>On</A> <A href='?src=\ref[src];eqp=3'>Auto</A>"
			if (1)
				t += "<A href='?src=\ref[src];eqp=1'>Off</A> <A href='?src=\ref[src];eqp=2'>On</A> <strong>Auto (Off)</strong>"
			if (2)
				t += "<A href='?src=\ref[src];eqp=1'>Off</A> <strong>On</strong> <A href='?src=\ref[src];eqp=3'>Auto</A>"
			if (3)
				t += "<A href='?src=\ref[src];eqp=1'>Off</A> <A href='?src=\ref[src];eqp=2'>On</A> <strong>Auto (On)</strong>"
		t +="<BR>"
		...
		switch(lighting)
			if (0)
				t += "<strong>Off</strong> <A href='?src=\ref[src];lgt=2'>On</A> <A href='?src=\ref[src];lgt=3'>Auto</A>"
			if (1)
				t += "<A href='?src=\ref[src];lgt=1'>Off</A> <A href='?src=\ref[src];lgt=2'>On</A> <strong>Auto (Off)</strong>"
			if (2)
				t += "<A href='?src=\ref[src];lgt=1'>Off</A> <strong>On</strong> <A href='?src=\ref[src];lgt=3'>Auto</A>"
			if (3)
				t += "<A href='?src=\ref[src];lgt=1'>Off</A> <A href='?src=\ref[src];lgt=2'>On</A> <strong>Auto (On)</strong>"
		t +="<BR>"
		...
		switch(environ)
			if (0)
				t += "<strong>Off</strong> <A href='?src=\ref[src];env=2'>On</A> <A href='?src=\ref[src];env=3'>Auto</A>"
			if (1)
				t += "<A href='?src=\ref[src];env=1'>Off</A> <A href='?src=\ref[src];env=2'>On</A> <strong>Auto (Off)</strong>"
			if (2)
				t += "<A href='?src=\ref[src];env=1'>Off</A> <strong>On</strong> <A href='?src=\ref[src];env=3'>Auto</A>"
			if (3)
				t += "<A href='?src=\ref[src];env=1'>Off</A> <A href='?src=\ref[src];env=2'>On</A> <strong>Auto (On)</strong>"



		t += "<BR>Total load: [lastused_light + lastused_equip + lastused_environ] W</PRE>"
		t += "<HR>Cover lock: [coverlocked ? "<strong><A href='?src=\ref[src];lock=1'>Engaged</A></strong>" : "<strong><A href='?src=\ref[src];lock=1'>Disengaged</A></strong>"]"


		if (istype(user, /mob/living/silicon))
			t += "<BR><HR><A href='?src=\ref[src];overload=1'><em>Overload lighting circuit</em></A><BR>"
...
/obj/machinery/power/apc/proc/shock(mob/user, prb, bite)
	...
	if (!net)		// cable is unpowered
		return FALSE
	return apcelectrocute(user, prb, net, bite)
...
/obj/machinery/power/apc/proc/cut(var/wireColor)
	...
	switch(wireIndex)
		if (APC_WIRE_MAIN_POWER1)
			shock(usr, 50, 0)			//this doesn't work for some reason, give me a while I'll figure it out
			shorted = 1
			updateUsrDialog()
...
/obj/machinery/power/apc/proc/bite(var/wireColor) // are you fuckin retarded
	...
	switch(wireIndex)
		if (APC_WIRE_MAIN_POWER1)
			shock(usr, 90, 1)			//this doesn't work for some reason, give me a while I'll figure it out
			shorted = 1
			updateUsrDialog()
		if (APC_WIRE_MAIN_POWER2)
			shock(usr, 90, 1)
...
/obj/machinery/power/apc/process()
	...
	if (setup_networkapc && host_id && terminal)
		if (timeout == 0)
...
/obj/machinery/power/apc/proc/post_status(var/target_id, var/key, var/value, var/key2, var/value2, var/key3, var/value3)
	if (!istype(terminal, /obj/machinery/power/terminal/netlink) || !target_id)
		return
	...
	var/obj/machinery/power/terminal/netlink/theLink = terminal
	theLink.post_signal(src, signal)

/obj/machinery/power/apc/receive_signal(signal/signal)
	if ((stat & BROKEN) || !setup_networkapc || aidisabled)
		return
	...
	if (signal.data["address_1"] != net_id)
		if ((signal.data["address_1"] == "ping") && signal.data["sender"])
			spawn (5)
				post_status(target, "command", "ping_reply", "device", "PNET_PWR_CNTRL", "netid", net_id)
	...
	switch(sigcommand)
		if ("term_connect")
			...
			if (signal.data["data"] != "noreply")
				post_status(target, "command","term_connect","data","noreply","device","PNET_PWR_CNTRL")
			//updateUsrDialog()
			spawn (2)
				post_status(target,"command","term_message","data","command=register&data=[ckey("[area]")]")
			return
