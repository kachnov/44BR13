...
/obj/machinery/power/pt_laser/proc/power_sold()
	...
	if (price >= accounts.len*2) //otherwise not enough to split evenly so don't bother I guess
		...
		for (var/data/record/t in accounts)
...
/obj/machinery/power/pt_laser/proc/melt_blocking_objects()
	for (var/obj/O in blocking_objects)
		if (istype(O, /obj/machinery/door/poddoor))
			...
			O.visible_message("<strong>[O.name] is melted away by the [src]!</strong>")
			qdel(O)
...
/obj/machinery/power/pt_laser/proc/interact(mob/user)
	...
	var/t = "<TT><strong>Power Transmission Laser</strong><HR><PRE>"

	t += "Efficiency: 0.05<BR><BR>"

	t += "Stored capacity: [engineering_notation(charge)]J ([round(100.0*charge/capacity, 0.1)]%)<BR>"

	t += "Input: [charging ? "Charging" : "Not Charging"]    [charging ? "<strong>On</strong> <A href = '?src=\ref[src];cmode=1'>Off</A>" : "<A href = '?src=\ref[src];cmode=1'>On</A> <strong>Off</strong> "]<BR>"

	switch(input_multi)
		if (1)
			t += "Input level: <A href = '?src=\ref[src];set_input=1'>[input_number]</A> <strong>W</strong> <A href = '?src=\ref[src];input=1'>kW</A> <A href = '?src=\ref[src];input=2'>MW</A> <A href = '?src=\ref[src];input=3'>GW</A> <A href = '?src=\ref[src];input=4'>TW</A><BR>"
		if (1e3)
			t += "Input level: <A href = '?src=\ref[src];set_input=1'>[input_number]</A> <A href = '?src=\ref[src];input=0'>W</A> <strong>kW</strong> <A href = '?src=\ref[src];input=2'>MW</A> <A href = '?src=\ref[src];input=3'>GW</A> <A href = '?src=\ref[src];input=4'>TW</A><BR>"
		if (1e6)
			t += "Input level: <A href = '?src=\ref[src];set_input=1'>[input_number]</A> <A href = '?src=\ref[src];input=0'>W</A> <A href = '?src=\ref[src];input=1'>kW</A> <strong>MW</strong> <A href = '?src=\ref[src];input=3'>GW</A> <A href = '?src=\ref[src];input=4'>TW</A><BR>"
		if (1e9)
			t += "Input level: <A href = '?src=\ref[src];set_input=1'>[input_number]</A> <A href = '?src=\ref[src];input=0'>W</A> <A href = '?src=\ref[src];input=1'>kW</A> <A href = '?src=\ref[src];input=2'>MW</A> <strong>GW</strong> <A href = '?src=\ref[src];input=4'>TW</A><BR>"
		if (1e12)
			t += "Input level: <A href = '?src=\ref[src];set_input=1'>[input_number]</A> <A href = '?src=\ref[src];input=0'>W</A> <A href = '?src=\ref[src];input=1'>kW</A> <A href = '?src=\ref[src];input=2'>MW</A> <A href = '?src=\ref[src];input=3'>GW</A> <strong>TW</strong><BR>"

	t += "<BR><BR>"

	t += "Output: [online ? "<strong>Online</strong> <A href = '?src=\ref[src];online=1'>Offline</A>" : "<A href = '?src=\ref[src];online=1'>Online</A> <strong>Offline</strong> "]<BR>"

	switch(output_multi)
		if (1e6)
			t += "Output level: <A href = '?src=\ref[src];set_output=1'>[output_number]</A> <strong>MW</strong> <A href = '?src=\ref[src];output=3'>GW</A> <A href = '?src=\ref[src];output=4'>TW</A><BR>"
		if (1e9)
			t += "Output level: <A href = '?src=\ref[src];set_output=1'>[output_number]</A> <A href = '?src=\ref[src];output=2'>MW</A> <strong>GW</strong> <A href = '?src=\ref[src];output=4'>TW</A><BR>"
		if (1e12)
			t += "Output level: <A href = '?src=\ref[src];set_output=1'>[output_number]</A> <A href = '?src=\ref[src];output=2'>MW</A> <A href = '?src=\ref[src];output=3'>GW</A> <strong>TW</strong><BR>"
...
/obj/lpt_laser/New()
	...
	spawn (0)
		...
		if (active)
			if (istype(loc, /turf) && power > 5e7)
				loc:hotspot_expose(power/1e5,5) //1000K at 100MW
			if (istype(loc, /turf/simulated/floor) && prob(power/1e6))
				loc:burn_tile()
...
/obj/lpt_laser/HasEntered(var/atom/movable/AM)
	if (active && isliving(AM) && !isintangible(AM))
...
/obj/proc/burn_living(var/mob/living/L,var/power = 0)
	...
	if (L.dir == turn(dir,180) && ishuman(L)) //they're looking into the beam!
		var/safety = 1
	...
	switch(power)
		...
		if (1e7+1 to 5e8)
			...
			if (ishuman(L) && prob(min(power/1e7,50)))
				...
				L:sever_limb(limb)
				L.visible_message("<strong>The [name] slices off one of [L.name]'s limbs!</strong>")
		if (5e8+1 to 1e11) //you really fucked up this time buddy
			new /obj/decal/cleanable/ash(loc)
			L.elecgib()
			L.visible_message("<strong>[L.name] is vaporised by the [src]!</strong>")
			return TRUE //tells the caller to remove L from the laser's affecting_mobs
		if (1e11+1 to INFINITY) //you really, REALLY fucked up this time buddy
			L.visible_message("<strong>[L.name] is detonated by the [src]!</strong>")
