	...
	if (istype(W, /obj/item/card/id))
		if (allowed(user, req_only_one_required))
			if (open)
				...
				return
			locked = !locked
			boutput(user, "<span style=\"color:blue\">You [locked ? "lock" : "unlock"] the switchgear access panel.</span>")
		else
...
/obj/machinery/power/switchgear/proc/interact(mob/user)
	...
	user.machine = src
	var/t = "<TT><strong>Switchgear Control</strong><BR><A href='?src=\ref[src];close=1'>Close Panel</A><HR>"
