...
/atom/proc/electrocute(mob/user, prb, netnum, var/ignore_gloves)
	...
	return user.shock(src, PN.get_avail(), user.hand == 1 ? "l_arm": "r_arm", 1, ignore_gloves ? 1 : 0)
...
/obj/cable
	...
	var/material/insulator = null
	var/material/conductor = null
...
/obj/cable/New(var/newloc, var/obj/item/cable_coil/source)
	...
	cableimg = image(icon, loc, icon_state)
...
/obj/cable/disposing()		// called when a cable is deleted
	...
	if (!defer_powernet_rebuild)	// set if network will be rebuilt manually
		...
		if (netnum && powernets && powernets.len >= netnum)		// make sure cable & powernet data is valid
			var/powernet/PN = powernets[netnum]
...
/obj/cable/attackby(obj/item/W, mob/user)
	...
	if (istype(W, /obj/item/wirecutters))
		...
		if (d1)	// 0-X cables are 1 unit, X-X cables are 2 units long
			...
			applyCableMaterials(A, insulator, conductor)
		else
			var/atom/A = new/obj/item/cable_coil(T, 1)
			applyCableMaterials(A, insulator, conductor)

		visible_message("<span style=\"color:red\">[user] cuts the cable.</span>")
		log_wirelaying(user, 1)
		...
		var/powernet/PN = get_powernet()		// find the powernet

		if (PN && (PN.get_avail() > 0))		// is it powered?
			boutput(user, "<span style=\"color:red\">[PN.get_avail()]W in power network.</span>")
...
/obj/cable/proc/shock(mob/user, prb)
	...
	return electrocute(user, prb, netnum)

		//var/shock_damage = 0

// cogwerks - simplifying this stack of ranges below into something that feels better

		//shock_damage = (max(rand(10,20), round(PN.get_avail() * 0.00004)))*prot // adjust the multiplier as needed

/*	if (PN.get_avail() > 700000)	//someone juiced up the grid enough, people going to die!
			shock_damage = min(rand(70,145),rand(70,145))*prot
		else if (PN.get_avail() > 200000)
...
//cogwerks - fun horrible things
		...
		switch(shock_damage)
			...
			if (60 to 99)
				...
				boutput(user, "<span style=\"color:red\"><strong>[src] discharges a violent arc of electricity!</strong></span>")
			...
			if (100 to INFINITY)  // cogwerks - here are the big fuckin murderflashes
				...
				if (istype(user, /mob/living/carbon/human))
					...
					if (prob(20))
						boutput(M, "<span style=\"color:red\"><strong>[src] vaporizes you with a lethal arc of electricity!</strong></span>")
						if (M.shoes)
						...
						boutput(M, "<span style=\"color:red\"><strong>[src] blasts you with an arc flash!</strong></span>")
						if (M.shoes)
							M.drop_from_slot(M.shoes)
...
/obj/cable/ex_act(severity)
	switch (severity)
		...
		if (2)
			if (prob(15))
				var/atom/A = new/obj/item/cable_coil(loc, d1 ? 2 : 1)
				applyCableMaterials(A, insulator, conductor)
			qdel(src)
...
/obj/cable/proc/update_network()
	...
	if (cable_d1 && cable_d2)
		if (cable_d1.netnum == cable_d2.netnum)
			var/powernet/PN = powernets[cable_d1.netnum]
			PN.cables += src
			netnum = cable_d1.netnum
			if (PT && istype(PT))
		...
		var/powernet/PN = powernets[cable_d2.netnum]
		PN.cables += src
		netnum = cable_d2.netnum
...
/obj/cable/proc/log_wirelaying(var/mob/user, var/cut = 0)
	...
	var/powered = 0
	var/powernet/PN = get_powernet()
	if (PN && istype(PN) && (PN.get_avail() > 0))
		powered = 1
