...
/obj/machinery/power/proc/surplus()
	return 100000
	if (powernet)
		return powernet._avail-powernet.load
	else
		return FALSE
...
/proc/makepowernets()
	...
	for (var/obj/cable/C in allcables)
		var/powernet/PN = powernets[C.netnum]
		PN.cables += C
...
/obj/cable/proc/get_connections()
	...
	var/turf/T
	if (!d1)
		T = loc		// if d1=0, same turf as src
	else
		T = get_step(src, d1)
...
/powernet/proc/cut_cable(var/obj/cable/C)
...
/powernet/proc/reset()
	...
	if (numapc)
		perapc = _avail/numapc

	netexcess = _avail - load
