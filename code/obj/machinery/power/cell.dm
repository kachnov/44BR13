/obj/item/cell
	...
	onMaterialChanged()
		...
		if (istype(material))
			...
			maxcharge = ((15000 * material.getProperty(PROP_ELECTRICAL)) + (material.getProperty(PROP_DIELECTRIC) * 750)) //should be 0 to 20000
			charge = maxcharge
		return
...
/obj/item/cell/examine()
	...
	if (usr && !usr.stat)
		if (maxcharge <= 2500)
			boutput(usr, "[desc]<br>The manufacturer's label states this cell has a power rating of [maxcharge], and that you should not swallow it.<br>The charge meter reads [round(percent() )]%.")
		else
			boutput(usr, "This power cell has an exciting chrome finish, as it is an uber-capacity cell type! It has a power rating of [maxcharge]!!!<br>The charge meter reads [round(percent() )]%.")
...
/obj/item/cell/proc/zap(mob/user as mob, var/ignores_gloves = 0)
	if (user.shock(src, charge, user.hand == 1 ? "l_arm" : "r_arm", 1, ignores_gloves))
...
/obj/item/cell/ex_act(severity)
	if (istype(src,/obj/item/cell/erebite)) explode()
	else ..()

/obj/item/cell/temperature_expose(null, temp, volume)
	if (istype(src,/obj/item/cell/erebite))
		visible_message("<span style=\"color:red\">[src] violently detonates!</span>")
		explode()
	else ..()
...
/obj/item/cell/detonator_act(event, var/obj/item/assembly/detonator/det)
	switch (event)
		...
		if ("cut")
			visible_message("<span class='bold' style='color: #B7410E;'>The failsafe timer buzzes refusingly before going quiet forever.</span>")
			spawn (0)
				det.detonate()