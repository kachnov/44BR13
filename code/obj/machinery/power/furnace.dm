/obj/machinery/power/furnace
	...
	process()
		...
		//overlays = null
		//if (active) overlays += image('icons/obj/power.dmi', "furn-burn")
	...
	proc/update_icon()
		...
		var/fuel_state = round(min((fuel / maxfuel) * 5, 4))
	...
	attack_hand(var/mob/user as mob)
		if (!fuel) boutput(user, "<span style=\"color:red\">There is no fuel in the furnace!</span>")
		else
			active = !active
			boutput(user, "You switch [active ? "on" : "off"] the furnace.")
		...
		if (istype(W, /obj/item/grab))
			if (!active)
				...
				if (W:affecting && active)
					...
					return
		else if (istype(W, /obj/item/raw_material/char)) fuel += 60
		else if (istype(W, /obj/item/raw_material/plasmastone)) fuel += 800
		else if (istype(W, /obj/item/paper)) fuel += 6
		else if (istype(W, /obj/item/spacecash)) fuel += 6
	...
	MouseDrop_T(atom/movable/O as mob|obj, mob/user as mob)
		...
		if (istype(O, /obj/storage/crate))
			...
			fuel += initial(C.health) * 8
			stoked += max(C.quality / 2, 0)
			qdel(O)
		else ..()
		updateUsrDialog()

	suicide(var/mob/user as mob)
		user.visible_message("<span style=\"color:red\"><strong>[user] climbs into the furnace!</strong></span>")
		user.death(1)
		if (user.mind)
			user.ghostize()
			qdel(user)
