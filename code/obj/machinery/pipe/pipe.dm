...
//	var/dbg = (suffix == "d") && Debug
	...
	if (tot_node>0.1)		// no pipe contents, don't heat
		for (var/obj/machinery/pipes/P in nodes)		// for each segment of pipe
...
/obj/machinery/pipes/proc/update()
	...
	if ((level == 1 && isturf(loc) && T.intact))
		invisibility = 101
		is += "-f"
...
/*
	...
	if (strength < 773.0)
		...
		//if ((gas.temperature > (450+T0C) && gas.plasma == 1600000.0))
		...
		if (strength > (450+T0C))
			...
			//if ((gas.temperature > (300+T0C) && gas.plasma == 1600000.0))
			if (strength > (300+T0C))
				var/turf/sw = locate(max(T.x - 4, 1), max(T.y - 4, 1), T.z)
				var/turf/ne = locate(min(T.x + 4, world.maxx), min(T.y + 4, world.maxy), T.z)
				defer_powernet_rebuild = 1
...
/obj/machinery/pipes/buildnodes(var/linenum)
	...
	node1 = get_machine(level, loc, dirs[1])
	node2 = get_machine(level, loc, dirs[2])
...
/obj/machinery/pipes/heat_exch/buildnodes(var/linenum)
	...
	level = 2		// h/e pipe cannot be put underfloor

	var/list/dirs = get_dirs()

	node1 = get_he_machine(level, loc, dirs[1])
	node2 = get_he_machine(level, loc, dirs[2])
...
/obj/machinery/pipes/proc/heat_exchange(var/gas_mixture/gas, var/tot_node, var/numnodes, var/temp, var/dbg=0)

/*	var/turf/T = loc		// turf location of pipe
	if (T.density) return
...
/obj/machinery/circulator/New()
	..()
	gas1 = unpool(/gas_mixture)
	gas2 = unpool(/gas_mixture)

	ngas1 = unpool(/gas_mixture)
	ngas2 = unpool(/gas_mixture)
...
/obj/machinery/connector/New()
	...
	gas = new /gas_mixture()
	ngas = new /gas_mixture()
	//agas = new/obj/substance/gas()

	gasflowlist += src
	spawn (5)
		var/obj/machinery/atmoalter/A = locate(/obj/machinery/atmoalter, loc)
...
/obj/machinery/proc/flow_to_turf(var/gas_mixture/sgas, var/gas_mixture/sngas, var/turf/T, var/dbg = 0)
/*
...
/obj/machinery/valve/mvalve/New()
	..()
	gas1 = new/gas_mixture/(src)
	ngas1 = new/gas_mixture/()
	gas2 = new/gas_mixture/(src)
	ngas2 = new/gas_mixture/()
...
/obj/machinery/inlet/filter/proc/updateicon()
	...
	if (stat & NOPOWER)
		...
		return
	if (gas.total_moles() > gas.maximum/2)
		icon_state = "inlet_filter-4"
	else if (gas.total_moles() > gas.maximum/3)
		icon_state = "inlet_filter-3"
	else if (gas.total_moles() > gas.maximum/4)
		icon_state = "inlet_filter-2"
	else if (gas.total_moles() >= 1 || f_mask >= 1)
