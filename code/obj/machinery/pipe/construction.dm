...
/obj/item/weapon/pipe/attackby(var/obj/item/weapon/W as obj, var/mob/user as mob)
	...
	if (istype(W, /obj/item/weapon/wrench))
		...
		var/pipedir = get_pdir()|get_hdir()		// all possible pipe dirs including h/e#

		for (var/obj/machinery/M in loc)
			if (M.level == level)		// only on same level
		...
		switch(pipe_type)
			...
			if (2,3)		// straight or bent h/e pipe
				P = new/obj/machinery/pipes/heat_exch( loc )
				P.h_dir = get_hdir()
			...
			if (4)		// connector
				var/obj/machinery/connector/C = new( loc )
				C.dir = dir
				C.p_dir = dir
				C.level = level
			...
			if (5)		//manifold
				var/obj/machinery/manifold/M = new( loc )
				M.dir = dir
				M.p_dir = pipedir
				M.level = level
				M.buildnodes()
			...
			if (6)		//junctions
				var/obj/machinery/junction/J = new( loc )
				J.dir = dir
				J.p_dir = get_pdir()
				J.h_dir = get_hdir()
				J.level = 2
			...
			if (7)		// vent
				var/obj/machinery/vent/V = new( loc )
				V.dir = dir
				V.p_dir = dir
				V.level = level
			...
			if (8)		//valve
				var/obj/machinery/valve/mvalve/V = new( loc)
				V.dir = dir
				switch(dir)
					if (1, 2)
						V.p_dir = 3
			...
			if (9)		//Pipe pump
				var/obj/machinery/oneway/pipepump/PP = new(loc)
			...
			if (10)		//filter inlet
				var/obj/machinery/inlet/filter/F = new(loc)
				F.dir = dir
				F.p_dir = dir
				F.level = level
...
/obj/item/weapon/pipe_meter/attackby(var/obj/item/weapon/W as obj, var/mob/user as mob)
	...
	if (istype(W, /obj/item/weapon/wrench))
		if (locate(/obj/machinery/pipes, loc))
...
/obj/item/weapon/filter_control/verb/set_control()
	set src in usr
	control = input("Enter the name of the control?.", "Control", "") as text
...
/obj/item/weapon/filter_control/proc/wall_place(turf/simulated/F, mob/user)
	...
	if (get_dist(F,user) > 1)
		...
		var/obj/machinery/filter_control/FC = new(loc)
		...
		switch(dirn)
			...
			if (SOUTHWEST)
				...
				boutput(world, "Invalid direction [dirn], [x], [y], [z]. Please report this.")
