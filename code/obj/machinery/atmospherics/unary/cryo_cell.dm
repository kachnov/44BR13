/obj/machinery/atmospherics/unary/cryo_cell
	...
	attack_hand(mob/user as mob)
		...
		if (occupant)
			if (occupant.stat == 2)
				...
				health_text = "<FONT color=red>[occupant.health]</FONT>"
			else
				health_text = "[occupant.health]"
		...
		if (beaker)
			beaker_text = "<strong>Beaker:</strong> <A href='?src=\ref[src];eject=1'>Eject</A>"
		else
			beaker_text = "<strong>Beaker:</strong> <FONT color=red>No beaker loaded</FONT>"
		var/dat = {"<strong>Cryo cell control system</strong><BR>
			<strong>Current cell temperature:</strong> [temp_text]K<BR>
			<strong>Cryo status:</strong> [ on ? "<A href='?src=\ref[src];start=1'>Off</A> <strong>On</strong>" : "<strong>Off</strong> <A href='?src=\ref[src];start=1'>On</A>"]<BR>
			[beaker_text]<BR><BR>
			<strong>Current occupant:</strong> [src.occupant ? "<BR>Name: [src.occupant]<BR>Health: [health_text]<BR>Oxygen deprivation: [src.occupant.get_oxygen_deprivation()]<BR>Brute damage: [src.occupant.get_brute_damage()]<BR>Fire damage: [src.occupant.get_burn_damage()]<BR>Toxin damage: [src.occupant.get_toxin_damage()]<BR>Body temperature: [src.occupant.bodytemperature]" : "<FONT color=red>None</FONT>"]<BR>
		"}
	...
	verb/move_inside()
		...
		if (usr.stat != 0 || stat & (NOPOWER|BROKEN))
			return
		if (occupant)
			boutput(usr, "<span style=\"color:blue\"><strong>The cell is already occupied!</strong></span>")
			return
		if (!node)
...
/mob/living/carbon/human/abiotic()
	if ((l_hand && !( l_hand.abstract )) || (r_hand && !( r_hand.abstract )) || (back || wear_mask || head || shoes || w_uniform || wear_suit || glasses || ears || gloves))
		return TRUE
	else
		return FALSE
	return
...
/mob/proc/abiotic()
	if ((l_hand && !( l_hand.abstract )) || (r_hand && !( r_hand.abstract )) || back || wear_mask)
		return TRUE
	else
		return FALSE
	return

/data/function/proc/reset()
	return

/data/function/proc/r_input(href, href_list, mob/user as mob)
	return

/data/function/proc/display()
	return