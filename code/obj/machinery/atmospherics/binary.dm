...
//
	var/gas_mixture/air1
	var/gas_mixture/air2

	var/obj/machinery/atmospherics/node1
	var/obj/machinery/atmospherics/node2

	var/pipe_network/network1
	var/pipe_network/network2
	...
	New()
		...
		switch(dir)
			...
			if (WEST)
				initialize_directions = EAST|WEST
		air1 = unpool(/gas_mixture)
		air2 = unpool(/gas_mixture)
...
// Housekeeping and pipe network stuff below
	...
	build_network()
		if (!network1 && node1)
			network1 = new /pipe_network()
			network1.normal_members += src
			network1.build_network(node1, src)

		if (!network2 && node2)
			network2 = new /pipe_network()
