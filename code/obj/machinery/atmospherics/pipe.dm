...
//
	...
	var/gas_mixture/air_temporary //used when reconstructing a pipeline that broke
	var/pipeline/parent
	...
	simple
		...
		process()
			...
			if (!node1)
				...
				if (!nodealert)
					//boutput(world, "Missing node from [src] at [x],[y],[z]")
					nodealert = 1
				...
				var/gas_mixture/gas = return_air()
				...
				if (pressure > 0)
					...
					var/gas_mixture/removed = gas.remove(transfer_moles)
				...
				var/gas_mixture/pipe_air = return_air()
		...
		attackby(var/obj/item/W as obj, var/mob/user as mob)
			if (istype(W, /obj/item/weldingtool) && W:welding)
				...
				W:eyecheck(user)
				boutput(user, "You start to repair the [name].")
				...
				if (do_after(user, 20))
					...
					return
				if (!ruptured)
					boutput(user, "You have fully repaired the [name].")
					icon_state = initial_icon_state
				else boutput(user, "You have partially repaired the [name].")
				return
	...
	tank
		...
		carbon_dioxide
			...
			New()
				air_temporary = unpool(/gas_mixture)
		...
		oxygen_agent_b
			...
			New()
				...
				var/gas/oxygen_agent_b/trace_gas = new
