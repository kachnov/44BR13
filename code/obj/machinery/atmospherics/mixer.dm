...
//
	...
	var/gas_mixture/air_in1
	var/gas_mixture/air_in2
	var/gas_mixture/air_out

	var/obj/machinery/atmospherics/node_in1
	var/obj/machinery/atmospherics/node_in2
	var/obj/machinery/atmospherics/node_out

	var/pipe_network/network_in1
	var/pipe_network/network_in2
	var/pipe_network/network_out
	...
	New()
		...
		air_in1 = unpool(/gas_mixture)
		air_in2 = unpool(/gas_mixture)
		air_out = unpool(/gas_mixture)
	...
	proc/report_status() // Report the status of this mixer over the radio.
		if (!(stat & (NOPOWER | BROKEN)))
			...
			signal.data["target_pressure"] = target_pressure
			if (on == 0)
			...
			if (i1total_moles > 0)
				...
				if (air_in1.trace_gases && air_in1.trace_gases.len)
					for (var/gas/trace_gas in air_in1.trace_gases)
			...
			if (i2total_moles > 0)
				...
				if (air_in2.trace_gases && air_in2.trace_gases.len)
					for (var/gas/trace_gas in air_in2.trace_gases)
						tgmoles += trace_gas.moles
			...
			if (ototal_moles > 0)
				...
				if (air_out.trace_gases && air_out.trace_gases.len)
					for (var/gas/trace_gas in air_out.trace_gases)
						tgmoles += trace_gas.moles
	...
	process()
		...
		if (transfer_moles1 > 0)
			var/gas_mixture/removed1 = air_in1.remove(transfer_moles1)
			air_out.merge(removed1)

		if (transfer_moles2 > 0)
			var/gas_mixture/removed2 = air_in2.remove(transfer_moles2)
