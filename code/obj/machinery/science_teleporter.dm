...
/obj/machinery/computer/science_teleport
	...
	proc/link_pad()
		if (!linked_pad)
			for (var/obj/machinery/science_teleport_pad/S in orange(8, loc))
				linked_pad = S
				break
	...
	attack_hand(var/mob/user as mob)
		...
		if (allow_bookmarks && bookmarks.len)
			...
			for (var/teleporter_bookmark/b in bookmarks)
		...
		user.machine = src
		user << browse("<TITLE>Teleport Computer</TITLE><strong>Target Coordinates</strong><BR>[dat]", "window=t_computer;size=400x600")
	...
	Topic(href, href_list)
		...
		if (href_list["restorebookmark"])
			var/teleporter_bookmark/bm = locate(href_list["restorebookmark"])
			if (!bm) return
			xtarget = bm.x
			ytarget = bm.y
			ztarget = bm.z
		...
		if (href_list["deletebookmark"])
			var/teleporter_bookmark/bm = locate(href_list["deletebookmark"])
			if (!bm) return
			bookmarks.Remove(bm)
			updateUsrDialog()
			return
		...
		if (href_list["addbookmark"])
			if (bookmarks.len >= max_bookmarks)
				...
				return
			var/teleporter_bookmark/bm = new
	...
	proc/send(var/turf/target)
		if (!target)
			...
			return
		var/list/stuff = list()
		for (var/atom/movable/O as obj|mob in linked_pad.loc)
			if (O.anchored) continue
			stuff.Add(O)
		if (stuff.len)
			var/atom/movable/which = pick(stuff)
			which.set_loc(target)
		showswirl(linked_pad.loc)
		showswirl(target)
		...
		/*if (prob(1) && !locate(/obj/dfissure_to) in get_step(linked_pad, EAST))
			new/obj/dfissure_to(get_step(linked_pad, EAST))*/ // why would anyone ever want to complete Hemera Station if spamming bad coords on this is so much easier
	...
	proc/doubleportal(var/turf/target)
		...
		if (prob(2))
			...
			ZSUBTRACT = rand(0,world.maxz)
			spawn (10) processbadeffect(pick("flash","buzz","scatter","ignite","chill"))
		if (prob(5) && !locate(/obj/dfissure_to) in get_step(linked_pad, EAST))
			new/obj/dfissure_to(get_step(linked_pad, EAST))
		else
			makeportal(linked_pad.loc, target)
			makeportal(target, linked_pad.loc)
	...
	proc/processbadeffect(var/effect)
		switch(effect)
			if ("")
				return
			if ("flash")
				for (var/mob/O in AIviewers(linked_pad, null)) O.show_message("<span style=\"color:red\">A bright flash emnates from the [linked_pad]!</span>", 1)
				playsound(linked_pad.loc, "sound/weapons/flashbang.ogg", 50, 1)
				for (var/mob/N in viewers(linked_pad, null))
					if (get_dist(N, linked_pad) <= 6)
						N.flash(30)
						N.weakened = max(N.weakened, 5)
					if (N.client) shake_camera(N, 6, 4)
				return
			if ("buzz")
				for (var/mob/O in AIviewers(linked_pad, null)) O.show_message("<span style=\"color:red\">You hear a loud buzz coming from the [linked_pad]!</span>", 1)
				playsound(linked_pad.loc, "sound/machines/buzz-sigh.ogg", 50, 1)
				return
			if ("scatter") //stolen from hand tele, heh
				var/list/turfs = new
				var/turf/target = null
				for (var/turf/T in orange(5,linked_pad.loc))
				...
				return
			if ("ignite")
				for (var/mob/living/carbon/M in linked_pad.loc)
					M.update_burning(30)
			...
			if ("sorium") // stolen from sorium, obviously
				explosion(src, linked_pad.loc, -1, -1, -1, 4)
				var/myturf = linked_pad.loc
				...
				return
			if ("rads")
				for (var/turf/T in view(5,linked_pad.loc))
					if (!T.reagents)
						var/reagents/R = new/reagents(1000)
						T.reagents = R
						R.my_atom = T
					T.reagents.add_reagent("radium", 20)
				for (var/mob/O in AIviewers(linked_pad, null)) O.show_message("<span style=\"color:red\">The area surrounding the [linked_pad] begins to glow bright green!</span>", 1)
				return
			if ("fire")
				fireflash(linked_pad.loc, 6) // cogwerks - lowered from 8, too laggy
				for (var/mob/O in AIviewers(linked_pad, null)) O.show_message("<span style=\"color:red\">A huge wave of fire explodes out from the [linked_pad]!</span>", 1)
				return
			if ("widescatter")
				var/list/turfs = new
				var/turf/target = null
				for (var/turf/T in orange(30,linked_pad.loc))
					if (T.x>world.maxx-4 || T.x<4)	continue
					if (T.y>world.maxy-4 || T.y<4)	continue
					if (is_allowed(T))
						turfs += T
				...
				return
			if ("brute")
				for (var/mob/living/M in linked_pad.loc)
			...
			if ("majormutate")
				...
				return
			if ("mutatearea")
				for (var/mob/living/carbon/M in orange(5,linked_pad.loc))
					M:bioHolder:RandomEffect("bad")
				for (var/mob/O in AIviewers(linked_pad, null)) O.show_message("<span style=\"color:red\">A bright green pulse emnates from the [linked_pad]!</span>", 1)
				return
			if ("explosion")
				explosion(src, linked_pad.loc, 0, 0, 5, 10)
				return
			if ("fullscatter")
				var/list/turfs = new
				var/turf/target = null
			...
			if ("minorsummon")
				...
				switch(summon)
					if ("pig")
						...
						P.set_loc(linked_pad.loc)
					if ("mouse")
						for (var/i=1,i<rand(1,3),i++)
							var/obj/critter/mouse/M = new /obj/critter/mouse
							M.set_loc(linked_pad.loc)
							i ++
					if ("roach")
						for (var/i=1,i<rand(3,8),i++)
							var/obj/critter/roach/R = new /obj/critter/roach
							R.set_loc(linked_pad.loc)
							i ++
				return
			if ("tinyfire")
				fireflash(linked_pad.loc, 3)
				for (var/mob/O in AIviewers(linked_pad, null)) O.show_message("<span style=\"color:red\">The area surrounding the [linked_pad] bursts into flame!</span>", 1)
				return
			if ("mediumsummon")
			...
			if ("areascatter")
				var/list/turfs = new
				var/turf/target = null
				for (var/turf/T in orange(10,linked_pad.loc))
					if (T.x>world.maxx-4 || T.x<4)	continue
					if (T.y>world.maxy-4 || T.y<4)	continue
					if (is_allowed(T))
						turfs += T
				if (turfs && turfs.len)
					for (var/atom/movable/O as obj|mob in oview(linked_pad,5))
						if (O.anchored) continue
						target = pick(turfs)
						if (target) O.set_loc(target)
				qdel(turfs)
