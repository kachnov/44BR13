/obj/machinery/cashreg
	...
	anchored = 1
	mats = 6
	var/data/record/mainaccount = null
	...
	attackby(obj/item/W as obj, mob/user as mob)
		...
		if (istype(W, /obj/item/card/id))
			...
			user.visible_message("<strong>[src]</strong> beeps, \"[mainaccount.fields["name"]] now holds [mainaccount.fields["current_money"]] credits. Thank you for your service!\"")
	...
	attack_hand(mob/user as mob)
		...
		switch(alert("Reset the reader?",,"Yes","No"))
			if ("Yes")
				...
				user.visible_message("<span style=\"color:red\"><strong>[user]</strong> resets [src].</span>")
				mainaccount = null
			if ("No")
				return