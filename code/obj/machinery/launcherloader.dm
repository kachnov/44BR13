	...
	New()
		...
		spawn (5)
			...
			if (drivers.len)
				...
				dir = get_dir(src,driver)

	proc/activate()
		if (operating || !isturf(loc)) return
		operating = 1
...
/obj/machinery/cargo_router
	...
	proc/activate()
		...
		if (next_dest) dir = next_dest
		else dir = default_direction

		flick("amdl_1",src)
		playsound(src, "sound/effects/pump.ogg",50, 1)
		sleep(3)

		for (var/atom/movable/AM2 in loc)
			if (AM2.anchored || AM2 == src) continue
			step(AM2,dir)

		driver = (locate(/obj/machinery/mass_driver) in get_step(src,dir))
...
/obj/machinery/computer/barcode
	...
	attack_hand(var/mob/user as mob)
		...
		var/dat = ""
		dat += "<strong>Available Destinations:</strong><BR>"
		for (var/I in destinations)
			dat += "<strong><A href='?src=\ref[src];print=[I]'>[I]</A></strong><BR>"

		dat += "<BR><strong><A href='?src=\ref[src];add=1'>Add Tag</A></strong>"
	...
	Topic(href, href_list)
		...
		if (href_list["print"] && !printing)
			printing = 1
			playsound(loc, "sound/machines/printer_thermal.ogg", 50, 0)
			sleep(28)
			var/obj/item/barcodesticker/B = new/obj/item/barcodesticker(loc)
...
/obj/machinery/computer/barcode/qm //has trader tags if there is one
	...
	attack_hand(var/mob/user as mob)
		...
		dat += "<strong>Available Destinations:</strong><BR>"
		for (var/I in destinations)
			dat += "<strong><A href='?src=\ref[src];print=[I]'>[I]</A></strong><BR><BR>"

		dat += "<strong>Available Traders:</strong><BR>"
		for (var/trader/T in shippingmarket.active_traders)
			if (!T.hidden)
				dat += "<br><strong><A href='?src=\ref[src];print=[T.crate_tag]'>Sell to [T.name]</A></strong><BR>"
...
/obj/item/barcodesticker
	...
	proc/attachTo(atom/target)
		if (get_dist(get_turf(target), get_turf(src)) <= 1 && istype(target, /atom/movable))
			...
			usr.visible_message("<span style=\"color:blue\">[usr] puts a [name] on [target].</span>")
