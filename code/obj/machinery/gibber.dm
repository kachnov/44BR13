...
/obj/machinery/gibber/New()
	..()
	overlays += image('icons/obj/kitchen.dmi', "grindnotinuse")
	UnsubscribeProcess()

/obj/machinery/gibber/suicide(var/mob/user as mob)
	if (user.client)
		user.visible_message("<span style=\"color:red\"><strong>[user] climbs into the gibber and switches it on.</strong></span>")
		user.set_loc(src)
		occupant = user
		startgibbing(user)
		return TRUE
...
/obj/machinery/gibber/proc/startgibbing(mob/user as mob)
	...
	if (!occupant)
		...
		if (occupant.mind && occupant.mind.assigned_role)
			sourcejob = occupant.mind.assigned_role
		else if (occupant.ghost && occupant.ghost.mind && occupant.ghost.mind.assigned_role)
			sourcejob = occupant.ghost.mind.assigned_role
		else
			sourcejob = "Stowaway"

		var/decomp = 0
		if (ishuman(occupant))
			decomp = occupant:decomp_stage

			bdna = occupant.bioHolder.Uid // Ditto (Convair880).
			btype = occupant.bioHolder.bloodType

		if (user != occupant) //for suiciding with gibber
			logTheThing("combat", user, occupant, "grinds %target% in a gibber at [log_loc(src)].")
			message_admins("[key_name(occupant, 1)] is ground up in a gibber by [key_name(user)] at [log_loc(src)].")
		occupant.death(1)
		...
		spawn (gibtime)
			...
			if (dirty == 1)
				overlays += image('icons/obj/kitchen.dmi', "grindbloody")
