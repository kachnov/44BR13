...
/obj/machinery/alarm/process()
	...
	icon_state = "alarm[!safe]"

	if (safe == 2) skipprocess = 1
...
/obj/machinery/alarm/attackby(W as obj, user as mob)
	if (istype(W, /obj/item/wirecutters))
		...
		visible_message("<span style=\"color:red\">[user] has [(stat & BROKEN) ? "de" : "re"]activated [src]!</span>")
		return
...
/obj/machinery/alarm/proc/return_text(mob/user)
	...
	var/output = "<strong>[name] Interface: </strong><BR><HR>"
	if (!istype(environment))
		...
		output += "<strong>Composition:</strong><BR>"
	...
	if (locked && (!istype(user, /mob/living/silicon)))
		output += "<em><FONT color = 'gray'>No atmospheric pipe network detected.<BR>Control functions unavailable.</FONT></em>"
	else
		if (!istype(user, /mob/living/silicon))
			output += "<em>Swipe card to lock interface.</em><BR><BR>"
...
/obj/machinery/alarm/Topic(href, href_list)
	...
	if (href_list["toggle_override"])
		var/radio_frequency/frequency = radio_controller.return_frequency(control_frequency)
