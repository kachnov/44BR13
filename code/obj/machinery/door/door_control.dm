...
/obj/machinery/door_control/oneshot/attack_hand(mob/user as mob)
	...
	if (!(stat & BROKEN))
		stat |= BROKEN
		visible_message("<span style=\"color:red\">[src] emits a sad thunk.  That can't be good.</span>")
		playsound(loc, "sound/effects/thunk.ogg", 50, 1)
	else
...
/obj/machinery/r_door_control
		...
		if (get_dist(usr, src) < 16)
			if (istype(usr.loc, /obj/machinery/vehicle))
				...
				if (!access_type)
					...
					if (V.com_system.access_type == access_type)
						open_door()
					else
	...
	attack_hand(mob/user as mob)
		boutput(user, "<span style=\"color:blue\">The password is \[[pass]\]</span>")
		return
	...
	receive_signal(signal/signal)
		...
		if (signal.data["command"] =="open door")
			if (!signal.data["doorpass"])
				return
			if (!signal.data["access_type"] || signal.data["access_type"] != access_type)
				return

			if (signal.data["doorpass"] == pass)
				if (stat & (NOPOWER|BROKEN))
					return
				use_power(5)
