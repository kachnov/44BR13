...
//			return
	...
	proc/interact(var/mob/user, var/ai = 0)
		...
		if (locked && !ai)
			t = {"<TT><strong>Navigation Beacon</strong><HR><BR>
<em>(swipe card to unlock controls)</em><BR>
...
Transponder Codes:<UL>"}
			...
			t = {"<TT><strong>Navigation Beacon</strong><HR><BR>
<em>(swipe card to lock controls)</em><BR>
Frequency:
...
/obj/machinery/wirenav
	...
	hide(var/intact)
		invisibility = intact ? 101 : 0
		icon_state = "wirednav[invisibility ? "-f" : ""]"
	...
	receive_signal(signal/signal)
		...
		if ((signal.data["address_1"] in list(net_id, "ping")) && sender)
			...
			reply.data["data"] = nav_tag
			reply.data["navdat"] = "x=[x]&y=[y]&z=[z]"
			spawn (5)
