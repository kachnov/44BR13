...
/obj/stool/attackby(obj/item/W as obj, mob/user as mob)
	if (istype(W, /obj/item/wrench) && deconstructable)
		playsound(loc, "sound/items/Ratchet.ogg", 50, 1)
		var/atom/C = new /obj/item/sheet(loc)
		if (material)
...
/obj/stool/proc/unbuckle() //Ditto but for unbuckling
	return
...
/obj/stool/bench
	...
	proc/set_up(var/setup_others = 0)
		if (!auto || !ispath(auto_path))
			return
		var/dirs = 0
		for (var/dir in cardinal)
			var/turf/T = get_step(src, dir)
			if (locate(auto_path) in T)
				dirs |= dir
		icon_state = num2text(dirs)
		if (setup_others)
			for (var/obj/stool/bench/B in orange(1))
				if (istype(B, auto_path))
					B.set_up()
...
/obj/stool/bed
	...
	Move()
		..()
		if (buckled_guy)
			var/mob/living/carbon/C = buckled_guy
			C.buckled = null
			C.Move(loc)
			C.buckled = src
	...
	attackby(obj/item/W as obj, mob/user as mob)
		...
		if (istype(W, /obj/item/wrench))
			if (security)
				...
				playsound(loc, "sound/items/Ratchet.ogg", 50, 1)
				var/obj/item/sheet/C = new /obj/item/sheet( loc )
				if (material)
					C.setMaterial(material)
				else
					var/material/M = getCachedMaterial("steel")
	...
	proc/buckle_mob(var/mob/living/carbon/C as mob, var/mob/user as mob)
		...
		if (get_dist(src, user) > 1)
			...
			return
		if ((!(iscarbon(C)) || C.loc != loc || user.restrained() || user.stat))
			return

		if (C == user)
			user.visible_message("<span style=\"color:blue\"><strong>[C]</strong> buckles in!</span>", "<span style=\"color:blue\">You buckle yourself in.</span>")
		else
			user.visible_message("<span style=\"color:blue\"><strong>[C]</strong> is buckled in by [user].</span>", "<span style=\"color:blue\">You buckle in [C].</span>")
		buckle_in(C)
	...
	proc/unbuckle_mob(var/mob/M as mob, var/mob/user as mob)
		if (M.buckled && !user.restrained())
			if (allow_unbuckle)
				if (M != user)
					user.visible_message("<span style=\"color:blue\"><strong>[M]</strong> is unbuckled by [user].</span>", "<span style=\"color:blue\">You unbuckle [M].</span>")
				else
					user.visible_message("<span style=\"color:blue\"><strong>[M]</strong> unbuckles.</span>", "<span style=\"color:blue\">You unbuckle.</span>")
				unbuckle()
			else
	...
	buckle_in(mob/living/to_buckle)
		...
		buckled_guy = to_buckle
		to_buckle.set_loc(loc)
	...
	proc/tuck_sheet(var/obj/item/clothing/suit/bedsheet/newSheet as obj, var/mob/user as mob)
		...
		if (Sheet && Sheet.loc != loc) // a safety check: do we have a sheet and is it not where we are?
			if (Sheet.Bed && Sheet.Bed == src) // does our sheet have us listed as its bed?
				Sheet.Bed = null // set its bed to null
			Sheet = null // then set our sheet to null: it's not where we are!

		if (Sheet && Sheet != newSheet) // do we have a sheet, and is the new sheet we've been given not our sheet?
			user.show_text("You try to kinda cram [newSheet] into the edges of [src], but there's not enough room with [Sheet] tucked in already!", "red")
			return // they're crappy beds, okay?  there's not enough space!

		if (!Sheet && (newSheet.loc == loc || user.find_in_hand(newSheet))) // finally, do we have room for the new sheet, and is the sheet where we are or in the hand of the user?
			Sheet = newSheet // let's get this shit DONE!
			newSheet.Bed = src
			user.u_equip(newSheet)
			newSheet.set_loc(loc)
			mutual_attach(src, newSheet)

			var/mob/somebody
			if (buckled_guy)
				somebody = buckled_guy
			else
				somebody = locate(/mob/living/carbon) in get_turf(src)
			if (somebody && somebody.lying)
				user.tri_message("<span style=\"color:blue\"><strong>[user]</strong> tucks [somebody == user ? "[him_or_her(user)]self" : "[somebody]"] into bed.</span>",\
				user, "<span style=\"color:blue\">You tuck [somebody == user ? "yourself" : "[somebody]"] into bed.</span>",\
				somebody, "<span style=\"color:blue\">[somebody == user ? "You tuck yourself" : "<strong>[user]</strong> tucks you"] into bed.</span>")
				newSheet.layer = EFFECTS_LAYER_BASE-1
				return
			else
				user.visible_message("<span style=\"color:blue\"><strong>[user]</strong> tucks [newSheet] into [src].</span>",\
	...
	proc/untuck_sheet(var/mob/user as mob)
		...
		var/obj/item/clothing/suit/bedsheet/oldSheet = Sheet
		...
		if (user)
			...
			if (buckled_guy)
				...
				somebody = locate(/mob/living/carbon) in get_turf(src)
			if (somebody && somebody.lying)
				user.tri_message("<span style=\"color:blue\"><strong>[user]</strong> untucks [somebody == user ? "[him_or_her(user)]self" : "[somebody]"] from bed.</span>",\
				user, "<span style=\"color:blue\">You untuck [somebody == user ? "yourself" : "[somebody]"] from bed.</span>",\
				somebody, "<span style=\"color:blue\">[somebody == user ? "You untuck yourself" : "<strong>[user]</strong> untucks you"] from bed.</span>")
				oldSheet.layer = initial(oldSheet.layer)
			else
				user.visible_message("<span style=\"color:blue\"><strong>[user]</strong> untucks [oldSheet] from [src].</span>",\
	...
	MouseDrop_T(atom/A as mob|obj, mob/user as mob)
		...
		if (istype(A, /obj/item/clothing/suit/bedsheet))
			if ((!Sheet || (Sheet && Sheet.loc != loc)) && A.loc == loc)
				tuck_sheet(A, user)
				return
			if (Sheet && A == Sheet)
				untuck_sheet(user)
	...
	verb/rest_in()
		...
		if ((get_turf(user) != loc) || (!user.lying))
...
/obj/stool/chair/attackby(obj/item/W as obj, mob/user as mob)
	...
	if (istype(W, /obj/item/assembly/shock_kit))
		var/obj/stool/chair/e_chair/E = new /obj/stool/chair/e_chair(loc)
		if (material)
...
/obj/stool/chair/attack_hand(mob/user as mob)
	...
	for (var/mob/M in loc)
		...
		if (H.on_chair == 1)
			if (M == user)
				user.visible_message("<span style=\"color:blue\"><strong>[M]</strong> steps off [src].</span>", "<span style=\"color:blue\">You step off [src].</span>")
				add_fingerprint(user)
				unbuckle()
				return
	...
	if (!buckledIn && foldable)
		user.visible_message("<strong>[user.name] folds [src].</strong>")
		if ((chump) && (chump != user))
			chump.visible_message("<span style=\"color:red\"><strong>[chump.name] falls off of [src]!</strong></span>")
			chump.on_chair = 0
			chump.pixel_y = 0
			chump.weakened++
			chump.stunned+=2
		...
		var/obj/item/chair/folded/C = new/obj/item/chair/folded(loc)
		if (material)
			C.setMaterial(material)
		if (icon_state)
			C.c_color = icon_state
...
/obj/stool/chair/MouseDrop_T(mob/M as mob, mob/user as mob)
	...
	if (M == usr)
		if (usr.a_intent == INTENT_GRAB)
			user.visible_message("<span style=\"color:blue\"><strong>[M]</strong> climbs up on [src]!</span>", "<span style=\"color:blue\">You climb up on [src].</span>")
			buckle_in(M, 1)
		else
			user.visible_message("<span style=\"color:blue\"><strong>[M]</strong> buckles in!</span>", "<span style=\"color:blue\">You buckle yourself in.</span>")
			buckle_in(M)
	else
		user.visible_message("<span style=\"color:blue\"><strong>[M]</strong> is buckled in by [user].</span>", "<span style=\"color:blue\">You buckle in [M].</span>")
		buckle_in(M)

/obj/stool/chair/buckle_in(mob/living/to_buckle, var/stand = 0)
	if (!istype(to_buckle)) 
		return
...
/obj/stool/chair/unbuckle()
	if (!buckled_guy) 
		return

	var/mob/living/M = buckled_guy
	var/mob/living/carbon/human/H = buckled_guy
...
/obj/stool/chair/blob_act(var/power)
	if (prob(power * 2.5))
		for (var/mob/M in loc)
			if (M.buckled == src)
				M.buckled = null
...
/obj/stool/chair/lawnmower
	icon = 'icons/obj/lawnmower.dmi'
	icon_state = ""
	foldable = FALSE
	var/moving = FALSE
	var/last_move_was_northsouth = FALSE
...
/obj/stool/chair/lawnmower/proc/_Move()
	...
	// alternate between NORTH/SOUTH and EAST/WEST turns
	var/list/steps = list(get_step(src, NORTH), get_step(src, SOUTH), get_step(src, EAST), get_step(src, WEST))

	if (last_move_was_northsouth)
		steps = reverse_list(steps)

	steps.Insert(1, get_step(src, dir))

	for (var/turf/T in steps)
		var/test = locate(/obj/chair_path_helper) in T
		if (test)

			var/dirto = get_dir(src, T)

			var/obj/stool/chair/lawnmower/_1 = locate(type) in T
			var/obj/stool/chair/lawnmower/_2 = locate(type) in get_step(T, dirto)
			var/obj/stool/chair/lawnmower/_3 = locate(type) in get_step(get_step(T, dirto), dirto)

			var/mowercheck = _1 || (_2 && !_2.moving) || (_3 && !_3.moving)
			...
			if (!istype(test, /obj/chair_path_helper/stop) && !mowercheck)
				...
				for (var/mob/living/L in T)
					if (L != buckled_guy)
						visible_message("<span style = \"color:red\"><strong>[L] is [pick("DESTROYED", "CRUSHED", "ANNIHILATED")] by the lawnmower.</strong></span>")
						if (ishuman(L))
							L.emote("scream")
						L.gib()
			else
...
/obj/stool/chair/syndicate
	...
	HasProximity(atom/movable/AM as mob|obj)
		if (ishuman(AM) && prob(40))
			visible_message("<span style=\"color:red\">[src] trips [AM]!</span>", "<span style=\"color:red\">You hear someone fall</span>")
			AM:weakened += 2
		return
...
/* what in the unholy mother of god was this about
		overl = new /atom/movable/overlay( loc )
		overl.icon = 'icons/obj/objects.dmi'
		overl.icon_state = "arm"
		overl.layer = 6// TODO Layer wtf
		overl.name = "chair arm"
		overl.master = src
		overl.dir = dir
*/
	rotate()
...
//		overl.dir = dir
	...
	proc/update_icon()
		if (dir == NORTH)
			...
			layer = OBJ_LAYER
			if ((dir == WEST || dir == EAST) && !arm_image)
				arm_image = image(icon, arm_icon_state)
				arm_image.layer = FLY_LAYER+1
				UpdateOverlays(arm_image, "arm")
...
/obj/stool/chair/e_chair
		...
		spawn (20)
			if (src)
				if (!(part1 && istype(part1)))
					part1 = new /obj/item/assembly/shock_kit(src)
					part1.master = src
				update_icon()
		return
	...
	proc/control_interface(mob/user as mob)
		if (!user.handcuffed && user.stat == 0)
			...
			if (!isarea(A) || !A.powered(EQUIP))
				dat += "\n<font color='red'>ERROR:</font> No power source detected!</strong>"
			else
			...
			user << browse("<TITLE>Electric Chair</TITLE><strong>Electric Chair</strong><BR>[dat]", "window=e_chair;size=180x180")
	...
	Topic(href, href_list)
		...
		if (href_list["on"])
			...
			if (buckled_guy)
				...
				logTheThing("combat", usr, buckled_guy, "activated an electric chair (setting: [lethal ? "lethal" : "non-lethal"]), shocking %target% at [log_loc(src)].")
			shock(lethal)
	...
	proc/update_icon()
		icon_state = "e_chair[on]"
		if (!image_belt)
			image_belt = image(icon, "e_chairo[on][lethal]", layer = FLY_LAYER + 1)
			UpdateOverlays(image_belt, "belts")
			return
		image_belt.icon_state = "e_chairo[on][lethal]"
		UpdateOverlays(image_belt, "belts")
	...
	proc/shock(lethal)
		...
		for (var/mob/M in AIviewers(src, null))
			...
			if (lethal)
				playsound(loc, "sound/effects/electric_shock.ogg", 100, 0)
			else
				playsound(loc, "sound/effects/sparks4.ogg", 100, 0)

		if (buckled_guy && ishuman(buckled_guy))
			var/mob/living/carbon/human/H = buckled_guy

			if (lethal)
				var/net = get_connection() // Are we wired-powered (Convair880)?
				var/power = get_gridpower()
				if (!net || (net && (power < 2000000)))
					...
					electrocute(H, 100, net, 1) // We are, great. Let that global proc calculate the damage.
			else
