...
/obj/displaycase/attack_hand(mob/user as mob)
	if (destroyed && occupied)
		new /obj/item/captaingun( loc )
		boutput(user, "<strong>You deactivate the hover field built into the case.</strong>")
		occupied = 0
...
/obj/item/captaingun
	...
	var/projectile/our_projectile = null
	var/projectile/our_projectile2 = null // Reserved for top-notch replacement parts.
	...
	*/
	attackby(obj/item/O as obj, mob/user as mob)
		if (istype(O, /obj/item/screwdriver))
			...
			else if (repair_stage == 7)
				user.visible_message("<span style=\"color:blue\">[user] finishes repairing the [name].</span>", "<span style=\"color:blue\">You close the maintenance panel and power up the gun.</span>")

				generate_properties(user) // Type of projectile(s) based on material quality.
				var/obj/item/gun/energy/laser_gun/antique/L = new /obj/item/gun/energy/laser_gun/antique(get_turf(user))
				L.current_projectile = new our_projectile
				if (!isnull(our_projectile2))
					our_projectiles = list(new our_projectile, new our_projectile2)
					L.projectiles = our_projectiles
				our_cell.set_loc(L)
				L.cell = our_cell

				// The man with the golden gun.
				if (quality_counter >= q_threshold2)
			...
			if (repair_stage == 1)
				...
				if (C.amount >= 10)
					...
					if (do_after(user, 35))
						...
						repair_stage = 2
						if (C.material)
							quality_counter += C.material.quality
					else
			...
			if (repair_stage == 2)
				...
				if (do_after(user, 35))
					...
					repair_stage = 3
					if (O.material)
						quality_counter += O.material.quality
					user.u_equip(O)
					qdel(O)
				else
					user.show_text("You were interrupted!", "red")
			...
			if (repair_stage == 6)
				...
				if (do_after(user, 35))
					...
					our_cell = P
					if (P.material)
						quality_counter += P.material.quality
				else
					user.show_text("You were interrupted!", "red")
					return
	...
	proc/generate_properties(var/mob/user)
		...
		if (quality_counter < q_threshold1)
			our_projectile = /projectile/laser
			if (user && ismob(user))
				user.show_text("The [name] looks a little worn, but appears to work alright.", "blue")

		// Player put some effort into it, so let's give him something a little more powerful.
		else if (quality_counter >= q_threshold1 && quality_counter < q_threshold2)
			if (user && ismob(user))
				user.show_text("The [name] seems to work better than expected thanks to above-average replacment parts.", "blue")
			our_projectile = /projectile/laser/old

		// Now we're talking about top-notch stuff.
		else if (quality_counter >= q_threshold2)
			if (user && ismob(user))
				user.show_text("The [name]'s high-quality replacement parts fit together perfectly, increasing the gun's output.", "blue")
			our_projectile = /projectile/laser/old
			our_projectile2 = /projectile/laser/old_burst

		//DEBUG("[name]'s quality_counter: [quality_counter]")
		return