...
/obj/storage/secure/closet/brig/automatic
		...
		spawn (5)
			if (src)
				...
				for (var/obj/machinery/door_timer/DT in range(30, src))
					if (DT && DT.id == id)
						our_timer = DT
						if (name == "\improper Automatic Locker")
							name = "\improper Automatic Locker ([id])"
						break
				if (!our_timer)
					message_admins("Automatic locker: couldn't find brig timer with ID [isnull(id) ? "*null*" : "[id]"] in [get_area(src)].")
					logTheThing("debug", null, null, "<strong>Convair880:</strong> couldn't find brig timer with ID [isnull(id) ? "*null*" : "[id]"] for automatic locker at [log_loc(src)].")
		return
	...
	MouseDrop(over_object, src_location, over_location)
		...
		if (alert("Link locker to this brig timer?",,"Yes","No") == "Yes")
			...
			our_timer = DT
			name = "\improper Automatic Locker ([id])"
			usr.visible_message("<span style=\"color:blue\"><strong>[usr.name]</strong> links [name] to a brig timer.</span>", "<span style=\"color:blue\">Brig timer linked: [id].</span>")
		return
...
/obj/item/paper/blood_fridge_note
	...
	info = "This fridge is for BLOOD PACKS <u>ONLY</u>! If I ever catch the idiot who keeps leaving their lunch in here, you're taking a one-way trip to the goddamn solarium!<br><br><em>L. Alliman</em><br>"
