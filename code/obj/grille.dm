/obj/grille
	...
	damage_blunt(var/amount)
		...
		if (material)
			...
			if (material.quality >= 25)
				armor += material.quality * 0.25
			else if (quality < 10)
				armor = 0
				amount += rand(1,3)
	...
	attack_hand(obj/M, mob/user)
		if (!shock(usr, 70))
			...
			visible_message("<span style=\"color:red\"><strong>[usr]</strong> [text]!</span>")
	...
	attackby(obj/item/W, mob/user)
		...
		if (istype(W,/obj/item/device/multitool) || istype(W, /obj/item/device/t_scanner))
			...
			if (S.material && S.material.material_flags & MATERIAL_CRYSTAL)
				...
				if (WI && istype(WI))
					...
					user.show_text("<strong>Error:</strong> Couldn't spawn window. Try again and please inform a coder if the problem persists.", "red")
					return
		...
		if (istype(W, /obj/item/wirecutters))
			damage_slashing(health_max)
			visible_message("<span style=\"color:red\"><strong>[usr]</strong> cuts apart the [src] with [W].</span>")
			playsound(loc, 'sound/items/Wirecutter.ogg', 100, 1)

		else if ((istype(W, /obj/item/screwdriver) && (istype(loc, /turf/simulated) || anchored)))
			playsound(loc, 'sound/items/Screwdriver.ogg', 100, 1)
			anchored = !( anchored )
			visible_message("<span style=\"color:red\"><strong>[usr]</strong> [anchored ? "fastens" : "unfastens"] [src].</span>")
			return

		else
			visible_message("<span style=\"color:red\"><strong>[usr]</strong> attacks [src] with [W].</span>")
			playsound(loc, 'sound/effects/grillehit.ogg', 80, 1)
			switch(W.damtype)
				if ("fire")
					damage_heat(W.force)
	...
	proc/shock(mob/user, prb, var/ignore_gloves = 0)
		...
		return electrocute(user, prb, net, ignore_gloves)
		...
		if (shock_when_entered)
			if (ismob(AM))
				...
				if (shock(M, s_chance, rand(0,1))) // you get a 50/50 shot to accidentally touch the grille with something other than your hands
					M.show_text("<strong>You brush against [src] while moving past it and it shocks you!</strong>", "red")
