...
/obj/item/lens
	...
	onMaterialChanged()
		..()
		if (istype(material))
			clarity = 80 + initial(clarity) - ((material.alpha / 255) * 100)
...
/obj/item/c_tube
	...
	suicide(var/mob/user as mob)
		user.visible_message("<span style=\"color:red\"><strong>[user] attempts to beat \himself to death with the cardboard tube, but fails!</strong></span>")
		user.suiciding = 0
		return TRUE
...
/obj/item/saxophone/attack_self(mob/user as mob)
	if (spam_flag == 0)
		...
		user.visible_message("<strong>[user]</strong> lays down a [pick("sexy", "sensuous", "libidinous","spicy","flirtatious","salacious","sizzling","carnal","hedonistic")] riff on \his saxophone!")
		playsound(get_turf(src), pick(sounds_sax), 50, 1)
...
/obj/item/bagpipe/attack_self(mob/user as mob)
	if (spam_flag == 0)
		...
		if (ishuman(user))
			...
			if (H.sims)
				H.sims.affectMotive("fun", 5)
		user.visible_message("<strong>[user]</strong> plays a [pick("patriotic", "rowdy", "wee","grand","free","Glaswegian","sizzling","carnal","hedonistic")] tune on \his bagpipe!")
		playsound(get_turf(src), pick(sounds_bagpipe), 50, 1)
		for (var/obj/critter/dog/george/G in range(user,6))
			if (prob(60))
				G.howl()
		add_fingerprint(user)
...
/obj/item/fiddle
	...
	attack_self(mob/user as mob) //Charge it by FIDDLING IN A SICK MANNER
		if (!fiddling)
			fiddling++
			user.visible_message("<strong>[user]</strong> lays down a [pick("devilish","hellish","satanic", "enviable")] tune on \his fiddle!")
			//playsound(loc, pick(sounds_fiddle), 50, 1)
			for (var/obj/critter/dog/george/G in range(user,6))
				if (prob(60))
					G.howl()
			add_fingerprint(user)
	...
	attack_self(mob/user as mob)
		if (spam_flag == 1)
			...
			if (ishuman(user))
				...
				if (H.sims)
					H.sims.affectMotive("fun", 5)
			user.visible_message("<strong>[user]</strong> plays a [pick("slick", "egotistical", "snazzy", "technical", "impressive")] [pick("riff", "jam", "bar", "tune")] on \his trumpet!")
//			playsound(loc, pick(sounds_trumpet), 50, 1)
			for (var/obj/critter/dog/george/G in range(user,6))
				if (prob(60))
					G.howl()
			add_fingerprint(user)
...
/obj/item/trumpet/dootdoot/proc/dootize(var/mob/living/carbon/human/S)
	if (istype(S.mutantrace, /mutantrace/skeleton))
		S.visible_message("<span style=\"color:blue\"><strong>[S.name]</strong> claks in appreciation!</span>")
		playsound(S.loc, "sound/items/Scissor.ogg", 50, 0)
		return
	else
		S.visible_message("<span style=\"color:red\"><strong>[S.name]'s skeleton rips itself free upon hearing the song of its people!</strong></span>")
		if (S.gender == "female")
		...
		S.set_mutantrace(/mutantrace/skeleton)
...
/obj/item/trumpet/dootdoot/attack_self(var/mob/living/carbon/human/user as mob)
	if (spam_flag == 1)
		...
		user.visible_message("<strong>[user]</strong> doots a [pick("spooky", "scary", "boney", "creepy", "squawking", "squeaky", "low-quality", "compressed")] tune on \his trumpet!")
	...
	attack(mob/M as mob, mob/user as mob, def_zone)
		...
		if (user == M)
			...
			for (var/mob/O in viewers(user, null))
				O.show_message(text("<strong>[]</strong> takes a reading with the [].", user, src), 1)
			return
		else
...
/obj/head_surgeon
	...
	New()
		..()
		if (prob(50))
			new /obj/machinery/bot/medbot/head_surgeon(loc)
			qdel(src)
...
/obj/item/rubber_hammer
	...
	attack(mob/M as mob, mob/user as mob)
		...
		for (var/mob/O in viewers(M))
			if (O.client)	O.show_message("<span style=\"color:red\"><strong>[user] bonks [M] on the head with the rubber hammer!</strong></span>", 1, "<span style=\"color:red\">You hear something squeak.</span>", 2)