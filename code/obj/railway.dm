	...
	set_loc(var/dest)
		...
		if (istype(from_t) && istype(dest_t))
			...
			for (var/mob/living/M in dest_t)
				M.TakeDamage("chest", road_rage_force, 0)
				M.visible_message("<span style=\"color:red\"><strong>[M] was hit by [src]!</strong></span>", "<span style=\"color:red\"><strong>You were hit by [src]!</strong></span>")
			...
			for (var/obj/O in dest_t)
				...
				playsound(loc, "sound/misc/meteorimpact.ogg", 40, 1)
				if (O.anchored && magically_destructive)
					visible_message("<span style=\"color:red\"><strong>[src] crashes into [O].</strong></span>")
					qdel(O)
				else if (!O.anchored)
					spawn
						visible_message("<span style=\"color:red\"><strong>[O] was hit by [src]!</strong></span>")
		...
		on_trigger()
			if (contents.len > 0)
				...
				for (var/atom/movable/AM in src)
					visible_message("<span style='color:blue'><strong>[src] dumps out [AM].</strong></span>")
					AM.set_loc(T)
