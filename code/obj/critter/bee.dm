...
/* -------------------- BEES -------------------- */
	...
	queen
		...
		ChaseAttack(mob/M)
			...
			if (M.stat || M.paralysis)
				task = "thinking"
				return
			visible_message("<span style=\"color:red\"><strong>[src]</strong> pokes [M] with its [prob(50) ? "IMMENSE" : "COLOSSAL"] stinger!</span>")
			...
			if (isliving(M))
				var/mob/living/L = M
				var/ailment_data/disease/aids = L.find_ailment_by_type(/ailment/disease/space_aids)
				if (istype(aids,/ailment_data/disease))
		...
		CritterAttack(mob/M)
			...
			if ((M.loc != src) && ((issilicon(M) && prob(20)) || prob(5)))
				visible_message("<span style=\"color:red\"><strong>[src]</strong> swallows [M] whole!</span>")
				M.set_loc(src)
				spawn (20)
					...
					honeycube.set_loc(loc)
					visible_message("<strong>[src]</strong> regurgitates [M]!")
			...
			visible_message("<span style=\"color:red\"><strong>[src]</strong> bites [M] with its [pick("rather large","big","expansive","proportionally small but still sizable")] [prob(50) ? "mandibles" : "bee-teeth"]!</span>")
	...
	heisenbee
		...
		New()
			...
			if (masked)
				if (prob(50))
					...
					overlays += image(icon, "halloweenmask")
				else
					overlays += image(icon, "halloweenmask2")
		...
		attack_hand(mob/user as mob)
			if (alive)
				if (user.a_intent == INTENT_HELP)
					visible_message("<span style=\"color:blue\"><strong>[user]</strong> [pick("pets","hugs","snuggles","cuddles")] [src]!</span>")

					if (masked == 1)
						visible_message("<span style=\"color:red\">[src]'s halloween mask falls off!<br>[src] stares at the fallen mask for a moment, then buzzes wearily.</span>")
						masked = 0
						overlays = list()
		...
		attackby(obj/item/W as obj, mob/living/user as mob)
			...
			if (istype(W, /obj/item/device/gps))
				...
				spawn (rand(10,20))
					visible_message("<span style=\"color:red\"><strong>[src] begins to move at unpredicable speeds!</strong></span>")
					animate_bumble(src, floatspeed = 3)
					sleep(rand(30,50))
					visible_message("<span style=\"color:red\">[W] goes flying!</span>")
					if (W)
						W.set_loc(loc)
					...
					animate_bumble(src)
					visible_message("<strong>[src]</strong> gives off a dizzy buzz.")

			else if (istype(W, /obj/item/photo/heisenbee))
				user.visible_message("[user] shows [src] the [W.name].","You show [src] the [W.name].")
				visible_message("[src] bumbles in a slightly embarrassed manner.[prob(30) ? "  You can discern this degree of emotion from bumbling, ok." : null]")
	...
	bubs
		...
		CritterAttack(mob/M)
			...
			visible_message("<span style=\"color:red\"><strong>[src]</strong> shanks [M] with its [pick("tiny","eeny-weeny","minute","little")] switchblade!</span>")
		...
		attack_hand(mob/user as mob)
			if (alive && user.a_intent == INTENT_GRAB)
				visible_message("<span style=\"color:red\"><strong>[user]</strong> attempts to wrangle [src], but [src] is far, FAR too sassy!</span>")
				return
		...
		attackby(obj/item/W as obj, mob/user as mob)
			if (istype(W, /obj/item/reagent_containers/glass/bottle/bubblebath) && alive && !cleaned && task != "attacking")
				...
				user.visible_message("<span style=\"color:blue\"><strong>[user]</strong> washes [src]!</span>", "<span style=\"color:blue\">You clean the HECK out of [src]!</span>")
				visible_message("<span style=\"color:blue\">[src] bumbles really happily!  Also, a little squeakily.</span>")
		...
		proc/perhaps_go_to_work()
			...
			if (. == "Sun" || . == "Sat")
				return FALSE		//No working the weekends!
			...
			if (. >= 1 && . < 9)
				...
				if (istype(bubs_bee_job))
					hat = new /obj/item/clothing/head/flatcap (src)
					hat_that_bee(hat)
					update_icon()

					set_loc(get_turf(bubs_bee_job))
	...
	overbee
		...
		puke_honey()
			var/turf/T = locate(x + rand(-2,2), y + rand(-2,2), z)
			if (!T)
				return null
			;
			...
			reagents.trans_to(honey, honey_production_amount)
			visible_message("<strong>[src]</strong> wills a blob of honey into existence![prob(10) ? " Weird!" : null]")
			playsound(loc, "sound/effects/mag_forcewall.ogg", 50, 1)
		...
		CritterAttack(mob/M)
			...
			attacking = 1
			visible_message("<span style=\"color:red\"><strong>[src]</strong> stares at [M.name]!</span>")
			playsound(loc, "sound/vox/Buzz.ogg", 100, 1)
			boutput(M, "<span style=\"color:red\">You feel a horrible pain in your head!</span>")
			M.stunned += rand(1,2)
			spawn (25)
				if ((get_dist(src, M) <= 6) && alive)
					M.visible_message("<span style=\"color:red\"><strong>[M.name] clutches their temples!</strong></span>")
					M.emote("scream")
		...
		attackby(obj/item/W as obj, mob/living/user as mob)
			...
			if (istype(W, /obj/item/device/key))
				if (dd_hasprefix(lowertext(W.name), "gold"))
					boutput(user, "<strong>[src]</strong> respectfully declines, as it didn't stay down the first time.")
					return
				if (!dd_hasprefix(lowertext(W.name), "lead"))
					boutput(user, "<strong>[src]</strong> doesn't seem to be interested.  Maybe it's the color?  The metal?")
					return

				W.layer = initial(layer)
				user.u_equip(W)
				W.set_loc(src)
				user.visible_message("<strong>[user]</strong> feeds [W] to [src]!","You feed [W] to [src]. Fuck!")
				spawn (20)
					W.icon_state = "key_gold"
					W.desc += "  It appears to be covered in honey.  Gross."
					visible_message("<strong>[src]</strong> regurgitates [W]!")
					W.name = "golden key"
					playsound(loc, "sound/effects/splat.ogg", 50, 1)
			...
			if (alive)
				if (user.a_intent == INTENT_HARM)
					...
					if (task == "attacking" && target)
						visible_message("<span style=\"color:red\"><strong>[user]</strong> attempts to wrangle [src], but [src] is [pick("mad","grumpy","hecka grumpy","agitated", "too angry")] and resists!</span>")
						return
					...
					if (task == "wandering")
						task = "thinking"
					wrangler = user
					visible_message("<span style=\"color:red\"><strong>[user]</strong> wrangles [src].</span>")
					...
					switch (++hug_count)
						if (10)
							visible_message("<strong>[src]</strong> burps!  It smells like beeswax.")

						if (25)
							src.visible_message("<strong>[src]</strong> burps!  It smells...coppery.  What'd that bee eat?")

						if (100)
							src.visible_message("<strong>[src]</strong> regurgitates a...key? Huh!")
							playsound(loc, "sound/effects/splat.ogg", 50, 1)
							if (name == "sun bee")
								new /obj/item/device/key {name = "solar key"; desc = "A metal key with a sun icon on the bow.";} (loc)
							else
								new /obj/item/device/key {name = "lunar key"; desc = "A metal key with a moon icon on the bow.";} (loc)
	...
	creepy
		desc = "Genetically engineered for extreme size and indistinct segmen-<br>oh god what is wrong with its face<br><strong>oh god it's looking at you</strong>"
		...
		CritterDeath()
			...
			if (!stay_dead)
				spawn (rand(100,1000))
					...
					on_revive()
					visible_message("<span style=\"color:red\">[src] seems to rise from the dead!</span>")
...
/* -------------------- BASE BEE STUFF -------------------- */
	...
	New()
		..()
		var/reagents/R = new/reagents(honey_production_amount)
		reagents = R
		R.my_atom = src
		...
		//spawn (10)
		pixel_x = rand(-max_offset,max_offset)
		pixel_y = rand(-max_offset,max_offset)
	...
	ai_think()
		wanderer = !(wrangler && wrangler.pulling == src)
			...
			if (!beeMom && beeMomCkey)
				for (var/mob/maybeOurMom in hearers(src, null))
					if (maybeOurMom.stat != 2 && beeMomCkey == maybeOurMom.ckey)
						beeMom = maybeOurMom
						visible_message("<span style=\"color:blue\"><strong>[src]</strong> stares at [maybeOurMom] for a moment, then bumbles happily!</span>")
						break

			else if ((beeMom in hearers(src, null)))
				if (beeMom.stat == 2)
					beeMom = null //beeMomCkey still set.
					visible_message("<span style=\"color:red\"><strong>[src]</strong> bumbles MOURNFULLY.</span>")
					return

				if (beeMom.lastattacker && beeMom.lastattacker != beeMom && (beeMom.lastattackertime + 140) >= world.time)
					target = beeMom.lastattacker
					oldtarget_name = "[target]"
					visible_message("<span style=\"color:red\"><strong>[src] buzzes angrily at [beeMom.lastattacker]!</strong></span>")
					task = "chasing"
					return ..()
	...
	on_grump()
		if (target)
			for (var/obj/critter/domestic_bee/fellow_bee in view(7, src))
				...
				fellow_bee.target = target
				fellow_bee.oldtarget_name = oldtarget_name
	...
	CritterAttack(mob/M)
		...
		if (istype(M, /obj/machinery/plantpot))
			...
			var/nectarTransferAmt = min(  min( (reagents.maximum_volume - reagents.total_volume), planterNectarAmt), 25  )
			...
			if (planter.current.assoc_reagents.len || (planter.plantgenes && planter.plantgenes.mutation && planter.plantgenes.mutation.assoc_reagents.len))
				...
				reagents.add_reagent("honey", nectarTransferAmt*0.75)
				...
				reagents.add_reagent("honey", nectarTransferAmt)
			...
			visible_message("<strong>[src]</strong> [pick("slurps","sips","drinks")] nectar out of [planter].")
		...
		visible_message("<span style=\"color:red\"><strong>[src]</strong> bites [M] with its [pick("tiny","eeny-weeny","minute","little", "nubby")] [prob(50) ? "mandibles" : "bee-teeth"]!</span>")
	...
	ChaseAttack(mob/M)
		...
		if (M.stat || M.paralysis)
			task = "thinking"
			return
		visible_message("<span style=\"color:red\"><strong>[src]</strong> pokes [M] with its [pick("nubby","stubby","tiny")] little stinger!</span>")
	...
	emag_act(var/mob/user, var/obj/item/card/emag/E)
		...
		if (E.icon_state == "gold")
			boutput(user, "<strong>[src]</strong> respectfully declines, as it didn't stay down the first time.")
			return
		E.layer = initial(layer)
		user.u_equip(E)
		E.set_loc(src)
		if (user)
			user.visible_message("<strong>[user]</strong> feeds [E] to [src]!","You feed [E] to [src]. Fuck!")
		spawn (20)
			if (istype(src, /obj/critter/domestic_bee/bubs)) //The fattest and hungriest bee
				qdel(E)
				visible_message("<strong>[src]</strong> burps.")
				spawn (10)
					visible_message("<strong>[src]</strong> bumbles happily!")
					dance()
				spawn (180)
					if (task != "chasing" && task != "attacking" && user && get_dist(src, user) <= 7)
						visible_message("<strong>[src]</strong> buzzes in a clueless manner as to why [user] looks so dejected.[prob(5)?" You can tell because you studied bee linguistics, ok?": null]")

						//Is this a bad idea? It probably is a bad idea.
						spawn (20)
							var/obj/item/dagger/D = new /obj/item/dagger/syndicate(loc)
							D.name = "tiny switchblade"
							D.desc = "Why would a bee even have this!?"
							visible_message("<strong>[src]</strong> drops \a [D] on the floor in an attempt to cheer [user] up!")
							playsound(D.loc, 'sound/effects/crystalhit.ogg' , 30, 1)
			else
				E.icon_state = "gold"
				E.desc += "  It appears to be covered in honey.  Gross."
				visible_message("<strong>[src]</strong> regurgitates [E]!")
	...
	attackby(obj/item/W as obj, mob/living/user as mob)
		...
		if (istype(W, /obj/item/reagent_containers/food/snacks))
			if (findtext(W.name,"bee") && !istype(W, /obj/item/reagent_containers/food/snacks/beefood)) // You just know somebody will do this
				visible_message("<strong>[src]</strong> buzzes in a repulsed manner!", 1)
			...
			if (!W.reagents)
				boutput(user, "<strong>[src]</strong> respectfully declines, being a strict nectarian.")
				return
			...
			visible_message("<strong>[src]</strong> buzzes delightedly.", 1)
			health = min(initial(health), health+10)
			W.reagents.del_reagent("nectar")

			reagents.add_reagent("honey", nectarAmt)
	...
	proc/update_icon()
		...
		if (!alive)
			icon_state = "[icon_body]-dead"
			if (beeKid)
				var/image/color_overlay = image(icon, "[icon_body]-dead-color")
				color_overlay.color = beeKid
				overlays += color_overlay
		else if (alive && sleeping)
			icon_state = "[icon_body]-sleep"
			if (beeKid)
				var/image/color_overlay = image(icon, "[icon_body]-sleep-color")
				color_overlay.color = beeKid
				overlays += color_overlay
			overlays += image(icon, icon_sleep)
		else
			icon_state = "[icon_body]-wings"
			if (beeKid)
				var/image/color_overlay = image(icon, "[icon_body]-color")
				color_overlay.color = beeKid
				overlays += color_overlay

		if (royal)
			var/image/crown_image = image(icon, "crown-[icon_body]")
			var/image/antenna_image = image(icon, "[icon_antenna]")
			if (!alive)
				crown_image.pixel_y -= sleep_y_offset
				antenna_image.icon_state = "[icon_antenna]-dead"
			else if (alive && sleeping)
				crown_image.pixel_y -= sleep_y_offset
				antenna_image.icon_state = "[icon_antenna]-sleep"
			overlays += crown_image
			overlays += antenna_image

		else if (hat && !cant_take_hat)
			if (hat_overlay_left)
				hat_overlay_left.pixel_x = hat_x_offset_left
				hat_overlay_left.pixel_y = hat_y_offset
			if (hat_overlay_right)
				hat_overlay_right.pixel_x = hat_x_offset_right
				hat_overlay_right.pixel_y = hat_y_offset
			var/image/antenna_image = image(icon, "[icon_antenna]")
			if (!alive)
				if (hat_overlay_left)
					hat_overlay_left.pixel_y -= sleep_y_offset
				if (hat_overlay_right)
					hat_overlay_right.pixel_y -= sleep_y_offset
				antenna_image.icon_state = "[icon_antenna]-dead"
			else if (alive && sleeping)
				if (hat_overlay_left)
					hat_overlay_left.pixel_y -= sleep_y_offset
			...
			overlays += hat_overlay_left
			overlays += hat_overlay_right
			overlays += antenna_image
	...
	proc/dance_response()
		if (is_dancing || !alive || sleeping)
			return

		if (prob(dance_chance))
			visible_message("<strong>[src]</strong> responds with a dance of its own!")
			dance()
		else
			if (istype(src, /obj/critter/domestic_bee/trauma))
				visible_message("<strong>[src]</strong> buzzes in short-lived comfort.")
			else
				visible_message("<strong>[src]</strong> buzzes [pick("to the beat", "in tune", "approvingly", "happily")].")
			...
			spawn (time_time * 8)
				if (istype(src, /obj/critter/domestic_bee/bubs))
					...
					spawn (20)
						...
						task = "sleeping"
						on_sleep()
						visible_message("<span style=\"color:blue\">[src] gets tired from all that work and takes a nap!</span>")
						is_dancing = 0
				else
					animate_bumble(src)
					is_dancing = 0
	...
	proc/puke_honey()
		...
		if (honey.reagents)
			honey.reagents.maximum_volume = honey_production_amount
		reagents.trans_to(honey, honey_production_amount)
		visible_message("<strong>[src]</strong> regurgitates a blob of honey![prob(10) ? " Gross!" : null]")
		playsound(loc, "sound/effects/splat.ogg", 50, 1)
		if (honey_color)
	...
	attackby(obj/item/W as obj, mob/living/user as mob)
		if (istype(W, /obj/item/clothing/head))
			...
			hat_that_bee(hat)
			update_icon()
			user.visible_message("<span style=\"color:blue\"><strong>[user]</strong> puts a hat on [src]!</span>",\
...
/obj/critter/domestic_bee_larva
	...
	ai_think()
		...
		if (growth_timer-- == 0)
			if (royal == 1)
				visible_message("[src] pupates!")
				icon = 'icons/misc/bigcritter.dmi'
				icon_state = grow_anim
				pixel_x = -16
				pixel_y = -16
				spawn (25)
					...
					grownbee.name = replacetext(name, "larva", "bee")
					if (color)
						grownbee.beeKid = color
					if (custom_desc)
						grownbee.desc = custom_desc
					if (reagents)
						grownbee.reagents = reagents
						grownbee.reagents.my_atom = grownbee

					grownbee.beeMom = beeMom
					grownbee.beeMom = beeMomCkey
					grownbee.update_icon()
					qdel(src)
				return
			else
				visible_message("[src] pupates!")
				icon_state = "[initial(icon_state)]-grow"
				spawn (25)
					...
					grownbee.beeMom = beeMom
					grownbee.beeMomCkey = beeMomCkey
					grownbee.update_icon()
					qdel(src)

		else if (task != "attacking" && !scolded)
	...
	CritterAttack(mob/M)
		if (istype(target, /obj/item/clothing/under))
			...
			if (!attacking)
				attacking = 1
				visible_message("<strong>[src]</strong> [pick("nibbles on", "nips at", "chews on", "gnaws")] [target]!")
				spawn (100)
					attacking = 0
		else
			return ..()
	...
	attackby(obj/item/W as obj, mob/living/user as mob)
		...
		if (istype(W, /obj/item/reagent_containers/food/snacks))
			if (findtext(W.name,"bee")) // You just know somebody will do this
				visible_message("<strong>[src]</strong> squeals in a repulsed manner!", 1)
			...
			if (!W.reagents || !W.reagents.has_reagent("royal_jelly"))
				boutput(user, "<strong>[src]</strong> stares at [W], confused.")
				return

			if (royal != 0)
				boutput(user, "<strong>[src]</strong> doesn't seem hungry.  Oh well.")
				return

			user.visible_message("<strong>[user]</strong> feeds [W] to [src]!","You feed [W] to [src].")
			visible_message("<strong>[src]</strong> squeals delightedly.", 1)
			health = min(initial(health), health+10)
			royal = 1

			qdel(W)

		else if (istype(W, /obj/item/paper) && istype(target, /obj/item/clothing))
			user.visible_message("<strong>[user]</strong> [prob(50) ? "bops" : "boops"] [src] with a rolled paper!","You roll up the paper and gently bop [src] on the...nose ? area??")
			user.say("No!")
			task = "thinking"
			attacking = 0
			target = null
			scolded = 1
			visible_message("<strong>[src]</strong> squeals in a SCOLDED MANNER.")
...
/obj/item/reagent_containers/food/snacks/ingredient/egg/bee
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/pen))
			...
			var/t = input(user, "Enter new bee name", name, bee_name) as null|text
	...
	throw_impact(var/turf/T)
		...
		newLarva.throw_at(get_edge_target_turf(src, dir), 2, 1)
		qdel (src)
...
/obj/item/bee_egg_carton
	...
	attackby(obj/item/W as obj, mob/living/user as mob)
		if (istype(W, /obj/item/reagent_containers/food/snacks/ingredient/egg/bee))
			if (!open)
				boutput(user, "<span style=\"color:red\">For <em>some reason</em>, you are unable to place the egg into a closed carton.</span>")
				return
	...
	attack_hand(mob/user as mob)
		if (loc == user && ourEgg && open)
			user.put_in_hand_or_drop(ourEgg)
			boutput(user, "You take [ourEgg] out of [src].")
			ourEgg = null
	...
	proc/update_icon()
		if (open)
			icon_state = "petbee_carton[ourEgg != null]"
		else
			icon_state = "petbee_carton"
...
/obj/critter/fake_bee //Based on the deer botfly, a bumblebee mimic that plants eggs in the noses of deer.  Oh, except when it confuses a human eye for a deer nose.
	...
	CritterAttack(mob/M)
		...
		visible_message("<span style=\"color:red\"><strong>[src]</strong> bites [M]!</span>")
		random_brute_damage(M, 2)
		if (M.stat || M.paralysis)
			task = "thinking"
			attacking = 0
	...
	attack_hand(mob/user as mob)
		if (alive)
			if (user.a_intent == INTENT_HARM)
				...
				visible_message("<span style=\"color:red\"><strong>[user]</strong> attempts to wrangle [src], but [src] squirms away.</span>")
				return

			else

				user.visible_message("<span style=\"color:red\"><strong>[user]</strong> pets [src]. Both parties look uncomfortable.</span>","<span style=\"color:red\">You pet [src]. [src] looks uncomfortable.  You don't feel much better.</span>")
				if (prob(15))
					for (var/mob/O in hearers(src, null))
	...
	attackby(obj/item/W as obj, mob/living/user as mob)
		if (!alive)
			return
		if (istype(W, /obj/item/reagent_containers/food/snacks))
			visible_message("<strong>[src]</strong> stares blankly at [W].")

		else if (istype(W, /obj/item/luggable_computer/cheget))
			if (!W:locked)
				visible_message("<strong>[src]</strong> stares blankly at [W].")
			else
				visible_message("<strong>[src]</strong> stares blankly at [W] for a moment, then bops against its keypad several times.")
