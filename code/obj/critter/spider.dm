...
/obj/critter/nicespider
	...
	CritterAttack(mob/M)
		attacking = 1
		M.visible_message("<span style=\"color:red\"><strong>[src]</strong> nibbles [target]!</span>")
	...
	attack_hand(mob/user as mob)
		if (alive)
			if (user.a_intent == INTENT_HARM)
				...
				visible_message("<span style=\"color:red\"><strong>[user]</strong> [pick("pets","hugs","snuggles","cuddles")] [src]!</span>")
				if (prob(15))
					for (var/mob/O in hearers(src, null))
						O.show_message("[src] coos[prob(50) ? " happily!" : ""]!",2)
						playsound(loc, "sound/misc/babynoise.ogg", 40, 0)
				return
		else
			..()
	...
	proc/spiderspaz(mob/M) //todo: centralize this fucking mess of spiders into one base type
		...
		spazzing = 10
		playsound(loc, "rustle", 50, 0)
		spawn (0)
			while (spazzing-- > 0 && alive)
				set_loc(M.loc)
				pixel_x = rand(-2,2) * 2
				pixel_y = rand(-2,2) * 2
				dir = pick(alldirs)
...
/obj/critter/spider
	...
	New()
		...
		if (!icon_state && !babyspider)
			...
			dead_state = "[icon_state]-dead"

	CritterDeath()
		if (!alive) return
		..()
		playsound(loc, deathsound, 50, 0)
		reagents.add_reagent(venom1, 50, null)
		reagents.add_reagent(venom2, 50, null)
	...
	seek_target()
		...
		for (var/mob/living/C in hearers(seekrange,src))
			...
			visible_message("<span style=\"color:red\"><strong>[src]</strong> hisses!</span>")
			break

	Move()
		if (stepsound)
			if (prob(30))
				playsound(loc, stepsound, 50, 0)
		..()
	...
	CritterAttack(mob/M)
		if (ismob(M))
			...
			if (prob(20))
				...
				playsound(loc, "sound/weapons/thudswoosh.ogg", 50, 0)
				M.weakened += rand(2,4)
				M.stunned += rand(1,3)
				...
				if (ishuman(M))
					...
					reagents.add_reagent("[venom2]", 2)
					if (reacting)
						reagents.reaction(M, INGEST)
					else
						reagents.trans_to(M, 4)
				else if (issilicon(M))
					switch(rand(1,4))
						if (1)
			...
			if (ishuman(M))
				if (M.stat == 1) // kill KOd people faster
					set_loc(M.loc)
					visible_message("<span style=\"color:red\"><strong>[src]</strong> jumps onto [target]!</span>")
					sleep(5)
					visible_message("<span style=\"color:red\"><strong>[src]</strong> sinks its fangs into [target]!</span>")
					playsound(loc, "sound/misc/fuse.ogg", 50, 1)
				...
				if (M.stat == 2 && !feeding) // drain corpses into husks
					if (ishuman(M))
						...
						feeding = 1
						spiderspaz(target)
						visible_message("<span style=\"color:red\"><strong>[src]</strong> starts draining the fluids out of [T]!</span>")
						set_loc(T.loc)
						sleep(20)
						playsound(loc, "sound/misc/pourdrink.ogg", 50, 1)
						sleep(50)
						if (target && T.stat && loc == T.loc) // check to see if the target is still passed out and under the spider
							visible_message("<span style=\"color:red\"><strong>[src]</strong> drains [T] dry!</span>")
							T.death(0)
							T.real_name = "Unknown"
							T.bioHolder.AddEffect("husk")
							sleep(2)
							playsound(loc, "sound/misc/fuse.ogg", 50, 1)
							set_loc(get_step(src, pick(alldirs))) // get the fuck out of the way of the ice cube
							sleep(2)
							var/obj/icecube/cube = new /obj/icecube(get_turf(M), M)
							M.set_loc(cube)
							if (encase_in_web)
								visible_message("<span style=\"color:red\"><strong>[src]</strong> encases [target] in web!</span>")
								...
								visible_message("<span style=\"color:red\"><strong>[src]</strong> encases [target] in ice!</span>")

							feeding = 0
							if (babyspider) // dawww
								visible_message("<span style=\"color:red\"><strong>[src]</strong> grows up!</span>")
								var/adult = text2path(adultpath)
								new adult(loc)
								qdel(src)
						else
							feeding = 0
	...
	on_pet()
		playsound(loc, 'sound/misc/babynoise.ogg', 50, 1)
		visible_message("<span style=\"color:red\"><strong>[src] coos!</strong></span>", 1)
