...
/obj/decal/cleanable
	...
	New(var/loc,var/list/viral_list)
		...
		if (viral_list && viral_list.len > 0)
			for (var/ailment_data/AD in viral_list)
				diseases += AD
		if (can_dry)
			Dry()
		..()
	...
	HasEntered(AM as mob|obj)
		...
		if (iscarbon(AM))
			...
			if (!M.can_slip(thing = src))
				return
			#ifndef SLIPPING_SUCKS_LOL
			if (prob(slippery))
				M.pulling = null
				M.visible_message("<span style=\"color:red\"><strong>[M]</strong> slips on [src]!</span>",\
				...
				if (slipped_in_blood)
					M.add_blood()
			#endif
	#undef SLIPPING_SUCKS_LOL

	attackby(obj/item/W, mob/user)
		if (can_sample && W.is_open_container() && W.reagents)
			Sample(W, user)
		else
			return ..()
	...
	proc/Sample(var/obj/item/W as obj, var/mob/user as mob)
		if (!can_sample || !W.reagents)
			return FALSE
		if (sampled)
			user.show_text("There's not enough left of [src] to [sample_verb] into [W].", "red")
			return FALSE
		...
		if (reagents)
			if (W.reagents.total_volume >= W.reagents.maximum_volume - (src.reagents.total_volume - 1))
				...
				user.visible_message("<span style=\"color:blue\"><strong>[user]</strong> [sample_verb]s some of [src] into [W].</span>",\
				"<span style=\"color:blue\">You [sample_verb] some of [src] into [W].</span>")
				W.reagents.handle_reactions()
				sampled = 1
				return TRUE

		else if (sample_amt && sample_reagent)
			if (W.reagents.total_volume >= W.reagents.maximum_volume - (sample_amt - 1))
				user.show_text("[W] is too full!", "red")
				return FALSE
			else
				W.reagents.add_reagent(sample_reagent, sample_amt)
				user.visible_message("<span style=\"color:blue\"><strong>[user]</strong> [sample_verb]s some of [src] into [W].</span>",\
				"<span style=\"color:blue\">You [sample_verb] some of [src] into [W].</span>")
				W.reagents.handle_reactions()
				sampled = 1
...
/obj/decal/cleanable/blood
	...
	New()
		..()
		var/reagents/R = new/reagents(10) // 9u is the max since we wanna leave at least 1u of blood in the blood puddle
		reagents = R
		R.my_atom = src
		if (ling_blood)
			reagents.add_reagent("bloodc", 10)
	...
	Dry(var/time = rand(200,500))
		...
		spawn (time)
			dry = 1
			name = "dried [real_name]"
			desc = "It's all flakey and dry now."
			return TRUE

	disposing()
		diseases = list()
		var/obj/decal/bloodtrace/B = new /obj/decal/bloodtrace(loc)
		if (B.blood_DNA)
...
/obj/decal/cleanable/blood/dynamic
	...
	// amount should only be 1-5 if you want anything to happen
		blood_volume ++
		if (reagents.maximum_volume < 100)
		...
		switch (amount)
			if (-INFINITY to 0)
				return
			if (1)
				create_overlay(low_icon_states, add_color)
			if (2)
				create_overlay(med_icon_states, add_color)
			if (3)
				create_overlay(high_icon_states, add_color)
			if (4)
				create_overlay(max_icon_states, add_color)
			if (5)
				create_overlay(violent_icon_states, add_color) // for when you wanna create a BIG MESS
			if (6 to INFINITY)
				return
...
/obj/decal/cleanable/blood/gibs
	...
	attack_hand(var/mob/user as mob)
		if (istype(user, /mob/living/carbon/human))
			...
			if (H.job == "Chef")
				user.visible_message("<span style=\"color:blue\"><strong>[H]</strong> starts rifling through [src] with their hands. What a weirdo.</span>",\
				"<span style=\"color:blue\">You rake through the gibs with your bare hands.</span>")
				playsound(loc, "sound/effects/splat.ogg", 50, 1)
				if (H.gloves)
					H.gloves.blood_DNA = blood_DNA
				else
					H.blood_DNA = blood_DNA
				if (sampled)
					...
					new /obj/item/reagent_containers/food/snacks/ingredient/meat/mysterymeat(loc)
					sampled = 1
			else
				return ..()
		else
		...
		spawn (0)
			...
			for (var/i = 0, i < pick(1, 200; 2, 150; 3, 50; 4), i++)
				...
				if (i > 0)
					...
					if (diseases)
						b.diseases += diseases
					if (blood_DNA && blood_type) // For forensics (Convair880).
						b.blood_DNA = blood_DNA
						b.blood_type = blood_type
...
/obj/decal/cleanable/writing
	...
	get_desc(dist)
		. = "<br><span style='color: blue'>It says:</span><br><span style='color: [font_color]'>[words]</span>"
		//. = "[webfont ? "<link href='http://fonts.googleapis.com/css?family=[font]' rel='stylesheet' type='text/css'>" : null]<span style='color: blue'>It says:</span><br><span style='[font ? "font-family: [font][webfont ? ", cursive" : null];" : null]color: [font_color]'>[words]</span>"
	...
	attackby(obj/item/W as obj, mob/living/user as mob)
		if (istype(W, /obj/item/pen))
			...
			if (!t)
				pen.in_use = 0
				return
			if ((length(words) + length(t)) > max_message)
				user.show_text("All that won't fit on [src]!", "red")
				pen.in_use = 0
				return
			logTheThing("station", user, null, "writes on [src] with [pen] at [showCoords(x, y, z)]: [t]")
			t = copytext(html_encode(t), 1, MAX_MESSAGE_LEN)
			if (pen.font_color)
				color = pen.font_color
			if (pen.uses_handwriting && user && user.mind && user.mind.handwriting)
				font = user.mind.handwriting
				webfont = 1
			else if (pen.font)
				font = pen.font
				if (pen.webfont)
			...
			if (copytext(search_t, 1, 4) == "no ")
				icon_state = "postit-no"
			else if (copytext(search_t, -1) == "?")
				icon_state = "postit-quest"
			else if (copytext(search_t, -1) == "!")
				icon_state = "postit-excl"
	...
	Sample(var/obj/item/W as obj, var/mob/user as mob)
		...
		if (W.is_open_container() && W.reagents)
			if (W.reagents.total_volume >= W.reagents.maximum_volume - 2)
				...
				user.visible_message("<span style=\"color:blue\"><strong>[user]</strong> is splashing the urine puddle into \the [W]. How singular.</span>",\
...
/obj/decal/cleanable/vomit
	...
	Sample(var/obj/item/W as obj, var/mob/user as mob)
		...
		if (sample_amt && sample_reagent)
			if (W.reagents.total_volume >= W.reagents.maximum_volume - (sample_amt - 1))
				user.show_text("[W] is too full!", "red")
				return FALSE
			else
				W.reagents.add_reagent(sample_reagent, sample_amt)
				user.visible_message("<span style=\"color:blue\"><strong>[user]</strong> is sticking their fingers into [src] and pushing it into [W]. It's probably best not to ask.</span>",\
				"<span style=\"color:blue\">You [sample_verb] some of the puke into [W]. You are absolutely disgusting.</span>")
				W.reagents.handle_reactions()
				playsound(loc, "sound/effects/splat.ogg", 50, 1)
				sampled = 1
				return TRUE
...
/obj/decal/cleanable/vomit/spiders
	...
	Sample(var/obj/item/W as obj, var/mob/user as mob)
		...
		if (W.reagents.total_volume >= W.reagents.maximum_volume - (sample_amt - 1))
			...
			user.visible_message("<span style=\"color:blue\"><strong>[user]</strong> is sticking their fingers into [src] and pushing it into [W].<span style=\"color:red\">It [fluff] a bit.[fluff2]</span></span>",\
			"<span style=\"color:blue\">You [sample_verb] some of the puke into [W].<span style=\"color:red\">It [fluff] a bit.[fluff2]</span></span>")
			W.reagents.handle_reactions()
			playsound(loc, "sound/effects/splat.ogg", 50, 1)
			sampled = 1
			return TRUE
		...
		if (sample_amt && sample_reagent)
			if (W.reagents.total_volume >= W.reagents.maximum_volume - (sample_amt - 1))
				...
				for (var/mob/O in AIviewers(user, null))
					if (O != user)
						O.show_message("<span style=\"color:blue\"><strong>[user]</strong> is sticking their fingers into [src] and pushing it into [W]. It's all slimy and stringy. Oh god.</span>", 1)
...
/obj/decal/cleanable/fungus
	...
	proc/update_icon()
		icon_state = "fungus[max(1,min(3, amount))]"
			...
			for (var/i = 0, i < pick(1, 200; 2, 150; 3, 50; 4), i++)
				...
				if (i > 0)
					if (prob(40))
						...
						new /obj/decal/cleanable/oil/streak(loc)
					else if (prob(10))
						var/effects/system/spark_spread/s = unpool(/effects/system/spark_spread)
						s.set_up(3, 1, src)
						s.start()
				...
				if (loc == T && !disposed && on_fire)
					...
					if (burn_time <= 2)
						for (var/D in cardinal)
							...
							if (M)
								M.ignite()
						if (loc && loc.reagents && loc.reagents.total_volume)
							for (var/i = 0, i < 10, i++)
								loc.reagents.temperature_reagents(T0C + 3100, 10)
						if (loc)
...
/turf/proc/getSaltHere()
	for (var/obj/decal/cleanable/saltpile/S in contents)
		return S
...
/obj/decal/cleanable/nitrotriiodide
	...
	HasEntered(AM as mob|obj)
		if ( !dry || !(isliving(AM) || isobj(AM)) ) return
		bang()
	...
	proc/bang()
		visible_message("<strong>The dust emits a loud bang!</strong>")
		if (prob(20))
			var/effects/system/bad_smoke_spread/smoke = new /effects/system/bad_smoke_spread()
			smoke.set_up(5, 0, loc, ,"#cb5e97")
			smoke.start()
		/*
		var/effects/system/spark_spread/s = unpool(/effects/system/spark_spread)
		s.set_up(5, 1, loc)
...
/obj/decal/tile_edge
	...
	New()
		if (merge_with_turf)
			...
			if (!T)
				return ..()
			var/image/I = image(icon, T, icon_state, layer, dir)
			if (color)
				I.color = color
...
/obj/decal/icefloor/HasEntered(var/atom/movable/AM)
	if (iscarbon(AM))
		...
		if (prob(5))
			...
			M.visible_message("<span style=\"color:red\"><strong>[M]</strong> hits their head on [src]!</span>")
...
/*	New()
	...
	disposing(var/uncapture = 1)
		...
		if (tagarea.gang_owners == owners && uncapture)
...
/obj/decal/mule
	...
	beacon
		...
		New()
			..()
			var/turf/T = get_turf(src)
			if (T && isturf(T) && auto_dropoff_spawn == 1)
