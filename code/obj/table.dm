/obj/table
	...
	New()
		...
		spawn (10)
			if (auto)
				if (ispath(auto_type) && icon_state == "0") // if someone's set up a special icon state don't mess with it
					set_up()
					spawn (1)
	...
	suicide(var/mob/user as mob) //if this is TOO ridiculous just remove it idc
		user.visible_message("<span style=\"color:red\"><strong>[user] contorts \himself so that \his head is underneath one of the [name]'s legs and \his heels are resting on top of it, then raises \his feet and slams them back down over and over again!</strong></span>")
...
/obj/table/blob_act(var/power)
	if (prob(power * 2.5))
		var/atom/A = new /obj/item/table_parts(loc)
		if (material)
...
/obj/table/attack_hand(mob/user as mob)
	if (user.bioHolder && user.bioHolder.HasEffect("hulk"))
		...
		if (istype(src, /obj/table/reinforced))
			var/atom/A = new /obj/item/table_parts/reinforced(loc)
			if (material)
				A.setMaterial(material)
		else
			var/atom/A = new /obj/item/table_parts(loc)
	...
	if (ishuman(user))
		...
		if (istype(H.w_uniform, /obj/item/clothing/under/misc/lawyer))
			visible_message("<span style=\"color:red\"><strong>[H] slams their palms against [src]!</strong></span>")
...
/obj/table/MouseDrop_T(obj/O as obj, mob/user as mob)
	...
	if (istype(O,/obj/item/satchel))
		if (O.contents.len < 1)
			...
			for (var/obj/item/I in O.contents) I.set_loc(loc)
...
/obj/table/proc/smash_bottle(var/mob/user as mob, var/obj/item/reagent_containers/food/drinks/bottle/bottle as obj)
	...
	if (bottle.broken)
		user.visible_message("<span style=\"color:red\"><strong>[user] smashes [bottle] on [src]! [bottle] shatters completely!</span>")
	...
	if (prob(success_prob))
		user.visible_message("<span style=\"color:red\"><strong>[user] smashes [bottle] on [src], shattering it open![prob(50) ? " [user] looks like they're ready for a fight!" : " [bottle] has one mean edge on it!"]</span>")
		playsound(U, pick('sound/effects/Glassbr1.ogg','sound/effects/Glassbr2.ogg','sound/effects/Glassbr3.ogg'), 100, 1)
		new /obj/item/raw_material/shard/glass(U)
		bottle.broken = 1
		bottle.reagents.reaction(U)
...
/obj/table/attackby(obj/item/W as obj, mob/user as mob, params)
	if (istype(W, /obj/item/grab))
		...
		if (!G.state)
			...
			return
		G.affecting.set_loc(loc)
		G.affecting.weakened = 2
		visible_message("<span style=\"color:red\">[G.assailant] puts [G.affecting] on the table.</span>")
