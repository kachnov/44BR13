...
/obj/submachine/robomoduler
	...
	attack_hand(var/mob/user as mob)
		user.machine = src
		if (!working)
			var/dat = {"<strong>Module Rewriter</strong><BR>
			<HR><BR>
			<strong>Modules Available:</strong> [modules]<BR>
			<HR><BR>
		...
		if (href_list["module"])
			if (modules < 1)
				for (var/mob/O in hearers(src, null))
					O.show_message(text("<strong>[]</strong> states, 'No modules available for write.'", src), 1)
				return
...
/obj/item/atmosporter
	...
	attack_self(var/mob/user as mob)
		if (contents.len == 0) boutput(user, "<span style=\"color:red\">You have nothing stored!</span>")
		else
			var/selection = input("What do you want to drop?", "Atmos Transporter", null, null) as null|anything in contents
			if (!selection) return
...
/obj/item/robot_chemaster
	...
	attackby(obj/item/W as obj, mob/user as mob)
		...
		if (!B.reagents.reagent_list.len || B.reagents.total_volume < 1)
			boutput(user, "<span style=\"color:red\">That beaker is empty! There are no reagents for the [name] to process!</span>")
			return
		if (working)
...
/obj/item/robot_foodsynthesizer
	...
	attack_self(var/mob/user as mob)
		if (!vend_this)
			var/pickme = input("Please make your selection!", "Item selection", vend_this) in list("Burger", "Cheeseburger", "Meat sandwich", "Cheese sandwich", "Snack", "Cola", "Milk")
			vend_this = pickme
			user.show_text("[pickme] selected. Click with the synthesizer on yourself to pick a different item.", "blue")
			return

		if (last_use && world.time < last_use + 50)
			...
			switch(vend_this)
				...
				if ("Milk")
					...
					user.show_text("<strong>ERROR</strong> - Invalid item! Resetting...", "red")
					logTheThing("debug", user, null, "<strong>Convair880</strong>: [user]'s food synthesizer was set to an invalid value.")
					vend_this = null
					return
			...
			user.visible_message("<span style=\"color:blue\">[user] dispenses a [vend_this]!</span>", "<span style=\"color:blue\">You dispense a [vend_this]!</span>")
			last_use = world.time
			return
...
/obj/item/reagent_containers/glass/oilcan
	...
	New()
		var/reagents/R = new/reagents(120)
		reagents = R
		R.my_atom = src
	...
	afterattack(obj/target, mob/user)
		...
		if (istype(target, /obj/item/reagent_containers/food/snacks/plant)) // Check to make sure they're juicing food.
			if ((target.reagents.total_volume + src.reagents.total_volume) > src.reagents.maximum_volume)
				var/transamnt = reagents.maximum_volume - reagents.total_volume
				...
				playsound(loc, "sound/machines/mixer.ogg", 50, 1) // Play a sound effect.
	...
	get_desc(dist)
		if (dist <= 0)
			if (reagents && reagents.reagent_list.len)
				. += "<br>It contains:"
				for (var/reagent/R in reagents.reagent_list)
...
/obj/item/borghose
	...
	proc/create_tank(var/reagent) // The actual add_reagent function to add new reagents to the hose.
		...
		tanks += new_tank
		hydro_reagent_names += new_tank.label_name // the name list is so we don't have to call reagent_id_to_name() each time we wanna know the names of our reagents
	...
	proc/regenerate_reagents()
		if (isrobot(loc))
			var/mob/living/silicon/robot/R = loc // I'm not sure why it's loc and not  (src is the hose, loc is where the hose is)
			if (R && R.cell) // If the robot's alive and there's power.
				...
				for (var/obj/item/reagent_containers/borghose_tank/tank in tanks) // Regenerate all formulas at once.
					...
					var/add_amt = min((tank_max - tank_vol), recharge_per_tick) // how much we'll be adding, in case the room left in the tank is less than recharge_per_tick
	...
	attack_self(mob/user)
		var/switch_tank = input(user, "What reagent do you want to dispense?") as null|anything in hydro_reagent_names
		if (!switch_tank)
			return
		for (var/obj/item/reagent_containers/borghose_tank/tank in tanks)
			if (active_tank == tank)
				return
	...
	afterattack(obj/target, mob/user)
		if (istype(target, /obj/machinery/plantpot))
			...
			if (!active_tank.reagents || !active_tank.reagents.total_volume) // vOv
			...
			var/trans = active_tank.reagents.trans_to(target, amt_to_transfer)
