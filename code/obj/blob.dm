/obj/blob
	...
	var/movable = 0
	var/in_disposing = 0
	var/action/bar/blob_health/healthbar //Hack.
	...
	New()
		...
		var/controller/process/blob/B = get_master_blob_controller()
		B.blobs += src
	...
	CanPass(atom/movable/mover, turf/target, height=0, air_group=0)
		...
		if (istype(P))
			if (P.proj_data.type == /projectile/slime)
				return TRUE
	...
	proc/attack(var/turf/T)
		particleMaster.SpawnSystem(new /particleSystem/blobattack(T,overmind.color))
		if (T.density)
	...
	ex_act(severity)
		...
		take_damage(damage,damage_mult,"mixed")
		return
	...
	attack_hand(var/mob/user)
		...
		visible_message("<span class='combat'><strong>[user.name]</strong> [adj1] [act1] [src]! That's [adj2] [act2]!</span>")
		return

	attackby(var/obj/item/W, var/mob/user)
		visible_message("<span class='combat'><strong>[user.name]</strong> attacks [src] with [W]!</span>")
		playsound(loc, "sound/effects/attackblob.ogg", 50, 1)
		if (W.hitsound)
		...
		if (damage)
			...
			if (type == /obj/blob && damtype == "brute")
				var/chunk_chance = 2
		...
		take_damage(damage,damage_mult,damtype,user)
	...
	proc/take_damage(var/amount,var/damage_mult = 1,var/damtype = "brute",var/mob/user)
		...
		switch (damtype)
			...
			if ("poison","self_poison")
				if (!has_upgrade(/blob_upgrade/poison_resist))
					ignore_armor = 1
				else
		...
		health -= amount
		health = max(0,min(health_max,health))
		...
		if (poison)
			...
			if (spread.len)
				...
				for (var/obj/blob/B in spread)
					B.poison += amt
		for (var/obj/material_deposit/M in loc)
	...
	proc/heal_damage(var/amount)
		...
		health += amount
		health = max(0,min(health_max,health))
		particleMaster.SpawnSystem(new /particleSystem/blobheal(get_turf(src),color))
		update_icon()
		healthbar.onUpdate()
	...
	proc/update_icon()
		...
		var/healthperc = get_x_percentage_of_y(health,health_max)
		switch(healthperc)
			if (-INFINITY to 33)
				alpha *= 0.25
			if (34 to 66)
	...
	proc/spread(var/turf/T)
		...
		if (ispath(spread_type))
			blob_type = spread_type
	...
	proc/Life()
		...
		if (overmind.tutorial)
			if (!overmind.tutorial.PerformSilentAction("blob-life", src))
				return FALSE
		if (poison)
			var/damage_taken = min(5, poison)
			take_damage(damage_taken, 1, "self_poison")
			poison -= damage_taken * poison_depletion
			poison = max(poison, 0)
			updatePoisonOverlay()
	...
	proc/has_upgrade(var/upgrade_path)
		if (!ispath(upgrade_path) && !istype(overmind))
			return FALSE
		if (overmind && overmind.has_upgrade(upgrade_path))
			return TRUE
		else
			return FALSE
	...
	MouseDrop_T(atom/movable/O as mob|obj, mob/user as mob)
		...
		if (istype(O, /obj/item))
			if (get_dist(O, src) > 1)
				return
			var/blob_ability/devour_item/D = overmind.get_ability(/blob_ability/devour_item)
			if (D)
				D.onUse(O)
			return
		...
		if (overmind.tutorial)
			if (!overmind.tutorial.PerformAction("mousedrop", list(O, src)))
				return
		if (istype(O, /obj/blob) && type == /obj/blob) // I'M LAZY. i'll fix this up if needed
			var/obj/blob/Q = O
			if (Q.movable)
				Q.onMove(src)
...
/obj/blob/nucleus
	...
	take_damage(amount, mult, damtype, mob/user)
		...
		if (!lastAttackMsg || now - lastAttackMsg >= 50) //every 5 seconds supposedly
			boutput(overmind, "<span style=\"color:red\"><strong>Your nucleus in [get_area(src)] is taking damage!</strong></span>")
			lastAttackMsg = now
...
/obj/blob/deposit
	...
	proc/update_reagent_overlay()
		...
		if (reagents.total_volume <= 0)
			overlays.len = 0
			return
		var/curr_color = reagents.get_average_rgb()
	...
	proc/build_reclaimer()
		if (disposed || building)
			return
		building = 1
		var/obj/blob/deposit/reclaimer/B = new(loc)
		B.setOvermind(overmind)
		B.reagents = reagents
	...
	proc/build_replicator()
		if (disposed || building)
			return
		building = 1
		var/obj/blob/deposit/replicator/B = new(loc)
		B.setOvermind(overmind)
		B.reagents = reagents
		B.reagents.my_atom = B
	...
	replicator
		...
		var/reagents/converting = null
		var/action/bar/blob_replicator/progress = new
		...
		proc/set_master_reagent()
			if (master_reagent_id)
				return
			if (reagents && reagents.total_volume)
				master_reagent_id = reagents.get_master_reagent()
				name = "[initial(name)] ([reagents.get_master_reagent_name()])"
		...
		onMove(var/obj/blob/B)
			...
			qdel(B)
			var/trans_amt = reagents.total_volume
			D.reagents = new /reagents(trans_amt)
			D.reagents.my_atom = D
			reagents.trans_to(D, trans_amt)
...
/obj/blob/launcher
	...
	var/firing_range = 7
	var/last_color = null
	var/projectile/slime/current_projectile = new
	...
	Life()
		...
		visible_message("<span style=\"color:red\"><strong>[src] fires slime at [Target]!</strong></span>")
		L.launch()
...
/turf/proc/get_blob_on_this_turf()
	...
	for (var/obj/blob/B in contents)
		return B
