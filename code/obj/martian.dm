...
/obj/crevice/attack_hand(var/mob/user as mob)
	...
	if (id)
		...
		boutput(user, "<span style=\"color:red\"><strong>Something has clamped down on your hand!</strong></span>")
		user.stunned += 10
		spawn (30)
			if (prob(25))
				boutput(user, "<span style=\"color:red\"><strong>You fail to break free!</strong></span>")
				...
				qdel(user)
				sleep(30)
				playsound(loc, "sound/misc/burp_alien.ogg", 50, 1)
				var/obj/decal/cleanable/blood/gibs/gib =new /obj/decal/cleanable/blood/gibs/core( loc )
				gib.streak(dir)
				...
				gib = new limb_type(loc)
				gib.throw_at(get_edge_target_turf(loc, dir), 4, 3)
				icon_state = "crevice1"
