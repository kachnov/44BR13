...
/proc/sell_crate(var/obj/storage/crate/sell_crate)
			...
			if (istype(R, /obj/item/reagent_containers) || istype(R, /mob/living/carbon/human)) //heh
				...
				for (var/uid in patho)
					if (!(uid in QM_CDC.analysis_by_uid))
						var/pathogen/P = patho[uid]
						var/cdc_contact_analysis/D = new
						D.uid = uid
						...
						var/pathogen/copy = unpool(/pathogen)
	...
	var/data/record/account = sell_crate.account
	...
	for (var/obj/M in sell_crate.contents)
		...
		if (isnum(to_sell.Find(M.type)))
			...
			//boutput(world, "<span style=\"color:red\"><strong> HEY I SHOULD BE SELLING [M.type]</strong></span>")
		else
			to_sell[M.type] = amount
			//boutput(world, "<span style=\"color:red\"><strong> [M.type] in the house</strong></span>")
		qdel(M)
	qdel(sell_crate)
	...
	for (var/item_type in to_sell)
		...
		//boutput(world, "<strong>PRESANITIZED SWITCH FOR [item_type]</strong>") ////////////////////////////////////////////////////////
		...
		var/commodity/C = shippingmarket.commodities["[sanitizer]"]
		//boutput(world, "<strong> WE ARE LOOKING FOR [item_type]")
		amount = to_sell[item_type]

		if (C)
			//boutput(world, "<strong> [C] should fetch [C.price]")
			if (C.indemand)
				...
				//boutput(world, "<strong> IN DEMAND: SELLING [amount] of [C] for [C.price] times [shippingmarket.demand_multiplier]</strong>")
			else
				//wagesystem.shipping_budget += amount * C.price
				duckets += amount * C.price
				//boutput(world, "<strong> NOT IN DEMAND: SELLING [amount] of [C] for [C.price]</strong>")
		else
			//wagesystem.shipping_budget += amount
			duckets += amount
			//boutput(world, "<strong> [C] NEVER SOLD RIGHT, FUCKIN HELL</strong>")
...
/proc/sell_to_trader(var/obj/storage/crate/sellcrate,var/trader/the_trader)
	//return
	...
	for (var/obj/M in sellcrate:contents)
		...
		if (istype(M,shippingmarket.trader:tradegoods))
			...
			sellcount++
			qdel(M)*/
		for (var/commodity/C in the_trader.goods_buy)
			//if (M.type == C.comtype)
...
/proc/buy_thing(var/atom/movable/O as obj|mob)
	...
	spawn (0)
		...
		if (!spawnpoint)
			logTheThing("debug", null, null, "<strong>Shipping: </strong> No spawn turfs found! Can't deliver crate")
			return

		if (!target)
			logTheThing("debug", null, null, "<strong>Shipping: </strong> No target turfs found! Can't deliver crate")
			return
...
/proc/buy_from_trader(var/trader/the_trader)
	...
	spawn (0)
		...
		if (sanity == 30)
			logTheThing("debug", null, null, "<strong>Shipping: </strong> Timed out waiting to fire crate, what a bummer!")
		...
		var/total_price = 0
		for (var/commodity/trader/C in the_trader.shopping_cart)
...
/proc/process_supply_order(var/supply_order/SO)
	//could be multiple possible spawn turfs
	var/turf/spawnpoint
	for (var/turf/T in get_area_turfs(/area/supply/spawn_point))
		spawnpoint = T
