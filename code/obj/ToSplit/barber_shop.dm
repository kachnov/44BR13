...
/obj/item/reagent_containers/food/drinks/hairgrowth
	...
	on_reagent_change()
		icon_state = "tonic[reagents.total_volume ? "1" : "0"]"
...
/obj/item/dye_bottle/attack(mob/living/carbon/human/M as mob, mob/user as mob)
	...
	if (empty)
		...
		*/
		...
		M.bioHolder.mobAppearance.customization_first_color = customization_first_color
		M.bioHolder.mobAppearance.customization_second_color = customization_first_color
...
/obj/item/razor_blade/attack(mob/living/carbon/human/M as mob, mob/user as mob)
	...
	if (M.cust_two_state == "wiz")
		if (user == M)
			...
			return
		visible_message("<span style=\"color:red\"><strong>[user]</strong> quickly shaves off [M]'s beard!</span>")
	...
	if (istype(M.buckled, /obj/stool/barber_chair))
		...
		//playsound(loc, "Scissor.ogg", 100, 1)
		sleep(70)
		if (M.loc == T && TM.loc == user.loc  && (user.equipped() == src || istype(user, /mob/living/silicon)))
			return
...
/obj/machinery/hair_dye_dispenser
	...
	attack_hand(mob/user as mob)
		...
		var/dat = "<TT><strong>Dye Bottle Dispenser Unit</strong><BR><HR><BR>"
