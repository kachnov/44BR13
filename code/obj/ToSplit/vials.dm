...
/obj/item/reagent_containers/glass/vial
	...
	New()
		var/reagents/R = new/reagents(30)
		reagents = R
		R.my_atom = src
...
/obj/item/reagent_containers/glass/vial/proc/shatter()
	...
	var/atom/sourceloc = get_turf(loc)
	src = null
	qdel(A)
	...
	if (istype(contained,/ailment))
		for (var/mob/living/carbon/H in view(5, sourceloc))
			H.contract_disease(contained,null,null,0)
		var/i
		for (i=0, i<5, i++)
			for (var/mob/living/carbon/H in view(5, sourceloc))
				H.contract_disease(contained,null,null,0)
...
/obj/item/reagent_containers/glass/vial/proc/drink(user)
	...
	if (istype(contained,/ailment))
		...
		switch(pick(1,2))
			if (1)
				M.weakened += 5
				M.contract_disease(contained,null,null,1)
			if (2)
				spawn (200)
					M.contract_disease(contained,null,null,1)
	return
...
/mob/proc/drop_vial()
	var/obj/item/reagent_containers/glass/vial/W = equipped()
	if (W)
		u_equip(W)
		if (W)
			W.set_loc(loc)
...
/obj/testtuberack
	...
	attack_hand(mob/user as mob)
		if (contents.len > 0)
			...
			var/obj/item/reagent_containers/glass/vial/V = pick(contents)
			contents -= V
			V.set_loc(loc)
		else
