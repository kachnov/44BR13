...
/text_roamer
	...
	proc
		...
		in_word()
			if (prev_char != "" && prev_char != " " && next_char != "" && next_char != " ") return TRUE
			else return FALSE
...
/proc/elvis_parse(var/text_roamer/R)
	var/new_string = ""
	var/used = 0
...
/proc/borkborkbork_parse(var/text_roamer/R)
	var/new_string = ""
	var/used = 0
...
/proc/tommy_parse(var/text_roamer/R)
	var/S = R.curr_char
	var/new_string = ""
	var/used = 1
...
/proc/tommify(var/string)
	...
	for (var/i = 0, i < length(string), i=i)
		var/parse_result/P = tommy_parse(T)
		modded += P.string
		i += P.chars_used
...
/proc/borkborkbork(var/string)
	...
	for (var/i = 0, i < length(string), i=i)
		var/parse_result/P = borkborkbork_parse(T)
		modded += P.string
		i += P.chars_used
		T.curr_char_pos = T.curr_char_pos + P.chars_used
		T.update()
...
/proc/elvisfy(var/string)
	...
	for (var/i = 0, i < length(string), i=i)
		var/parse_result/P = elvis_parse(T)
		modded += P.string
		i += P.chars_used
		T.curr_char_pos = T.curr_char_pos + P.chars_used
		T.update()
...
/proc/voidSpeak(var/message) // sharing the creepiness with everyone!!
	...
	return "<em><strong>[processedMessage]</strong></em>"
