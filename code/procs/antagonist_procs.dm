/client/proc/gearspawn_traitor()
	...
	if (usr.mind && istype(usr.mind))
		...
		usr.mind.store_memory("<strong>Uplink password:</strong> [U.lock_code].")
...
/proc/equip_traitor(mob/living/carbon/human/traitor_mob)
	...
	if (!R)
		...
		if (!(ticker && ticker.mode && istype(ticker.mode, /game_mode/revolution)) && !(traitor_mob.mind && traitor_mob.mind.special_role == "Spy"))
		...
		if (istype(R, /obj/item/device/radio))
			...
			traitor_mob.mind.store_memory("<strong>Radio Freq:</strong> [format_frequency(freq)] ([RR.name] [loc]).")
			...
			traitor_mob.mind.store_memory("<strong>Set your ringtone to:</strong> [pda_pass] (In the Messenger menu in the [P.name] [loc]).")
