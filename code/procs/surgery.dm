...
/obj/item/proc/surgeryConfusion(var/mob/living/carbon/human/patient as mob, var/mob/surgeon as mob, var/damage as num)
	...
	if (prob(33)) // if they REALLY fuck up
		...
		patient.tri_message("<span style=\"color:red\"><strong>[surgeon]</strong> makes a [fluff]cut into [patient]'s [target_area] with [src]!</span>",\
		patient, "<span style=\"color:red\"><strong>[surgeon]</strong> makes a [fluff]cut into your [target_area] with [src]!</span>",\
		...
		patient.visible_message("<span style=\"color:red\"><strong>Blood gushes from the incision!</strong> That can't have been the correct thing to do!</span>")
		return
		...
		patient.tri_message("<span style=\"color:red\"><strong>[surgeon]</strong> [fluff][fluff2]s at [patient]'s [target_area] with [src].[fluff3]</span>",\
		patient, "<span style=\"color:red\"><strong>[surgeon]</strong> [fluff][fluff2]s at your [target_area] with [src].[fluff3]</span>",\
...
/proc/calc_screw_up_prob(var/mob/living/carbon/human/patient as mob, var/mob/surgeon as mob, var/screw_up_prob = 25)
	...
	DEBUG_MESSAGE("<strong>[patient]'s surgery (performed by [surgeon]) has screw_up_prob set to [screw_up_prob]</strong>")
...
/obj/item/proc/remove_bandage(var/mob/living/carbon/human/H as mob, var/mob/user as mob)
	...
	H.tri_message("<span style=\"color:blue\"><strong>[user]</strong> begins removing [H == user ? "[his_or_her(H)]" : "[H]'s"] bandage.</span>",\
	user, "<span style=\"color:blue\">You begin removing [H == user ? "your" : "[H]'s"] bandage.</span>",\
	H, "<span style=\"color:blue\">[H == user ? "You begin" : "<strong>[user]</strong> begins"] removing your bandage.</span>")

	if (!do_mob(user, H, 50))
		user.show_text("You were interrupted!", "red")
		return TRUE

	H.tri_message("<span style=\"color:blue\"><strong>[user]</strong> removes [H == user ? "[his_or_her(H)]" : "[H]'s"] bandage.</span>",\
	user, "<span style=\"color:blue\">You remove [H == user ? "your" : "[H]'s"] bandage.</span>",\
	H, "<span style=\"color:blue\">[H == user ? "You remove" : "<strong>[user]</strong> removes"] your bandage.</span>")
...
/mob/living/carbon/human/get_surgery_status(var/zone)
	...
	if (!zone || zone == "chest")
		...
		else if (butt_op_stage < 5)
			return_thing += butt_op_stage

	if (!zone || zone in list("l_arm","r_arm","l_leg","r_leg"))
		var/obj/item/parts/surgery_limb = limbs.vars[zone]
...
/obj/item/proc/defibrillate(var/mob/living/carbon/human/patient as mob, var/mob/living/user as mob, var/emagged = 0, var/faulty = 0, var/obj/item/cell/cell = null)
	...
	var/shockcure = 0
	for (var/ailment_data/V in patient.ailments)
		if (V.cure == "Electric Shock")
			shockcure = 1
			break

	if (emagged || (patient.health < 0 && !faulty) || (shockcure && !faulty) || (faulty && prob(25)))
		user.visible_message("<span style=\"color:blue\"><strong>[user]</strong> shocks [patient] with [src].</span>",\
		...
		if (patient.bioHolder.HasEffect("resist_electric"))
			user.show_text("<strong>[patient]</strong> doesn't respond at all!", "red")
			...
			if (patient.health < 0)
				if (sumdamage >= 90)
					user.show_text("<strong>[patient]</strong> looks horribly injured. Resuscitation alone may not help revive them.", "red")
				if (prob(66))
					user.show_text("<strong>[patient]</strong> inhales deeply!", "blue")
					patient.take_oxygen_deprivation(-50)
					patient.updatehealth()
				else
					user.show_text("<strong>[patient]</strong> doesn't respond!", "red")
			...
			if (emagged && !faulty && prob(10))
				...
				patient.contract_disease(/ailment/disease/flatline, null, null, 1) // path, name, strain, bypass resist
			return TRUE
...
/obj/item/proc/scalpel_surgery(var/mob/living/carbon/human/patient as mob, var/mob/living/surgeon as mob)
	...
	if (surgeon.bioHolder.HasEffect("clumsy") && prob(50))
		surgeon.visible_message("<span style=\"color:red\"><strong>[surgeon]</strong> fumbles and stabs [him_or_her(surgeon)]self in the eye with [src]!</span>", \
	...
	var/damage_low = calc_surgery_damage(surgeon, screw_up_prob, rand(5,15)/*, adj1, adj2*/)
	var/damage_high = calc_surgery_damage(surgeon, screw_up_prob, rand(15,25)/*, adj1, adj2*/)

	DEBUG_MESSAGE("<strong>[patient]'s surgery (performed by [surgeon]) damage_low is [damage_low], damage_high is [damage_high]</strong>")
		...
		if (surgeon.a_intent == INTENT_HARM && patient.organHolder.head)
			if (patient.organHolder.head.op_stage == 0.0)
				...
				if (prob(screw_up_prob))
					surgeon.visible_message("<span style=\"color:red\"><strong>[surgeon][fluff]!</strong></span>")
					patient.TakeDamage("head", damage_low, 0)
					take_bleeding_damage(patient, surgeon, damage_low)
					return TRUE

				patient.tri_message("<span style=\"color:red\"><strong>[surgeon]</strong> cuts the skin of [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] neck open with [src]!</span>",\
				surgeon, "<span style=\"color:red\">You cut the skin of [surgeon == patient ? "your" : "[patient]'s"] neck open with [src]!</span>", \
				patient, "<span style=\"color:red\">[patient == surgeon ? "You cut" : "<strong>[surgeon]</strong> cuts"] the skin of your neck open with [src]!</span>")
				...
				if (prob(screw_up_prob))
					surgeon.visible_message("<span style=\"color:red\"><strong>[surgeon][fluff2]!</strong></span>")
					patient.TakeDamage("head", damage_high, 0)
					take_bleeding_damage(patient, surgeon, damage_high)
					return TRUE

				patient.tri_message("<span style=\"color:red\"><strong>[surgeon]</strong> slices the tissue around [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] spine with [src]!</span>",\
				surgeon, "<span style=\"color:red\">You slice the tissue around [surgeon == patient ? "your" : "[patient]'s"] spine with [src]!</span>",\
				patient, "<span style=\"color:red\">[patient == surgeon ? "You slice" : "<strong>[surgeon]</strong> slices"] the tissue around your spine with [src]!</span>")
			...
			patient.tri_message("<span style=\"color:red\"><strong>[surgeon]</strong> cuts away the flesh holding [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] right eye in with [src]!</span>",\
			surgeon, "<span style=\"color:red\">You cut away the flesh holding [surgeon == patient ? "your" : "[patient]'s"] right eye in with [src]!</span>", \
			patient, "<span style=\"color:red\">[patient == surgeon ? "You cut" : "<strong>[surgeon]</strong> cuts"] away the flesh holding your right eye in with [src]!</span>")
			...
			patient.tri_message("<span style=\"color:red\"><strong>[surgeon]</strong> cuts away the flesh holding [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] left eye in with [src]!</span>",\
			surgeon, "<span style=\"color:red\">You cut away the flesh holding [surgeon == patient ? "your" : "[patient]'s"] left eye in with [src]!</span>", \
			patient, "<span style=\"color:red\">[patient == surgeon ? "You cut" : "<strong>[surgeon]</strong> cuts"] away the flesh holding your left eye in with [src]!</span>")
			...
			if (patient.organHolder.brain.op_stage == 0.0)
				...
				patient.tri_message("<span style=\"color:red\"><strong>[surgeon]</strong> cuts [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] head open with [src]!</span>",\
				surgeon, "<span style=\"color:red\">You cut [surgeon == patient ? "your" : "[patient]'s"] head open with [src]!</span>", \
				patient, "<span style=\"color:red\">[patient == surgeon ? "You cut" : "<strong>[surgeon]</strong> cuts"] your head open with [src]!</span>")
				...
				patient.tri_message("<span style=\"color:red\"><strong>[surgeon]</strong> removes the connections to [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] brain with [src]!</span>",\
				...
				patient, "<span style=\"color:red\">[patient == surgeon ? "You remove" : "<strong>[surgeon]</strong> removes"] the connections to your brain with [src]!</span>")
				...
				surgeryConfusion(patient, surgeon, damage_high)
				return TRUE
			...
			if (patient.organHolder.skull.op_stage == 0.0)
				...
				patient.tri_message("<span style=\"color:red\"><strong>[surgeon]</strong> cuts [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] skull away from the skin with [src]!</span>",\
				surgeon, "<span style=\"color:red\">You cut [surgeon == patient ? "your" : "[patient]'s"] skull away from the skin with [src]!</span>",\
				patient, "<span style=\"color:red\">[patient == surgeon ? "You cut" : "<strong>[surgeon]</strong> cuts"] your skull away from the skin with [src]!</span>")
...
/* ---------- SCALPEL - BUTT ---------- */
		...
		if (patient.butt_op_stage == 0.0)
			...
			patient.tri_message("<span style=\"color:red\"><strong>[surgeon]</strong> cuts [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] butt open with [src]!</span>",\
			surgeon, "<span style=\"color:red\">You cut [surgeon == patient ? "your" : "[patient]'s"] butt open with [src]!</span>",\
			patient, "<span style=\"color:red\">[patient == surgeon ? "You cut" : "<strong>[surgeon]</strong> cuts"] your butt open with [src]!</span>")
			...
			patient.tri_message("<span style=\"color:red\"><strong>[surgeon]</strong> severs [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] intestines with [src]!</span>",\
			surgeon, "<span style=\"color:red\">You sever [surgeon == patient ? "your" : "[patient]'s"] intestines with [src]!</span>",\
			patient, "<span style=\"color:red\">[patient == surgeon ? "You sever" : "<strong>[surgeon]</strong> severs"] your intestines with [src]!</span>")
...
/* ---------- SCALPEL - IMPLANT ---------- */
		...
		if (patient.implant.len > 0)
			playsound(get_turf(patient), "sound/weapons/squishcut.ogg", 50, 1)
			patient.tri_message("<span style=\"color:red\"><strong>[surgeon]</strong> cuts into [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] chest with [src]!</span>",\
			surgeon, "<span style=\"color:red\">You cut into [surgeon == patient ? "your" : "[patient]'s"] chest with [src]!</span>",\
			patient, "<span style=\"color:red\">[patient == surgeon ? "You cut" : "<strong>[surgeon]</strong> cuts"] into your chest with [src]!</span>")

			var/attempted_parasite_removal = 0
			for (var/ailment_data/an_ailment in patient.ailments)
			...
			for (var/obj/item/implant/projectile/I in patient.implant)
				patient.tri_message("<span style=\"color:red\"><strong>[surgeon]</strong> cuts out \an [I] from [patient == surgeon ? "[him_or_her(patient)]self" : "[patient]"] with [src]!</span>",\
				surgeon, "<span style=\"color:red\">You cut out \an [I] from [surgeon == patient ? "yourself" : "[patient]"] with [src]!</span>",\
				patient, "<span style=\"color:red\">[patient == surgeon ? "You cut" : "<strong>[surgeon]</strong> cuts"] out \an [I] from you with [src]!</span>")
			...
			for (var/obj/item/implant/I in patient.implant)
				...
				patient.tri_message("<span style=\"color:red\"><strong>[surgeon]</strong> cuts out an implant from [patient == surgeon ? "[him_or_her(patient)]self" : "[patient]"] with [src]!</span>",\
				surgeon, "<span style=\"color:red\">You cut out an implant from [surgeon == patient ? "yourself" : "[patient]"] with [src]!</span>",\
				patient, "<span style=\"color:red\">[patient == surgeon ? "You cut" : "<strong>[surgeon]</strong> cuts"] out an implant from you with [src]!</span>")
...
/* ---------- SCALPEL - HEART ---------- */
			...
			if (patient.organHolder.heart.op_stage == 0.0)
				...
				patient.tri_message("<span style=\"color:red\"><strong>[surgeon]</strong> cuts [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] chest open with [src]!</span>",\
				surgeon, "<span style=\"color:red\">You cut [surgeon == patient ? "your" : "[patient]'s"] chest open with [src]!</span>",\
				patient, "<span style=\"color:red\">[patient == surgeon ? "You cut" : "<strong>[surgeon]</strong> cuts"] your chest open with [src]!</span>")
				...
				patient.tri_message("<span style=\"color:red\"><strong>[surgeon]</strong> cuts [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] aorta and vena cava with [src]!</span>",\
				surgeon, "<span style=\"color:red\">You cut [surgeon == patient ? "your" : "[patient]'s"] aorta and vena cava with [src]!</span>",\
				patient, "<span style=\"color:red\">[patient == surgeon ? "You cut" : "<strong>[surgeon]</strong> cuts"] your aorta and vena cava with [src]!</span>")
...
/obj/item/proc/saw_surgery(var/mob/living/carbon/human/patient as mob, var/mob/surgeon as mob)
	...
	if (surgeon.bioHolder.HasEffect("clumsy") && prob(50))
		surgeon.visible_message("<span style=\"color:red\"><strong>[surgeon]</strong> mishandles [src] and cuts [him_or_her(surgeon)]self!</span>",\
	...
	var/damage_low = calc_surgery_damage(surgeon, screw_up_prob, rand(10,20)/*, adj1, adj2*/)
	var/damage_high = calc_surgery_damage(surgeon, screw_up_prob, rand(20,30)/*, adj1, adj2*/)
	...
	if (surgeon.zone_sel.selecting == "head")
		...
		if (surgeon.a_intent == INTENT_HARM && patient.organHolder.head)
			if (patient.organHolder.head.op_stage == 1.0)
				...
				patient.tri_message("<span style=\"color:red\"><strong>[surgeon]</strong> severs most of [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] neck with [src]!</span>",\
				surgeon, "<span style=\"color:red\">You sever most of [surgeon == patient ? "your" : "[patient]'s"] neck with [src]!</span>",\
				patient, "<span style=\"color:red\">[patient == surgeon ? "You sever" : "<strong>[surgeon]</strong> severs"] most of your neck with [src]!</span>")
				...
				patient.tri_message("<span style=\"color:red\"><strong>[surgeon]</strong> saws through the last of [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] head's connections to [surgeon == patient ? "[his_or_her(patient)]" : "[patient]'s"] body with [src]!</span>",\
				...
				patient, "<span style=\"color:red\">[patient == surgeon ? "You saw" : "<strong>[surgeon]</strong> saws"] through the last of your head's connection to your body with [src]!</span>")
			...
			if (patient.organHolder.brain.op_stage == 1.0)
				...
				patient.tri_message("<span style=\"color:red\"><strong>[surgeon]</strong> saws open [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] skull with [src]!</span>",\
				surgeon, "<span style=\"color:red\">You saw open [surgeon == patient ? "your" : "[patient]'s"] skull with [src]!</span>",\
				patient, "<span style=\"color:red\">[patient == surgeon ? "You saw" : "<strong>[surgeon]</strong> saws"] open your skull with [src]!</span>")
				...
				patient.tri_message("<span style=\"color:red\"><strong>[surgeon]</strong> severs [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] brain's connection to the spine with [src]!</span>",\
				surgeon, "<span style=\"color:red\">You sever [surgeon == patient ? "your" : "[patient]'s"] brain's connection to the spine with [src]!</span>",\
				patient, "<span style=\"color:red\">[patient == surgeon ? "You sever" : "<strong>[surgeon]</strong> severs"] your brain's connection to the spine with [src]!</span>")
			...
			if (patient.organHolder.skull.op_stage == 1.0)
				...
				patient.tri_message("<span style=\"color:red\"><strong>[surgeon]</strong> saws [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] skull out with [src]!</span>",\
				surgeon, "<span style=\"color:red\">You saw [surgeon == patient ? "your" : "[patient]'s"] skull out with [src]!</span>",\
				patient, "<span style=\"color:red\">[patient == surgeon ? "You saw" : "<strong>[surgeon]</strong> saws"] your skull out with [src]!</span>")

				patient.visible_message("<span style=\"color:red\"><strong>[patient]</strong>'s head collapses into a useless pile of skin with no skull to keep it in its proper shape!</span>",\
...
/* ---------- SAW - BUTT ---------- */
		...
		switch (patient.butt_op_stage)
			if (1.0)
				...
				patient.tri_message("<span style=\"color:red\"><strong>[surgeon]</strong> saws open [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] butt with [src]!</span>",\
				surgeon, "<span style=\"color:red\">You saw open [surgeon == patient ? "your" : "[patient]'s"] butt with [src]!</span>",\
				patient, "<span style=\"color:red\">[patient == surgeon ? "You saw" : "<strong>[surgeon]</strong> saws"] open your butt with [src]!</span>")
			...
			if (3.0)
				...
				patient.tri_message("<span style=\"color:red\"><strong>[surgeon]</strong> severs [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] butt's connection to the stomach with [src]!</span>",\
				surgeon , "<span style=\"color:red\">You sever [surgeon == patient ? "your" : "[patient]'s"] butt's connection to the stomach with [src]!</span>",\
				patient, "<span style=\"color:red\">[patient == surgeon ? "You sever" : "<strong>[surgeon]</strong> severs"] your butt's connection to the stomach with [src]!</span>")
		...
		if (patient.organHolder.heart)
			switch (patient.organHolder.heart.op_stage)
				if (1.0)
					...
					patient.tri_message("<span style=\"color:red\"><strong>[surgeon]</strong> saws open [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] ribcage with [src]!</span>",\
					surgeon, "<span style=\"color:red\">You saw open [surgeon == patient ? "your" : "[patient]'s"] ribcage with [src]!</span>",\
					patient, "<span style=\"color:red\">[patient == surgeon ? "You saw" : "<strong>[surgeon]</strong> saws"] open your ribcage with [src]!</span>")
				...
				if (3.0)
					...
					patient.tri_message("<span style=\"color:red\"><strong>[surgeon]</strong> cuts out [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] heart with [src]!</span>",\
					surgeon, "<span style=\"color:red\">You cut out [surgeon == patient ? "your" : "[patient]'s"] heart with [src]!</span>",\
					patient, "<span style=\"color:red\">[patient == surgeon ? "You cut" : "<strong>[surgeon]</strong> cuts"] out your heart with [src]!</span>")
					...
					//patient.contract_disease(/ailment/disease/noheart,null,null,1)
...
/obj/item/proc/suture_surgery(var/mob/living/carbon/human/patient as mob, var/mob/surgeon as mob)
	...
	if (surgeon.bioHolder.HasEffect("clumsy") && prob(33))
		surgeon.visible_message("<span style=\"color:red\"><strong>[surgeon]</strong> pricks [his_or_her(surgeon)] finger with [src]!</span>",\
...
/* ---------- SUTURE - HEAD ---------- */
	...
	if (surgeon.zone_sel.selecting == "head")
		if (patient.organHolder && patient.organHolder.head && patient.organHolder.head.op_stage > 0.0)
			patient.tri_message("<span style=\"color:blue\"><strong>[surgeon]</strong> sews the incision on [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] neck closed with [src].</span>",\
			surgeon, "<span style=\"color:blue\">You sew the incision on [surgeon == patient ? "your" : "[patient]'s"] neck closed with [src].</span>",\
			patient, "<span style=\"color:blue\">[patient == surgeon ? "You sew" : "<strong>[surgeon]</strong> sews"] the incision on your neck closed with [src].</span>")
			...
			patient.tri_message("<span style=\"color:blue\"><strong>[surgeon]</strong> sews the incision on [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] head closed with [src].</span>",\
			surgeon, "<span style=\"color:blue\">You sew the incision on [surgeon == patient ? "your" : "[patient]'s"] head closed with [src].</span>",\
			patient, "<span style=\"color:blue\">[patient == surgeon ? "You sew" : "<strong>[surgeon]</strong> sews"] the incision on your head closed with [src].</span>")
			...
			patient.tri_message("<span style=\"color:blue\"><strong>[surgeon]</strong> sews the incision in [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] right eye socket closed with [src].</span>",\
			...
			patient, "<span style=\"color:blue\">[patient == surgeon ? "You sew" : "<strong>[surgeon]</strong> sews"] the incision in your right eye socket closed with [src].</span>")
			...
			patient.tri_message("<span style=\"color:blue\"><strong>[surgeon]</strong> sews the incision in [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] left eye socket closed with [src].</span>",\
			...
			patient, "<span style=\"color:blue\">[patient == surgeon ? "You sew" : "<strong>[surgeon]</strong> sews"] the incision in your left eye socket closed with [src].</span>")
			...
			patient.tri_message("<span style=\"color:blue\"><strong>[surgeon]</strong> sews [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] wounds closed with [src].</span>",\
			surgeon, "You sew [surgeon == patient ? "your" : "[patient]'s"] wounds closed with [src].",\
			patient, "<span style=\"color:blue\">[patient == surgeon ? "You sew" : "<strong>[surgeon]</strong> sews"] your wounds closed with [src].</span>")
...
/* ---------- SUTURE - CHEST ---------- */
		...
		if (patient.organHolder && patient.organHolder.heart && patient.organHolder.heart.op_stage > 0.0)
			patient.tri_message("<span style=\"color:blue\"><strong>[surgeon]</strong> sews the incision on [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] chest closed with [src].</span>",\
			surgeon, "<span style=\"color:blue\">You sew the incision on [surgeon == patient ? "your" : "[patient]'s"] chest closed with [src].</span>",\
			patient, "<span style=\"color:blue\">[patient == surgeon ? "You sew" : "<strong>[surgeon]</strong> sews"] the incision on your chest closed with [src].</span>")
			...
			patient.tri_message("<span style=\"color:blue\"><strong>[surgeon]</strong> sews the incision on [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] butt closed with [src].</span>",\
			surgeon, "<span style=\"color:blue\">You sew the incision on [surgeon == patient ? "your" : "[patient]'s"] butt closed with [src].</span>",\
			patient, "<span style=\"color:blue\">[patient == surgeon ? "You sew" : "<strong>[surgeon]</strong> sews"] the incision on your butt closed with [src].</span>")
...
// okay I gotta make this proc work differently than the others because holy shit all those return TRUE/return FALSEs are driving me batty
...
/obj/item/proc/cautery_surgery(var/mob/living/carbon/human/patient as mob, var/mob/surgeon as mob, var/damage as num, var/lit = 1)
	...
	if (patient.is_heat_resistant())
		patient.visible_message("<span style=\"color:red\"><strong>Nothing happens!</strong></span>")
		return FALSE
	...
	if (surgeon.bioHolder.HasEffect("clumsy") && prob(33))
		surgeon.visible_message("<span style=\"color:red\"><strong>[surgeon]</strong> burns [him_or_her(surgeon)]self with [src]!</span>",\
...
/* ---------- CAUTERY - HEAD ---------- */
	...
	if (surgeon.zone_sel.selecting == "head" && patient.organHolder && patient.organHolder.head && patient.organHolder.head.op_stage > 0.0)
		...
		if (!lit)
			patient.tri_message("<strong>[surgeon]</strong> tries to use [src] on [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] incision, but [src] isn't lit! Sheesh.",\
			surgeon, "You try to use [src] on [surgeon == patient ? "your" : "[patient]'s"] incision, but [src] isn't lit! Sheesh.",\
			patient, "[patient == surgeon ? "You try" : "<strong>[surgeon]</strong> tries"] to use [src] on your incision, but [src] isn't lit! Sheesh.")
			return FALSE
		...
		if (quick_surgery)
			patient.tri_message("<span style=\"color:blue\"><strong>[surgeon]</strong> cauterizes the incision on [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] neck closed with [src].</span>",\
			surgeon, "<span style=\"color:blue\">You cauterize the incision on [surgeon == patient ? "your" : "[patient]'s"] neck closed with [src].</span>",\
			patient, "<span style=\"color:blue\">[patient == surgeon ? "You cauterize" : "<strong>[surgeon]</strong> cauterizes"] the incision on your neck closed with [src].</span>")
			...
			patient.tri_message("<span style=\"color:blue\"><strong>[surgeon]</strong> begins cauterizing the incision on [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] neck closed with [src].</span>",\
			...
			patient, "<span style=\"color:blue\">[patient == surgeon ? "You begin" : "<strong>[surgeon]</strong> begins"] cauterizing incision on your neck closed with [src].</span>")
			...
			if (do_mob(patient, surgeon, max(100 - (damage * 2)), 0))
				...
				surgeon.show_text("<strong>You were interrupted!</strong>", "red")
				return TRUE
...
/* ---------- CAUTERY - BUTT ---------- */
		...
		if (quick_surgery)
			patient.tri_message("<span style=\"color:blue\"><strong>[surgeon]</strong> cauterizes the incision on [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] butt closed with [src].</span>",\
			surgeon, "<span style=\"color:blue\">You cauterize the incision on [surgeon == patient ? "your" : "[patient]'s"] butt closed with [src].</span>",\
			patient, "<span style=\"color:blue\">[patient == surgeon ? "You cauterize" : "<strong>[surgeon]</strong> cauterizes"] the incision on your butt closed with [src].</span>")
			...
			patient.tri_message("<span style=\"color:blue\"><strong>[surgeon]</strong> begins cauterizing the incision on [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] butt closed with [src].</span>",\
			...
			patient, "<span style=\"color:blue\">[patient == surgeon ? "You begin" : "<strong>[surgeon]</strong> begins"] cauterizing incision on your butt closed with [src].</span>")
...
/* ---------- CAUTERY - BLEEDING ---------- */
		...
		if (!lit)
			patient.tri_message("<strong>[surgeon]</strong> tries to use [src] on [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] wounds, but [src] isn't lit! Sheesh.",\
			surgeon, "You try to use [src] on [surgeon == patient ? "your" : "[patient]'s"] wounds, but [src] isn't lit! Sheesh.",\
			patient, "[patient == surgeon ? "You try" : "<strong>[surgeon]</strong> tries"] to use [src] on your wounds, but [src] isn't lit! Sheesh.")
			return TRUE
		...
		if (quick_surgery)
			patient.tri_message("<span style=\"color:blue\"><strong>[surgeon]</strong> cauterizes [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] wounds closed with [src].</span>",\
			surgeon, "<span style=\"color:blue\">You cauterize [surgeon == patient ? "your" : "[patient]'s"] wounds closed with [src].</span>",\
			patient, "<span style=\"color:blue\">[patient == surgeon ? "You cauterizes" : "<strong>[surgeon]</strong> cauterizes"] your wounds closed with [src].</span>")

			repair_bleeding_damage(patient, 100, 10)
			return TRUE

		else
			patient.tri_message("<span style=\"color:blue\"><strong>[surgeon]</strong> begins cauterizing [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] wounds closed with [src].</span>",\
			surgeon, "<span style=\"color:blue\">You begin cauterizing [surgeon == patient ? "your" : "[patient]'s"] wounds closed with [src].</span>",\
			patient, "<span style=\"color:blue\">[patient == surgeon ? "You begin" : "<strong>[surgeon]</strong> begins"] cauterizing your wounds closed with [src].</span>")

			if (do_mob(patient, surgeon, max((patient.bleeding * 10) - (damage * 2), 0)))
				patient.tri_message("<span style=\"color:blue\"><strong>[surgeon]</strong> cauterizes [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] wounds closed with [src].</span>",\
				surgeon, "<span style=\"color:blue\">You cauterize [surgeon == patient ? "your" : "[patient]'s"] wounds closed with [src].</span>",\
				patient, "<span style=\"color:blue\">[patient == surgeon ? "You cauterize" : "<strong>[surgeon]</strong> cauterizes"] your wounds closed with [src].</span>")
...
/* ---------- SPOON - RIGHT EYE ---------- */
		...
		if (surgeon.find_in_hand(src) == surgeon.r_hand && patient.organHolder.right_eye)
			if (patient.organHolder.right_eye.op_stage == 0.0)
				...
				patient.tri_message("<span style=\"color:red\"><strong>[surgeon]</strong> inserts [src] into [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] right eye socket!</span>",\
				surgeon, "<span style=\"color:red\">You insert [src] into [surgeon == patient ? "your" : "[patient]'s"] right eye socket!</span>", \
				patient, "<span style=\"color:red\">[patient == surgeon ? "You insert" : "<strong>[surgeon]</strong> inserts"] [src] into your right eye socket!</span>")
				...
				patient.tri_message("<span style=\"color:red\"><strong>[surgeon]</strong> removes [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] right eye with [src]!</span>",\
				surgeon, "<span style=\"color:red\">You remove [surgeon == patient ? "your" : "[patient]'s"] right eye with [src]!</span>",\
				patient, "<span style=\"color:red\">[patient == surgeon ? "You remove" : "<strong>[surgeon]</strong> removes"] your right eye with [src]!</span>")
...
/* ---------- SPOON - LEFT EYE ---------- */
			...
			if (patient.organHolder.left_eye.op_stage == 0.0)
				...
				patient.tri_message("<span style=\"color:red\"><strong>[surgeon]</strong> inserts [src] into [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] left eye socket!</span>",\
				surgeon, "<span style=\"color:red\">You insert [src] into [surgeon == patient ? "your" : "[patient]'s"] left eye socket!</span>", \
				patient, "<span style=\"color:red\">[patient == surgeon ? "You insert" : "<strong>[surgeon]</strong> inserts"] [src] into your left eye socket!</span>")
				...
				patient.tri_message("<span style=\"color:red\"><strong>[surgeon]</strong> removes [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] left eye with [src]!</span>",\
				surgeon, "<span style=\"color:red\">You remove [surgeon == patient ? "your" : "[patient]'s"] left eye with [src]!</span>",\
				patient, "<span style=\"color:red\">[patient == surgeon ? "You remove" : "<strong>[surgeon]</strong> removes"] your left eye with [src]!</span>")
