...
/proc/statlog_ticket(var/ticket/T, var/mob/living/M)
	var/message[] = new()
...
/proc/statlog_fine(var/fine/F, var/mob/living/M)
	var/message[] = new()
...
/proc/statlog_traitors()
	var/list/mind/traitors = get_all_enemies()
	...
	for (var/mind/M in traitors)
		...
		switch(traitor_type)
			if ("changeling")
				if (M.current)
					var/abilityHolder/changeling/C = M.current.get_ability_holder(/abilityHolder/changeling)
					if (C && istype(C))
			...
			if ("wizard")
				if (M.current)
					var/abilityHolder/wizard/W = M.current.get_ability_holder(/abilityHolder/wizard)
					if (W && istype(W))
						var/spells = ""
						for (var/targetable/spell/S in W.abilities)
							if (spells != "")
								spells += ", "
							spells += S.name
			if ("werewolf")
				for (var/objective/specialist/werewolf/feed/O in M.objectives)
