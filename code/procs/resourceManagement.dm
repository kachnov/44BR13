...
/proc/recursiveFileLoader(dir)
	for (var/i in flist(dir))
		//logTheThing("wiredebug", "<strong>LOADER:</strong> [i]")
...
/client/proc/loadResources()
	if ((CDN_ENABLED && config.env == "prod") || resourcesLoaded) return FALSE
	boutput(src, "<span style='color: blue;'><strong>Resources are now loading, browser windows will open normally when complete.</strong></span>")

	loadResourcesFromList(localResources)
