...
/proc/holy_jam()
	...
	for (var/mob/N in AIviewers(M, null))
		if (get_dist(N, M) <= 6)
			if (N != M)
				N.apply_flash(30, 5)
				if (ishuman(N) && istype(N:mutantrace, /mutantrace/zombie))
					N.gib()
		if (N.client)
			shake_camera(N, 6, 4)
...
/proc/clown_jam(mob/living/target as mob in oview(6))
	...
	if (target.job != "Clown")
		boutput(target, "<span style=\"color:red\"><strong>You HONK painfully!</strong></span>")
		target.take_brain_damage(80)
		target.stuttering = 120
		target.job = "Clown"
		target.contract_disease(/ailment/disease/cluwneing_around, null, null, 1) // path, name, strain, bypass resist
		target.contract_disease(/ailment/disability/clumsy, null, null, 1) // path, name, strain, bypass resist
		...
		if (!istype(H))
			return
		boutput(H, "<span style=\"color:red\"><strong>You don't feel very funny.</strong></span>")
		H.take_brain_damage(-120)
		H.stuttering = 0
		H.job = "Lawyer"
		H.change_misstep_chance(-INFINITY)
		H.nutrition = 0
		for (var/ailment_data/A in H.ailments)
			if (istype(A.master,/ailment/disability/clumsy))
				H.cure_disease(A)
...
/proc/chaos_dunk()
	...
	logTheThing("combat", M, null, "<strong>triggers a chaos dunk in [M.loc.loc] ([showCoords(M.x, M.y, M.z)])!</strong>")
...
/obj/item/bball_uplink/attack_self(mob/user as mob)
	...
	if (selfdestruct)
		...
		if (temp)
			...
			dat = "<strong>ZauberTech Baller Uplink Console:</strong><BR>"
			dat += "Tele-Crystals left: [uses]<BR>"
			dat += "<HR>"
			dat += "<strong>Request item:</strong><BR>"
			dat += "<em>Each item costs 1 telecrystal. The number afterwards is the cooldown time.</em><BR>"
...
/obj/item/bball_uplink/Topic(href, href_list)
	...
	if ((usr.contents.Find(src) || (in_range(src,usr) && istype(loc, /turf))))
		...
		if (href_list["spell_nova"])
			if (uses >= 1)
				uses -= 1
				temp = "This jam will cause an eruption of explosive basketballs from your location."
				usr.verbs += /proc/bball_nova
		if (href_list["spell_showboat"])
			if (uses >= 1)
				uses -= 1
				usr.verbs += /proc/showboat_slam
				temp = "Leap up high above your target and slam them for massive damage."
		if (href_list["spell_holy"])
			if (uses >= 1)
				uses -= 1
				usr.verbs += /proc/holy_jam
				temp = "A powerful and sacred jam that blinds surrounding enemies."
		if (href_list["spell_blink"])
			if (uses >= 1)
				uses -= 1
				usr.verbs += /proc/blitz_slam
				temp = "This slam will allow you to teleport randomly at a short distance."
		if (href_list["spell_revengeclown"])
			if (uses >= 1)
				uses -= 1
				usr.verbs += /proc/clown_jam
				temp = "This unspoken jam bamboozles your target to the extent that they will become an obese, idiotic, horrible, and useless clown."
		if (href_list["spell_spin"])
			usr.verbs += /proc/spin
			temp = "This spell lets you do a 360 spin, knocking down any fools tailing you."
/*
