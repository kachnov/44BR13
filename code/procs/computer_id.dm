...
/proc/load_compids(var/ckey)
	...
	var/list/compid_info/cid_list = list()
...
/proc/save_compids(var/ckey, var/list/compid_info/cid_list)
	var/savefile/SF = initialize_compid_savefile()
	var/path = "/[copytext(ckey, 1, 2)]/[ckey]"

	SF.cd = path
	SF.eof = -1
	for (var/compid_info/CI in cid_list)
		SF << CI
...
/proc/check_compid_list(var/client/C)
	...
	for (var/compid_info/CI in C.compid_info_list)
	...
	if (append_CID) //Did not find an entry
		...
		if (C.compid_info_list.len >= 2) //Do they have more than 2 CID's on file? Weird
			...
			var/hits = 0
			for (var/compid_info/CII in C.compid_info_list)
...
/proc/view_client_compid_list(mob/user, var/C)
	...
	var/list/compid_info/cid_list = null
	var/ckey = ""
	if (istype(C, /client))
		var/client/CL = C
