...
/proc/can_act(var/mob/M, var/include_cuffs = 1)
	if (include_cuffs) if (M.handcuffed) return FALSE
	if (M.stunned) return FALSE
	if (M.weakened) return FALSE
	if (M.paralysis) return FALSE
...
/proc/process_accents(var/mob/living/carbon/human/H, var/message)
	...
	var/ailment_data/disease/berserker = H.find_ailment_by_type(/ailment/disease/berserker/)
	if (istype(berserker,/ailment_data/disease) && berserker.stage > 1)
		if (prob(10))
...
/color	//Simple datum for RGBA colours
...
//	if (prob(5)) testmove = 1
	...
	for (var/turf/T in turfs_src)
		refined_src += T
		refined_src[T] = new/coords
		var/coords/C = refined_src[T]
	...
	for (var/turf/T in turfs_trg)
		refined_trg += T
		refined_trg[T] = new/coords
		var/coords/C = refined_trg[T]
	...
	moving:
		for (var/turf/T in refined_src)
			var/coords/C_src = refined_src[T]
			for (var/turf/B in refined_trg)
				var/coords/C_trg = refined_trg[B]
