...
/obj/New()
	...
	if (req_access_txt && req_access_txt != "0")
...
/obj/proc/allowed(mob/M, acceptIfAny=0)
	...
	if (check_access(null))
		return TRUE
	if (M && ismob(M))
		if (check_access(M.equipped(), acceptIfAny))
			return TRUE
		if (ishuman(M))
			var/mob/living/carbon/human/H = M
			//if they are holding or wearing a card that has access, that works
			if (check_access(H.wear_id, acceptIfAny))
				return TRUE
		else if (issilicon(M))
			var/mob/living/silicon/S = M
			if (check_access(S.botcard, acceptIfAny))
				return TRUE
	return FALSE
...
var/list/access_name_lookup = null //Generated at round start.
