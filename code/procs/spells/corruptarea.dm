/client/proc/wiz_corruption()
	...
	for (var/mob/O in AIviewers(usr, null)) O.show_message(text("<span style=\"color:red\"><strong>[]</strong> begins to channel dark energy!</span>", usr), 1)
...
/proc/get_total_corruptible_terrain()
	...
	logTheThing("debug", null, null, "<strong>I Said No/Corruptible Terrain:</strong> Found [turfcount] corruptible terrains, 40% corruption is [percentage]")
	return turfcount
...
/client/proc/sense_corruption()
	...
	boutput(usr, "<strong>[corrupt]%</strong> of the station has been corrupted.")
...
/client/proc/remove_corruption()
	...
	for (var/mob/O in AIviewers(usr, null)) O.show_message(text("<span style=\"color:red\"><strong>[]</strong> closes \his eyes and begins to pray!</span>", usr), 1)
