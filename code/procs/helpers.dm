...
var/global/obj/flashDummy = null
...
/proc/movable_area_check(var/atom/A)
	...
	if (!A) return FALSE
	if (A.x > world.maxx) return FALSE
	if (A.x < 1) return FALSE
	if (A.y > world.maxy) return FALSE
...
/proc/uncapitalize(var/t as text)
	. = lowertext(copytext(t, 1, 2)) + copytext(t, 2)
...
/proc/key_name(var/whom, var/include_details = 1, var/admins = 1, var/mentor = 0)
	...
	if (include_details)
		if (!isnull(the_mob))
			...
			text += "<a href='byond://?src=[linkSrc]&action=adminplayeropts&targetckey=[the_mob.ckey]' class='popt'><i class='icon-info-sign'></em></a>"
...
/client/verb/windowclose(var/atomref as text)
	...
	if (src && mob)
		//boutput(world, "[src] was [mob.machine], setting to null")
		if (mob.machine && istype(mob.machine, /obj/machinery))
			mob.machine.current_user = null
		mob.machine = null
	return
...
/proc/check_facing(var/atom/A, var/atom/B)
	...
	if (turn(A.dir,180) == B.dir)
		...
		return TRUE // they are facing eachother
...
/proc/get_all_enemies()
	if (ticker && ticker.mode && ticker.current_state >= GAME_STATE_PLAYING)
		var/mind/enemies[] = new()
		var/mind/someEnemies[] = new()
		...
		if (istype(ticker.mode, /game_mode/revolution))
			...
			for (var/mind/M in someEnemies)
				if (M.current)
					enemies += M
					for (var/mind/G in M.gang.members) //This may be fucked. Dunno how these are stored.
						enemies += G
...
/proc/get_all_functional_reagent_ids()
	var/reagent/Read = null
	for (var/X in typesof(/reagent))
		Read = new X
...
/proc/reagent_id_to_name(var/reagent_id)
	if (!reagent_id || !reagents_cache.len)
		return
	var/reagent/R = reagents_cache[reagent_id]
	if (!R)
		return "nothing"
	else
		return R.name
...
/proc/get_english_num(var/num, var/sep) // can only do up to 999,999 because of scientific notation kicking in after 6 digits
	...
	DEBUG_MESSAGE("<strong>get_english_num recieves num \"[num]\"</strong>")
	...
	if (num_return)
		DEBUG_MESSAGE("<strong>get_english_num returns num \"[num_return]\"</strong>")
		return num_return
...
/atom/proc/debug_loverlay()
	...
	if (!dir)
		return
	letter_overlay(letter, lcolor, text2dir(dir))
...
/proc/dead_player_list(var/return_minds = 0, var/confirmation_spawn = 0, var/list/text_messages = list())
	...
	if (confirmation_spawn > 0)
		...
		text_chat_failed = "<span style=\"color:red\"><strong>[strip_html(text_chat_failed, MAX_MESSAGE_LEN)]</strong></span>"
		text_chat_toolate = "<span style=\"color:red\"><strong>[strip_html(text_chat_toolate, MAX_MESSAGE_LEN)]</strong></span>"
...
/proc/remove_mindslave_status(var/mob/M, var/slave_type ="", var/removal_type ="")
	...
	switch (slave_type)
		...
		if ("otherslave")
			...
			logTheThing("debug", M, null, "<strong>Convair880</strong>: [M] isn't a mindslave or vampire thrall, can't remove mindslave status.")
			return
