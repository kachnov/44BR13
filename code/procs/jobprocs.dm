/*/proc/SetupOccupationsList()
	...
	for (var/occupation in occupations.get_all_jobs())
...
/proc/FindOccupationCandidates(list/unassigned, job, level)
	...
	for (var/mob/new_player/player in unassigned)
		...
		if (checktraitor(player))
			if ((ticker && ticker.mode && istype(ticker.mode, /game_mode/revolution)) && J.cant_spawn_as_rev)
				...
				continue
			else if ((ticker && ticker.mode && istype(ticker.mode, /game_mode/gang)) && (job != "Staff Assistant"))
				continue
...
/proc/DivideOccupations()
	...
	for (var/job/JOB in job_controls.staple_jobs)
	...
	if (master_mode != "sandbox")
		for (var/job/JOB in high_priority_jobs)
			...
			for (var/mob/new_player/candidate in pick1)
				...
				logTheThing("debug", null, null, "<strong>I Said No/Jobs:</strong> [candidate] took [JOB.name] from High Priority Job Picker Lv1")
			...
			for (var/mob/new_player/candidate in pick2)
				if (JOB.assigned >= JOB.limit || unassigned.len == 0) break
				logTheThing("debug", null, null, "<strong>I Said No/Jobs:</strong> [candidate] took [JOB.name] from High Priority Job Picker Lv2")
				candidate.mind.assigned_role = JOB.name
				logTheThing("debug", candidate, null, "assigned job: [candidate.mind.assigned_role]")
				unassigned -= candidate
				JOB.assigned++
			for (var/mob/new_player/candidate in pick3)
				if (JOB.assigned >= JOB.limit || unassigned.len == 0) break
				logTheThing("debug", null, null, "<strong>I Said No/Jobs:</strong> [candidate] took [JOB.name] from High Priority Job Picker Lv3")
				candidate.mind.assigned_role = JOB.name
				logTheThing("debug", candidate, null, "assigned job: [candidate.mind.assigned_role]")
				unassigned -= candidate
				JOB.assigned++
	...
	for (var/mob/new_player/player in unassigned)
		...
		var/job/JOB = find_job_in_controller_by_string(player.client.preferences.job_favorite)
		...
		if (JOB.limit < 0 || !(JOB.assigned >= JOB.limit))
			logTheThing("debug", null, null, "<strong>I Said No/Jobs:</strong> [player] took [JOB.name] from favorite selector")
	...
	for (var/job/JOB in available_job_roles)
		...
		for (var/mob/new_player/candidate in pick2)
			if (JOB.assigned >= JOB.limit || unassigned.len == 0)
				break
			logTheThing("debug", null, null, "<strong>I Said No/Jobs:</strong> [candidate] took [JOB.name] from Level 2 Job Picker")
			candidate.mind.assigned_role = JOB.name
			logTheThing("debug", candidate, null, "assigned job: [candidate.mind.assigned_role]")
			unassigned -= candidate
			JOB.assigned++
	...
	for (var/job/JOB in available_job_roles)
		...
		for (var/mob/new_player/candidate in pick3)
			if (JOB.assigned >= JOB.limit || unassigned.len == 0) break
			logTheThing("debug", null, null, "<strong>I Said No/Jobs:</strong> [candidate] took [JOB.name] from Level 3 Job Picker")
			candidate.mind.assigned_role = JOB.name
			logTheThing("debug", candidate, null, "assigned job: [candidate.mind.assigned_role]")
			unassigned -= candidate
			JOB.assigned++
...
/* commenting this out because it's causing people to randomly become monkeys and shit and I guess it was set up before special jobs were enabled on default
	...
	if (job_controls.allow_special_jobs)
		for (var/job/JOB in job_controls.special_jobs)
			if (unassigned.len == 0)
				break
			if (JOB.limit == 0)
				continue
			...
			logTheThing("debug", null, null, "<strong>I Said No/Jobs:</strong> [candidate] took [JOB.name] from special job picker")
			candidate.mind.assigned_role = JOB.name
			logTheThing("debug", candidate, null, "assigned job: [candidate.mind.assigned_role]")
			unassigned -= candidate
			JOB.assigned++
*/
	...
	if (!low_priority_jobs.len)
		...
		low_priority_jobs += "Boomer Soldier"
	for (var/mob/new_player/player in unassigned)
		logTheThing("debug", null, null, "<strong>I Said No/Jobs:</strong> [player] given a low priority role")
...
/mob/living/carbon/human/proc/Equip_Rank(rank, joined_late)
	...
	if (!JOB)
		boutput(src, "<span style=\"color:red\"><strong>Something went wrong setting up your rank and equipment! Report this to a coder.</strong></span>")
		message_coders("[src]: Failed to find job datum for job \"[rank]\" in Equip_Rank()")
		return

	//if (JOB.name == "Captain")
		//boutput(world, "<strong>[src] is the Captain!</strong>")
	if (JOB.announce_on_join)
		boutput(world, "<strong><big>[src] is the [JOB.name]!</big></strong>")
	boutput(src, "<strong>You are the [JOB.name].</strong>")
	job = JOB.name
	mind.assigned_role = JOB.name
	
	job_spawn(mind.assigned_role, joined_late)

	if (time2text(world.realtime, "MM DD") == "12 25")
		unlock_medal("A Holly Jolly Spacemas")

	if (JOB.slot_back)
		equip_if_possible(new JOB.slot_back(src), slot_back)
		if (JOB.items_in_backpack.len && istype(back, /obj/item/storage))
	...
	if (JOB.slot_belt)
		if (bioHolder && bioHolder.HasEffect("fat"))
			equip_if_possible(new JOB.slot_belt(src), slot_in_backpack)
		else
			equip_if_possible(new JOB.slot_belt(src), slot_belt)
			if (traitHolder && traitHolder.hasTrait("immigrant") && istype(belt, /obj/item/device/pda2))
				del(belt) //UGHUGHUGHUGUUUUUUUU
		if (JOB.items_in_belt.len && istype(belt, /obj/item/storage))
			for (var/X in JOB.items_in_belt)
				equip_if_possible(new X(src), slot_in_belt)
	...
	if (JOB.slot_poc1)
		if (bioHolder && bioHolder.HasEffect("fat"))
			equip_if_possible(new JOB.slot_poc1(src), slot_in_backpack)
		else
			equip_if_possible(new JOB.slot_poc1(src), slot_l_store)
	if (JOB.slot_poc2)
		if (bioHolder && bioHolder.HasEffect("fat"))
			equip_if_possible(new JOB.slot_poc2(src), slot_in_backpack)
		else
	...
	if (traitHolder && traitHolder.hasTrait("loyalist"))
		trinket = new/obj/item/clothing/head/NTberet(src)
	else if (traitHolder && traitHolder.hasTrait("petasusaphilic"))
	...
	if (trinket) // rewrote this a little bit so hopefully people will always get their trinket
		trinket.name = "[real_name][pick(trinket_names)] [trinket.name]"
		trinket.quality = rand(5,80)
		var/equipped = 0
		if (istype(back, /obj/item/storage) && equip_if_possible(trinket, slot_in_backpack))
			equipped = 1
		else if (istype(belt, /obj/item/storage) && equip_if_possible(trinket, slot_in_belt))
			equipped = 1
		if (!equipped)
			if (!l_store && equip_if_possible(trinket, slot_l_store))
				equipped = 1
			else if (!r_store && equip_if_possible(trinket, slot_r_store))
				equipped = 1
			else if (!l_hand && equip_if_possible(trinket, slot_l_hand))
				equipped = 1
			else if (!r_hand && equip_if_possible(trinket, slot_r_hand))
				equipped = 1
	...
	spawn (0)
		...
		if (prob(10) && islist(random_pod_codes) && random_pod_codes.len)
			...
			if (V && V.lock && V.lock.code)
				...
				if (mind)
					mind.store_memory("The unlock code to your pod ([V]) is: [V.lock.code]")

		if (mind && mind.late_special_role == 1 && mind.special_role == "traitor")
...
/mob/living/proc/job_spawn(var/job/JOB, joined_late = FALSE)
		
	JOB = istype(JOB) ? JOB : find_job_in_controller_by_string(JOB)
	...
	if (job_start_locations && islist(job_start_locations[JOB.name]))
		...
		for (var/obj/landmark/start/sloc in world)
			...
			if (locate(/mob/living) in get_turf(sloc))
				continue
			target = get_turf(sloc)
			break

	if (!target)
		target = get_turf(locate("start*[JOB.name]")) // use old stype
...
/mob/living/carbon/human/proc/spawnId(rank)
	...
	if (C)
		...
		if (traitHolder && traitHolder.hasTrait("clericalerror"))
		...
		if (bioHolder && bioHolder.HasEffect("fat"))
			equip_if_possible(C, slot_in_backpack)
		else
			equip_if_possible(C, slot_wear_id)
	...
	if (mind)
		mind.store_memory("Your pin to your ID is: [C.pin]")

	if (wagesystem.jobs[JOB.name])
		equip_if_possible(new /obj/item/spacecash(src,wagesystem.jobs[JOB.name]), slot_r_store)
	else
		var/shitstore = rand(1,3)
		switch(shitstore)
			if (1)
				equip_if_possible(new /obj/item/pen(src), slot_r_store)
			if (2)
				equip_if_possible(new /obj/item/reagent_containers/food/drinks/water(src), slot_r_store)
