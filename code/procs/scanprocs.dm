...
/proc/scan_health(var/mob/M as mob, var/verbose_reagent_info = 0, var/disease_detection = 1)
				...
				if (isvampire(H))
					...
					switch (H.blood_volume)
						if (401 to INFINITY)
							blood_data = "<span style='color:red'>Blood level: NORMAL</span>"
						if (251 to 400)
							blood_data = "<span style='color:red'>Blood level: <strong>LOW</strong></span>"
						if (151 to 250)
							blood_data = "<span style='color:red'>Blood level: <strong>VERY LOW</strong></span>"
						if (1 to 150)
							blood_data = "<span style='color:red'>Blood level: <strong>DANGEROUSLY LOW</strong></span>"
						if (-INFINITY to 0)
							blood_data = "<span style='color:red'>Blood level: <strong>NO BLOOD DETECTED</strong></span>"
				switch (H.bleeding)
					if (1 to 3)
						blood_data += " | <span style='color:red'><strong>Minor bleeding wounds detected</strong></span>"
					if (4 to 6)
						blood_data += " | <span style='color:red'><strong>Bleeding wounds detected</strong></span>"
					if (7 to INFINITY)
						blood_data += " | <span style='color:red'><strong>Major bleeding wounds detected</strong></span>"
			if (H.implant && H.implant.len > 0)
				...
				for (var/obj/item/implant/I in H)
					if (istype(I, /obj/item/implant/projectile))
						bad_stuff ++
				if (bad_stuff)
					blood_data += " | <span style='color:red'><strong>Foreign object[bad_stuff == 1 ? "" : "s"] detected</strong></span>"
	...
	for (var/ailment_data/A in M.ailments)
	...
	if (A.reagents)
		if (A.reagents.reagent_list.len > 0)
			...
			for (var/current_id in A.reagents.reagent_list)
				var/reagent/current_reagent = A.reagents.reagent_list[current_id]
				if (single_line)
...
/proc/scan_forensic(var/atom/A as turf|obj|mob)
	...
	var/data = "--------------------------------<br>\
	<span style='color:blue'>Forensic analysis of <strong>[A]</strong></span> [contraband_data ? "[contraband_data]" : null]<br>\
	<br>\
	<em>Isolated fingerprints:</em>[fingerprint_data]<br>\
	<br>\
	<em>Isolated blood samples:</em>[blood_data]<br>\
	[forensic_data ? "<br><em>Additional forensic data:</em>[forensic_data]<br>" : null]\
	[glove_data ? "<br><em>Material analysis:</em><span style='color:blue'> [glove_data]</span>" : null]\
	"
...
/proc/scan_atmospheric(var/atom/A as turf|obj, var/pda_readout = 0, var/simple_output = 0)
	...
	if (!check_me || !istype(check_me, /gas_mixture))
		if (pda_readout == 1)
			return "[A] does not contain any gas."
		else if (simple_output == 1)
			return "(<em>[A] has no gas holder</em>)"
		else
	...
	if (total_moles > 0)
		...
		if (pda_readout == 1) // Output goes into PDA interface, not the user's chatbox.
			...
			data = "(<strong>Pressure:</strong> <em>[round(pressure, 0.1)] kPa</em>, <strong>Temp:</strong> <em>[round(check_me.temperature - T0C)]&deg;C</em>\
			, <strong>Contents:</strong> <em>[round(n2_concentration * 100)]% N2 / [round(o2_concentration * 100)]% O2 / [round(co2_concentration * 100)]% CO2 / [round(plasma_concentration * 100)]% PL</em>\
			  [unknown_concentration > 0.01 ? "<em> / [round(unknown_concentration * 100)]% other</em>)" : ")"]"

		else
			data = "--------------------------------<br>\
			<span style='color:blue'>Atmospheric analysis of <strong>[A]</strong></span><br>\
			<br>\
		...
		if (pda_readout == 1)
			...
			data = "(<strong>Contents:</strong> <em>empty</em></strong>)"
		else
...
/proc/scan_plant(var/atom/A as turf|obj, var/mob/user as mob)
	...
	if (!P || !istype(P, /plant/) || !DNA || !istype(DNA, /plantgenes))
		return "<span style=\"color:red\">Cannot scan.</span>"
