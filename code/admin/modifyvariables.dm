...
/client/proc/mod_list_add_ass(var/list/L, var/index) //haha
	...
	switch(class)
		...
		if ("type")
			var_value = input("Enter type:","Type") in null|subtypesof(/datum)

		if ("reference")
			var_value = input("Select reference:","Reference") as null|anything in world
		...
		if ("reference picker")
			...
			var/mob/M = usr
			if (istype(M))
				var/targetable/listrefpicker/R = new()
				R.target = L
				R.varname = index
				M.targeting_spell = R
				M.update_cursor()
...
/client/proc/mod_list_add(var/list/L)
	...
	switch(class)
		...
		if ("reference picker")
			...
			var/mob/M = usr
			if (istype(M))
				var/targetable/addtolistrefpicker/R = new()
				R.target = L
				M.targeting_spell = R
				M.update_cursor()
			return
...
/client/proc/modify_variables(var/atom/O)
	...
	if (isnull(var_value))
		...
		boutput(usr, "Variable appears to be <strong>MOB REFERENCE</strong>.")
		...
		boutput(usr, "Variable appears to be <strong>DATUM</strong>.")
	...
	switch(class)
		...
		if ("reference")
			O.vars[variable] = input("Select reference:","Reference",\
				O.vars[variable]) as anything in world
		...
		if ("reference picker")
			...
			var/mob/M = usr
			if (istype(M))
				var/targetable/refpicker/R = new()
				R.target = O
				R.varname = variable
				M.targeting_spell = R
				M.update_cursor()
