...
/admins/proc/voteres,\
/admins/proc/toggleooc,\
/admins/proc/togglelooc,\
/admins/proc/toggleoocdead,\
/admins/proc/toggletraitorscaling,\
/admins/proc/pcap,\
/admins/proc/toggleenter,\
/admins/proc/toggleAI,\
/admins/proc/toggle_soundpref_override,\
/admins/proc/toggle_respawns,\
/admins/proc/adsound,\
/admins/proc/adspawn,\
/admins/proc/adrev,\
/admins/proc/toggledeadchat,\
/admins/proc/togglefarting,\
/admins/proc/toggle_blood_system,\
/admins/proc/toggle_bone_system,\
/admins/proc/togglesuicide,\
/admins/proc/togglethetoggles,\
/admins/proc/toggleautoending,\
/admins/proc/toggleaprilfools,\
/admins/proc/togglemonkeyspeakhuman,\
/admins/proc/togglelatetraitors,\
/admins/proc/togglesoundwaiting,\
/admins/proc/adjump,\
/admins/proc/togglesimsmode,\
...
/client/proc/toggle_server_toggles_tab()
	...
	if (final_verblist.len)
		if (!holder.servertoggles_toggle)
			for (var/V in final_verblist)
				verbs -= V
		else
			for (var/V in final_verblist)
				verbs += V
		holder.servertoggles_toggle = !holder.servertoggles_toggle

		boutput(usr, "<span style=\"color:blue\">Toggled Server Toggle tab [holder.servertoggles_toggle?"off":"on"]!</span>")
...
/client/proc/toggle_extra_verbs()//Going to put some things in here that we dont need to see every single second when trying to play though atm only the add_r is in it
	...
	ADMIN_CHECK(src)
	if (!holder.extratoggle)
		verbs -= /client/proc/addreagents
		...
		verbs -= /client/proc/main_loop_context
		verbs -= /client/proc/main_loop_tick_detail
		verbs -= /client/proc/ticklag

		holder.extratoggle = 1
		boutput(src, "Extra Toggled Off")
	else
		verbs += /client/proc/addreagents
		holder.extratoggle = 0
...
/client/proc/toggle_hearing_all_looc()
	...
	only_local_looc = !only_local_looc
	boutput(usr, "<span style=\"color:blue\">Toggled seeing all LOOC messages [only_local_looc ?"off":"on"]!</span>")
...
/client/proc/toggle_attack_messages()
	...
	holder.attacktoggle = !holder.attacktoggle
	boutput(usr, "<span style=\"color:blue\">Toggled attack log messages [holder.attacktoggle ?"on":"off"]!</span>")
...
/client/proc/toggle_hear_prayers()
	...
	holder.hear_prayers = !holder.hear_prayers
	boutput(usr, "<span style=\"color:blue\">Toggled prayers [holder.hear_prayers ?"on":"off"]!</span>")
...
/client/proc/cmd_admin_godmode(mob/M as mob in world)
	...
	boutput(usr, "<span style=\"color:blue\"><strong>[M]'s godmode is now [usr.nodamage ? "ON" : "OFF"]</strong></span>")
...
/client/proc/cmd_admin_godmode_self()
	...
	boutput(usr, "<span style=\"color:blue\"><strong>Your godmode is now [usr.nodamage ? "ON" : "OFF"]</strong></span>")
...
/client/proc/toggle_atom_verbs() // I hate calling them "atom verbs" but wtf else should they be called, fuck
	...
	ADMIN_CHECK(src)
	if (!holder.animtoggle)
		holder.animtoggle = 1
		boutput(src, "Atom Verbs Toggled Off")

		verbs -= /client/proc/cmd_atom_emergency_stop

		verbs -= /client/proc/cmd_emag_all
		verbs -= /client/proc/cmd_emag_type
		verbs -= /client/proc/cmd_emag_target

		verbs -= /client/proc/cmd_transmute_type

		verbs -= /client/proc/cmd_scale_all
		verbs -= /client/proc/cmd_scale_type
		verbs -= /client/proc/cmd_scale_target

		verbs -= /client/proc/cmd_rotate_all
		verbs -= /client/proc/cmd_rotate_type
		verbs -= /client/proc/cmd_rotate_target

		verbs -= /client/proc/cmd_spin_all
		verbs -= /client/proc/cmd_spin_type
		verbs -= /client/proc/cmd_spin_target

		verbs -= /client/proc/cmd_get_type

	else
		holder.animtoggle = 0
		boutput(src, "Atom Verbs Toggled On")

		if (holder.level >= LEVEL_SHITGUY)
			verbs += /client/proc/cmd_emag_all
			verbs += /client/proc/cmd_scale_all
			verbs += /client/proc/cmd_rotate_all
			verbs += /client/proc/cmd_spin_all
...
/client/proc/toggle_toggles()
	...
	if (!toggles_enabled && !(holder.rank in list("Host", "Coder")))
...
/client/proc/toggle_jobban_announcements()
	...
	if (!toggles_enabled && !(holder.rank in list("Host", "Coder", "Shit Person")))
		alert("Toggles are disabled. Sorry, bro!")
		return
...
/admins/proc/voteres()
	...
	if (confirm == "Restart [config.allow_vote_restart ? "Off" : "On"]")
		...
		boutput(world, "<strong>Player restart voting toggled to [config.allow_vote_restart ? "On" : "Off"]</strong>.")
	...
	if (confirm == "Change Game Mode [config.allow_vote_mode ? "Off" : "On"]")
		...
		boutput(world, "<strong>Player mode voting toggled to [config.allow_vote_mode ? "On" : "Off"]</strong>.")
...
/admins/proc/toggleooc()
	...
	boutput(world, "<strong>The OOC channel has been globally [ooc_allowed ? "en" : "dis"]abled!</strong>")
...
/admins/proc/togglelooc()
	...
	boutput(world, "<strong>The LOOC channel has been globally [looc_allowed ? "en" : "dis"]abled!</strong>")
...
/admins/proc/pcap()
	...
	if (player_capa)
		boutput(world, "<strong>The global player cap has been enabled at [player_cap] players.</strong>")
	else
		boutput(world, "<strong>The global player cap has been disabled.</strong>")
...
/admins/proc/toggleenter()
	...
	if (!( enter_allowed ))
		boutput(world, "<strong>You may no longer enter the game.</strong>")
	else
		boutput(world, "<strong>You may now enter the game.</strong>")
...
/admins/proc/toggleAI()
	...
	if (!( config.allow_ai ))
		boutput(world, "<strong>The AI job is no longer chooseable.</strong>")
	else
		boutput(world, "<strong>The AI job is chooseable now.</strong>")
...
/admins/proc/toggle_respawns()
	...
	if (abandon_allowed)
		boutput(world, "<strong>You may now respawn.</strong>")
	else
		boutput(world, "<strong>You may no longer respawn :(</strong>")
...
/admins/proc/toggledeadchat()
	...
	if (deadchat_allowed)
		boutput(world, "<strong>The Deadsay channel has been enabled.</strong>")
	else
		boutput(world, "<strong>The Deadsay channel has been disabled.</strong>")
...
/admins/proc/togglefarting()
	...
	if (farting_allowed)
		boutput(world, "<strong>Farting has been enabled.</strong>")
	else
		boutput(world, "<strong>Farting has been disabled.</strong>")
...
/admins/proc/toggle_blood_system()
	...
	boutput(world, "<strong>Blood system has been [blood_system ? "enabled" : "disabled"].</strong>")
...
/admins/proc/toggle_bone_system()
	...
	boutput(world, "<strong>Bone system has been [bone_system ? "enabled" : "disabled"].</strong>")
...
/admins/proc/toggleaprilfools()
	...
	if (breathing == "On")
		manualbreathing = 1
		if (priorbreathing != manualbreathing) boutput(world, "<strong>You must now breathe manually using the *inhale and *exhale emotes!</strong>")
	else
		manualbreathing = 0
		if (priorbreathing != manualbreathing) boutput(world, "<strong>You no longer need to breathe manually!</strong>")

	var/priorblinking = manualblinking
	var/blinking = alert("Manual blinking mode?","Toggle","On","Off")
	if (blinking == "On")
		manualblinking = 1
		if (priorblinking != manualblinking) boutput(world, "<strong>You must now blink manually using the *closeeyes and *openeyes emotes!</strong>")
	else
		manualblinking = 0
		if (priorblinking != manualblinking) boutput(world, "<strong>You no longer need to blink manually!</strong>")
...
/admins/proc/togglemonkeyspeakhuman()
	...
	if (monkeysspeakhuman)
		boutput(world, "<strong>Monkeys can now speak to humans.</strong>")
	else
		boutput(world, "<strong>Monkeys can no longer speak to humans.</strong>")
...
/admins/proc/togglesimsmode()
	...
	for (var/mob/M in world)
		boutput(M, "<strong>Motives have been globally [global_sims_mode ? "enabled" : "disabled"].</strong>")
		if (ishuman(M))
			var/mob/living/carbon/human/H = M
			if (global_sims_mode && !H.sims)
				if (map_setting == "DESTINY")
					H.sims = new /simsHolder/destiny(H)
				else
					H.sims = new /simsHolder/human(H)
