/admins/proc/player_notes(var/player)
	if (!player)
		return
	if (level < 0)
	...
	dat += content
	dat = "<h1>Player Notes for <strong>[player]</strong></h1><HR></FONT><br><A href='?src=\ref[src];action=notes2;target=[player];type=add'>Add Note</A><br><HR>[dat]"
	usr << browse(dat, "window=notesp;size=875x400")
	return
