...
/admins/var/partmod_holder/part_modifier = null
...
/client/proc/modify_parts(var/mob/living/carbon/human/target as mob)
	...
	if (!holder.part_modifier)
		holder.part_modifier = new

	holder.part_modifier.show_window(target, src)
...
// sever limbs
			if ("sever_limb")
				var/human_limbs/limbs = locate(href_list["target"])
...
// drop organs
			if ("drop_organ")
				var/organHolder/organs = locate(href_list["target"])
...
// replace all organs
	...
	proc/show_window(var/mob/living/carbon/human/target as mob, var/mob/user)
		...
		var/human_limbs/limbs = target.limbs
		var/organHolder/organs = target.organHolder
