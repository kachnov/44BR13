	...
	if (thetype)
		...
		for (var/i=0, i<argnum, i++)
			...
			switch(class)
				...
				if ("type")
					listargs += input("Enter type:","Type", null) in null|subtypesof(/datum)

				if ("reference")
					listargs += input("Select reference:","Reference", null) as null|anything in world
...
/client/proc/cmd_admin_makeai(var/mob/M in world)
	...
	if (ishuman(M))
		...
		if ((istype(S, /obj/landmark/start) && istype(S.loc, /turf)))
			boutput(M, "<span style=\"color:blue\"><strong>You have been teleported to your new starting location!</strong></span>")
			M.set_loc(S.loc)
			M.buckled = null
...
/client/proc/cmd_debug_del_all()
	...
	for (var/V in blocked)
		if (V == hsbitem)
			...
			return	
	if (hsbitem)
		delete_state = DELETE_RUNNING
		verbs += /client/proc/cmd_debug_del_all_cancel
		verbs += /client/proc/cmd_debug_del_all_check
		...
		for (var/atom/O in world)
			if (istype(O, hsbitem))
				...
				numdeleted ++
			LAGCHECK(80)
			if (delete_state == DELETE_STOP)
				break
			else if (delete_state == DELETE_CHECK)
...
/client/proc/cmd_debug_mutantrace(var/mob/mob in world)
	...
	var/race = input("Select a mutant race","Races",null) as null|anything in typesof(/mutantrace)
...
/client/proc/debug_reaction_list()
	...
	for (var/reagent in total_chem_reactions)
		T += "<h3>[reagent]</h3>"
		for (var/chemical_reaction/R in total_chem_reactions[reagent])
			T += "   - [R.type]<br>"
		T+="<hr>"
...
/client/proc/debug_reagents_cache()
	...
	var/T = "<h1>Reagents Cache</h1><hr><table border=1><tr><td><strong><center>ID</center></strong></td><td><strong><center>Name</center></strong></td><td><strong><center>Type</center></strong></td>"
	for (var/reagent in reagents_cache)
		var/reagent/R = reagents_cache[reagent]
...
/client/proc/debug_check_possible_reactions(var/atom/O as mob|obj|turf)
	...
	if (O.reagents && O.reagents.total_volume)
		var/T = "<TT><h1>Possible Reactions</h1><center>for reagents inside<BR><strong>[O]</strong></center><hr>"
		if (O.reagents.possible_reactions.len)
			for (var/chemical_reaction/CR in O.reagents.possible_reactions)
				T += "  -  [CR.type]<br>"
		else
			T += "Nothing at all!"
...
/client/proc/showMyCoords(var/x, var/y, var/z)
	return replacetext(showCoords(x,y,z), "%admin_ref%", "\ref[holder]")
...
/client/proc/find_one_of(var/typename as text)
	...
	if (thetype)
		...
		if (!theinstance)
			...
			return
		boutput(usr, "<span style=\"color:blue\"><strong>Found instance of [thetype]:</strong></span>")
...
/client/proc/find_all_of(var/typename as text)
	...
	if (thetype)
		var/counter = 0
		boutput(usr, "<span style=\"color:blue\"><strong>All instances of [thetype]: </strong></span>")
		for (var/atom/theinstance in world)
			if (!istype(theinstance, thetype))
				continue
			counter++
...
/client/proc/find_thing(var/atom/A as null|anything in world)
	...
	boutput(usr, "<span style=\"color:blue\"><strong>Located [A] ([A.type]): </strong></span>")
	print_instance(A)
	...
	if (thetype)
		...
		for (var/atom/theinstance in world)
			if (!istype(theinstance, thetype))
				continue
			counter++
		boutput(usr, "<span style=\"color:blue\">There are <strong>[counter]</strong> instances total of [thetype].</span>")
	else
		boutput(usr, "<span style=\"color:red\"><strong>No type matches for [typename].</strong></span>")
		return
...
/proc/display_camera_paths()
	...
	for (var/obj/machinery/camera/C in machines)
		if (C.c_north)
			camera_path_list.Add(particleMaster.SpawnSystem(new /particleSystem/mechanic(C.loc, C.c_north.loc)))

		if (C.c_east)
			camera_path_list.Add(particleMaster.SpawnSystem(new /particleSystem/mechanic(C.loc, C.c_east.loc)))

		if (C.c_south)
			camera_path_list.Add(particleMaster.SpawnSystem(new /particleSystem/mechanic(C.loc, C.c_south.loc)))
		if (C.c_west)
			camera_path_list.Add(particleMaster.SpawnSystem(new /particleSystem/mechanic(C.loc, C.c_west.loc)))
...
/proc/remove_camera_paths()
	for (var/particleSystem/mechanic/M in camera_path_list)
		M.Die()
...
/client/proc/cmd_randomize_handwriting()
	...
	ADMIN_CHECK(src)
	if (mob && mob.mind)
		mob.mind.handwriting = pick(handwriting_styles)
		boutput(usr, "<span style=\"color:blue\">Handwriting style is now: [mob.mind.handwriting]</span>")
		logTheThing("admin", usr, null, "randomized their handwriting style: [mob.mind.handwriting]")
		logTheThing("diary", usr, null, "randomized their handwriting style: [mob.mind.handwriting]", "admin")
