...
/client/proc/enableDrunkMode()
	...
	if (alert("Enable drunk mode for yourself?", "Confirmation", "Yes", "No") == "Yes")
		if (src)
			verbs -= /client/proc/enableDrunkMode
			verbs += /client/proc/disableDrunkMode
			toggleDrunkMode(src)
	return
...
/client/proc/toggleDrunkMode(var/client/C)
	...
	if (C.holder.drunk)
		...
		boutput(C, "<span style=\"color:red\"><strong><big>You are now in drunk-mode!</big></strong><br>You will have reduced powers so you can't fuck shit up so much.<br>Use \"Disable Drunk Mode\" to disable this.</span>")
