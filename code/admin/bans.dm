...
//A return FALSE is a "this guy is ok let him in"
...
/proc/checkBan(ckey, compID, ip)
	set background = 1
	if (!ckey && !compID && !ip)
		logTheThing("debug", null, null, "<strong>Bans Error</strong>: No details passed to <strong>checkBan</strong>")
		logTheThing("diary", null, null, "Bans Error: No details passed to checkBan", "debug")
		return FALSE

	if (!ip) return FALSE //Ignore if localhost
	...
	if (data["error"]) //Error returned from the API welp
		logTheThing("debug", null, null, "<strong>Bans Error</strong>: Error returned in <strong>checkBan</strong> for <strong>[ckey]</strong>: [data["error"]]")
	...
	*/
	...
	if (row["ckey"] != ckey || row["ip"] != ip || row["compID"] != compID) //Insert a new ban for this JERK
		...
		if (rVal)
			logTheThing("debug", null, null, "<strong>Bans Error</strong>: Add ban in checkBan failed with message <strong>[rVal]</strong>")
	...
	if (text2num(row["timestamp"]) > 0) //Temp ban found, determine if it should expire or not
		if (!getExpiry(row["timestamp"])) //It expired! Go you!
			...
			if (rVal)
				logTheThing("debug", null, null, "<strong>Bans Error</strong>: Delete temp ban in checkBan failed with message <strong>[rVal]</strong>")
...
/proc/addBan(step = 1, data)
	...
	if (step == 2 || !centralConn)
		...
		if (targetC)
			...
			boutput(targetC, "<span style=\"color:red\"><BIG><strong>You have been banned by [row["akey"]].<br>Reason: [row["reason"]]</strong></BIG></span>")
...
/client/proc/addBanDialog(target)
	if (holder && usr.level >= LEVEL_SA)
		...
		boutput(usr, "<span style=\"color:red\"><strong>You are currently banning the following player:</strong></span>")
		boutput(usr, "<strong>Mob:</strong> [mobRef ? M.name : "N/A"]")
		boutput(usr, "<strong>Key:</strong> [data["ckey"] ? data["ckey"] : "N/A"] (IP: [data["ip"] ? data["ip"] : "N/A"], CompID: [data["compID"] ? data["compID"] : "N/A"])")
		boutput(usr, "<span style=\"color:red\"><strong>Make sure this is who you want to ban before continuing!</strong></span>")
...
/proc/forceUpdateLocalBans(latestLocalID, latestRemoteID)
	...
	if (data["error"]) //Error returned from the API welp
		logTheThing("debug", null, null, "<strong>Bans Error</strong>: Error returned in <strong>forceUpdateLocalBans</strong>: [data["error"]]")
	...
	for (var/row in data)
		...
		if (returnData["error"]) //Error returned from the local db jeeeeez aint nothing going our way
			logTheThing("debug", null, null, "<strong>Local API Error</strong> - Callback failed in <strong>[type]BanApiFallback</strong> with message: <strong>[returnData["error"]]</strong>")
			logTheThing("diary", null, null, "<strong>Local API Error</strong> - Callback failed in [type]BanApiFallback with message: [returnData["error"]]", "debug")
			if (returnData["showAdmins"])
				message_admins("<span style=\"color:blue\"><strong>Failed for route [type]BanApiFallback</strong>: [returnData["error"]]</span>")
	...
	for (var/e = 1, e <= data.len, e++) //each ban
		...
		if (returnData["error"]) //Error returned from the local db jeeeeez aint nothing going our way
			...
			logTheThing("diary", null, null, "<strong>Local API Error</strong> - Callback failed in [type]BanApiFallback with message: [returnData["error"]]", "debug")
			if (returnData["showAdmins"])
				message_admins("<span style=\"color:blue\"><strong>Failed for route [type]BanApiFallback</strong>: [returnData[</span>"error"]]")
