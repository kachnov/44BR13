...
/admins/Topic(href, href_list)
	...
	switch(href_list["action"])
		...
		if ("toggle_auto_stealth")
			if (level >= LEVEL_SA)
				...
				boutput(usr, "<span style=\"color:blue\">Auto Stealth [auto_stealth ? "enabled" : "disabled"].</span>")
				if (auto_stealth)
					if (auto_alt_key)
						auto_alt_key = 0
					if (usr.client.alt_key)
						set_alt_key()
					if (!usr.client.stealth && !isnull(auto_stealth_name))
						set_stealth_mode(auto_stealth_name)
					else if (isnull(auto_stealth_name))
						var/new_key = input("Enter your desired display name.", "Fake Key", usr.client.key) as null|text
						if (!new_key)
							auto_stealth_name = null
							boutput(usr, "<span style=\"color:blue\">Auto Stealth name removed.</span>")
							return show_pref_window(usr)
						if (alert("Use \"[new_key]\" as your Auto Stealth name?", "Confirmation", "OK", "Cancel") == "OK")
							auto_stealth_name = new_key
							set_stealth_mode(auto_stealth_name)
						else
							auto_stealth_name = null
							boutput(usr, "<span style=\"color:blue\">Auto Stealth name removed.</span>")
		...
		if ("toggle_auto_alt_key")
			if (level >= LEVEL_SA)
				auto_alt_key = !(auto_alt_key)
				boutput(usr, "<span style=\"color:blue\">Auto Alt Key [auto_alt_key ? "enabled" : "disabled"].</span>")
				if (auto_alt_key)
					if (auto_stealth)
						auto_stealth = 0
					if (usr.client.stealth)
						set_stealth_mode()
					if (!usr.client.alt_key && !isnull(auto_alt_key_name))
						set_alt_key(auto_alt_key_name)
					else if (isnull(auto_alt_key_name))
						var/new_key = input("Enter your desired display name.", "Alt Key", usr.client.key) as null|text
						if (!new_key)
							auto_alt_key_name = null
					...
					if ("2")
						...
						switch(emergency_shuttle.direction)
							...
							if (1)
								...
								boutput(world, "<span style=\"color:blue\"><strong>Alert: The shuttle is going back!</strong></span>")
		...
		if ("notes2")
			...
			if (!player)
				return
			//if (rank_to_level(admins[player]) > level) <- Pointless.
		...
		if ("jobbanpanel")
			var/mob/M = locate(href_list["target"])
			var/dat = ""
			var/header = "<strong>Pick Job to ban this guy from.<br>"
			var/body
	//		var/list/alljobs = get_all_jobs()
			...
			for (var/job in uniquelist(occupations.get_all_jobs()))
		...
		if ("jobban")
			if (level >= LEVEL_SA)
				...
				if (!M) return
				if ((M.client && M.client.holder && (M.client.holder.level > level)))
				...
				if (jobban_isbanned(M, job))
					...
					jobban_unban(M, job)
					if (announce_jobbans) boutput(M, "<span style=\"color:red\"><strong>[key_name(usr)] has lifted your [job] job-ban.</strong></span>")
				else
					...
					if (announce_jobbans) boutput(M, "<span style=\"color:red\"><strong>[key_name(usr)] has job-banned you from [job].</strong></span>")
			else
		...
		if ("c_mode_panel") // I removed some broken/discontinued game modes here (Convair880).
			if (level >= LEVEL_SA)
				...
				var/dat = {"
							...
							<strong>What mode do you wish to play[addltext]?</strong><br>
							Current mode is: <em>[master_mode]</em><br>
							Mode is <A href='?src=\ref[src];action=toggle_hide_mode'>[ticker.hide_mode ? "hidden" : "not hidden"]</a><br/>
							<HR>
							<strong>Regular Modes:</strong><br>
			...
			if (level >= LEVEL_SA)
				...
				if (requestedMode in valid_modes)
					...
					if (alert("Declare mode change to all players?","Mode Change","Yes","No") == "Yes")
						boutput(world, "<span style=\"color:blue\"><strong>The mode is now: [requestedMode]</strong></span>")
				else
					boutput(usr, "<span style=\"color:red\"><strong>That is not a valid game mode!</strong></span>")
			else
				alert("You need to be at least a Secondary Adminstrator to change the game mode.")
		...
		if ("c_mode_next")
			if (level >= LEVEL_SA)
				...
				if (alert("Declare mode change to all players?","Mode Change","Yes","No") == "Yes")
					boutput(world, "<span style=\"color:blue\"><strong>The next round's mode will be: [newmode]</strong></span>")
			else
				alert("You need to be at least a Secondary Adminstrator to change the game mode.")
		...
		if ("tdome")
			if (level >= LEVEL_SA)
				...
				boutput(M, "<span style=\"color:blue\"><strong>You have been sent to the Thunderdome. You are on [team].</strong></span>")
				boutput(M, "<span style=\"color:blue\"><strong>Prepare for combat. If you are not let out of the preparation area within a few minutes, please adminhelp. (F1 key)</strong></span>")
				...
				if (which == "cancel")
					...
					if (kind == "normal arm")
						if (which == "left arm" || which == "both arms")
							...
							limb.holder = M
							limb.remove_stage = 0
							M.show_message("<span style=\"color:blue\"><strong>Your left arm magically regrows!</strong></span>")
						...
						if (which == "right arm" || which == "both arms")
							...
							limb.holder = M
							limb.remove_stage = 0
							M.show_message("<span style=\"color:blue\"><strong>Your right arm magically regrows!</strong></span>")
					...
					if (kind == "synth arm")
						if (which == "left arm" || which == "both arms")
							...
							limb.holder = M
							limb.remove_stage = 0
							M.show_message("<span style=\"color:blue\"><strong>Your left arm magically becomes a synth arm!</strong></span>")
						...
						if (which == "right arm" || which == "both arms")
							...
							limb.holder = M
							limb.remove_stage = 0
							M.show_message("<span style=\"color:blue\"><strong>Your right arm magically becomes a synth arm!</strong></span>")
					...
					if (kind == "blooming synth arm")
						if (which == "left arm" || which == "both arms")
							...
							limb.holder = M
							limb.remove_stage = 0
							M.show_message("<span style=\"color:blue\"><strong>Your left arm magically becomes a blooming synth arm!</strong></span>")
						...
						if (which == "right arm" || which == "both arms")
							...
							limb.holder = M
							limb.remove_stage = 0
							M.show_message("<span style=\"color:blue\"><strong>Your right arm magically becomes a blooming synth arm!</strong></span>")
					...
					if (kind == "cyborg arm")
						if (which == "left arm" || which == "both arms")
							...
							limb.holder = M
							limb.remove_stage = 0
							M.show_message("<span style=\"color:blue\"><strong>Your left arm magically becomes a cyborg arm!</strong></span>")
						...
						if (which == "right arm" || which == "both arms")
							...
							limb.holder = M
							limb.remove_stage = 0
							M.show_message("<span style=\"color:blue\"><strong>Your right arm magically becomes a cyborg arm!</strong></span>")
					...
					if (kind == "light cyborg arm")
						if (which == "left arm" || which == "both arms")
							...
							limb.holder = M
							limb.remove_stage = 0
							M.show_message("<span style=\"color:blue\"><strong>Your left arm magically becomes a light cyborg arm!</strong></span>")
						...
						if (which == "right arm" || which == "both arms")
							...
							limb.holder = M
							limb.remove_stage = 0
							M.show_message("<span style=\"color:blue\"><strong>Your right arm magically becomes a light cyborg arm!</strong></span>")
					...
					if (kind == "wendigo arm")
						if (which == "left arm" || which == "both arms")
							...
							limb.holder = M
							limb.remove_stage = 0
							M.show_message("<span style=\"color:blue\"><strong>Your left arm eeriely becomes a wendigo arm!</strong></span>")
						...
						if (which == "right arm" || which == "both arms")
							...
							limb.holder = M
							limb.remove_stage = 0
							M.show_message("<span style=\"color:blue\"><strong>Your right arm eeriely becomes a wendigo arm!</strong></span>")
					...
					if (kind == "bear arm")
						if (which == "left arm" || which == "both arms")
							...
							limb.holder = M
							limb.remove_stage = 0
							M.show_message("<span style=\"color:blue\"><strong>Your left arm eeriely becomes a bear arm!</strong></span>")
						...
						if (which == "right arm" || which == "both arms")
							...
							limb.holder = M
							limb.remove_stage = 0
							M.show_message("<span style=\"color:blue\"><strong>Your right arm eeriely becomes a bear arm!</strong></span>")
					...
					if (kind == "abomination arm")
						if (which == "left arm" || which == "both arms")
							...
							limb.holder = M
							limb.remove_stage = 0
							M.show_message("<span style=\"color:blue\"><strong>Your left arm becomes a chitinous tendril!</strong></span>")
						...
						if (which == "right arm" || which == "both arms")
							...
							limb.holder = M
							limb.remove_stage = 0
							M.show_message("<span style=\"color:blue\"><strong>Your right arm becomes a chitinous tendril!</strong></span>")
					...
					if (kind == "werewolf arm")
						if (which == "left arm" || which == "both arms")
							...
							limb.holder = M
							limb.remove_stage = 0
							M.show_message("<span style=\"color:blue\"><strong>Your left arm becomes a werewolf arm!</strong></span>")
						...
						if (which == "right arm" || which == "both arms")
							...
							limb.holder = M
							limb.remove_stage = 0
							M.show_message("<span style=\"color:blue\"><strong>Your right arm becomes a werewolf arm!</strong></span>")
					...
					if (kind == "predator arm")
						if (which == "left arm" || which == "both arms")
							...
							limb.holder = M
							limb.remove_stage = 0
							M.show_message("<span style=\"color:blue\"><strong>Your left arm becomes a predator arm!</strong></span>")
						...
						if (which == "right arm" || which == "both arms")
							...
							limb.holder = M
							limb.remove_stage = 0
							M.show_message("<span style=\"color:blue\"><strong>Your right arm becomes a predator arm!</strong></span>")
						...
						if (which == "left arm" || which == "both arms")
							...
							limb.remove_stage = 0
							limb:set_item(item)
							M.show_message("<span style=\"color:blue\"><strong>Your left arm [pick("magically ", "weirdly ", "suddenly ", "unwillingly ", "grodily ", "")]becomes [item]!</strong></span>")
						...
						if (which == "right arm" || which == "both arms")
							...
							limb.remove_stage = 0
							limb:set_item(item)
							M.show_message("<span style=\"color:blue\"><strong>Your right arm [pick("magically ", "weirdly ", "suddenly ", "unwillingly ", "grodily ", "")]becomes [item]!</strong></span>")
					...
					if (type == "sever")
						...
						if (which == "left leg" || which == "both legs")
							...
							limb:holder = M
							limb:remove_stage = 0
							M.show_message("<span style=\"color:blue\"><strong>Your left leg becomes a brand new [limb]!</strong></span>")
						...
						if (which == "right leg" || which == "both legs")
							...
							limb:holder = M
							limb:remove_stage = 0
							M.show_message("<span style=\"color:blue\"><strong>Your right leg becomes a brand new [limb]!</strong></span>")
		...
		if ("getmob")
			if (( level >= LEVEL_SHITGUY ) || ((level >= LEVEL_PA) ))
				var/mob/M = locate(href_list["target"])
				if (!M) return
				usr.client.Getmob(M)
			else
		...
		if ("transform")
			if (( level >= LEVEL_PA ) || ((level >= LEVEL_SA) ))
				var/mob/M = locate(href_list["target"])
				if (!ishuman(M))
				...
				switch(which)
					...
					if ("Cyborg")
						H.Robotize_MK2()
					if ("Lizardman")
						H.set_mutantrace(/mutantrace/lizard)
						. = 1
					if ("Squidman")
						H.set_mutantrace(/mutantrace/ithillid)
						. = 1
					if ("Martian")
						H.set_mutantrace(/mutantrace/martian)
						. = 1
					if ("Skeleton")
						H.set_mutantrace(/mutantrace/skeleton)
						. = 1
					if ("Flashman")
						H.set_mutantrace(/mutantrace/flashy)
						. = 1
					if ("Ghostdrone")
						droneize(H, 0)
				if (.)
		...
		if ("addabil")
			if (level >= LEVEL_PA)
				...
				if (!M.abilityHolder)
					...
					return
				var/ab_to_add = input("Which ability?", "Ability", null) as anything in (subtypesof(/targetable))
		...
		if ("abilholder")
			if (level >= LEVEL_PA)
				var/mob/M = locate(href_list["target"])
				if (!M) return
				var/ab_to_add = input("Which holder?", "Ability", null) as anything in (subtypesof(/abilityHolder))
			...
			if (alert("Make [M] a wraith?", "Make Wraith", "Yes", "No") == "Yes")
				...
				switch (alert("Objectives?", "Objectives", "Custom", "Random", "None"))
					if ("Custom")
						var/WO = null
						do
							WO = input("What objective?", "Objective", null) as null|anything in (subtypesof(/objective/specialist/wraith))
							if (WO)
								var/objective/specialist/wraith/WObj = new WO()
								WObj.owner = mind
								WObj.set_up()
				...
				if (mind.objectives.len)
					boutput(Wr, "<strong>Your objectives:</strong>")
					var/obj_count = 1
					for (var/objective/objective in mind.objectives)
						boutput(Wr, "<strong>Objective #[obj_count]</strong>: [objective.explanation_text]")
						obj_count++
			...
			if (alert("Make [M] a blob?", "Make Blob", "Yes", "No") == "Yes")
				...
				if (B)
					if (B.mind)
						...
						var/objective/specialist/blob/Bl = new()
						Bl.owner = B.mind
						B.mind.objectives = list(Bl)

						var/i = 1
						for (var/objective/Obj in B.mind.objectives)
							boutput(B, "<strong>Objective #[i]</strong>: [Obj.explanation_text]")
							i++
		...
		if ("makecritter")
			...
			if (alert("Make [M] a critter?", "Make Critter", "Yes", "No") == "Yes")
				var/CT = input("What kind of critter?", "Make Critter", null) as null|anything in (subtypesof(/mob/living/critter))
				if (CT != null)
					M.critterize(CT)
		...
		if ("traitor")
			...
			if (M.mind && (M.mind in miscreants))
				var/t = ""
				for (var/objective/O in M.mind.objectives)
					if (istype(O, /objective/miscreant))
						t += "[O.explanation_text]\n"
				alert("Miscreant! Objective: [t]")

			var/game_mode/current_mode = ticker.mode
			if (istype(current_mode, /game_mode/revolution))
				if (M.mind in current_mode:head_revolutionaries)
					...
					alert("Revolutionary!")
					return
			else if (istype(current_mode, /game_mode/nuclear))
				if (M.mind in current_mode:syndicates)
					alert("Syndicate Operative!", "[M.key]")
					return
			else if (istype(current_mode, /game_mode/spy))
				if (M.mind in current_mode:leaders)
					var/mind/antagonist = M.mind
					var/t = ""
					for (var/objective/OB in antagonist.objectives)
						if (istype(OB, /objective/crew) || istype(OB, /objective/miscreant))
							continue
					...
					return
			else if (istype(current_mode, /game_mode/gang))
				if (M.mind in current_mode:leaders)
					alert("Leader of [M.mind.gang.gang_name].", "[M.key]")
					return
				for (var/gang/G in current_mode:gangs)
					if (M in G.members)
			...
			if (ishuman(M) || isAI(M) || isrobot(M) || iscritter(M))
				...
				if (traitorize == "Yes")
					if (issilicon(M))
						...
						if (ticker && ticker.mode && istype(ticker.mode, /game_mode/gang))
		...
		if ("create_mob")
			if (level >= LEVEL_PA) // Moved from SG to PA. They can do this through build mode anyway (Convair880).
				create_mob(usr)
			else
		...
		if ("prom_demot")
			if (level >= LEVEL_PA)
				...
				if (C.holder && (C.holder.level >= level))
		...
		if ("object_list")
			if (level >= LEVEL_PA)
				if (config.allow_admin_spawning && (state == 2 || level >= LEVEL_PA))
					...
					for (var/dirty_path in dirty_paths)
						...
						if (!path)
							...
							removed_paths += dirty_path
						else if (dirty_path in do_not_spawn && level < LEVEL_PA)
							removed_paths += dirty_path
						else if (ispath(path, /mob) && level < LEVEL_PA)
							removed_paths += dirty_path
						else
							paths += path
						sleep(-1)
				...
				switch(href_list["type"])
					...
					if ("traitor_all")
						if (level >= LEVEL_SA)
							...
							if (!custom_objective)
								return
							var/escape_objective = input("Which escaping objective?") as null|anything in typesof(/objective/escape/) + "None"
							if (!escape_objective)
								return
						...
						if (level >= LEVEL_SHITGUY)
							...
							for (var/mob/living/carbon/human/H in mobs)
								if (H.get_brain_damage() < 60)
									if (H.client)
										H.show_text("<strong>You suddenly feel stupid.</strong>","red")
					...
					// FUN SECRETS CODE
					if ("bioeffect_help")
						var/be_string = "To add or remove multiple bioeffects enter multiple IDs separated by semicolons.<br><br><strong>All Bio Effect IDs</strong><hr>"
						for (var/S in bioEffectList)
							be_string += "[S]<br>"
						usr << browse(be_string,"window=bioeffect_help;size=300x600")

					if ("reagent_help")
						var/r_string = "To add or remove multiple reagents enter multiple IDs separated by semicolons.<br><br><strong>All Reagent IDs</strong><hr>"
						...
						if (level >= LEVEL_SHITGUY)
							for (var/mob/living/carbon/human/H in mobs) //Only humans can be zombies!
								if (H.stat != 2) //Not dead!
									continue
								if (istype(H.mutantrace, /mutantrace/zombie))
									continue //Already a zombie!

								qdel(H.mutantrace)
								H.set_mutantrace(/mutantrace/zombie)
					...
					if ("the_great_switcharoo")
						if (level >= LEVEL_SHITGUY) //Will be SG when tested
		...
		if ("secretsdebug")
			if (level >= LEVEL_CODER)
				switch(href_list["type"])
					if ("budget")
						owner:debug_variables(wagesystem)
					if ("market")
						owner:debug_variables(shippingmarket)
					if ("genetics")
						owner:debug_variables(genResearch)
					if ("jobs")
						owner:debug_variables(job_controls)
					if ("hydro")
						owner:debug_variables(hydro_controls)
					if ("manuf")
						owner:debug_variables(manuf_controls)
					if ("radio")
						owner:debug_variables(radio_controller)
					if ("randevent")
						owner:debug_variables(random_events)
					if ("disease")
						owner:debug_variables(disease_controls)
					if ("mechanic")
						owner:debug_variables(mechanic_controls)
					if ("artifact")
						owner:debug_variables(artifact_controls)
					if ("gauntlet")
						owner:debug_variables(gauntlet_controller)
					if ("colosseum")
						owner:debug_variables(colosseum_controller)
					if ("stock")
						owner:debug_variables(stockExchange)
					if ("emshuttle")
						owner:debug_variables(emergency_shuttle)
					if ("overlays")
						overlaytest()
					if ("overlaysrem")
						removerlays()
	...
	*/
					...
					if ("check_antagonist")
						if (ticker && ticker.mode && ticker.current_state >= GAME_STATE_PLAYING)
							var/dat = "<html><head><title>Round Status</title></head><body><h1><strong>Round Status</strong></h1>"
							dat += "Current Game Mode: <strong>[ticker.mode.name]</strong><BR>"
							dat += "Round Duration: <strong>[round(world.time / 36000)]:[add_zero(world.time / 600 % 60, 2)]:[world.time / 100 % 6][world.time / 100 % 10]</strong><BR>"

							if (istype(ticker.mode, /game_mode/nuclear))
								var/game_mode/nuclear/NN = ticker.mode
								dat += "<br><table cellspacing=5><tr><td><strong>Syndicates</strong></td><td></td></tr>"
								for (var/mind/N in NN.syndicates)
									var/mob/M = N.current
									if (!M) continue
									dat += "<tr><td><a href='?src=\ref[src];action=adminplayeropts;target=\ref[M]'>[M.real_name]</a>[M.client ? "" : " <em>(logged out)</em>"][M.stat == 2 ? " <strong><font color=red>(DEAD)</font></strong>" : ""]</td>"
								...
								dat += "</table><br><table><tr><td><strong>Nuclear bomb:</strong></td></tr>"
							...
							else if (istype(ticker.mode, /game_mode/revolution))
								dat += "<br><table cellspacing=5><tr><td><strong>Revolutionaries</strong></td><td></td></tr>"
								for (var/mind/N in ticker.mode:head_revolutionaries)
									var/mob/M = N.current
									if (!M) continue
									dat += "<tr><td><a href='?src=\ref[src];action=adminplayeropts;target=\ref[M]'>[M.real_name]</a> <strong>(Leader)</strong>[M.client ? "" : " <em>(logged out)</em>"][M.stat == 2 ? " <strong><font color=red>(DEAD)</font></strong>" : ""]</td>"
									dat += "<td><a href='?action=priv_msg&target=[M.ckey]'>PM</A></td></tr>"
								for (var/mind/N in ticker.mode:revolutionaries)
									var/mob/M = N.current
									if (!M) continue
									dat += "<tr><td><a href='?src=\ref[src];action=adminplayeropts;target=\ref[M]'>[M.real_name]</a>[M.client ? "" : " <em>(logged out)</em>"][M.stat == 2 ? " <strong><font color=red>(DEAD)</font></strong>" : ""]</td>"
									dat += "<td><a href='?action=priv_msg&target=[M.ckey]'>PM</A></td></tr>"
								dat += "</table><table cellspacing=5><tr><td><strong>Target(s)</strong></td><td></td><td><strong>Location</strong></td></tr>"
								for (var/mind/N in ticker.mode:get_living_heads())
									var/mob/M = N.current
									if (!M) continue
									dat += "<tr><td><a href='?src=\ref[src];action=adminplayeropts;target=\ref[M]'>[M.real_name]</a>[M.client ? "" : " <em>(logged out)</em>"][M.stat == 2 ? " <strong><font color=red>(DEAD)</font></strong>" : ""]</td>"
								...
								if (ticker.mode:leaders.len > 0)
									dat += "<br><table cellspacing=5><tr><td><strong>Infiltrators:</strong></td><td></td><tr>"
									for (var/mind/leader in ticker.mode:leaders)
										var/mob/M = leader.current
										if (!M) continue
										dat += "<tr><td><a href='?src=\ref[src];action=adminplayeropts;target=\ref[M]'>[key_name(M)]</a>[M.client ? "" : " <em>(logged out)</em>"][M.stat == 2 ? " <strong><font color=red>(DEAD)</font></strong>" : ""]</td>"
										dat += "<td><a href='?action=priv_msg&target=[M.ckey]'>PM</A></td>"
								...
								if (ticker.mode:spies.len > 0)
									dat += "<br><table cellspacing=5><tr><td><strong>Brainwashed Followers:</strong></td><td></td><tr>"
									for (var/mind/spy in ticker.mode:spies)
										var/mob/M = spy.current
										if (!M) continue
										dat += "<tr><td><a href='?src=\ref[src];action=adminplayeropts;target=\ref[M]'>[M.real_name]</a>[M.client ? "" : " <em>(logged out)</em>"][M.stat == 2 ? " <strong><font color=red>(DEAD)</font></strong>" : ""]</td>"
										dat += "<td>Obeys: "
										var/mind/obeycheck = ticker.mode:spies[spy]
								...
								if (ticker.mode:leaders.len > 0)
									for (var/mind/leader in ticker.mode:leaders)
										var/mob/M = leader.current
										var/gang/gang = leader.gang
										dat += "<br><table cellspacing=5><tr><td>([format_frequency(gang.gang_frequency)]) <strong>[gang.gang_name]:</strong></td><td></td><tr>"
										dat += "<tr><td><a href='?src=\ref[src];action=adminplayeropts;target=\ref[M]'>[key_name(M)]</a>[M.client ? "" : " <em>(logged out)</em>"][M.stat == 2 ? " <strong><font color=red>(DEAD)</font></strong>" : ""]</td>"
										dat += "<td><a href='?action=priv_msg&target=[M.ckey]'>PM</A></td>"
										dat += "<td><A HREF='?src=\ref[src];action=traitor;target=\ref[M]'>Show Objective</A></td></tr>"
										for (var/mob/member in gang.members)
											dat += "<tr><td><a href='?src=\ref[src];action=adminplayeropts;target=\ref[M]'>[key_name(member)]</a>[member.client ? "" : " <em>(logged out)</em>"][member.stat == 2 ? " <strong><font color=red>(DEAD)</font></strong>" : ""]</td>"
							...
							if (ticker.mode.traitors.len > 0)
								dat += "<br><table cellspacing=5><tr><td><strong>Traitors</strong></td><td></td><td></td></tr>"
								for (var/mind/traitor in ticker.mode.traitors)
							...
							if (ticker.mode.Agimmicks.len > 0)
								dat += "<br><table cellspacing=5><tr><td><strong>Misc Foes</strong></td><td></td><td></td></tr>"
								for (var/mind/gimmick in ticker.mode.Agimmicks)
							...
							if (miscreants.len > 0)
								dat += "<br><table cellspacing=5><tr><td><strong>Miscreants</strong></td><td></td><td></td></tr>"
								for (var/mind/miscreant in miscreants)
					...
					if ("shuttle_panel")
						if (ticker && ticker.current_state >= GAME_STATE_PLAYING)
							var/dat = "<html><head><title>Shuttle Controls</title></head><body><h1><strong>Shuttle Controls</strong></h1>"
							dat += "Round Duration: <strong>[round(world.time / 36000)]:[add_zero(world.time / 600 % 60, 2)]:[world.time / 100 % 6][world.time / 100 % 10]</strong><BR>"
							dat += "<strong>Emergency shuttle:</strong><BR>"
							...
							usr << browse(dat, "window=roundstatus;size=400x500")
					if ("manifest")
						var/dat = "<strong>Showing Crew Manifest.</strong><HR>"
					...
					if ("DNA")
						var/dat = "<strong>Showing DNA from blood.</strong><HR>"
					...
					if ("fingerprints")
						var/dat = "<strong>Showing Fingerprints.</strong><HR>"
				...
				if (usr)
					...
					if (ok)
						boutput(world, text("<strong>A secret has been activated by []!</strong>", usr.key))
				return
			else
		...
		if ("view_logs")
			if (level >= LEVEL_MOD)
				...
				dat = "<tr><td colspan='3' class='header text-normal [logType]'><strong>Logs</strong>[gettxt ? " (Searched for '[gettxt]')" : ""]. Found <strong>[foundCount]</strong> results.</td></tr>" + dat
		...
		if ("s_rez")
			if (level >= LEVEL_SHITGUY)
				switch(href_list["type"])
					if ("spawn_syndies")
						var/special_respawn/SR = new /special_respawn/
	...
	*/
		...
		if ("humanize")
			if (level >= LEVEL_PA) // Moved from SG to PA (Convair880).
				var/mob/M = locate(href_list["target"])
				if (!M) return
...
/admins/proc/player()
	...
	dat += "<body><table border=1 cellspacing=5><strong><tr><th>Name</th><th>Real Name</th><th>Key</th><th>Options</th><th>PM</th><th>Traitor?</th><th>Computer ID</th><th>IP:</th><th>Joined:</th></tr></strong>"
...
/admins/var/current_jobbans_rev = 0
/admins/var/jobbans_last_cached = 0
/admins/proc/Jobbans()
	set background = 1
	if (level >= LEVEL_CODER)
...
/admins/proc/Game()
	...
	dat += "<center><strong>Game Panel</strong></center><hr><br>"
...
/admins/proc/Secrets()
	...
	if (ticker)
		if (ticker.current_state >= GAME_STATE_PLAYING)
			dat += "<strong>Game Info:</strong> [ticker.mode.name] Mode, Timer at [round(world.time / 36000)]:[add_zero(world.time / 600 % 60, 2)]:[world.time / 100 % 6][world.time / 100 % 10]<br>"
			if (emergency_shuttle.online)
				dat += "<a href='?src=\ref[src];action=call_shuttle&type=2'><strong>Shuttle Status:</strong></a> <a href='?src=\ref[src];action=edit_shuttle_time'>[shuttletext]</a>"
			else
				dat += "<a href='?src=\ref[src];action=call_shuttle&type=1'><strong>Shuttle Status:</strong></a> <a href='?src=\ref[src];action=edit_shuttle_time'>[shuttletext]</a>"
		else if (ticker.current_state == GAME_STATE_PREGAME)
			dat += "<strong>Game Info:</strong> Game has not started yet.<br>"
			dat += "<strong>Force players to use random names:</strong> <A href='?src=\ref[src];action=secretsfun;type=forcerandomnames'>[force_random_names ? "Yes" : "No"]</a><br>"
			dat += "<strong>Force players to use random appearances:</strong> <A href='?src=\ref[src];action=secretsfun;type=forcerandomlooks'>[force_random_looks ? "Yes" : "No"]</a><br>"
			//dat += "<A href='?src=\ref[src];action=secretsfun;type=forcerandomnames'>Politely suggest all players use random names</a>" // lol

	dat += {"<br><br>
				<u><strong>Admin Tools:</strong></u><BR>
	...
	if (level >= LEVEL_SHITGUY)
		dat += {"<br><br>
					<u><strong>Coder Tools:</strong></u><BR>
	...
	dat += {"<br><br>
			...
			"}
	// FUN SECRETS PANEL
	if (level >= LEVEL_PA || (level == LEVEL_SA && usr.client.holder.state == 2))
		dat += {"<u><strong>Fun Secrets:</strong></u><BR>
					...
					<strong>Add Bio-Effect<A href='?src=\ref[src];action=secretsfun;type=bioeffect_help'>*</a>:</strong>
					...
					<strong>Remove Bio-Effect:</strong>
					...
					<strong>Add Reagent<A href='?src=\ref[src];action=secretsfun;type=reagent_help'>*</a>:</strong>
...
/admins/proc/Voting()
	var/dat = "<center><strong>Voting</strong></center><hr><br>"
...
/admins/proc/vmode()
	...
	if (confirm == "Change Game Mode")
		...
		if (!ticker)
			if (going)
				boutput(world, "<strong>The game start has been delayed.</strong>")
				going = 0
	vote.voting = 1
						// now voting
	...
	spawn (config.vote_period*10)
		vote.endvote()
	boutput(world, "<span style=\"color:red\"><strong>*** A vote to [vote.mode?"change game mode":"restart"] has been initiated by Admin [usr.key].</strong></span>")
...
/admins/proc/votekill()
	...
	if (vote.voting == 0)
		...
		return
	boutput(world, "<span style=\"color:red\"><strong>*** Voting aborted by [admin_key(usr.client, 1)].</strong></span>")
...
/admins/proc/restart()
	...
	if (confirm == "Cancel")
		return
	if (confirm == "Yes")
		boutput(world, "<span style=\"color:red\"><strong>Restarting world!</strong></span> <span style=\"color:blue\">Initiated by [admin_key(usr.client, 1)]!</span>")
...
/admins/proc/announce()
	...
	if (message)
		...
		boutput(world, "<span style=\"color:blue\"><strong>[admin_key(usr.client, 1)] Announces:</strong><br>&emsp; [message]</span>")
		logTheThing("admin", usr, null, ": [message]")
		logTheThing("diary", usr, null, ": [message]", "admin")

/admins/proc/startnow()
	set category = "Special Verbs"
...
/admins/proc/delay_start()
	...
	if (game_start_delayed)
		...
		boutput(world, "<strong>The game will start soon.</strong>")
...
/proc/checktraitor(mob/M as mob)
	...
	if (istype(ticker.mode, /game_mode/revolution))
		...
		if (M.mind in (ticker.mode:leaders))
			return TRUE
		for (var/gang/G in ticker.mode:gangs)
			if (M in G.members)
				return TRUE
...
/admins/proc/evilize(mob/M as mob, var/traitor_type, var/special = null, var/mass_traitor_obj = null, var/mass_traitor_esc = null)
	if (!M || !traitor_type)
	...
	if (checktraitor(M))
		...
		return
	if (!(ticker && ticker.mode && istype(ticker.mode, /game_mode/gang)) && traitor_type == "gang leader")
	...
	if (mass_traitor_obj)
		var/objective/custom_objective = new /objective(mass_traitor_obj)
		custom_objective.owner = M.mind
		M.mind.objectives += custom_objective

		if (mass_traitor_esc)
			var/objective/escape/escape_objective = new mass_traitor_esc
			escape_objective.owner = M.mind
			M.mind.objectives += escape_objective
	else
		var/list/eligible_objectives = list()
		if (ishuman(M) || iscritter(M))
			eligible_objectives = typesof(/objective/regular/) + typesof(/objective/escape/)
		else if (issilicon(M))
			eligible_objectives = list(/objective/regular,/objective/regular/assassinate,
			/objective/regular/force_evac_time,/objective/regular/gimmick,/objective/escape,/objective/escape/hijack,
			/objective/escape/survive,/objective/escape/kamikaze)
			if (istype(M,/mob/living/silicon/robot))
				eligible_objectives += /objective/regular/borgdeath
			traitor_type = "traitor"
		switch(traitor_type)
			if ("changeling")
				eligible_objectives += /objective/specialist/absorb
			if ("werewolf")
				eligible_objectives += /objective/specialist/werewolf/feed
			if ("vampire")
				eligible_objectives += /objective/specialist/drinkblood
			if ("predator")
				eligible_objectives += /objective/specialist/predator/trophy
			if ("grinch")
				eligible_objectives += /objective/specialist/ruin_xmas
			if ("gang leader")
				var/objective/gangObjective = new /objective/specialist/gang(  )
				gangObjective.owner = M.mind
				M.mind.objectives += gangObjective
		var/done = 0
		var/select_objective = null
		var/objective/new_objective = null
		...
		while (done != 1)
			...
			if (!select_objective)
				...
				break
			if (select_objective == /objective/regular)
	...
	if (isAI(M))
		...
		switch(traitor_type)
			if ("traitor")
				M.show_text("<h2><font color=red><strong>You have defected and become a traitor!</strong></font></h2>", "red")
				if (special != "hardmode")
			...
			if ("changeling")
				...
				M.show_text("<h2><font color=red><strong>You have mutated into a changeling!</strong></font></h2>", "red")
				M.make_changeling()
			if ("wizard")
				M.mind.special_role = "wizard"
				M.show_text("<h2><font color=red><strong>You have been seduced by magic and become a wizard!</strong></font></h2>", "red")
			...
			if ("vampire")
				...
				M.show_text("<h2><font color=red><strong>You have joined the ranks of the undead and are now a vampire!</strong></font></h2>", "red")
				M.make_vampire()
			if ("predator")
				M.mind.special_role = "predator"
				M.mind.assigned_role = "Predator"
				M.show_text("<h2><font color=red><strong>You have become a predator!</strong></font></h2>", "red")
				M.make_predator()
			if ("wrestler")
				M.mind.special_role = "wrestler"
				M.show_text("<h2><font color=red><strong>You feel an urgent need to wrestle!</strong></font></h2>", "red")
				M.make_wrestler(1)
			if ("werewolf")
				M.mind.special_role = "werewolf"
				M.show_text("<h2><font color=red><strong>You have become a werewolf!</strong></font></h2>", "red")
				M.make_werewolf()
			if ("grinch")
				M.mind.special_role = "grinch"
				M.make_grinch()
				M.show_text("<h2><font color=red><strong>You have become a grinch!</strong></font></h2>", "red")
			if ("gang leader")
				...
				boutput(M, "<span style=\"color:red\">Your objective is simple: <strong>Kill the opposing gang leaders</strong>.</span>")
			...
			if ("omnitraitor")
				...
				M.make_wrestler(1)
				M.make_grinch()
				M.show_text("<h2><font color=red><strong>You have become an omnitraitor!</strong></font></h2>", "red")
		...
		M.show_text("<h2><font color=red><strong>You have become evil and are now an antagonist!</strong></font></h2>", "red")
	...
	var/obj_count = 1
	for (var/objective/OBJ in M.mind.objectives)
		boutput(M, "<strong>Objective #[obj_count]</strong>: [OBJ.explanation_text]")
		obj_count++
...
/admins/proc/get_item_desc(var/target)
	switch (target)
		if (1)
...
/admins/proc/spawn_atom(var/object as text)
	set category = "Special Verbs"
...
/client/proc/respawn_target(mob/M as mob in world)
	...
	boutput(newM, "<strong>You have been respawned.</strong>")
	return newM
