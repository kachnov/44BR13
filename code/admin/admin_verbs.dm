...
/client/proc/respawn_as_self,\
/admins/proc/toggletraitorscaling,\
/admins/proc/togglerandomaiblobs\
*/),\
...
/client/proc/update_admins(var/rank)
	...
	switch (rank)
		if ("Host")
			holder.level = LEVEL_HOST
		if ("Coder")
			holder.level = LEVEL_CODER
		if ("Shit Person")
			holder.level = LEVEL_SHITGUY
		if ("Primary Administrator")
			holder.level = LEVEL_PA
		if ("Administrator")
			holder.level = LEVEL_ADMIN
		if ("Secondary Administrator")
			holder.level = LEVEL_SA
		if ("Moderator")
			holder.level = LEVEL_MOD
		if ("Goat Fart", "Ayn Rand's Armpit")
			holder.level = LEVEL_BABBY

		if ("Inactive")
			holder.dispose()
			holder = null
			boutput(src, "<span style='color:red;font-size:150%'><strong>You are set to Inactive admin status! Please join #ss13admin on irc.synirc.net if you would like to become active again!</strong></span>")
			return
	...
	if (holder)
		holder.owner = src
		for (var/i = 1; i < 9; i++)
			if (holder.level + 2 >= i && admin_verbs.len >= i && !isnull(admin_verbs[i]))
				verbs += admin_verbs[i]

		// certain ranks get special treatment while observing
		if ( holder.state ) // literally the laziest way to do this
			if ( holder.level > LEVEL_MOD)
				deadchat = 1
				verbs += special_admin_observing_verbs
			if ( holder.level > LEVEL_ADMIN )
				verbs += special_pa_observing_verbs
		else
			if ( holder.level > LEVEL_ADMIN)
				deadchat = 1
				verbs += special_admin_observing_verbs
			if ( holder.level > LEVEL_PA ) //SHIT GUY PLUS
				verbs += special_pa_observing_verbs
...
*/
	...
	if (!holder.popuptoggle) //Hrngh
		var/rank = holder.rank
		clear_admin_verbs()
		holder.state = 2
	//	mob.mind.observing = 1
		update_admins(rank)
	if (!istype(mob, /mob/dead/observer) && !istype(mob, /mob/dead/target_observer))
		mob.ghostize()
...
/client/proc/admin_play()
	...
	if (!holder.popuptoggle) //Hrngh x2
		var/rank = holder.rank
		clear_admin_verbs()
		holder.state = 1
	//	mob.mind.observing = 0
		update_admins(rank)
...
/client/proc/stealth()
	...
	//fuck u
	holder.set_stealth_mode(null, 0)

/admins/proc/set_stealth_mode(var/new_key = null, var/force_on = 0)
	if (!owner || !istype(owner, /client))
		return // farte
	if (force_on)
		owner:stealth = 1
	else
		owner:stealth = !(owner:stealth)

	if (owner:stealth)
		if (!new_key)
			new_key = input(usr, "Enter your desired display name.", "Fake Key", owner:key) as null|text
			if (!new_key)
				owner:stealth = 0
		if (owner:stealth && owner:alt_key)
			set_alt_key()
		if (new_key)
			...
			owner:fakekey = new_key
	else
		owner:fakekey = null
		owner:stealth_hide_fakekey = 0

	logTheThing("admin", owner, null, "has turned stealth mode [owner:stealth ? "ON using key \"[owner:fakekey]\"" : "OFF"]")
	logTheThing("diary", owner, null, "has turned stealth mode [owner:stealth ? "ON using key \"[owner:fakekey]\"" : "OFF"]", "admin")
	message_admins("[key_name(owner)] has turned stealth mode [owner:stealth ? "ON using key \"[owner:fakekey]\"" : "OFF"]")

	if (owner:stealth)
		var/ircmsg[] = new()
		ircmsg["key"] = owner:key
		ircmsg["name"] = (usr && usr.real_name) ? usr.real_name : "NULL"
		ircmsg["msg"] = "Has enabled stealth mode as ([owner:fakekey])"
...
/client/proc/alt_key()
	...
	holder.set_alt_key(null, 0)

/admins/proc/set_alt_key(var/new_key = null, var/force_on = 0)
	if (!owner || !istype(owner, /client))
		return // farte
	if (force_on)
		owner:alt_key = 1
	else
		owner:alt_key = !(owner:alt_key)
	...
	if (owner:alt_key)
		if (!new_key)
			...
			if (!new_key)
				owner:alt_key = 0
		if (owner:alt_key && owner:stealth)
			set_stealth_mode()
		if (new_key)
			new_key = trim(new_key)
			new_key = strip_html(new_key)
	...
	logTheThing("admin", owner, null, "has changed their displayed key to [owner:alt_key ? "\"[owner:fakekey]\"" : "\"[owner:key]\""]")
	logTheThing("diary", owner, null, "has changed their displayed key to [owner:alt_key ? "\"[owner:fakekey]\"" : "\"[owner:key]\""]", "admin")
	message_admins("[key_name(owner)] has changed their displayed key to [owner:alt_key ? "\"[owner:fakekey]\"" : "\"[owner:key]\""]")
/*
	if (alt_key)
		var/ircmsg[] = new()
		ircmsg["key"] = owner:key
		ircmsg["name"] = (usr && usr.real_name) ? usr.real_name : "NULL"
		ircmsg["msg"] = "Has set their displayed key to ([owner:fakekey])"
		ircbot.export("admin", ircmsg)
*/
/client/proc/banooc()
	set category = "Admin"
...
/client/proc/warn(var/mob/M in world)
	...
	if (!M.client.warned)
		...
		boutput(M, "<span style=\"color:red\"><strong>You have been warned by an administrator. This is the only warning you will receive.</strong></span>")
		M.client.warned = 1
		message_admins("<span style=\"color:blue\">[ckey] warned [M.ckey].</span>")
	else
		var/addData[] = new()
		addData["ckey"] = M.ckey
...
/client/proc/show_image_to_all()
	...
	if (fun_images.len)
		switch(alert("There is already an existing image.", "Warning", "Ignore", "Clear", "Cancel"))
			if ("Clear")
				for (var/hud/funimage/fun_image)
	...
	if (I)
		var/hud/funimage/fun_image = new(I)
...
/client/proc/show_rules_to_player(mob/M as mob in world)
	...
	boutput(M, "<span style=\"color:red\"><strong>WARNING: An admin is likely very cross with you and wants you to read the rules right fucking now!</strong></span>")
	M << browse(rules, "window=rules;size=480x320")
...
/client/proc/view_fingerprints(obj/O as obj in world)
	...
	boutput(src, "<strong>Hidden Fingerprints on [O]:</strong>")
	for (var/i in O.fingerprintshidden)
		boutput(src, i)

	boutput(src, "<strong>Last touched by:</strong> [O.fingerprintslast].")
	return
...
/client/proc/respawn_as(var/client/cli in clients)
	...
	cli.preferences.copy_to(H,mob,1)
	if (!mymob.mind)
		mymob.mind = new /mind()
		mymob.mind.key = key
...
/client/proc/cmd_admin_humanize(var/mob/M in world)
	...
	if (send_to_arrival_shuttle == 1)
		M.show_text("<h2><font color=red><strong>You have been respawned as a human and send to the arrival shuttle. If this is an unexpected development, please inquire about it in adminhelp.</strong></font></h2>", "red")
		return M.humanize(1)
	else
		M.show_text("<h2><font color=red><strong>You have been respawned as a human. If this is an unexpected development, please inquire about it in adminhelp.</strong></font></h2>", "red")
		return M.humanize(0)
...
/client/proc/cmd_chat_debug(var/client/C in clients)
	...
	if (src != C)
		var/trigger = (C.holder ? key : (stealth || fakekey ? fakekey : key))
...
/client/proc/blobsay(msg as text)
	...
	if (stealth || alt_key)
		show_other_key = 1
	var/rendered = "<span class='game blobsay'><span class='prefix'>BLOB:</span> <span class='name'>ADMIN([show_other_key ? fakekey : key])</span> says, <span class='message'>\"[msg]\"</span></span>"
	var/adminrendered = "<span class='game blobsay'><span class='prefix'>BLOB:</span> <span class='name' data-ctx='\ref[mob.mind]'>[show_other_key ? "ADMIN([key] (as [fakekey])" : "ADMIN([key]"])</span> says, <span class='message'>\"[msg]\"</span></span>"
...
/client/proc/cmd_dectalk()
	...
	if (audio && audio["audio"])
		...
		for (var/client/C in clients)
			var/trigger = key
			if (holder && (stealth || alt_key))
				trigger = (C.holder ? "[key] (as [fakekey])" : fakekey)
...
/client/proc/admin_changes()
	...
	if (winget(src, "adminchanges", "is-visible") == "true")
		Browse(null, "window=adminchanges")
	else
		...
		Browse(changelogHtml, "window=adminchanges;size=500x650;title=Admin+Changelog;")