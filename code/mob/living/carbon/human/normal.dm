	...
	New()
		...
		spawn (0)
			if (gender && gender == "female")
				real_name = pick(wiz_female)
			else
				real_name = pick(wiz_male)
...
/mob/living/carbon/human/normal/meteor_rescue
	New()
		...
		spawn (0)
			...
			equip_if_possible(new /obj/item/clothing/suit/armor/vest(src), slot_wear_suit)
			equip_if_possible(new /obj/item/clothing/mask/gas(src), slot_wear_mask)
			equip_if_possible(new /obj/item/clothing/gloves/black(src), slot_gloves)
			equip_if_possible(new /obj/item/clothing/glasses/meson(src), slot_glasses)
			spawnId("Rescue Worker")
