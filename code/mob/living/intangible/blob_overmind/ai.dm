...
/mob/living/intangible/blob_overmind/ai
	...
	var/counter = 4
	var/refresh_lists = 0
	var/blob_ability/deploy = null
	var/blob_ability/attack = null
	var/blob_ability/spread = null
	var/blob_ability/lipid = null
	var/blob_ability/mito = null
	var/blob_ability/wall = null
	var/blob_ability/absorb = null
	var/blob_upgrade/spread_up = null
	var/blob_upgrade/gen_up = null
	var/blob_upgrade/fireres_up = null
	...
	New()
		...
		name = "Blob AI #[ai_id]"
		real_name = name
		deploy = locate(/blob_ability/plant_nucleus) in abilities
	...
	Life(controller/process/mobs/parent)
		...
		if (refresh_lists > 50 && state > 1)
			logTheThing("debug", src, null, "<strong>Marquesas/AI Blob:</strong> Refreshing lists.")
			refresh_lists = 0
		...
		if (state > 1)
			if (fireres_up)
				if (fireres_up.check_requirements())
					...
					fireres_up = null
					logTheThing("debug", src, null, "<strong>Marquesas/AI Blob:</strong> Took fire resistance upgrade.")
					...
					if (!(locate(/obj/blob) in H.loc))
						...
						if (has_adjacent_blob(T) && prob(15))
							...
							if (T.can_blob_spread_here())
								spread_to(T, 0)
							logTheThing("debug", src, null, "<strong>Marquesas/AI Blob:</strong> Can't absorb [H] (no blob on tile), attacking instead at [log_loc(H)].")
						continue
					if (prob(118 - 3 * get_dist(src, H)))
						absorb.onUse(H.loc)
						logTheThing("debug", src, null, "<strong>Marquesas/AI Blob:</strong> Absorbing [H].")
		...
		switch (state)
			...
			if (STATE_DEPLOYING)
				...
				if (counter <= 0)
					...
					set_loc(T)
					if (!deploy)
						logTheThing("debug", src, null, "<strong>Marquesas/AI Blob:</strong> Invalid state for [src]: Cannot find deploy ability in state DEPLOYING.")
						state = 0
						return
					...
					if (deploy in abilities)
						logTheThing("debug", src, null, "<strong>Marquesas/AI Blob:</strong> Deploy failed.")
						return
					state = STATE_EXPANDING
					last_spread = T
					update_lists(T)
					spread = locate(/blob_ability/spread) in abilities
					attack = locate(/blob_ability/attack) in abilities
					lipid = locate(/blob_ability/build/ribosome) in abilities
					mito = locate(/blob_ability/build/mitochondria) in abilities
					wall = locate(/blob_ability/build/wall) in abilities
					absorb = locate(/blob_ability/absorb) in abilities
					spread_up = locate(/blob_upgrade/quick_spread) in available_upgrades
					gen_up = locate(/blob_upgrade/extra_genrate) in available_upgrades
					fireres_up = locate(/blob_upgrade/fire_resist) in available_upgrades
					logTheThing("debug", src, null, "<strong>Marquesas/AI Blob:</strong> Deployed blob to ([T.x], [T.y], [T.z]).")
					counter = 0
			if (STATE_EXPANDING)
				refresh_lists++
				...
				if (gen_up)
					if (gen_up.check_requirements())
						gen_up.take_upgrade()
						logTheThing("debug", src, null, "<strong>Marquesas/AI Blob:</strong> Took generation rate upgrade while expanding.")
				if (spread_up)
					if (spread_up.check_requirements())
						spread_up.take_upgrade()
						logTheThing("debug", src, null, "<strong>Marquesas/AI Blob:</strong> Took spread upgrade while expanding.")
				var/turf/ST = null
				...
				if (ST && !destroying)
					if (!spread)
						logTheThing("debug", src, null, "<strong>Marquesas/AI Blob:</strong> Invalid state for [src]: Cannot find spread ability in state EXPANDING.")
					spread_to(ST, 1)
					return
				if ((open_medium.len && !destroying) || (destroying && destroy_level == 1))
					...
					if (ST)
						...
						set_loc(ST)
						attack.onUse(ST)
						logTheThing("debug", src, null, "<strong>Marquesas/AI Blob:</strong> Attacking tile at [showCoords(ST.x, ST.y, ST.z)].")
			...
			if (STATE_DO_LIPIDS)
				...
				if (!A)
					state = STATE_EXPANDING
					logTheThing("debug", src, null, "<strong>Marquesas/AI Blob:</strong> Failed to find suitable lipid candidate in 20 attempts.")
					return
				...
				if (L)
					lipid_count++
				logTheThing("debug", src, null, "<strong>Marquesas/AI Blob:</strong> Creating lipid at [showCoords(T.x, T.y, T.z)].")
				state = STATE_EXPANDING
			if (STATE_FORTIFYING)
				if (!fortifying)
					state = STATE_EXPANDING
				...
				if (attacker)
					...
					for (var/obj/reagent_dispensers/fueltank/FU in view(attacker))
						if (has_adjacent_blob(FU.loc))
							...
							logTheThing("debug", src, null, "<strong>Marquesas/AI Blob:</strong> Attacking fuel tank at [showCoords(FU.x, FU.y, FU.z)] in response to attack force.")
					if (has_adjacent_blob(AT))
						attack_now(AT, attack_used)
						logTheThing("debug", src, null, "<strong>Marquesas/AI Blob:</strong> Hitting [attacker] at [showCoords(attacker.loc.x, attacker.loc.y, attacker.loc.z)] [attacks - attack_used] times.")
					...
					if (!spreaded)
						for (var/turf/T in range(2, attacker))
							if (T.can_blob_spread_here())
								spread_to(T, 0)
								logTheThing("debug", src, null, "<strong>Marquesas/AI Blob:</strong> Spreading near [attacker] to [showCoords(T.x, T.y, T.z)] in response to attack force.")
								break
				else if (!attacker)
					var/obj/blob/F = null
					//var/preferred = 0
					...
					for (var/turf/T in range(5, nearest))
						if (T.can_blob_spread_here())
							spread_to(T, 0)
							logTheThing("debug", src, null, "<strong>Marquesas/AI Blob:</strong> Spreading near nearest [nearest] to [showCoords(T.x, T.y, T.z)] in response to attack force.")
							break
	...
	proc/spread_to(var/turf/ST, var/is_calm)
		...
		if (locate(/obj/blob) in ST)
			...
			if (is_calm)
				last_spread = ST
			logTheThing("debug", src, null, "<strong>Marquesas/AI Blob:</strong> Spreading to [showCoords(ST.x, ST.y, ST.z)].")
		else
			return
		update_lists(ST)
		if (!is_calm)
	...
	proc/create_mitochondria_if_possible(var/turf/T)
		if (bio_points >= mito.bio_point_cost && mito.last_used <= world.time)
			set_loc(T)
			mito.onUse(T)
			logTheThing("debug", src, null, "<strong>Marquesas/AI Blob:</strong> Creating mitochondria at [showCoords(T.x, T.y, T.z)].")
			return TRUE
		return FALSE
	...
	proc/create_wall_if_possible(var/turf/T)
		if (bio_points >= wall.bio_point_cost && wall.last_used <= world.time)
			set_loc(T)
			wall.onUse(T)
			logTheThing("debug", src, null, "<strong>Marquesas/AI Blob:</strong> Creating wall at [showCoords(T.x, T.y, T.z)].")
			return TRUE
		return FALSE
