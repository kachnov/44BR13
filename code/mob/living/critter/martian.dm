/mob/living/critter/martian
	...
	setup_healths()
		...
		add_health_holder(/healthHolder/suffocation)
		var/healthHolder/Brain = add_health_holder(/healthHolder/brain)
	...
	New()
		..()
		abilityHolder.addAbility(/targetable/critter/psyblast/martian)
		abilityHolder.addAbility(/targetable/critter/teleport)
	...
	say(message, involuntary = 0)
		...
		for (var/mob/living/critter/martian/M in mobs)
			if (M.client && !M.stat)
				boutput(M, "<span style='color:#E89235'><strong>[real_name]</strong> telepathically messages, \"[message]\"</span>")
	...
	soldier
		...
		setup_hands()
			...
			var/handHolder/HH = hands[2]
			HH.limb = new /limb/hitscan
	...
	mutant
		...
		New()
			..()
			abilityHolder.addAbility(/targetable/critter/gibstare)
