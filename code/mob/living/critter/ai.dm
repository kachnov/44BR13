/aiHolder
	...
	var/aiTask/current_task = null  // what the critter is currently doing
	var/aiTask/default_task = null  // what behavior the critter will fall back on
	...
	proc/tick()
		...
		if (current_task)
			...
			var/aiTask/T = current_task.next_task()
			if (T)
				current_task = T
				T.reset()
...
/aiTask/prioritizer
	...
	tick()
		..()
		for (var/aiTask/T in transition_tasks)
...
/aiTask/timed
	...
	var/aiTask/transition_task = null
