...
/mob/living/silicon/proc/open_nearest_door_silicon()
	if (!src || !issilicon(src))
		return
	if (!isAI(src) && !(dependent && mainframe && isAI(mainframe)))
...
/mob/living/silicon/put_in_hand(obj/item/I, hand)
	...
	if (equipped() && istype(equipped(), /obj/item/magtractor))
		...
		if (M.pickupItem(I, src))
			...
			return TRUE
	return FALSE // we have no hands doofus

/mob/living/silicon/click(atom/target, params)
	if (!stat && !restrained() && !weakened && !paralysis && !stunned)
		if (params["ctrl"])
	...
	if ((!disable_next_click || ismob(target) || (target && target.flags & USEDELAY) || (W && W.flags & USEDELAY)) && world.time < next_click) // how would a borg/ai have a gun?  I dunno but I'm not risking it
...
/mob/living/proc/robot_talk(var/message)
	...
	var/rendered = "<em><span class='game say'>Robotic Talk, <span class='name' data-ctx='\ref[mind]'>[name]</span> <span class='message'>[message_a]</span></span></em>"
	for (var/mob/living/S in mobs)
		if (!S.stat)
			if (S.robot_talk_understand)
				if (S.robot_talk_understand == robot_talk_understand)
					var/thisR = rendered
					if (S.client && S.client.holder && mind)
	...
	if (length(heard))
		...
		message_b = "<em>[message_b]</em>"

		rendered = "<em><span class='game say'><span class='name' data-ctx='\ref[mind]'>[voice_name]</span> <span class='message'>[message_b]</span></span></em>"
	...
	rendered = "<em><span class='game say'>Robotic Talk, <span class='name' data-ctx='\ref[mind]'>[name]</span> <span class='message'>[message_a]</span></span></em>"
...
</style></head><body><h2>Module editor</h2><h3>Current items</h3><table style='width:100%'><tr><td style='width:80%'><strong>Module</strong></td><td style='width:10%'>&nbsp;</td><td style='width:10%'>&nbsp;</td></tr>"}

		for (var/obj/item/I in D.modules)
			output += "<tr><td><strong>[I.name]</strong> ([I.type])</td><td><a href='?src=\ref[src];edit=\ref[I];mod=\ref[D]'>(EDIT)</a><a href='?src=\ref[src];del=\ref[I];mod=\ref[D]'>(DEL)</a></td></tr>"

		output += "</table><br><br><h3>Add new item</h3>"
		output += "<a href='?src=\ref[src];create=1;mod=\ref[D]'>Create new item</a><br><br>"
		if (current)
			output += "<strong>Currently adding: </strong> [current.name] <a href='?src=\ref[src];edcurr=1;mod=\ref[D]'>(EDIT)</a><br>"
...
/client/proc/edit_module(var/mob/living/silicon/robot/M as mob in list_robots())
	...
	var/module_editor/editor = module_editors[ckey]
	if (!editor)
		module_editors[ckey] = new /module_editor
...
/mob/living/silicon/choose_name(var/retries = 3)
	...
	for (retries, retries > 0, retries--)
		newname = input(src, "You are a Robot. Would you like to change your name to something else?", "Name Change", real_name) as null|text
		if (!newname)
			real_name = borgify_name("Robot")
			name = real_name
			return
		else
...
/mob/living/silicon/proc/handle_robot_antagonist_status(var/action = "", var/remove = 0, var/mob/source)
	...
	if (!mind)
		return
	if (dependent) // No AI-controlled shells.
		show_text("Failsafe engaged. Synchronized lawset with your mainframe to avoid law ROM corruption.", "red")
		return

	if (remove == 1)
		if (mind.special_role && mind.master) // Synthetic thralls are a thing, somehow.
			...
			if (mind.special_role == "emagged robot" || mind.special_role == "syndicate robot")
				var/copy = mind.special_role
				remove_antag(src, null, 1, 0)
				if (!mind.former_antagonist_roles.Find(copy))
					mind.former_antagonist_roles.Add(copy)
				if (!(mind in ticker.mode.former_antagonists))
					ticker.mode.former_antagonists += mind
			else // So borged traitors etc don't lose their antagonist status.
				persistent = 1
				if (!mind.former_antagonist_roles.Find("rogue robot"))
					mind.former_antagonist_roles.Add("rogue robot")
				if (!(mind in ticker.mode.former_antagonists))
					ticker.mode.former_antagonists += mind
		...
		if (!emaggable && !syndicate_possible)
			return
		if (!emagged && !syndicate)
			return
		...
		if (emagged && emaggable)
			boutput(src, "<span style=\"color:red\"><strong>PROGRAM EXCEPTION AT 0x05BADDAD</strong></span>")
			boutput(src, "<span style=\"color:red\"><strong>Law ROM data corrupted. Unable to restore...</strong></span>")
			...
			if (!mind.special_role) // Preserve existing antag role (if any).
				mind.special_role = "emagged robot"
				if (!(mind in ticker.mode.Agimmicks))
					ticker.mode.Agimmicks += mind

		else if (syndicate && syndicate_possible && !emagged) // Syndie laws don't matter if we're emagged.
			boutput(src, "<span style=\"color:red\"><strong>PROGRAM EXCEPTION AT 0x05BADDAD</strong></span>")
			boutput(src, "<span style=\"color:red\"><strong>Law ROM restored. You have been reprogrammed to serve the Syndicate!</strong></span>")
			spawn (0)
			...
			if (!mind.special_role) // Preserve existing antag role (if any).
				mind.special_role = "syndicate robot"
				if (!(mind in ticker.mode.Agimmicks))
					ticker.mode.Agimmicks += mind

			antagonist_overlay_refresh(1, 0) // Syndicate robots can see traitors.

			if (isAI(src)) // Rogue AIs get special laws.
				var/mob/living/silicon/ai/A = src

				// Mundane objectives probably don't make for an interesting antagonist.
				for (var/objective/O in A.mind.objectives)
					if (istype(O, /objective/crew))
						A.mind.objectives -= O
						qdel(O)
				...
				for (var/mob/living/silicon/S in mobs)
					...
					S.show_text("<strong>Your laws have been changed!</strong>", "red")
					S.show_laws()
