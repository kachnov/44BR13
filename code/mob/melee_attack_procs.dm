...
/mob/proc/do_help(var/mob/living/M)
	if (!istype(M))
		return
	if (M.health <= 0 && health >= -75.0)
		if (src == M && is_bleeding())
			staunch_bleeding(M) // if they've got SOMETHING to do let's not just harass them for trying to do CPR on themselves
		else
			administer_CPR(M)
	else if (is_bleeding())
		staunch_bleeding(M)
...
/mob/proc/shake_awake(var/mob/living/target)
	...
	visible_message("<span style=\"color:blue\">[src] shakes [target], trying to wake them up!</span>")
	target.hit_twitch()
...
/mob/living/carbon/human/administer_CPR(var/mob/living/carbon/human/target)
	...
	visible_message("<span style=\"color:red\"><strong>[src] is trying to perform CPR on [target]!</strong></span>")
	if (do_mob(src, target, 40))
		...
		if (target.health < 0)
			...
			/*if (prob(50)) // this doesn't work yet
				for (var/ailment/disability/D in target.ailments)
					if (istype(D,/ailment/disease/heartfailure))
						target.resistances += D.type
						target.ailments -= D
						boutput(world, "<span style=\"color:red\">CURED [D] in [target]</span>")
					if (istype(D,/ailment/disease/flatline))
						target.resistances += D.type
						target.ailments -= D
						boutput(world, "<span style=\"color:red\">CURED [D] in [target]</span>")*/
			...
			if (src)
				visible_message("<span style=\"color:red\">[src] performs CPR on [target]!</span>")
...
/mob/living/carbon/proc/grab_self()
	...
	if (istype(wear_mask,/obj/item/clothing/mask/moustache))
		visible_message("<span style=\"color:red\"><strong>[src] twirls [his_or_her(src)] moustache and laughs [pick_string("tweak_yo_self.txt", "moustache")]!</strong></span>")
	else
		visible_message("<span style=\"color:red\"><strong>[src] tweaks [his_or_her(src)] own nipples! That's [pick_string("tweak_yo_self.txt", "tweakadj")] [pick_string("tweak_yo_self.txt", "tweak")]!</strong></span>")
...
/mob/living/proc/grab_other(var/mob/living/carbon/human/target, var/suppress_final_message = 0)
	...
	if (check_target_immunity(target) == 1)
		...
		target.visible_message("<span style=\"color:red\"><strong>[src] tries to grab [target], but can't get a good grip!</strong></span>")
		return
	...
	if (istype(H))
		...
		if (!target.stat && !target.weakened && !target.stunned && !target.paralysis && target.a_intent == "disarm" && target.stamina > STAMINA_DEFAULT_BLOCK_COST && prob(STAMINA_GRAB_BLOCK_CHANCE) && !target.equipped())
			visible_message("<span style=\"color:red\"><strong>[target] blocks [src]'s attempt to grab [him_or_her(target)]!</span>")
	...
	if (istype(H))
		for (var/uid in H.pathogens)
			var/pathogen/P = H.pathogens[uid]
			P.ongrab(target)
...
/mob/proc/disarm(var/mob/living/target, var/extra_damage = 0, var/suppress_flags = 0, var/damtype = DAMAGE_BLUNT)
	...
	var/attackResults/disarm/msgs = calculate_disarm_attack(target, affecting, 0, 0, extra_damage)
...
/mob/proc/calculate_disarm_attack(var/mob/target, var/obj/item/affecting, var/base_damage_low = 0, var/base_damage_high = 0, var/extra_damage = 0)
	var/attackResults/disarm/msgs = new(src)
	msgs.clear(target)
	msgs.valid = 1
	msgs.disarm = 1
		...
		if (ishuman(src))
			...
			if (prob((stampart + 10) * mult))
				for (var/uid in H.pathogens)
					...
					if (!ret)
						msgs.base_attack_message = "<span style=\"color:red\"><strong>[src] shoves [target]!</strong></span>"
						return msgs
				msgs.base_attack_message = "<span style=\"color:red\"><strong>[src] shoves [target] to the ground!</strong></span>"
	...
	if (I)
		if (I.cant_other_remove)
			...
			msgs.base_attack_message = "<span style=\"color:red\"><strong>[src] vainly tries to knock [I] out of [target]'s hand!</strong></span>"
			...
			if (ishuman(src))
				...
				for (var/uid in H2.pathogens)
					var/pathogen/P = H2.pathogens[uid]
					var/ret = P.ondisarm(target, 1)
					if (!ret)
						msgs.base_attack_message = "<span style=\"color:red\"><strong>[src] tries to knock [I] out of [target]'s hand!</strong></span>"
						return msgs
			msgs.base_attack_message = "<span style=\"color:red\"><strong>[src] knocks [I] out of [target]'s hand!</strong></span>"
			msgs.played_sound = 'sound/weapons/thudswoosh.ogg'
...
/mob/proc/do_block(var/mob/attacker)
	if (check_block())
		visible_message("<span style=\"color:red\"><strong>[src] blocks [attacker]'s attack!</span>")
...
/mob/living/carbon/human/proc/stun_glove_attack(var/mob/living/target)
	...
	if (gloves.uses > 0)
		...
		unlock_medal("High Five!", 1)

		var/effects/system/spark_spread/s = unpool(/effects/system/spark_spread)
		s.set_up(5, 1, target.loc)
		s.start()

		gloves.uses = max(0, gloves.uses - 1)
		if (gloves.uses < 1)
			gloves.icon_state = "yellow"
			src.gloves.item_state = "ygloves"
			update_clothing() // Was missing (Convair880).

		if (gloves.uses <= 0)
			show_text("The gloves are no longer electrically charged.", "red")
		else
			src.show_text("The gloves have [src.gloves.uses]/[src.gloves.max_uses] charges left!", "red")

		target.visible_message("<span style=\"color:red\"><strong>[src] touches [target] with the stun gloves!</strong></span>")
		if (check_target_immunity(target) == 1)
			target.visible_message("<span style=\"color:red\"><strong>...but it has no effect whatsoever!</strong></span>")
			return
...
/mob/proc/melee_attack_normal(var/mob/living/carbon/human/target, var/extra_damage = 0, var/suppress_flags = 0, var/damtype = DAMAGE_BLUNT)
	...
	var/obj/item/affecting = target.get_affecting(src)
	var/attackResults/msgs = calculate_melee_attack(target, affecting, 2, 9, extra_damage)
	msgs.damage_type = damtype
...
/mob/proc/calculate_melee_attack(var/mob/target, var/obj/item/affecting, var/base_damage_low = 2, var/base_damage_high = 9, var/extra_damage = 0)
	var/attackResults/msgs = new(src)
	msgs.clear(target)
	msgs.valid = 1
	...
	if (!punchedmult)
		...
		msgs.visible_message_self("<span style=\"color:red\"><strong>[src] [punchMessage] [target], but it does absolutely nothing!</strong></span>")
		return

	if (!punchmult)
		msgs.played_sound = 'sound/effects/snap.ogg'
		msgs.visible_message_self("<span style=\"color:red\"><strong>[src] hits [target] with a ridiculously feeble attack!</strong></span>")
		return
	...
	if (!target.canmove && target.lying)
		...
		msgs.base_attack_message = "<span style=\"color:red\"><strong>[src] [kickMessage] [target]!</strong></span>"
		msgs.logs = list("[kickMessage] %target%")

		if (STAMINA_LOW_COST_KICK)
			msgs.stamina_self += STAMINA_HTH_COST / 3
	else
		msgs.base_attack_message = "<span style=\"color:red\"><strong>[src] [punchMessage] [target]!</strong></span>"
		msgs.played_sound = "punch"

		if (src != target && iswrestler(src) && prob(66))
			msgs.base_attack_message = "<span style=\"color:red\"><strong>[src]</strong> winds up and delivers a backfist to [target], sending them flying!</span>"
			damage += 4
...
/proc/special_attack_silicon(var/mob/target, var/mob/living/user)
	...
	if (check_target_immunity(target) == 1)
		...
		user.visible_message("<span style=\"color:red\"><strong>[user]'s attack bounces off [target] uselessly!</strong></span>")
		return
	...
	if (isrobot(target))
		...
		if (!BORG.part_head)
			user.visible_message("<span style=\"color:red\"><strong>[user] smashes [BORG.name] to pieces!</strong></span>")
			playsound(user.loc, "sound/effects/wbreak.wav", 70, 1)
			BORG.gib()
		else
			if (BORG.part_head.ropart_get_damage_percentage() >= 85)
				user.visible_message("<span style=\"color:red\"><strong>[user] grabs [BORG.name]'s head and wrenches it right off!</strong></span>")
				playsound(user.loc, "sound/effects/wbreak.wav", 70, 1)
				BORG.compborg_lose_limb(BORG.part_head)
			else
				user.visible_message("<span style=\"color:red\"><strong>[user] pounds on [BORG.name]'s head furiously!</strong></span>")
				...
				if (!BORG.anchored && prob(30))
					user.visible_message("<span style=\"color:red\"><strong>...and sends them flying!</strong></span>")
					send_flying = 2

	else if (isAI(target))
		user.visible_message("<span style=\"color:red\"><strong>[user] [pick("wails", "pounds", "slams")] on [target]'s terminal furiously!</strong></span>")
		playsound(user.loc, "sound/effects/metal_bang.ogg", 50, 1)
		damage = 10

	else
		user.visible_message("<span style=\"color:red\"><strong>[user] smashes [target] furiously!</strong></span>")
		playsound(user.loc, "sound/effects/metal_bang.ogg", 50, 1)
		damage = 10
...
/attackResults
	...
	proc/clear(var/mob/M)
		...
		show_self = list()
		show_target = list()
		if (istype(src, /attackResults/disarm))
	...
	proc/flush(var/suppress = 0)
		if (!target)
			clear(null)
			logTheThing("debug", owner, null, "<strong>Marquesas/Melee Attack Refactor:</strong> NO TARGET FLUSH! EMERGENCY!")
			return

		if (!affecting)
			clear(null)
			logTheThing("debug", owner, null, "<strong>Marquesas/Melee Attack Refactor:</strong> NO AFFECTING FLUSH! WARNING!")
			return
		...
		if (disarm == 1)
			...
			if (!isnull(disarm_RNG_result))
				switch (disarm_RNG_result)
					if ("drop_item")
		...
		if (damage > 0 || disarm == 1)
			if (disarm == 1 && damage <= 0)
...
/mob/proc/melee_attack_test(var/mob/attacker, var/obj/item/I, var/def_zone, var/disarm_check = 0)
	if (check_target_immunity(src) == 1)
		...
		visible_message("<span style=\"color:red\"><strong>[attacker]'s attack bounces off [src] uselessly!</strong></span>")
		return FALSE

	if (disarm_check == 1 && (ishuman(src) && lying == 1))
		visible_message("<span style=\"color:red\"><strong>[attacker]</strong> makes a threatening gesture at [src]!</span>")
		return FALSE
...
/mob/living/silicon/robot/melee_attack_test(var/mob/attacker, var/obj/item/I, var/def_zone, var/disarm_check = 0)
	...
	if (I)
		...
		if (!prob(hit_chance))
			playsound(loc, "sound/weapons/punchmiss.ogg", 50, 1, 1)
			visible_message("<span style=\"color:red\"><strong>[attacker] swings at [src] with [I], but misses!</strong></span>")
			return FALSE
...
/mob/proc/calculate_bonus_damage(var/attackResults/msgs)
	return FALSE

/mob/living/carbon/human/calculate_bonus_damage(var/attackResults/msgs)
	var/damage = 0
	if (gloves)
		damage += gloves.damage_bonus()

	if (reagents && (reagents.get_reagent_amount("ethanol") >= 100) && prob(40))
		damage += rand(3,5)
		if (msgs)
