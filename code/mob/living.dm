...
/mob/living/disposing()
	...
	observers.len = 0
	if (static_image)
		mob_static_icons.Remove(static_image)
		static_image = null
	..()

/mob/living/death(gibbed)
	if (key) statlog_death(src,gibbed)
	if (client && (ticker.round_elapsed_ticks >= 12000))
		var/num_players = 0
		...
		if (num_players <= 5)
			if (!emergency_shuttle.online && ticker && ticker.current_state != GAME_STATE_FINISHED && ticker.mode.crew_shortage_enabled)
				...
				boutput(world, "<span style=\"color:blue\"><strong>Alert: Due to crew shortages and fatalities, the emergency shuttle has been called. It will arrive in [round(emergency_shuttle.timeleft()/60)] minutes.</strong></span>")
...
/mob/living/Life(controller/process/mobs/parent)
	#ifdef MAP_OVERRIDE_DESTINY
	if (hibernating)
		if (istype(loc, /obj/cryotron))
			if (!stat)
				stat = 1
			return TRUE
		else
...
/mob/living/update_camera()
	for (var/mob/dead/target_observer/observer in observers)
		if (observer.client)
			apply_camera(observer.client)
	..()

/mob/living/attach_hud(hud/hud)
	for (var/mob/dead/target_observer/observer in observers)
		observer.attach_hud(hud)
	return ..()

/mob/living/detach_hud(hud/hud)
	for (var/mob/dead/target_observer/observer in observers)
		observer.detach_hud(hud)
	return ..()

/mob/living/projCanHit(projectile/P)
	if (!lying || (src:lying && prob(P.hit_ground_chance))) return TRUE
	return FALSE
...
/mob/living/click(atom/target, params)
	...
	#ifdef MAP_OVERRIDE_DESTINY
	if (hibernating && istype(loc, /obj/cryotron))
		var/obj/cryotron/cryo = loc
	...
	if (!stat && !restrained() && !weakened && !paralysis && !stunned)
		...
		if (reach || (W && (W.flags & EXTRADELAY))) //Fuck you, magic number prickjerk
			...
			if (invisibility > 0 && get_dist(src, target) > 0) // dont want to check for a cloaker every click if we're not invisible
			...
			if (ears) ears.equipment_click(target, src)
			...
			if (l_hand) l_hand.equipment_click(target, src)
			if (hasvar(src, "l_store") && src:l_store) src:l_store:equipment_click(target, src)
			if (r_hand) r_hand.equipment_click(target, src)
			...
			if (wear_mask) wear_mask.equipment_click(target, src)
...
/mob/living/update_cursor()
	...
	if (client)
		if (in_point_mode)
			set_cursor('icons/cursors/point.dmi')
			return

		if (client.check_key("alt"))
			set_cursor('icons/cursors/examine.dmi')
			return

		if (client.check_key("ctrl"))
			set_cursor('icons/cursors/pull.dmi')
			return
...
/mob/living/verb/point(var/atom/target as mob|obj|turf in oview())
	...
	if (reagents && reagents.has_reagent("capulettium_plus"))
		show_text("You are completely paralysed and can't point!", "red")
		return

	if (istype(target, /obj/decal/point))
		return

	var/obj/item/gun/G = equipped()
	if (!istype(G) || !ismob(target))
		visible_message("<strong>[src]</strong> points to [target].")
	else
		visible_message("<span style='font-weight:bold;color:#f00;font-size:120%;'>[src] points \the [G] at [target]!</span>")
...
/mob/living/proc/talk_into_equipment(var/mode, var/messages, var/param, var/lang_id)
	switch (mode)
		if ("headset")
			if (ears)
				ears.talk_into(src, messages, param, real_name, lang_id)
		...
		if ("right hand")
			if (r_hand)
				r_hand.talk_into(src, messages, param, real_name, lang_id)
			else
				spawn (0)
					emote("handpuppet")
		...
		if ("left hand")
			if (l_hand)
				l_hand.talk_into(src, messages, param, real_name, lang_id)
			else
				spawn (0)
					emote("handpuppet")
...
//	var/turf/T = get_turf(src)
	...
	if (istype(loc, /obj/machinery/colosseum_putt))
		...
		logTheThing("say", src, null, "<em>broadcasted between Colosseum Putts:</em> \"[message]\"")
		C.broadcast(message)
		return
	...
	if (istype(src, /mob/living/intangible/blob_overmind))
		...
		rendered += "<span class='name text-normal' data-ctx='\ref[mind]'>[get_heard_name()]</span> "
			...
			switch (message_mode)
				if ("internal 1")
					if (R1 && !(A.stat || A.stunned || A.weakened)) // Mainframe may be stunned when the shell isn't.
						...
						show_text("Mainframe radio inoperable or unavailable.", "red")
				if ("internal 2")
	...
	message = say_quote(messages[1])
	if (italics)
		message = "<em>[message]</em>"

	rendered = "<span class='game say'>[get_heard_name()] <span class='message'>[message]</span></span>"
...
/mob/proc/get_heard_name()
	return "<span class='name' data-ctx='\ref[mind]'>[name]</span>"

// say filters

/proc/say_drunk(var/string)
	var/modded = ""
	var/text_roamer/T = new/text_roamer(string)
...
/proc/say_furious(var/string)
	...
	for (var/i = 0, i < length(string), i=i)
		var/parse_result/P = say_furious_parse(T)
		modded += P.string
		i += P.chars_used
...
/proc/say_furious_parse(var/text_roamer/R)
	var/new_string = ""
	var/used = 0
	...
	var/parse_result/P = new/parse_result
	P.string = new_string
	P.chars_used = used
	return P
...
/proc/say_gurgle(var/string)
	...
	for (var/i = 0, i < length(string), i=i)
		var/parse_result/P = say_gurgle_parse(T)
		modded += P.string
		i += P.chars_used
		T.curr_char_pos = T.curr_char_pos + P.chars_used
		T.update()
...
/proc/say_gurgle_parse(var/text_roamer/R)
	var/new_string = ""
	var/used = 0
...
/proc/chavify(var/string)
	...
	for (var/i = 0, i < length(string), i=i)
		var/parse_result/P = chav_parse(T)
		modded += P.string
		i += P.chars_used
		T.curr_char_pos = T.curr_char_pos + P.chars_used
		T.update()
...
/proc/chav_parse(var/text_roamer/R)
	var/new_string = ""
	var/used = 0
...
/proc/smilify(var/string)
	...
	for (var/i = 0, i < length(string), i=i)
		var/parse_result/P = smile_parse(T)
		modded += P.string
		i += P.chars_used
		T.curr_char_pos = T.curr_char_pos + P.chars_used
		T.update()
...
/proc/smile_parse(var/text_roamer/R)
	var/used = 0
	var/new_string = ""
...
/mob/living/change_misstep_chance(var/amount)
	...
	misstep_chance = max(0,min(misstep_chance + amount,100))
	return
...
/mob/living/proc/get_static_image()
	...
	if (static_image)
		mob_static_icons.Remove(static_image)
	static_image = getTexturedImage(build_flat_icon(), "static", ICON_OVERLAY)//src, "static", ICON_OVERLAY)
	if (static_image)
		static_image.override = 1
		static_image.loc = src
		mob_static_icons.Add(static_image)
		DEBUG(bicon(static_image))
		return static_image
