...
/gas_mixture
	...
	var/list/gas/trace_gases
	var/tmp/fuel_burnt = 0
	
	
// Overrides
/gas_mixture/disposing()
	if (trace_gases)
		trace_gases = null
	..()
...
// Gas equation procs
/gas_mixture/proc/heat_capacity()
	var/heat_capacity = HEAT_CAPACITY_CALCULATION(oxygen,carbon_dioxide,nitrogen,toxins)

	if (trace_gases && trace_gases.len)
		for (var/gas/trace_gas in trace_gases)
			heat_capacity += trace_gas.moles*trace_gas.specific_heat


	return heat_capacity
	
/gas_mixture/proc/heat_capacity_archived()
...
/gas_mixture/proc/return_pressure()
	return (total_moles()*R_IDEAL_GAS_EQUATION*temperature/volume)

/gas_mixture/proc/thermal_energy()
	return temperature*heat_capacity()

// Mutator procs
// For specific events
/gas_mixture/proc/zero()
	trace_gases = null
	oxygen = 0
	carbon_dioxide = 0
	nitrogen = 0
...
/gas_mixture/proc/check_tile_graphic()
	...
	if (toxins > MOLES_PLASMA_VISIBLE)
		...
		var/gas/sleeping_agent = locate(/gas/sleeping_agent) in trace_gases
		if (sleeping_agent && (sleeping_agent.moles > 1))
			graphic += "n2o;"
		var/gas/rad_particles = locate(/gas/rad_particles) in trace_gases
		if (rad_particles && (rad_particles.moles > 1))
			graphic += "n2o;"

	return graphic != graphic_archived

/gas_mixture/proc/react(atom/dump_location)
	var/reacting = 0 //set to 1 if a notable reaction occured (used by pipe_network)

	if (trace_gases && trace_gases.len > 0)
		if (temperature > 900)
			if (toxins > MINIMUM_HEAT_CAPACITY && carbon_dioxide > MINIMUM_HEAT_CAPACITY)
				var/gas/oxygen_agent_b/trace_gas = locate(/gas/oxygen_agent_b/) in trace_gases
				if (trace_gas)
...
/gas_mixture/proc/fire()
	var/energy_released = 0
	var/old_heat_capacity = heat_capacity()

	if (trace_gases && trace_gases.len)
		var/gas/volatile_fuel/fuel_store = locate(/gas/volatile_fuel/) in trace_gases
...
/gas_mixture/proc/check_then_merge(gas_mixture/giver)
	if (!giver)
		return FALSE
	...
	if (giver.trace_gases && giver.trace_gases.len)
		for (var/gas/trace_gas in giver.trace_gases)
			var/gas/corresponding
			if (trace_gases && trace_gases.len)
...
/gas_mixture/proc/merge(gas_mixture/giver)
	if (!giver)
		return FALSE
...
/gas_mixture/proc/remove(amount)
	var/sum = total_moles()
	amount = min(amount,sum) //Can not take more air than tile has!
	if (amount <= 0)
		return null

	var/gas_mixture/removed = unpool(/gas_mixture)
	...
	if (trace_gases && trace_gases.len)
		for (var/gas/trace_gas in trace_gases)
			var/gas/corresponding = new trace_gas.type()
...
//Proportionally removes amount of gas from the gas_mixture
//Returns: gas_mixture with the gases removed
/gas_mixture/proc/remove_ratio(ratio)
	if (ratio <= 0)
		return null
...
/gas_mixture/proc/check_then_remove(amount)
...
/gas_mixture/proc/copy_from(gas_mixture/sample)
	if (sample == null)
		return
	...
	if (sample.trace_gases && sample.trace_gases.len > 0)
		trace_gases = list()
		for (var/gas/trace_gas in sample.trace_gases)
			var/gas/corresponding = new trace_gas.type()
			trace_gases += corresponding
...
/gas_mixture/proc/subtract(gas_mixture/right_side)
	...
	if (right_side.trace_gases && right_side.trace_gases.len > 0)
		...
		for (var/gas/trace_gas in right_side.trace_gases)
			var/gas/corresponding = locate(trace_gas.type) in trace_gases
			if (!corresponding)
				corresponding = new trace_gas.type()
				trace_gases += corresponding
...
/gas_mixture/proc/check_gas_mixture(gas_mixture/sharer)
	if (!sharer)
		return FALSE
	...
	if (sharer.trace_gases && sharer.trace_gases.len)
		if (!trace_gases || !trace_gases.len)
			return FALSE
		for (var/gas/trace_gas in sharer.trace_gases)
...
/gas_mixture/proc/check_turf(turf/model)
...
/gas_mixture/proc/share(gas_mixture/sharer)
	if (!sharer)
		return
...
/gas_mixture/proc/mimic(turf/model, border_multiplier = 1)
...
/gas_mixture/proc/check_both_then_temperature_share(gas_mixture/sharer, conduction_coefficient)
	var/delta_temperature = (temperature_archived - sharer.temperature_archived)
...
/gas_mixture/proc/check_me_then_temperature_share(gas_mixture/sharer, conduction_coefficient)
	var/delta_temperature = (temperature_archived - sharer.temperature_archived)
...
/gas_mixture/proc/check_me_then_temperature_turf_share(turf/simulated/sharer, conduction_coefficient)
...
/gas_mixture/proc/check_me_then_temperature_mimic(turf/model, conduction_coefficient)
	var/delta_temperature = (temperature_archived - model.temperature)
	var/self_temperature_delta = 0
...
/gas_mixture/proc/temperature_share(gas_mixture/sharer, conduction_coefficient)
	var/delta_temperature = (temperature_archived - sharer.temperature_archived)
	if (abs(delta_temperature) > MINIMUM_TEMPERATURE_DELTA_TO_CONSIDER)
		var/self_heat_capacity = heat_capacity_archived()
		var/sharer_heat_capacity = sharer.heat_capacity_archived()
...
/gas_mixture/proc/temperature_mimic(turf/model, conduction_coefficient, border_multiplier)
...
/gas_mixture/proc/temperature_turf_share(turf/simulated/sharer, conduction_coefficient)
	var/delta_temperature = (temperature_archived - sharer.temperature)
	if (abs(delta_temperature) > MINIMUM_TEMPERATURE_DELTA_TO_CONSIDER)
...
/gas_mixture/proc/compare(gas_mixture/sample)
	if (!sample)
		return FALSE
...
// /gas_mixture/proc/check_me_then_share(gas_mixture/sharer)
...
// /gas_mixture/proc/check_me_then_mimic(turf/model)
...
// /gas_mixture/proc/check_both_then_share(gas_mixture/sharer)
