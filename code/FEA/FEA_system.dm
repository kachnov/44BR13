...
Important variables:
	...
	turf/simulated.archive() and air_group.archive()
	...
	atom/CanPass(atom/movable/mover, turf/target, height, air_group)
		...
		Cases 2 and 3 would be different for doors or other objects open and close fairly often.
			(Case 3 would return FALSE always while Case 2 would return FALSE only when the door is open)
...
var/global/controller/air_system/air_master = null
var/global/total_gas_mixtures = 0

/controller
	air_system
		//Geoemetry lists
		var/list/air_group/air_groups = list()
		...
		var/current_cycle = 0
		var/controller/process/air_system/parent_controller = null


		proc
			setup(controller/process/air_system/controller)
...
//				assemble_group_object(obj/movable/floor/base)
			...
			set_controller(controller/process/air_system/controller)
...
/*
		assemble_group_object(obj/movable/floor/base)
			...
			if (members.len > 1)
				var/air_group/object/group = new
...
*/
		process()
			...
			if (current_cycle%10==5) //Check for groups of tiles to resume group processing every 10 cycles
				...
				for (var/air_group/AG in air_groups)
					AG.check_regroup()
					LAGCHECK(90)
...
*/
		...
		process_rebuild_select_groups()
			...
			for (var/air_group/turf_AG in groups_to_rebuild) //Deconstruct groups, gathering their old members
...
/*
			for (var/air_group/object/object_AG in groups_to_rebuild) //Deconstruct groups, gathering their old members
