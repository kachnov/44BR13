/air_group
	...
	var/air_group/list/border_group
...
/air_group/proc/update_group_from_tiles()
	...
	if (sample_member:air)
		var/gas_mixture/sample_air = sample_member:air
...
/air_group/proc/process_group(var/controller/process/parent_controller)
	...
	if (group_processing) //See if processing this group as a group
		...
		for (var/turf/simulated/border_tile in borders)
		...
		var/border_index = 1
		if (border_group)
			for (var/air_group/AG in border_group)
...
/air_group/proc/space_fastpath(var/controller/process/parent_controller)
	var/minDist
	var/dist
	var/turf/space/sample = locate()
	var/totalPressure = 0
