...
/turf
	assume_air(gas_mixture/giver) //use this for machines to adjust air
...
/turf
	simulated
		...
		proc
			...
			update_visuals(gas_mixture/model)
				if (disposed)
					return
								...
								if (air) //Both tiles are open
									...
									if (modeled_neighbor.parent && modeled_neighbor.parent.group_processing)
										if (parent && parent.group_processing)
											...
											//modified using construct developed in air_group/share_air_with_group(...)
