...
/air_group/var/marker = null
/air_group/var/debugging = 0
/pipe_network/var/marker = null
...
/turf/simulated/floor
	verb
		parent_info()
			set src in world
			if (parent)
				boutput(usr, "<strong>[x],[y] parent:</strong> Processing: [parent.group_processing]")
				if (parent.members)
	...
	verb
		adjust_mixture(temp as num, tox as num, oxy as num)
			set src in world
			var/gas_mixture/stuff = return_air()
	...
	verb
		...
		mark_pipe_networks()
			...
			for (var/obj/machinery/atmospherics/pipe/P in atmos_machines)
				...
				var/pipe_network/master = P.return_network()
				if (master)
...
/turf/simulated
	...
	verb
		...
		add_sleeping_agent(amount as num)
			...
			if (amount>1)
				var/gas_mixture/adding = unpool(/gas_mixture)
...
/mob
	...
	verb
		...
		fire_report()
			set category = "Debug"
			boutput(usr, "<span style=\"color:red\"><strong>Fire Report</strong></span>")
