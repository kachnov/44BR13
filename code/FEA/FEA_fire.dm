/atom
	proc
		temperature_expose(gas_mixture/air, exposed_temperature, exposed_volume)
			...
			if (material) material.triggerTemp(src, exposed_temperature)
			return null

/turf
	proc/hotspot_expose(exposed_temperature, exposed_volume, soh)
		if (material)
			material.triggerTemp(src, exposed_temperature)
	...
	simulated
		hotspot_expose(exposed_temperature, exposed_volume, soh)
			...
			var/gas_mixture/air_contents = return_air()
			if (material)
				material.triggerTemp(src, exposed_temperature)
			...
			if (igniting)
				...
				if (locate(/obj/fire_foam) in src) return FALSE
...
/obj
	hotspot
		...
		proc/set_real_color()
			...
			// debug_log += "[x],[y]:[temperature] max diff was [max_color_diff], [red] [green] [blue] vs [sd_ColorRed] [sd_ColorGreen] [sd_ColorBlue]"
		...
		proc/perform_exposure()
			...
			if (bypassing)
				...
				var/gas_mixture/affected = location.air.remove_ratio(volume/max((location.air.volume/5),1))
